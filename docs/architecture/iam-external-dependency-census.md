...
> выше, и подменять одно другим значило бы назвать разными мерками один предмет.
...
> но и оставить под корнем платформы значило держать стрелку `kaname → kacho` у 22
> контрактов службы ради одной только разметки. Словарь не принадлежит ни платформе,
> ни службе — им размечает свои методы каждый домен, — поэтому его дом нейтрален, и
> обе стороны зависят от него ВНИЗ.
>
> Ниже — запись переписи на её ревизии, сохранённая дословно. Координаты в ней верны
> для `0c3b3313a798b5844bad9937220aa01dd22909c4` и с тех пор изменились; предикаты
> раздела на сегодняшнем дереве дадут ноль, и это ожидаемо, а не находка.

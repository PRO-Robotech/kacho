...
> координаты нельзя.
>
> | что переехало | где лежит сегодня | чем переехало |
> |---|---|---|
> | словарь аннотаций доступа (назван в перечне ниже прежним адресом) | `corelib/authz/v1/authz_options.proto` | решение владельца Р12, kacho#2089 — см. врезку §5 |
> | чтение потолка личности (там же) | `kaname/cloud/iam/v1/identity_quota_service.proto` | kacho#2362 — служба ушла в контракт домена, см. врезку §1.2 |
>
> ```sh

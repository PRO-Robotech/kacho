...
> [!important] ДВЕ КООРДИНАТЫ ЭТОГО РАЗДЕЛА ПЕРЕЕХАЛИ ПОСЛЕ ЕГО РЕВИЗИИ
> Предмет у обеих прежний — файл по-прежнему нужен генерации iam, — изменился
> **адрес**. Перечень ниже сохранён записью своей ревизии; читать его как живые
> координаты нельзя.
>
> | названо ниже | где лежит сегодня | чем переехало |
> |---|---|---|
> | `kacho/iam/authz/v1/authz_options.proto` | `corelib/authz/v1/authz_options.proto` | решение владельца Р12, kacho#2089 — см. врезку §5 |
> | `kacho/cloud/quota/v1/identity_quota_service.proto` | `kaname/cloud/iam/v1/identity_quota_service.proto` | kacho#2362 — чтение потолка личности ушло в контракт службы, см. врезку §1.2 |
>
> ```sh
> git ls-files proto/corelib/authz/v1/authz_options.proto             # непусто
> git ls-files proto/kaname/cloud/iam/v1/identity_quota_service.proto # непусто
> git ls-files 'proto/kacho/iam*' | wc -l                             # 0
> git ls-files proto/kacho/cloud/quota/v1                             # только quota.proto
> ```
>
> **Группировка и числа таблицы НЕ пересчитывались, и это решение, а не недосмотр.**
> Второй переезд меняет не состав замыкания, а его разнесение по каталогам:
> `identity_quota_service.proto` ушёл **внутрь** домена iam, поэтому строки
> `kaname/cloud/iam/v1`, `kacho/cloud/quota/v1` и счёт «вне домена iam — 8 файлов»
> связаны между собой и меняются только вместе. Пересчёт замыкания — работа со своим
> предикатом; здесь она не делалась, и числа остаются числами ревизии `0c3b3313`.
>
> Косвенный контроль того, что переезды учтены верно: §5 насчитал **22** контракта
> iam, импортирующих словарь разметки, а сегодня их **23**
> (`grep -rl 'corelib/authz/v1/authz_options.proto' --include='*.proto' proto/kaname/ | wc -l`) —
> ровно на тот файл, который во втором переезде вошёл в домен.
...
>
> **Предикат раздела при этом СМЕШИВАЕТ ДВЕ ЭПОХИ, и на своей ревизии он тоже не
> воспроизводит 46.** Координата файла в нём — допереездная (`kacho/iam/authz/…`,
> до kacho#2089), а фильтр — послепереездный (`^proto/kaname/cloud/iam/`, после
> переименования домена в kacho#2118). Прогон на `0c3b3313` даёт **68 и 68**:
> фильтр не матчит там ничего, потому что контрактов `proto/kaname/` на той ревизии
> ещё нет. Напечатанное **46** получено фильтром `^proto/kacho/cloud/iam/` — тем,
> что был текущим в тот день.
>
> Это не описка, а класс: обновили **половину** предиката вслед за одним переездом
> и не тронули вторую вслед за другим. Половинчатое обновление хуже нетронутого —
> нетронутое хотя бы воспроизводимо на своей ревизии. Живой предикат с сегодняшними
> координатами и его числа — Приложение А, п. 7.
...
#    Сам фильтр в §5 тоже принадлежит другой эпохе: 46 там получено с
#    '^proto/kacho/cloud/iam/' — до переименования домена в kaname (kacho#2118).

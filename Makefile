...
##
## Цена ИЗМЕРЕНА, а не оценена, и приведена ДИАПАЗОНОМ, потому что наблюдений
## больше одного (2026-08-06, `-race -p 1`, шесть пакетов, 562 пройденных теста,
## 0 пропущенных во всех трёх):
##
##   - машина разработчика (Docker 29.3.1): 59 с wall, из них самый дорогой пакет
##     `api/access_binding` — 15.4 и 17.6 с в двух прогонах;
##   - ubuntu-latest в конвейере (прогон 31093732308): 143 с от старта цели до её
##     вердикта, `api/access_binding` — 21.3 с.
##
## Одно число здесь было бы ложной точностью: разброс между прогонами на одной
## машине уже 14 %, а между машинами — вдвое. Что устойчиво: цель укладывается в
## единицы минут, и это на два порядка меньше прежнего замера того же пакета
## (7.6 мин, 2026-08-01), устаревшего через четыре часа после того, как был
## сделан, — контейнер стал подниматься один на пакет вместо одного на тест.
test-authz-fga:
	@set -o pipefail; \
	log=$$(mktemp); \
	trap 'rm -f "$$log"' EXIT; \

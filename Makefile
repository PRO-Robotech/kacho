...
# одним процессом. Два самых дорогих гейта вынесены в `internal/repohygiene/artifactgates`
# по ПРЕДМЕТУ обхода (не-Go корпус), и теперь время определяется более долгим пакетом, а не
# их суммой. Замер на одной машине, одной командой, до и после:
#

...
# которого пойдёт в кеш (страж — internal/treecorpus, cachedverdict.go; провязка —
...
# `internal/treecorpus` (обычный пакет, импортирует кто угодно), у каждого пакета гейтов
...
#
# Новый дом помощника (`internal/treecorpus`) в этот предикат не входит — он не под

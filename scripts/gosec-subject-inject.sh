...
#
#   B    ведомость: запись, которой нечего исключать       →  находка
#   B'   ведомость: запись без причины                     →  код 2, не 0 и не 1
#   A/A' ведомость: та же синтетика, число на единицу       ↔  ТА ЖЕ запись с
#        больше фактического (находка)                         ТОЧНЫМ числом —
#                                                              молчание
...
# красное у C не может прийти «от соседа». Пара A/A' устроена так же по другой
# оси: один и тот же отчёт скана, одна и та же запись ведомости, разное ЧИСЛО.
#
# НИ ОДНА ПАРА НЕ ОПИРАЕТСЯ НА СОДЕРЖИМОЕ ВЕДОМОСТИ ДЕРЕВА, и это условие
# годности, а не аккуратность: ведомость пуста по замыслу — пустая и есть её цель.
# Проба, которой нужна живая запись, красна ровно тогда, когда механизм сработал.
#
...
# ── A и A': ВЕДОМОСТЬ. ТА ЖЕ инертная директива, разное ЧИСЛО в записи ───────
# Синтетика C всё ещё в дереве и всё ещё инертна, поэтому обе половины пары
# сверяются с ОДНИМ И ТЕМ ЖЕ отчётом скана: между A и A' различается ровно один
# факт — число в записи ведомости, — и красное у A не может прийти «от соседа».
#
# ПОЧЕМУ ПРЕДМЕТ ЗАВОДИТСЯ ПРОБОЙ, А НЕ БЕРЁТСЯ ИЗ ВЕДОМОСТИ ДЕРЕВА. Прежняя
# редакция правила число в живой строке ведомости — и умерла в тот день, когда
# ведомость опустела: цель механизма достигнута, а его самопроверка на этом
# ПОКРАСНЕЛА (`awk` не нашёл строки, ведомость осталась прежней, гейт справедливо
# промолчал — код 0 вместо 1). Доказательство, привязанное к живому послаблению,
# истекает вместе с ним и уносит с собой единственное свидетельство того, что
# гейт вообще способен упасть по этой оси.
LEDGERROW="tools/gosecsubject/gosecsubject.go\tG115"

say "A: ведомость объявляет на одну инертную директиву больше, чем их есть"
{ cat "$WORK/ledger.orig"
  printf "$LEDGERROW\t2\tсинтетика инъекции\n"; } > "$LEDGER"
rc=$(gate)
[ "$rc" -eq 1 ] && ok "код 1" || bad "код $rc, ожидалась находка"
grep -q 'число разошлось' "$WORK/gate.txt" && ok "отказ называет предмет: число ТОЧНОЕ, а не потолок" \
    || bad "отказ не называет предмет: $(head -c 200 "$WORK/gate.txt")"

say "A': ТА ЖЕ запись с ТОЧНЫМ числом — законный близнец обязан молчать"
{ cat "$WORK/ledger.orig"
  printf "$LEDGERROW\t1\tсинтетика инъекции\n"; } > "$LEDGER"
rc=$(gate)
[ "$rc" -eq 0 ] && ok "код 0 — запись с живым предметом и точным числом молчит" \
    || bad "код $rc на записи, у которой предмет есть"
cp "$WORK/ledger.orig" "$LEDGER"
...
# Предмет — РЕАЛЬНЫЙ модуль дерева (взятый у `git ls-files`, а не выписанный
# именем): выписанное имя `services/iam` было верно только пока этот модуль
# существовал в дереве. Служба доступа выехала в свой репозиторий (kaname), и
# `grep -v '^services/iam'` перестал находить что вычёркивать — манифест
# оставался прежним, гейт справедливо молчал (код 0), а находка выглядела так,
# будто сама проверка ослепла. Слепой была ПРОБА, а не гейт: `git grep -c
# '^services/iam' $ROOT` на этом дереве даёт 0, и вычёркивать было нечего.
REALMOD=$(git -C "$ROOT" ls-files '*go.mod' | while read -r f; do dirname "$f"; done | sort -u | head -1)
say "F: модуль службы пропал из перечня скана (взят $REALMOD)"
awk -F'\t' -v m="$REALMOD" 'BEGIN{OFS="\t"} $1!=m' \
    "$WORK/gosec-suppressions-manifest.txt" > "$WORK/m.tmp"
# Модуль в этом дереве может быть единственным: тогда вычёркивание опустошает
# манифест целиком, и отказ пришёл бы от ПЕРВОГО чтения («манифест пуст — ни
# один модуль не сканировался»), а не от ВТОРОГО, независимого («модули Go,
# которых нет в перечне скана»), — а F именно про второе. Декой держит
# манифест непустым и без второго реального go.mod; собственных отчётов у него
# нет, и это неважно: второе чтение падает раньше, чем гейт до них дойдёт.
if [ ! -s "$WORK/m.tmp" ]; then
    printf 'zzz-fixture-decoy-module-without-go-mod\t/nonexistent-report.json\t/nonexistent-scan.log\n' \
        > "$WORK/m.tmp"
fi
...
grep -qF "скана: $REALMOD" "$WORK/gate.txt" && ok "отказ называет модуль поимённо ($REALMOD)" \
    || bad "отказ не называет модуль $REALMOD: $(head -c 200 "$WORK/gate.txt")"

...
# Четвёртый предел — поглощение нормализацией — обязан быть НАЗВАН ЧИСЛОМ, а не
# оговорён прозой: ключ `--rename` способен спрятать честно снятое имя, и без
# этой строки переписи такое поглощение неотличимо от законного.
assert "перепись называет поглощения нормализацией числом" "yes" \
       "$(grep -q 'перепись: поглощений нормализацией .* подозрительных' <<<"$out_a" && echo yes || echo no)"
# Легенда обязана утверждать только то, что метка различает. Оценка
# вероятности намерения различителем не является — на чужих массовых
# изменениях та же метка стоит на заведомо намеренных снятиях.
assert "легенда не приписывает метке различение намерения" "no" \
       "$(grep -q 'вероятнее всего непреднамеренно' <<<"$out_a" && echo yes || echo no)"

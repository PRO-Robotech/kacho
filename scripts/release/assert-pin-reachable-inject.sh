...
#
# ─────────────────────────────────────────────────────────────────────────────
# ЧЕГО ЭТА ИНЪЕКЦИЯ НЕ РАЗБИРАЕТ — НАЗВАНО, А НЕ УМОЛЧАНО
#
# Класс «вердикт берётся из трубы» (SIGPIPE под `pipefail`) здесь НЕ ловится и
# ловиться не может: фикстуры несут три тега, то есть вход на три порядка ниже
# буфера трубы, и `grep -q` успевает дочитать его прежде, чем выйти. Испытуемый
# ДО починки проходит все оси зелёным. Класс держит гейт дерева
# `internal/repohygiene` `TestPipefailVerdictNeverComesFromAPipe` — он судит по
# синтаксису, а не по размеру входа, и потому от объёма фикстуры не зависит.
# Отсюда правило: правя сравнение в испытуемом, зови ОБА — эту инъекцию (она
# держит семантику, ось 9) и тот гейт (он держит форму).
#
# ─────────────────────────────────────────────────────────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════════
# Ось 9. ИМЯ ЯКОРЯ СВЕРЯЕТСЯ ЦЕЛИКОМ — строгий префикс НЕ закрепляет
#
# Держатель семантики `isDurableTag`: якоря `$'\n'` и есть то, что делает
# сравнение равносильным `grep -qxF`. Без них пин `v0.1` «закреплялся» бы тегом
# `v0.1.0`, которого не существует, — снова fail-open. Законный близнец — ось 6.
# ══════════════════════════════════════════════════════════════════════════════
F9="$tmp/ax9"
read -r _BC _SC _DC _TC <<< "$(mkfixture "$F9")"
( cd "$F9" && setpin "v0.1" && G tag v0.1.0 >/dev/null 2>&1 )
RC="$(rc_subject "$F9")"; RCN="$(rc_neighbour "$F9")"
if [ "$RC" = "1" ]; then pass "9  пин — строгий ПРЕФИКС существующего тега -> находка"
else fail "9  строгий префикс" "ожидался 1, получен $RC — тега v0.1 нет, закреплять нечем:\n$(run_subject "$F9")"; fi
if [ "$RCN" = "0" ]; then pass "9' инъекция уронила ТОЛЬКО проверяемое: гейт согласия молчит"
else fail "9' чистота инъекции" "гейт согласия тоже покраснел ($RCN) — красное пришло от соседа"; fi

# ══════════════════════════════════════════════════════════════════════════════
# Ось 10. ТЕГ СВЕРЯЕТСЯ ПО ОБЪЕКТУ — одного имени недостаточно
#
# Локальный тег переставлен, на удалённом остался прежним, и ревизия пина с
# удалённой версии НЕ достижима: origin её не держит ничем. Сверка по
# ИМЕНАМ объявила бы пин закреплённым — единственная fail-open полоса,
# какая здесь была. Законный близнец — ось 7 (объекты совпадают -> молчит).
# ══════════════════════════════════════════════════════════════════════════════
F10="$tmp/ax10"
read -r _BD SD DD _TD <<< "$(mkfixture "$F10")"
BARE3="$tmp/ax10-bare.git"
git init -q --bare "$BARE3" >/dev/null 2>&1
( cd "$F10" && setpin "$(pseudo "$DD")" \
  && git tag -d "archive/kaname-pin-${DD:0:12}" >/dev/null 2>&1 \
  && git remote add origin "$BARE3" >/dev/null 2>&1 \
  && G push -q origin main >/dev/null 2>&1 \
  && G tag release-anchor "$SD" >/dev/null 2>&1 \
  && G push -q origin refs/tags/release-anchor >/dev/null 2>&1 \
  && G tag -f release-anchor "$DD" >/dev/null 2>&1 \
  && git fetch -q origin >/dev/null 2>&1 )
RC="$(KACHO_PIN_REACH_NETWORK=1 rc_subject "$F10")"
RCN="$(rc_neighbour "$F10")"
if [ "$RC" = "1" ]; then pass "10  тег переставлен локально, на удалённом другой объект -> находка"
else fail "10  сверка по объекту" "ожидался 1, получен $RC — одноимённый тег origin ревизию не держит:\n$(cd "$F10" && KACHO_PIN_REACH_NETWORK=1 "$SUBJECT" 2>&1)"; fi
if [ "$RCN" = "0" ]; then pass "10' инъекция уронила ТОЛЬКО проверяемое: гейт согласия молчит"
else fail "10' чистота инъекции" "гейт согласия тоже покраснел ($RCN)"; fi

# Законный близнец оси 10: АННОТИРОВАННЫЙ тег, совпадающий с удалённым.
# Аннотированный тег даёт ДВЕ строки `ls-remote`: объект тега и его
# разыменование `^{}`. Сверка локального `^{}` против НЕпилёной
# удалённой объявила бы его расхождением — ЛОЖНАЯ находка на ровном месте.
# На живом origin такой тег есть (1 из 17 общих), поэтому близнец не выдуман.
F10c="$tmp/ax10c"
read -r _BF _SF DF _TF <<< "$(mkfixture "$F10c")"
BARE4="$tmp/ax10c-bare.git"
git init -q --bare "$BARE4" >/dev/null 2>&1
( cd "$F10c" && setpin "$(pseudo "$DF")" \
  && git tag -d "archive/kaname-pin-${DF:0:12}" >/dev/null 2>&1 \
  && G tag -a -m "аннотация" release-anchor "$DF" >/dev/null 2>&1 \
  && git remote add origin "$BARE4" >/dev/null 2>&1 \
  && G push -q origin main >/dev/null 2>&1 \
  && G push -q origin refs/tags/release-anchor >/dev/null 2>&1 \
  && git fetch -q origin >/dev/null 2>&1 )
RC="$(KACHO_PIN_REACH_NETWORK=1 rc_subject "$F10c")"
if [ "$RC" = "0" ]; then pass "10'' законный близнец: АННОТИРОВАННЫЙ тег совпадает -> молчит"
else fail "10'' аннотированный близнец" "ожидался 0, получен $RC — разыменование потеряно, аннотация стала ложной находкой:\n$(cd "$F10c" && KACHO_PIN_REACH_NETWORK=1 "$SUBJECT" 2>&1)"; fi

# ══════════════════════════════════════════════════════════════════════════════
# Ось 11. УДАЛЁННЫЙ НЕ ОТВЕТИЛ — теги считаются НЕдолговечными
# ══════════════════════════════════════════════════════════════════════════════
F11="$tmp/ax11"
read -r _BE _SE DE _TE <<< "$(mkfixture "$F11")"
( cd "$F11" && setpin "$(pseudo "$DE")" \
  && git remote add origin "$tmp/нет-такого-удалённого.git" >/dev/null 2>&1 )
OUT11="$(cd "$F11" && KACHO_PIN_REACH_NETWORK=1 "$SUBJECT" 2>&1)"
RC="$(KACHO_PIN_REACH_NETWORK=1 rc_subject "$F11")"
if [ "$RC" = "1" ]; then pass "11  удалённый недостижим -> fail-closed находка, а не зелёное"
else fail "11  недостижимый удалённый" "ожидался 1, получен $RC:\n$OUT11"; fi
if [[ "$OUT11" == *"спросить не удалось"* ]]; then
    pass "11' состояние НАЗВАНО: «спросить не удалось» отличимо от «тегов нет»"
else fail "11' состояние сети" "в выводе нет причины:\n$OUT11"; fi

printf '\nперепись: утверждений исполнено %d, провалено %d\n' "$checked" "$failed"
printf 'вне разбора: класс «вердикт из трубы» — фикстуры на 3 тега ниже буфера;\n'

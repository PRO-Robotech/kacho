...
# находки, ни зелёного, и отличить это от долгой работы нечем. Отсутствие
# `timeout` не роняет прогон, а лишь снимает предел (полоса и так fail-closed).
#
# ПРЕДЕЛ ОГРАНИЧИВАЕТ ЗАВИСАНИЕ, А НЕ СУДИТ ЗАДЕРЖКУ, и запас здесь несущий:
# полоса fail-closed, поэтому СРАБОТАВШИЙ предел даёт НАХОДКУ. Порог, задетый
# работающим удалённым, — это ложная находка, а гейт с ложными находками
# отключают первым. Замер, из-за которого запас поднят: при неисправном агенте
# ключей `git ls-remote` отвечает за 21.9 с (агент отвергает ключ, git идёт
# запасным путём) — прежнее умолчание в 20 с обращало исправное дерево из пяти
# закреплённых пинов в находку «спросить не удалось».
NET_TIMEOUT="${KACHO_PIN_REACH_NET_TIMEOUT:-120}"
netrun() {

...
subj_run() {
    ...
    # Пустой вход подаётся НУЛЁМ байт, а не пустой строкой: так его подаёт git на
    # отправке без изменений, и фикстура не снисходительнее продукта.
    local out rc
    out="$(cd "$subj" && { [ -z "$input" ] || printf '%s\n' "$input"; } \
...
#
# ДЕФЕКТОВ ТРИ, И ВСЕ ОБЯЗАНЫ КРАСНЕТЬ. Узкий: удаление гонит прогон (форма до
...
# та же строка удаления рядом с ненулевой вершиной. Пустой: «все строки —
# удаления» спрошено без «строки есть», а на пустом множестве «для всех»
# истинно — и пустой вход уходит нулём мимо прогона. Его ловит близнец с другой
# стороны — строк входа ноль.
assert_delete_only() { # $1 — метка, $2 — файл для числа провалов
    ...
    # ЗАКОННЫЙ БЛИЗНЕЦ С ДРУГОЙ СТОРОНЫ: строк входа НОЛЬ. От «одна строка
    # удаления» он отличается ровно одним фактом, и ответ у него другой. Пустой
    # вход — ручной вызов либо отправка без изменений (git зовёт хук и тогда, со
    # входом в ноль байт); различить их хуку нечем, ревизии он не видел — и
    # судится рабочая копия КАК ЕСТЬ. Копия стоит на ревизии, которую заглушка
    # объявляет красной: прогон виден и записью вызова, и кодом.
    : > "$calls"
    r="$(subj_run feature/bad - "" KACHO_X=1 KACHO_STUB_CALLS="$calls")"
    rc="$(rc_of "$r")"; n="$(calls_of)"
    if [ "$n" = "1" ]; then xok "пустой вход: прогонщик вызван — ветвь одних удалений его не забрала"
    else xfail "пустой вход: ждали 1 вызов прогонщика, получили $n — «все строки — удаления» принято на пустом множестве"; fi
    if [ "$rc" = "1" ]; then xok "…и красная рабочая копия отправку останавливает"
    else xfail "…но красная рабочая копия отправку не остановила (rc=$rc)"; fi
    case "$r" in
        *"отправка несёт только удаление ссылок"*)
            xfail "…и пустой вход назван отправкой одних удалений — строк удаления в нём нет" ;;
        *) xok "…и отправкой одних удалений пустой вход не назван" ;;
    esac
    case "$r" in
        *"MARKER:broken"*) xok "…и судилась рабочая копия — иного дерева у пустого входа нет" ;;
        *) xfail "…но судилась не рабочая копия" ;;
    esac
    case "$r" in
        *"она и есть отправляемая ревизия"*)
            xfail "…и копия объявлена отправляемой ревизией, хотя вход не назвал ни одной" ;;
        *) xok "…и копия НЕ объявлена отправляемой ревизией" ;;
    esac
    case "$r" in
        *"вход git не назвал ни одной вершины"*) xok "…и печать говорит, что вершин во входе нет" ;;
        *) xfail "…но печать не говорит, что вершин во входе нет: о каком дереве вердикт, читателю не сказано" ;;
    esac
...
# Пустой дефект — снят первый конъюнкт: «все строки — удаления» спрошено без
# «строки есть». Тоже та правка, которую сделают «проще»: второе условие само
# по себе читается полным.
make_broken_delete_empty() {
    sed 's/^    \[ "\$push_refs_total" -gt 0 \] && \(\[ "\$push_refs_deleted" -eq "\$push_refs_total" \]\)$/    \1/' \
        "$HOOK" > "$tmp/hook-delete-empty"
    grep -q '^    \[ "\$push_refs_deleted" -eq "\$push_refs_total" \]$' "$tmp/hook-delete-empty" &&
        ! grep -q '\[ "\$push_refs_total" -gt 0 \] && \[ "\$push_refs_deleted"' "$tmp/hook-delete-empty"
}
...
del_empty_broken="н/д"
if make_broken_delete_empty; then
    echo
    echo "── прогон против дефекта «пустой вход принят за одни удаления» (ждём хотя бы один провал)"
    subject_fixture "$tmp/hook-delete-empty"
    assert_delete_only дефект-пустого "$tmp/del-empty-broken.n"; del_empty_broken="$(cat "$tmp/del-empty-broken.n")"
fi
...
printf '  провалов у дефекта «пустой вход — одни удаления»: %s (норма ≥1 — близнец «строк ноль»)\n' "$del_empty_broken"
...
if [ "$del_empty_broken" = "н/д" ]; then
    echo "ОТКАЗ: дефект «пустой вход — одни удаления» не воссоздан — форма условия одних удалений изменилась" >&2
    rc=1
elif [ "${del_empty_broken:-0}" -lt 1 ]; then
    echo "ОТКАЗ: проба ЗЕЛЁНАЯ на ветви одних удалений без «строки есть» — пустой вход уходит мимо прогона" >&2
    rc=1
fi
exit "$rc"

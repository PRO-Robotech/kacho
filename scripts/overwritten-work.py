...
#
# РЕЖИМ СВЕРКИ (`--against РЕВИЗИЯ`) — ДРУГОЙ ВОПРОС, А НЕ ДРУГОЙ ВЫВОД.
# Без него прибор отвечает о МОМЕНТЕ изменения: что оно сняло. Этого мало для
# закрытия задачи об откате, потому что работа возвращается ПОСЛЕ — соседними
# полосами, вразнобой, и ведомость об этом не знает. Ключ переводит вопрос в
# «что из снятого НЕ ВЕРНУЛОСЬ к названной ревизии», и вердикт выносится об
# ОСТАТКЕ. Цена отсутствия этого вопроса измерена: три имени предшественника
# `053d3157d9` простояли откаченными, пока все три пункта предиката снятия
# #2172 закрывались соседними полосами по отдельности, — спросить «а что
# осталось» было нечем, и «восстановлено» держалось везением переписи.
#
#   * молчание режима сверки говорит «ВЕРНУЛОСЬ», а не «ничего не снято» —
#     это разные утверждения, и слить их значило бы объявить дерево целым
#     там, где откат просто уже разобран кем-то другим;
#   * пустой обход ревизии сверки — НАХОДКА: «об остатке не прочитано ничего»
#     за «всё вернулось» не выдаётся;
#   * ПРЕДЕЛЫ 1-4 ВЫШЕ ДЕЙСТВУЮТ ЦЕЛИКОМ. Полнота остатка — полнота ПО СЧЁТУ
#     ИМЁН: проза, выпотрошенное тело при живом имени и намеренное снятие
#     после изменения остаются за наблюдением ровно так же. Предел 3 в этом
#     режиме даже шире: имя вправе быть снято осознанно и ПОСЛЕ изменения, и
#     от невозвращённого это формой неотличимо. Замер на известном случае:
#     из 4 не вернувшихся имён 1 (`ourPackagePrefix`) — то самое намеренное
#     снятие, названное пределом 3, а 3 — настоящий остаток отката.
#
# ИСХОДОВ ЧЕТЫРЕ, И ОНИ РАЗНЫЕ:
#   0 — сверх объявленного преобразования изменение не сняло ни одного имени
#       (в режиме сверки — всё снятое вернулось к названной ревизии);
#   1 — НАХОДКА: сняты имена (в режиме сверки — не вернулись), перечень с
#       привязкой к предшественникам;
...
def main():
    ...
    ap.add_argument("--against", default=None, metavar="РЕВИЗИЯ",
                    help="спросить не «что изменение сняло», а «что из снятого "
                         "НЕ ВЕРНУЛОСЬ» к этой ревизии: вердикт выносится об "
                         "ОСТАТКЕ, а не о самом изменении")
    ...
    try:
        ...
        against = None
        if args.against:
            against = git("rev-parse", "--verify",
                          f"{args.against}^{{commit}}").strip()
        if args.since:
    ...
    if against:
        print(f"сверка с:  {git('log', '-1', '--format=%h %ci %s', against).strip()}")

    paths_against, n_against, files_against = set(), set(), 0
    try:
        ...
        if against:
            paths_against = paths_at(against)
            n_against, files_against = names_at(against, paths_against)
    except Exception as e:                                   # noqa: BLE001
    ...
    if not n_parent or not files_parent:
        ...
        return 1
    if against and (not n_against or not files_against):
        print(f"перепись: файлов у ревизии сверки {files_against}, "
              f"имён {len(n_against)}")
        print("НАХОДКА: обход ревизии сверки пуст — об остатке не прочитано "
              "ничего, вердикт беспредметен", file=sys.stderr)
        return 1
    ...
    # ── Режим сверки: вопрос не «что снято», а «что НЕ ВЕРНУЛОСЬ» ──────────
    # Ведомость отката отвечает о МОМЕНТЕ изменения и ничего не говорит о том,
    # дожил ли остаток до сегодня. Пока спросить об этом было нечем, работа
    # возвращалась полосами, каждая находила свой экземпляр случайно, а
    # вердикт «восстановлено» держался переписью, пополнявшейся везением.
    subject_names, subject_paths = lost_names, lost_paths
    if against:
        subject_names = {n for n in lost_names
                         if n not in n_against
                         and normalize(n, renames) not in n_against}
        subject_paths = {p for p in lost_paths
                         if p not in paths_against
                         and normalize(p, renames) not in paths_against}
        print(f"перепись: файлов у ревизии сверки {files_against}, "
              f"имён {len(n_against)}")
        print(f"перепись: из снятого НЕ ВЕРНУЛОСЬ имён {len(subject_names)}, "
              f"файлов {len(subject_paths)}")

    if not subject_names and not subject_paths:
        if against:
            print("✓ всё снятое сверх объявленного преобразования вернулось "
                  "к ревизии сверки")
        else:
    ...
    orphan = {"names": [], "paths": []}
    for kind, tokens in (("names", sorted(subject_names)),
                         ("paths", sorted(subject_paths))):
        for tok in tokens:
    ...
    if against:
        print(f"\nНАХОДКА: из снятого изменением к ревизии сверки не вернулось "
              f"{len(subject_names)} имён и {len(subject_paths)} файлов.")
    else:
    ...
    for h, d in rows:
        ...
        print(f"  {mark} {meta}")
        verb = "не вернулось" if against else "снято"
        print(f"      {verb} имён {len(d['names']):3d}, "
              f"файлов {len(d['paths']):2d}")

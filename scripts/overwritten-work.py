...
#
# КОНТРОЛЬ НА ПОПУЛЯЦИИ, ГДЕ ОТВЕТ ИЗВЕСТЕН. На `d46aaa7280` предикат находит
...
def main():
    ...
    print(f"перепись: снято имён {len(n_parent - n_rev)}, из них объявленным "
          f"преобразованием объяснено {len(n_parent - n_rev) - len(lost_names)}, "
    ...
    print("\n◆ — предшественник внутри окна риска: его работа посажена уже "
          "после базы,\n    на которой собиралось изменение, и снята "
          "вероятнее всего непреднамеренно.")
    return 1

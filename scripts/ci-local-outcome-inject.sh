...
# гоняется шесть раз: против настоящего прогонщика (ждём ноль провалов) и против
# ПЯТИ воссозданных дефектов — «признак снят», «признак расширен до любого
# отказа» и три дефекта чтения доказательства по контракту (#2821): «контракт не
# читается», «предпосылка разбора снята», «контракт расширен на всякий шаг».
...
# литералы `/tmp/go-build/...` живут в дереве (corelib/treecorpus), и признак,
...
printf 'ci-local-outcome-inject: утверждений на прогон %d, прогонов 6; сценариев %d\n' \

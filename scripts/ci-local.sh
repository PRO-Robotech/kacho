...
go_group() {
    ...
    if ! command -v jq > /dev/null; then
        ...
        if GOBIN="$gosec_bin" go install "github.com/securego/${gosec_pin}" > "$WORK/gosec-install.txt" 2>&1; then
            ...
            # КОД ВОЗВРАТА СКАНА — ДАННЫЕ, И ИХ ТРИ (#2125). Прежде код здесь не
            # читался вовсе: скан всегда выходил нулём, и «анализ не состоялся»
            # приходило сюда неотличимым от чистого прогона — различала их
            # только метка-файл. Теперь третья категория доезжает кодом 3 и
            # называется словом; метка читается рядом, потому что несёт ПРИЧИНУ
            # поимённо, а код — только категорию.
            local scan_rc=0
            GOSEC_BIN="$gosec_bin/gosec" GOSEC_OUT="$WORK" \
                "$ROOT/scripts/gosec-scan-modules.sh" > "$WORK/gosec-run.txt" 2>&1 || scan_rc=$?
            if [ "$scan_rc" = "3" ]; then
                echo "   НЕ ВЫПОЛНИЛОСЬ (код 3): анализ не состоялся или состоялся"
                echo "   не по всему дереву. Это НЕ «находок нет» — вердикт не выносится."
            elif [ "$scan_rc" != "0" ]; then
                echo "   ОТКАЗ: скан вернул неизвестный код $scan_rc — он не входит"
                echo "   в два объявленных исхода (0 — анализ состоялся, 3 — не выполнилось)."
            fi

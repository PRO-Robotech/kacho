...
go_group() {
    ...
    # Инъекция ЧТЕНИЯ ИСХОДОВ судьи формы (#1851) — доли секунды, и предмет у неё
    # свой: шаг ниже проверяет, что судью зовут, а этот — что его ответ читают
    # ЧЕТЫРЬМЯ исходами, а не двумя. Способность различать наблюдается на дереве
    # продукта только в одном случае из четырёх (сегодня — VOID), поэтому «зелёный»
    # там неотличим от «мёртвый»; проба гоняет набор дважды — против настоящего
    # классификатора и против копии, где VOID схлопнут в успех.
    run "манифесты: чтение исходов" bash "$ROOT/scripts/manifest-form-outcome-inject.sh"

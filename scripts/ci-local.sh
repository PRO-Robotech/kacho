...
ui_group() {
    ...
    for m in "$ROOT"/ui-future/*/package.json; do
        ...
        # prettier: их в дереве три (3.8.3 · 3.9.4 · 3.9.6), и они не согласны
        # между собой — один и тот же файл одна принимает, другая отвергает.
        # Замер одной версией на все девять пакетов даёт неверное число; сам
        # раскол заведён предметом #1674.

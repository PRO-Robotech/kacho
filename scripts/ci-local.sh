...
go_group() {
    ...
    run "откат: полнота ведомости по счёту имён" bash "$ROOT/scripts/overwritten-work-inject.sh"
    ...
    run "выпуск: пин закреплён стволом либо тегом" bash "$ROOT/scripts/release/assert-pin-reachable-inject.sh"
    run "выпуск: пол фундамента — исчезнувший пакет" bash "$ROOT/scripts/release/assert-foundation-floor-inject.sh"
    ...
    run "go.mod: сверка способна упасть (инъекция)" bash "$ROOT/scripts/go-mod-tidy-check-inject.sh"

    # ПРЕДПИСАНИЕ ХУКА ОТПРАВКИ ОБЯЗАНО БЫТЬ ИСПОЛНИМЫМ — И ЭТО СУДИТСЯ ЗДЕСЬ,
    # В ГРУППЕ, КОТОРАЯ ИДЁТ ПРИ ЛЮБОЙ ОТПРАВКЕ.
    #
    # Предмет судьи — текст, который получает человек, когда проверки консоли не
    # выполнялись. Отказ верен по существу («не выполнено» не равно «зелено»), но
    # печатал `npm ci --prefix <пакет>` — команду с заполнителем вместо имени, — и
    # сводил одиннадцать пропусков в одну строку без координат. Дословно
    # скопированное давало тот же исход: выход был закрыт, семь веток дневной
    # работы простояли, каждая в единственном экземпляре.
    #
    # В группу консоли судья НЕ ставится намеренно: она добавляется, только когда
    # тронут `ui-future`, — а отказ этот печатается и тем, кто консоль не трогал.
    # Стоимость — около секунды: синтетическое дерево и настоящий хук на нём.
    run "хук отправки: предписание исполнимо (инъекция)" \
        bash "$ROOT/scripts/hooks/prepush-console-remedy-executable-inject.sh"
    go_mod_tidy_check
...
# КОМАНДА СОЗДАНИЯ УСЛОВИЯ — БЕРЁТСЯ У ПРЕДИКАТА, А НЕ СОЧИНЯЕТСЯ ЗДЕСЬ.
#
# `deps-ready.mjs` уже знает, куда ставить: у члена воркспейса зависимости лежат в
# КОРНЕ `ui-future`, у standalone-пакета — у него самого, и команда у этих двух
# случаев РАЗНАЯ. Он же печатает её готовой строкой. Прежде этот вывод глотался
# (`2>&1` в никуда), и пропуск печатался текстом «npm ci --prefix» — без имени
# пакета, то есть предписанием, которое нельзя исполнить. Каталог у цикла был в
# руках всё это время.
#
# Второй копии предиката здесь не заводится намеренно: два ответа на вопрос «куда
# ставить» разошлись бы молча и именно там, где расхождение не видно.
npm_deps_remedy() { # npm_deps_remedy <каталог модуля>
    node "$ROOT/ui-future/scripts/deps-ready.mjs" "$1" 2>&1 \
...
ui_types_group() {
    ...
    for m in "$ROOT"/ui-future/*/package.json; do
        ...
        npm_deps_ready "$dir" || {
            skip "ui-types $name" "зависимости не установлены — условие: $(npm_deps_remedy "$dir")"
            continue
        }
...
ui_group() {
    ...
    for m in "$ROOT"/ui-future/*/package.json; do
        local dir name; dir="$(dirname "$m")"; name="$(basename "$dir")"
        npm_deps_ready "$dir" || {
            skip "ui $name" "зависимости не установлены — условие: $(npm_deps_remedy "$dir")"
            continue
        }

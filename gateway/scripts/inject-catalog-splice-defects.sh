...
# ---------------------------------------------------------------------------
# Ось 1: снятие вклада СЛУЖБЫ — корень `kaname` перестаёт быть ОБЪЯВЛЕННЫМ.
#
# ЗДЕСЬ ИНЪЕКЦИЯ БЫЛА ДРУГОЙ, И ЕЁ ПРЕДМЕТ УЕХАЛ. Прежняя редакция временно
# уносила каталог `proto/kaname` из дерева. Решением владельца (kacho#2616,
# исход C, 2026-09-13) контракты службы уехали в её репозиторий: каталога в
# дереве больше нет, и инъекция роняла себя же сообщением «предмета инъекции
# нет» — верно и по назначению, но чинить надо было её, а не дерево.
#
# ГДЕ ПРЕДМЕТ ТЕПЕРЬ. Дерево службы приезжает модулем, закреплённым `go.mod`;
# унести его из кэша инъекция не вправе и не должна. Единственное место, где
# доля службы МОЛЧА пропадает из входа, — ОБЪЯВЛЕНИЕ корней: перечень
# `KACHO_PROTO_ROOTS` и карта `KACHO_PROTO_ROOT_MODULES` в библиотеке раскладки.
# Именно так дефект и наблюдался: генератор выходил кодом 0 и печатал
# «OK: 233 entries» вместо 350.
#
# ПРАВЯТСЯ ОБА ОБЪЯВЛЕНИЯ, И ЭТО НЕ ПЕРЕСТРАХОВКА. Сборщик входа отказывает на
# карте, называющей корень вне закрытого множества, — то есть правка одного
# объявления дала бы ГРОМКИЙ отказ сборки входа вместо молчаливой потери доли,
# а инъекция проверяла бы не тот класс.
# ---------------------------------------------------------------------------
LIB="${ROOT}/gateway/scripts/lib/stage-proto-tree.sh"
LIB_BAK="$(mktemp -d)/stage-proto-tree.sh"
restore_lib() { [[ -f "${LIB_BAK}" ]] && cp "${LIB_BAK}" "${LIB}"; }
trap 'restore_lib' EXIT

echo "== ось 1: корень kaname снят из ОБЪЯВЛЕНИЯ — доля службы пропадает =="
if ! grep -q '^KACHO_PROTO_ROOTS=(kacho kaname corelib)$' "${LIB}" \
   || ! grep -q '^KACHO_PROTO_ROOT_MODULES=(kaname=' "${LIB}"; then
  echo "  ✗ ось 1: объявления корней в ${LIB} не той формы — предмета инъекции нет" >&2
  fails=$((fails + 1))
else
  cp "${LIB}" "${LIB_BAK}"
  sed -i \
    -e 's/^KACHO_PROTO_ROOTS=(kacho kaname corelib)$/KACHO_PROTO_ROOTS=(kacho corelib)/' \
    -e 's/^KACHO_PROTO_ROOT_MODULES=(kaname=[^)]*)$/KACHO_PROTO_ROOT_MODULES=()/' \
    "${LIB}"
  gate_axis "kaname не объявлен — гейт краснеет и называет причину" red \

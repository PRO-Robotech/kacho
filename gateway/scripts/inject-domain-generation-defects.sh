...
# фундамента, у которого каталога `cloud` нет вовсе. Как ОБЛАЧНЫЙ домен он
# перестал существовать, и фикстуры, называвшие его, роняли не гейт, а сам
# харнесс — «выбранного домена нет в дереве контрактов». Замена не косметика:
# фикстура обязана называть домен, который дерево производит.
#
...
new = (
    ...
    # Карта нейтральных корней ОСТАЁТСЯ объявленной: на неё опирается отказ
    # «объявленный корень не дал ни одного дерева» ниже в той же функции, и
    # снятие объявления уронило бы прогон синтаксисом, а не находкой A12.
    '  local -A _is_neutral=([corelib]=1)\n'
...
echo "== ось 11: копия наследует режим источника (A13 + A1) =="
# ЗДЕСЬ СТОЯЛО «краснеет РОВНО A13», и это перестало быть верным — сказано прямо,
# потому что требование «инъекция обязана ронять ТОЛЬКО проверяемое» никуда не
# делось, а выполнить его на этом предмете больше нельзя.
#
# ПОЧЕМУ. Прежняя редакция опиралась на то, что остальные оси читают ЗАПИСЫВАЕМЫЙ
# корень (рабочую копию), и read-only источник появляется ТОЛЬКО в воспроизводке
# A13. После того как вход генераторов стал СОБИРАТЬСЯ ИЗ МОДУЛЕЙ (kacho#2616,
# исход C), read-only источник читает КАЖДЫЙ прогон: дерево контрактов службы
# приезжает из кэша модулей, а кэш — каталоги 555. Значит приведение режима стало
# несущим на всех путях, и его снятие ловится A1 раньше, чем до A13 доходит дело.
#
# ЧТО ОСЬ УТВЕРЖДАЕТ ТЕПЕРЬ: снятие приведения режима роняет прогон и называет
# отказ порождения — то есть свойство по-прежнему СПОСОБНО упасть, и падает
# громче, чем прежде. Изолировать A13 отдельным близнецом нечем: её предмет
# поглощён общим путём, и утверждать обратное было бы утверждением о
# непрочитанном.
#
# ТОЧКА ОДНА. Приведение режима сведено в `kacho_make_tree_ours` — прежде обходов
# было два (сборка входа и раскладка стадии), свойство держалось дважды, и
# снятие одного оставляло второй.
pyedit "ось 11: приведение режима стадии снято" "$LIB" <<'PYX'
import sys
p = sys.argv[1]; s = open(p, encoding='utf-8').read()
# ТОЧКА ИНЪЕКЦИИ — ТЕЛО ОБЩЕЙ ФУНКЦИИ, а не строка у одного вызывающего.
# Здесь стоял обход у раскладки стадии; со сборкой входа из модулей приведение
# режима понадобилось ДВАЖДЫ, свойство стало держаться в двух местах, и снятие
# одного оставляло второй — A13 зеленела при снятом предмете. Обходы сведены в
# `kacho_make_tree_ours`, и её тело есть единственное место, где свойство живёт.
old = '  find "$1" -type d -exec chmod u+w {} +\n'
assert old in s, 'ось 11: тело kacho_make_tree_ours не найдено'
# ИНЪЕКЦИЯ: копия остаётся с режимом источника — как было до kacho#1110
open(p, 'w', encoding='utf-8').write(s.replace(old, '  :\n', 1))
PYX
gate_axis "режим копии не приведён — прогон краснеет и называет отказ порождения" \
    red "порождение полного каталога отказало"
restore
...
new = '  find "$1" -type d -print0 | xargs -0 chmod u+w\n'
open(p, 'w', encoding='utf-8').write(s.replace(old, new, 1))
PYX

...
#   S5  overlap НЕ учитывается арифметикой (не суммой, а множеством): записи,
#       общие обеим долям (сегодня — службы нейтрального корня фундамента,
#       `corelib.operation` и `corelib.subscription`: стадия несёт его ЦЕЛИКОМ
#       при любом отборе, поэтому они есть и у службы, и у платформы), не дают
#       ложного расхождения «сумма больше вшитого». Величина overlap печатается,
#       а не требуется положительной — исчезновение общих записей гейт не роняет;

...
Gateway — это периметр платформы: за ним сервисы доменов (`kaname`, `kacho-vpc`,
...
Скалярного пина издателя (`KACHO_HYDRA_ISSUER` / `KACHO_HYDRA_JWKS_URL`) больше нет:
ручки сняты вместе с ветвью, которая ВЫВОДИЛА адрес издателя из домена установки,
когда перечень не объявлен. Необъявленный перечень теперь **отказывает в старте**
под любой меткой окружения — прежде край на этом пути поднимался и отвергал каждый
токен при первом же запросе.

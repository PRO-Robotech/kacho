...
// затем по FQN. Строковое сравнение Template дает «path-then-method»
// группировку (короткий префикс раньше вложенного, `:verb` после базового пути).
//
// ТРЕТИЙ КЛЮЧ НЕ УКРАШЕНИЕ — без него порядок не определён.
//
// Пара (метод, шаблон) перестала быть уникальной, как только один ресурс стал
// служиться двумя сервисами сразу: административная поверхность пула адресов
// публикуется рядом с внутренней и НА ТОМ ЖЕ каноническом пути (ADM-1 S1,
// решение владельца — второго адреса у ресурса быть не должно). До этого дубля
// не было ни одного, поэтому неполнота ключа ничего не стоила и была невидима.
//
// `sort.Slice` НЕ стабилен. На равных ключах он вправе выдать любой из двух
// порядков, и выбор зависит от входного порядка и от внутренностей реализации —
// то есть golden-файл переставал бы быть функцией МНОЖЕСТВА маршрутов и
// становился функцией того, в каком порядке их собрал обход дескрипторов.
// Расхождение вылезало бы не отказом, а перемежающейся краснотой гейта
// свежести — самой дорогой формой отказа: невоспроизводимой.
func sortRoutes(routes []Route) {
	sort.Slice(routes, func(i, j int) bool {
		...
		return routes[i].FQN < routes[j].FQN
	})
}

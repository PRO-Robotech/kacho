...
// Штамп сборки: значения подставляет СБОРКА (`-ldflags -X` на оба символа), а не
// ручка профиля. Ставит ли штамп ЭТОТ образ — здесь НЕ утверждается: состояние
// принадлежит держателю `internal/repohygiene`
// `TestBuildStampReachesEveryBinaryThatDeclaresIt`, и он же роняет прогон на
// всяком объявившем, до кого подстановка не дошла. Прежде здесь стояло
// «инжектится через -ldflags» — обещание, которого ни одна сборка этого образа
// не исполняла; оно было снято (#2521), а САМА подстановка заведена (#2527), и
// теперь образ штамп ставит. Число «сколько образов ставят» здесь по-прежнему не
// пишется: в прозе у него нет владельца — оно стареет молча, и ровно этим
// кончилась прежняя формулировка. Умолчания ниже — то, что видит сборка БЕЗ
// подстановки: `go run`, `go test` и всякая сборка руками. В образе они означали
// бы, что аргумент сборки пуст.
var (
	buildVersion = "dev"
	buildCommit  = "unknown"
)
...
// ЗДЕСЬ СТОЯЛА `surfaceMode` — вывод посадки для диагностической поверхности.
//
// Она выводила ту же величину, что теперь выводит `describePosture`, и делала
// это МЯГКО: неразборчивая метка читалась как боевая. Такой исход fail-closed и
// потому защитим, но оператор, опечатавшийся в ручке, не узнавал об этом ниоткуда
// — процесс поднимался и работал в режиме, которого никто не выбирал.
//
// Посадка выводится теперь ОДИН раз на процесс, центральным дескриптором
// (задача продукта #1407), и неразборчивая метка роняет старт, называя ручку.
// Поверхность берёт готовое значение из принятого дескриптора: второй вывод той
// же величины разъехался бы с первым молча.

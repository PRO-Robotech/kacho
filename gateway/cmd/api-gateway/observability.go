...
// не исполняла. Предмет остатка назван ВЕДОМОСТЬЮ держателя, а не здесь: число в
// прозе не имеет владельца — оно стареет молча, и ровно этим кончилась прежняя
// формулировка. Умолчания ниже — то, что видит локальная сборка, а в образе они
// означают «штамп не поставлен».
var (
	buildVersion = "dev"
	buildCommit  = "unknown"
)

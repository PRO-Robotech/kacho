...
func main() {
	...
	// ЖУРНАЛ ДОСТУПА — САМЫМ ВНЕШНИМ, и это то же решение, что принято двумя
	// экранами выше для измерителя задержки на НАТИВНОЙ полосе: он обязан
	// накрывать всё, что край делает ради запроса, включая отказ по правам,
	// отказ маршрутизации и отказ по потолку тела.
	//
	// Звено, стоящее ЗА полосой прав, не видит ни одного запроса, который та
	// отвергла сама: она отвечает и следующего не зовёт. Пока журнал стоял
	// внутри, анонимный перебор внешней поверхности не оставлял НИ ОДНОЙ строки
	// — измерено: 42 запроса к необслуживаемым путям давали 0 строк. Это ровно
	// тот класс событий, ради которого приходят в разбор происшествия.
	//
	// У полосы HTTP измерителя задержки нет ни одного (`edgeLatency` живёт
	// только на нативной), поэтому журнал доступа здесь — ЕДИНСТВЕННАЯ запись о
	// запросе, и место ему соответствующее.
	//
	// Идентификатор запроса ставится снаружи и потому уже в контексте, когда
	// журнал читает его; принципала журнал не пишет, поэтому вынос за полосы
	// личности ничего не теряет.
	//
	// Порядок закреплён гейтом TestHTTPAccessLogIsOutsideTheRightsLane.
	...
	httpSrv := &http.Server{
		...
		// ПОТОЛОК СТРОКИ ЗАПРОСА И ЗАГОЛОВКОВ — РЕШЕНИЕ, а не умолчание.
		//
		// Умолчание `net/http` — мегабайт, и оно ничьим решением не является: его
		// никто не выбирал под этот край. Величина несущая, потому что это
		// потолок того, сколько байт запросчик БЕЗ удостоверения заставит край
		// принять, разобрать и ЗАПИСАТЬ: путь из строки запроса уезжает в журнал,
		// а журнал — общий ресурс, который чинят не быстрее, чем он кончается.
		//
		// 64 КиБ: предъявитель с полным набором утверждений и куки сессии
		// укладывается с запасом в порядок, а место для строки запроса остаётся
		// кратно длиннее самого длинного маршрута контракта (65 байт по обходу
		// дескрипторов). Смежная защита от того же класса — усечение пути в
		// записи (middleware.EdgeMaxLoggedPathBytes): потолок ограничивает вход,
		// усечение — след, и одно другое не заменяет.

...
func main() {
	...
	// --- отзыв УДОСТОВЕРЕНИЯ доезжает до открытых потоков ---
	//
	// Соседний блок закрывает отзыв ПРАВ: сняли привязку — журнал смены субъекта
	// назвал принципала, потоки принципала закрылись. Отзыв УДОСТОВЕРЕНИЯ —
	// выход человека, административное принудительное завершение, отзыв токена —
	// строк в тот журнал НЕ ПИШЕТ и тем читателем не виден вовсе: до kacho#1410
	// проверка предъявленного делалась один раз, при открытии, и границей отзыва
	// на открытом соединении был срок жизни потока.
	//
	// Дверь ОДНА и та же: тот же реестр, та же отмена контекста. Отличается
	// ключ — отзыв прав называет ПРИНЦИПАЛА, отзыв удостоверения называет
	// УДОСТОВЕРЕНИЕ, и радиусы у них разные по существу: выход из одной вкладки
	// не вправе закрыть остальные, а вошедший заново обязан работать.
	//
	// Не зависит от слоя прав, в отличие от соседа: предмет здесь — предъявленное
	// удостоверение, и кэш вердиктов к нему отношения не имеет.
	if iamConn := backends["iamInternal"]; iamConn != nil {
		sweeper, swErr := buildCredentialRevocationSweeper(
			cfg, clients.NewSessionRevocationsAdapter(iamConn), subscriptionStream, logger)
		if swErr != nil {
			log.Fatalf("credential-revocation sweeper: %v", swErr)
		}
		go sweeper.Run(ctx)
		// Самоотчёт называет ОКНО ЧИСЛОМ — иначе «отзыв удостоверения доезжает»
		// осталось бы утверждением без величины, а величиной по умолчанию был бы
		// срок жизни соединения, то есть ровно то, что здесь и чинится.
		// Печатается НАБЛЮДЕНИЕ, а не литерал.
		logger.Info("credential-revocation sweeper started",
			"interval", sweeper.Interval().String(),
			"window", sweeper.Window().String(),
			"stale_after", sweeper.StaleAfter().String(),
			"closes_streams", sweeper.ClosesStreams())
	}

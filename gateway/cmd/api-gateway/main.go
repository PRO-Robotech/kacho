...
func main() {
	...
	// ретрансляция полосы формы (Ф3 Р15).
	//
	// Негодное значение отвергает РАЗБОР, незаданное — СТРАЖ строкой ниже. Две
	// проверки, а не одна, потому что это два разных исхода: «названо не то» и
	// «не названо ничего». Второй до заведения стража проходил молча — а
	// незаданное не есть `own`, поэтому все три места оставались непровязанными
	// и войти было нельзя ни при каком вводе.
	identityLane, ipErr := cfg.ResolvedIdentityProvider()
	if ipErr != nil {
		log.Fatalf("identity posture startup-validation: %v", ipErr)
	}
	if ipErr := validateIdentityPosture(identityLane); ipErr != nil {
		log.Fatalf("identity posture startup-validation: %v", ipErr)
	}

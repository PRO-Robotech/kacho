...
// том, что в нём есть.
//
// Произведение поэтому ВЫВОДИТСЯ ЦЕЛИКОМ: посадки берутся из словаря
// (`identityposture.Values`), состояния носителя — как непустые подмножества
// тех же сторон. Третья сторона, появись она, растит обе оси разом: посадок 3,
// состояний 7, пар 21 — и проба краснеет, а не молчит.
...
// 6» и проходила, тогда как произведение — 21 (посадок 3 × непустых
// подмножеств трёх сторон 7): число читалось как полнота, которой у него не
// было. Дыру закрывал соседний гейт, и это было сказано честно, — но честная
// оговорка не делает число верным.
//
// Число 21 ИЗМЕРЕНО подстановкой третьей стороны в словарь, а не выведено на
// бумаге: в первой редакции этой шапки стояло 14, и оно было моей арифметикой,
// а не замером.
//

...
:::note Четвёртой строки здесь больше нет
Гейтвей поднимал ещё и собственный cluster-internal **gRPC**-listener — ради единственной службы
сброса authz-кэша, которую вызывал `kaname`. Служба и listener сняты вместе: кэш решений
гейтвей гасит сам, читая журнал смены субъекта у `kaname`. Порт под приём больше не
открывается и в профиле не объявляется.
:::

Одной ручки режима для локального старта мало: край требует ещё четыре. Команда и причина
каждой ручки — в [Конфигурации → Локальный запуск](/install/configuration#local-run).

```mermaid
flowchart LR
  cli["Внешние клиенты"] -->|"TLS :443 → cmux"| agw["api-gateway"]
  ui["UI / admin-tooling"] -->|"cluster-internal REST :8081"| agw
  agw -->|"backends :9090 / :9091"| dom["домены (iam/vpc/compute/geo/nlb/registry)"]
  agw -->|"PollSubjectChanges (курсор)"| dom
```

## Контейнерный образ

Образ собирается как **single-repo**: зависимости (`pkg/` + proto-stubs доменов) тянутся
...
<CodeBlock language="bash">
  {dedent`
    ...
    curl http://<gateway>/readyz     # readiness — 200, если доступен kaname
  `}
</CodeBlock>

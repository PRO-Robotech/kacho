...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>internal/middleware</code></td><td>AuthN (JWT / DPoP / mTLS / сессия платформы), AuthZ, кэши, OIDC, idempotency</td></tr>
...
```
RequestID → Recovery → AuthN(dev-HMAC / сессия платформы) → DPoP/JWT(по издателю) →
...
<table>
  ...
  <tbody>
    <tr><td>JWKS</td><td>Публичные ключи проверки подписи JWT — по одной записи на объявленного издателя</td><td>TTL (<code>KACHO&#95;JWKS&#95;CACHE&#95;TTL&#95;SECONDS</code>, дефолт 300 с)</td></tr>

...
**потребитель**: край сам читает журнал смены субъекта у `kaname` и гасит свой кэш сам.
...
- **опрос журнала** — `gateway/internal/watcher/subject_change_watcher.go` зовёт
  ...
  `kaname`, читая `kacho_iam.subject_change_outbox` курсором по возрастанию `id`. Непустая
...
арендатора и не повышает привилегий — следующий запрос всё равно перепроверяется в `kaname`.
...
> cluster-internal gRPC-листенере края (`:9091`), который вызывал дренаж `kaname`, и вся
...
> листенером: это было ребро **из листа обратно к потребителю** — `kaname` объявлен листом

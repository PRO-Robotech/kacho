...
// сведено в ОДНО объявление (`corelib/principalwire`), читатель ключа пишет не
...
//
// САМО ОБЪЯВЛЕНИЕ ЧИТАТЕЛЕМ НЕ ЯВЛЯЕТСЯ. Пакет `corelib/principalwire` из обхода
...
// говорит о чужой копии, а не о нашем коде. Общий корпус (corelib/treecorpus)

...
//
// # Цена этой формы — названа, а не спрятана
//
// Гейт дерева `internal/repohygiene`
// `TestDeclaredRetentionSweepersHaveAProductionCaller` узнаёт уборщика по
// литералу удаления ВНУТРИ тела функции, а оператор, собранный из кусков, не
// видит — это его СОБСТВЕННАЯ названная слепая зона. Значит `Reap` из-под его
// наблюдения вышел, и уборщиков по сроку он насчитывает на одного меньше.
//
// Свойство «у уборщика есть вызывающий» держит здесь не он, а
// `TestReapSweep_JanitorStepFollowsTheRecordLifetime`: она утверждает ИСХОД —
// просроченная строка исчезает без единого вызова уборки из пробы, — а не
// наличие вызова, то есть держит больше, чем синтаксический признак. Соседняя
// уборка вышла из-под того же наблюдения тем же способом и по той же причине
// (#1293).

...
func f1bGuardCases() []f1bGuardCase {
	return []f1bGuardCase{
		...
		{
			// Перечня нет вовсе. Прежде на этом входе край строил запись сам,
			// выводя издателя из домена установки; теперь это отказ, и различитель
			// у него свой — не «ноль элементов», а «не объявлено».
			Name: "перечень издателей не объявлен",
			Cfg: func() config.Config {
				c := f1bBase("production")
				c.PlatformTokenIssuer = f1bOurs
				return c
			}(),
			Discriminator: "is not declared: the edge has no accepted token issuer",
		},
	}
}

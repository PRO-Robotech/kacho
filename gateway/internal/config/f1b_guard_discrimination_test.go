...
func f1bGuardCases() []f1bGuardCase {
	return []f1bGuardCase{
		...
		// ЗДЕСЬ СТОЯЛИ ДВА РЯДА, И ОБА СНЯТЫ ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ:
		//
		//   - «адрес набора не разбирается как URL на ЗАПАСНОМ пути» — запасного
		//     пути больше нет, адрес приезжает только объявленной записью, и её
		//     разбор судит сосед выше;
		//   - «два объявления об одном предмете» — скалярный пин снят из
		//     объявления процесса, второго объявления стало неоткуда взяться, и
		//     вход этого ряда непредставим.
		//
		// Ряд, чей вход непредставим, — не «лишняя строгость», а подпорка:
		// отрицательный кейс замолкает МОЛЧА, его инъекция остаётся зелёной, и
		// перепись рядов начинает считать то, чего нет.
		{

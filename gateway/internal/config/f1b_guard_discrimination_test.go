	...
	return []f1bGuardCase{
		...
		},
		{
			...
			Cfg: func() config.Config {
				...
				//
				// ИЗДАТЕЛЬ ОБЪЯВЛЯЕТСЯ ЗДЕСЬ ЯВНО, и это не украшение фикстуры.
				// Прежде он ВЫВОДИЛСЯ из домена установки, поэтому пин выглядел
				// полным, ничего не объявляя. Теперь неполный пин отвергается
				// раньше — стражем «приём токена не объявлен», — и фикстура,
				// объявляющая одну половину, получала бы отказ СОСЕДА, а этот
				// страж не мог бы упасть вовсе.

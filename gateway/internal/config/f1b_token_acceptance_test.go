...
func TestF1b01_UnsetDeclarationFallsBackToTheDeclaredLegacyRecord(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("посадка, не объявляющая перечня, отвергнута: %v — состояние это в "+
			"дереве беспредметно (перечень объявляет каждый стенд), но ПРЕДСТАВИМО, и "+
			"разбор обязан его различать, пока ручки прежнего издателя существуют", err)
	}
	if len(got) != 1 {
...
func TestF1b06_UnsetLegacyPinIsNotATrustAnchorDerivedFromTheDomain(t *testing.T) {
	...
	// Стенда, идущего сюда, в дереве больше нет; вход остаётся представимым,
	// пока представимы сами ручки, и разбор обязан его принимать.
	declaredPin := bare

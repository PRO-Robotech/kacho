...
// ─────────────────────────────────────────────────────────────────────────────
// ПРОБЕЛЫ: два вопроса заданы двум разным величинам, и обе половины проверены.
//
// Текст отказа обещает, что разделяющие пробелы снимаются, а прочее не
// прощается. Обещание обязано иметь пробу: соседние строки разбора
// спрашивают величину КАК ЕСТЬ и ОБРЕЗАННУЮ, и без пробы читатель не отличит
// решение от описки.
func TestSessionCarriers_SeparatingWhitespaceIsTrimmedAndNothingElseIsForgiven(t *testing.T) {
	// Пробелы ВОКРУГ элементов — разделители: перечень остаётся тем же.
	spaced, err := resolveCarriers(t, "own", " own , external ")
	if err != nil {
		t.Fatalf("перечень с пробелами вокруг элементов отвергнут: %v", err)
	}
	tight, err := resolveCarriers(t, "own", "own,external")
	if err != nil {
		t.Fatalf("перечень без пробелов отвергнут: %v", err)
	}
	if spaced != tight {
		t.Fatalf("пробелы вокруг элементов изменили результат: %v против %v", spaced, tight)
	}

	// Значение из ОДНИХ ПРОБЕЛОВ — объявление, а не «не задано»: оператор его
	// набрал, и молча вывести множество из посадки значило бы принять
	// объявление и не исполнить его.
	if _, err := resolveCarriers(t, "own", "   "); err == nil {
		t.Fatal("значение из одних пробелов принято как «не задано» — объявление принято и " +
			"не действует")
	} else if !strings.Contains(err.Error(), "elements") {
		t.Fatalf("отказ не называет число элементов: %v", err)
	}

	// Пробел ВНУТРИ имени — не разделитель: имя сверяется целиком.
	if _, err := resolveCarriers(t, "own", "ow n"); err == nil {
		t.Fatal("имя с пробелом внутри принято — прощено больше, чем обещает текст отказа")
	}
	t.Logf("перепись: форм записи проверено 4 · принятых 2 · отвергнутых 2")
}

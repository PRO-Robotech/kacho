...
type Config struct {
	...
	//
	// УМОЛЧАНИЯ НЕТ НАМЕРЕННО — НИ ЗДЕСЬ, НИ В ЧАРТЕ. Значение разводит ТРИ
	// места провязки композиционного корня: полосу личности, ответ «кто я» и
	// ретрансляцию глаголов формы службы доступа. Любое умолчание отвечает за
	// стенд на вопрос, на который отвечает оператор, и отвечает МОЛЧА: `own`
	// снял бы провайдерские требования с профиля, который забыли обновить, а
	// `external` объявил бы настроенным стенд, с которого поставщик уже снят, —
	// войти на нём нельзя ни при каком вводе, и ни одного отказа при этом не
	// произносится.
	//
	// Незаданное значение ОТВЕРГАЕТ СТАРТ (страж —
	// cmd/api-gateway/identity_posture_validation.go, безусловно, без
	// послабления по метке окружения). Что умолчания нет и у чарта, держит
	// gateway/deploy/identity_posture_reaches_every_stand_test.go: посадку
	// объявляет КАЖДЫЙ развёртываемый стенд.
	//
	...
	//
	// Не задано ⇒ ОТКАЗ В СТАРТЕ: принимаемого издателя край знает только
	// объявленной записью, а адрес его набора ключей не выводится ниоткуда.
	// Задано, но элементов ноль (`","`, пробелы) ⇒ тот же отказ по другому
	// основанию: пустой перечень означает «принимаем любого издателя». Страж
	// считает ЭЛЕМЕНТЫ, а не длину строки, и оба отказа безусловны — от метки
	// окружения они не зависят.

...
type Config struct {
	...
	//
	// ИЗДАТЕЛЬ ПРЕЖНЕГО ПОСТАВЩИКА ОТСЮДА БОЛЬШЕ НЕ ВЫВОДИТСЯ. Пока выводился,
	// «прежний издатель не объявлен» не наступало ни на одной посадке, и край
	// забирал якорь доверия проверки подписи по адресу, которого ему никто не
	// давал (см. DeclaredHydraIssuer).
	//
	...
	// выводился из него, «адресат не объявлен» не наступало ни на одной
	// посадке. Адресат объявляется своей ручкой без умолчания: TokenAudience
	// ниже.
	//
	// ЗДЕСЬ СТОЯЛО «издатель прежнего поставщика отсюда выводится ПО-ПРЕЖНЕМУ»
	// с доводом «issuers не задано — сегодняшнее, работающее и повсеместное
	// состояние». Оба утверждения пережили свой предмет. Вывод издателя из
	// домена снят (`DeclaredHydraIssuer`), и домен на приём токена не влияет
	// вовсе. Повсеместным незаданный перечень тоже быть перестал: его объявляет
	// КАЖДЫЙ стенд дерева, и это не память, а перепись —
	// `TestF1b07_EveryLiveStackDeclaresItsIssuerSetExplicitly`
	// (gateway/deploy) печатает её и краснеет на первом же стенде, получившем
	// перечень умолчанием.
	//
	// Величина оставлена: её читают соседние поля разбора, а не приём токена.
	//
	...
	// (утверждение `aud`). Задача #2567.
	//
	// УМОЛЧАНИЯ НЕТ НАМЕРЕННО, и довод тот же, по которому его нет у посадки
	// личности выше и у адресата чеканящей службы: адресат — единственное, чем
	// удостоверение говорит, КАКОЙ УСТАНОВКЕ оно выдано. Подставленное
	// построением имя означало бы, что адресата каждого принимаемого токена
	// выбрал не тот, кто ставит край, — и установка выглядела бы настроенной.
	//
	// Прежде значение выводилось построением («https://» + APIDomain), у
	// которого умолчание есть. Следствие наблюдалось не как отказ, а как его
	// ОТСУТСТВИЕ: пустой величина не бывала, ручки не было, стража не было, а
	// сужение по адресату применялось лишь при непустом ожидаемом значении —
	// то есть край принимал токен, выпущенный для ДРУГОЙ установки.
	//
	...
	//
	// Не задано ⇒ строится ОДНА запись из HydraIssuer/HydraJWKSURL. ЗДЕСЬ
	// СТОЯЛО «сегодняшняя посадка»: с объявлением перечня на последнем стенде
	// (`deploy/helm/umbrella/values.dev.yaml`) по этому пути не идёт НИ ОДИН —
	// путь остаётся представимым, но предмета в дереве у него нет, и это
	// печатает перепись, а не помнит комментарий. Задано, но элементов ноль
	// (`","`, пробелы) ⇒ ОТКАЗ
...
//
// ИМЯ БЕЗ СЛОВА «TOKEN» — НЕ СОКРАЩЕНИЕ, А ТРЕБОВАНИЕ СКАНЕРА. Разбор
// безопасности признаёт предполагаемым удостоверением всякую строковую
// константу, чьё ИМЯ несёт признак секрета (в их числе «token»), и `Token…`
// давал находку G101 на имени переменной окружения. Соседние ручки того же
...
//
// Имя говорит «объявленный», а не «ожидаемый», намеренно: у величины нет

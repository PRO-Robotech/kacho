...
type Config struct {
	...
	// отзыва.
	//
	// По этому хопу едет ПРЕДЪЯВЛЕННЫЙ токен, поэтому он идёт под клиентским
	// сертификатом края, а сертификат авторитета проверяется против этой
	// связки. Пусто ⇒ транспорт по умолчанию; задано ⇒ связка становится

...
const (
	// knobDeclaredKeySets — настройка, из которой приезжает адрес набора.
	// Названа константой, потому что её называет ОТКАЗ, а отказ, указывающий не
	// на ту настройку, хуже отсутствующего: он отправляет оператора править то,
	// что в этой посадке пусто.
	//
	// Рядом стояла вторая — `knobLegacyKeySet` (`KACHO_HYDRA_JWKS_URL`), и она
	// снята вместе со скалярным пином: настройки, которую не читает никто,
	// отказ называть не вправе.

...
// RPC появлялся в proto, получал REST-биндинг (REST выводится из дескрипторов
// и отстать не может) — и оставался недостижим по нативному gRPC, потому что
// добавить его в этот файл забыли.
//

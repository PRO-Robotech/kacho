...
// Registerer отдаёт реестр края как ПРИЁМНИК регистрации.
//
// Окно нужно измерителю задержки обслуженного вызова (`pkg/grpcsrv.ServerLatency`):
// он заводит своё семейство серий СВОИМИ руками, а не отдаёт читателя готовых
// величин, как соседнее окно `RegisterAuthz`. Тогда несогласованное объявление
// (то же имя с другой размерностью) становится отказом подъёма, а не молчаливой
// пропажей семейства со скрейпа.
//
// Край — восьмая поверхность платформы и единственная, которая не поднимается
// носителем входящего пути: оба его слушателя строятся здесь, поэтому и
// провязывать измеритель приходится здесь.

// Copyright (c) PRO-Robotech
// SPDX-License-Identifier: BUSL-1.1

package subscriptionstream

import (
	"context"
	"sync"
)

// registry — открытые потоки, сгруппированные по субъекту.
//
// # Зачем группировка ПО СУБЪЕКТУ, а не плоский список
//
// Она заведена ради kacho#1022 («отзыв прав закрывает открытый поток»), и это
// сказано прямо, чтобы её не приняли за преждевременное обобщение. Отзыв
// приходит с ИМЕНЕМ СУБЪЕКТА — край уже получает его на своём внутреннем
// слушателе, — поэтому закрытие обязано быть обходом одного ключа. Плоский
// список потребовал бы перебора всех открытых потоков на каждый отзыв, а их
// столько, сколько вкладок у всех арендаторов сразу.
//
// Здесь реализовано ТОЛЬКО ведение реестра. Подписку на событие отзыва держат
// его читатели ([Handler.CloseSubject], [Handler.CloseAll]), а провязывает их
// композиционный корень края.
// Credential — УДОСТОВЕРЕНИЕ, которым открыт поток, в том объёме, в каком про
// него можно спросить авторитет отзыва.
//
// # Зачем оно рядом с субъектом, если субъект уже есть
//
// Ключ реестра `bySubj` называет ПРИНЦИПАЛА, и им адресуется отзыв ПРАВ
// (kacho#1022): привязку сняли — закрылись все потоки субъекта. Отзыв
// УДОСТОВЕРЕНИЯ адресуется иначе и радиус у него другой: выход из одной вкладки
// не вправе закрыть остальные, а человек, вошедший заново, обязан продолжать
// работать. По субъекту это неразличимо by construction.
//
// # Почему поля именно эти, а не «весь контекст удостоверения»
//
// Здесь ровно то, чем задаётся ВОПРОС авторитету, и ничего сверх:
//
//   - [Credential.JTI] — идентификатор подписанного удостоверения. Пуст у
//     браузерной сессии: у неё удостоверения нет вовсе, и полоса
//     аутентификации этого заголовка не выставляет;
//   - [Credential.UserID] — человек. Непуст ТОЛЬКО у человека: отсечка сессий
//     ключуется им, и спрашивать по словарю людей про служебную учётку значило
//     бы задавать вопрос про субъекта, которого в таблице не бывает;
//   - [Credential.AuthAt] — момент аутентификации, секунды эпохи. Ноль означает
//     «источник момента не назвал» — это НЕ «давно» и не подтверждение в 1970
//     году, и читатель обязан различать.
//
// Предъявленный СЕКРЕТ здесь не хранится и храниться не может: держать его
// живым весь срок потока значило бы завести поверхность там, где её не было.
// Следствие названо честно — про удостоверение, чей вопрос требует секрета
// (базовый секрет края), спросить с потока нечем; такой поток считается
// отдельно, а не выбрасывается молча.
//
// Тип сравним, поэтому служит ключом карты напрямую: собирать из полей строку
// значило бы завести второй кодек, который разошёлся бы с первым молча.
type Credential struct {
	JTI    string
	UserID string
	AuthAt int64
}

// Askable — есть ли у этого удостоверения вопрос, который можно задать
// авторитету отзыва.
//
// Объявлено ЗДЕСЬ, а не у сметателя: свойство принадлежит удостоверению, и
// второе его выражение разошлось бы с первым — причём разошлось бы молча, дав
// либо необъяснимый остаток, либо вопрос ни о ком.
func (c Credential) Askable() bool { return c.JTI != "" || c.UserID != "" }

type registry struct {
	mu     sync.Mutex
	next   uint64
	bySubj map[string]map[uint64]*stream
	// byCred — ТЕ ЖЕ записи, вторым ключом. Не второй реестр и не вторая дверь:
	// закрытие по-прежнему одно ([stream.close]), меняется только то, каким
	// именем отзыв называет закрываемое.
	byCred map[Credential]map[uint64]*stream
}

func newRegistry() *registry {
	return &registry{
		bySubj: make(map[string]map[uint64]*stream),
		byCred: make(map[Credential]map[uint64]*stream),
	}
}

// stream — один учтённый поток.
//
// Отмена ставится ПОЗЖЕ учёта: место в реестре занимается до того, как заведён
// контекст, — иначе проверка предела и постановка на учёт разъезжаются, и два
// одновременных запроса одного субъекта оба увидят свободное место. Это
// software check-then-act, и лечится он не аккуратностью, а тем, что решение и
// его следствие происходят под одним замком.
//
// Из-за этого зазора появляется [stream.closed]: субъекта могут закрыть между
// учётом и заведением контекста, и тогда отмена обязана сработать НЕМЕДЛЕННО
// при постановке. Иначе поток, закрытый по отзыву прав, продолжил бы жить —
// молча и ровно в том окне, где отзыв и происходит.
type stream struct {
	mu     sync.Mutex
	cancel context.CancelFunc
	closed bool
}

// arm ставит отмену уже учтённому потоку.
func (s *stream) arm(cancel context.CancelFunc) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.closed {
		// Субъекта закрыли в зазоре между учётом и заведением контекста.
		cancel()
		return
	}
	s.cancel = cancel
}

// close отменяет поток и помечает его закрытым.
func (s *stream) close() {
	s.mu.Lock()
	cancel := s.cancel
	s.closed = true
	s.mu.Unlock()
	if cancel != nil {
		cancel()
	}
}

// tryAdd ставит поток на учёт, если субъект не исчерпал СВОЙ предел.
//
// # Почему предел на субъекта, а не только на реплику
//
// Потолок реплики один на всех, поэтому один арендатор занимает его целиком, и
// остальные получают отказ, не имея НИ ОДНОГО собственного потока. Консоль
// открывает поток на вкладку — десяток вкладок одного пользователя закрывал бы
// подписку всем прочим. Предел реплики защищает ПРОЦЕСС, предел субъекта
// защищает АРЕНДАТОРОВ ДРУГ ОТ ДРУГА, и одно другого не заменяет.
//
// # Почему проверка и постановка — под одним замком
//
// «Посмотреть, сколько занято, и занять ещё одно» есть software check-then-act:
// два одновременных запроса одного субъекта оба увидят свободное место и оба
// его займут. Предел, обходимый параллелизмом, — это предел ровно до той
// нагрузки, ради которой он заведён.
//
// Возвращается замыкание снятия, а не пара (субъект, номер): вызывающему не
// приходится хранить ключ, а значит и терять его. Снятие идемпотентно, поэтому
// `defer` безопасен на любом пути выхода.
func (r *registry) tryAdd(subject string, cred Credential, perSubject int) (*stream, func(), bool) {
	if subject == "" {
		// Безымянный поток закрыть нечем: у отзыва нет ключа, которым его
		// назвать. Отсекается здесь, а не только у вызывающего, — страж у одной
		// двери переживает появление второго вызывающего.
		return nil, nil, false
	}
	r.mu.Lock()
	streams, ok := r.bySubj[subject]
	if ok && perSubject > 0 && len(streams) >= perSubject {
		r.mu.Unlock()
		return nil, nil, false
	}
	r.next++
	id := r.next
	if !ok {
		streams = make(map[uint64]*stream)
		r.bySubj[subject] = streams
	}
	entry := &stream{}
	streams[id] = entry
	byCred, ok := r.byCred[cred]
	if !ok {
		byCred = make(map[uint64]*stream)
		r.byCred[cred] = byCred
	}
	byCred[id] = entry
	r.mu.Unlock()

	var once sync.Once
	return entry, func() {
		once.Do(func() {
			r.mu.Lock()
			defer r.mu.Unlock()
			if streams, ok := r.bySubj[subject]; ok {
				delete(streams, id)
				// Пустая карта субъекта снимается вместе с последним потоком:
				// иначе реестр растёт по числу когда-либо подписавшихся, а не
				// по числу подписанных сейчас.
				if len(streams) == 0 {
					delete(r.bySubj, subject)
				}
			}
			// Тот же порядок у второго ключа, и по той же причине: иначе реестр
			// удостоверений растёт по числу когда-либо предъявленных, а обход
			// его — по числу давно ушедших.
			if streams, ok := r.byCred[cred]; ok {
				delete(streams, id)
				if len(streams) == 0 {
					delete(r.byCred, cred)
				}
			}
		})
	}, true
}

// closeSubject отменяет контексты всех потоков субъекта и возвращает их число.
//
// Снятие с учёта делает САМ поток, выходя, а не эта функция: отмена контекста
// его разбудит, и он снимется своим `defer`. Снимай их здесь — снятие
// произошло бы дважды, а второй раз уже не по своему потоку.
func (r *registry) closeSubject(subject string) int {
	r.mu.Lock()
	streams := r.bySubj[subject]
	entries := make([]*stream, 0, len(streams))
	for _, entry := range streams {
		entries = append(entries, entry)
	}
	r.mu.Unlock()

	for _, entry := range entries {
		entry.close()
	}
	return len(entries)
}

// closeAll отменяет контексты ВСЕХ учтённых потоков и возвращает их число.
//
// Снятие с учёта, как и у [registry.closeSubject], делает сам поток, выходя:
// отмена контекста его разбудит, и он снимется своим `defer`.
func (r *registry) closeAll() int {
	r.mu.Lock()
	entries := make([]*stream, 0, len(r.bySubj))
	for _, streams := range r.bySubj {
		for _, entry := range streams {
			entries = append(entries, entry)
		}
	}
	r.mu.Unlock()

	for _, entry := range entries {
		entry.close()
	}
	return len(entries)
}

// credentials — РАЗЛИЧНЫЕ удостоверения открытых сейчас потоков.
//
// Различные, а не по одному на поток: консоль открывает поток на вкладку, и
// десяток вкладок одного человека предъявляет ОДНО удостоверение. Спрашивать
// про него десять раз значило бы платить соседу за собственную небрежность —
// притом ровно в том случае, который и есть обычный.
//
// Число возвращаемых ограничено сверху потолком одновременных потоков реплики
// (Config.MaxStreams) by construction: реестр не хранит ушедших.
func (r *registry) credentials() []Credential {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]Credential, 0, len(r.byCred))
	for cred := range r.byCred {
		out = append(out, cred)
	}
	return out
}

// closeCredential отменяет контексты всех потоков, открытых ЭТИМ
// удостоверением, и возвращает их число.
//
// Снятие с учёта делает сам поток, выходя, — как и у [registry.closeSubject], и
// по той же причине: отмена контекста его разбудит, и он снимется своим
// `defer`.
func (r *registry) closeCredential(cred Credential) int {
	r.mu.Lock()
	streams := r.byCred[cred]
	entries := make([]*stream, 0, len(streams))
	for _, entry := range streams {
		entries = append(entries, entry)
	}
	r.mu.Unlock()

	for _, entry := range entries {
		entry.close()
	}
	return len(entries)
}

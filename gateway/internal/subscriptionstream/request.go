...
//
// Годные значения `kinds` вызывающий читает НЕ здесь и не подбором: их несёт
// первый же кадр потока (`knownKinds` служебного сообщения открытия), и край
// отдаёт его как есть, ничего о видах не зная. Своего перечня край не держит
// намеренно — он разошёлся бы со словарём владельца молча, а владелец меняет
// его собственным изменением.
const (
	paramOwner     = "owner"
	paramKinds     = "kinds"

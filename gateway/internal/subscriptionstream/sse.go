...
func (s *sseWriter) frame(name, position, data string) error {
	...
	if position != "" {
		...
		// события в одно. Форму позиции выбирает НЕ кадрировщик: сегодня она
		// кодируется стандартным base64 с дополнением (`pkg/pagetoken`,
		// `subscriptionPositionCodec`), то есть её алфавит несёт `+`, `/` и `=`,
		// а завтра владелец вправе сменить и это. Поэтому проверка стоит здесь, а
		// не в вере в какую бы то ни было форму: кадрировщик обязан быть верен
		// при ЛЮБОЙ.

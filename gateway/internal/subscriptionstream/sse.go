...
func (s *sseWriter) frame(name, position, data string) error {
	...
	if position != "" {
		...
		// события в одно. Форму позиции выбирает её ВЛАДЕЛЕЦ (`corelib/pagetoken`),
		// а не кадрировщик, поэтому проверка стоит здесь, а не в вере в чужую
		// форму: кадрировщик обязан быть верен при любой из них — и сегодняшней,
		// и той, на которую владелец её сменит.

...
type ownerStub struct {
	...
	//
	// Прежняя редакция ЗАКРЫВАЛА канал на первом потоке — чтобы второй не уронил
	// процесс двойным закрытием. Цена этого решения названа задачей #1485:
	// закрытый канал отдаёт ВСЕМ и СРАЗУ, поэтому ожидание n потоков
	// выполнялось тождественно после первого. Проба двух потолков из-за этого
	// шла дальше, не дождавшись второго удерживающего потока: предел субъекта
	// ещё не был выбран, третий запрос проходил, и её вердикт становился
	// функцией загрузки машины, а не свойства кода. Условие, выполняющееся
	// тождественно, условием не является.
	//
	// Ждут через [ownerStub.awaitStreams]; глубина буфера — [startedDepth].
	started chan struct{}
}

// startedDepth — глубина канала [ownerStub.started].
//
...
// теряет сигнал, и [ownerStub.awaitStreams] называет это числом, а не молчит.
// Восемь взято с запасом: самая многопоточная проба пакета держит два.
...
// awaitStreams ждёт, пока владелец не примет n потоков.
//
// Ждётся УСЛОВИЕ — приход n-го потока, — а не срок: до владельца поток доходит
// уже УЧТЁННЫМ (и слот реплики, и запись субъекта берутся в [Handler.ServeHTTP]
// прежде дозвона), поэтому возврат отсюда означает, что n потоков заняли свои
// места. Срок здесь страховочный, и его истечение — падение с числами: «ноль
// сигналов» обязано быть отличимо от «ноль потоков».
func (o *ownerStub) awaitStreams(t *testing.T, n int) {
	t.Helper()
	for got := 0; got < n; got++ {
		select {
		case <-o.started:
		case <-time.After(10 * time.Second):
			o.mu.Lock()
			served := len(o.requests)
			o.mu.Unlock()
			t.Fatalf("владелец принял потоков %d из ожидавшихся %d (обслужено всего %d) — "+

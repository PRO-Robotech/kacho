...
var bespokeConsoleMutationSites = map[string]string{
	...
	// Полоса входа. Глаголы `/iam/v1/auth/*` край не разбирает в message, а
	// ретранслирует слушателю службы доступа (перечень —
	// `gateway/internal/middleware/login_lane_paths.go`, ретрансляция —
	// `handler.LoginLaneRelay`): тело — форма службы, в нашем контракте его
	// message нет, сверять не с чем.
	// Клиент поставщика личности, стоявший здесь прежде, снят #1274 вместе со
	// своей записью; клиент полосы — один, и он назван строкой ниже.
	"shared/src/api/login-lane.ts": "клиент глаголов полосы входа /iam/v1/auth/*: край ретранслирует тело службе, message нашего контракта у него нет",

	// Пробы консоли. Набор e2e лежит в обходимом дереве и шлёт тела сам: либо
	// теми же глаголами полосы входа, либо серверу петли, который завела сама
	// оснастка. Страницы проб, шлющие тело петлевому серверу и подставленному
	// ответу, выпускают его транспортом пробы (`kacho.probe.fetch`) и под
	// детектор больше не подпадают — их записи сняты #1274.
	"e2e/specs/ceremony-seed.ts":             "посев «Дано» проб F8 глаголами полосы входа своим контекстом запросов, тело — форма службы",
	"e2e/scripts/issuance-guard-selftest.ts": "самопроверка стража мест выпуска шлёт тела серверу петли самой самопроверки — формой мимо транспорта и упорядочивающим транспортом; край в этом не участвует",

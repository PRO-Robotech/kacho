...
var absentRouteBodyWaiver = map[string]string{
	...
	// Здесь стояли ДЕСЯТЬ записей конформанса единого фасада — пробы службы
	// доступа, утверждавшие, что чеканка загрузочного удостоверения и
	// поверхности внешнего провайдера подписи краем НЕ обслуживаются.
	//
	// Все десять сняты ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ: коллекции, в которых эти
	// шаги живут, уехали вместе со службой (задача #1111), и освобождению
	// стало нечего освобождать. Запись, не совпадающая ни с одним шагом
	// дерева, — находка, а не безобидный след: неиспользованное разрешение
	// наследует следующая ошибка. Утверждения при этом не утрачены — их
	// судит теперь дерево службы, где лежат и кейсы, и коллекции.

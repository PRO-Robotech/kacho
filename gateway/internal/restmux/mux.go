...
func NewMux(
	...
	//
	//  1. Консоль собирает тело PATCH как «весь ответ GET + update_mask»
	//     (ui-future ResourceEditPage / InlineResourceEditForm и их копии в
	//     каждом remote). В теле поэтому едут `id`, `createdAt`, `status`,
	//     output-only зеркала — ни одного из них нет в Update-message. Отказ
	//     превратил бы каждую форму редактирования в жёсткий `400`. Набор
	//     ключей там не перечислим статически by construction: это то, что
	//     вернул сервер.

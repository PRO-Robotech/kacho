...
func TestDocsEnumGateReadsEveryLegalExampleForm(t *testing.T) {
	...
	// ФОРМА БЕЗ ПРЕДСТАВИТЕЛЯ — НАЗВАННАЯ СЛЕПАЯ ЗОНА, А НЕ ОТКАЗ.
	//
	// Здесь стояло требование, чтобы В КОРПУСЕ были представлены ОБЕ формы, и
	// оно роняло прогон, когда одна из них перестала встречаться: страницы,
	// писавшие пример забором, принадлежали службе доступа и уехали вместе с
	// ней (задача #1111). Требование при этом судило не распознаватель, а
	// СОСТАВ ЧУЖИХ ДОКУМЕНТОВ — то есть краснело на дереве, в котором ничего не
	// сломано, и толкало держать страницу ради зелёного.
	//
	// Что осталось нормой и держится ниже: КАЖДЫЙ объявленный пример обязан
	// быть распознан. Форма, у которой представителей ноль, из этого требования
	// выпадает by construction (ноль из нуля), поэтому её отсутствие
	// ПЕЧАТАЕТСЯ отдельной строкой: ветвь распознавателя жива, но деревом не
	// исполняется, и молчание по ней не означает «форма распознаётся».
	//
	// Самовосстановление: появится страница с примером этой формы — она сразу
	// попадёт в требование ниже, без чьей-либо памяти. Обе ветви
	// распознавателя при этом СОХРАНЕНЫ: снять ту, чей представитель временно
	// исчез, значило бы завести слепую зону, которая не краснеет никогда.
	if declaredFence == 0 {
		t.Log("в корпусе НЕТ ни одного примера формы «забор» (```json): ветвь " +
			"распознавателя жива, но деревом не исполняется — «распознано 0 из 0» " +
			"молчит и о верной работе, и о поломке")
	}
	if declaredBlock == 0 {
		t.Log("в корпусе НЕТ ни одного примера формы «разметка» (<CodeBlock language=\"json\">): " +
			"ветвь распознавателя жива, но деревом не исполняется")
	}
	if declaredFence == 0 && declaredBlock == 0 {
		t.Fatal("примеров не найдено НИ ОДНОЙ формы: обход прочитал ноль, и вердикт " +
			"беспредметен — «распознаны все» здесь было бы истинным by construction")
	}

...
//
// ОСЬ ВРЕМЕНИ ОТВЕТА — ЗАКРЫТА РЕШЕНИЕМ, А НЕ ПРОБОЙ. Читать это надо буквально:
// по этой оси свойство НЕ доказано и доказано не будет, и зелёное проб ниже
// читается УЖЕ, чем слово «неразличимо» — оно про код, заголовки и тело, и
// только про них.
//
// ЧТО ЗДЕСЬ БЫЛО РАНЬШЕ И ПОЧЕМУ СНЯТО. Стоял предикат снятия из двух условий:
// отношение медиан не выше порога и разница ниже джиттера сети. Оба УЖЕ
// выполнены сегодняшним деревом на всех сверяемых парах, а первое неспособно
// покраснеть в принципе: разбор маршрута возвращается на ПЕРВОМ совпадении,
// поэтому промах — самая длинная ветвь, отношение «есть» к «нет» структурно
// меньше единицы, а порог стоял только сверху. Оракул же работает ОБРАТНОЙ
// стороной: «существует» опознаётся по тому, что ответ пришёл БЫСТРЕЕ, — и этой
// стороны предикат не видел вовсе. Ось, объявленная открытой по предикату,
// который уже выполнен, — это «забыли» с видом решения.
//
// ЧТО ИМЕННО РЕШЕНО. Различие во времени остаётся, и оно не двоичное: время
// зависит от ПОЛОЖЕНИЯ маршрута в порядке разбора, поэтому наружу просачивается
// грубое положение в таблице, а не «есть или нет». У путей, совпадающих поздно,
// разница с промахом мала; у совпадающих рано — велика, и разброс между самими
// существующими путями того же порядка, что разница «есть»/«нет».
//
// ДОВОД, ПО КОТОРОМУ ЭТО ПРИНЯТО. Величины — микросекундные, а край выставлен
// наружу через сеть, где джиттер на два-три порядка больше; различие тонет в
// нём, и сотни тысяч запросов на усреднение сами по себе заметны полосой
// отказов и её счётчиком. Тот, кто стоит достаточно близко, чтобы мерить
// микросекунды, стоит внутри — а там различимость законна и обеспечивается
// внутренним слушателем намеренно.
//
// ЦЕНА ЗАКРЫТИЯ ИНАЧЕ. Снять различие значит отказаться от раннего возврата на
// пути, исполняемом на КАЖДОМ запросе края, либо завести второй разборщик с
// постоянным временем. Это другой предмет и другая цена, и платить её ради
// сигнала, тонущего в джиттере, — плохая сделка.
//
// КОГДА РЕШЕНИЕ ПЕРЕСТАЁТ ДЕЙСТВОВАТЬ (условие на ВНЕШНЕМ факте, а не срок):
// если внешний слушатель окажется достижим по каналу, чей джиттер сопоставим с
// внутрипроцессной разницей, — тот же хост, та же стойка, местный посредник
// рядом с процессом, — довод отпадает целиком, потому что отпадает его
// единственная посылка. Тогда ось открывается заново, и закрывать её придётся
// устройством разбора, а не рассуждением.

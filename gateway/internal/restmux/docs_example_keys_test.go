...
//
// ФРАГМЕНТ ответа словаря сегодня не имеет, и это решение, а не упущение.
// Пример, показывающий только под-объект (скажем, одну `metadata` операции),
// разбирается как объект и судится против ВЕРХНЕГО сообщения — то есть даст
// находку. В дереве таких нет (перепись: неразбираемых/не-объектов 0, находок
// 0), и заводить запись словаря без единого производителя значило бы держать
// полосу, которую ничто не исполняет: её молчание было бы неотличимо от
// чистоты. Появится фрагмент — словарь расширяют вместе с ним, и расширение
// получает свою сторону инъекции.
//

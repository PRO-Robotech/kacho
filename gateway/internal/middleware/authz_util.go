...
//
// ГЛАГОЛЫ ФОРМЫ (Ф3 Р2, Р8) И КООРДИНАТЫ ЦЕРЕМОНИИ АВТОРИЗАЦИИ (замысел
// LINE-A-1 §5.1а) — все, что перечислены в ЕДИНОМ объявлении
// `login_lane_paths.go`, а не литералами здесь: то же объявление читают ветка
// полосы сессии и монтаж ретрансляции, и второй перечень тех же путей разошёлся
// бы с первым молча. Что членство снимает: решение по каталогу прав, пол
// уверенности, полосу привязки предъявителя и вопрос об отзыве (для координат
// церемонии это решение, и его довод — в шапке объявления). Чего НЕ снимает:
// полосу сессии — она исполняется на каждом пути и на записях объявления
// отвергает отсечённый носитель до ретрансляции (Ф3-51, §1.8 приёмки).

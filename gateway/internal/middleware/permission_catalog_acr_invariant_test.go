...
func TestPermissionCatalog_ACR_Counts(t *testing.T) {
	...
	//
	// Ф3 (kacho#1269, до края дошло подъёмом пина службы на её ствол af0ca8f3)
	// добавила ОДНУ запись — `InternalHumanSessionService/Resolve`, вопрос края
	// к НАШЕЙ сессии по носителю. Освобождена по тому же основанию, что соседи
	// по внутреннему слушателю (INTERNAL_LISTENER), и порога подтверждения
	// личности не несёт: край задаёт вопрос НА СЛОЕ АУТЕНТИФИКАЦИИ, где решённой
	// личности ещё нет и порог требовать не у кого. Полосы «чувствительное» и

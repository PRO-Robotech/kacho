...
func TestPermissionCatalog_ACR_CountsAndByteIdentity(t *testing.T) {
	...
	//
	// ЧИСЛА НИЖЕ СНЯТЫ ЗАМЕРОМ СЛИТОГО ДЕРЕВА, а не сложением двух переписей.
	// Сложение приведено ОТДЕЛЬНО и только как контроль замера: 263 рутинных
	// ствола + 1 снятие аренды адреса + 11 публичных глаголов пула = 275.
	// Сошлось — и это единственное, ради чего его стоит называть. Разойдись оно
	// с замером, верным был бы замер, а расхождение означало бы, что одна из
	// сторон слияния потеряла записи молча.
	assert.Equal(t, 28, n2, "sensitive count")
	assert.Equal(t, 275, n1, "routine count")
	assert.Equal(t, 34, nEmpty, "no-requirement (exempt) count")
	assert.Equal(t, 337, n2+n1+nEmpty, "catalog total")

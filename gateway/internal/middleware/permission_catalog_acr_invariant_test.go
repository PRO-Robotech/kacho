...
func TestPermissionCatalog_ACR_CountsAndByteIdentity(t *testing.T) {
	...
	// 34 − 4 глагола администрирования движка = 30; итог 342 − 5 = 337. Сошлось — и
	// это единственное, ради чего его стоит называть. Разойдись оно с замером,
	// верным был бы замер.
	assert.Equal(t, 28, n2, "sensitive count")
	assert.Equal(t, 279, n1, "routine count")
	assert.Equal(t, 30, nEmpty, "no-requirement (exempt) count")

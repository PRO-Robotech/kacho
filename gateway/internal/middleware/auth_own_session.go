...
// (Ф1-56, F4d-29).
//
// # ЧЬЯ ЛИЧНОСТЬ ДЕЙСТВУЕТ, когда предъявлены ОБА носителя
//
// В переходном состоянии (профиль назвал обе стороны) браузер может принести
// наше печенье и чужое разом. Действует НАША личность, и чужой читатель на
// таком запросе НЕ СПРАШИВАЕТСЯ ВОВСЕ. Доводы — о полномочии, не о порядке
// строк: отсечку нашей сессии держит наш авторитет и на нашем носителе он
// исполняется, чужую сессию мы отозвать не можем; пол второго фактора край
// читает по НАШЕЙ сессии с оси каталога (Ф11 Р7), у чужой этого поля в наших
// терминах нет; переезд идёт К нашей чеканке, и наш носитель — более новый
// факт о том же человеке.
//
// Второе решение, из первого НЕ выводимое: наш носитель решает НА КАЖДОМ СВОЁМ
// ИСХОДЕ, включая отказ. Мёртвое наше печенье не откатывает запрос на чужую
// полосу — иначе предъявитель, придя с протухшим `kaname_session` и живым
// чужим, выбирал бы полосу сам, то есть старшинство назначал бы он. Чужая
// полоса спрашивается РОВНО тогда, когда нашего носителя в запросе нет. Это
// свойство — структурное: полоса терминальна на каждом отказе, и держит его
// `session_carrier_precedence_test.go` счётчиком обращений к чужой стороне.
//
...
func (a *AuthInterceptor) tryOwnSession(w http.ResponseWriter, r *http.Request) (next *http.Request, injected, handled bool) {
	...
	//
	// Предикат ОБЩИЙ с маршрутом «кто я» (`ourSessionCarrierOf`): в переходном
	// состоянии обе полосы обязаны одинаково отвечать на вопрос «наш ли это
	// запрос», иначе действующая личность зависела бы от задетой полосы.

...
//
// Законный близнец — имя носителя ПОСТАВЩИКА, пока его читатель под `external`
// жив (до S3, `kacho#1276`): читатель есть, гейт молчит. Снятие читателя делает
// имя находкой — так предикат истекает сам, а не по памяти.
//
// # Форма
//

...
// TestF1b08_AudienceIsRequiredAndChecked — адресат сужает на ОБЪЯВЛЕННОМ
// проверяющем: чужой контур отвергнут, отсутствующий `aud` отвергнут, свой
// принят.
//
// Здесь стояло «незаданный адресат означает „любой"» — верно на день записи и
// снято задачей #2567: построиться с незаявленным адресатом больше нельзя.
// Тело пробы этого состояния не касалось НИКОГДА (оно строит проверяющего с
// объявленным адресатом), то есть заголовок был шире тела, а комментарий
// объявлял дефект принятым. Само состояние судят
// audience_unconditional_test.go и страж старта края.

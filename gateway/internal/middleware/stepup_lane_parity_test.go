...
func newLaneRig(t *testing.T) *laneRig {
	...
	it := middleware.NewAuthInterceptor(
		...
		// Читатель НАШЕЙ сессии (Ф3). В бою провязывается РЯДОМ с поставщиком,
		// когда профиль назвал обе стороны носителя: корень заводит их двумя
		// независимыми ветками по `config.SessionCarrierSet`, и состояний три —
		// только чужой · оба · только наш.
		//
		// Прежде здесь стояло «ВМЕСТО поставщика», и это перестало быть правдой
		// с изменением, заведшим состояние «оба». Здесь оба провязаны потому,
		// что сравниваются ПОЛОСЫ, а не сборки: у каждой свой носитель, и на
		// чужом носителе полоса молчит.

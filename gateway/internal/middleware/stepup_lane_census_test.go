...
func TestIdentityLaneCensus_PredicateSeparatesLanesFromTheirNeighbours(t *testing.T) {
	...
	require.Contains(t, c.lanes, "tryOwnSession",
		"положительный контроль: полоса НАШЕЙ сессии устанавливает личность и обязана "+
			"выводиться (прежде контролем стояла полоса чужого поставщика, снятая вместе "+
			"с ним — контроль на снятом предмете зеленел бы на пустом выводе)")

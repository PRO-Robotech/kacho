...
//
// Гейт опознаёт ОДНУ форму записи: `switch <что-то>.outcome`. Мимо него проходят
// другие записи ТОГО ЖЕ решения, и вот они поимённо:
//
//   - разбор без тега: `switch { case d.outcome == outcomeAllow: … }`;
//   - разбор с присваиванием в теге: `switch o := d.outcome; o { … }` —
//     тег здесь идентификатор, а не выбор поля;
//   - цепочка `if … else if` по тем же константам;
//   - разбор по значению, положенному в промежуточную переменную выше.
//
// Все четыре сегодня в дереве отсутствуют — гейт покрывает ВСЕ существующие
// разборы, их четыре, и это напечатано переписью. Но граница не в том, чего нет
// сегодня: переписав любой из четырёх разборов в любую из этих форм, автор
// выведет его из-под гейта МОЛЧА — перепись покажет три вместо четырёх, а
// упадёт гейт только если разборов станет ноль.
//
// Чем это закрывать, когда понадобится: сверять число найденных разборов с
// числом функций, ветвящихся по исходу, а не принимать найденное за всё
// существующее. Заводить это сейчас — гейт ради формы, которой в дереве нет.
package middleware

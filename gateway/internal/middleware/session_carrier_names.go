...
//
// # ИМЯ ОДНО, И У НЕГО ЕСТЬ ЧИТАТЕЛЬ
//
//   - [OurSessionCarrierName] — печенье НАШЕЙ сессии (Ф3 Р3); читает полоса
//     личности (`tryOwnSession`) и маршрут «кто я».
//
// Имён было два: рядом стояло печенье чужой службы личности. Его читатель снят
// вместе с самим поставщиком, а имя — ТЕМ ЖЕ изменением: имя, которое гасят и
// никто не читает, есть печенье, стёртое у клиента без основания, и ровно это
// гейт называет находкой.
//

...
type AuthzCounts struct {
	...
	// ── отказ БЕЗ ответа модели прав: один механизм, одна полоса ──────────────
	...
	// запрещён» это не значит.
	//
	// Стоит ОТДЕЛЬНО от шести полос допуска выше, а не среди них: те считают
	// пропущенных, эта — отвергнутых, и общего у них только «модель не
	// спрашивали». Слитая с Denied, она записала бы перебор внешней поверхности
	// в решения о правах; слитая с полосами допуска — в пропущенных.
	Unserved uint64

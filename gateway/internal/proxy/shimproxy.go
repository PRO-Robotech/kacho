...
func Handler(resolve methodResolverInternal) grpc.StreamHandler {
	return func(srv interface{}, ss grpc.ServerStream) error {
		...
		// Bidirectional copy.
		//
		// «Is this io.EOF?» is asked in three places below, and it is asked ONE
		// way — errors.Is, never ==. Against the grpc-go in this module the two
		// predicates are indistinguishable: it hands back the bare sentinel, and
		// that is asserted rather than quoted, by
		// TestGrpcGoStillHandsTheSendHalfABareEOF. They part company only if
		// something wraps it — a future grpc-go, or a stream interceptor on a
		// backend connection. Neither exists today: the tree declares no stream
		// client interceptor at all. errors.Is is chosen because, between two
		// predicates that agree on every input this tree can produce, the wider
		// one cannot be wrong in the direction that hurts — identity would miss
		// a wrapped signal and report it as the RPC's error, which is exactly
		// the defect the send-half branch below exists to prevent, silently
		// reintroduced.
		//
		// Nor can the wider predicate swallow a real failure, and that is a
		// property of the type rather than a hope: a status error answers
		// errors.Is only for another status error and offers no Unwrap, so an
		// error this side generated never matches io.EOF.
		...
		go func() {
			...
			for {
				...
				if err := clientStream.SendMsg(f); err != nil {
					// io.EOF here is NOT a status. grpc-go returns it from
					// SendMsg to say only «the backend already finished this
					// stream»; the status that finished it is delivered to the
					// receive half by RecvMsg, and status.Code(io.EOF) is
					// Unknown. Reporting it would answer the caller with a code
					// no backend ever sent — and only when this half happens to
					// observe the finished stream before the other one does,
					// which is why it surfaces as an occasional wrong code
					// rather than a constant one. Stand down and let the receive
					// half report; it is the only authority on the backend's
					// status. Anything else SendMsg returns was generated by
					// this side, carries a real status, and still travels.
					if errors.Is(err, io.EOF) {
						errCh <- nil
						return
					}

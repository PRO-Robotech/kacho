...
//
// Служба гасит только своё имя, и без дополнения ответ выхода, который
// предлагает форма, не нёс бы гашения чужого имени вовсе — хотя в переходном
// состоянии оно читается. Обработчик `/oauth/logout` гашение по всему перечню
// выдаёт, но выход формы идёт не через него. Дополнение выравнивает оба
// выхода: каждый выдаёт гашение по всему перечню одной формой.
//
// # Чего дополнение НЕ расширяет
//
// Того, что гашение края закрывает. Оно идёт без `Domain` и чужое печенье
// закрывает только там, где поставщик выдаёт его тоже без `Domain`. На
// цепочке, где поставщик ставит `Domain`, переходное состояние не
// объявляется — предусловие и его предикат снятия названы в шапке
...
func endEveryCarrierOnLogout(resp *http.Response) error {
	...
	// ИСХОД ВЫХОДА КЛАССИФИЦИРУЕТСЯ ТРЕМЯ ВЕТВЯМИ, И КОРЗИНЫ «ПРОЧЕЕ» НЕТ.
	//
	// Прежняя редакция делила исходы надвое: 2xx дополняем, всё остальное нет.
	// «Остальное» не было ни пустым, ни экзотическим — ПЕРЕНАПРАВЛЕНИЕ есть
	// обычная форма успеха браузерного выхода, и оно попадало в ветку «ничего
	// не гасим», то есть в сторону «оставить вошедшим».
	switch logoutOutcomeOf(resp.StatusCode) {
	case logoutRefused:
		// Исход судит СЛУЖБА по записи: на её отказе гашение означало бы, что
		// человек выброшен ровно тогда, когда она сказала, что не выбрасывала.
		return nil
	case logoutPerformed, logoutUnrecognised:
		// Дополняем. Для нераспознанного это РЕШЕНИЕ, а не умолчание: форму
		// ответа чужого слушателя отсюда не измерить, и «не знаю» обязано вести
		// к состоянию, из которого человек может войти заново, а не к
		// состоянию, в котором он считается вошедшим.
	}
...
// logoutOutcome — исход глагола выхода. Значений ТРИ, и третье названо: оно не
// «прочее», а «не распознан», и решение по нему принято явно.
type logoutOutcome int

const (
	// logoutPerformed — служба выход выполнила: успех либо перенаправление,
	// которым браузерный выход обычно и завершается.
	logoutPerformed logoutOutcome = iota
	// logoutRefused — служба выход ОТВЕРГЛА и сказала это своим ответом.
	logoutRefused
	// logoutUnrecognised — код, не попавший ни в один из двух классов.
	logoutUnrecognised
)

// logoutOutcomeOf классифицирует код ответа службы.
//
// Границы названы кодами, а не «успехом вообще»: 1xx исходом выхода не
// является — он о нём ничего не говорит, — и потому попадает в нераспознанное,
// а не в успех.
func logoutOutcomeOf(code int) logoutOutcome {
	switch {
	case code >= 200 && code <= 299:
		return logoutPerformed
	case code >= 300 && code <= 399:
		// Перенаправление — обычная форма успеха браузерного выхода: служба
		// отвечает им и ведёт человека на страницу входа.
		return logoutPerformed
	case code >= 400 && code <= 599:
		return logoutRefused
	}
	return logoutUnrecognised
}

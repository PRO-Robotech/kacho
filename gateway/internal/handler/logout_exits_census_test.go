...
// logout_exits_census_test.go — ОТВЕТ КАЖДОГО ВЫХОДА, КОТОРЫЙ ПРЕДЛАГАЕТ
// ПРОДУКТ, НЕСЁТ ГАШЕНИЕ ПО ОБОИМ ИМЕНАМ, а не ответ одного обработчика.
//
// Судится ответ, а не хранилище печений браузера: совпадёт ли гашение края
// (оно идёт без `Domain`) с печеньем, решает `Domain` выдачи, и предусловие,
// которое из этого следует, названо в шапке `middleware/session_carrier_names.go`.
//

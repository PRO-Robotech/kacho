...
//
// Здесь стоял более узкий страж (`render_reflection_test.go`), запрещавший ОДНО
// имя — переменную отзыва отражения схемы. Его предмет поглощён этим: отражение
// подавалось ИМЕННО на внутреннем листенере, и запрет одного имени на чарте, где
// листенера нет вовсе, утверждал бы меньше, чем есть. Страж заменён, а не
// ослаблен: запрещается теперь весь префикс, включая прежнее имя.
//

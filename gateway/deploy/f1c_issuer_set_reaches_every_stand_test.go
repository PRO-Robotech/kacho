...
// f1c_issuer_set_reaches_every_stand_test.go — НИ ОДИН профиль, называющий край,
// не оставляет перечень принимаемых издателей невыведенным из домена.
//
// # Чего не хватало, пока этого файла не было
//
// Сосед (f1b_token_acceptance_declared_test.go) спрашивает «поднимется ли
// процесс с тем, что объявил профиль». На профиле, не объявившем перечень,
// процесс поднимается — поэтому сосед ПЕЧАТАЕТ величину «остаются на прежнем
// скалярном пине N» и вердикта о ней не выносит. Измерение, напечатанное без
// вердикта, читается как «всё в порядке»: его зелёное выглядит как «все профили
// объявили», хотя означает «все профили поднимутся».
//
// Три соседних гейта молчат на необъявивших ПО ПОСТРОЕНИЮ: оба гейта авторитета
// отзыва входят в предмет только через объявленного `platformIssuer`, а гейт
// адресата края судит `tokenAudience`, ручку другого предмета.
//
// # Чем это опасно — и почему опасность тише, чем отказ
//
// При пустом перечне И пустом скалярном пине край адрес чужого издателя не
// требует, а ВЫВОДИТ: `https://hydra.` + домен установки, набор ключей
// `+ /.well-known/jwks.json`. Отказа старта на этом пути нет. Край поднимается,
// под становится готовым, объявляет ОДНУ запись приёма на хост, которого на
// стенде нет, и отвергает каждый токен при первом же запросе. Ни отказа старта,
// ни красного — одно сплошное 401, неотличимое для арендатора от «у меня
...
//	    чарта плюс цепочка `-f`), даёт записи приёма, ОБЪЯВЛЕННЫЕ перечнем, а не
//	    выведенные из домена;
...
// TestF1c_EveryStandAcceptsADeclaredIssuerSetNotADerivedOne — каждый стенд
// таблицы принимает издателей, ОБЪЯВЛЕННЫХ перечнем.
//
// Вердикт выносит `config.Config.TokenAcceptance` — тот же предикат, который
// исполняет процесс при старте, и `SourceKnob` каждой записи называет ту
// настройку, ИЗ КОТОРОЙ приехал адрес. Запись, приехавшая прежним скалярным
// пином, означает, что перечень не объявлен, — а при пустом пине адрес ещё и
// выведен из домена.
func TestF1c_EveryStandAcceptsADeclaredIssuerSetNotADerivedOne(t *testing.T) {
	...
	for _, name := range names {
		...
		if !f1cDeclares(gw) {
			t.Errorf("стенд %s (цепочка %v) НЕ ОБЪЯВЛЯЕТ перечень принимаемых издателей.\n\n"+
				"Отказа старта на этом пути нет, и в этом вся беда: край поднимется, под станет "+
				"готовым, и записи приёма он построит САМ — при пустом скалярном пине выведя "+
				"адрес чужого издателя из домена установки (https://hydra.<домен>). Хоста с "+
				"таким именем на стенде нет, поэтому набор ключей не приедет НИКОГДА, и край "+
				"отвергнет каждый предъявленный токен при первом же запросе. Для арендатора "+
				"это 401, неотличимый от неверных учётных данных; для оператора — зелёный "+
				"выкат и готовый под.\n"+
				"Что построилось сейчас: %v", name, chain, f1cRecords(bindings))
			continue
		}
		for _, b := range bindings {
			if b.SourceKnob != "KACHO_API_GATEWAY_TOKEN_ISSUER_KEYSETS" {
				t.Errorf("стенд %s: адрес набора ключей издателя %q приехал настройкой %s, "+
					"а не объявленным перечнем — перечень объявлен, а запись строится мимо него",
					name, b.Issuer, b.SourceKnob)
			}
		}
...
// f1cRecords — записи приёма в читаемом виде. Называет и издателя, и источник:
// отказ, назвавший одного издателя, не говорит, откуда приехал его адрес.
func f1cRecords(bindings []config.TokenIssuerBinding) []string {
	out := make([]string, 0, len(bindings))
	for _, b := range bindings {
		out = append(out, fmt.Sprintf("%s ← %s [%s]", b.Issuer, b.KeySetURL, b.SourceKnob))
	}
	return out
}
...
func TestF1c_EveryProfileNamingTheEdgeDeclaresTheIssuerSetOrInheritsItFromBelow(t *testing.T) {
	...
	for _, profile := range profiles {
			...
			for _, stack := range naming {
				...
				if !found {
					...
					t.Errorf("профиль %s называет край, перечень издателей не объявляет, и в "+
						...
						"Тогда край этого стенда выведет адрес чужого издателя из домена "+
						"установки, поднимется на нём и отвергнет каждый токен при первом же "+
						"запросе — без отказа старта и без красного.",
		...
		t.Errorf("профиль %s называет край, перечень издателей не объявляет и НИ ОДНОЙ "+
			"цепочкой не назван — значит, применяется как есть, и край на нём выведет адрес "+
			"чужого издателя из домена установки.\n\n"+

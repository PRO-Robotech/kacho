...
//
// # ПРЕДПОСЫЛКА ВЕРНА ОТНОСИТЕЛЬНО ПОПУЛЯЦИИ, и популяция расширилась
//
// Гейт писался на пяти владельцах, у каждого из которых каталог лежит под
// `services/`, и предпосылка «потолок владельца объявлен чартом ЭТОГО дерева»
// была верна — потому и невидима. Шестым владельцем объявлена служба доступа,
// чьи исходники вынесены в свой репозиторий: каталога у неё здесь нет и быть не
// может, а её чарт и проба потолка живут в её дереве.
//
// Без разреза популяции она стала бы нарушителем, НИЧЕГО НЕ НАРУШИВ, — тот же
// класс, что уже стоил этому дереву ложного красного на трёх проверках, чья
// предпосылка срабатывает раньше первого рендера. Поэтому владелец из другой
// стороны не судится и НЕ ЗАСЧИТЫВАЕТСЯ В ОСМОТРЕННОЕ: перепись печатает обе
// величины порознь.
//
// # Что этот разрез НЕ закрывает — сказано прямо
//
// Арифметика у такого владельца не выполняется НИГДЕ: произведение объявляет
// край (это дерево), потолок — владелец (то), и ни одно дерево не видит обеих
// сторон. Гейт называет остаток, а не закрывает его; закрыть его может только
// проверка, читающая обе стороны, — то есть решение о том, чьё это дерево.
func TestReplicaFanoutFitsUnderTheOwnerCeiling(t *testing.T) {
	...
	// Популяция делится ДО переписи: владелец, чьи исходники живут в другом
	// репозитории, не судится здесь и не засчитывается в осмотренное.
	judged, elsewhere := make([]string, 0, len(owners)), make([]string, 0, 1)
	for _, owner := range owners {
		if repo, external := ownerSourcedElsewhere(owner); external {
			elsewhere = append(elsewhere, owner+" ("+repo+")")
			continue
		}
		judged = append(judged, owner)
	}

	product := values.Replicas * values.SubscriptionStream.MaxStreams
	t.Logf("перепись: владельцев объявлено %d %v · судится здесь %d %v · "+
		"исходники в другом репозитории %d %v · реплик %d · потолок реплики %d · "+
		"произведение %d", len(owners), owners, len(judged), judged,
		len(elsewhere), elsewhere, values.Replicas,
	...
	if len(elsewhere) > 0 {
		// Третья категория названа ВСЛУХ и в успех не засчитывается: о владельцах
		// ниже гейт не вынес вердикта вовсе, и молчание о них читалось бы как
		// «осмотрели и сошлось».
		t.Logf("НЕ СУДИЛОСЬ: у владельцев %v потолок объявлен в ИХ репозитории, и "+
			"арифметика «реплики × потолок» здесь не выполнена ни разу — это остаток, "+
			"а не проход", elsewhere)
	}
	if len(owners) == 0 {
	...
	if len(judged) == 0 {
		// ВАКУУМНЫЙ ЗЕЛЁНЫЙ, закрытый явно.
		//
		// Владельцы объявлены, а судить оказалось некого: разрез популяции забрал
		// её целиком. Молчание здесь неотличимо от исправной работы — гейт обошёл
		// бы ноль владельцев и вышел зелёным, ровно как если бы всё сошлось.
		//
		// Отдельной ветвью, а не общей с пустым перечнем выше, потому что предметы
		// РАЗНЫЕ: там оператор ничего не объявил (его право), здесь объявленное
		// исчезло по дороге — либо ведомость вынесенных частей стала тотальной,
		// либо у края не осталось ни одного владельца в этом дереве.
		t.Fatalf("владельцев объявлено %d %v, и ВСЕ отнесены к другому репозиторию %v: "+
			"арифметика «реплики × потолок» не выполнена НИ РАЗУ, и зелёное здесь было бы "+
			"вакуумным. Оба объяснения требуют решения, а не молчания",
			len(owners), owners, elsewhere)
	}

	for _, owner := range judged {
...
// ownerSourcedElsewhere — исходники владельца живут в ДРУГОМ репозитории.
//
// Возвращает репозиторий-источник и признак.
//
// # Почему ведомость ОБЩАЯ, а не второй словарь рядом
//
// Сторона части продукта объявлена в дереве один раз
// (`productnaming.ExternallySourcedServices`), и второе объявление того же
// предмета разошлось бы с первым молча — причём разошлось бы в сторону «потолок
// не найден» у владельца, который ничего не нарушал. Тем же словарём судит
// сосед по каталогу (subscription_owner_naming_test.go), и это намеренно: у
// вопроса «чьё это дерево» один ответ на пакет.
//
// # Чем это НЕ является
//
// Не послаблением: владелец не исключается из наблюдения, он переносится в
// ТРЕТЬЮ категорию, которая печатается переписью и в проход не засчитывается.
// Самоистечение держит не эта функция, а
// `productnaming.TestExternallySourcedLedgerExpiresOnItsOwn`: вернутся исходники
// в дерево — запись станет находкой ТАМ, и владелец вернётся в судимую
// популяцию здесь сам, ничьей памяти не потребовав.
func ownerSourcedElsewhere(owner string) (string, bool) {
	dir, ours := productnaming.ServiceDir(productnaming.ChartName(owner))
	if !ours {
		// Имя не распознано как часть продукта — судить его тут нечем, и
		// «в другом репозитории» было бы утверждением, которого мы не делали.
		return "", false
	}
	repo, external := productnaming.ExternallySourcedServices()[dir]

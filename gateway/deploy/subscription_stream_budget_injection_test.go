...
// ─────────────────────────────────────────────────────────────────────────────
// РАЗРЕЗ ПОПУЛЯЦИИ РАЗЛИЧАЕТ, А НЕ СЪЕДАЕТ ЕЁ ЦЕЛИКОМ
//
// `ownerSourcedElsewhere` выводит владельца из-под суждения о потолке. Ошибись он
// в сторону «да» — гейт перестал бы судить кого бы то ни было и остался бы
// ЗЕЛЁНЫМ: вакуумный проход, неотличимый от исправной работы. Поэтому
// утверждений три, и каждое про свой вход.
//
// Проверяется НАСТОЯЩАЯ ведомость, а не синтетическая: предмет разреза — факт о
// дереве («чьи это исходники»), и подмена ведомости доказывала бы свойство копии.
// Отсюда самоистечение: вернутся исходники службы в это дерево — первое
// утверждение покраснеет и скажет, что разрез больше не нужен.

// TestOwnerSourceSplitNamesTheForeignOwner — владелец, вынесенный в свой
// репозиторий, опознаётся, и репозиторий назван.
func TestOwnerSourceSplitNamesTheForeignOwner(t *testing.T) {
	repo, external := ownerSourcedElsewhere("iam")
	if !external {
		t.Errorf("владелец %q не опознан как вынесенный, хотя ведомость "+
			"productnaming.ExternallySourcedServices его объявляет. Если исходники службы "+
			"ВЕРНУЛИСЬ в это дерево — разрез популяции больше не нужен: снимите его вместе "+
			"с этим утверждением, и потолок владельца снова станет судимым здесь", "iam")
	}
	if repo == "" {
		t.Error("владелец опознан вынесенным, а репозиторий-источник не назван: находка " +
			"сказала бы «судить не будем» и не сказала бы ГДЕ судится — читателю некуда идти")
	}
}

// TestOwnerSourceSplitIsSilentOnAPlatformOwner — ЗАКОННЫЙ БЛИЗНЕЦ: владелец
// этого дерева из-под суждения не выводится.
//
// Без этого утверждения молчание гейта на вынесенном владельце было бы
// неотличимо от молчания разреза, отвечающего «да» всем подряд.
func TestOwnerSourceSplitIsSilentOnAPlatformOwner(t *testing.T) {
	for _, owner := range []string{"vpc", "compute", "storage", "registry", "loadbalancer"} {
		if repo, external := ownerSourcedElsewhere(owner); external {
			t.Errorf("владелец %q отнесён к чужому репозиторию (%s), а его исходники лежат "+
				"в этом дереве: разрез забирает популяцию у гейта, и потолок такого "+
				"владельца перестаёт сверяться молча", owner, repo)
		}
	}
}

// TestOwnerSourceSplitDoesNotSwallowAnUnknownName — имя, не распознанное как
// часть продукта, вынесенным НЕ объявляется.
//
// Иначе опечатка в перечне владельцев уходила бы из-под проверки потолка вместо
// того, чтобы дать названную находку: «потолок не найден» — отказ верный, а
// «судить не будем» — тишина.
func TestOwnerSourceSplitDoesNotSwallowAnUnknownName(t *testing.T) {
	for _, name := range []string{"no-such-domain-xyz", "", "nlb"} {
		if repo, external := ownerSourcedElsewhere(name); external {
			t.Errorf("имя %q объявлено вынесенным (%s), хотя частью продукта не "+
				"распознаётся: такое имя обязано дойти до проверки потолка и получить "+
				"названный отказ, а не выпасть из популяции", name, repo)
		}
	}
}

...
func TestRollbackPredicates_RecogniseTheRealTree(t *testing.T) {
	...
	if len(f.noDown) == 0 {
		t.Errorf("миграций без секции отката не найдено ни одной — либо признак %q "+
			"перестал узнаваться, либо предмет ведомости исчез целиком", gooseDown)
	}
	if len(f.byDrop) == 0 {
		t.Errorf("ни одна миграция без секции отката не объявлена снятиями dropguard — " +
			"разбор манифеста ослеп, и ведомость приняла бы на себя чужой предмет")
	}
	if len(f.ledger) == 0 {
		t.Errorf("ведомость %s не прочитана — форма строки узнаётся неверно", rollbackLedger)
	}

...
var recordedIdentityDevConcessions = map[string]string{
	...
	// Две записи ниже появились не потому, что правило ослабили, а потому, что
	// эти два стенда впервые попали под него. Их цепочки объявляли на слой
	// меньше, чем поднимает рецепт, — без среднего слоя посадка читалась как
	// «объявлен режим разработки», и правило («только объявившие боевую
	// посадку») к ним не относилось. Цепочки приведены к тому, чем стенды
	// поднимают на самом деле, и оба оказались боевыми.
	"prorobotech/kratos.kratos.development": "накладка публичных образов не объявляет ничего о " +
		"безопасности и наследует посадку среднего слоя целиком — включая его транспорт: " +
		"тот же HTTP без терминации TLS, что у dev-prod, и та же привязка Secure к ручке. " +
		"Своего значения профиль не объявляет вовсе. Условие снятия — общее с dev-prod: " +
		"появление HTTPS у стенда ЛИБО перевод входа на путь, не зависящий от cookie браузера; " +
		"снятое там, оно снимается и здесь, и обе записи обязаны уйти вместе",

	"a8f60d/kratos.kratos.development": "профиль управляемого кластера объявляет размеры, " +
		"образы и отсутствие постоянных томов — и ничего о транспорте: контроллера ingress " +
		"в кластере нет ни одного, внешний доступ идёт Service'ом типа LoadBalancer, TLS не " +
		"терминируется. Посадка наследуется у среднего слоя целиком, значение ручки — вместе " +
		"с ней. Условие снятия то же, что у dev-prod и prorobotech",
}

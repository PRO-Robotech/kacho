...
#
# ЧЕМ ЭТО ПРОВЕРЯЮТ НА ЖИВОМ КЛАСТЕРЕ — И ЧЕМ НЕЛЬЗЯ (замер 2026-08-10, kind,
# helm v4.2.3, стенд с уже записанной заданием аннотацией, то есть ровно в том
# состоянии, где конфликт и возникает):
#
#   * `helm upgrade --dry-run=server` — НЕ ГОДИТСЯ: на восстановленном дефекте
#     он вышел с кодом 0. То есть проба, построенная на нём, не может упасть, и
#     «зелёный dry-run» не является свидетельством ни о чём. Числа: с инъекцией
#     exit=0, без инъекции exit=0 — исход не зависит от предмета.
#   * `kubectl apply --server-side --field-manager=helm --dry-run=server` —
#     ГОДИТСЯ и КЛАСТЕР НЕ МЕНЯЕТ: на инъекции даёт ровно ту ошибку, что стоит в
#     kacho#3 («Apply failed with 1 conflict: conflict with "kubectl-patch" …»),
#     без инъекции — `serverside-applied (server dry run)`.
#   * настоящий `helm upgrade` (без dry-run) — годится и проверен: revision 60,
#     exit 0, у поля остался один писатель.
#
# Разница в том, что дефект живёт во ВЛАДЕНИИ полем, а не в тексте манифеста:
# рендер его не содержит by construction, поэтому и `helm template`, и всё, что
# на нём стоит, остаются зелёными. Эта проверка ловит его по дереву — пересечением
# двух множеств, — а на кластере его ловит только настоящее применение либо

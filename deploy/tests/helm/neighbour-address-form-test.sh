...
CENSUS="$DEPLOY_ROOT/tests/helm/admin-hop-address-census-test.sh"
...
note() { echo "    · $1"; }
refuse() { echo "REFUSE: $SCRIPT — $1"; exit 2; }
...
python3 -c 'import yaml' 2>/dev/null || refuse "нет python3-yaml — рендер нечем разобрать документно"
...
if [ "${1:-}" = "--self-test" ]; then
  ...
  probe() {
    ...
    if [ "$rc" -ne "$want" ]; then
      ...
      printf '%s\n' "$out" | sed 's/^/      /'
      st_rc=1
    elif [ -n "$grep_for" ] && ! printf '%s' "$out" | grep -q -- "$grep_for"; then
...
[ -d "$UMBRELLA" ] || refuse "нет чарта умбреллы: $UMBRELLA"
[ -f "$CENSUS" ]   || refuse "нет $CENSUS — таблицу стеков неоткуда прочитать"
command -v helm >/dev/null 2>&1 || refuse "нет helm — рендер собрать нечем"
[ -d "$UMBRELLA/charts" ] || refuse "зависимости чарта не материализованы (helm dep update) — обход увидел бы часть стека и отчитался полнотой"

# ── УСТАРЕВШИЙ АРХИВ — ОТКАЗ, А НЕ ВЕРДИКТ ───────────────────────────────────
#
# Локальные зависимости приезжают в рендер АРХИВОМ `charts/<имя>-<версия>.tgz`,
# собранным из каталога сервиса. Правка исходника чарта в архив НЕ ПОПАДАЕТ, пока
# его не пересоберут, — поэтому рендер показывает ПРОШЛОЕ состояние дерева.
#
# Это не гипотеза: ровно на этом гейт при разработке дважды выдал находки по
# файлам, которые были уже исправлены, а затем — что опаснее — мог бы выдать
# ЗЕЛЁНОЕ по архиву, собранному до внесения дефекта. Вердикт о дереве, вынесенный
# по его снимку недельной давности, — это «не выполнилось», а не «чисто».
#
# В CI предпосылка выполняется сама (свежий checkout → `helm dep update` в цели
# `helm-manifest-test` собирает архивы ПОСЛЕ выкладки исходников), поэтому
# проверка там молчит и ничего не стоит.
stale=""
...
if [ -n "$stale" ]; then
  echo "REFUSE: $SCRIPT — исходник чарта НОВЕЕ собранного архива:$stale"
  echo "        Рендер показал бы ПРОШЛОЕ состояние дерева, и вердикт относился бы"
  echo "        не к тому, что лежит в исходниках. Это «не выполнилось», а не «чисто»."
  echo "        Пересоберите зависимости:  (cd $UMBRELLA && helm dep update)"
  exit 2
fi

# ТАБЛИЦА СТЕКОВ ЧИТАЕТСЯ ИЗ СОСЕДНЕЙ ПРОВЕРКИ, А НЕ ДЕРЖИТСЯ КОПИЕЙ: два места
# об одном предмете разъезжаются молча, и разъедутся они там, где это не видно.
STACKS="$(sed -n "/^STACKS='/,/'$/p" "$CENSUS" | sed "s/^STACKS='//; s/'$//" | grep -E '^[a-z0-9-]+:')"
stack_n="$(printf '%s\n' "$STACKS" | grep -c . || true)"
assertion
if [ "${stack_n:-0}" -lt 1 ]; then
  refuse "таблица стеков не разобрана из $(basename "$CENSUS") — обходить нечего"
fi
ok "таблица стеков прочитана из $(basename "$CENSUS"): $stack_n стеков"
...
while IFS= read -r line; do
  ...
  if ! (cd "$UMBRELLA" && helm template kacho-umbrella . $args --namespace kacho) \
        >"$render" 2>"$work/$stack.err"; then
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    fail "[$stack] адресов дефектной формы: ${n:-?} (документов $d, строк $s)"
  else
    ok "[$stack] чисто: 0 находок (документов $d, строк $s)"
  fi
...
if [ "$rendered" -lt 1 ]; then
  refuse "не собрался НИ ОДИН рендер — «ноль находок» здесь означает «ноль прочитанного»"
fi
if [ "$tot_docs" -lt 1 ]; then
  refuse "прочитано 0 документов — обходить было нечего"
fi
ok "объём половины 1: стеков $rendered/$stack_n, документов $tot_docs, строк $tot_strings"
...
if [ "${allow_n:-0}" -lt 1 ]; then
  ...
  while IFS= read -r nm; do
    [ -z "$nm" ] && continue
    (cd "$REPO_ROOT" && git grep -q -F -- "$nm" >/dev/null 2>&1)
    case $? in
      0) : ;;
      1) dead="$dead $nm" ;;
      *) refuse "обход дерева за предметом исключения не выполнился (git grep, код $?) — вердикт об исключениях НЕ ВЫНЕСЕН" ;;
    esac
  done <<<"$allow_names"
  if [ -n "$dead" ]; then
    fail "записям законных полных форм нечего исключать в дереве:$dead"
    note "исключение без предмета унаследует следующую слепую зону — снять его"
  else
    ok "все записи законных полных форм ($allow_n) имеют предмет в отслеживаемом дереве"
  fi
fi
...
cd "$REPO_ROOT" || refuse "не перейти в корень репозитория"
...
assertion
if [ "${go_n:-0}" -lt 1 ]; then
  refuse "git ls-files не вернул ни одного не-тестового .go — половина 2 НЕ ВЫПОЛНЕНА"
fi
...
ok "объём половины 2: прочитано не-тестовых .go — $go_n файлов"
assertion
if [ "${bad_n:-0}" -gt 0 ]; then
  printf '%s\n' "$bad" | sed 's/^/      /'
  fail "вкомпилированных умолчаний дефектной формы: $bad_n"
  ...
  ok "вкомпилированных умолчаний дефектной формы: 0"
fi

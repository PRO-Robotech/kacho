...
if [ "${stack_n:-0}" -lt 1 ]; then
  refuse "таблица стеков не разобрана — обходить нечего"
fi
ok "таблица стеков прочитана из deploy/stacks.txt: $stack_n стеков"
...
if [ -z "$allow_block" ]; then
  refuse "блок ALLOWED не найден в классификаторе — «записей нет» и «блок не прочитан» неразличимы; вердикт об исключениях НЕ ВЫНЕСЕН"
fi

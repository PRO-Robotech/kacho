...
# Состав каждого стека — из единственной таблицы дерева. Слой посадки боевого
# набора объявляет отладочный флаг соседа явно, тогда как без него стек попадал
# под ветку «умолчание чарта»: классификация от этого не меняется, но меряется
# объявление боевой цепочки, а не умолчание. Ровно поэтому таблица обязана быть
# одна — цепочка, знающая на слой меньше, меняет ответ, не покраснев.
STACKS="$(stacks_table)"

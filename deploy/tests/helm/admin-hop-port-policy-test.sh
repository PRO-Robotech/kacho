...
' $vfiles)"
      if [ -z "$term" ]; then
        ...
        # Список ЗАМЕЩАЕТСЯ целиком (helm сливает карты, но заменяет списки),
        # поэтому умолчания обязаны быть перечислены заново — иначе публичный
        # листенер потеряет свои источники молча.
        lost=""
        IFS=','; for cidr in $DEFAULT_TERMINATION; do
          grep -q "$(printf '%s' "$cidr" | sed 's/[.[\*^$/]/\\&/g')" <<<"$term" || lost="$lost $cidr"
        done; unset IFS
        if [ -n "$lost" ]; then
          fail "[$stack] петля добавлена, но умолчания чарта ПОТЕРЯНЫ:$lost"
          note "список замещается целиком — публичный листенер лишится своих источников."
        else
          ok "[$stack] боевой класс: петля объявлена В ДОПОЛНЕНИЕ к умолчаниям ([$term])"
        fi
      fi

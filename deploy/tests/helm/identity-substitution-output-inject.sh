...
# КОД ЧИТАТЕЛЯ ЦЕПОЧКИ ПОТРЕБОВАН ОТДЕЛЬНО ОТ КОДА `helm`. Внутри аргумента
# подстановка роняет свой код, и отказ читателя приписывался бы РЕНДЕРУ — тогда
# как здесь отказ рендера объявлен находкой о дереве (см. абзац выше). Разбор
# цены — в шапке `stacks.sh`.

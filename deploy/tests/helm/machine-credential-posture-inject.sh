...
# contour <каталог> <контур переведён: true|false> — СИНТЕТИЧЕСКИЙ профиль с
# включённой ручкой связанности. Единственный профиль каталога.
#
# ПОЧЕМУ СИНТЕТИКА, А НЕ ПРОФИЛЬ ДЕРЕВА. Пара (2)/(3) судит ОТНОШЕНИЕ «ручка ×
# переведённость контура», а не состояние какого-то профиля. Прежде близнец (3)
# ставил ручку на живой values.dev.yaml как на непереведённый контур — и
# покраснел, когда cc25c579f0c перевёл контур и там (#2735): переведены стали
# все стенды, непереведённого профиля в дереве не осталось, и доказательство
# потеряло свой вход вместе с предметом, которого не касалось. Синтетический
# профиль от перевода стендов не зависит; инъекция и близнец отличаются в нём
# РОВНО одним фактом — переведённостью контура. Что проба читает профили дерева
# и называет виновника по имени, держат (1), (4) и (6).
contour() {
  mkdir -p "$1"
  cat > "$1/values.synthetic-contour.yaml" <<YAML
kaname:
  config:
    authn:
      clientToken:
...
#     и проба обязана молчать. От (2) отличается одним фактом.
contour "$TMP/has-reader" false

...
# колонка kacho_iam.token_signing_keys.private_key_wrapped, поэтому повторный
...
SCRIPT="$(basename "$0")"
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"   # …/deploy
...
LEDGER=(
  "kacho-iam-hook-token|общий секрет двух читателей (iam ↔ Hydra token-hook); класс разбирается задачей #948, посадка PR #1061 — здесь чужой предмет не чинится. Запись снимается вместе с посадкой #1061."
)

fail() { echo "FAIL: $1"; exit 1; }

# census <каталог> — по строке на каждый вызов создания секрета:
...
verdict() {
  local dir="$1" out
  out="$(census "$dir")" || { echo "ОТКАЗ РАЗБОРА"; return; }
  python3 - "$out" "${LEDGER[@]}" <<'PY'
import sys
...
self_test() {
  ...
  # В каждой синтетической фикстуре ниже прощённый ведомостью секрет ОСТАВЛЕН
  # намеренно: без него ведомость теряет предмет, гейт справедливо краснеет на
  # ЭТО, и инъекция перестаёт отличаться от контроля ровно тем, что проверяет.
  # (Само это свойство проверяется отдельно — пробой (D).)
  #
...
INJ
  ...
  #     запись потеряла предмет и обязана быть снята.
  mkscripts "$tmp/d"
...
INJ
  out="$(verdict "$tmp/d" | grep -v '^#' || true)"
  if [[ "$out" == *"ИСКЛЮЧЕНИЕ ПОТЕРЯЛО ПРЕДМЕТ"* && "$out" == *"kacho-iam-hook-token"* ]]; then
...
command -v python3 >/dev/null || fail "нужен python3 (разбор посева)"
...
echo "PASS: $SCRIPT (перепись выше; находок 0)"

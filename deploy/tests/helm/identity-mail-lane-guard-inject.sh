...
# КОД ЧИТАТЕЛЯ ЦЕПОЧКИ ПОТРЕБОВАН: `set -e` здесь нет (см. шапку), а пустая
# цепочка при нулевом коде отдала бы `helm` без единого `-f`. Отказ тогда
# приписывался бы рендеру, а не таблице, — красное с ЛОЖНОЙ причиной. Разбор —
# в шапке `stacks.sh`.
...
# не размножен: копия у каждого из семи разошлась бы молча, и разошлась бы туда,
# где расхождение не ломает прогон, а делает его ВАКУУМНЫМ — то есть зелёным.
# Довод и замер — в шапке premise.sh.
...
assert_rel() { # релиз · имя · ожидание(RED|GREEN) · фраза-улика · --set …
  ...
  local got
  # ARGS это намеренно разбиваемая цепочка `-f`, и разбить её обязана оболочка.
  # ПОЯСНЕНИЕ СТОИТ ОТДЕЛЬНОЙ СТРОКОЙ, А НЕ В САМОЙ ДИРЕКТИВЕ (задача #1770):
  # разбор директив прозы после кода правила не допускает — он читает её как
  # продолжение списка правил и отвечает ошибкой РАЗБОРА (SC1072/SC1073).
  # Цена не в одной строке: файл выпадает из-под проверки ЦЕЛИКОМ, то есть
  # отключение, задуманное узким, оказывается полным — и это невидимо, вывод
  # выглядит как одна ошибка в одной строке.
...
# ДЕФЕКТ ПОДАЁТСЯ ВХОДОМ, А НЕ БЕРЁТСЯ ИЗ ДЕРЕВА. Прежде эти две оси не
# передавали `--set` вовсе: они опирались на ЛИТЕРАЛ профиля, который расходился
# с именем релиза сам. Литерала больше нет — профиль называет приёмник
# выражением, — и обе оси стали зелёными, то есть доказательство молча потеряло
# предмет (`testing.md` §«Гейт на класс», п. 9). Ветвь стража при этом жива:
# написанный от руки узел разойтись с релизом по-прежнему может, и подать такой
# узел — работа инъекции, а не дерева.
assert_rel kacho "полоса названа на ЧУЖОЙ приёмник"  RED "поднимает приёмник под именем" \
  --set 'global.kacho.identity.smtp.connectionURI=smtp://kacho-umbrella-mailpit:1025/'
...
# выше остались бы красными, а чинить было бы нечего.
assert_rel ci "полоса-выражение при чужом имени релиза" GREEN ""

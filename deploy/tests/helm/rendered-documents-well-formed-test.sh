...
# 2 условие не создано (плюс текст самого helm). Нехватка инструмента и отказ
# рендера объявлялись здесь кодом 1 — тем же, каким объявляется склеенный документ
# или задвоенная переменная, — и вызывающий не мог их различить машинно (#1214).
# Код 2 у этого гейта уже был, но только на одной ветке («таблица не прочиталась»).
# shellcheck source=deploy/tests/helm/outcome.sh
. "$HERE/outcome.sh"
require_helm
command -v python3 >/dev/null 2>&1 || fatal "нужен python3 — разбирать рендер нечем"
...
check_profile() {
  ...
  # а не свойство отрендеренных документов, о котором эта проверка написана.
  # Прежде он выходил кодом 1 и печатал первые пять строк ошибки helm в свой же
  # перечень находок — читатель шёл искать склеенный документ, которого нет.
  helm_try kacho-umbrella "$CHART" -n kacho "$@" "${IMG_ARG[@]}"
  render_or_fatal "профиль $label"
  printf '%s\n' "$HELM_OUT" >"$out"
...
[ "$profiles_checked" -ge 1 ] \
  || fatal "не проверено ни одного стека — таблица стеков не прочиталась"
...
if [ "$fail" -ne 0 ]; then
  ...
  fail "документы стеков не прошли проверку — перечень находок выше"
fi

outcome_verdict "стеков осмотрено: $profiles_checked"

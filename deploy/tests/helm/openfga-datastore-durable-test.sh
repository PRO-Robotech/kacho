...
# Состав и порядок наложения — из ЕДИНСТВЕННОЙ таблицы дерева (deploy/stacks.txt).
# Формат: <имя>|<файл> <файл> … — файлы накладываются слева направо, как в
# рецепте. Здесь стояла своя копия, и она знала о боевой цепочке на слой меньше
# соседних проверок: её вердикт относился к другому стенду, оставаясь зелёным.
mapfile -t PROFILES < <(stacks_table | tr ':,' '| ')

...
ЧЕГО НЕ УТВЕРЖДАЕТ (граница названа, чтобы «зелено» не читалось шире)
  ...
  · ничего о конфигурации, которую не читает ни один процесс. Такие карты
    настроек ПЕРЕСЧИТЫВАЮТСЯ отдельной строкой переписи и названы поимённо:
    «ноль находок» обязано быть отличимо от «ноль прочитанного», а
    несмонтированная конфигурация — предмет со своим номером, а не умолчание.
...
def main():
    ...
    # Карты настроек, объявляющие полосу, которую не читает ни один процесс.
    ...
    if declared_unread:
        print(
            "  вне области: конфигурации объявляют полосу, но их не читает ни один процесс — "
            + ", ".join(declared_unread)
            + " (предмет монтирования ведёт задача #904; здесь это ГРАНИЦА, а не умолчание)"
        )

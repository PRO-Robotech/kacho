...
#      отдаёт 0 либо 2, и при 2 — ТЕКСТ, который сказал сам helm. Ноль законен:
...
SYN
  ...
  echo
  echo "случаев проверено: $st_checked (законных 3, инъекций 5, предпосылка 1)"
  [ "$st_checked" -eq 9 ] || { echo "FAIL: исполнено $st_checked случаев из 9"; st_rc=1; }
...
for t in "$WORK/deploy/tests/helm/"*-test.sh; do
  ...
  if declares_contract "$t"; then
    ...
    case "$rc" in
      0) ;;
      2) helm_text_relayed "$out" || contract_bad="$contract_bad $b(нет текста helm)" ;;

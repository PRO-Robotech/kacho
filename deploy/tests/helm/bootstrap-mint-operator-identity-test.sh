...
# своего предмета.
# ПОЛОСА СНИМАЕТСЯ ВМЕСТЕ С ПРИЁМНИКОМ, А НЕ ОТДЕЛЬНО ОТ НЕГО. Выключить узел и
# оставить полосу, которая его называет, — это ровно то состояние, ради которого
# страж полосы и заведён: приёмник не поднимает никто, письма уходят в никуда
# молча. Страж отвергает такой рендер справедливо, и отказ приходит ЗДЕСЬ третьей

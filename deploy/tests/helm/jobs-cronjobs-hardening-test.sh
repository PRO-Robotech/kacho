...
# CronJob удалён как вестигиальный, и довод был НЕ «ключа подписи у платформы
# нет»: здесь стояло «издатель и подписант — Hydra; iam лишь проксирует её
# публичный JWKS», и это утверждение пережило свой предмет — ключница у
# платформы есть, и её набор публикуется второй записью того же слушателя.
#
# Настоящий довод: ротация живёт ВНУТРИ службы (`authn.token-signing`), а не
# отдельным заданием расписания, поэтому у задания нет предмета. Проверять
# PSS-floor больше не на чем — секция снята вместе с шаблоном.

...
ALLOWED_TREE=(
  ...
  "deploy/tests/helm/outcome-contract-inject.sh|инъекция контракта исходов: подставляет ДЕФЕКТНЫЙ адрес во временное зеркало, чтобы доказать, что гейт транспорта его находит. Развёрнутой нагрузке адреса не задаёт: зеркало снимается в том же прогоне, живой чарт не трогается. Убрать адрес отсюда значило бы ослабить доказательство — инъекция обязана подставлять настоящую форму дефекта"
)

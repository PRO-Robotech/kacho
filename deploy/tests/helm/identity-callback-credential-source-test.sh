...
for stack in $STACKS; do
  # КОД ЧИТАТЕЛЯ ЦЕПОЧКИ ПОТРЕБОВАН: потерянный отказ давал ПУСТУЮ цепочку при
  # нулевом коде, и `helm` шёл БЕЗ ЕДИНОГО `-f`. Разбор цены — в шапке
  # `stacks.sh` §«КОД ОТКАЗА ТАБЛИЦЫ ПОТРЕБОВАН КАЖДЫМ ПРОИЗВОДНЫМ»; здесь он не

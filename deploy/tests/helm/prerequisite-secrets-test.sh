...
self_test() {
  ...
  #
  # НОСИТЕЛЬ ОБЯЗАТЕЛЬНОЙ ССЫЛКИ — ПОД ИЗДАТЕЛЯ ПОСТАВЩИКА, ПОДНЯТЫЙ ВНУТРИ
  # РЕНДЕРА. На настоящей цепочке обязательных ссылок на секреты скрипта сегодня
  # НЕТ: их брал под издателя (величина обратного вызова, `kaname-hook-token`,
  # обязательной ссылкой с #948), а издатель не поднимается ни на одном стенде
  # (база зонта, #2735). Инъекции (A) тогда было бы не на чем краснеть, и
  # самопроверка доказывала бы ничто. Поэтому издателя поднимает сама
  # самопроверка — ОДНИМ фактом поверх настоящей цепочки dev-prod, чей профиль
  # его настройки объявляет; (0) и (A) судят ОДИН И ТОТ ЖЕ рендер и отличаются
  # ровно перечнем заводимого. Основной проход издателя НЕ поднимает: он судит
  # то, что стенд разворачивает на самом деле.
  # shellcheck source=deploy/tests/helm/provider-up.sh
  . "$HERE/provider-up.sh"
  # shellcheck disable=SC2086
  helm_try kacho-umbrella "$UMBRELLA" $ARGS_DEV_PROD "${ISSUER_UP_ARGS[@]}"
  render_or_fatal "стек dev-prod (самопроверка, издатель поставщика поднят пробой)"
  printf '%s\n' "$HELM_OUT" > "$render"

...
# Профили, которыми РЕАЛЬНО поднимается стенд (deploy/Makefile): dev-up рендерит
# values.dev.yaml; dev-prod-up накладывает values.dev-prod.yaml поверх него;
# values.prod.yaml — канонический боевой. `<имя>|<файлы через запятую>`.
PROFILES="
dev|values.dev.yaml

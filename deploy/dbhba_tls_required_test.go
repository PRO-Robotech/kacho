...
//
//   - Проверяются ВСЕ инстансы Postgres умбреллы (включая хранилища Ory) —
//     но ТОЛЬКО те, у кого `tls.enabled: true` в этом стеке.
...
// предмет — `sslmode` у НАШЕГО клиента, а строку соединения Ory держит она
// сама, решать за неё нечего.
//

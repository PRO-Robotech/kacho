...
//
// ПОСЛЕДНЕЕ ВЫСКАЗЫВАНИЕ ПОБЕЖДАЕТ — ровно так цепочку читает helm. Прежняя
// редакция искала `enabled: true` в ЛЮБОМ профиле и возвращала true на первом
// же совпадении: накладка, выключившая службу, предикатом НЕ ЧИТАЛАСЬ, и четыре
// гейта, открывающиеся этим условием, объявляли стенд поднимающим службу,
// которой на нём нет. Это не молчание о дефекте, это ЛОЖНАЯ ПЕРЕПИСЬ: число
// осмотренного росло бы на стенд, чьи настройки поставщика никогда не поедут.
//
// Класс наблюдался живьём при переводе стенда `own` (kacho#2735): накладка
// объявила `kratos.enabled: false`, слитые значения дали шесть поднимающих
// стендов из семи, а три гейта продолжали печатать семь. Держит доказательство
// инъекцией — ось «накладка, выключившая службу» в соседнем файле; на дереве
// сегодня обе редакции дают одно число, и поэтому предикат чинится ДО того, как
// разойдётся.
func identityChainRaisesIdentity(texts []string) bool {
	raises := false
	for _, t := range texts {
		if m := kratosEnabledDecl.FindStringSubmatch(t); m != nil {
			raises = m[1] == "true"
		}
	}
	return raises
...
// kratosEnabledDecl — ВЫСКАЗЫВАНИЕ профиля о флаге, вместе с его величиной.
// Ловить только `true` нельзя: профиль, выключивший службу, был бы неотличим
// от профиля, о ней промолчавшего.
var kratosEnabledDecl = regexp.MustCompile(`(?m)^kratos:\n(?:[ \t].*\n|\n)*?\s+enabled:\s*(true|false)`)

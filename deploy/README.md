...
- **Чужого стека личности на стенде НЕТ** (#2735, #2777). Посадку `own` обеим
  ...
  Что чем подписано, задаёт **посадка**: своя чеканка объявлена в профилях
  `values.dev-prod.yaml`, `values.prod.yaml`, `values.fe3455-prod.yaml` (перемерять
  `grep -A1 'tokenSigning:' helm/umbrella/values*.yaml`); на `values.dev.yaml` она
  выключена, и предъявителя подписи там не проходит никто — человек входит
  печеньем нашей полосы, а не токеном.

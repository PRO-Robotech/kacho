...
//
// MAIL-16 (identity_verification_mirrors_the_requirement_test.go) — ближайший
// сосед, и шов с ним проходит по ЗНАЧЕНИЮ: здесь находкой является само наличие
// второго объявления, каким бы оно ни было, там — противоречие СУММЫ значений
// («вход требует подтверждённого адреса, а поток подтверждения выключен»).
// Отсюда два состояния, в которых гейты расходятся, и оба реальны: согласное
// второе мнение — находка ЗДЕСЬ и молчание там; поток, выключенный в
// ЕДИНСТВЕННОМ объявлении, — находка ТАМ и молчание здесь, потому что второго
// места в этом состоянии нет by construction.
//

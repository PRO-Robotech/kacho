...
func TestShellReaderCacheIsPerTree(t *testing.T) {
	...
	a, b := ask("/tmp/дерево-A"), ask("/tmp/дерево-B")
	if a == "" || b == "" {
		t.Fatal("путь кеша прочитался пустым — сверять нечего, вердикт беспредметен")
	}
	if a == b {
		t.Errorf("две копии дерева дали ОДИН путь сборки (%s) — соседняя копия "+
			"перезапишет инструмент этой, и ответ придёт по чужой ведомости", a)
	}
	if again := ask("/tmp/дерево-A"); again != a {
		t.Errorf("путь для одного и того же дерева непостоянен (%s ≠ %s) — "+
			"инструмент пересобирался бы на каждом обращении", a, again)
	}
	t.Logf("перепись: корней сверено 2, путей различных 2, повтор устойчив")
}

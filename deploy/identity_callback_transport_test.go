...
//     Полоса, объявленная в конфигурации, которую сегодня никто не монтирует,
//     остаётся под проверкой намеренно: она станет живой в тот день, когда её
//     смонтируют, и транспорт к тому дню обязан быть уже верным. Что конфигурация
//     службы личности сегодня не монтируется — измерено рендером и заведено
//     ОТДЕЛЬНЫМ предметом (#904) с тремя исходами и предикатом по каждому; здесь
//     это граница, а не умолчание.
package deploy_test

...
ЧЕМ ЭТОТ ГЕЙТ ОТЛИЧАЕТСЯ ОТ СОСЕДНЕГО ПО КЛАССУ. Проверка «уборка снимает
потомка, КОТОРОГО КЕЙС ЗАВЁЛ САМ» строит тройку (кейс, потомок, родитель) из
...
WRITER_DELETE = re.compile(r"\bw\.([A-Za-z]+?)W?\(\)\.Delete\(")

...
def walk_verdict(cols: int, born: int, findings: list, client_pairs: list):
    """→ (код возврата, имя состояния). Вердикт ОБХОДА КОЛЛЕКЦИЙ, один производитель.

    Зовут его прод-путь и самопроверка: второй кодек разошёлся бы с первым молча
    и именно там, где расхождение не видно.

    ЧТО ЗДЕСЬ БЕЗУСЛОВНО, А ЧТО НЕТ — различие несущее:

      * НАХОДКА красит при ЛЮБЫХ парах. Обход остаётся вооружённым и после того,
        как пара уехала: коллекции этого дерева ходят к тому же краю, и заведи
        любая из них аккаунт — сага потомка приедет вместе с ним;
      * «НОЛЬ КОЛЛЕКЦИЙ» красит безусловно: «ноль находок» обязано быть отличимо
        от «ноль прочитанного»;
      * «НОЛЬ СОЗДАННЫХ АККАУНТОВ» красит ТОЛЬКО при живой клиентской паре — паре,
        чью уборку делает кейс (значение None) и чей производитель в этом дереве.
        Нет таких пар — предмета у этой предпосылки нет, и отказ по ней был бы
        вечным. Состояние называется, а не молчит, и истекает само: появится
        клиентская пара — предпосылка снова красит.
    """
    if cols == 0:
        return 1, "ноль прочитанных коллекций"
    if findings:
        return 1, "находки"
    if born == 0:
        if client_pairs:
            return 1, "клиентская пара есть, а созданных аккаунтов ноль"
        return 0, "беспредметно"
    return 0, "чисто"
...
def self_test() -> int:
    ...
    # (л3) ВЕРДИКТ ОБХОДА — пять сочетаний, и главная половина первая: НАХОДКА
    #      красит при ЛЮБЫХ парах. Без неё смягчение «ноль созданных» накрыло бы и
    #      настоящую находку, то есть стало бы маской.
    _pair = [("iam", "account", "Projects")]
    for label, args, want in (
        ("находка при НУЛЕ клиентских пар → красит", (1, 1, ["x"], []), 1),
        ("находка при живой клиентской паре → красит", (1, 1, ["x"], _pair), 1),
        ("ноль созданных при ЖИВОЙ клиентской паре → красит", (1, 0, [], _pair), 1),
        ("ноль созданных при НУЛЕ клиентских пар → беспредметно", (1, 0, [], []), 0),
        ("ноль прочитанных коллекций → красит при любых парах", (0, 0, [], []), 1),
        ("коллекции есть, создания есть, находок нет → чисто", (1, 3, [], _pair), 0),
    ):
        got, state = walk_verdict(*args)
        if got == want:
            print(f"  ОК  {label} ({state})")
        else:
            print(f"  ПРОВАЛ {label}: получили {got} ({state}), ждали {want}")
            rc = 1
...
def main() -> int:
    ...
    walk_rc, walk_state = walk_verdict(cols, born, findings, client_pairs)
    if walk_state == "ноль прочитанных коллекций":
        print("FAIL: обход не нашёл НИ ОДНОЙ коллекции — «ноль находок» стало")
        print("      неотличимо от «ноль прочитанного».")
        return walk_rc
    if walk_state == "клиентская пара есть, а созданных аккаунтов ноль":

...
# путём, ИМЕНЕМ ОБЪЯВЛЕНИЯ КОНВЕЙЕРА и причиной. Дом выбирается ЗАМЕРОМ: код
# возврата, длительность и чистота дерева после прогона — см. блок замера ниже.
#
...
#
# Замер, из которого получена запись. Среда задания `helm lint · template`
# воспроизведена сужением PATH до того, что задание объявляет (helm · go ·
# python3 · git); дерево сверялось `git status --porcelain` до и после каждого
# прогона:
#
#   доказательство                       код   сек   дерево   дом
#   assert-pin-agrees-inject.sh            0    0.6   чисто    DECLARED
#   assert-trunk-green-inject.sh           0    0.6   чисто    DECLARED
#   publish-tag-inject.sh                  0    0.6   чисто    DECLARED
#   summarize-run-inject.sh                0    0.2   чисто    DECLARED
#   publish-service-artifact-inject.sh     0    6.2   чисто    DECLARED
#   breaking-since-release-inject.sh       2     —      —      ELSEWHERE (нет buf)
#
# `assert-trunk-green-inject.sh` даёт 0 и БЕЗ `gh` в PATH: он ставит свою
# заглушку и потому герметичен. `publish-service-artifact-inject.sh` требует go,
# и go в задании обхода есть (setup-go), поэтому его дом здесь.
ELSEWHERE="

...
#
# Формы доказательства две, и обе живые: `<тема>-inject.sh` (девять файлов) и
...
# вызываться, и десять доказательств снова станут немыми — молча.
if [ "$TREE" = "$REPO_ROOT" ]; then
  workflow_invokes "$REPO_ROOT" "$MAKE_TARGET" || {
    echo "FAIL: цель $MAKE_TARGET не вызывается НИ ОДНИМ объявлением конвейера."
    echo "      Тогда этот обходчик исполняется только руками, а десять доказательств"

...
"""Асинхронная мутация: идентификатор операции — свой, а исход удаления назван.
...
одним предметом образуют ЦЕПОЧКУ читателей этой операции.

  А. ТОЛЬКО ЕСЛИ ПРЕДМЕТ — `DELETE`: исход операции обязан назвать хотя бы один
     читатель цепочки — прочитать `error` внутри `pm.expect(...)`. Утверждения
     одного лишь `done` недостаточно: у завершившейся с ошибкой операции `done`
     тоже `true`;
  Б. НА ЛЮБОЙ МУТАЦИИ: шаг, ЗАПИСЫВАЮЩИЙ имя, которое читает опрос, обязан его
     сперва сбросить — снять (`pm.environment.unset('<имя>')`) либо задать
     пустым (`pm.environment.set('<имя>', '')`). Тогда несостоявшийся захват не
     подменяется чужой операцией.

ПОЧЕМУ ОБЛАСТИ У ДВУХ КЛАССОВ РАЗНЫЕ — И ЭТО НЕ АСИММЕТРИЯ ПО НЕДОСМОТРУ.
Класс Б спрашивает, ЧЬЮ операцию читает опрос; ответ не зависит от глагола —
у отвергнутого создания тела с `id` нет ровно так же, как у отвергнутого
удаления. Класс А спрашивает, ЧЕМ операция кончилась, а ЗНАК ожидаемого исхода
у создания задаёт предмет кейса, а не глагол: перепись на ревизии 1577829c7
нашла вне удаления 183 цепочки без утверждения исхода, и у 26 из них имя кейса
объявляет отказ (`-NEG-`, `-VAL-`, `DENY`) — требование «операция успешна»
утверждало бы ПРОТИВ предмета кейса. Расширение класса А требует разбора по
кейсу, а не предиката; предмет заведён задачей PRO-Robotech/kacho#1223.

ЧТО ДАЛО РАСШИРЕНИЕ КЛАССА Б (перепись ЭТИМ ЖЕ предикатом, ревизия 1577829c7 —
снимок на момент находки, а не инвариант; живые числа печатает сам проход):
область выросла с 1008 цепочек-удалений до 2692 цепочек-мутаций, и в новой
части нашлось 204 непокрытых захвата (PATCH 18, POST 186) сверх одного на
удалении. Закрыты они не здесь, а в генераторах — проходом
`_reset_captured_operation_id`, который задаёт ТОТ ЖЕ вопрос по тому же
признаку (имя берётся из адреса опроса, а не из соглашения об именовании).
...
def scan(root: str):
    ...
    n_col = n_step = n_del = n_poll = n_del_poll = n_chain = n_mut_chain = 0
    for rel in collection_files(root):
        ...
        for sidx, polls in chains.items():
            mtd = method(steps[sidx][0])
            # КЛАСС Б СУДИТСЯ НА ЛЮБОЙ МУТАЦИИ, А НЕ ТОЛЬКО НА УДАЛЕНИИ.
            # Подмена чужой операцией происходит от НЕСОСТОЯВШЕГОСЯ ЗАХВАТА, а не
            # от глагола: у отвергнутого создания тела с `id` нет ровно так же,
            # как у отвергнутого удаления, и опрос уезжает на предыдущую операцию
            # тем же путём. Собственная документация помощника захвата приводит
            # живым примером именно СОЗДАНИЕ (повторное приглашение отвечает 400,
            # опрос подтверждает ПРЕДЫДУЩЕЕ приглашение). Сужение до удаления было
            # границей ПЕРВОЙ волны, а не свойством класса.
            n_mut_chain += 1
            ...
            if writes_var(subj_code, var) and not clears_var(subj_code, var):
                b_findings.append(f"{rel} :: " + " :: ".join(steps[sidx][1])
                                  + f"  [{mtd} {var}]")
            # КЛАСС А ОСТАЁТСЯ ЗА УДАЛЕНИЕМ — и это не робость, а отсутствие
            # производителя знака. У удаления успех означает «ошибки нет», и знак
            # известен из глагола. У создания знак задаёт ПРЕДМЕТ КЕЙСА: перепись
            # на 1577829c7 нашла 183 цепочки вне удаления без утверждения исхода,
            # и у 26 из них имя кейса объявляет отказ (`-NEG-`, `-VAL-`, `DENY`),
            # то есть требование «операция успешна» утверждало бы ПРОТИВ предмета.
            # Расширение класса А требует разбора по кейсу, а не предиката —
            # предмет заведён задачей PRO-Robotech/kacho#1223.
            if mtd != "DELETE":
                continue
            n_chain += 1
    ...
    return (a_findings, b_findings,
            (n_col, n_step, n_del, n_poll, n_del_poll, n_chain, n_mut_chain))
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cases = [
            ...
            # ── ЗАКОННЫЙ БЛИЗНЕЦ ШИРОКОГО КЛАССА Б: то же СОЗДАНИЕ, что выше,
            # но со снятием перед захватом. Без него расширение ловило бы ГЛАГОЛ,
            # а не существо: проверка, краснеющая на каждом создании подряд, была
            # бы снята первой же волной как ложно-срабатывающая.
            _case("CASE-create-clean", [
                _step("create-clean", "POST", "/x/v1/things",
                      ["pm.environment.unset('opId');",
                       "pm.test('status 200', () => pm.expect(pm.response.code).to.eql(200));",
                       *CAPTURE]),
                _step("poll-op-12", "GET", "/operations/{{opId}}", POLL_DONE_ONLY),
            ]),
        ...
        a_f, b_f, (n_col, n_step, n_del, n_poll, n_del_poll, n_chain,
                   n_mut_chain) = scan(tmp)
        ...
        for quiet, why in (("poll-op-2", "исход утверждён `j.error`"),
                          ...
                          ("poll-op-12", "класс А остаётся за удалением: знак исхода "
                                         "создания задаёт предмет кейса, не глагол"),
        ...
        for want, why in (("del-thing", "захват без снятия принят за безопасный"),
                          ...
                          ("create-thing", "класс Б сужен до удаления — но подмена чужой "
                                           "операцией приходит от отказа захвата, а не от глагола")):
            if want in b_names:
        ...
        for quiet, why in (("del-clean", "снятие записано unset"),
                           ...
                           ("create-clean", "снятие есть — законная форма на СОЗДАНИИ")):
            if quiet in b_names:
        ...
        if (n_col, n_del, n_del_poll, n_chain, n_mut_chain) == (1, 10, 13, 10, 12):
            print(f"  ОК  объём осмотренного считается: коллекций {n_col}, шагов {n_step}, "
                  f"DELETE {n_del}, опросов {n_poll}; цепочек «мутация → опрос» "
                  f"{n_mut_chain}, из них с предметом-удалением {n_del_poll} в "
                  f"{n_chain} цепочках")
        else:
            print(f"  ПРОВАЛ объём осмотренного неверен: коллекций {n_col}, шагов {n_step}, "
                  ...
                  f"цепочек-удалений {n_chain}, цепочек-мутаций {n_mut_chain} "
                  f"(ожидалось 1 / 10 / 13 / 10 / 12)")
            rc = 1

        # ПРЕДПОСЫЛКА: пустое дерево — ОТКАЗ, а не «чисто».
        with tempfile.TemporaryDirectory() as empty:
            _, _, (e_col, _, _, _, e_dp, _, _) = scan(empty)
            if (e_col, e_dp) == (0, 0):
...
def main() -> int:
    ...
    a_f, b_f, (n_col, n_step, n_del, n_poll, n_del_poll, n_chain,
               n_mut_chain) = scan(root)

    print(f"=== опрос операции: осмотрено коллекций {n_col}, шагов {n_step}, "
          f"DELETE {n_del}, опросов операции {n_poll}; цепочек «мутация → опрос» "
          f"{n_mut_chain} (область класса Б), из них с предметом-удалением "
          f"{n_del_poll} шагов в {n_chain} цепочках (область класса А) ===")
    if n_col == 0 or n_del_poll == 0 or n_mut_chain == 0:
    ...
    if b_f:
        rc = 1
        print(f"FAIL (класс Б): {len(b_f)} шаг(ов)-мутаций захватывают идентификатор операции,")
    ...
    print(f"PASS: все {n_chain} цепочек опроса удаления утверждают исход "
          f"({n_del_poll} шагов опроса); все {n_mut_chain} цепочек «мутация → опрос» "
          f"опрашивают СВОЮ операцию")
    return 0

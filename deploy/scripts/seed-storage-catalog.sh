...
# Отсутствие компонента ОБЪЯВЛЯЕТСЯ, а не угадывается опросом.
#
# Стенд собирается шардами, и состав объявляет тот, кто его собирает:
# POSTURE_SKIP перечисляет сервисы, которых на этом стенде НЕТ по решению
# оператора, — тем же значением, которым это уже сказано гейту посадки.
#
# Почему объявление, а не опрос кластера. Опрос не отличает «компонента нет» от
# «компонент ещё не появился»: на медленном стенде посев решил бы, что сеять
# нечего, и вышел бы успехом, оставив каталог пустым. Объявление такой
# неоднозначности не имеет by construction.
for skipped in $POSTURE_SKIP; do
  if [ "$skipped" = "storage" ]; then
    echo "[seed-storage] storage объявлен ОТСУТСТВУЮЩИМ на этом стенде (POSTURE_SKIP)."
    echo "               Сеять нечего — предмета у шага нет. Это не поломка: так"
    echo "               собираются шарды vpc/nlb/edge."
    exit 0
  fi
done

# Компонент объявлен присутствующим — значит его база ОБЯЗАНА быть. Здесь опрос
# уже законен: он отвечает не на «есть ли компонент», а на «поднялся ли он».
if ! kubectl -n "$NS" get deploy kacho-storage >/dev/null 2>&1; then
  echo "FATAL: storage не объявлен отсутствующим, но его развёртывания нет в ns/$NS." >&2
  echo "       Либо стенд не поднялся, либо состав объявлен неверно: назовите" >&2
  echo "       компонент в POSTURE_SKIP, если его тут не должно быть." >&2
  exit 1
fi

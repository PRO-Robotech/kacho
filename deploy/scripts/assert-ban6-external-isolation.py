...
# Корней дерева контрактов ТРИ, и у третьего раскладка ДРУГАЯ. Платформа и
# служба доступа кладут контракты под `proto/<корень>/cloud/<домен>/`; фундамент
# — под `proto/corelib/<форма>/`, без сегмента `cloud`: сегмент называет ОБЛАКО,
# а фундамент облаком не является и своих доменов не имеет.
#
# Третий корень заведён не теорией: форма подписки переименована из
# платформенного `kacho.cloud.subscription` в `corelib.subscription` (её
# потребители — оба продукта). Пока образец знал два корня, её Internal-методы
# не попадали в перечень — не находкой и не тишиной, а МОЛЧАНИЕМ, — при том что
# перепись объявляла домен провязанным. Поймала это предпосылка перечисления
# самопробы: домен есть, методов в предмете ноль.
PROTO_ROOTS = ("kacho", "kaname")
PROTO_GLOBS = ([os.path.join(REPO_ROOT, "proto", r, "cloud", "**", "*.proto") for r in PROTO_ROOTS]
               + [os.path.join(REPO_ROOT, "proto", "corelib", "**", "*.proto")])
...
def domain_of(pkg: str) -> str:
    """Домен по имени пакета контракта — ДВЕ раскладки, а не одна.

    Платформа и служба доступа называют пакет `<корень>.cloud.<домен>[.<версия>]`,
    и домен там третьим сегментом. Фундамент сегмента `cloud` НЕ несёт — он не
    облако и своих доменов не имеет: его пакеты зовутся `corelib.<форма>`, и
    форма стоит ВТОРЫМ сегментом.

    Знать обе обязательно. Форма подписки переименована из платформенного
    `kacho.cloud.subscription` в `corelib.subscription` (её потребители — оба
    продукта), и разбор по третьему сегменту вернул бы для неё ВЕСЬ пакет
    целиком. Домен `subscription` перестал бы совпадать сам с собой: перепись
    объявляла бы его провязанным, а предмет пробы не содержал бы о нём ни одного
    метода — предпосылка перечисления это и поймала.
    """
    parts = pkg.split(".")
...
def self_test() -> int:
    ...
    #
    # ИСКЛЮЧЕНИЕ РОВНО ОДНО, И ОНО ВЫВЕДЕНО ИЗ ДЕРЕВА, А НЕ ВЫПИСАНО: носитель,
    # чьих ИСХОДНИКОВ в этом дереве нет, композиционного корня здесь не имеет by
    # construction — его часть собирается в своём репозитории, а нам остаются
    # чарт, эндпоинт и обязанность спросить его листенер встречным контролем.
    # Требовать от него регистрации значило бы требовать координаты того, чего
    # здесь нет; это тот же довод, по которому из предмета исключён домен без
    # регистрации (см. `subject_rpcs`).
    #
    # Послабление САМОИСТЕКАЕТ и печатается, а не умалчивается: в день, когда
    # каталог части вернётся в дерево, носитель снова окажется в `in_tree`, и
    # контроль потребует от него регистрации ровно как от остальных. Молчаливый
    # пропуск был бы неотличим от «осмотрели и сошлось».
    ...
    # Пустое множество каталогов — «ноль прочитанного», а не «все части внешние».
    # На нём сужение НЕ применяется вовсе: иначе контроль разоружился бы целиком
    # и молча — ровно тот исход, против которого он и заведён.
    ...
    print(f"  контроль в обратную сторону: носителей с известным эндпоинтом {len(known)}; "
          f"каталогов частей в дереве {len(in_tree)}; "
          f"не служат НИ ОДНОГО провязанного домена: {', '.join(lost) if lost else 'нет'} — "
          f"{'ПРОВАЛ' if lost else 'ОК'}")
    for h in external:
        print(f"    вне дерева: {h} — исходников под services/{h} нет, корня здесь "
              f"не может быть; эндпоинт остаётся, встречный контроль по нему идёт")
    rc |= 1 if lost else 0

...
#
# Все четыре образца ниже сняты с ПИННУТОГО helm (v4.2.3 — тот же, что ставит
...
CASES
  ...
  #
  # Образцы сняты с пиннутого helm v4.2.3 (тот же, что в e2e-newman.yml), а не
  ...
  echo
  echo "  --- чей отказ (образцы helm v4.2.3, инъекция в обе стороны)"
  cls_case() {

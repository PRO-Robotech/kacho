...
own_front_forward() {  # <public|internal> <локальный порт> <имя переменной проб>
  ...
  # УСЛОВИЕ — ОБЪЯВЛЕНИЕ ПОСАДКИ, А НЕ НАЛИЧИЕ СЕКРЕТА (#2372).
  #
  # Прежде лист подавался, «если секрет есть». Это другой вопрос, и он расходится
  # с предметом в ОБЕ стороны: секрет может лежать в пространстве имён при
  # невзаимном ребре — тогда лист подан впустую и ничего не доказывает; а при
  # взаимном ребре без секрета молчание выглядело бы как «листа не требуется»,
  # хотя это «условие не создано».
  #
  # Спрашиваем то, что процесс РЕАЛЬНО объявил поверхностью: ось own_rest_internal_tls
  # его самоотчёта при старте. Она заполняется не сырой ручкой, а тем, что поднято,
  # поэтому не врёт при частично применённой настройке.
  local posture=""
  if [ "$front" = internal ]; then
    posture="$(kubectl -n "$NS" logs "deploy/$svc" --tail=-1 2>/dev/null \
      | grep -o '"own_rest_internal_tls":"[^"]*"' | tail -1 | cut -d'"' -f4)"
    case "$posture" in
      true)
        : ;;  # ребро взаимное — лист обязателен, ниже он и берётся
      false|"n/a")
        echo "[parallel] $var: посадка объявила внутренний фронт БЕЗ взаимного режима" \
             "(own_rest_internal_tls=$posture) — клиентский лист НЕ подаём: он ничего бы не доказал"
        return 0 ;;
      *)
        echo "[parallel] $var: посадку внутреннего фронта прочитать НЕ УДАЛОСЬ" \
             "(самоотчёт не найден в журнале) — это «условие не создано», а не «лист не нужен»"
        return 0 ;;
    esac
  fi

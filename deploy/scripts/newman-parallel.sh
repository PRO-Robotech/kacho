...
HYDRA_PORT="${HYDRA_PUBLIC_PORT:-14444}"   # ТОЛЬКО волна церемонии (см. блок посадки ниже)
...
# Публичная поверхность прежнего издателя. ПОСЕВ ЕЁ БОЛЬШЕ НЕ НАБИРАЕТ: обмен идёт у
# нашего издателя (`POST /iam/v1/token`, PLATFORM_TOKEN_URL ниже), поэтому и проверка
# достижимости, и передача HYDRA_PUBLIC_PORT в посев сняты вместе со своим читателем.
# Проброс остаётся РОВНО из-за одного предмета — волны церемонии (prodseed_ceremony.py),
# которая ведёт интерактивный вход человека через прежнего поставщика личности. Снимается
# вместе с ней, одним изменением: предикат снятия — ноль читателей KRATOS_*/HYDRA_* URL
# в tests/authz-fixtures.
...
if [ "$SEED" = "true" ]; then
  ...
  # ЗДЕСЬ ПЕРЕДАВАЛИСЬ HYDRA_TOKEN_URL и HYDRA_PUBLIC_PORT — адрес обмена у прежнего
  # издателя и порт его проброса. Читателей не осталось ни у того, ни у другого: ключ
  # служебной учётки зеркала у поставщика не заводит и обменивается только у нашего
  # издателя (PLATFORM_TOKEN_URL ниже, задача #1120), а проверка достижимости прежнего
  # издателя снята вместе с посевом. Переменная, которую никто не читает, читается
  # следующим как действующая полоса, поэтому снята вместе с ней.
...
#
# Гейт ждал, пока дренаж применит к внешнему движку прав строки `kaname.fga_outbox`,
...
# ─── ЗДЕСЬ СТОЯЛА ВОЛНА 3 — снята вместе со своим предметом (kacho#2605) ─────
#
# Волна гоняла суиту, которой нужен ВЫКЛЮЧЕННЫЙ store прав, и наводилась на её
# прогонщика в каталоге суиты службы доступа, под services/iam. Служба вынесена
# отдельным продуктом (PRO-Robotech/kaname): каталога services/iam в этом дереве
# нет вовсе, поэтому проверка существования файла ложна ПО ПОСТРОЕНИЮ. Ложно и
# `[[ " $SERVICES " == *" iam "* ]]` — умолчание SERVICES суиты службы больше не
# называет (см. его объявление выше).
#
# СНЯТО, А НЕ ПЕРЕНАЦЕЛЕНО: суита и её прогонщик живут в ДРУГОМ репозитории, и
# пути к ним из $REPO_ROOT не существует — перенацеливать не на что. Мёртвая
# координата в исполняемом коде хуже отсутствующей: блок не исполнялся НИ ПРИ
# КАКОМ входе и молчал вместо отказа.
#
# РУЧНОЙ ПУТЬ ОСТАЁТСЯ ГРОМКИМ: `SERVICES="… iam …"` упирается в регенерацию
# выше (`gen iam FAILED`, код 1) — отказ, а не тишина.
#
# ИМЯ СНЯТОГО НАМЕРЕННО НЕ ПИШЕТСЯ КООРДИНАТОЙ. Мёртвый путь в форме координаты
# читается следующим как живое утверждение об ЭТОМ дереве. Сверх того обходчик
# достижимости (`make dialled-transport-discipline`) разрешает ссылки по всему
# телу файла, комментарии включая, и заносит нерезолвящийся путь в перепись
# «ссылок не разрешено» — число, которое он печатает как свойство прогонщика;
  ...
  if [ "$CEREMONY_WAVE" = "true" ]; then
    ...
    if ( cd "$REPO_ROOT/services/iam/tests/newman" \
          && env SETUP_NS="$NS" DELAY="$DELAY" \
             ...
             EXTRA_NEWMAN_ARGS="--env-var baseUrl=http://localhost:$GW_PORT --env-var internalBaseUrl=http://localhost:$GW_INTERNAL_PORT --env-var externalBaseUrl=https://127.0.0.1:$GW_TLS_PORT --env-var iamJwksBaseUrl=https://127.0.0.1:$IAM_JWKS_PORT --env-var iamRegistryTokenBaseUrl=https://127.0.0.1:$IAM_REGTOKEN_PORT$OWN_FRONT_ENV_STR$OPT_ENV_ARGS" \

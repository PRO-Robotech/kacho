...
NOT_A_TRANSPORT: dict[str, str] = {
    ...
    "HYDRA_ASSERTION_AUDIENCE": (
        "получатель утверждения клиента — это ОБЪЯВЛЕННАЯ издателем конечная точка "
        "выдачи. Значение попадает в подписываемое утверждение и обязано совпадать с "
        "тем, что издатель говорит о себе, а не с локальным пробросом."
    ),
}
...
def _self_test() -> int:
    ...
    # (б) законный близнец — та же форма, проброс есть: гейт обязан МОЛЧАТЬ.
    with tempfile.TemporaryDirectory() as tmp:
        build(tmp, with_forward=True)
        n, f, _ = check(tmp)
        hit = n == 0
        print(f"  {'ОК ' if hit else 'ПРОВАЛ'} законный близнец: та же форма с пробросом — молчит"
              + ("" if hit else f" ({f})"))
        ok &= hit
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        n, f, _ = check(tmp)
        hit = n == len(NOT_A_TRANSPORT) and all(
            any(name in x for x in f) for name in NOT_A_TRANSPORT)

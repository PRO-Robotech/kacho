...
ИСХОДОВ С КОДОМ НОЛЬ ДВА, И РАЗЛИЧАЕТ ИХ ЗНАК ПЕРВОЙ СТРОКИ (`VOID_MARK` против
`PASS_MARK`). До объявления знаков различала их только проза, а два разных мира,
печатающих одну строку, суть один мир: «условие не создано» и «проверено, чисто»
становились одним исходом. Знаки утверждаются ПОВЕДЕНЧЕСКИ — снятием вывода обоих
путей, — и чего это не закрывает, сказано при `VOID_MARK`.
...
# ЗНАК ИСХОДА — ОБЪЯВЛЕНИЕМ, А НЕ ПРОЗОЙ ПЕРВОЙ СТРОКИ.
#
# ОБА исхода этого гейта отдают КОД НОЛЬ, и миры за ними разные: «проверено,
# запаса хватает» и «мерить нечего, предмет в другом дереве». Различить их
# вызывающий обязан машинно, а не глазами, — иначе третья категория подана как
# успех, и «условие не создано» становится «чисто».
#
# ПОЧЕМУ НЕ СВОЙ КОД ВОЗВРАТА. Рассмотрено и отвергнуто: суита службы доступа
# живёт в дереве другого продукта, поэтому условие здесь создать НЕЛЬЗЯ. Ненулевой
# код дал бы ВЕЧНОЕ красное, а вечное красное снимают вместе с гейтом — теряя и
# ту половину, которая ещё жива. Поэтому ноль, но с НАЗВАННЫМ знаком.
#
# ЧЕМ ДЕРЖИТСЯ. Самопроверка снимает вывод ОБОИХ путей и требует: на беспредметном
# стоит `VOID_MARK` и НЕТ `PASS_MARK`, на существенном — наоборот, и знаки не
# равны. Прозу первой строки правят свободно, знак — нет.
#
# ЧЕГО ЭТО НЕ ЗАКРЫВАЕТ, СКАЗАНО ПРЯМО: сегодняшний вызывающий (процесс конвейера)
# знака НЕ читает — он читает код возврата, и для него оба исхода по-прежнему один.
# Эта половина держится вниманием; предикат для того, кто возьмётся: вызывающий
# обязан ветвиться по знаку первой строки, а не по одному коду.
VOID_MARK = "БЕСПРЕДМЕТНО"
PASS_MARK = "ПРОВЕРЕНО"

# КОД БЕСПРЕДМЕТНОГО ИСХОДА — ОДНИМ ОБЪЯВЛЕНИЕМ, ЧТОБЫ ЕГО МОЖНО БЫЛО СМЕНИТЬ.
#
# У дерева ЕСТЬ свой контракт трёх исходов, и он объявлен не здесь:
# `deploy/tests/helm/outcome.sh` — 0 зелено, 1 находка о дереве, 2 «условие не
# создано» (не вердикт, в зачёт «прошло» не идёт). Беспредметность — та же третья
# категория, и по контракту дерева ей полагался бы код 2.
#
# ЗДЕСЬ СТОИТ НОЛЬ, И ЭТО РАЗМЕН, А НЕ НЕДОСМОТР. Двойка осмысленна ровно там, где
# вызывающий её КЛАССИФИЦИРУЕТ: так делают `run-gate-self-tests.sh` (`case "$rc"`,
# сводка «УСЛОВИЕ НЕ СОЗДАНО») и цель `helm-manifest-test`. Вызывающий ПРОД-пути
# этих двух гейтов — шаг конвейера — кодов не разбирает вовсе: любой ненулевой
# роняет шаг. При двойке шаг стал бы красным НАВСЕГДА (суита службы доступа живёт
# в дереве другого продукта, условие здесь создать нельзя), а вечное красное
# снимают вместе с гейтом — теряя и живую половину.
#
# ПРЕДИКАТ СМЕНЫ, чтобы размен не стал вечным: как только вызывающий прод-путь
# классифицирует код 2 тем же способом, что два вызывающих выше, — поставить здесь
# 2. Правка ОДНОСТРОЧНАЯ и в ОДНОМ месте именно затем, чтобы её цена была нулевой.
# До тех пор миры различает знак первой строки, и это доказано поведенчески.
VOID_EXIT = 0
...
def synthetic_wave_root(*, with_wave: bool, with_seed: bool,
                        seed_body: str = "", with_base: bool = False) -> str:
    ...
    `with_base` — лежит ли источник БАЗОВОГО УРОВНЯ. Умолчание ЛОЖЬ, и это не
    вкус: доказательства порядка останавливаются раньше базового уровня, поэтому
    при умолчании их исход не меняется ни на один факт — расширение фикстуры не
    переписывает того, что она уже доказывала. ИСТИНА нужна единственному пути,
    который доходит до конца: СУЩЕСТВЕННОМУ исходу, где вердикт печатается и код
    ноль. Без него тот исход недостижим в этом дереве вовсе (служба доступа уехала
    вместе со своим источником), а значит и знак существенного исхода не был бы
    доказан ничем, кроме объявления константы.

    ФИКСТУРА НЕ СНИСХОДИТЕЛЬНЕЕ ПРОДУКТА: она не подменяет предикат, а создаёт
    корень, на котором предикат выполняется. Обратная сторона утверждается там же,
    где положительная: при `with_base=False` тот же путь обязан ОТКАЗАТЬ, назвав
    непрочитанный источник.
    """
    ...
    if with_seed:
        ...
        with open(os.path.join(mig, "0001_initial.sql"), "w", encoding="utf-8") as fh:
            fh.write("-- +goose Up\n" + seed_body)
    if with_base:
        # Источник базового уровня — СИНТЕТИЧЕСКИЙ и назван таковым: копировать его
        # из дерева нельзя, службы доступа здесь нет вовсе. Несёт РОВНО ту ветвь,
        # которую читает `read_base_components` (`BOOTSTRAP_ON_FIRST_LOGIN`), и
        # ничего сверх неё: фикстура, похожая на настоящий файл, прятала бы разницу
        # между «ветвь есть» и «файл выглядит правдоподобно».
        bs = os.path.join(d, BOOTSTRAP_SOURCE)
        os.makedirs(os.path.dirname(bs), exist_ok=True)
        with open(bs, "w", encoding="utf-8") as fh:
            fh.write("// СИНТЕТИКА фикстуры, не продукт: несёт одну ветвь, которую\n"
                     "// читает вывод базового уровня.\n"
                     "if ownedAccounts == 0 { /* завести личный аккаунт */ }\n")
    return d
...
def report_subject_elsewhere(census: dict, what: str = "пик аккаунтов") -> int:
    ...
    """
    print(f"{VOID_MARK}: волна церемонии в этом дереве не производится.")
...
def main(argv=None) -> int:
    ...
    print(f"{PASS_MARK}: запас не меньше {HEADROOM_REQUIRED} — одиночное новое "
          f"создание в волну помещается.")
    return code
...
def self_test() -> int:
    ...
    # отдают НОЛЬ. До объявления знаков различала их только проза первой строки, а
    # два разных мира, печатающих одну строку, суть один мир. Почему не свой код
    # возврата и чего это не закрывает — сказано при `VOID_MARK`.
    #
    # Утверждение ПОВЕДЕНЧЕСКОЕ: снимается вывод путей и сверяется знак. Проверка
    # объявления константы осталась бы зелёной при знаке, потерянном из строки.
    ...
    note(VOID_MARK != PASS_MARK,
         f"знаки РАЗНЫЕ: беспредметный {VOID_MARK!r}, существенный {PASS_MARK!r}")
    ...
    _r = synthetic_wave_root(with_wave=False, with_seed=True, seed_body=_CANON)
    _code, _out = _run(_r)
    note(_code == VOID_EXIT and _out.startswith(VOID_MARK + ":")
         and PASS_MARK not in _out,
         f"беспредметный путь: код {_code} (объявлен {VOID_EXIT}), знак "
         f"{_out.split(':')[0]!r}, существенного знака нет")
    _shutil.rmtree(_r, ignore_errors=True)

    # Размен назван ЧИСЛОМ и сверяется: ноль означает «вызывающий кодов не
    # разбирает». Станет два — утверждение ниже заставит перечитать вызывающего.
    note(VOID_EXIT in (0, 2),
         f"код беспредметности объявлен: {VOID_EXIT}"
         + (" (вызывающий прод-пути кодов не разбирает — размен назван при "
            "`VOID_EXIT`)" if VOID_EXIT == 0 else " (третья категория по контракту "
            "дерева — вызывающий обязан её классифицировать)"))

    # Существенный исход достижим только с источником базового уровня: `with_base`.
    # Меняется один факт против прогона ниже — величина потолка в затравке.
    _WIDE = _CANON.replace("'iam.account', 5,", "'iam.account', 9,")
    note(_WIDE != _CANON, "затравка с широким потолком отличается от настоящей")
    _r = synthetic_wave_root(with_wave=True, with_seed=True, seed_body=_WIDE,
                             with_base=True)
    _code, _out = _run(_r)
    note(_code == 0 and (PASS_MARK + ":") in _out and VOID_MARK not in _out,
         f"существенный путь (запас есть): код {_code}, знак {PASS_MARK!r} "
         f"напечатан, беспредметного знака нет")
    _shutil.rmtree(_r, ignore_errors=True)

    # Обратная сторона `with_base`: снят источник базового уровня — ОТКАЗ, а не
    # зелёное. Иначе расширение фикстуры было бы способом добыть зелёное.
    _r = synthetic_wave_root(with_wave=True, with_seed=True, seed_body=_WIDE,
                             with_base=False)
    try:
        audit(_r)
        ...
        note("личного аккаунта" in str(exc) or "базов" in str(exc),
             f"источника базового уровня нет → ОТКАЗ")
    _shutil.rmtree(_r, ignore_errors=True)

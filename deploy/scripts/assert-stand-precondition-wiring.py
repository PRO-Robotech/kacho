...
ЧТО ПРОВЕРЯЕТСЯ — СВЯЗНОСТЬ, А НЕ НАМЕРЕНИЕ. Звеньев семь, и любое разорванное
возвращает исходный дефект МОЛЧА:
  ...
  7. работа, ПРОИЗВОДЯЩАЯ отметку, её же и ЧИТАЕТ. Звенья 2 и 4 судят шаги:
     первое — что подъём делегирует владельцу, второе — что шаг-вердикт гасится.
     Ни одно не спрашивает про РАБОТУ ЦЕЛИКОМ, и между ними пролезает форма, в
     которой отметка выставляется и не читается НИКЕМ: производителей ≥ 1,
     читателей 0. Тогда третья категория работе невидима — шаг подъёма выходит
     нулём (так решено #2250), гасить некого, и работа выходит `success`, не
     вынеся вердикта ни одной проверкой.
     ЗАМЕРЕНО, А НЕ ПРЕДПОЛОЖЕНО (2026-09-12): у четырёх работ подъёма читателей
     отметки было 5 · 6 · 11 · **0**, и ноль стоял у `production-posture.yml`/
     `stand` — при ДВУХ производителях в ней же (владелец подъёма и провенанс
     ревизии). Исполнение настоящих тел шагов дало `success` при выставленной
     отметке: ни один шаг о посадке не сказал ничего.
     Предикат ВЫВЕДЕН из дерева в обе стороны: производителем считается скрипт,
...
владелец — перепись исходов `.github/scripts/assert-declared-verdicts-ran.py`,
последний шаг работы. Требовать её здесь от ВСЕХ четырёх работ гейт не вправе:
три из них — предмет соседних полос, и такое требование краснело бы на чужой
незакрытой работе, то есть стало бы недостижимым by construction. Держатель
«работа зовёт перепись» — предмет общего гейта гарантии, а не этого.
...
def produces_flag(step: dict, producers: list[str]) -> bool:
    """Шаг ПРОИЗВОДИТ отметку, если ЗАПУСКАЕТ скрипт-производитель.

    ИНТЕРПРЕТАТОР ПЕРЕД ПУТЁМ ОБЯЗАТЕЛЕН В ПРЕДИКАТЕ, и это не догадка: первая
    редакция требовала имя в позиции команды и молча теряла форму
    `bash "$GITHUB_WORKSPACE/.../stand-revision-verdict.sh"` — то есть ровно тот
    второй производитель, ради которого звено и заведено. Два моих же замера одной
    работы разошлись (2 против 1), и слеп был предикат, а не дерево.
    """
    run = str(step.get("run") or "")
    for line in run.splitlines():
        # САМОПРОВЕРКА — НЕ ПРОИЗВОДСТВО. `stand-up.sh --self-test` работает в
        # своём временном дереве и в `$GITHUB_ENV` работы не пишет НИЧЕГО. Единица
        # счёта — ШАГ конвейера, и замер по дереву даёт: нынешний предикат 7 · без
        # этой оговорки 9 · без формы с интерпретатором (первая редакция) 4. Два
        # лишних в девятке — вызовы самопроверки, то есть без оговорки звено 7
        # требовало бы читателя от работы, которая отметку не производит.
        # Оговорка судит СТРОКУ вызова;
        # перенос `--self-test` на продолжение строки она не увидит, и это названо
        # здесь, а не оставлено на догадку.
        if "--self-test" in line:
            continue
        for name in producers:
            if re.search(r"(^|[;&|(]|\$\()[ \t]*(?:(?:bash|sh|python3|python)[ \t]+)?\S*"
                         + re.escape(name) + r"\b", line):
                return True
    return False
...
def check() -> tuple[list[str], dict[str, int]]:
    ...
    seen = {"jobs": 0, "steps": 0, "verdict": 0, "guarded": 0, "carrier": 0, "noif": 0,
            "producers": 0, "readers": 0}
    runners_hit = {r: 0 for r in VERDICT_RUNNERS}
    exempt_hit = {k: 0 for k in DELEGATION_EXEMPT}

    # ЗВЕНО 7 (см. шапку): предикат обязан иметь ВХОД. Ноль скриптов-производителей
    # означает «ничего не прочитано», а не «связно»: тогда каждая работа проходит
    # звено 7 by construction, и оно превращается во всеразрешение.
    producers = flag_producers()
    if not producers:
        findings.append(
            f"в `.github/scripts` ни один файл не дописывает `{FLAG}=1` в `$GITHUB_ENV` — "
            f"звено 7 осталось БЕЗ ВХОДА и прошло бы по каждой работе даром. Либо отметка "
            f"переименована, либо производитель уехал: «ноль находок» здесь неотличимо от "
            f"«ноль прочитанного»")
    ...
    for wf, jn, steps, stand in stand_jobs():
        ...
        # ── звено 7: работа, производящая отметку, её же и читает ─────────────
        made = [s for s in steps if produces_flag(s, producers)]
        read = [s for s in steps if guarded(s)]
        seen["producers"] += len(made)
        seen["readers"] += len(read)
        if made and not read:
            findings.append(
                f"{where}: отметку {FLAG} в этой работе ПРОИЗВОДЯТ {len(made)} шаг(а) — "
                + ", ".join(f"«{s.get('name') or s.get('id') or '?'}»" for s in made)
                + " — и не читает НИ ОДИН. Третья категория работе невидима: шаг подъёма "
                "выходит нулём (так решено #2250), гасить некого, и работа выйдет `success`, "
                "не вынеся вердикта ни одной проверкой. Зелёное такой работы означает «мы не "
                "спрашивали», а владелец 2026-09-12 потребовал обратного: зелёное — гарантия")
...
def _self_test() -> int:
    ...
    # ── (з) ЗВЕНО 7: производитель отметки без читателя — ИНЪЕКЦИЯ В ОБЕ СТОРОНЫ
    #
    # Три прогона, и меньше нельзя. Контроль говорит, что предикат способен
    # МОЛЧАТЬ на живом дереве; инъекция — что он способен НАЙТИ; законный близнец
    # отличается от инъекции РОВНО ОДНИМ фактом (остались ли производители) и
    # обязан снова молчать. Без близнеца звено 7 требовало бы читателя от работы,
    # которой нечего читать, — то есть стало бы недостижимым by construction.
    say(not any("не читает НИ ОДИН" in x for x in check()[0]),
        "контроль: у каждой работы-производителя есть читатель отметки")

    prod_now = flag_producers()
    say(bool(prod_now),
        f"СКРИПТОВ-производителей отметки в дереве найдено {len(prod_now)} "
        f"({', '.join(prod_now)}) — у звена 7 есть ВХОД",
        "ни одного: предикат судил бы даром")

    def strip_reader(steps: list[dict]) -> list[dict]:
        out = []
        for s in steps:
            cond = str(s.get("if") or "")
            if f"env.{FLAG}" in cond:
                s = dict(s)
                s["if"] = re.sub(r"\s*&&\s*env\." + FLAG + r"\s*!=\s*'1'", "", cond) or "always()"
                s["if"] = s["if"].replace(f"env.{FLAG} != '1'", "true")
            out.append(s)
        return out

    def hurt_reader():
        return [(wf, jn, strip_reader(steps), stand) for wf, jn, steps, stand in real()]

    globals()["stand_jobs"] = hurt_reader
    f, seen7 = check()
    globals()["stand_jobs"] = real
    hits = [x for x in f if "не читает НИ ОДИН" in x]
    say(len(hits) >= 4,
        f"отметку перестали читать → находка по КАЖДОЙ работе-производителю "
        f"(нашлось {len(hits)})",
        f"нашлось {len(hits)}, работ-производителей в дереве не меньше четырёх")

    # ЗАКОННЫЙ БЛИЗНЕЦ: тот же снятый читатель, но и производителя в работе НЕТ.
    # Отличается от инъекции выше РОВНО ОДНИМ фактом.
    def hurt_both():
        out = []
        for wf, jn, steps, stand in real():
            new_steps, new_stand = [], stand
            for s in strip_reader(steps):
                orig = s
                if produces_flag(s, prod_now):
                    s = dict(s, run="echo производителя в этой работе нет\n")
                new_steps.append(s)
                # Шаг подъёма обязан быть ТЕМ ЖЕ объектом, что лежит в списке:
                # `check()` ищет его через `steps.index(stand)`, и копия дала бы
                # ValueError — отказ пробы, неотличимый на глаз от находки.
                if orig is stand:
                    new_stand = s
            out.append((wf, jn, new_steps, new_stand))
        return out

    globals()["stand_jobs"] = hurt_both
    f, _ = check()
    globals()["stand_jobs"] = real
    say(not any("не читает НИ ОДИН" in x for x in f),
        "производителя в работе нет → звено 7 молчит (читателя не требует)")

    # САМОИСТЕЧЕНИЕ ВХОДА: ноль скриптов-производителей — «ноль прочитанного», а не
    # «связно». Без этой половины переименование отметки сделало бы звено 7
    # всеразрешением, и заметить это было бы нельзя ничем.
    real_prod = flag_producers
    globals()["flag_producers"] = lambda: []
    f, _ = check()
...
def main() -> int:
    ...
    print(f"осмотрено: работ подъёма {seen['jobs']} "
          ...
          f"ШАГОВ-производителей отметки {seen['producers']} при ШАГАХ-читателях "
          f"{seen['readers']} (производят её {len(flag_producers())} скрипта: "
          f"{', '.join(flag_producers()) or 'НИ ОДНОГО'}), "

...
DELEGATION_EXEMPT = {
    ("ci.yaml", "helm"):
        "материализация зависимостей ради рендера чартов, а не подъём стенда: код 3 "
        "доезжает до шага НАПРЯМУЮ (make в цепочке нет), но за ним идут 20 шагов "
        "рендера, каждому из которых нужен свой пропуск. Работа своей формы и своего "
        "объёма — kacho#2257",
}

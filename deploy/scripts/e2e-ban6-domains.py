...
# КОРНЕЙ ТРИ, И У ТРЕТЬЕГО РАСКЛАДКА ДРУГАЯ. Платформа и служба доступа кладут
# контракты под `proto/<корень>/cloud/<домен>/`; фундамент — под
# `proto/corelib/<домен>/`, без сегмента `cloud`: этот сегмент называет ОБЛАКО,
# а фундамент облаком не является и своих доменов не имеет — он несёт формы,
# которыми пользуются оба продукта.
#
# Третий корень появился здесь не теорией: форма подписки переименована из
# платформенного `kacho.cloud.subscription` в `corelib.subscription` (её
# потребители — ОБА продукта, значит по существу она принадлежит фундаменту).
# Пока корней было два, домен `subscription` выпал из популяции МОЛЧА, а
# манифест шардов продолжал заявлять для него ban #6 — гейт покрытия сказал это
# первым, и он был прав: измерять стало нечего, потому что перепись перестала
# видеть контракт, а не потому, что контракт исчез.
PROTO_BASES = tuple(("proto", r, "cloud") for r in PROTO_ROOTS) + (("proto", "corelib"),)

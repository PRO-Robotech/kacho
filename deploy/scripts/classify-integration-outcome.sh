...
# Признак берётся из строки, которую печатает САМ производитель при отказе
# подъёма. Это не догадка по тексту докера: сообщений докера много и они
# меняются, а эта строка — наша и стабильна.
#
# Литерала здесь БОЛЬШЕ НЕТ: он жил вторым местом об одном предмете (первое —
# сам производитель) и разошёлся бы молча. Величина объявлена один раз, соседний
# распознаватель полос читает её оттуда же.
# shellcheck source=deploy/scripts/pgtest-unavailable-mark.sh
. "$(dirname "$0")/pgtest-unavailable-mark.sh"
UNAVAILABLE_MARK="$PGTEST_UNAVAILABLE_MARK"
...
if grep -q "$UNAVAILABLE_MARK" "$log"; then
    ...
    if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
        echo "::error title=УСЛОВИЕ НЕ СОЗДАНО::Postgres для integration не поднялся — прогон недействителен. Повторить после устранения причины." >&2
    fi
    exit 75
fi

...
ПАДАЕТ, если они перестали держаться, — иначе он тихо превратится в ложь:
  ...
  4. перепись объявляется числами (коллекций прочитано, листьев обойдено, шагов
     ...
     ЧИСЛО ТРЕБУЕТСЯ ВЕТВЬЮ, А НЕ ПЕЧАТАЕТСЯ. Напечатанное число ничего не
     утверждает, пока его не потребовало решение, и это здесь измерено, а не
     выведено: первая редакция послабления печатала «0 коллекц(ий), 0 шаг(ов)» и
     выходила НУЛЁМ — то есть выносила вердикт о дереве, которого не читала. От
     «ноль населения» ноль находок был отличим, от «ноль прочитанного» — нет.
     Теперь `leaves_read == 0` — ОТКАЗ, и он стоит первым среди вердиктных ветвей
     `decide`, раньше любого послабления.
...
ПОСЛАБЛЕНИЕ НЕ БЕРЁТСЯ НА ПУСТОМ ОБХОДЕ, и это условие отдельно от трёх
состояний выше. Состояние населения приходит от переписи СУИТЫ, а обход дерева
делает `scan` — два предиката с двумя объёмами; пока послабление спрашивало только
первый, оно накрывало и случай «прочитано ноль листьев вообще». Замер: сняв из
дерева все 57 коллекций, гейт печатал нули и выходил нулём. Поэтому ветвь
`leaves_read == 0 → отказ` стоит ПЕРВОЙ среди вердиктных, раньше и выпотрошенной
суиты, и беспредметности: вердикта по обходу, не прочитавшему ни одного листа, не
бывает никакого — ни «расхождений нет», ни «населения здесь нет».
...
«население исчезло при живой суите». Исчезнут коллекции дерева — вернётся отказ
«обход пуст». Ни одна ветвь послабления не зависит от перечня прощённых, потому
что его нет.
...
def decide(root: str) -> tuple[int, list[str]]:
    ...
    # ПУСТОЙ ОБХОД — ОТКАЗ, И ОН ВЫШЕ ВСЕХ ОСТАЛЬНЫХ ВЕТВЕЙ.
    #
    # Ветвь заведена замером, а не соображением: сняв из дерева все 57 коллекций,
    # прежняя редакция этого места печатала «осмотрено: 0 коллекц(ий), 0 шаг(ов)»,
    # объявляла беспредметность и выходила НУЛЁМ — то есть выносила вердикт о дереве,
    # о котором не прочитала ничего. Печать числа ничего не утверждает, пока его не
    # потребовала ветвь решения; до этой ветви ни одно из напечатанных чисел не
    # требовалось нигде.
    #
    # ЕДИНИЦА ТРЕБОВАНИЯ — ЛИСТ, а не файл: 57 коллекций без единого шага дают тот же
    # пустой предмет, что и ноль коллекций, и вердикт по ним был бы столь же пуст.
    #
    # ПОЧЕМУ ВЫШЕ ВЫПОТРОШЕННОЙ СУИТЫ, хотя обе ветви отказные. Состояние населения
    # приходит от ДРУГОГО предиката (перепись суиты в объявлении церемонии), и его
    # объём обхода — не наш. Утверждение «вердикта по пустому обходу не бывает»
    # держится тогда, когда оно не зависит от чужого ответа вовсе: иначе завтра
    # появится четвёртое состояние, и пустота уедет в него.
    if res["leaves_read"] == 0:
        out.append(
            f"  ПРОВАЛ обход пуст: прочитано {res['files_read']} коллекц(ий) и "
            f"{res['leaves_read']} шаг(ов) — гейт не прочитал НИЧЕГО. Любой вердикт по "
            f"такому обходу, включая беспредметность, был бы утверждением о другом "
            f"дереве: отбор шагов ({', '.join(COLLECTION_GLOBS)}) не нашёл ни одного "
            f"листа, и это отказ, а не «расхождений нет» и не «населения здесь нет»")
        return 2, out
    ...
    if state == decl.POPULATION_ELSEWHERE and res["human_steps"] == 0:
        ...
        #
        # ТРЕБОВАНИЕ `leaves_read > 0` ЗДЕСЬ ДЕРЖИТСЯ ВЕТВЬЮ ВЫШЕ, а не повторяется в
        # этом условии: повторённое, оно было бы неопровержимо — ветвь пустого обхода
        # возвращает отказ раньше, поэтому ложным оно не станет ни на каком входе, и
        # обе его стороны утверждать было бы нечем. Мёртвый страж — тот же класс, что
        # ловит корпус. Условие послабления читается так: население уехало И обход
        # непуст (первое — факт дерева от объявления, второе — от ветви выше).
...
def self_test() -> int:
    ...
    def verdict(line: str) -> None:
        """Печать исхода ОДНОГО утверждения — и его счёт, одним действием.

        Печать и счёт неразделимы намеренно. Разойдясь, они дали бы закрывающую
        строку «исполнено N», не связанную с числом напечатанных исходов, — то есть
        второе место об одном предмете внутри одной функции.
        """
        nonlocal asserted
        asserted += 1
        print(line)
    ...
    def closing() -> int:
        """Закрывающая строка: счёт ИСПОЛНЕННЫХ утверждений ИЗ СЧЁТЧИКА.

        Ноль исполненных — ПРОВАЛ, а не «нечего было проверять». Прежняя редакция
        печатала «ИТОГ САМОПРОВЕРКИ: OK» побайтово одинаково и при двадцати
        исполненных утверждениях, и при нуле; вердикта в такой строке нет, есть его
        вид. Единственный выход из самопроверки — здесь, поэтому счёт печатается и
        на путях раннего отказа.
        """
        out = rc
        if asserted == 0:
            # Печать БЕЗ счёта: этот отказ говорит о самом счётчике, и засчитав себя
            # в него, он напечатал бы «исполнено утверждений: 1» ровно там, где
            # исполнено ноль.
            print("  ПРОВАЛ самопроверка не исполнила НИ ОДНОГО утверждения — "
                  "строка ниже была бы видом вердикта, а не вердиктом")
            out = 1
        print(f"ИТОГ САМОПРОВЕРКИ: {'ПРОВАЛ' if out else 'OK'}; "
              f"исполнено утверждений: {asserted}")
        return out
    ...
    if premises:
        ...
        verdict(f"  ПРОВАЛ предпосылок гейта сломано: {len(premises)} — инъекцию "
                  f"строить не на чем, об остальных утверждениях не известно ничего")
        rc = 1
        return closing()
    ...
    routes = load_routes(root)
    if not routes:
        verdict(f"  ПРОВАЛ таблица маршрутов {ROUTE_TABLE_REL} не разобрана — "
    ...
    if not sensitive or not routine:
        verdict("  ПРОВАЛ в каталоге не нашлось пары ручек с ОБЪЯВЛЕННЫМИ порогами "
    ...
    if not slots:
        verdict("  ПРОВАЛ объявление не отдало ни одного слота — выводить нечего")
        rc = 1
    elif missing:
        verdict(f"  ПРОВАЛ предъявители слотов вне набора уровней: {missing}")
        rc = 1
    else:
        verdict(f"  OK слот(ов) {len(slots)}, предъявителей в наборе {len(levels)} "
    ...
    if not any("ADMISSION_SLOTS" in b for b in bad_decl):
        verdict(f"  ПРОВАЛ пропажа объявления слотов прошла молча: {bad_decl}")
        rc = 1
    else:
        verdict("  OK пропажа объявления слотов названа предпосылкой")
    os.symlink(os.path.join(root, CEREMONY_DECL_REL), os.path.join(tmp, CEREMONY_DECL_REL))
    ...
    if len(res["findings"]) != 1 or "delete-undergraded" not in res["findings"][0]:
        verdict(f"  ПРОВАЛ дефект не найден либо без координаты: {res['findings']}")
        rc = 1
    else:
        verdict(f"  OK найден с координатой: {res['findings'][0]}")
    ...
    if len(res["findings"]) != 1 or "create-overgraded" not in res["findings"][0]:
        verdict(f"  ПРОВАЛ перебор уровня не найден: {res['findings']}")
        rc = 1
    else:
        verdict(f"  OK перебор найден: {res['findings'][0]}")
    ...
    res = scan(tmp, catalog)
    if res["findings"]:
        verdict(f"  ПРОВАЛ гейт краснеет на законном: {res['findings']}")
        rc = 1
    elif res["mapped"] == 0:
        ...
        verdict("  ПРОВАЛ ни один законный шаг не сопоставлен — молчание вакуумно")
        rc = 1
    else:
        verdict(f"  OK промолчал на {res['mapped']} сопоставленных законных шагах")
    ...
    for method, template, fqn in ordered:
        ...
        if pair:
            break
    if not pair:
        verdict("  ПРОВАЛ в дереве нет под-ресурсного маршрута, чей порог отличается от "
        ...
        res = scan(tmp, catalog)
        if res["findings"]:
            verdict(f"  ПРОВАЛ верный под-ресурсный шаг объявлен расхождением: {res['findings']}")
            rc = 1
        elif res["mapped"] != 1:
            verdict(f"  ПРОВАЛ под-ресурсный шаг вообще не сопоставлен (mapped={res['mapped']}) — "
                    f"это слепое пятно, а не согласие")
            rc = 1
        else:
            verdict("  OK верный под-ресурсный шаг сопоставлен со своим маршрутом и не оговорён")
        ...
        if len(res["findings"]) != 1 or fqn not in res["findings"][0]:
            verdict(f"  ПРОВАЛ дефект под-ресурса не найден либо назван чужим именем: "
                    f"{res['findings']}")
            rc = 1
        elif pfqn in res["findings"][0]:
            verdict(f"  ПРОВАЛ находка называет РОДИТЕЛЬСКУЮ ручку: {res['findings'][0]}")
            rc = 1
        else:
            verdict(f"  OK найден и назван своим именем: {res['findings'][0]}")
    ...
    if not silent:
        verdict("  ПРОВАЛ в каталоге нет ручки без объявленного порога — "
        ...
        res = scan(tmp, catalog)
        if res["findings"]:
            verdict(f"  ПРОВАЛ гейт судит порог, которого каталог не объявлял: {res['findings']}")
            rc = 1
        elif res["undeclared"] < 1:
            verdict("  ПРОВАЛ несудимый шаг не попал в перепись — «не судили» стало "
                    "неотличимо от «расхождений нет»")
            rc = 1
        else:
            verdict(f"  OK {silent} не судится, и это названо числом "
    ...
    # КОЛЛЕКЦИЯ БЕЗ ЧЕЛОВЕЧЕСКИХ ШАГОВ — производитель НЕПУСТОГО обхода при пустом
    # населении утверждения. Без неё оба состояния населения утверждались бы на пустом
    # корпусе, а такой случай не проверяет ветвь, а ЗАКРЕПЛЯЕТ дыру: ровно на нём
    # прежняя редакция объявляла беспредметность, ничего не прочитав, и самопроверка
    # называла это успехом. Шаги здесь машинный и анонимный: порог их не касается,
    # поэтому листья считаются, а население утверждения остаётся нулевым.
    quiet = os.path.join(coll_dir, "quiet.json")

    def plant_quiet():
        with open(quiet, "w", encoding="utf-8") as fh:
            json.dump(_collection([
                _step("machine-sensitive", sens_method, sens_path, "jwtAccountAdminA"),
                _step("anon-sensitive", sens_method, sens_path),
            ]), fh)
    ...
    print("=== обход ПУСТ (ни одного листа) ⇒ ОТКАЗ, а не беспредметность ===")
    clear_outside()
    clear_quiet()
    clear_suite()
    rc_empty, lines = decide(tmp)
    if rc_empty != 2 or not said(lines, "обход пуст"):
        verdict(f"  ПРОВАЛ пустой обход не назван отказом (rc={rc_empty}): {lines[-2:]}")
        rc = 1
    elif said(lines, decl_live.VOID_MARK):
        verdict("  ПРОВАЛ пустой обход уехал в беспредметное и замолчал вместе с ним")
        rc = 1
    else:
        verdict("  OK пустой обход назван отказом, код 2")

    print("=== порядок: пустой обход ПРИ выпотрошенной суите ⇒ всё равно пустой обход ===")
    # Обе ветви отказные, и вердикт не изменился бы; предмет утверждения — ЧТО
    # сказано. Диагноз «население исчезло при живой суите» на пустом обходе был бы
    # ложен: суиту не с чем сравнить, когда не прочитано ни одного листа вообще.
    os.makedirs(suite_dir, exist_ok=True)
    rc_ord, lines = decide(tmp)
    if rc_ord != 2 or not said(lines, "обход пуст"):
        verdict(f"  ПРОВАЛ пустой обход не назван первым (rc={rc_ord}): {lines[-2:]}")
        rc = 1
    elif said(lines, "исчезло при живой суите"):
        verdict("  ПРОВАЛ пустой обход выдан за выпотрошенную суиту — диагноз о суите "
                "при нуле прочитанных листьев не на чем построить")
        rc = 1
    else:
        verdict("  OK пустой обход назван раньше состояния населения")
    clear_suite()

    print("=== население: суиты нет, обход НЕПУСТ, человеческих шагов нет ⇒ БЕСПРЕДМЕТНО ===")
    # Законный близнец пустого обхода: отличается РОВНО ОДНИМ фактом — в дереве есть
    # коллекция, и её листья прочитаны. Оба условия проверяются числами до вердикта,
    # иначе «человеческих шагов нет» стало бы неотличимо от «шагов нет вовсе».
    plant_quiet()
    res_q = scan(tmp, catalog)
    if res_q["leaves_read"] < 1 or res_q["human_steps"] != 0:
        verdict(f"  ПРОВАЛ близнец построен не тем: листьев {res_q['leaves_read']}, "
                f"человеческих шагов {res_q['human_steps']} — нужен непустой обход при "
                f"нулевом населении")
        rc = 1
    else:
        verdict(f"  OK близнец построен: листьев {res_q['leaves_read']}, "
                f"человеческих шагов {res_q['human_steps']}")
    rc_void, lines = decide(tmp)
    if rc_void != 0 or not said(lines, decl_live.VOID_MARK):
        verdict(f"  ПРОВАЛ беспредметность не объявлена (rc={rc_void}): {lines[-1:]}")
        rc = 1
    elif said(lines, "ПРОВАЛ"):
        verdict(f"  ПРОВАЛ беспредметное несёт отказ: {[l for l in lines if 'ПРОВАЛ' in l]}")
        rc = 1
    elif not said(lines, "население отбора:"):
        verdict("  ПРОВАЛ перепись населения не напечатана — «ноль» стало неотличимо "
                "от «не смотрели»")
        rc = 1
    else:
        verdict(f"  OK {decl_live.VOID_MARK} с переписью, код 0")

    print("=== население: каталог суиты ЕСТЬ, шагов в нём НЕТ, обход НЕПУСТ ⇒ ОТКАЗ ===")
    os.makedirs(suite_dir, exist_ok=True)
    rc_gut, lines = decide(tmp)
    if rc_gut != 2 or not said(lines, "исчезло при живой суите"):
        verdict(f"  ПРОВАЛ выпотрошенная суита не названа отказом (rc={rc_gut}): {lines[-2:]}")
        rc = 1
    elif said(lines, decl_live.VOID_MARK):
        verdict("  ПРОВАЛ выпотрошенная суита уехала в беспредметное и замолчала с ним")
        rc = 1
    elif said(lines, "обход пуст"):
        verdict("  ПРОВАЛ выпотрошенная суита выдана за пустой обход — тогда ветвь о "
                "суите неисполнима, и её отсутствие было бы неотличимо от молчания")
        rc = 1
    else:
        verdict("  OK выпотрошенная суита названа отказом, а не беспредметностью")
    clear_suite()
    ...
    if rc_back != 1 or not said(lines, "returned-undergraded"):
        verdict(f"  ПРОВАЛ дефект вернувшейся суиты не судится (rc={rc_back}): {lines[-2:]}")
        rc = 1
    elif said(lines, decl_live.VOID_MARK):
        verdict("  ПРОВАЛ вернувшаяся суита всё ещё объявлена беспредметной")
        rc = 1
    else:
        verdict("  OK дефект вернувшейся суиты назван находкой")
    ...
    if rc_ok != 0 or not said(lines, decl_live.PASS_MARK):
        verdict(f"  ПРОВАЛ законная суита не признана проверенной (rc={rc_ok}): {lines[-2:]}")
        rc = 1
    elif said(lines, decl_live.VOID_MARK):
        verdict("  ПРОВАЛ вердикт по живой суите объявлен беспредметным")
        rc = 1
    else:
        verdict(f"  OK {decl_live.PASS_MARK} по живой суите")
    clear_suite()
    ...
    if rc_out != 1 or not said(lines, "outside-undergraded"):
        verdict(f"  ПРОВАЛ шаг вне суиты не судится (rc={rc_out}): {lines[-2:]}")
        rc = 1
    elif said(lines, decl_live.VOID_MARK):
        verdict("  ПРОВАЛ шаг вне суиты накрыт беспредметностью")
        rc = 1
    else:
        verdict("  OK шаг вне суиты судится, послабление истекло")
    ...
    if rc_unm != 2 or not said(lines, "ничего не прочитано"):
        verdict(f"  ПРОВАЛ нулевое сопоставление при живом населении не названо отказом "
                f"(rc={rc_unm}): {lines[-2:]}")
        rc = 1
    else:
        verdict("  OK нулевое сопоставление при живом населении остаётся отказом")
    clear_outside()
    ...
    # человеческих шагов нет, обход НЕПУСТ — тихая коллекция на месте), и при нём
    # ломается УЖЕ СУЩЕСТВУЮЩЕЕ утверждение. Без этого прогона молчание предпосылок
    # было бы неотличимо от их смерти. Непустой обход здесь несущий: совпади случай
    # с пустым обходом, он утверждал бы порядок «предпосылка выше пустого обхода», а
    # не «предпосылка выше послабления», — то есть не тот предмет.
    ...
    if rc_mask != 2 or not said(lines, ROUTE_TABLE_REL):
        verdict(f"  ПРОВАЛ сломанная предпосылка накрыта беспредметностью (rc={rc_mask}): {lines}")
        rc = 1
    elif said(lines, decl_live.VOID_MARK):
        verdict("  ПРОВАЛ вердикт объявлен беспредметным при сломанной предпосылке")
        rc = 1
    else:
        verdict("  OK предпосылка судится ПЕРВОЙ и беспредметностью не гасится")
    os.remove(os.path.join(tmp, ROUTE_TABLE_REL))
    os.symlink(os.path.join(root, ROUTE_TABLE_REL), os.path.join(tmp, ROUTE_TABLE_REL))
    ...
    if not any("wave_population" in b for b in bad_pop):
        verdict(f"  ПРОВАЛ пропажа состояния населения прошла молча: {bad_pop}")
        rc = 1
    else:
        verdict("  OK «спросить нечем» названо предпосылкой, а не беспредметностью")
    os.remove(os.path.join(tmp, CEREMONY_DECL_REL))
    os.symlink(os.path.join(root, CEREMONY_DECL_REL), os.path.join(tmp, CEREMONY_DECL_REL))
    ...
    if not any(ROUTE_TABLE_REL in b for b in bad_routes):
        verdict(f"  ПРОВАЛ пропажа таблицы маршрутов прошла молча: {bad_routes}")
        rc = 1
    else:
        verdict("  OK пустая таблица маршрутов названа предпосылкой")
    os.remove(os.path.join(tmp, ROUTE_TABLE_REL))
    os.symlink(os.path.join(root, ROUTE_TABLE_REL), os.path.join(tmp, ROUTE_TABLE_REL))
    ...
    if not any("jwtHumanCeremonyStepUp" in b for b in bad):
        verdict(f"  ПРОВАЛ исчезновение предъявителя не замечено: {bad}")
        rc = 1
    else:
        verdict("  OK предпосылка проверяется и падает")

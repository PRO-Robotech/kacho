...
# is_our_image <образ> — образ продукта или чужой? Судится по ПОСЛЕДНЕМУ сегменту
# ссылки, а не по всей строке: `docker.io/library/kacho-vpc:dev` и
# `prorobotech/kacho-vpc:<тег>` — наши, `docker.io/bitnami/postgresql:16.4.0`,
...
is_our_image() {
  case "${1##*/}" in kacho-*) return 0 ;; *) return 1 ;; esac
}

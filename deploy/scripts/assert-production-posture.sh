...
# должно быть. У vpc механизма тоже нет, но по ДРУГОЙ причине — он принимает
# переданную личность и никого не пинит. Это открытый долг, а не норма: как только
# сервис получит ручку и стражу, он добавляется сюда, и с этого момента его посадка
# градуируется. Пустой список отключает проверку целиком.
#
...
FORWARDER_NARROWING_REQUIRED="${FORWARDER_NARROWING_REQUIRED:-geo compute nlb storage registry iam}"

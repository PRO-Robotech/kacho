...
#   POSTURE_PROFILE=dev — ТОЛЬКО измерение круга отправителей (A).
#
# ДОВОД СУЖЕНИЯ ПЕРЕЖИЛ СВОЙ ПРЕДМЕТ — сказано здесь, а не умолчано (задача #2390).
#
# Здесь стояло: «dev-профиль осознанно расслабляет auth_mode / db_sslmode, и
# требовать их здесь значило бы красить гейт на каждом dev-стенде до полной
# бесполезности». Это было верно, пока перевод в боевую посадку жил только в
# `dev-prod-up`. Он переехал в рецепт `dev-up` (см. §«ПОЧЕМУ ФАЗА 3 ПЕРЕЕХАЛА» в
# Makefile), поэтому стенд, поднятый `dev-up`, названных величин НЕ расслабляет:
# замер по цепочке, которой подъём заканчивается, даёт боевую посадку у ВСЕХ
# служб с ручкой посадки, а живой процесс объявляет о себе `auth_mode=
# production-strict, db_sslmode=require`.
#
# Следствие, названное прямо: стенд шарда сквозных проб поднимает боевую посадку
# и проверяет из четырёх измерений ОДНО. Свойство есть, держателя ЖИВОГО процесса
# у него на этом стенде нет — объявление держит гейт дерева
# (deploy/stand_posture_terminal_stack_test.go), процесс не держит никто.
#
# Почему сужение снимается не здесь: вердикт даёт только подъём, и цена ему —
# один полный `dev-up` на каждый из пяти шардов. Что именно надо измерить до
# правки (свежесть самоотчёта, ось own_rest_*_tls из #2335, POSTURE_SKIP шардов,
# измерение D на шарде) — в #2390. Это ПРЕДМЕТ с предикатом снятия, а не отсрочка:
# сужение остаётся ровно до его закрытия.
#

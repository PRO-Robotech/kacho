...
# должно быть. У vpc и iam механизма тоже нет, но по ДРУГОЙ причине — они
# принимают переданную личность и никого не пинят. Это открытый долг, а не норма:
# как только сервис получит ручку и стражу, он добавляется сюда, и с этого момента
# его посадка градуируется. Пустой список отключает проверку целиком.
#
...
FORWARDER_NARROWING_REQUIRED="${FORWARDER_NARROWING_REQUIRED:-geo compute nlb storage registry}"

...
#
# ОБЛАСТЬ ИСПОЛНЕНИЯ: ПРОФИЛЬ ОДИН, И СУЖЕНИЯ БОЛЬШЕ НЕТ (задача #2390).
#
# ЗДЕСЬ БЫЛО ДВА ПРОФИЛЯ. `production` градуировал все четыре измерения,
# `dev` — только круг отправителей чужой личности, и сужение обосновывалось
# дословно так: «dev-профиль осознанно расслабляет auth_mode / db_sslmode, и
# требовать их здесь значило бы красить гейт на каждом dev-стенде до полной
# бесполезности».
#
# ДОВОД ПЕРЕЖИЛ СВОЙ ПРЕДМЕТ. Фаза боевой посадки переехала в рецепт `dev-up`
# (§«ПОЧЕМУ ФАЗА 3 ПЕРЕЕХАЛА» в deploy/Makefile), и расслаблять стало нечего:
# каждая цепочка `deploy/stacks.txt` ЗАКАНЧИВАЕТСЯ профилем, объявляющим боевую
# посадку, а живой процесс на поднятом стенде объявляет о себе
# `auth_mode=production-strict, db_sslmode=require`.
#
# Следствие сужения было названо прямо и стоило задачи: стенд шарда сквозных
# проб поднимал боевую посадку и проверял из четырёх измерений ОДНО. Свойство
# было, держателя ЖИВОГО процесса у него не было — объявление держал гейт дерева
# (deploy/stand_posture_terminal_stack_test.go), процесс не держал никто.
#
# Профиль снят ВМЕСТЕ с предметом, а не оставлен «на всякий случай»: ветвь,
# которую никто не выбирает, отличается от исправной проверки только тем, что
# её молчание нечем объяснить. Узкое измерение при этом не потеряно — круг
# отправителей градуируется независимо от посадки (`needs_forwarder_narrowing`
# ниже), и именно поэтому боевой профиль его тоже спрашивает.
#
# ЧТО ЭТО СТОИТ НА СТЕНДЕ ШАРДА. Ни одного лишнего подъёма: `dev-prod-up` не
# накладывает поверх `dev-up` НИ ОДНОЙ фазы helm — он лишь добавляет гейты,
# поэтому стенд, который шард уже поднимает, и есть тот стенд, на котором эти же
# четыре измерения зелены у полного стенда. Прибавляются секции B, C и D, а не
# подъём.
#
# POSTURE_SKIP обязателен для стенда, поднимающего ПОДМНОЖЕСТВО служб: перечень
# ожидаемых ниже ЖЁСТКИЙ намеренно, и отсутствие службы в кластере исключением
# не является.
# ─────────────────────────────────────────────────────────────────────────────
set -uo pipefail
...
# ПРОФИЛЬ ОДИН. Ручка сохранена и ОТВЕРГАЕТ всякое значение, кроме боевого:
# снятый профиль, принимаемый молча, неотличим от работающего — вызывающий,
# у которого в окружении остался `dev`, получил бы боевой гейт и решил бы, что
# сузил его. Отказ называет, что случилось.
POSTURE_PROFILE="${POSTURE_PROFILE:-production}"
case "$POSTURE_PROFILE" in
  production) ;;
  dev) echo "!!! POSTURE_PROFILE=dev СНЯТ (задача #2390): профиль градуировал одно измерение из четырёх," \
            "и его довод — «стенд расслабляет посадку» — пережил предмет: подъём заканчивается боевой" \
            "посадкой. Уберите переменную; стенд, поднимающий подмножество служб, называет отсутствующие" \
            "через POSTURE_SKIP."; exit 2 ;;
  *) echo "!!! POSTURE_PROFILE='$POSTURE_PROFILE' — допустимо только production"; exit 2 ;;
esac
...
# случая (см. corelib/observability/bootposture.go). У api-gateway измерения нет вовсе —
  ...
  for p in $pods; do
    ...
    verdict="$(printf '%s' "$line" | jq -r --argjson need_fwd "$need_fwd" '
      ...
      [ (if ((.auth_mode // "") | test("^production(-strict)?$")) then empty
        ...
        (if (.own_rest_public_tls == "true") or (.own_rest_public_tls == "n/a") then empty
         else "own_rest_public_tls=\(shown("own_rest_public_tls"))" end),
        (if (.own_rest_internal_tls == "true") or (.own_rest_internal_tls == "n/a") then empty
    ...
    if [ -n "$verdict" ]; then
      ...
      # Круг отправителей — измерение, от посадки НЕ зависящее, поэтому его
      # отказ разбирается отдельно: пустой список не означает «не принимаем ни
      # от кого» — транспорт сужает круг только на непустом, а на пустом
      # принимает переданную личность от ЛЮБОГО пира с сертификатом внутреннего
      # CA. Круг заполняется по ФАКТИЧЕСКИМ отправителям (граф импортов службы),
      # затем под перекатывается: ручка читается один раз, на старте процесса.
      case "$verdict" in
        *trusted_forwarders=*)
          echo "      КРУГ ОТПРАВИТЕЛЕЙ ЧУЖОЙ ЛИЧНОСТИ НЕ СУЖЕН — заполнить его в профиле"
          echo "      стенда по ФАКТИЧЕСКИМ отправителям и перекатить под." ;;
      esac
    else
...
#
# Здесь стояло «только боевой профиль: dev-профиль держит pg без TLS». Профиля
# `dev` больше нет (#2390), и держать pg без TLS больше некому: каждая цепочка
# подъёма заканчивается боевой посадкой.
# ─────────────────────────────────────────────────────────────────────────────
...
#
# Здесь стояло «только боевой профиль: dev-профиль несёт эти значения ЗАКОННО».
# Профиля `dev` больше нет (#2390), и законного носителя у `mode: dev` и
# `sslmode=disable` в дереве не осталось ни одного.
# ─────────────────────────────────────────────────────────────────────────────

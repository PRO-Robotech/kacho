...
#     append-and-retain (drainer помечает sent_at, строк не удаляет), поэтому
...
#
# Последняя колонка — известный и НАЗВАННЫЙ пробел, а не молчание: обе таблицы
# iam клеймятся тем же дренажем и так же append-and-retain, но своей миграции с
# per-table порогом у них нет. Это чинится в kacho-iam (миграция вида 0064), а

...
// объявления полосы не является — ни у одного из двух читателей: у обоих он
// приезжает переменной окружения РАБОЧЕГО ОБЪЕКТА (`SSL_CERT_FILE` у
// поставщика, `KANAME_INVITE_MAIL__CA_BUNDLE_FILE` у нас), и что он выдан
// обоим на каждом стенде с поднимаемым приёмником, судит
// `identity_mail_sender_trust_anchor_test.go`. Решение Р23 называет три
// величины — узел, адрес отправителя и удостоверение, — и этот гейт судит
// ровно их питание.
//

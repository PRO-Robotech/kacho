...
//	    Проверяется: цепочка обязана объявлять посадку личности
//	    (identityChainLandsIdentity) И доводить наши настройки до процесса
...
var identityReplacedListDecisions = map[string]string{
	...
	// ЗДЕСЬ СТОЯЛИ `secrets.cookie` и `secrets.cipher` — СНЯТЫ ВМЕСТЕ С ПРЕДМЕТОМ
	// (#1733). Наш файл этих ключей больше не объявляет, поэтому решать нечего, и
	// гейт самоистечения потребовал снятия записей САМ — ровно как задуман.
	//
	// Стоит назвать и вторую половину: обоснование этих записей было НЕВЕРНЫМ.
	// Они утверждали, что наша запись «подставляет настоящий секрет из окружения
	// пода». Подстановки этих величин в поде нет вовсе — init-контейнер
	// подставляет ТОЛЬКО величину обратного вызова, — а сверх того чарт
	// поставщика выставляет `SECRETS_COOKIE`/`SECRETS_CIPHER` переменными, и
	// переменная перебивает файл. То есть запись не подставляла ничего и ни на
	// что не влияла: решала переменная. Владелец этих величин ОДИН — секрет
	// чарта поставщика.
...
func TestIdentityReplacedListsAreDecided(t *testing.T) {
	...
	for _, name := range names {
		...
		// П1: без объявленной посадки личности и без доведения наших настроек
		// до процесса замещения не происходит — предмета нет, и молчание здесь
		// законно.
	...
	t.Logf("перепись: стеков %d · объявляют посадку личности %d · доводят наши настройки %d · "+

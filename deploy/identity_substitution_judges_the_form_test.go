...
//
//	value: "KANAME_HOOK_TOKEN{{ if … }} KACHO_IDENTITY_SMTP_CREDENTIAL{{ end }}"
//
...
func TestSubstitutedListLiteralKnowsBothForms(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			name:    "условный хвост",
			literal: "KANAME_HOOK_TOKEN{{ if $mailCredDeclared }} KACHO_IDENTITY_SMTP_CREDENTIAL{{ end }}",
			want:    []string{"KANAME_HOOK_TOKEN", "KACHO_IDENTITY_SMTP_CREDENTIAL"},
		},
		{
	...
	rawFields := uniqueSorted(strings.Fields(
		"KANAME_HOOK_TOKEN{{ if $mailCredDeclared }} KACHO_IDENTITY_SMTP_CREDENTIAL{{ end }}"))
	if len(rawFields) <= 2 {

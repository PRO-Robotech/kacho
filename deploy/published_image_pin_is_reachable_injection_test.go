...
// предел реестра: на сорока семи ссылках анонимный Docker Hub начинает отвечать
// `429`, и часть ссылок остаётся несверенной. СКОЛЬКО именно — наблюдение одного
// прогона, а не замер (порог зависит от того, сколько обращений машина сделала до
// него), поэтому числом здесь не стоит: его печатает перепись гейта на каждом
// прогоне. Детерминированно измеряется другое — ЧИСЛО ОБРАЩЕНИЙ, и его перепись
// печатает вместе с обоими множителями.
//
// Память — оптимизация, и оптимизация обязана быть проверена с той стороны, с
// которой она могла бы стать послаблением: закрытый репозиторий обязан давать «не
// выполнилось» КАЖДОЙ своей ссылке, а не только первой.
...
// TestTallyByComponentCountsTheUnitItNames — перепись по компонентам считает ту
// единицу, которую называет, и различает выключенное измерение от чистого.
//
// ПРЕДМЕТ. Перепись заведена затем, чтобы всякое число о компонентах в этом файле
// имело производителя: прежняя шапка называла «одиннадцать ссылок у семи
// компонентов» там, где компонентов восьми, и разошлась молча. Значит и самой
// переписи нужно доказательство — иначе она такой же итог прозой, только
// напечатанный.
//
// ЧТО ИМЕННО ОНА МОГЛА БЫ СОЛГАТЬ, ВЫГЛЯДЯ РАБОЧЕЙ, — три вещи, и каждая проверена
// отдельным фактом:
//
//	приписать ссылки одного образа другому   → строк станет столько же, а числа поедут;
//	расщепить один образ на две строки       → компонентов станет больше, чем есть;
//	зачесть неспрошенное как сверенное       → «сверено 0» перестанет отличаться от
//	                                           «сверено и чисто».
func TestTallyByComponentCountsTheUnitItNames(t *testing.T) {
	const (
		kaname1 = "docker.io/prorobotech/kaname:main-be5a45a2"
		kaname2 = "docker.io/prorobotech/kaname:main-7bb0a6e1"
		vpcRef  = "docker.io/prorobotech/kacho-vpc:main-d941344b"
	)
	// ЗАКОННЫЙ БЛИЗНЕЦ ВСТРОЕН ВО ВХОД: один образ (`kaname`) запинен ДВУМЯ
	// стендами разными тегами. Это норма дерева, и перепись обязана дать по нему
	// ОДНУ строку с двумя ссылками — а не две строки.
	where := map[string][]pinnedImage{
		kaname1: {
			{Stand: "dev", Component: "kaname.image", Repo: "docker.io/prorobotech/kaname", Tag: "main-be5a45a2"},
			{Stand: "prod", Component: "kaname.image", Repo: "docker.io/prorobotech/kaname", Tag: "main-be5a45a2"},
		...
		kaname2: {
			{Stand: "a8f60d", Component: "kaname.image", Repo: "docker.io/prorobotech/kaname", Tag: "main-7bb0a6e1"},
		},
		vpcRef: {
			{Stand: "prod", Component: "vpc.image", Repo: "docker.io/prorobotech/kacho-vpc", Tag: "main-d941344b"},
		},
	...
	// (а) ИЗМЕРЕНИЕ ПРОШЛО: одна ссылка kaname без манифеста, остальные тянутся.
	got := tallyByComponent(where, []pinAnswer{
		{Ref: kaname1, Answer: answerAbsent, Why: "манифест → 404"},
		{Ref: kaname2, Answer: answerPresent},
		{Ref: vpcRef, Answer: answerPresent},
	})
	if len(got) != 2 {
		t.Fatalf("компонентов %d, ожидалось два (kaname и kacho-vpc): %+v", len(got), got)
	}
	if got[0].Image != "kacho-vpc" || got[1].Image != "kaname" {
		t.Errorf("перепись не упорядочена по имени образа: %+v", got)
	}
	want := map[string]componentTally{
		"kaname":    {Image: "kaname", Refs: 2, Checked: 2, Absent: 1},
		"kacho-vpc": {Image: "kacho-vpc", Refs: 1, Checked: 1},
	}
	for _, row := range got {
		if row != want[row.Image] {
			t.Errorf("строка %s = %+v, ожидалось %+v — числа переписи поехали, а строк "+
				"столько же: одно число этого не показало бы", row.Image, row, want[row.Image])
		}
	}

	// (б) ОДИН ИЗМЕНЁННЫЙ ФАКТ — измерение ВЫКЛЮЧЕНО (ответов нет). Ссылки
	//     по-прежнему сосчитаны, а сверок ноль: «сверено 0» обязано отличаться от
	//     «сверено и чисто», иначе выключенная ручка читается как зелёный вердикт.
	off := tallyByComponent(where, nil)
	if len(off) != 2 {
		t.Fatalf("при выключенном измерении компонентов %d вместо двух: %+v", len(off), off)
	}
	for _, row := range off {
		if row.Refs == 0 {
			t.Errorf("%s: при выключенном измерении потеряны и ССЫЛКИ — тогда перепись "+
				"перестаёт быть объёмом осмотренного", row.Image)
		}
		if row.Checked != 0 || row.Absent != 0 || row.Unresolved != 0 {
			t.Errorf("%s: неспрошенное зачтено как сверенное (%+v) — вердикт выключенной "+
				"ручки неотличим от чистого", row.Image, row)
		}
	}

	// (в) ВТОРОЙ ИЗМЕНЁННЫЙ ФАКТ — спросить не удалось. Ни сверкой, ни находкой.
	unres := tallyByComponent(where, []pinAnswer{
		{Ref: kaname1, Answer: answerUnresolved, Why: "репозиторий не читается"},
		{Ref: kaname2, Answer: answerUnresolved, Why: "токен не получен"},
		{Ref: vpcRef, Answer: answerPresent},
	})
	for _, row := range unres {
		if row.Image == "kaname" && (row.Unresolved != 2 || row.Checked != 0 || row.Absent != 0) {
			t.Errorf("kaname: %+v — несостоявшееся измерение смешано со сверкой", row)
		}
	}

	// (г) ПУСТОЙ ВХОД — ноль компонентов. Именно это гейт превращает в отказ
	//     предпосылки: перепись, молчащая на пустом обходе, была бы вакуумной.
	if n := len(tallyByComponent(map[string][]pinnedImage{}, nil)); n != 0 {
		t.Errorf("на пустом входе перепись назвала %d компонентов — «ноль прочитанного» "+
			"выдано за «ноль находок»", n)
	}
}

// TestPublishedPinCensusCountsRepositoriesNotImages — множитель числа обращений к
// реестру — РЕПОЗИТОРИЙ, а не образ, и перепись обязана считать именно его.
//
// ПРЕДМЕТ. Шапка `registryProbe` называла «141 запрос к 65». 141 верно (3×47), 65
// — ни при какой единице: оно выводится из девяти репозиториев, а их семнадцать.
// Ошибка держалась потому, что число считалось в уме, а не печаталось. Теперь его
// печатает перепись, и множителей у неё два; здесь доказывается, что второй — это
// репозиторий, и что образ на его месте дал бы ДРУГОЕ число.
//
// В этом дереве образов и репозиториев поровну (все под одним владельцем), поэтому
// подмена сегодня незаметна — ровно тот случай, когда доказательство обязано
// подать вход, на котором две единицы РАСХОДЯТСЯ.
func TestPublishedPinCensusCountsRepositoriesNotImages(t *testing.T) {
	// Один образ `console` под ДВУМЯ владельцами — две ссылки, два репозитория,
	// один образ. Единица «образ» насчитала бы здесь один множитель вместо двух.
	pins := []pinnedImage{
		{Stand: "dev", Component: "a.image", Repo: "docker.io/prorobotech/console", Tag: "main-1"},
		{Stand: "dev", Component: "b.image", Repo: "docker.io/foreign/console", Tag: "main-1"},
	}
	...
	if len(images) != 1 {
		t.Fatalf("образов %d вместо одного — вход доказательства не расходит единицы", len(images))
	}
	// Спрашивается САМ СЧЁТЧИК ГЕЙТА, а не повторная арифметика доказательства:
	// иначе проба утверждала бы о своём выражении, а подмена единицы в гейте
	// осталась бы зелёной — что и наблюдалось при заведении этой пробы.
	repos := distinctRepositories(pins)
	if repos != 2 {
		t.Fatalf("distinctRepositories вернул %d вместо двух — счётчик гейта считает не "+
			"репозитории, а что-то другое (на сегодняшнем дереве обе единицы совпадают, "+
			"поэтому подмена не меняет ни одного печатаемого числа)", repos)
	}
	// Формула переписи — та же, что исполняет askRegistry: маркер и управляющий
	// вопрос по разу на репозиторий, манифест — на каждую ссылку.
	byRepo := 2*repos + len(pins)
	byImage := 2*len(images) + len(pins)
	if byRepo == byImage {
		t.Fatalf("на этом входе обе единицы дают %d — подмена множителя осталась бы "+
			"незаметной, и доказательство ничего не доказывает", byRepo)
	}
	if byRepo != 6 {
		t.Errorf("по репозиториям обращений %d, ожидалось 6 (2×2+2)", byRepo)
	}
}

...
// его как непонятный.
//
// ТЕГА `helmcharts` ЗДЕСЬ БОЛЬШЕ НЕТ, И ЭТО НЕ ОСЛАБЛЕНИЕ. Он стоял по одной
// причине: архивы ЧУЖИХ чартов не отслеживались git, значит условие проверки
// создавало не всякое задание, а только то, что их материализует. Задача #2256
// вендорила внешние архивы точным пином — `kratos-*.tgz` лежит в дереве после
// обычного клона, — и причина исчезла. Проверка вернулась в общий прогон:
// держать её под тегом значило бы объяснять раскол фактом, которого нет.
//
// Условие проверки при этом НЕ ослабло: архив по-прежнему обязан читаться, и
// его отсутствие остаётся отказом (см. chartArchiveMember ниже), а не пропуском.
// Отличие ровно одно — отказ теперь наступает в общем прогоне, а не только в
// задании, материализующем зависимости.
//
// Тег остался у проверок, которые РЕНДЕРЯТ умбреллу: их условие создаётся не
// вендорингом, а `helm dependency build`, потому что ЛОКАЛЬНЫЕ сабчарты
// (vpc, compute, kacho-nlb, api-gateway, uif, registry, storage) собираются из
// исходников и в git не вендорятся осознанно. Что у того раскола остался
// предмет — утверждает TestUmbrellaRenderStillNeedsMaterializedDeps в
// identity_chart_premise_reachability_test.go.
package deploy_test
...
func chartArchiveMember(t *testing.T, archive, suffix string) (string, string) {
	...
	if err != nil {
		t.Fatalf("архив чарта %s не читается (%v) — предпосылка проверки исчезла, "+
			"а не раздача настроек стала симметричной. Архив внешнего чарта ВЕНДОРЕН "+
			"(#2256) и обязан лежать в дереве после обычного клона", archive, err)
	}
	defer f.Close()

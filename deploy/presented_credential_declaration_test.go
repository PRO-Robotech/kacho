...
// величины; профиль дев-посадки без своей чеканки — не обязан.
//
// # Пара, на которой держится утверждение
//
// Различие ЖИВОЕ и решает его профиль, а не сборка: боевые профили включают
// читателя, УМОЛЧАНИЕ ЧАРТА — нет. Без второй половины «профиль включает»
// зеленело бы на чарте, включающем читателя ВСЕГДА, — то есть на том, что
// KAN-LAND-01 как раз и запрещает.
//
// Второй половиной здесь прежде служил профиль первой фазы подъёма
// (`values.dev.yaml`): он чеканки не объявлял, и читателя у него не было. Под
// посадкой `own` такой фазы нет — без своей чеканки у публикатора набора ключей
// не остаётся ни одной записи, и служба отказывает в старте (#2735), поэтому
// первая фаза чеканит и попадает в первую половину. Вторая половина — ТО, ЧТО
// ПРОФИЛЬ ПЕРЕКРЫВАЕТ: умолчание чарта, прочитанное здесь же, а не выписанное.
// Дев-профили без чеканки по-прежнему судятся (читатель на них — находка), но
// их число вправе быть нулём.
func TestKAN_LAND_02_EveryProductionProfileRaisingTheFrontDeclaresTheReader(t *testing.T) {
	...
	// Вторая половина пары — умолчание чарта, которое боевые профили перекрывают.
	// Включённое или нечитаемое умолчание делает «включение решает профиль»
	// ничем не доказанным.
	chartDefault := readYAML(t, filepath.Join(umbrellaDir, "charts", "kaname", "values.yaml"))
	pc, _ := dig(chartDefault, "config", "authn", "presentedCredential").(map[string]any)
	if pc == nil {
		t.Fatal("перепись беспредметна: у чарта нет узла config.authn.presentedCredential — " +
			"вторая половина пары пуста, и «включение решает профиль» ничем не доказывается")
	}
	if on, isBool := pc["enabled"].(bool); !isBool || on {
		t.Fatalf("вторая половина пары не держится: умолчание чарта presentedCredential.enabled=%v "+
			"— не выключенный bool, и «включение решает профиль» ничем не доказывается", pc["enabled"])
	}
	t.Logf("вторая половина пары: умолчание чарта presentedCredential.enabled=false · "+
		"дев-профилей без чеканки %d", devPosture)
}

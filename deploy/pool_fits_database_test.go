...
var knownUnfitting = map[string]string{
	"a8f60d/kaname/слагаемое вне пула не выведено":      "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
	"dev/kaname/слагаемое вне пула не выведено":         "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
	"dev-prod/kaname/слагаемое вне пула не выведено":    "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
	"fe3455/kaname/слагаемое вне пула не выведено":      "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
	"prod/kaname/слагаемое вне пула не выведено":        "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
	"own/kaname/слагаемое вне пула не выведено":         "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
	"prorobotech/kaname/слагаемое вне пула не выведено": "исходники службы доступа вынесены отдельным репозиторием, а её чарт остаётся подчартом зонта: слагаемое вне пула выводится из ДЕРЕВА, и вывести его в этом дереве нечем. Запись объявляет пробел, а не прощает несоответствие — арифметика пула kaname против pg-iam не считается теперь НИ В ОДНОМ дереве: чарт здесь, код там. Истечёт сама, как только чарт уедет вслед за кодом либо код вернётся",
}

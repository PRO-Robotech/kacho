...
# ─── Почтовая полоса СНЯТА с этой конфигурации (задача #943) ──────────────────
#
# Здесь объявлялся `courier.smtp`. Читателя у объявления не было: письма шлёт
# ОТДЕЛЬНЫЙ процесс, и он работает на конфигурации подчарта поставщика —
# указание на наш файл приезжает из ручки основного рабочего объекта, а
# почтовый берёт свою, и там его нет.
#
# Замер на живом стенде, а не рассуждение: почтовый процесс запускается с
# `--config /etc/config/kratos.yaml` — файлом ПОСТАВЩИКА, — и получает адрес
# почты переменной окружения. Наш раздел не участвовал ни в одной отправке.
#
# ПОЧЕМУ СНЯТО, А НЕ ПОДКЛЮЧЕНО. Исходов было два, и выбор продуктовый: научить
# почтовый процесс читать наш файл значит взять на себя политику отправки писем
# на ВСЕХ стендах и оплатить это прогоном сквозных проб восстановления доступа.
# Почтой мы не управляем: действующие значения живут в слое поставщика, и там у
# них читатель есть.
#
# Объявление без читателя — «принято-и-проигнорировано» на уровне настроек: со
# стороны выглядит как «почта настроена нами», а настроена она не нами. Снятие
# делает объявление честным и не трогает ни одной отправки.
#
# ЧТО ВЕРНЁТ ЕГО ОБРАТНО: решение управлять отправкой писем из нашего слоя.
# Тогда раздел заводится ВМЕСТЕ с указанием почтовому процессу читать наш файл
# и с прогоном полосы восстановления — а не вперёд них.

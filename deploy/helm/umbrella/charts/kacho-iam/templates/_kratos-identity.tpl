...
{{- $mailURI := toString ($mailSmtp.connectionURI | default "") }}
  ...
  args:
    - |
      ...
      for n in $KACHO_IDENTITY_SUBSTITUTED_VARS; do
        ...
        if [ "$v" = "$n" ]; then
          echo "ОТКАЗ: величина \${$n} равна собственному ИМЕНИ переменной — источник подставил имя вместо содержимого (Secret заведён строкой имени либо ссылка не разрешилась). Отправленная такой, она была бы отвергнута принимающей стороной" >&2
          exit 1
        fi
      ...
      if [ -n "$bare" ]; then
        echo "ОТКАЗ: в отрендеренной конфигурации имя переменной стоит ГОЛЫМ ИМЕНЕМ, без скобок:$bare. Это НЕ ссылка и подстановкой не лечится — строка уедет в заголовок дословно и будет отвергнута принимающей стороной кодом 401, а стенд при этом будет выглядеть поднятым. Ищите её в исходнике настроек: там она записана без \${…}" >&2
        exit 1
      fi

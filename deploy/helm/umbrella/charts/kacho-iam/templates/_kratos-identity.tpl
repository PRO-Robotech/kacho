...
- name: identity-config-render
  ...
  args:
    - |
      ...
      # законно и обязано остаться), оставшиеся ссылки формы `${ИМЯ}` снимаются
      # тоже (их предмет — проверка выше, и здесь они дали бы второе обвинение об
      # одном). В остатке ищется имя ЦЕЛЫМ СЛОВОМ: `${ИМЯ}` этим поиском уже не
      # ловится, а `ИМЯ`, стоящее значением, — ловится.
      #
      ...
      bare=""
      for n in $judged; do
        if sed 's/#.*$//; s/\${[A-Za-z_][A-Za-z0-9_]*}//g' "$out" | grep -qw -- "$n"; then
          bare="$bare $n"
        fi
      done
      if [ -n "$bare" ]; then
        ...
        for n in $bare; do
          eval "v=\${$n-}"
          insrc=$(grep -cw -- "$n" "$src" 2>/dev/null || true)
          case "$v" in
            *"$n"*)
            ...
            *)
              echo "  ${n}: в исходнике настроек строк с этим именем — ${insrc:-0}; величина имени не содержит (длина $(printf %s "$v" | wc -c), отпечаток $(printf %s "$v" | md5sum | cut -c1-12))" >&2 ;;
          esac
          sed 's/#.*$//; s/\${[A-Za-z_][A-Za-z0-9_]*}//g' "$out" | grep -nw -- "$n" | head -5 > "$out.hits" || true
        ...
        echo "ОТКАЗ: в отрендеренной конфигурации имя переменной стоит ГОЛЫМ ИМЕНЕМ, без скобок:$bare. Это НЕ ссылка и подстановкой не лечится — строка уедет в заголовок дословно и будет отвергнута принимающей стороной кодом 401, а стенд при этом будет выглядеть поднятым. Где именно — названо строками выше: они говорят, пришло ли имя из исходника настроек или его принесла сама величина" >&2
        exit 1
      fi

...
//
// # ЗДЕСЬ СТОЯЛО «у семейства службы доступа такой неоднозначности сегодня нет»
//
// Оговорка сама называла своё истечение: «предикат назван ниже, и он же обязан покраснеть,
// когда она появится». Она появилась, и он покраснел — служба стала называть на
// опубликованной странице ряды своей витрины, а они носят ту же приставку
// (`kaname_authz_check_duration_seconds`, `kaname_lro_inflight`, `kaname_db_pool_idle_conns`;
// замер на день переезда: токенов на страницах 104, из них рядов витрины 52 — половина
// населения).
//
// Исход выбран тот, который оговорка и предписывала: предикат ПЕРЕЕХАЛ вместе с
// неоднозначностью. Кандидат больше не судится как тип, когда дерево ОБЪЯВЛЯЕТ его рядом
// витрины; откуда берётся это второе значение приставки и чем удержана его точность —
// `metric_series_names_test.go`. Ни списка прощённых токенов, ни послабления по странице
// здесь нет: первый пережил бы свой предмет, второе замаскировало бы и будущий фантом.
func TestPagesNameOnlyExistingAccessTypes(t *testing.T) {
	...
	// ВТОРОЕ значение приставки — имя ряда витрины. Собирается из объявлений дерева, а не
	// из списка; его непустоту и непересечение с реестром типов держит соседняя проба.
	series, seriesFiles, err := productMetricSeriesNames(root)
	if err != nil {
		t.Fatalf("сбор имён рядов витрины: %v", err)
	}
	switch {
	case !productnaming.SourcesInThisTree("iam"):
		// Второе значение приставки объявляет сама служба, а она вынесена
		// отдельным репозиторием (задача #1111). Вычитание здесь не
		// производится, и следствие названо прямо: страница, назвавшая РЯД
		// ВИТРИНЫ, будет осуждена как назвавшая несуществующий ТИП. Это
		// осознанный размен — строгость вместо списка прощённых токенов,
		// который пережил бы свой предмет молча.
		//
		// Замер, на котором размен принят: `kaname_*`-токенов на страницах
		// дерева девять, и все девять — имена типов провайдера, ни одного
		// имени ряда. Появится ряд на странице — гейт назовёт его, и это
		// правильный исход: смотреть будет человек, а не молчание.
		t.Logf("второе значение приставки в этом дереве не собирается: исходники " +
			"службы вынесены отдельным репозиторием, каталога объявлений рядов нет. " +
			"Кандидаты судятся ТОЛЬКО как имена типов")
	case seriesFiles == 0 || len(series) == 0:
		t.Fatal("второе значение приставки не собрано — вычитание вакуумно, и ряды витрины " +
			"судились бы как типы")
	}
	...
	var naming, checked, asSeries int
	for _, path := range pages {
		...
		for _, tok := range found {
			...
			if isProductMetricSeries(series, tok) {
				asSeries++
				continue
			}
	...
	// Вычитание не вправе съесть ВСЮ популяцию: тогда «фантомов нет» означало бы «типов
	// больше не судят», а не «типы названы верно».
	if checked-asSeries == 0 {
		t.Fatal("после вычитания рядов витрины не осталось ни одного кандидата в типы — " +
			"вычитание съело всю популяцию, и проба перестала утверждать что-либо о типах")
	}
	t.Logf("осмотрено: страниц %d, из них называют приставку службы доступа %d, токенов "+
		"сверено %d (из них рядами витрины %d, типами %d), типов в реестре %d, рядов витрины %d",
		len(pages), naming, checked, asSeries, checked-asSeries, len(known), len(series))
}

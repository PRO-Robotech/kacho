...
var geoRegionCatalog = catalogSpec{
	...
	fields: []dsField{
		...
		{name: "name", kind: dsString, doc: "Человекочитаемое имя региона."},
	...
	fields: []dsField{
		...
		{name: "region_id", kind: dsString,
			doc: "Регион, которому принадлежит зона. Берите регион ОТСЮДА, а не отрезанием " +
				"суффикса от имени зоны: имена региона и зоны — произвольные строки, и связи " +
				"между ними по написанию нет."},
		{name: "name", kind: dsString, doc: "Человекочитаемое имя зоны."},
		{name: "created_at", kind: dsString, doc: "Момент создания по данным края."},
		{name: "open_for_placement", kind: dsBool,

...
func TestApplyInstance_AbsentOptionalsStayNull(t *testing.T) {
	...
	t.Run("край не вернул группу — атрибут остаётся null", func(t *testing.T) {
		m := instanceModel{PlacementGroupID: types.StringNull(), ServiceAccountID: types.StringNull()}
		applyInstance(ctx, &m, &instanceJSON{ID: "ins-1", Name: "м"})
	...
	t.Run("край вернул значения — они доезжают", func(t *testing.T) {
		m := instanceModel{}
		applyInstance(ctx, &m, &instanceJSON{
			ID: "ins-1", Name: "м", PlacementGroupID: "plg-x",

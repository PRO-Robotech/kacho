...
// asyncResource — общий каркас ресурса Kachō.
//
// Все мутации платформы асинхронны и устроены ОДИНАКОВО: запрос возвращает операцию,
// операция дожидается, из её метаданных берётся идентификатор. Одинаково устроены и
// чтение (одиночное «не найдено» ничего не устанавливает), и обновление (маска строится
// из разницы, пустая не отправляется).
//
// Каркас вынесен, чтобы восемь ресурсов не повторяли эту механику восемь раз. Повторение
// опасно не объёмом, а расхождением: девятый ресурс завели бы копированием, и одна из
// копий однажды забыла бы проверку отказа операции — ту самую, что отделяет настоящий
// идентификатор от фантомного.
//
// Специфика ресурса живёт в интерфейсе resourceKind: как собрать тело, как разложить
// ответ, что считать изменившимся.
type resourceKind[M any] interface {
	// typeName — имя ресурса в конфигурации, например "kacho_iam_project".
	typeName() string
	// collectionPath — путь коллекции у края, например "/iam/v1/projects".
	collectionPath() string
	// idField — имя поля идентификатора в метаданных операции, например "projectId".
	idField() string
	// idPrefix — префикс идентификатора из общего каталога платформы (pkg/ids).
	idPrefix() string

	// createBody — тело создания из плана.
	createBody(ctx context.Context, plan *M) (proto.Message, error)
	// updateBody — тело и маска из разницы плана и состояния; пустая маска означает
	// «этим запросом менять нечего», и запрос не отправляется.
	updateBody(ctx context.Context, plan, state *M) (proto.Message, []string, error)
	// apply — разложить ответ края в модель.
	apply(ctx context.Context, m *M, raw []byte) error
	// identity — проект-владелец и имя: ими подтверждается отсутствие ресурса.
	identity(m *M) (projectID, name string)
	// setID — записать идентификатор в модель.
	setID(m *M, id string)
	// getID — прочитать идентификатор из модели.
	getID(m *M) string
}

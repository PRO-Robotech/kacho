...
func contractTreeRoot(t *testing.T) string {
	...
	// ПРОПУСК, А НЕ ОТКАЗ — и это не смягчение.
	//
	// Прежняя редакция называла состояние верно («не вердикт о продукте») и
	// роняла пробу: `t.Fatalf` даёт КРАСНОЕ, неотличимое от находки. Красное у
	// каждого, кто склонирует модуль, вердиктом о продукте не является, а
	// «условие не создано» обязано быть отличимо от «дефект найден» — иначе
	// перепись прогона считает одно другим.
	//
	// Предпосылку назначает ДЕРЕВО, а не проба: пропуск истекает сам, как только
	// каталог контрактов окажется рядом.
	t.Skipf("УСЛОВИЕ НЕ СОЗДАНО (не находка): над рабочим каталогом нет дерева, несущего %s — "+
		"в отдельном клоне сервиса контракт приезжает зависимостью, и читать его "+
		"из дерева нечем", roleContractRel)
	return ""
}

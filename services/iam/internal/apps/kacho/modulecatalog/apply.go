...
// Apply применяет манифест, СВЕРЯЯ опору внутри транзакции.
//
...
func (v *VerbApplier) Apply(ctx context.Context, m *manifest.Manifest) (Report, error) {
	if v == nil || v.core == nil {
...
func (a *Applier) Apply(ctx context.Context, m *manifest.Manifest) (Report, error) {
	...
	err = a.tx.RunInWriteTx(ctx, func(ctx context.Context, w CatalogWriter) error {
		...
		// Шаг 8 — СВЕРКА ОПОРЫ, до коммита, и ТОЛЬКО на пути глагола.
		//

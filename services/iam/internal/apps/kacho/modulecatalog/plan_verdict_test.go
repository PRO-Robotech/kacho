...
//	красно   вердикт, произведённый ПРОДУКТОМ: производителя в дереве нет
//
...
// errNoPlanVerdictProducer — ШОВ НАБОРА.
var errNoPlanVerdictProducer = errors.New(
	"производителя вердикта плана в дереве нет: ни поля `verdict`, ни построения " +
		"гипотетического состояния после применения, ни вызова seed.AssertCatalogParity " +
		"с пути Plan — свойство §2.11 не реализовано")

// planVerdictProducer — вердикт, произведённый ПРОДУКТОМ по состоянию каталога и
// объявленным строкам.
//
// Сегодня производителя нет, и функция возвращает отказ: набор падает
// УТВЕРЖДЕНИЕМ, называющим отсутствующее свойство, а не отказом сборки. Проба, не
// собирающаяся, о поведении не утверждает ничего (§5.0 приёмки), поэтому шов
// объявлен ЗДЕСЬ и ровно один на набор.
//
// Реализующему: заменить тело вызовом производителя — одна строка, — и весь набор
// начинает утверждать ПОВЕДЕНИЕ. Наивная реализация в памяти, подающая стражу
// только живое множество, обязана быть отвергнута пробой
// `TestPlanHypotheticalStateCarriesTheRetiredSideToo`.
func planVerdictProducer(live, retired catalog.Rows, declared modulecatalog.Declared) (planVerdict, error) {
	_, _, _ = live, retired, declared
	return "", errNoPlanVerdictProducer
}

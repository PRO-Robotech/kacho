...
//
// «Всё задаётся переменной окружения» — НЕВЕРНО, и неверно оно ровно для двух
// ключей из перечня. Випер разрешает переменную только для ключа, который УЖЕ
// знает: у ключа есть умолчание в defaults.go либо явная привязка в load.go.
// У `authn.trusted-forwarder-sans` и `api-server.registry-token.service` нет ни
// того, ни другого — и переменная до поля НЕ ДОЕЗЖАЕТ.
//
// Цена этого измерена, а не предположена: текст отказа стража круга отправителей
// НАЗЫВАЕТ переменную `KACHO_IAM_AUTHN__TRUSTED_FORWARDER_SANS`, оператор задаёт
// ровно её и получает тот же отказ снова. Документ, повторивший имя переменной
// из текста отказа, отправил бы читателя по кругу — поэтому путь подачи здесь
// объявлен полем и доказан прогоном, а не выведен читателем.
//
// ─────────────────────────────────────────────────────────────────────────────

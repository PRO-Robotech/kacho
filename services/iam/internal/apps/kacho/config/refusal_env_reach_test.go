...
//
// `KACHO_IAM_METRICS_ADDR` встречается в дереве iam в тексте самоотчёта
// композиционного корня и исход `Load`+`Validate` не меняет. Разбор, судящий
// исходный текст, объявил бы её находкой; этот молчит — она не названа НИ ОДНИМ
// отказом стража настройки. Прав ли соседний текст, называя её, — предмет
// другого разбора и другой задачи; здесь утверждается только область.
func TestRefusalEnvGate_SaysNothingAboutNamesMentionedOnlyInProse(t *testing.T) {
	saved := snapshotEnv()
	defer restoreEnv(saved)

	const proseOnly = "KACHO_IAM_METRICS_ADDR"

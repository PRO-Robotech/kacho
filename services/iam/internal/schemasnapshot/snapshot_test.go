...
//
// # Это ИНСТРУМЕНТ, а не проба, и потому у него СВОЙ пакет
//
...
// приборы (`internal/repo/kacho/pg/relverdict`, `.../scalegrid`), — запросом по
// переменной окружения, и без запроса выходит пропуском ПЕРВЫМ же оператором.
//
// Пакет отдельный не ради опрятности. Он лежал в `internal/migrations`, а тот
// назван в `PG_OUTSIDE_SELECTION_PKGS` корневого Makefile, где действует
// «ПРОПУСК = ОТКАЗ»: цель считает строки `--- SKIP` и краснеет на любой. Пропуск
// по незапрошенному снимку неотличим там от пакета, который не исполнился вовсе,
// — а именно ради этой неразличимости цель и заведена. Инструмент ронял её
// КАЖДЫМ прогоном конвейера, и починка со стороны цели была бы ведомостью
// прощений, то есть послаблением без предмета.
//
// # Почему у него НЕТ собственного признака сборки
//
// Он здесь был (`integration_snapshot`) и покупал изоляцию, которая уже куплена
// переменной окружения. Признак сверх этого не давал ничего — а стоил того, что
// пакет переставал попадать в область ХОТЬ КАКОГО-НИБУДЬ объявленного прогона:
// обычная сборка файлы под признаком не читает, короткий прогон идёт с -short, а
// интеграционная джоба отбирает пакеты по пути. Пробы под таким признаком не
// исполняются НИ ОДНИМ прогоном, и заметно это только гейту
// `internal/repohygiene/buildtagrunreach_test.go`.
//
// Вернуть признак вместо переезда было НЕЛЬЗЯ: этот пакет отбор интеграционной
// джобы (`/internal/(repo|clients|reconciler|subscriptionjournal)`) не достаёт,
// поэтому признак сделал бы красным тот гейт. Два гейта связывали файл с разных
// сторон, и законного входа у него в прежнем пакете не было вовсе.

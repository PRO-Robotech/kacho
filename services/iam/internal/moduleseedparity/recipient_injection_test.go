...
// отношений с отступом. Два судимых типа и четыре отношения нужны, чтобы обе
// изоляции — между отношениями одного типа и между одноимёнными отношениями
// разных типов — проверялись на входе, где перепутать ЕСТЬ что.
//
// # Здесь недоставало объявлений типов субъектов, и это было НЕ мелочью
//
// Прежняя редакция называла в прямых userset'ах `service_account` и
// `group#member`, не объявляя ни `type service_account`, ни `type group`. Такой
// вход каноном НЕ является: связность объявлений — часть формы, и настоящий
// разбор (`authzplan.ParseModel`) его отвергает. Прежний собственный построчный
// разбор отвечал по нему уверенно — то есть все оси ниже судились на входе,
// которого модель принять не может.
//
// Типы объявлены, и `group.member` — не украшение: членство группы есть
// отношение типа `group`, и без него запись `group#member` ссылалась бы в
// пустоту.

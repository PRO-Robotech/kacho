...
func refuseCall(stderr io.Writer, reason string) int {
	fmt.Fprintf(stderr, "вызов НЕ ИСПОЛНЕН: %s\n", reason)
	fmt.Fprintf(stderr, "что сделать: назовите одно из пяти действий — %s\n",
		strings.Join(actionNames, " · "))
	fmt.Fprintln(stderr, "  iamctl validate [-root=КАТАЛОГ]")
	fmt.Fprintln(stderr, "  iamctl plan МОДУЛЬ")
	fmt.Fprintln(stderr, "  iamctl apply МОДУЛЬ -expected-state=ОТПЕЧАТОК")
	fmt.Fprintln(stderr, "  iamctl export {МОДУЛЬ | -all}")
	fmt.Fprintln(stderr, "  iamctl doctor")
	return ExitNotRun
}
...
func withService(ctx context.Context, stderr io.Writer, deps Deps, fn func(ModuleService) int) int {
	if deps.Connect == nil {
		fmt.Fprintln(stderr, "вызов НЕ ИСПОЛНЕН: соединение со службой не внесено")
		return ExitNotRun
	}
...
func wantModule(positional []string, action string, stderr io.Writer) (string, bool) {
	...
	case 0:
		fmt.Fprintf(stderr, "вызов НЕ ИСПОЛНЕН: module: обязательное поле — имя модуля не названо\n")
		fmt.Fprintf(stderr, "что сделать: iamctl %s МОДУЛЬ; имена действующих модулей показывает "+
			"iamctl export --all\n", action)
		return "", false
	default:
		fmt.Fprintf(stderr, "вызов НЕ ИСПОЛНЕН: module: названо имён %d, а действие судит ОДИН модуль: %v\n",
			len(positional), positional)
		fmt.Fprintf(stderr, "что сделать: iamctl %s МОДУЛЬ — по одному вызову на модуль; "+
			"имена показывает iamctl export --all\n", action)
		return "", false
	}
	fmt.Fprintln(stderr, "вызов НЕ ИСПОЛНЕН: module: обязательное поле — имя модуля пусто")
	fmt.Fprintf(stderr, "что сделать: iamctl %s МОДУЛЬ; имена показывает iamctl export --all\n", action)
	return "", false
}
...
func classifyRefusal(stderr io.Writer, endpoint, what string, err error) int {
	...
	case codes.InvalidArgument, codes.FailedPrecondition, codes.NotFound,
		...
		fmt.Fprintf(stderr, "НАХОДКА: %s: служба отвергла запрос (%s): %s\n", what, st.Code(), msg)
		return ExitFinding
	case codes.PermissionDenied:
		fmt.Fprintf(stderr, "НЕ ИСПОЛНЕНО: %s: доступ не разрешён (%s): %s\n", what, st.Code(), msg)
		fmt.Fprintf(stderr, "что сделать: глаголы каталога модуля требуют отношения system_admin "+
...
func runValidate(args []string, stdout, stderr io.Writer, deps Deps) int {
	...
	if err != nil {
		...
		fmt.Fprintf(stderr, "вызов НЕ ИСПОЛНЕН: %v\n", err)
		fmt.Fprintln(stderr, "что сделать: iamctl validate [-root=КАТАЛОГ] — корень задаётся "+
			"флагом -root, позиционных аргументов действие не принимает")
		return ExitNotRun
	}
	if deps.Validate == nil {
		fmt.Fprintln(stderr, "вызов НЕ ИСПОЛНЕН: локальная проверка дерева не внесена")
		return ExitNotRun
	}
...
func planWithService(ctx context.Context, stdout, stderr io.Writer, svc ModuleService, deps Deps, module string) int {
	...
	fmt.Fprintf(stdout, "вердикт: применение сойдётся с якорем\n")
	fmt.Fprintf(stdout, "следующий шаг: iamctl apply %s -expected-state=%s\n",
...
func printPlan(stdout io.Writer, resp *iamv1.PlanModuleResponse) {
	fmt.Fprintf(stdout, "модуль: %s\n", resp.GetModule())
	fmt.Fprintf(stdout, "план: записать ресурсов %d · глаголов %d\n",
		resp.GetWrittenResourceCount(), resp.GetWrittenVerbCount())
	fmt.Fprintf(stdout, "план: снять ресурсов %d · глаголов %d\n",
	...
	if resp.GetWithdrawnListTruncated() {
		fmt.Fprintln(stdout, "  перечень снимаемого УСЕЧЁН службой — числа выше полны, имена нет")
	}
	fmt.Fprintf(stdout, "на момент плана: переселить правил-ссылок %d · глаголов ролей %d\n",
		resp.GetResettledRuleRefsAtPlanTime(), resp.GetResettledRoleVerbsAtPlanTime())
	fmt.Fprintf(stdout, "на момент плана: вычистить строк отбора %d (отбросить %d) · типов %d\n",
		resp.GetPrunedSelectorRowsAtPlanTime(), resp.GetPrunedSelectorRowsDroppedAtPlanTime(),
		resp.GetPrunedSelectorTypesAtPlanTime())
	fmt.Fprintf(stdout, "отпечаток состояния: %s\n", resp.GetExpectedState())
}

func printBeyondAnchor(stderr io.Writer, resp *iamv1.PlanModuleResponse) {
	fmt.Fprintf(stderr, "НАХОДКА: модуль %s: применение вышло бы ЗА ЯКОРЬ и было бы отвергнуто "+
	...
	fmt.Fprintln(stderr, "что сделать: править ИСТОЧНИК манифеста — повтор применения не поможет "+
...
func runApply(ctx context.Context, args []string, stdout, stderr io.Writer, deps Deps) int {
	...
	if *expected == "" {
		fmt.Fprintln(stderr, "вызов НЕ ИСПОЛНЕН: -expected-state: обязательное поле — "+
			"отпечаток состояния не назван")
		fmt.Fprintf(stderr, "что сделать: возьмите отпечаток у плана — iamctl plan %s — и передайте "+
...
func applyWithService(ctx context.Context, stdout, stderr io.Writer, svc ModuleService, deps Deps,
	...
	if err != nil {
		...
		if code == ExitFinding {
			fmt.Fprintf(stderr, "что сделать: перечитайте план — iamctl plan %s — и повторите "+
...
func reportApply(stdout, stderr io.Writer, module string, op *operationv1.Operation) int {
	if op == nil {
		fmt.Fprintf(stderr, "НЕ ИСПОЛНЕНО: применение модуля %s: служба вернула пустой конверт операции\n", module)
		fmt.Fprintf(stderr, "что сделать: повторите — вердикта о применении НЕ получено\n")
		return ExitNotRun
	}
	if !op.GetDone() {
		fmt.Fprintf(stderr, "НЕ ИСПОЛНЕНО: применение модуля %s: конверт операции %s объявлен "+
			"терминальным контрактом, а пришёл незавершённым\n", module, op.GetId())
		fmt.Fprintf(stderr, "что сделать: перечитайте план — iamctl plan %s — и сверьте состояние; "+
			"вердикта о применении НЕ получено\n", module)
		return ExitNotRun
	}
	if e := op.GetError(); e != nil {
		fmt.Fprintf(stderr, "НАХОДКА: применение модуля %s отвергнуто (%s): %s\n",
	...
	if any := op.GetResponse(); any != nil {
		if err := any.UnmarshalTo(&res); err != nil {
			fmt.Fprintf(stderr, "НЕ ИСПОЛНЕНО: применение модуля %s: тело ответа не разобрано: %v\n", module, err)
			fmt.Fprintln(stderr, "что сделать: образ службы и инструмент собраны от разных деревьев — "+
				"сверьте версии развёртывания")
			return ExitNotRun
		}
	}
	fmt.Fprintf(stdout, "модуль: %s\n", module)
	fmt.Fprintf(stdout, "применение: состояние изменено=%t · строка модуля записана=%t\n",
		res.GetChanged(), res.GetModuleWritten())
	fmt.Fprintf(stdout, "записано: ресурсов %d · глаголов %d\n",
		res.GetWrittenResources(), res.GetWrittenVerbs())
	fmt.Fprintf(stdout, "снято: ресурсов %d · глаголов %d\n",
		res.GetRetiredResources(), res.GetRetiredVerbs())
	fmt.Fprintf(stdout, "переселено: правил-ссылок %d · глаголов ролей %d (на момент плана было %d и %d)\n",
		res.GetResettledRuleRefs(), res.GetResettledRoleVerbs(),
		res.GetPlannedResettledRuleRefs(), res.GetPlannedResettledRoleVerbs())
	fmt.Fprintf(stdout, "вычищено: строк отбора %d (отброшено %d) · типов %d\n",
...
func runExport(ctx context.Context, args []string, stdout, stderr io.Writer, deps Deps) int {
	...
	case *all && len(positional) > 0:
		fmt.Fprintf(stderr, "вызов НЕ ИСПОЛНЕН: названы обе формы сразу — -all и модуль %v\n", positional)
		fmt.Fprintln(stderr, "что сделать: выберите одну — iamctl export МОДУЛЬ либо iamctl export -all")
		return ExitNotRun
	case !*all && len(positional) == 0:
		fmt.Fprintln(stderr, "вызов НЕ ИСПОЛНЕН: не названо ни модуля, ни -all — выгружать нечего")
		fmt.Fprintln(stderr, "что сделать: выберите форму — iamctl export МОДУЛЬ либо iamctl export -all")
		return ExitNotRun
	case *all:
		return withService(ctx, stderr, deps, func(svc ModuleService) int {
...
func exportOne(ctx context.Context, stdout, stderr io.Writer, svc ModuleService, deps Deps, module string) int {
	...
	fmt.Fprintln(stdout, sourceLine)
	printCatalog(stdout, cat)
	fmt.Fprintf(stdout, "перепись: модулей выгружено 1 · строк ресурсов %d · строк глаголов %d\n",
...
func exportAll(ctx context.Context, stdout, stderr io.Writer, svc ModuleService, deps Deps) int {
	...
	if len(summaries) == 0 {
		fmt.Fprintln(stderr, "ВЫГРУЖАТЬ НЕЧЕГО: перечень не назвал НИ ОДНОГО живого модуля — "+
			"это НЕ успех, а отсутствие предмета")
		fmt.Fprintln(stderr, "что сделать: примените манифест хотя бы одного модуля — "+
	...
	for _, s := range summaries {
		...
		if gotRes != s.GetLiveResourceCount() || gotVerbs != s.GetLiveVerbCount() {
			fmt.Fprintf(stderr, "НАХОДКА: модуль %s: перечень назвал живыми ресурсов %d · глаголов %d, "+
				"а выгружено %d и %d — выгрузка НЕПОЛНА\n",
				s.GetModule(), s.GetLiveResourceCount(), s.GetLiveVerbCount(), gotRes, gotVerbs)
			fmt.Fprintln(stderr, "что сделать: повторите выгрузку; если расхождение держится — "+
				"каталог менялся между перечнем и чтением либо чтение усечено")
			code = worse(code, ExitFinding)
		}
	}
	fmt.Fprintf(stdout, "перепись: модулей названо %d · выгружено %d · строк ресурсов %d · строк глаголов %d\n",
...
func (f finding) print(stderr io.Writer) {
	fmt.Fprintf(stderr, "неверно: %s\n", f.Wrong)
	fmt.Fprintf(stderr, "  проверено: %s\n", f.Checked)
	fmt.Fprintf(stderr, "  сделать: %s\n", f.Fix)
}
...
func runDoctor(ctx context.Context, args []string, stdout, stderr io.Writer, deps Deps) int {
	...
	if len(positional) > 0 {
		fmt.Fprintf(stderr, "вызов НЕ ИСПОЛНЕН: doctor судит каталог целиком и имён не принимает: %v\n", positional)
		fmt.Fprintln(stderr, "что сделать: iamctl doctor — без аргументов; один модуль смотрит iamctl plan МОДУЛЬ")
		return ExitNotRun
	}
...
func doctorWithService(ctx context.Context, stdout, stderr io.Writer, svc ModuleService, deps Deps) int {
	...
	list, err := svc.List(ctx, &iamv1.ListModulesRequest{})
	if err != nil {
		fmt.Fprintf(stdout, "перепись: проверок %d · модулей 0 · находок 0 · НЕ ИСПОЛНЕНО 1\n", checks)
		return classifyRefusal(stderr, deps.Endpoint, "перечень модулей", err)
	}
	...
	summaries := list.GetModules()
	if len(summaries) == 0 {
		fmt.Fprintf(stdout, "перепись: проверок %d · модулей 0 · находок 0\n", checks)
		fmt.Fprintln(stderr, "РАЗБИРАТЬ НЕЧЕГО: перечень не назвал НИ ОДНОГО живого модуля — "+
	...
	for _, s := range summaries {
		...
		fmt.Fprintf(stdout, "модуль %s: каталог сходится с доставленным манифестом "+
	...
	// «ноль проверенного».
	fmt.Fprintf(stdout, "перепись: проверок %d · модулей %d · находок %d\n",

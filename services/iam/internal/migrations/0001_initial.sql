...
--
--   go test -tags integration_snapshot ./services/iam/internal/migrations/ \
...
-- не монтирует том вовсе, и пару объявляют 2 профиля из 9. Следом за
...
--       можем выполнять»). Свод применим ТОЛЬКО к чистой базе: существующая
--       база несёт версию цепочки и этот номер уже считает применённым.

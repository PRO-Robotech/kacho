...
COMMENT ON COLUMN kacho_iam.roles.retired_by IS
  'Кто снял: процессный актор пути старта либо проверенная личность вызывающего глагола применения. Пустая строка означает «помечено до заведения колонки», а не «автора потеряли» — та же семантика, что у applied_by двух ведомостей (20260903215500).';
...
-- +goose Down
--
-- Откат снимает ФОРМУ, а не данные: ни одна роль не оживляется и не снимается,
-- потому что снятых нет — производителя пометки эта миграция не заводит. После
-- отката состояние «объявление убрано, роль жива навсегда» снова становится
-- единственно возможным, и это надо знать тому, кто откат применяет.
ALTER TABLE kacho_iam.roles

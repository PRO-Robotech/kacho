...
--
-- Снятие правит НЕКЛЮЧЕВОЙ столбец (`live`) и берёт `FOR NO KEY UPDATE`, с
-- которым `FOR KEY SHARE` НЕ конфликтует: страж прочитал бы строку живой и
-- пропустил бы выдачу на роль, снимаемую прямо сейчас. `FOR SHARE` конфликтует и
-- потому сериализует эти два. Замерено в ОБОИХ порядках (запись решения,
-- §«Страж обязан брать `FOR SHARE`»); обратный порядок зелен на всех трёх
-- замках, включая два негодных, и различающей силы не имеет.
--
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Замок `FOR SHARE`, а не `FOR KEY SHARE`: снятие берёт `FOR NO KEY UPDATE`,
    -- с которым второй не конфликтует, и страж прочитал бы роль живой в момент
    -- её снятия.

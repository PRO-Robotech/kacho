...
AS $$
BEGIN
  -- Выдача без пары субъекта в самой строке проецировать нечего. Такой строки
  ...
  -- ПРОВЕРКА ПЕРЕД ВСТАВКОЙ — и это не check-then-act, запрещённый ban #10:
  -- триггер ОТЛОЖЕН до конца транзакции (см. ниже), поэтому «сам вставил»
  -- уже случилось, а гонки за эту строку нет — она принадлежит выдаче,
  -- которую эта же транзакция и создала.
  --
  -- Почему проверка нужна: писателей, вставляющих строку субъекта своим
  -- оператором, в дереве 58, и почти все — пробы и посевы без ON CONFLICT.
  -- Безусловная вставка отсюда сделала бы их падающими на 23505, то есть
  -- инвариант чинил бы одно и ломал другое.
  IF EXISTS (SELECT 1 FROM kacho_iam.access_binding_subjects
              WHERE binding_id = NEW.id) THEN
    RETURN NULL;
  END IF;
...
-- ОТЛОЖЕННЫЙ триггер уровня транзакции, а не обычный AFTER INSERT.
--
-- Обычный сработал бы СРАЗУ после вставки выдачи — то есть РАНЬШЕ, чем
-- вызывающий, который пишет строку субъекта своим оператором, успел бы её
-- вставить. Его вставка получала бы 23505, и починка одного дефекта заводила
-- бы другой: в дереве 58 таких писателей. Отложенный до конца транзакции
-- триггер видит уже готовое состояние и добавляет строку ТОЛЬКО тогда, когда
-- её не добавил никто.
--
-- AFTER, а не BEFORE: дочерняя строка ссылается на выдачу внешним ключом, и до
-- вставки родителя ссылаться не на что.
CREATE CONSTRAINT TRIGGER access_bindings_projects_its_subject_trg
  AFTER INSERT ON kacho_iam.access_bindings
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  EXECUTE FUNCTION kacho_iam.access_binding_projects_its_subject();

-- ОБРАТНОЕ ЗАПОЛНЕНИЕ НЕ РАСШИРЯЕТ ДОСТУП, и это проверено, а не предположено.
-- Ветвь выдач читает только `status = 'ACTIVE' AND revoked_at IS NULL`
-- (`relverdict/query.go`), поэтому строка субъекта, восстановленная у отозванной
-- выдачи, ветвью не выбирается. Заполнение идёт по ВСЕМ выдачам намеренно:
-- дочерняя таблица — проекция состава выдачи, а не права; решение о праве
-- принимает ветвь, и держать это решение в двух местах значило бы завести
-- второй фильтр, который разойдётся с первым молча.
--

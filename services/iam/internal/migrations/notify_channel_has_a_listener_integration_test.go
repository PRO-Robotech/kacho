...
func TestIntegration_AuditEventChannelHasNoProducerLeft(t *testing.T) {
	...
	//
	// Здесь стоял `kacho_iam_fga_outbox`, и он контролем БЫТЬ ПЕРЕСТАЛ: дренаж
	// журнала намерений снят вместе с внешним движком отношений, а складка прямого
	// факта синхронна (миграция 0098) — будить некого. Предмет заведён задачей
	// #1436 и стоит в ведомости ниже; контроль перепривязан к каналу, чей
	// потребитель проверяется, а не объявляется.
	assert.Contains(t, channels, notifyChannelWithAProvenConsumer,
...
var notifyChannelConsumerExemptions = map[string]string{
	"kacho_iam_fga_outbox": "задача #1436: дренаж журнала намерений снят вместе с внешним " +
		"движком отношений (serve.go), а прямой факт складывается из журнала триггером " +
		"в том же коммите (миграция 0098) — будить некого. Снимать ли триггер, решает " +
		"линия реляционного вердикта: предмет журнала её, а не подписки.",
}

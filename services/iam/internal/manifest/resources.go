...
//
// Читателей три и они в разных пакетах: рендер блоков модели
// (`modelrender.Render`), сверка соединения (`roleexport`, набор отношений,
// на которых «едет» незаявленная запись каталога) и загрузчик
// (`declaredRelationNames` — что вправе стоять в `from`). Вторая копия правила
// разошлась бы с первой молча: обе стороны отвечают одинаково на тенантском
// действии, то есть на подавляющем большинстве входов.

...
// CanonicalVerbs возвращает КОПИЮ закрытого набора канонических глаголов в
// объявленном порядке.
//
// Экспортирована, потому что второй импортёр — рендер блоков модели (#1089), и без
// перечислителя он держал бы ВТОРУЮ копию набора: правило членства
// (ClassOfCanonicalVerb) отвечает про одно имя и перечислить набор не даёт.
//
...
func CanonicalVerbs() []string {
	return append([]string(nil), canonicalVerbClasses...)
}

...
// (`internalGetNetwork`, `internalAttach`, `internalDetach`). Каталог прав их
// знает, а раздел `resources` НЕ ОБЪЯВЛЯЕТ ни один манифест дерева — ни фикстура,
// ни поставляемый `services/vpc/manifest.yaml` (предикат:
// `grep -c internalGetNetwork services/vpc/manifest.yaml` → 0). Приёмка считала
// эти оси по ЧЕРНОВИКУ воркспейса, который внутренние действия объявляет.
//
// Значит у Given этих осей в дереве нет входа, и взять его неоткуда, кроме как
// объявить в самой пробе. Своя копия ВСЕГО раздела была бы вторым манифестом об
// одном предмете; здесь объявляется РОВНО то, что ось называет, — и это не
// копия, а вход, которого в дереве нет.
//
// Расхождение названо вслух, а не сглажено: раздел `resources` порождается из
// аннотаций контрактов, и его сегодняшняя неполнота по внутренним действиям —
// предмет своей задачи, а не этих проб.

...
//     непригодное действие (MOD-RL-19) она не считает, и ПРИЧИНА У ЭТОГО НЕ ТА,
//     что стояла здесь прежде.
//
//     Прежняя редакция объясняла это тем, что «написание действия в манифесте
//     есть контракт генератора раздела `resources`, а генератора сегодня нет».
//     Посылка ПЕРЕЖИЛА свой предмет: написание объявлено ОДНАЖДЫ — `Attribute`
//     (`attribution.go`, единственное объявление правила в дереве), — а
//     соединение «имя действия манифеста ↔ запись каталога» построено и
//     провязано в `make -C services/iam module-manifest-check`. Замер по всем
//     шести манифестам дерева: действий раздела **101**, записей каталога
//     **228**, без записи каталога **0**, без счёта **0** (#1844).
//
//     Настоящий блокер другой, и он ВЫШЕ по тракту: ПОИМЁННОЙ ФОРМЫ ПРАВА У
//     РОЛИ СЕГОДНЯ НЕ СУЩЕСТВУЕТ. Ключ `verbs` в правиле роли снят и
//     отвергается пред-разборной проверкой со своим сентинелом
//     (`manifest.ErrRoleRuleVerbsRetired`); единственная форма — `classes`.
//     Значит у обеих проверок нет ВХОДА: полноту считать не по чему (перечня
//     имён не бывает), и назвать непригодное действие правило не может — оно
//     называет класс. Это не отсрочка, а порядок: поимённая форма возвращается
//     ВМЕСТЕ с проверкой её полноты (#1844), потому что принять перечень имён,
//     не умея проверить его полноту по классу, значит свести его к классу
//     МОЛЧА — то есть выдать право шире просимого.

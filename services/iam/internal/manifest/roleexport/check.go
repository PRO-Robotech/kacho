...
//     непригодное действие (MOD-RL-19) она не считает. Препятствий ДВА, и
//     прежняя редакция называла только второе — то, которое НЕ является
//     связывающим.
//
//     ПЕРВОЕ, и оно решающее: ПОИМЁННОЙ ФОРМЫ ПРАВА У РОЛИ НЕ СУЩЕСТВУЕТ. Ключ
//     `verbs` в правиле роли снят и отвергается пред-разборной проверкой со
//     своим сентинелом (`manifest.ErrRoleRuleVerbsRetired`); единственная форма
//     — `classes`. Значит у обеих проверок нет ВХОДА: полноту считать не по
//     чему (перечня имён не бывает), и назвать непригодное действие правило не
//     может — оно называет класс. Это не отсрочка, а порядок: форма
//     возвращается ВМЕСТЕ с проверкой её полноты (#1844), потому что принять
//     перечень имён, не умея проверить его полноту по классу, значит свести его
//     к классу МОЛЧА — то есть выдать право шире просимого.
//
//     ВТОРОЕ — то, что стояло здесь прежде: «написание действия есть контракт
//     генератора раздела `resources`, а генератора сегодня нет». Оно ВЕРНО и
//     остаётся верным: производителя, пишущего `services/*/manifest.yaml`, в
//     дереве нет (предикат: `git grep -ln 'manifest.yaml' -- '*.go'
//     ':!*_test.go'` не даёт ни одной команды, эмитящей раздел; `authzmapgen`
//     пишет `tables_gen.go`, то есть в обратную сторону). Но связывающим оно не
//     является: даже с генератором обе проверки остались бы без входа по
//     первому препятствию.
//
//     ЧЕМ ДЕРЖИТСЯ ОТСРОЧКА (заведено по #1844): не прозой, а гейтом дерева
//     `internal/repohygiene` `TestNamedVerbFormReturnsOnlyWithItsCompletenessCheck`.
//     Он судит ПАРУ — возвращается ли сентинел прод-кодом манифеста И есть ли в
//     дереве пробы шести сценариев, — и падает ровно на том сочетании, которое
//     приёмка запрещает: форма принята, полнота не проверяется. Каждая половина
//     по отдельности законна, поэтому гейт, судящий одну, был бы либо красным
//     сегодня, либо слепым завтра.
//
//     Что за это время ПОЯВИЛОСЬ и чего прежняя редакция не называла: написание
//     действия объявлено ОДНАЖДЫ — `Attribute` (`attribution.go`, единственное
//     объявление правила в дереве), — и соединение «имя действия манифеста ↔
//     запись каталога» построено и провязано в `make -C services/iam
//     module-manifest-check`. Замер по всем шести манифестам: действий раздела
//     **101**, записей каталога **228**, без записи каталога **0**, без счёта
//     **0**. То есть половина, которую прежняя редакция объявляла отсутствующей
//     целиком, наполовину существует, и читатель обязан это знать.

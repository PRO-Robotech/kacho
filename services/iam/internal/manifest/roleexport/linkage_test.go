...
//
// Манифестов в дереве продукта сегодня ноль, поэтому перепись по дереву не
// может покраснеть НИ ПРИ КАКОМ входе: у неё нулевая популяция. Проба, стоящая
// только на ней, доказывала бы, что механизм жив, ничего не говоря о том,
// способен ли он отвергнуть. Поэтому вход производится здесь — и по каждой оси
...
//
// Это НЕ гейт биекции: фикстура написана по черновику, который действия
// переименовывает осознанно, а генератор раздела (#1092) ещё не заведён. Проба
// утверждает ровно то, что может утверждать сегодня, — что популяция непуста и
// сверка её читала; расхождение печатается числом, чтобы «ноль находок» было
// отличимо от «ноль прочитанного» и чтобы величина работы генератора была видна.

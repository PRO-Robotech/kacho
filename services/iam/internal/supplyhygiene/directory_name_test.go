...
// Поэтому путь МОДУЛЯ (`github.com/PRO-Robotech/kacho-iam`), пакет контракта
// (`kacho.cloud.iam.v1`) и общий слой проб платформы остаются прежними: их
// переименование есть предмет других полос, и захват их сюда сделал бы вердикт
...
//
// Главный способ ошибиться здесь — судить по слову `kacho`. Его несут ЧЕТЫРЕ
// соседних предмета, и ни один каталогом этой службы не является:
//
//	kacho-iam/…            путь МОДУЛЯ службы. Модуль переименовывается своей
//	                       полосой одновременно с контрактом, иначе граф
//	                       зависимостей даёт цикл;
...
// подстрокой: `kacho-iam` сегментом `kacho` не является, и засчитывать его

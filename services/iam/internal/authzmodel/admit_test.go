...
// собирает ПО ИМЕНИ типа, а отношения спрашивает у отдельного блока, — на двух
// блоках одного имени с РАЗНЫМИ именами указателей `t.Rel(rel)` даёт nil и
// `PointerTargets` падает. Допуск — первый прод-вызывающий этого метода, и текст
...
func TestAdmitSurvivesSplitPointerNamesAcrossDuplicateBlocks(t *testing.T) {
	...
	blockB := "type acme_widget\n  relations\n    define owner_project: [project]\n" +
		"    define super_admin: super_admin from owner_project\n"

	// Предпосылка пробы: чужой метод на этом входе действительно падает. Без неё
	// проба зеленела бы и тогда, когда защищать стало нечего.
	...
	func() {
		defer func() {
			if r := recover(); r == nil {
				t.Fatal("предпосылка пробы отпала: AssertOnePointerPerParentType больше не падает " +
					"на задвоенном имени типа — защиту в допуске надо перепроверить, а не сохранять по инерции")
			}
		}()
		_ = m.AssertOnePointerPerParentType()
	}()

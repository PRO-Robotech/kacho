...
// Admitted — допущена ли композиция. Третий исход успехом НЕ является.
func (r AdmissionReport) Admitted() bool {
	return len(r.Findings) == 0 && !r.NothingToJudge
}
...
func admit(canon, composed string) (AdmissionReport, error) {
	...
	//
	// Метод НЕ зовётся на задвоенном имени типа: карту указателей он собирает по
	// ИМЕНИ, а отношения спрашивает у отдельного блока, — на двух блоках одного
	// имени с разными именами указателей он разыменовывает nil (замерено; проба
	// TestAdmitSurvivesSplitPointerNamesAcrossDuplicateBlocks держит и это, и
	// свою собственную предпосылку). Такой вход уже отвергнут Д1 выше, поэтому
	// fail-closed не страдает: паника же означала бы не отказ, а падение службы
	// на доставленном тексте.

...
//
// # Записей было ТРИ, осталась ОДНА, и причина у неё ДРУГАЯ
//
// Прежнее основание всех трёх снято вместе с предметом (#1904): загрузчик
// требовал от `roles[].description` не менее шестнадцати знаков, а применённые
// миграции написали живым строкам от девяти. Предел выносил приговор по длине
// ИМЕНИ ТИПА (`Admin RouteTable` 16 знаков проходило, `Edit RouteTable` 15 — нет)
// и делал 27 живых ролей из 42 невыразимыми ни при каком написании. Величина и
// довод обеими сторонами — `manifest.minRoleDescriptionRunes`. `compute` (3 из 3)
// и `vpc` (18 из 18) объявлены и сверены прогоном.
//
// `iam` остался, и его запись НЕ прежняя отсрочка под новым номером: две живые
// роли называют класс, под которым нет ни одного пригодного действия, и разбор
// манифеста (`make -C services/iam module-manifest-check`) отвергает объявление,
// а применитель кладёт правила ДОСЛОВНО — то есть объявить их можно только
// вместе с пустым классом. Снятие требует новой миграции по живым правилам, то
// есть решения о том, что роль ГАРАНТИРУЕТ: другой предмет, другой предикат.
//
// На ПУСТОЙ ведомости гейт проходит — пустая ведомость и есть цель.
var postponedModules = []moduleroleparity.Postponement{
	{
		Module: "iam",
		Why: "роли `iam.role.edit` и `iam.role.view` несут класс `get` на ресурсе роли, " +
			"под которым нет ни одного пригодного действия (чтение роли освобождено от " +
			"гейта доступа, `v_get` не спрашивает ни одна запись каталога); применитель " +
			"кладёт правила дословно, поэтому объявление невозможно, пока живые правила " +
			"не приведены новой миграцией; #1916",
	},
}

...
func TestMODRD06BAResourceWithdrawnByDataIsRefusedByTheKeyAndNothingLands(t *testing.T) {
	...
	// # Что здесь утверждается ВМЕСТО `errors.Is`, и почему это не послабление
	//
	// На полосе ПИСАТЕЛЯ цепочка Go до вызывающего не доезжает: исполнитель
	// транзакций (`shared.DoWithWriteTxVoid`, единственное объявление паттерна)
	// приводит отказ к gRPC-статусу, и `moduleroles.ErrWriteFailed` перестаёт
	// находиться `errors.Is` — при том что на полосе ДОМЕНА (проба выше) тот же
	// приём работает, потому что отказ рождается до исполнителя. Утверждать
	// сентинел здесь значило бы утверждать то, чего вызывающий не получает.
	//
	// Поэтому утверждается ПАРА, которую он получает на самом деле: класс кодом
	// статуса и предмет — текстом. Асимметрия двух полос названа находкой, а не
	// подогнана под пробу: клиент, желающий различать полосы машинно, сегодня
	// может только разбирать прозу.

...
func (r *SubjectChangeRepo) PollSubjectChanges(ctx context.Context, sinceID int64, limit int32) ([]service.SubjectChange, int64, error) {
	...
	// ПОЛ спрашивается ПЕРЕД каждой выдачей, и это единственное, чем пропуск
	// вообще обнаружим (задача #1712).
	//
	// Окно ниже снятой строки не показывает НИЧЕМ: её просто нет в выборке,
	// курсор переезжает через неё по последней прочитанной позиции, и «строк не
	// было» становится неотличимо от «строки убрали». Для журнала, гасящего кэш
	// вердиктов края, первое означает норму, второе — НЕПРИМЕНЁННЫЙ ОТЗЫВ
	// ДОСТУПА, молча.
	//
	// Спрашивается ПЕРЕД чтением, а не после: ответ, уже собранный из окна, о
	// снятом префиксе не знает ничего, и решать по нему было бы поздно.
	//
	// Цена — один просмотр индекса по первичному ключу на вызов. Полное
	// наблюдение (`Advance` выше) несёт ещё и просмотр блокировок журнала;
	// платить за него дважды незачем, поэтому здесь отдельный узкий вопрос.
	if err := r.settled.RefreshEarliest(ctx, r.pool); err != nil {
		return nil, 0, fmt.Errorf("earliest retained row of subject_change_outbox: %w", err)
	}
	// Полоса удержания названа ЧИТАЮЩЕЙ стороной и означает «что читатель обязан
	// ВЫДЕРЖАТЬ», а не «что сегодня в таблице». На провод она не выходит вовсе.
	//
	// Объявить здесь `RetainsEverything` («нижней границы не существует») значило
	// бы получить пол, тождественно равный нулю, то есть проверку, которая
	// молчит ВСЕГДА — и молчала бы она ровно с того дня, когда уборка появится.
	// Проверка, умирающая вместе со своим предметом, хуже отсутствующей: она
	// выглядит рабочей.
	//
	// Сегодня журнал не чистится, и объявление СЕБЯ-НЕЙТРАЛЬНО: самая ранняя
	// строка — первая, пол равен нулю, отказ не производится ни при каком
	// курсоре. Проба на это стоит рядом.
	floor := r.settled.Floor(subscription.RetainsFromEarliestRow)

	// Вызывающий БЕЗ позиции не отвергается: ноль объявлен контрактом как
	// «позиции нет» («0 on first call»), и такой вызывающий страницу
	// отбрасывает, усваивая голову. Отказать ему значило бы отправить свежую
	// реплику проигрывать удержанный хвост вместо прыжка на голову — исход хуже
	// при нулевом выигрыше: ПОТЕРЯТЬ ПОЗИЦИЮ НЕЛЬЗЯ, НЕ ИМЕЯ ЕЁ.
	//

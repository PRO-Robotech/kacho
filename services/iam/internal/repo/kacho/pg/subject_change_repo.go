...
func (r *SubjectChangeRepo) PollSubjectChanges(ctx context.Context, sinceID int64, limit int32) ([]service.SubjectChange, int64, error) {
	...
	//
	// ОТДЕЛЬНОГО запроса нижней строки здесь НЕ ДЕЛАЕТСЯ, и это не экономия на
	// правильности. Полное наблюдение выше (`Advance`) спрашивает максимум и
	// МИНИМУМ одним запросом и кладёт оба в наблюдателя, поэтому нижняя граница
	// к этому месту уже свежа — свежее её не сделает ничто. Узкий вопрос
	// (`RefreshEarliest`) заведён для полосы, где полного наблюдения на каждой
	// партии НЕ происходит (поток подписки: там оно идёт один раз при посадке
	// подписчика, а партий много). Здесь оно идёт на КАЖДЫЙ вызов, и второй
	// запрос был бы round-trip за уже известным числом — на каждом такте каждой
	// реплики края.

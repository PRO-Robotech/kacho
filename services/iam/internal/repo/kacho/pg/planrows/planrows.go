...
3. УСТРАНЕНИЕ ДВОЙНОГО СЧЁТА — ПО РОДСТВУ, НИКОГДА ПО ИМЕНИ.
   ...
   его строки в сумму не входят. Схлопывание по ИМЕНИ отношения запрещено:
   два "Index Scan" по одному отношению бывают двумя ветвями UNION, и такое
   схлопывание потеряло бы половину настоящего доступа.

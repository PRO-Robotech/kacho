...
//
// # Найдено по дороге, и здесь НЕ чинится
//
// Контрактный текст стража живости («object_types: <элемент> is not a live
// platform resource (role <роль>)») до вызывающего НЕ доезжает: у приведения
// нет ветви на имя этого ограничения, и арендатор получает общее «Illegal
// argument: value violates a constraint» — без элемента и без роли. Предмет
// свой (текст отказа есть часть контракта и меняется осознанно), и смешивать
// его с порядком замков значило бы свести две линии в один вердикт.
//
// # Чего проба НЕ утверждает
//
// Она не закрывает перекрёстный порядок по НЕСКОЛЬКИМ типам сразу: снятие идёт
// по одной строке за оператор (`catalogWriter.RetireResource`), и порядок этих
// операторов задаёт разница манифестов, а не сортировка. Предмет чужой —
// применитель каталога, — и заявлять о нём здесь значило бы утверждать шире
// сделанного.

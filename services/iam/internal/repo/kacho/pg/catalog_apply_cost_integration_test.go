...
//
// Посадка: PostgreSQL 16.15 (shared_buffers 128MB, work_mem 4MB), linux/amd64,
// 32 ядра, `statement_timeout = 30000` из платформенного пула (`pkg/db/pool.go`).
// Числа — ориентир, а не гейт: их воспроизводит прогон, а не эта таблица.
//
//	ролей	посеяно	S0	S1	S2	строк S1 / S2
//	   10	    300	7.1 мс	   15.3 мс	    22.3 мс	   100 /    400
//	  100	  3 000	6.4 мс	   49.3 мс	   141.0 мс	 1 000 /  4 000
//	 1000	 30 000	6.7 мс	3 358.1 мс	12 630.7 мс	10 000 / 40 000
//	 2000	 60 000	7.1 мс	12617.5 мс	ПОРОГ 57014	20 000 /      —
//	 3000	 90 000	6.6 мс	24053.5 мс	ПОРОГ 57014	30 000 /      —
//
// ТРИ ВЫВОДА, и первый опровергает посылку эпика:
//
//  1. S0 ПЛОСКАЯ на трёх порядках роста ролей (7.1 → 6.6 мс при ×300). Штатное
//     применение проекций не трогает вовсе, поэтому «стоимость применения равна
//     сумме по ролям» верно ТОЛЬКО для применения, что-то снимающего;
//  2. полосы снятия растут СВЕРХЛИНЕЙНО: строк ×2 → времени ×3.76. Механизм
//     назван планом, а не угадан, — пооперационный триггер ссылочной целостности
//     `role_grant_orphan_role_id_fkey`: 379.6 мс на 20 000 вызовов, 46 % времени
//     оператора, плюс поддержка четырёхколоночного первичного ключа сирот;
//  3. ПОРОГ — это ОТКАЗ, а не медленность, и он платформенный: оператор снимает
//     сервер по `statement_timeout`. Снятие МОДУЛЯ перестаёт помещаться между
//     1000 и 2000 ролями; снятие ОДНОГО ресурса при 3000 занимает 80 % бюджета.
//     Отказ безопасен (транзакция откачена, каталог прежний, права не тронуты),
//     но ожиданием он не обходится: повтор даст то же самое.
//
// ─────────────────────────────────────────────────────────────────────────────

...
// служит ФОРМА РОСТА хвоста по оси: очередь на общем ключе даёт рост, линейный
// по числу одновременных, и никакая невидимая блокировка этого не скрывает.
//
...
// (медиана и максимум) на объект и общее время волны; рядом — то же число
// объектов ПОСЛЕДОВАТЕЛЬНО, потому что «сериализуется» проверяется сравнением
// параллельной волны с последовательной, а не абсолютной величиной.
//
// # Что утверждается, а что печатается
//
// Утверждается СТРУКТУРНОЕ: операторов блокировки у форварда ноль, у полного
// прохода больше нуля, и волна из N параллельных проходит быстрее тех же N
// последовательных. Абсолютные величины печатаются, но не утверждаются: они
// свойство машины, и порог возле измеренного краснел бы на занятом ранере.
...
func TestReconcileForward_BurstOnOneOwnerBindingDoesNotSerialize(t *testing.T) {
	...
	const (
		...
		// при удвоении оси.
		warmWave = twoN
		total    = warm + warmWave + 1 + n + twoN + n
	)
	now := time.Now()
	...
	t.Logf("ответ на вопрос «занимает ли ожидание ключа заметную долю хвоста»: "+
		"операторов блокировки, посланных клиентом, — ноль; хвост на объект растёт "+
		"%.1fx (1→N) и %.1fx (1→2N) при росте оси в %dx и %dx — рост СУБЛИНЕЙНЫЙ, "+
		"очереди на общем ключе нет",
		float64(waveN.median)/float64(oneObject), float64(wave2N.median)/float64(oneObject),
		n, twoN)

	// Единственное утверждение о времени — СРАВНЕНИЕ волны с последовательностью,
	// а не абсолютная величина. Сериализуйся волна на одном ключе — она заняла бы
	// не меньше последовательной; здесь требуется хотя бы четверть выигрыша, что
	// на порядок мягче наблюдаемого и потому не флейкует на занятой машине.
	...
	// ВТОРОЕ утверждение — о ФОРМЕ РОСТА, и оно не зависит от того, что видит
	// счётчик: очередь на общем ключе даёт хвост, растущий линейно по оси (при 2N
	// это было бы порядка 16 одиночных проходов). Требуется вчетверо — на порядок
	// мягче наблюдаемого и заведомо строже линейного роста.
	require.Less(t, wave2N.median, 4*oneObject,
		"хвост на объект при %d одновременных — %v против %v на одиночном проходе: "+
			"рост линеен по оси, то есть проходы стоят в очереди на общем ключе",
		twoN, wave2N.median, oneObject)
}

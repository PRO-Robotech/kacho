...
func TestPlanRows_InstrumentReadsTheProductsOwnStatement(t *testing.T) {
	...
	// Множитель циклов утверждается на плане БЕЗ Gather: рекурсивный обход даёт
	// больше одного цикла на КАЖДОЙ точке, тогда как параллельный план на
	// естественном плане не появился ни разу. Проба, поставленная на Gather,
	// была бы зелёной всегда.
	if m.WorkersLaunched != 0 {
		t.Logf("наблюдение: план оказался параллельным (работников %d) — "+
			"утверждение о множителе ниже от этого не зависит", m.WorkersLaunched)
	}
	...
	if len(multiCycle) == 0 {
		t.Fatalf("ни один отнесённый узел не идёт больше чем в один цикл — фикстура выродилась.\n"+
			"Рекурсивный обход цепи областей обязан дать больше одного цикла; если их нет, значит\n"+
			"цепь предков в фикстуре пуста, и проба мерила бы вырожденный случай, ничего об этом\n"+
			"не сообщая. Перепись:\n%s", m.Census)
	}
	t.Logf("узлов с числом циклов больше одного: %d (первый — %s на %s, циклов %d)",
		len(multiCycle), multiCycle[0].NodeType, multiCycle[0].Relation, multiCycle[0].Loops)

...
//
// Целиком новый тип модели вердикта НЕ ДОСТИГАЕТ: план права компилируется из
// модели, ВШИТОЙ В ДВОИЧНЫЙ ФАЙЛ (`authzmodel`, `//go:embed fga_model.fga`), и
// незнакомый ей тип отвергается раньше словаря — `Deny` с признаком «тип не
// объявлен». Это СЛЕДУЮЩЕЕ звено той же цепи, и оно заведено отдельно; здесь
// оно названо, чтобы фикстура не выглядела произвольной.
//

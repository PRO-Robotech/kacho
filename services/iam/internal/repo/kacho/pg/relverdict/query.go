...
--
-- Предел $7 НЕ УСЕКАЕТ и усечь не может: ключ уникален по глубине
...
-- считает РАЗЛИЧНЫЕ условия, а не сырые строки: усечение осталось тем же
-- состоянием, что было, и его порог не сдвинулся.
--
...
type Grounds struct {
	// LabelArm — меточная ветвь правила дала основание СРЕДИ ПРОЧИТАННЫХ строк.
	//
	// Оговорка «среди прочитанных» — не осторожность, а следствие раннего
	// выхода: читатель останавливается на первом безусловном основании, и
	// строки за ним не читает вовсе. Считать признак по полному набору значило
	// бы либо вернуть блокирующий узел, либо спросить меточную ветвь ВТОРЫМ
	// выражением — то есть завести второе место, знающее, что такое «подходит»,
	// и разойтись с первым молча.
	//
	// Поэтому неопределённость не прячется, а НАЗЫВАЕТСЯ: рядом стоит
	// SetExhausted, и «меточная ветвь основания не дала» отличимо от «до неё не
	// дочитали». Ровно эту различимость признак и обязан сохранять.
	LabelArm bool

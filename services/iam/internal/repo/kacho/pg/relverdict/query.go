...
-- Рёбра выше проекта не пишет НИКТО: они появляются регистрацией ресурса у
-- владельца прав, а зовут её пять сервисов-соседей только для СВОИХ ресурсов —
-- объектов типа project и account не регистрирует никто, включая сам iam.
-- Поэтому два верхних звена ВЫВОДЯТСЯ ИЗ СХЕМЫ: предок проекта — его аккаунт
-- (projects.account_id), предок аккаунта — кластер (синглтон clusters).
-- Обе величины принадлежат iam, лежат в его собственных строках и потому не
-- могут устареть: копии не заводится, обратного заполнения нет, переезд проекта
-- виден следующим же вопросом (миграция 740001, kacho#740).
--
-- Под представлением лежат kacho_iam.resource_parent_edge (присланное) плюс
-- kacho_iam.projects, kacho_iam.accounts и kacho_iam.clusters (выводимое). Имена
-- названы здесь ЯВНО, и не ради полноты рассказа: отпечаток предмета замера
-- выводит читаемые таблицы ИЗ ТЕКСТА вердикта, поэтому представление, названное
-- одним своим именем, увело бы собственные основания из-под прибора — их правку
-- он перестал бы замечать молча.
--

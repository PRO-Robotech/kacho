...
-- шлёт два (реестр и проект). Одноразовое чтение схлопнуло бы область до
-- «объект плюс его непосредственный предок»; обход оставляет её открытой вверх.
--
-- НО ВВЕРХ СЕГОДНЯ ИДТИ НЕКУДА, и это надо сказать здесь, а не подразумевать.
-- Обход поднимается по рёбрам, а рёбра появляются РОВНО ОДНИМ путём —
-- регистрацией ресурса у владельца прав. Зовут её пять сервисов-соседей и
-- только для СВОИХ ресурсов; объектов типа project и account не регистрирует
-- никто, включая сам iam (предикат: grep по вызовам RegisterResource вне проб —
-- ни одного из iam). Значит рёбер project → account и account → cluster в базе
-- НЕТ, и на цепи, которую производит дерево, обход доходит до проекта и там
-- останавливается: выдача на аккаунт и факт администратора облака на кластере
-- этой формой НЕ НАХОДЯТСЯ. Закреплено пробой, которая утверждает именно этот
-- ответ, а не желаемый.
--
-- Почему это всё же обход, а не одно чтение: одноразовая форма отнимает верхние
-- уровни НАВСЕГДА и by construction, обход — только пока недостающих рёбер не
-- пишет никто. Первое чинится сменой запроса, второе — заведением производителя,
-- и оно заведено предметом (kacho#740) с предикатом снятия.
--
-- Живой путь принятия решения этим сегодня не задет: реляционный вердикт
-- провязан единственным местом — теневым сравнителем, — а решение о доступе
-- принимает другая форма, берущая аккаунт резолвом на границе чтения. Именно
-- поэтому опасность здесь отложенная: сравнение форм не покраснеет на том, чего
-- корпус вопросов не спрашивает, и переключение сняло бы верхний ярус вместе с
-- аварийным путём администратора облака.
--

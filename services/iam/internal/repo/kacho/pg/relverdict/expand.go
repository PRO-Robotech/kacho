...
--
-- ОБХОД, А НЕ ОДНО ЧТЕНИЕ, и это не осторожность. Таблица рёбер хранит ТУ ЦЕПЬ,
-- КОТОРУЮ ПРИСЛАЛ ПРОИЗВОДИТЕЛЬ, а не замыкание: ключ (объект, глубина) и
-- проверка глубины 1..4 допускают обе формы, и производители в дереве шлют
-- КОРОТКУЮ — ресурсы vpc, storage и compute шлют ОДНО звено (проект), реестр
-- шлёт два (реестр и проект), кластер не шлёт никто. Аккаунт достигается с
-- проекта ЕГО СОБСТВЕННЫМ ребром, кластер — с аккаунта; это транзитивность, и
-- снять её значит схлопнуть область до «объект + его непосредственный предок».
--
...
-- чем менять эту форму, убедись, что замыкание полно ПО ПРОИЗВОДИТЕЛЯМ, а не по
-- схеме, — схема его не требует.
--
...
-- уникален по глубине), а сама глубина ограничена проверкой схемы тем же числом.

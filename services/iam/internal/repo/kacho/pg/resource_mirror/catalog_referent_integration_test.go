...
func TestResourceMirror_RetiredTypeKeepsItsAlreadyRegisteredRows(t *testing.T) {
	...
	require.NoError(t, err)
	// Откат — ОТЛОЖЕННЫМ вызовом, а не через t.Cleanup: уборка теста исполняется
	// ПОСЛЕ отложенных, то есть после `pool.Close()`, а закрытие пула ждёт
	// возврата занятого транзакцией соединения — и ждёт вечно. Проба, которая не
	// коммитит, обязана освобождать соединение раньше пула.

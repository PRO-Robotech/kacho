...
//
// Гейт судит ТЕКСТ посева, а живёт продукт ИСХОДОМ ЦЕПИ. Применённую миграцию
// править нельзя (ban #5), поэтому негодный литерал остаётся в `0001_initial.sql`
// навсегда, а исход даёт поздняя миграция. Текстовому гейту такой пары не
// видно by construction — её утверждает интеграционная проба по ЖИВОЙ СХЕМЕ
// (`internal/migrations`, TestIntegration_SeededSystemRoleIDsPassTheServiceOwnFormCheck),
// и она же доказана парой RED → GREEN.
//
// Ведомость прощает РОВНО такие пары и САМОИСТЕКАЕТ: запись, чьего литерала в
// названном файле больше нет, — находка. Без самоистечения ведомость пережила
// бы предмет и стала бы слепой зоной для следующего негодного посева.
...
func seededIDLedger() []seededSeedException {
	return []seededSeedException{{
		...
		reason: "применённую миграцию править нельзя (ban #5); форму даёт поздняя " +
			"миграция 20260905093000_system_viewer_role_id_fits_the_id_form, а исход " +
			"цепи утверждает интеграционная проба по живой схеме (задача #1808)",
	}}
}

...
**Замер на ревизии `a30d906b8e`** (единица счёта названа, потому что три единицы
дают три разных числа):

```sh
R=a30d906b8e
git grep -o 'shared\.ValidateResourceID(' $R -- 'services/iam/**/*.go' ':!*_test.go' | wc -l  # 53 вызова
git grep -l 'shared\.ValidateResourceID(' $R -- 'services/iam/**/*.go' ':!*_test.go' | wc -l  # 50 файлов
git grep -o 'corevalidate\.ResourceID('   $R -- 'services/iam/**/*.go' ':!*_test.go' | wc -l  # 3 вызова
```

> [!note] Ревизия называется ХЕШЕМ, и устаревание проверяется ПРЕДКОМ, а не резолвом
> Здесь стояло «замер на ревизии записи» — самоссылка, восстановимая только
> раскопками по истории. Числа за это время разошлись дважды: 51/48 в записи против
> **53/50** на названной ревизии.
>
> Годный предикат устаревания — **вхождение в историю**, а не существование объекта:
>
> ```sh
> git merge-base --is-ancestor a30d906b8e HEAD && echo "ревизия в истории — числа сверяемы"
> ```
>
> Проверка «резолвится ли ревизия» (`git rev-parse --verify`, `git cat-file -e`)
> для этого **негодна**: у рабочих копий одного клона **общая база объектов**,
> поэтому она отвечает «да» на ревизию чужой линии, которой в этой истории нет.
> Задача #1798 требовала именно резолва — и своей собственной находки не поймала бы.
...
отказ, не зовя `ValidateResourceID`, для него не существует by construction. Это
остаётся его границей, и она названа, а не умолчана.

```sh
git grep -n 'codes\.InvalidArgument, "invalid .* id' a30d906b8e -- 'services/iam/**/*.go' ':!*_test.go'
```

> [!note] Здесь стояло утверждение о ДВУХ живых копиях — оно пережило свой предмет
> Прежняя редакция называла `sa_keys/usecases.go` и `user_tokens/usecases.go`
> местами, где обрезанный идентификатор (`sva1`) проходит: они сверяли префикс, не
> сверяя длины, а текст отказа совпадал побайтово, и различие не видела ни одна
> проба, сверяющая сообщение.
>
> **Копий там больше нет.** Оба места зовут общего производителя —
> `services/iam/internal/apps/kacho/api/user_tokens/usecases.go:190` и
> `.../sa_keys/usecases.go:352`, оба через `shared.ValidateResourceID`, то есть
> строго и по длине тоже. Предмет закрыт, и абзац оставлен разбором, а не удалён:
> запись, объявляющая живым закрытый дефект, читается как действующее ограничение.

Замер на ревизии `a30d906b8e` — предикат даёт **3** попадания, и **ни одно не
является копией**: два в `access_binding/structural_gates.go` производят **свой**
вид идентификатора со своей формой (`access binding scope id`), третье — сам общий
производитель `shared/ids.go`.

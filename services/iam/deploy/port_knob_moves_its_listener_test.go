...
func judgePortKnobsIn(t *testing.T, dir string, files, knobs []string, baseListeners map[string]bool) (int, []portKnobVerdict, []string) {
	...
	for i, key := range knobs {
		...
		if err != nil {
			// СНЯТЫЙ КЛЮЧ. Отказ — законный исход и ровно тот, которого
			// требует запрет «принято-и-проигнорировано»: снятие, о котором
			// оператору не сказали, неотличимо от работающей ручки.

...
//
// ─────────────────────────────────────────────────────────────────────────────
// ПОЧЕМУ ОТСУТСТВИЕ helm — ОТКАЗ В CI, А НЕ ПРОПУСК ВЕЗДЕ
//
// Гейт, молча ставший инертным на джобе, гейтящей мёрж, гейтом не является. Та
// же дисциплина, что у deploy/identity_domainless_landing_is_expressible_test.go
// и deploy/helm/umbrella/iam_lane_service_aud_test.go.
package deploy_test
...
func renderStandaloneChart(t *testing.T, valueFiles []string, sets ...string) string {
	...
	if _, err := exec.LookPath("helm"); err != nil {
		if os.Getenv("CI") != "" {
			t.Fatal("helm не в PATH при CI — рендер-гейт обязан исполняться, а не пропускаться: " +
				"инертный гейт на джобе, гейтящей мёрж, гейтом не является")
		}
		t.Skip("helm не в PATH — рендер-гейт пропущен (вердикта нет: третья категория, не зелёное)")
	}

...
func buildHooksMux(
	...
	healthAgg := health.New([]health.Checker{
		...
		// ВЕРСИЯ СХЕМЫ — ОТДЕЛЬНАЯ ИМЕНОВАННАЯ ЗАВИСИМОСТЬ, а не часть проверки
		// базы. Мигратор идёт при каждом раскате, поэтому откат выкатки ставит
		// ПРЕЖНИЙ образ на НОВУЮ схему; база при этом отвечает на `Ping`, и без
		// этого чекера под объявлялся бы готовым и получал трафик. Отдельное имя
		// обязательно: оператор обязан отличить «база недоступна» от «образ не
		// той версии, что схема», не читая кода.
		//
		// Набор миграций читается как встроенные байты, у базы спрашивается ОДИН
		// SELECT применённой версии — least-privilege serve-бинаря сохраняется.

...
if [[ -n "$SERVICE" ]]; then
  ...
  #
  # gen.py ВСЕГДА эмитит collections/iam-subject-privileges-read.json, а
  # авторитетный гейт разбирает КАЖДУЮ collections/*.json, поэтому без этого
  # вызова коллекция не отрабатывает вовсе и докладывается
  # `iam-subject-privileges-read(no-report)` — шесть кейсов, которые не могут
  # упасть, потому что не исполняются.

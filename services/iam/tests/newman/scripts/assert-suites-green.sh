...
#   3 — УСЛОВИЕ НЕ СОЗДАНО: единственное, что не сошлось, — непроинъектированные
#       адреса. Вердикта о продукте у этого прогона нет.
...
if [ "${#failed_suites[@]}" -gt 0 ]; then
  if [ "$saw_finding" -eq 0 ] && [ "$tot_precond" -gt 0 ]; then
    ...
    # читателя посылает в ДРУГОЕ место — к прогонщику, который не дал адреса,
    # а не к продукту, который никто не спрашивал.
    echo "УСЛОВИЕ НЕ СОЗДАНО: ${tot_precond} утверждени(й) стража адреса не получили своей переменной; suites: ${failed_suites[*]}"
    echo "Это НЕ вердикт о продукте и НЕ зачёт: адреса поверхностей инъектирует прогонщик (deploy/scripts/newman-e2e.sh / newman-parallel.sh), а он их не разрешил."
    exit 3
  fi

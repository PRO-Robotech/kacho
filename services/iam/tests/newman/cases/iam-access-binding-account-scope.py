...
CASES.append(Case(
    ...
    title="IAM-AB-SIA-12: ?accountId=not-an-id → 400 INVALID_ARGUMENT, тело плоское "
          "\"invalid account id 'not-an-id'\" — тот же производитель, что у ListByAccount на этом же поле",
    classes=["NEG", "VAL"],
    priority="P0",
    steps=[
        Step(
        ...
        # Тот же вход у снимаемого глагола — то же тело. Сверяются ДВА
        # производителя, а не ответ с выписанной строкой: выписанная разошлась бы
        # с обоими молча.
        Step(
            ...
            test_script=[
                *assert_status(400),
                *assert_grpc_code(3, "INVALID_ARGUMENT"),
                "pm.test('оба глагола на одном поле отвечают одинаково', () =>",
                "  pm.expect(pm.response.json().message, pm.response.text())",
                "    .to.eql(\"invalid account id 'not-an-id'\"));",
            ],
        ),

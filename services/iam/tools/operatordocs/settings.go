...
//
// Колонка «как задать» — не украшение. У двух ключей перечня путь через
// переменную окружения НЕ РАБОТАЕТ, при том что текст отказа стража называет
// именно переменную. Оператор, повторивший имя из отказа, получает тот же отказ
// снова. Здесь этот случай назван прямо и доказан прогоном (таблица
// `config.RequiredSettings`, проба `TestRequiredSettings_TableCannotLie`).

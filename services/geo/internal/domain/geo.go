...
// Проверки имени здесь нет, и это не пропуск: у Region/Zone нет самого имени.
//
// История в двух шагах, потому что промежуточное состояние успело обрасти
// правилами. Сперва отсюда ушла длиновая проверка (253 знака, задача #718) —
// она была вчетверо шире формы и не отвергала НИ ОДНОГО имени, которого не
// отвергла бы форма. Затем поле `name` снято с ресурса целиком (#716): его
// значение дублировало admin-назначаемый `id`, человекочитаемый by
// construction.
//
// Поэтому судить сегодня нечего НИ ОДНОМУ из прежних мест: ограничение формы
// снято миграцией 716001 вместе со своей колонкой, а вызова `validate.Name` в
// прод-коде geo больше нет. Предикат — и он находит РОВНО эту строку, потому
// что имя стоит в ней текстом, а вызовов нет ни одного:
//   git grep -n 'validate\.Name' -- 'services/geo/**/*.go' | grep -v _test
// Контроль в обратную сторону (иначе «ноль» читался бы как «не искали»): тот же
// предикат без пути называет 57 файлов дерева. Прежняя редакция этого абзаца
// объявляла оба механизма действующими — утверждение пережило свой предмет и
// сообщало читателю, что имя geo как-то проверяется.

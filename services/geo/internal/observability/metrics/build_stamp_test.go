...
// build_stamp_test.go — ряд `kacho_geo_build_info` РАЗЛИЧАЕТ версию и её отсутствие.
//
...
func TestBuildInfo_StampedValuesReachTheSeriesUntouched(t *testing.T) {
	...
	want := `kacho_geo_build_info{commit="1eedc9e7ff",version="release/kaname-tail"} 1`
...
func TestBuildInfo_UnstampedBuildSaysSoInAWord(t *testing.T) {
	...
	want := `kacho_geo_build_info{commit="` + observability.BuildStampUnstamped +

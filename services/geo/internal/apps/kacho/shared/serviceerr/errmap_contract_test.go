...
func TestToStatusFailureBandsCharacterization(t *testing.T) {
	cases := []struct {
		...
		{"not_found/bare", geoerrors.ErrNotFound, codes.NotFound, "not found"},
		{"not_found/wrapped", fmt.Errorf("%w: Zone zone-1 not found", geoerrors.ErrNotFound), codes.NotFound, "Zone zone-1 not found"},

		{"already_exists/bare", geoerrors.ErrAlreadyExists, codes.AlreadyExists, "already exists"},
		{"already_exists/wrapped", fmt.Errorf("%w: Zone zone-1 already exists", geoerrors.ErrAlreadyExists), codes.AlreadyExists, "Zone zone-1 already exists"},

		{"failed_precondition/bare", geoerrors.ErrFailedPrecondition, codes.FailedPrecondition, "failed precondition"},
		{"failed_precondition/wrapped", fmt.Errorf("%w: region is not empty", geoerrors.ErrFailedPrecondition), codes.FailedPrecondition, "region is not empty"},

		{"invalid_argument/bare", geoerrors.ErrInvalidArg, codes.InvalidArgument, "invalid argument"},
		{"invalid_argument/wrapped", fmt.Errorf("%w: invalid zone id 'zzz'", geoerrors.ErrInvalidArg), codes.InvalidArgument, "invalid zone id 'zzz'"},

		{"canceled/bare", geoerrors.ErrCanceled, codes.Canceled, "request canceled"},
		{"canceled/wrapped", fmt.Errorf("%w: zone get", geoerrors.ErrCanceled), codes.Canceled, "request canceled"},

		{"deadline_exceeded/bare", geoerrors.ErrDeadlineExceeded, codes.DeadlineExceeded, "request deadline exceeded"},
		{"deadline_exceeded/wrapped", fmt.Errorf("%w: zone get", geoerrors.ErrDeadlineExceeded), codes.DeadlineExceeded, "request deadline exceeded"},

		{"internal/bare", geoerrors.ErrInternal, codes.Internal, "internal database error"},
		{"internal/wrapped", fmt.Errorf("%w: pgx: dial tcp 10.0.0.7:5432", geoerrors.ErrInternal), codes.Internal, "internal database error"},
...
func TestToStatusPreservesDetailsThroughSentinelWrap(t *testing.T) {
	rich := coreerrors.InvalidArgument().
		AddFieldViolation("regionId", "regionId is required").
		Err()
	wrapped := fmt.Errorf("%w: %w", geoerrors.ErrInvalidArg, rich)
	...
	if len(fields) != 1 || fields[0] != "regionId" {
		t.Fatalf("field violations = %v, want exactly [regionId]", fields)
	}
}

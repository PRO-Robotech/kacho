...
// Здесь жил `derefString` — разыменование указателя partial-update ради
// сообщения о занятом ИМЕНИ. Предмета у него не осталось (#716): единственный
// уникальный ключ каталога — первичный, и называть в отказе больше нечего.
// Снят вместе со своим вызовом, а не оставлен «пригодится»: мёртвый помощник
// читается следующим как действующий приём.

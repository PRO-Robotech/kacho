...
main() {
  ...
  # Предполётная самопроверка честности шагов-мутаций: отказ мутации каталога
  # приезжает полем `error` внутри 200-конверта, поэтому шаг, проверяющий только
  # форму конверта, зачитывает провал как принятую мутацию. selftest исполняет
  # утверждения из сгенерированных коллекций против подставленных ответов и
  # требует, чтобы они краснели на операции с ошибкой. Без сети и без стенда —
  # секунды, зато зелёный прогон перестаёт зависеть от того, спрашивает ли суита
  # хоть что-нибудь. node здесь заведомо есть: newman на нём и работает.
  if ! node "$NEWMAN_DIR/scripts/selftest-assertions.js"; then

...
> ⚠️ **Security-риск (осознанный) — и он закрыт там, где это важнее всего.** Breakglass
> снимает **обе** линии сразу: и проверку прав, и требование транспортной защиты. Поэтому
> он строго **emergency-only** — локальная отладка или инцидент, никогда не рабочий стенд.
>
> **Исправление ложного утверждения (сверено 2026-08-02).** Прежняя редакция заканчивалась
> словами, что режим аутентификации «breakglass не ограничивает». **Это неверно:**
> `cmd/kacho-geo/serve.go` **отказывает в старте**, если breakglass включён в
> `production`/`production-strict`. Текст отказа адресован оператору и потому остаётся
> дословным — он обязан называть ручку и причину, иначе стенд не поднять.
>
> Запись, объявлявшая достижимым состояние, которое продукт запрещает, — не осторожность,
> а приглашение «починить» код под документ. Поимённая цепочка (что подставить, куда
> дотянуться, что получить) снята отдельно: репозиторий публичен, и такая связка остаётся
> рецептом независимо от того, что вход в это состояние закрыт.

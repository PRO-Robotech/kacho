...
per-RPC Check через kaname везде, **кроме четырёх публичных чтений**,
...
- **Production-режим** (`KACHO_GEO_AUTH_MODE=production`) обязывает проверенный транспорт на
  обоих listener'ах и на ребре к kaname — иначе сервис не стартует. Ручки, снимающей
...
flowchart LR
  ...
  geo -->|"InternalIAMService.Check"| iam["kaname"]
```

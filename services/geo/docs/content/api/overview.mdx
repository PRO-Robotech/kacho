...
(internal не освобождён). Исключение — публичное чтение каталога, у которого per-RPC Check
снят намеренно (см. первый пункт):

- **read** (`RegionService` / `ZoneService` Get/List) — project-scope EXEMPT: per-RPC
  authz-Check снят, грант не нужен; аутентификация обязательна (без принципала —
  `UNAUTHENTICATED`);

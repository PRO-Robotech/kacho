...
Авторизация — **per-RPC, ReBAC** через **OpenFGA**: интерсептор перед выполнением метода
вызывает `InternalIAMService.Check` в kacho-iam, и это одинаково для **обоих** listener'ов

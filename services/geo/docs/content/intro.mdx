...
<header className={hero.hero}>
  ...
  <div className={hero.actions}>
    ...
    <a className={hero.btnGhost} href="https://github.com/PRO-Robotech/kacho/tree/main/services/geo">GitHub</a>
  </div>
</header>
...
ресурсов платформы id генерируется сервером через `corelib/ids`).
:::
...
<div className={hero.featureGrid}>
  ...
  <div className={hero.featureCard}>
    <span className={hero.featureIcon}>🔑</span>
    <span className={hero.featureTitle}>Аутентификация на обоих портах</span>
    <span className={hero.featureBody}>AuthN обязательна везде; per-RPC authz Check через kaname — на всех RPC, кроме публичного чтения справочника (задокументированное исключение).</span>
  </div>
  <div className={hero.featureCard}>
    <span className={hero.featureIcon}>📓</span>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Protocol Buffers / Buf</td><td>Контракт API (<code>proto/</code>, домен <code>kacho.cloud.geo.v1</code>)</td></tr>
...
<table>
  ...
  <tbody>
    <tr><td><strong>services/geo/</strong></td><td>Этот сервис: control-plane Geography (Region / Zone)</td></tr>
    <tr><td><strong>proto/</strong></td><td>Центральные <code>.proto</code> + сгенерированные Go-stubs</td></tr>
    <tr><td><strong>pkg/</strong></td><td>Общие пакеты (db, grpcsrv, grpcclient, config, observability, ...)</td></tr>
    <tr><td><strong>gateway/</strong></td><td>Edge: gRPC-proxy + REST mux</td></tr>
    <tr><td><strong>services/iam/</strong></td><td>Авторизация — per-RPC Check (ребро geo → iam)</td></tr>
    <tr><td><strong>services/compute/ · services/vpc/</strong></td><td>Peer-консумеры: валидируют <code>zoneId</code> / <code>regionId</code> через Kachō Geo</td></tr>
  </tbody>
</table>

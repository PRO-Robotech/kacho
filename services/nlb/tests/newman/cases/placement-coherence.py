...
def _cidr_pre():
    """Pre-request: run-scoped v4 /24 и v6 /64, ВЫРЕЗАННЫЕ ИЗ ПЛАНА сети посева.

    Здесь стоял собственный генератор, выводивший адрес из хеша прогона независимо от
    того, что объявила сеть. Сторона v6 собиралась склейкой `'fd' + число.toString(16)`,
    и это давало ТРЁХразрядный первый хекстет всякий раз, когда число укладывалось в
    один шестнадцатеричный разряд: текстовый префикс «fd» числовым не является. Ширину
    хекстета починили дополнением до двух цифр — починка верна ровно для плана
    `fd00::/8` и не значит ничего для любого другого. Второй посев набора объявляет
    `fd00:196::/48`, и мимо него уходил КАЖДЫЙ адрес этой формулы, при любой ширине.

    Разводка параллельных прогонов сохранена (хеш runId + порядковый номер + соль
    набора); изменился источник префикса — он читается из плана, который посев
    публикует. Разбор класса и гейт, который держит это свойство, — scripts/gen.py,
    раздел «АДРЕС НАРЕЗАЕМОЙ ПОДСЕТИ».
    """
    return carve_cidr_pre('placement-coherence', v4_var='_zcV4Cidr', v6_var='_zcV6Cidr')

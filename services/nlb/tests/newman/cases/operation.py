...
CASES.append(Case(
    ...
    steps=[
        ...
        # и терпим уже-удалённое. Терпимость здесь не маска — она названа: после
        # отмены удаления ресурс законно может как остаться, так и исчезнуть.
        Step(name="cleanup-idem", method="DELETE", path="/nlb/v1/networkLoadBalancers/{{nlbId}}",
             test_script=[
                 "pm.test('уборка: удалено либо уже отсутствует', () => "

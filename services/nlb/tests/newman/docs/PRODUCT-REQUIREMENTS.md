...
| **REQ-AZD-CATALOG** | The `loadbalancer` module and its resources (`networkLoadBalancers`, `listeners`, `targetGroups`) MUST be present and verb-bearing in the live grantable catalogue, with the closed verb set — otherwise a `loadbalancer.*` grant compiles to a silent no-op. The compiled per-RPC permission table is a separate artefact, generated from proto and byte-compared by `make permission-catalog-check`. | AZD-019 | `AZD-PERMISSION-CATALOG-COMPLETE` (reads `GET /iam/v1/permissionCatalog`) |

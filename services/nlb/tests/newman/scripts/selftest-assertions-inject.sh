...
K_HDR_EXTRA='шапка называет предикат(ы) F'
...
assert 'все дефектные оси названы' red "$K_A" "$K_D" "$K_Z" "$K_ZH" -- "$K_G" "$K_I" "$K_K" "$K_B" "$K_V" "$K_E" "$K_HDR_MISS" "$K_HDR_EXTRA" "$K_HDR_BLIND"

echo "── ось 4 с другой стороны: мир успеха всегда объявлен различающим ──"
mutate 'let distinguishes = false;' 'let distinguishes = true;'
assert 'те же дефектные оси названы' red "$K_A" "$K_D" "$K_Z" "$K_ZH" -- "$K_G" "$K_I" "$K_K" "$K_B" "$K_V" "$K_E" "$K_HDR_MISS" "$K_HDR_EXTRA" "$K_HDR_BLIND"
...
mutate '//   E (ведомость ожидания окна прав)' '//   F (запись без предиката) — такого накопителя вердикт не несёт

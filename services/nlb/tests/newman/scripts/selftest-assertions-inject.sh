...
assert 'все дефектные оси названы' red "$K_A" "$K_D" "$K_Z" "$K_ZH" -- "$K_G" "$K_I" "$K_K" "$K_B" "$K_V" "$K_E"

echo "── ось 4 с другой стороны: мир успеха всегда объявлен различающим ──"
mutate 'let distinguishes = false;' 'let distinguishes = true;'
assert 'те же дефектные оси названы' red "$K_A" "$K_D" "$K_Z" "$K_ZH" -- "$K_G" "$K_I" "$K_K" "$K_B" "$K_V" "$K_E"

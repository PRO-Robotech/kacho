...
//       вид законного утверждения обвинялся бы напрасно.              → ОТКАЗ
//
...
// caseObjectsWhenStepIsRefused — возражает ли КЕЙС, когда ЭТОТ шаг получил отказ, а
// прочие шаги — успех. Спрашивается ИСПОЛНЕНИЕМ, а не разбором текста: как именно
// исход публикуется (переменная окружения, ветка скрипта, что угодно ещё), гейт знать
// не обязан — он смотрит на последствия. Поэтому у ветки нет перечня «законных форм
// записи», который пришлось бы держать полным.
function caseObjectsWhenStepIsRefused(steps, target, refusal) {
  for (const shapeIdx of [0, 1]) {
    const env = baseEnv();
    for (const st of steps) {
      const world = st === target ? refusal : successShapesFor(st)[shapeIdx];
      const r = runStep(st, world, env);
      if (r.failed.length > 0 || r.threw) return true;
    }
  }
  return false;
}
...
function checkCaseA(caseItem, steps) {
  ...
  for (const st of steps) {
    ...
    for (const ref of REFUSALS) {
      ...
      //
      // Спрашиваем прямо: пусть ЭТОТ шаг получит отказ, а прочие — успех. Кейс,
      // связавший исход с последующим шагом, на такой разнородности возражает —
      // значит отказ для него НЕ безразличен, и находки нет. Кейс, опубликовавший
      // исход и ничем его не связавший, молчит и здесь — находка остаётся.
      //
      ...
      // не умел спросить. Обе стороны закреплены инъекцией (prove: A(г) и A(д)).
      if (caseObjectsWhenStepIsRefused(steps, st, ref)) {

...
_missing=0
for _res in load-balancer listener target-group targets operation authz-deny cross-resource list-filter placement-coherence; do
...
[[ "$_missing" -eq 0 ]] || { echo "FATAL: nlb suite produced incomplete reports — NOT green (false-green guard)" >&2; exit 1; }

...
<header className={hero.hero}>
  ...
  <div className={hero.actions}>
    ...
    <a className={hero.btnGhost} href="https://github.com/PRO-Robotech/kacho/tree/main/services/nlb">GitHub</a>
  </div>
</header>
...
Идентификаторы ресурсов генерирует сервер (`corelib/ids`): 3-символьный префикс +
...
- **LoadBalancer → TargetGroup** — связи напрямую **нет**: балансировщик знает о группах
  только через свои листенеры. Одну группу можно направить трафиком с нескольких листенеров и
  нескольких балансировщиков; источник истины связи — поле `target_group_id` листенера, и
  других мест, где она хранится, не существует.
...
flowchart LR
  ...
  subgraph cp["Control plane"]
    ...
    iam["kaname<br/>authz Check · Project · RegisterResource"]
  end
  ...
  nlb -->|"SQL · pgx"| pg
  nlb -->|"authz Check · Project · RegisterResource"| iam
...
<div className={hero.featureGrid}>
  <div className={hero.featureCard}>
    ...
    <span className={hero.featureBody}>Единый контракт на Protocol Buffers (<code>proto/</code>, домен <code>kacho.cloud.loadbalancer.v1</code>), REST-проекция через grpc-gateway.</span>
  </div>
  <div className={hero.featureCard}>
    <span className={hero.featureIcon}>◎</span>
  ...
  <div className={hero.featureCard}>
    ...
    <span className={hero.featureBody}>Per-RPC authz Check через kaname — и на public (:9090), и на internal (:9091).</span>
  </div>
</div>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Protocol Buffers / Buf</td><td>Контракт API (<code>proto/</code>, домен <code>kacho.cloud.loadbalancer.v1</code>)</td></tr>
...
<table>
  ...
  <tbody>
    <tr><td><strong>services/nlb/</strong></td><td>Этот сервис: control-plane Load Balancing (NLB / Listener / TargetGroup)</td></tr>
    <tr><td><strong>proto/</strong></td><td>Центральные <code>.proto</code> + сгенерированные Go-stubs</td></tr>
    <tr><td><strong>pkg/</strong></td><td>Общие пакеты (db, grpcsrv, grpcclient, operations, outbox, config, ...)</td></tr>
    <tr><td><strong>gateway/</strong></td><td>Edge: gRPC-proxy + REST mux</td></tr>
    <tr><td><strong>services/iam/</strong></td><td>Авторизация (Check), ProjectService, регистрация владения ресурсами</td></tr>
    <tr><td><strong>services/vpc/ · services/compute/ · services/geo/</strong></td><td>Владельцы ссылок: Address/Subnet/NIC · Instance · Region</td></tr>
  </tbody>
</table>

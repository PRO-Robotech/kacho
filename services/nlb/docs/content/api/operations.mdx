...
`GET /subscription/v1/events?owner=loadbalancer` (виды предмета — `nlb_load_balancer`,
`nlb_listener`, `nlb_target_group`). События несут вид, идентификатор, проект и род изменения.
Состояние предмета приходит **только у вида `nlb_listener`** (у создания и правки) — полный
`Listener`, тот же, что отдаёт `Get`, вместе с метками; у балансировщика и целевой группы
состояния нет, и предмет перечитывается по идентификатору. Отличать не по виду, а по самому
событию: отсутствие состояния приходит признаком `stateUnavailable.reason = NOT_PRODUCED`.

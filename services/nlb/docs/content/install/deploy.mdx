...
оба listener'а обязаны работать по mTLS, а адрес kaname для per-RPC authz — быть
...
<table>
  ...
  <tbody>
    ...
    <tr><td>FGA register-drainer</td><td>Досылает owner/hierarchy-tuples в kaname из <code>fga_register_outbox</code></td><td>по NOTIFY</td></tr>
  </tbody>
</table>
...
readiness остаётся `NotReady`, пока сервис не убедился, что kaname достижим (соединение

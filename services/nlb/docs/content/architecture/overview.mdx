...
observability) приходит из `pkg/` монорепо — не дублируется в сервисе.
...
`23514`→InvalidArgument, кроме формы имени: её проверяет сам сервис, поэтому
срабатывание `<таблица>_name_check` — дефект сервиса и `Internal`). Текст pgx
наружу не утекает (→ фиксированный INTERNAL).
:::
...
Отдельный **register-outbox** (канал `kacho_nlb_fga_register_outbox`) через corelib
drainer доставляет владение и место ресурса в цепи областей в kaname (at-least-once,
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>nlb → kaname</code></td><td><code>ProjectService.Get</code> (existence), <code>InternalIAMService.Check</code> (authz), <code>RegisterResource</code> (FGA owner-tuples)</td><td>sync + drainer</td></tr>
  </tbody>
</table>

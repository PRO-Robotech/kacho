...
listener'ах, verb-bearing отношения, scope-filtered List и регистрацию владения в kacho-iam.
...
- **AuthZ** — каждый RPC (public и internal) проходит per-RPC `InternalIAMService.Check` в
  kacho-iam. Internal-периметр **не считается доверенным**
...
flowchart LR
  ...
  nlb -->|"Check(principal, relation, scope-object)"| iam["kacho-iam"]
  iam -->|"allow / deny"| nlb
...
## Регистрация владения в kacho-iam
...
модель прав напрямую, только через kacho-iam.

...
## `InternalResourceLifecycleService.Subscribe` — потоковое чтение без потребителей

**Отступление от чего.** `api-conventions.md` объявляет, что потокового чтения в
платформе нет: `Get`/`List` синхронны, мутации асинхронны через `Operation`, клиент
поллит, «Watch RPC не существует». У nlb такой RPC есть — `kacho.cloud.loadbalancer.v1.
InternalResourceLifecycleService/Subscribe`, server-stream событий жизненного цикла.

**Потребителей — ноль, и это надо назвать точно.** Предикат:
`git grep NewInternalResourceLifecycleServiceClient` → сгенерированный код плюс ОДИН
вызов, и тот из интеграционной пробы самого сервиса. То есть единственный потребитель —
проба, доказывающая, что поверхность работает. Объявления того же сервиса в доменах vpc
и compute сняты раньше (надгробие `retiredRPCSurface`, формулировка «без сервера,
клиента и неgenerated-ссылок»); у nlb сервер есть, а вызывающего нет.

**Предмет НЕ равен очереди.** Строки очереди жизненного цикла пишутся независимо от
того, подписан ли кто-нибудь: их читают перемещение балансировщика и перемещение
целевой группы. Снятие RPC — это снятие ПОДПИСКИ, а не журнала. Тот, кто возьмётся,
обязан проверить это пофайлово, а не предположить.

**Исход назван, и он один: СНЯТЬ** — задача **#814**, там предикат снятия и радиус
(41 файл: контракт, стабы, обе копии каталога прав, регистрация в REST-mux края на
внутреннем слушателе, список проксирования, конфигурация края и сервиса, обработчик,
пробы, negative-кейс newman и его коллекция, документы проб). Запись существует затем,
чтобы поверхность без потребителя не выглядела решённым вопросом.

**Отдельно — что придётся сделать с negative-кейсом.** Проба
`/nlb/v1/internal/resourceLifecycle:subscribe` в `authz-deny` целит в этот метод. После
снятия она пройдёт потому, что метода нет, а не потому, что запрет держится, — то есть
станет проверкой без содержания. Её надо перенацелить на живой внутренний глагол либо
снять вместе с предметом; это часть #814, а не мелочь уборки.

_Reviewed 2026-08-20 (задача #761, исход назван; снятие — #814)._

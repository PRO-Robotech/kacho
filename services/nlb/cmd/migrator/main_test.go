...
//
// Фикстура отличается от законной посадки службы ОДНИМ фактом — в ней не
// сужен круг отправителей чужой личности, — и второй половиной пробы это
// утверждается прямо: та же самая строка через дверь СЛУЖБЫ обязана быть
// отвергнута с именем ручки. Без этой половины «накатчик прошёл» зеленело бы
// и на конфигурации, которая посадку службы проходит целиком, то есть проба
// не различала бы двери.
func TestBuildRunner_ConfigLaneAsksOnlyWhatTheApplyPointUses(t *testing.T) {
	...
	if _, err := config.Load(path); err == nil {
		t.Fatal("дверь службы приняла конфигурацию БЕЗ сужения круга отправителей — " +
			"страж посадки ослаблен, и первая половина пробы больше ничего не различает")
	} else if !strings.Contains(err.Error(), "trusted-forwarder-sans") {

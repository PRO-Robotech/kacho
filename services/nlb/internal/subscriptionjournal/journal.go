...
//
// Для `nlb_load_balancer` их сегодня восемь, и три из них — SQL:
//
//   - четыре пути Go (создание, правка, переезд, удаление) — запись у них на
//     руках, обогащаются прямо;
//   - переезд отдаёт только идентификаторы (`lbMovedPayload`) — то есть даже
//     среди путей Go есть заведомо частичный;
//   - пересчёт состояния — ТРИГГЕР БАЗЫ (`lb_status_recompute`), пишущий
//     `{id, status, recomputed}`. Он живёт в ПРИМЕНЁННЫХ миграциях, править их
//     нельзя, а собрать полный предмет в PL/pgSQL значит завести ВТОРУЮ проекцию
//     ресурса — на другом языке, рядом с первой, и расходиться они будут молча.
//
// Предикаты, чтобы это перемерить, а не поверить:
//
//	git grep -n OutboxResourceLoadBalancer -- 'services/nlb/**/*.go' | grep -v _test
//	git grep -n 'INSERT INTO kacho_nlb.nlb_outbox' -- 'services/nlb/internal/migrations/*.sql' \
//	  'services/nlb/internal/apps/**/*.go'
//

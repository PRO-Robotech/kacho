...
//
// nil checkClient or system/empty subject (breakglass/dev — the interceptor
// bypasses the source object's own Check for the same reason) → skip.
...
// targetGroupCheckErr maps a TG-authz Check error to a gRPC status (fail-closed):
// no-path → PermissionDenied; iam unavailable → Unavailable; bad args →
// InvalidArgument; anything else → Internal (fixed text, no leak).

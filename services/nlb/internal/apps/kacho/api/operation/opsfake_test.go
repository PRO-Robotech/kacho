...
// ListOwned возвращает операции владельца. Ownership-предикат — ТОТ ЖЕ ownerMatches,
// что и в GetOwned: в pgRepo он живёт в SQL WHERE, и разойдись fake с ним — unit-тест
// перестал бы ловить утечку чужих операций в tenant-facing List. Пагинация/фильтры не
// моделируются (обход map недетерминирован) — keyset проверяет integration-тест.
func (r *fakeOpsRepo) ListOwned(_ context.Context, _ operations.ListFilter, owner operations.Owner) ([]operations.Operation, string, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]operations.Operation, 0, len(r.ops))
	for _, op := range r.ops {
...
// ListOwned зеркалит getErr (репо «сломано» на любом чтении) — тесты error-пути
// ожидают ту же ошибку, что и от GetOwned.
func (r *failingOpsRepo) ListOwned(_ context.Context, _ operations.ListFilter, _ operations.Owner) ([]operations.Operation, string, error) {
	return nil, "", r.getErr
}

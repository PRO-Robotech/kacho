...
func TestDeletionRefusalCarriesTheLaneAndTheCode(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			src := refusal.Wrap(tc.lane, refusal.Ref{ResourceType: "thing", ResourceID: "id-1"},
				fmt.Errorf("%w: load balancer has listeners", domain.ErrFailedPrecondition))
			got := MapDomainErr(src)

			require.Equal(t, codes.FailedPrecondition, status.Code(got),
				"код полосы обязан остаться кодом предусловия")
			require.Equal(t, "load balancer has listeners", status.Convert(got).Message(),
	...
	plain := MapDomainErr(fmt.Errorf("%w: load balancer has listeners", domain.ErrFailedPrecondition))
	require.Equal(t, codes.FailedPrecondition, status.Code(plain))
	require.Equal(t, "load balancer has listeners", status.Convert(plain).Message())

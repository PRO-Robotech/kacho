...
func quotaRefusal(err error) (error, bool) {
	...
	// Префикс снимает ТОТ sentinel, который распознал switch выше, — поэтому
	// клиент видит предложение производителя и ничего сверх него, ровно как у
	// пяти остальных владельцев учёта (задача продукта #1658). Прежде здесь
	// передавалось `sent.Error()` в параметр `fallback`, который читался только
	// при `err == nil`: на этом пути значение вычислялось и выбрасывалось.

...
// микросекунды» — и это относится к каждому ресурсу И каждой под-записи.
// Operation — ответ КАЖДОЙ мутации, то есть самая частая поверхность, на
// которой микросекунды могут утечь.
//
// Здесь это тем важнее, что `OperationToProto` в этом пакете объявлен единым
// источником истины для всех use-case nlb (см. его комментарий: раньше он был
// скопирован byte-for-byte в четырёх местах и успел разойтись). Одно место —
// один дефект во всех четырёх; одна проба — один фикс.
//
// # Почему проба смотрит на остаток, а не на равенство
//
// Сравнение с заранее усечённым образцом зеленеет и тогда, когда усечения нет,
// но вход случайно оказался круглым. Вход здесь намеренно НЕ круглый;
// положительный контроль рядом утверждает, что секундная часть сохранена —
// иначе «усечено» было бы неотличимо от «обнулено».

...
// раз (см. TestMain и corelib/pgtest). Возвращает DSN с
...
func insertTargetGroup(t testing.TB, ctx context.Context, pool *pgxpool.Pool, delaySec int) (id, projectID string) {
	...
	_, err := pool.Exec(ctx, `
		INSERT INTO kacho_nlb.target_groups
			(id, project_id, region_id, name, deregistration_delay_seconds, status, port)
		VALUES ($1, $2, 'ru-central1', $1, $3, 'ACTIVE', 8080)

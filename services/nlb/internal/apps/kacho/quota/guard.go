...
//
// Ребро «владелец величин → владелец типа» завести нельзя: kacho-iam остаётся
...
type ResolvedLimit struct {
	...
	// двухчастный токен родительского типа у вложенного вида.
	//
	// Приезжает ОТВЕТОМ владельца величин и НЕ выводится из формы токена.
	// Прежде здесь считались точки: три части токена читались как «вложенный».
	// Совпадение это давало верное, но опиралось на форму имени, а не на факт
	// каталога, — а форма носителя не определяет (`iam.project` — двухчастный
	// вид, чей носитель не проект). Носитель теперь везут, а не угадывают.
	Carrier string

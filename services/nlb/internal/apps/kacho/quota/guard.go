...
// isNestedKind — вид считается в ОДНОМ родителе, а не в носителе целиком.
//
// Признак — три части токена (`<домен>.<родитель>.<ребёнок>`), ровно как его
// определяет каталог владельца величин. Своей копии каталога здесь нет и быть не
// должно: перечень видов приезжает ответом, и вторая копия разошлась бы с ним
// молча.
func isNestedKind(kind string) bool {
	return strings.Count(kind, ".") == 2
}

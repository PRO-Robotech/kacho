...
func (c Config) Validate() error {
	...
	errs = multierr.Append(errs, c.ValidateQuotaCeilings())
	...
	// носитель контура (`corelib/servicehost`) ставит звено решения ВСЕГДА, и поля,
	...
	if mode.IsProduction() {
		...
		// оба приходят из дома семантики DSN (`corelib/db`), где объявлены один раз

// Copyright (c) PRO-Robotech
// SPDX-License-Identifier: BUSL-1.1

package config

// ratelimit_test.go — ключи потолка допуска в файле настроек ДОЕЗЖАЮТ до полей.
//
// # Почему без этой пробы опечатка в ключе молчит
//
// Незаданная секция означает ПОЛ ПЛАТФОРМЫ, и это правильно. Цена правильного
// умолчания — что опечатка в ключе (`rate-limits` вместо `rate-limit`,
// `in-flight-limit` вместо `in-flight`) выглядит В ТОЧНОСТИ как «посадка не
// переопределяла»: viper незнакомый ключ просто игнорирует, поле остаётся нулём,
// подставляется пол, процесс поднимается и пишет в журнал взведённый потолок.
// Ручка объявлена, задокументирована — и задать её нельзя ни при каком вводе.

import "testing"

// TestRateLimitFileKeysArmTheFields — все восемь ключей (четыре оси × два
// слушателя) доезжают, и слушатели читают РАЗНЫЕ ключи.
func TestRateLimitFileKeysArmTheFields(t *testing.T) {
	p := writeYAML(t, `
mode: dev
logger:
  level: INFO
api-server:
  endpoint: tcp://0.0.0.0:9090
  internal-endpoint: tcp://0.0.0.0:9091
  graceful-shutdown: 10s
  rate-limit:
    public:
      read-per-sec: 7
      mutation-per-sec: 3
      burst-factor: 2
      in-flight: 5
    internal:
      read-per-sec: 70
      mutation-per-sec: 30
      burst-factor: 4
      in-flight: 50
repository:
  type: POSTGRES
  postgres:
    url: postgres://kacho:secret@db.example/kacho_nlb?sslmode=disable
`)
	c, err := Load(p)
	if err != nil {
		t.Fatalf("настройки не загрузились: %v", err)
	}
	if got := c.APIServer.RateLimit.Public; got.ReadPerSec != 7 || got.MutationPerSec != 3 ||
		got.BurstFactor != 2 || got.InFlight != 5 {
		t.Fatalf("группа публичного слушателя не доехала из файла: %+v\n\n"+
			"Опечатка в ключе неотличима от «посадка молчит»: поле остаётся нулём, "+
			"подставляется пол платформы, и ручка не работает ни при каком вводе", got)
	}
	if got := c.APIServer.RateLimit.Internal; got.ReadPerSec != 70 || got.MutationPerSec != 30 ||
		got.BurstFactor != 4 || got.InFlight != 50 {
		t.Fatalf("группа внутреннего слушателя не доехала из файла: %+v", got)
	}
	if c.APIServer.RateLimit.Public == c.APIServer.RateLimit.Internal {
		t.Fatalf("обе группы прочитали одно и то же (%+v) — значит ключи совпали, и посадка не "+
			"может задать слушателям разные величины", c.APIServer.RateLimit.Public)
	}
}

// TestRateLimitDefaultsToSilent — законный близнец: без секции ручки МОЛЧАТ,
// то есть носитель подставит пол платформы.
//
// Без него проба выше зеленела бы и на конфигурации, где величины проставляет
// чей-то `defaults.go`, — а тогда «посадка назвала свои» стало бы неотличимо от
// «посадка молчит», и страж, который на этом различии стоит, ослеп бы.
func TestRateLimitDefaultsToSilent(t *testing.T) {
	c, err := Load(writeYAML(t, minimalValidYAML))
	if err != nil {
		t.Fatalf("настройки не загрузились: %v", err)
	}
	if !c.APIServer.RateLimit.Public.IsSilent() || !c.APIServer.RateLimit.Internal.IsSilent() {
		t.Fatalf("незаданные ручки не молчат (public=%+v internal=%+v)",
			c.APIServer.RateLimit.Public, c.APIServer.RateLimit.Internal)
	}
}

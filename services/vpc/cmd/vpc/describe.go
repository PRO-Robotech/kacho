...
// dataplaneStreamLifetime — срок жизни подписки на намерение датаплейна.
//
// Ручкой конфигурации не является намеренно: величина — свойство контракта
// подписки, а не посадки. Оператору нечего выбирать здесь под свой стенд, а
// объявленная ручка требовала бы читателя, значения по умолчанию и отказа
// старта на бессмысленном значении — три сущности ради числа, у которого одна
// осмысленная величина.
//
// Обязана превосходить `HandlingBudget` (носитель проверяет это отказом
// старта): срок одиночного вызова и срок подписки — разные предметы, и подписка,
// накрытая сроком запроса, рвалась бы раз в этот срок, а клиент читал бы это как
// сетевой сбой.
const dataplaneStreamLifetime = time.Hour
...
func describe(
	...
	return servicecontract.New(servicecontract.Spec{
		...
		// Срок жизни подписки — ВЕЛИЧИНА. Прежде здесь стояло изъятие «серверных
		// стримов kacho-vpc не служит»; оно истекло от появления предмета:
		// `InternalDataplaneService/WatchIntent` отдаёт поток намерения
		// исполнителю датаплейна. Заявление судится СЛУЖИМЫМ набором, а не
		// памятью автора (О11), поэтому изъятие уронило бы старт и назвало метод.
		//
		// Величина — не «сколько живёт запрос», а сколько живёт ПОДПИСКА, и
		// разница здесь несущая: подписка обязана переживать любые тихие часы, но
		// не обязана жить вечно. Час выбран так, чтобы переподключение было
		// штатным событием, наблюдаемым в журнале, а не редкостью, о которую
		// спотыкаются: исполнитель продолжает с известной ревизии, поэтому
		// переподключение стоит одного запроса границ журнала. Долгоживущий
		// поток, никогда не переустанавливаемый, скрывает как раз обратное —
		// разучившийся продолжаться клиент.
		StreamBudget: servicecontract.Value(dataplaneStreamLifetime),

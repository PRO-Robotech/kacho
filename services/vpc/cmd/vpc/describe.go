...
func describe(
	...
	return servicecontract.New(servicecontract.Spec{
		...
		// Срок жизни подписки — ИЗЪЯТИЕ: серверных потоков kacho-vpc не служит.
		//
		// Изъятие здесь уже стояло, потом истекло от появления предмета
		// (`InternalDataplaneService/WatchIntent` отдавал поток намерения
		// исполнителю датаплейна) — и теперь возвращается вместе со снятием того
		// же предмета: шов снят целиком, исполнителя не существует (kacho#400).
		//
		// Возврат обязателен, а не косметичен. Величина, объявленная для потока,
		// которого нет, есть утверждение о посадке, за которым не стоит ни одного
		// служимого метода: дескриптор говорил бы, сколько живёт подписка, при
		// нулевом наборе подписок. Заявление судится СЛУЖИМЫМ набором, а не
		// памятью автора (О11) — поэтому оставленная величина уронила бы старт
		// ровно с той же строгостью, с какой раньше его роняло изъятие.
		//
		// Заведут поток снова — старт откажет и назовёт метод, и величину придётся
		// вернуть осознанно, а не унаследовать.
		StreamBudget: servicecontract.NotApplicable[time.Duration](
			"kacho-vpc не служит ни одного серверного потока: шов исполнителя " +
				"датаплейна снят целиком вместе со своим единственным потоком " +
				"(kacho#400), исполнителя не существует"),

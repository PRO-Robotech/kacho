...
func describe(
	...
	return servicecontract.New(servicecontract.Spec{
		...
		// Срок жизни подписки — ИЗЪЯТИЕ: vpc не служит ни одного серверного
		// стрима. Заявление судится СЛУЖИМЫМ набором, а не памятью автора:
		// носитель снимает признак стрима с дескрипторов методов у самих серверов
		// (О11). Появится у vpc первая подписка — процесс не поднимется и назовёт
		// её метод.
		StreamBudget: servicecontract.NotApplicable[time.Duration](
			"серверных стримов kacho-vpc не служит: ни один метод доменов " +
				"kacho.cloud.vpc.v1 и kacho.cloud.operation не отдаёт поток событий"),

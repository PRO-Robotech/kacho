...
type ResolvedLimit struct {
	...
	//
	// Приезжает ОТВЕТОМ и не выводится: форма токена носителя не определяет
	// (`iam.project` — двухчастный вид, чей носитель не проект). Догадка здесь
	// не отказывает громко — она заводит строку под неверным носителем, и
	// потребление такой строки не наполняется никогда.
	Carrier string

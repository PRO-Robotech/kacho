...
//
// ВОСЕМЬ, А НЕ ДВЕНАДЦАТЬ — свойство настоящего резолва, а не удобство дублёра.
// Каталог держит у домена двенадцать видов; четыре считаются в РОДИТЕЛЬСКОМ
// ресурсе (сколько подсетей в сети, сколько интерфейсов в подсети), и резолв по
// корню аренды их не отвечает: на уровне проекта у такого вида нет единственного
// значения. Пока фильтра не было, дублёр отдавал восемь там, где настоящий отдавал
// двенадцать, — то есть был СНИСХОДИТЕЛЬНЕЕ настоящего и делал невидимым ровно тот
// дефект, ради которого его подставляют: четыре строки уезжали арендатору с
...
// twelveKindsWithNested — набор, в котором плоские виды смешаны с вложенными.
//
// Настоящий резолв столько НЕ отдаёт: с тех пор как он отвечает только про виды
// корня аренды, вложенные четыре в ответ не попадают. Фикстура держит другое —
// устойчивость ПОТРЕБИТЕЛЯ к строке, которую считает не он: сосед вправе однажды
// нарушить контракт, и тогда важно, чтобы vpc не разложил такую строку по носителю
// «проект», а отбросил её. Проверка того, что сосед контракт соблюдает, живёт у
// соседа (`TestResolveEffective_AnswersOnlyTenancyRootKinds` в домене iam) — здесь
// она была бы утверждением о чужом коде.

...
// eightKinds — ПОДНАБОР ответа владельца величин: только виды, считаемые в
// проекте.
//
// ВОСЕМЬ — НЕ ответ настоящего резолва. Настоящий отдаёт домену двенадцать:
// восемь проектных и четыре считаемых в РОДИТЕЛЬСКОМ ресурсе (сколько подсетей
// в сети, сколько интерфейсов в подсети). Здесь недолго стояло обратное
// утверждение — резолв тогда вложенные вырезал, — но вырезание снято как дефект
// (`d13e167d`): оно лишало владельца типа величины, из которой берётся снимок
// при заведении родителя, и первый же ребёнок получал отказ при потолке,
// названном каталогом.
//
// ПОЧЕМУ ПОДНАБОР ЗДЕСЬ ЗАКОНЕН, а не снисходительность дублёра. Потребитель
// отбрасывает строку с чужим носителем на входе (`guard.go`, `states.go`),
// поэтому для проб допуска, полос и состояний исход на восьми и на двенадцати
// ТОЖДЕСТВЕН — лишние четыре были бы отброшены. Что отбрасывание действительно
// стоит, здесь не утверждается и утверждаться не может: это предмет
// `carrier_test.go`, где фикстура несёт вложенные виды и снятие отбрасывания
// краснеет.
//
// Носитель проставлен У КАЖДОЙ строки по той же причине: настоящий резолв его
// проставляет всегда, потому что вид приходит из каталога, а запись каталога есть
// ПАРА — вид и носитель. Дублёр, отдающий строку без носителя, был бы снисходительнее
// настоящего и сделал бы невидимым дефект, ради которого носитель повезли по проводу
// (`PRO-Robotech/kacho#401`).
//
// Вложенные четыре живут в `twelveKindsWithNested` рядом.
...
// twelveKindsWithNested — ПОЛНЫЙ ответ владельца величин: плоские виды вместе с
// вложенными.
//
// Именно столько настоящий резолв и отдаёт. Здесь недолго стояло обратное —
// «сосед столько не присылает, фикстура держит устойчивость к нарушению
// контракта», — и это перестало быть верным вместе со снятием вырезания
// (`d13e167d`): вложенные четыре приходят ВСЕГДА и по контракту, а не как чужая
// ошибка. Отбрасывать их по носителю — штатная работа потребителя, а не
// оборона от соседа.
//
// Поэтому набор годится как эталон входа: дублёр, отдающий меньше, был бы
// снисходительнее настоящего и сделал бы невидимым дефект, ради которого
// носитель повезли по проводу (`PRO-Robotech/kacho#401`).

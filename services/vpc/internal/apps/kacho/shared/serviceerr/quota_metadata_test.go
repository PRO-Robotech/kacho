...
func TestQuotaRefusalNotProvisionedCarriesTheCarrierWithoutAmounts(t *testing.T) {
	const detail = `{"carrier_type": "project", "carrier_id": "prj-1", "kind": "vpc.gateway"}`
	err := quotadetail.Attach(
		fmt.Errorf("%w: project prj-1 has no ceiling stated for vpc.gateway",
...
func TestQuotaDetailKeepsTheSentinelRecognisable(t *testing.T) {
	err := quotadetail.Attach(
		fmt.Errorf("%w: project prj-1 has reached its limit of 4 vpc.network",

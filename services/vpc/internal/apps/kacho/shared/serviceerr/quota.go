...
// классификация (`classifyRepoSentinel`) отвечает только за КОД. Учёту этого
// мало: клиент обязан различать полосы МАШИННО — по `reason`-токену в
// `google.rpc.ErrorInfo`, а не разбором прозы (`api-conventions.md` §By-lane
// code-split). Токен приклеивается здесь, в одном месте на оба публичных
// отображения, чтобы «отказ на внешнем слушателе» и «тот же отказ на внутреннем»
// не разошлись признаком.
...
const quotaReasonDomain = "vpc.kacho.cloud"
...
//
// Текст производителя (`kacho_quota_refuse`, миграция 0041) выносится наружу

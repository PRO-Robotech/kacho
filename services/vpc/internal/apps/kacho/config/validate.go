...
func (c Config) ValidatePeerTransport(m MTLSConfig) error {
	...
	// Ребра vpc→домен величин в этом перечне НЕТ, и это не пропуск. Провод снят
	// вместе с контрактом, которого не осталось ни в одном дереве; страж,
	// требующий удостоверения на несобираемом проводе, есть фантомное
	// требование, пережившее то, что охраняло.

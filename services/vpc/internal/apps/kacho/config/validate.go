...
func (c Config) Validate() error {
	...
	// production-режиме. Спрашиваем ТОТ ЖЕ объект и ТОТ ЖЕ предикат, что и
	// проводка с самоотчётом (grpcsrv.TrustedForwarders.IsNarrowed), а НЕ длину
	// сырого среза: там же, где сужение реально произойдёт, пустые записи
	// отбрасываются, поэтому `SANS=","` (срез из двух пустых строк) прошёл бы
	// проверку длины и вернул дыру. dev осознанно терпит пусто (in-process
	// фикстуры); на развёрнутом стенде dev-посадка запрещена отдельным правилом
	// (production-mode ВЕЗДЕ).
	if c.AuthN.Mode.IsProduction() && !c.TrustedForwarders().IsNarrowed() {
		errs = multierr.Append(errs,

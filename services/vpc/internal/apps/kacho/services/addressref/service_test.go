...
// Тесты референс-сервиса addressref (SetAddressReference/Mark/Clear/Get):
// используют `repomock.AddressRepo` напрямую и конструктор `NewService(repo)`.
...
func seedAddrForRef(ar *repomock.AddressRepo) *kachorepo.AddressRecord {
	rec := &kachorepo.AddressRecord{
		Address: domain.Address{
			...
			ExternalIpv4: &domain.ExternalIpv4Spec{Address: "203.0.113.5", ZoneID: "zone-a"},
		},
	}
	ar.Seed(rec)
	return rec
...
func TestAddressReferenceService_SetAddressReference_OK(t *testing.T) {
	...
	// idempotent re-set с ТЕМ ЖЕ referrer — допустимо (CAS matches).
	ref, err = svc.SetAddressReference(context.Background(), SetAddressReferenceReq{
	...
	// re-set с ДРУГИМ referrer → FailedPrecondition (CAS fail).

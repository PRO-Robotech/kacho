...
func quotaFixture(t *testing.T) (*Handler, *quotaRowsStub, *repoStub, *quotaLimitsStub) {
	...
	rows := &quotaRowsStub{byProject: map[string][]quotaread.State{
		"prj-mine": {
			{Kind: "vpc.network", Limit: 64, Used: 5,
				SourceScope: "DEFAULT",
				CarrierType: quotaread.CarrierProject, CarrierID: "prj-mine"},
			{Kind: "vpc.subnet", Limit: 16, Used: 2,
				SourceScope: "PROJECT", SourceScopeID: "prj-mine",
				CarrierType: quotaread.CarrierProject, CarrierID: "prj-mine"},
		},
		"prj-theirs": {
			{Kind: "vpc.subnet", Limit: 999, Used: 900,
	...
	guard := quotaband.NewGuard(repo, limits, quotaAccountsStub{}, "vpc")
	return NewHandler(guard, quotaread.AuthorityDeclared()), rows, repo, limits
}
...
func TestHandler_ListShowsTheCallersOwnLimitAndUsage(t *testing.T) {
	...
	resp, err := h.List(context.Background(), &vpcv1.ListQuotasRequest{ProjectId: "prj-mine"})
	...
	require.Equal(t, "vpc.subnet", got.GetKind())
...
func TestHandler_ListRefusesAnUnanswerableQuestionByName(t *testing.T) {
	...
	_, err := h.List(context.Background(), &vpcv1.ListQuotasRequest{})
	require.Error(t, err)
...
func TestHandler_ListWithNoBandRefusesInsteadOfClaimingNoQuotas(t *testing.T) {
	h := NewHandler(nil, quotaread.AuthorityDeclared())

	_, err := h.List(context.Background(), &vpcv1.ListQuotasRequest{ProjectId: "prj-mine"})
	require.Error(t, err)

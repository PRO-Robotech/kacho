...
//
// #nosec G101 -- все три значения ниже суть МАШИННЫЙ ПРИЗНАК ПРИЧИНЫ ОТКАЗА,
// уезжающий вызывающему в деталях ответа, а не секрет. Эвристика статического
// анализа ключуется на форме литерала (заглавные с подчёркиваниями) и не
// различает признак публичного контракта от учётных данных.
const (

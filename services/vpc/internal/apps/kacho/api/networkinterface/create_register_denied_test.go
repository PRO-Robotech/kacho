...
// Отказ sync-регистрации owner-tuple приходит ПОСЛЕ `w.Commit()` — на NIC,
// который уже создан (и, при InstanceID, уже приаттачен CAS-ом). Проброс наверх
// отдаёт вызывающему чужой код узла прав (status.FromError забирает вложенный
// статус) и служебную обёртку `rpc error: …` в тексте, оставляя NIC фантомом.
// Intent на тот же tuple durable в fga_register_outbox той же writer-TX.
func TestCreateUseCase_OwnerTupleRegisterDenied_OperationSucceedsAndNICExists(t *testing.T) {
	...
	uc := NewCreateNetworkInterfaceUseCase(kr, &repomock.ProjectClient{OK: true}, or).WithRegistrar(reg)

	op, err := uc.Execute(ctx, CreateInput{NetworkInterface: domain.NetworkInterface{
		ProjectID: "f1",
		Name:      "nic-reg-denied",
		SubnetID:  "e9bsub1",
	}})
	...
	require.Len(t, kr.NetworkInterfaces(), 1, "resource is durable: commit happened before registration")

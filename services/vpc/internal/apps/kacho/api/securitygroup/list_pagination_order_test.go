...
// Свойство закреплено на ЗАСЕЯННОМ репозитории в TestSecurityGroupListPerObject_EmptySubjectFailsClosed этого
...
func TestListPaginationFormatCheckedBeforeIdentityShortCircuit(t *testing.T) {
	...
	t.Run("названный вызывающий — отказ по формату (положительный контроль)", func(t *testing.T) {
		uc := NewListSecurityGroupsUseCase(kachomock.NewRepository(), narrowtest.AllowingAll())

		_, _, err := uc.Execute(narrowtest.Caller(), SecurityGroupFilter{ProjectID: "prj_1"}, Pagination{PageToken: garbageToken})
	...
	t.Run("вызывающий не опознан — тот же отказ по формату", func(t *testing.T) {
		...
		_, _, err := uc.Execute(context.Background(), SecurityGroupFilter{ProjectID: "prj_1"}, Pagination{PageToken: garbageToken})
	...
	t.Run("page_size вне диапазона — отказ независимо от личности", func(t *testing.T) {
		for name, caller := range map[string]context.Context{
			...
			t.Run(name, func(t *testing.T) {
				...
				_, _, err := uc.Execute(caller, SecurityGroupFilter{ProjectID: "prj_1"}, Pagination{PageSize: 1001})
	...
	t.Run("законная первая страница проходит — проба не отвергает всё подряд", func(t *testing.T) {
		...
		_, _, err := uc.Execute(narrowtest.Caller(), SecurityGroupFilter{ProjectID: "prj_1"}, Pagination{PageSize: 50})

...
// Проверки единственной формы имени (#715) для SecurityGroup: строка ресурса не
// может нести пустое имя, и снять имя правкой нельзя.

func nameCanonFixture(t *testing.T) (*CreateSecurityGroupUseCase, *repomock.OpsRepo, string) {
	t.Helper()
...
func createUnnamedSG(t *testing.T, uc *CreateSecurityGroupUseCase,
	or *repomock.OpsRepo, netID string) *vpcv1.SecurityGroup {
	t.Helper()
	op, err := uc.Execute(context.Background(), domain.SecurityGroup{ProjectID: "f1", NetworkID: netID})
	require.NoError(t, err)
	saved := repomock.AwaitOpDone(t, or, op.ID)
	require.Nil(t, saved.Error, "создание без имени обязано пройти: пустое имя означает «назови сам»")
	var got vpcv1.SecurityGroup
...
func TestCreate_EmptyName_WritesIdDerivedDefault(t *testing.T) {
	...
	got := createUnnamedSG(t, uc, or, netID)
...
func TestCreate_TwoEmptyNames_BothSucceedWithDistinctNames(t *testing.T) {
	uc, or, netID := nameCanonFixture(t)
	first := createUnnamedSG(t, uc, or, netID)
	second := createUnnamedSG(t, uc, or, netID)
...
func TestCreate_NameStillValidated(t *testing.T) {
	...
	op, err := uc.Execute(context.Background(), domain.SecurityGroup{
		ProjectID: "f1", NetworkID: netID, Name: domain.RcNameVPC("sg-1"),
	})
...
func TestUpdate_MaskNamesName_EmptyRejected(t *testing.T) {
	err := validateSGUpdate(UpdateInput{
		UpdateMask:    []string{"name"},
		SecurityGroup: domain.SecurityGroup{Name: ""},
	})
	...
	require.NoError(t, validateSGUpdate(UpdateInput{
		UpdateMask:    []string{"name"},
		SecurityGroup: domain.SecurityGroup{Name: "sg-2"},
...
func TestUpdate_EmptyMask_EmptyNameStillAccepted(t *testing.T) {
	require.NoError(t, validateSGUpdate(UpdateInput{SecurityGroup: domain.SecurityGroup{Name: ""}}))
}
...
func TestApply_EmptyMask_EmptyNameKeepsCurrent(t *testing.T) {
	cur := domain.SecurityGroup{Name: "sg-current"}
	applySGMask(&cur, UpdateInput{SecurityGroup: domain.SecurityGroup{Name: ""}})
	assert.Equal(t, domain.RcNameVPC("sg-current"), cur.Name, "полная правка без имени обязана сохранить текущее имя")
...
func TestApply_EmptyMask_NewNameApplied(t *testing.T) {
	cur := domain.SecurityGroup{Name: "sg-current"}
	applySGMask(&cur, UpdateInput{SecurityGroup: domain.SecurityGroup{Name: "sg-new"}})
	assert.Equal(t, domain.RcNameVPC("sg-new"), cur.Name, "непустое имя при полной правке обязано примениться")
}
...
func TestApply_MaskNamesName_NewNameApplied(t *testing.T) {
	cur := domain.SecurityGroup{Name: "sg-current"}
	applySGMask(&cur, UpdateInput{UpdateMask: []string{"name"}, SecurityGroup: domain.SecurityGroup{Name: "sg-new"}})
	assert.Equal(t, domain.RcNameVPC("sg-new"), cur.Name, "маска, назвавшая имя, обязана его применить")
}

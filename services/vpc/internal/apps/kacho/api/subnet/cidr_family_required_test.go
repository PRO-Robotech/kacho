...
func TestSubnetPerFieldRefusalsPrecedeTheAnchorRequirement(t *testing.T) {
	...
	_, err := uc.Execute(context.Background(), domain.Subnet{
		...
		// Имя, начинающееся с ЦИФРЫ, — заведомо неверное: правило домена требует
		// первым символом букву. Заглавные буквы правило ДОПУСКАЕТ
		// (`^([a-zA-Z]…`), поэтому первая редакция этой пробы взяла негодный вход
		// и падала при верном порядке — вход обязан быть проверен на то, что он
		// действительно отвергается, иначе проба измеряет свою опечатку.
		Name:   domain.RcNameVPC("9bad"),
		ZoneID: testZone,
	})

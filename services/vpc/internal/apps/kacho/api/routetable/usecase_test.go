...
// (in-memory CQRS-impl с TX-семантикой и outbox-буфером).

// makeNetwork — создает Network в kachomock-репо. RT.Create / Network-existence
...
func TestCreateUseCase_ProjectNotFound(t *testing.T) {
	...
	uc := NewCreateRouteTableUseCase(kr, &repomock.ProjectClient{OK: false}, or)

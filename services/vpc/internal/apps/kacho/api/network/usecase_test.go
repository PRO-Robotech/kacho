...
//
// Network use-case'ы работают через CQRS-Repository: Network-mock —
// `kachomock.NewRepository()` (in-memory CQRS-impl с TX-семантикой и outbox-
// буфером). Child-ресурсы (Subnet/RouteTable/SecurityGroup/...) подменяются
// `repomock.*`.
...
// TestCreateUseCase_ProjectNotFound — sync project.Exists precheck удален
...
// `CreateDefaultSGUseCase`: use-case работает в УЖЕ открытой caller'ом writer-TX,
// делает Insert(SG) + outbox-emit + SetDefaultSGID + outbox-emit и возвращает
// updated NetworkRecord. Сам TX не открывает и не commit'ит — это

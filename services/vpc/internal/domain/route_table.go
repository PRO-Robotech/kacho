...
type RouteTable struct {
	...
	// SystemOwned — таблицу завела платформа вместе с сетью, а не арендатор.
	//
	// Признак живёт НА САМОЙ СТРОКЕ, хотя у сети уже есть ссылка
	// `default_route_table_id`, и это не дубль: на пути вставки ребёнка ссылка
	// ещё не проставлена (`Network.Create` сначала вставляет строку, потом
	// линкует её), поэтому вопрос «системная ли ты» родителю в этот момент
	// задать нечем. Учёт квоты решает это в момент вставки — и без своего
	// признака счёл бы системную таблицу тенантской, молча превратив предел на
	// таблицы маршрутизации в предел на сети.
	//
	// Симметрия с группами правил: у них тот же признак зовётся
	// `DefaultForNetwork` и существует с baseline'а.
	SystemOwned bool
}

...
// Unit-тесты Equal-методов domain-типов и nested структур. Equal используется
// для noop-detection в Update-flow и для testing-equality в use-case-тестах.
//
// Контракт:
...
//     порядке дает equal=true
//   - Slices (reference-id, CIDR, StaticRoutes, Rules) — order-sensitive:
//     swap двух элементов дает equal=false
//   - CreatedAt НЕ участвует (он в repo-leaf Record, не в domain)
...
func TestLabelsEqual_OrderInsensitive(t *testing.T) {
	...
	// equal. RcLabels — map, итерация order-undefined, но Equal — set-equality,
	// поэтому equal независимо от порядка вставки.
...
func TestNetwork_Equal(t *testing.T) {
	...
	cases := []struct {
		...
		{"diff project", func(n *domain.Network) { n.ProjectID = "fldX" }},

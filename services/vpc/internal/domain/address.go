...
// GatewayAddressName — имя внешнего адреса, заведённого созданием шлюза.
//
// Адрес возникает как следствие создания шлюза, а не по заказу арендатора,
// поэтому имя ему даёт продукт. Оно производится от `id` и потому уникально
// by construction: `id` глобально уникален, значит подбирать свободное имя не
// нужно — а подбор был бы проверкой перед вставкой, запрещённой ban #10.
//
// Здесь имя раньше не задавалось вовсе. Допущение «пустые имена не коллизят,
// потому что частичный UNIQUE считает их различными» перестало быть верным,
// когда форма имени стала одной на дерево: пустое имя не доживает до записи,
// а уникальность держит полный UNIQUE(project_id, name).
func GatewayAddressName(addressID string) RcNameVPC {
	return RcNameVPC("gw-addr-" + TruncateID(addressID))
}

...
// сканирует в string).
// default_route_table_id — тоже nullable (0017, FK на route_tables ON DELETE SET
// NULL), читается через тот же COALESCE.
// ipv4_cidr_blocks / ipv6_cidr_blocks — NOT NULL DEFAULT (0015_network_supernet),
// поэтому читаются напрямую без COALESCE.

...
//  1. InitIPv6PoolCursor — идемпотентен, создает cursor=1.
//  2. AllocateExternalIPv6 — последовательно выдает IP'и pool_base+1, +2, +3,
...
func TestIntegration_AddressRepo_IPv6_AllocateAndFree(t *testing.T) {
	...
	projectID := "f-v6-alloc"
	addr1 := makeAddrShell(projectID, now, "a-v6-1")
	addr2 := makeAddrShell(projectID, now, "a-v6-2")
	addr3 := makeAddrShell(projectID, now, "a-v6-3")
	...
	addr4 := makeAddrShell(projectID, now, "a-v6-4")
	require.NoError(t, withTx(t, func(w kacho.RepositoryWriter) error {
...
func makeAddrShell(projectID string, _ time.Time, name string) *domain.Address {

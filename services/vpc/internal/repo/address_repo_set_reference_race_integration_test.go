...
// Гонка на address_references.SetReference. Защита (как у NIC SetUsedBy):
// SetReference (и MarkEphemeralInUse) делают атомарный single-statement CAS —
// `INSERT … ON CONFLICT (address_id) DO UPDATE … WHERE
// address_references.referrer_id = EXCLUDED.referrer_id`. 0 rows из RETURNING
// (через pgx.ErrNoRows) → helpers.ErrFailedPrecondition.
...
// Re-attach к тому же referrer должен быть idempotent.
func TestIntegration_AddressRepo_SetReferenceIdempotent(t *testing.T) {
	...
	addr := &domain.Address{
		ID:        ids.NewID(ids.PrefixAddress),
		ProjectID: "project-setref-idempotent",

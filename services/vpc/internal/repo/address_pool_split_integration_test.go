...
func TestMigration0022_C4_UniqueConstraintsIntact(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		INSERT INTO addresses (id, name, project_id, addr_type, ip_version, external_ipv4)
		VALUES ($1, $1, 'f1', 1, 1, jsonb_build_object('address','203.0.113.5','address_pool_id',$2::text))
	`, addr1, first)
	require.NoError(t, err)
...
func TestAddressPoolSplit_H2_ExternalPoolIPUniqueAfterSplit(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		INSERT INTO addresses (id, name, project_id, addr_type, ip_version, external_ipv4)
		VALUES ($1, $1, 'f1', 1, 1, jsonb_build_object('address','203.0.113.42','address_pool_id',$2::text))
	`, addr1, poolID)
	require.NoError(t, err)

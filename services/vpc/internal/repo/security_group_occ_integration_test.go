...
// UpdateRules по одной строке SecurityGroup и проверяет, что OCC-guard
// (WHERE-условие на xmin::text) держится: на каждую пару проходит максимум один
// UPDATE (никогда тихий lost-update), а проигравший вызов получает контракт
// конфликта (helpers.ErrFailedPrecondition). Каждый UpdateRules идет в своей
// writer-TX.
//
// Много итераций — чтобы гонка стреляла надежно; глобально требуем, чтобы
// конфликт наблюдался хотя бы раз (иначе OCC-путь — мертвый код).
func TestIntegration_SecurityGroup_UpdateRules_ConcurrentOCC(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		// Свежий SG на итерацию с одним seed-правилом.
		...
		// Каждая горутина открывает свою writer-TX и зовет UpdateRules.
		...
		// Проверка инварианта: какие бы update'ы ни прошли, строка обязана
		// содержать seed-правило плюс добавления каждого успешного writer'а.
		rd, err := r.Reader(ctx)
		require.NoError(t, err)

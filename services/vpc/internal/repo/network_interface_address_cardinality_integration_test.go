...
// DB-level CHECK инвариант: network_interfaces.v4_address_ids / v6_address_ids —
// массивы длины ≤ 1. Service-слой дает sync InvalidArgument до Operation, а
// DB-CHECK — финальный backstop на случай direct repo.Insert / direct SQL / race
// с обойденным валидатором.
func TestIntegration_NICRepo_AddressCardinality_DBCheck(t *testing.T) {
	...
	net := &domain.Network{
		ID: ids.NewID(ids.PrefixNetwork), ProjectID: "project-card", Name: domain.RcNameVPC("net-card"),
	}
	...
	mkNIC := func(suffix string, v4 []string, v6 []string) *domain.NetworkInterface {
		return &domain.NetworkInterface{
			ID: ids.NewID(ids.PrefixSubnet), ProjectID: "project-card",
	...
	require.NoError(t, insertNIC(t, mkNIC("02", []string{"e9bv4single"}, nil)), "ровно один v4 разрешен")

	// 3. v4=[] / v6=[1] — OK.
	require.NoError(t, insertNIC(t, mkNIC("03", nil, []string{"e9bv6single"})), "ровно один v6 разрешен")

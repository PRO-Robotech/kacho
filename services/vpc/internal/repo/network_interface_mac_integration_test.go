...
// внутри облака → Insert возвращает helpers.ErrMacCollision (не
// helpers.ErrAlreadyExists), чтобы NetworkInterfaceService.doCreate мог отличить
// retry-able MAC-коллизию от обычного duplicate-name UNIQUE-нарушения.
func TestIntegration_NICRepo_MacAddressUniqueness(t *testing.T) {
	...
	net := &domain.Network{ID: ids.NewID(ids.PrefixNetwork), ProjectID: "project-mac", Name: domain.RcNameVPC("net-mac")}
	...
	require.NoError(t, withTx(t, func(w kacho.RepositoryWriter) error {
		...
		if e == nil {
			require.Equal(t, "0e:aa:aa:aa:aa:aa", first.MAC, "Insert возвращает сохраненный MAC")
		}
		return e
	...
	// 4. Verify Get возвращает сохраненный MAC обратно.
	rd, err := r.Reader(ctx)
	require.NoError(t, err)

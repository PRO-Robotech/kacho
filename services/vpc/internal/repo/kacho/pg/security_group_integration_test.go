...
// из network_integration_test.go (один пакет).

// helper — создать Network в той же writer-TX (нужен parent для SG.network_id FK).
func insertNetworkInTx(t *testing.T, ctx context.Context, w kacho.RepositoryWriter, projectID, name string) *kacho.NetworkRecord {
	t.Helper()
	n := newNetwork(projectID, name)
...
// outbox-row не остаются. Это ключевое свойство, ради которого атомарное
// создание default-SG живет в одной writer-TX.
...
// Единая writer-TX исключает orphan-window между шагами.

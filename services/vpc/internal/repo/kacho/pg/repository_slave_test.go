...
// по разным pgxpool'ам. Реальная streaming replica на стенде еще не поднята;
// задача тестов — зафиксировать поведение pg-impl:
//   - Repository.Reader(ctx) идет на slave-pool, если тот настроен;
//   - Reader-fallback на master, если slave=nil;
//   - Writer всегда идет на master.
//
...
// указывающий на тот же мастер-контейнер: replication еще не настроена в test
// env, поэтому "реплика" — это второй pool на ту же физическую БД).
// Проверяет, что Reader-TX **берет connection из slave-pool'а**, а не из
...
func TestCQRS_SlavePool_RouterUsesSlavePool(t *testing.T) {
	...
	// После Close — conn возвращен в slave-pool.
...
// идет на master (writes на реплику ошибочны: streaming replica read-only).

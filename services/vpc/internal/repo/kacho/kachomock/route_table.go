...
// In-memory RouteTable reader/writer для kachomock — parity с Network/SG.
// Auto-association в мок не моделируется (это DB-side PL/pgSQL trigger; для
// unit-тестов use-case'ов он не нужен — integration-тест на pg покрывает trigger).
...
func (rw *routeTableWriter) GetForUpdate(ctx context.Context, id string) (*kacho.RouteTableRecord, error) {

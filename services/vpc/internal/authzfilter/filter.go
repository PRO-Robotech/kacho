...
//
// # Почему 5, и почему это не косметика (регрессия RT-LST-PAGESIZE-EXACTLY-1000)
//
...
// одиночное чтение (`<Resource>Service/Get` → `v_get`, permission_map.go и запись
...
// объект (`NetworkService/ListSubnets` → `v_list` на `vpc_network` — там вопрос
...
// DefaultConfig — sane defaults: фильтр включен, 1s per-call timeout, fan-out 5,

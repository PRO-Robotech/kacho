...
func TestFGAFilter_PageMembershipRequiresReadRelation(t *testing.T) {
	const (
		outsiderObj = "sub_tier_only" // держит ярус, не держит глагол чтения
		legitObj    = "sub_readable"  // держит ровно то, чем гейтится Get
	)
...
func TestFGAFilter_AsksExactlyTheReadRelation(t *testing.T) {
	cli := newFakeAuthorizeClient().allow(readRelation, "sub_a")
	f := NewFGAFilter(cli, DefaultConfig())

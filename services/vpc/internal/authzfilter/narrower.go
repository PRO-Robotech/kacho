...
// Package authzfilter — словарь kacho-vpc для общего сужателя списков
// (`pkg/listnarrow`): типы объектов модели прав, аудит-строки действий и предикат
// членства страницы.
//
// Механики сужения здесь БОЛЬШЕ НЕТ. Она жила в четырёх почти дословных копиях —
// 3753 строки в семнадцати файлах у vpc, compute, nlb и storage, с побайтово
// одинаковыми бюджетами и различием только в словаре ресурсов, — и разошлась ровно
// там, где расхождение не видно: в полярности ответа безымянному вызывающему и в
// том, что означает «модель не провязана». Копии сведены в один дом; здесь остаётся
// то, что у сервиса действительно СВОЁ.
package authzfilter

...
--
-- WHY: kacho_vpc.fga_register_outbox carries BOTH fga.register AND fga.unregister of
...
-- a fresh successor (attempt_count = 0) even at ApplyConcurrency = 1 (vpc's default)
...
--
--   AND NOT EXISTS (SELECT 1 FROM kacho_vpc.fga_register_outbox p
...
--
-- WHY resource_id is the right partition key: the writer emits ONE row per FGA object
-- (Network.Create emits the network tuple AND the inline default-SG tuple as two rows,
-- one per object) and fills resource_id from that row's tuple `Object` ("<kind>:<id>",
-- migration 0008); the id half is globally unique by construction (core rule #15), so
-- "same partition" is exactly "same iam-mirror object". Rows predating 0008 carry the
-- backfill default '' and would share one partition — harmless (over-coarse
...
CREATE INDEX IF NOT EXISTS fga_register_outbox_partition_head_idx
    ON kacho_vpc.fga_register_outbox (resource_id, id)
...
DROP INDEX IF EXISTS kacho_vpc.fga_register_outbox_partition_head_idx;

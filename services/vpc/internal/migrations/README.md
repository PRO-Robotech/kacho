...
> заголовок «`0002..0009`» не покрывал. Сегодня файлов в каталоге **54**, и форм
> номера три — четырёхзначная (45 файлов, `0001`…`0045`), шестизначная
> (5 файлов, `353002`…`912001`) и метка времени (4 файла). Перечислять их здесь
> значило бы заводить второе место об одном предмете, которое растёт с каждым
> изменением схемы и устаревает молча.
>

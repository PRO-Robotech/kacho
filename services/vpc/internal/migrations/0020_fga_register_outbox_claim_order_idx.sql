...
-- Completes migration 0018, which supplied only the index for the claim's correlated
-- NOT EXISTS.
--
-- WHY: the partition-head-only claim (corelib drainer, Config.PartitionColumn =
-- resource_id) reads
--
--   SELECT t.id FROM kacho_vpc.fga_register_outbox t
...
--    LIMIT $n
--
-- migration 0018 indexed the inner predecessor lookup, but nothing could produce the OUTER
...
CREATE INDEX IF NOT EXISTS fga_register_outbox_claim_order_idx
    ON kacho_vpc.fga_register_outbox (attempt_count, id)
...
DROP INDEX IF EXISTS kacho_vpc.fga_register_outbox_claim_order_idx;

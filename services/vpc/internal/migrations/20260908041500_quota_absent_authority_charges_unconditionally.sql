...
-- `353002_nested_quota_carrier.sql`.
...
BEGIN
    ...
    IF TG_OP = 'DELETE' THEN
        ...
        -- отвергает никогда, поэтому режим на него не влияет, а лишнее чтение на
        -- каждом удалении оплачивалось бы без предмета.
    ...
    SELECT authority_state = 'not-deployed' INTO v_absent
      FROM kacho_vpc.quota_sync_cursor
     WHERE id = 'limits';
...
COMMENT ON FUNCTION kacho_vpc.kacho_quota_count() IS
    ...
    'parent column and the parent has an accounting row — the parent. When the '
    'limit authority is DECLARED absent no ceiling applies and nothing is refused, '
    'yet the charge still happens unconditionally so that usage does not silently '
    'fall behind the rows it counts; "unknown" is not "absent". A parent without '
    'an accounting row is one whose project has no resolved nested value, so the '
    'nested axis is SKIPPED rather than refused. Refusals come from '
    'kacho_quota_refuse, shared with the advisory band';
-- +goose StatementEnd
...
-- сказано здесь, потому что иначе следующий читатель примет обратимость за
-- безопасность.
CREATE OR REPLACE FUNCTION kacho_vpc.kacho_quota_count()
RETURNS trigger
LANGUAGE plpgsql

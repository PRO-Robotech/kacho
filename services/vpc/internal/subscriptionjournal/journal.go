...
// (`InternalAddressPoolService`), проектного измерения у него нет по определению
// (`AddressPoolRecord` не несёт поля проекта вовсе), и типа объекта в модели прав
// у него тоже нет.
//
...
func Journal() subscription.Journal {
	return subscription.Journal{
		...
		Storage: subscription.Storage{
			...
			// (предикат — `git grep -in 'DELETE FROM.*vpc_outbox'` по
			// не-тестовому дереву даёт пусто). Значит отказ «позиция утрачена»
			// не наступает никогда. Заведётся чистка — эта величина обязана
			// поменяться ВМЕСТЕ с ней: обещание удержания живёт ровно столько,
			// сколько живёт его основание.

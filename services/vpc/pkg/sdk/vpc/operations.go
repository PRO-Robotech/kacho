...
// CancelOperation — sync cancel (если поддерживается доменом; VPC принимает
// запрос, но реальную отмену выполняет не для всех операций).
...
// (доли секунды для CRUD; sub-second polling дает быструю реакцию без
...
// отвечает grpc-config; в SDK retry-config на усмотрение интегратора.

...
// Package vpc — публичный SDK для Kachō VPC API.
...
// dial). Все публичные service-клиенты VPC и OperationService разделяют этот
// conn: gRPC рассчитан на единое долгоживущее соединение, переиспользуемое
// HTTP/2-стримами, по аналогии с единым connection pool.
//
...
// они доступны только на cluster-internal порту 9091 и не предназначены для
// внешних интеграторов.
type Client struct {
	conn *grpc.ClientConn

	// Public VPC services (6 ресурсов + NetworkInterface).
...
// NewClient создает VPC SDK client поверх gRPC-соединения по адресу addr.
//
...
// sdkDialOpts — собирает итоговый набор grpc.DialOption для NewClient:
...
// пригодиться, если интегратор хочет зарегистрировать еще один stub-клиент

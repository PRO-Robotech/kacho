...
|---|---|---|
| `SUBNET-LF-D-VISIBLE` / `SUBNET-LF-D-NOLEAK` / `SUBNET-LF-D-NONE` (list-filter-d) | subset-viewer / no-grant `List /vpc/v1/subnets` → **403**, устойчиво (не EC-окно: повтор через сутки даёт то же). Прежний диагноз в этой строке — «per-object грант не резолвится на request-path» — **неверен**: per-object грант резолвится, что и показывает `SUBNET-LF-D-GET-OK` (Get видимой подсети → 200 живьём). Отказ наступает РАНЬШЕ, на method-gate уровня проекта. Координата: `services/vpc/internal/apps/kacho/check/permission_map.go:209-214` требует `viewer` на project, каталог шлюза требует `v_list`, в модели (`proto/kacho/cloud/iam/v1/fga_model.fga`, `type project`) это **независимые** отношения — эффективное требование есть их пересечение, а фикстура (`tests/authz-fixtures/setup.sh:1245-1246`) выдаёт только `v_list`. Расхождение **сознательное и уже задокументировано** — `services/vpc/internal/apps/kacho/check/catalog_parity_test.go:11-28` (сужать нельзя «заодно»: снятие конъюнкта `viewer` РАСШИРИТ доступ). | **НЕ whitelist** (маска снята 7c44f8d9 и не возвращается). До 2026-07-29 три кейса были **зелёными и вакуумными**: утверждение читалось «список доступен (200) ИЛИ закрыт методом (403)», то есть принимало оба взаимоисключающих исхода и упасть не могло, а 403 — ровно то состояние, которое строка описывает как дефект. Проба сделана различающей (безусловный 200 + безусловный инвариант) и теперь **красная на живом продукте** — это её работа, а не регрессия. Канарейки `SUBNET-LF-D-GET-404` / `-GET-OK` различали всегда и проходят честно. `kacho-iam#276` |

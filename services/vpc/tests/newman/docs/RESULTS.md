...
| `ADR-GBV-CONF-EXT-BY-VALUE-UNREACHABLE` | address | [#104](https://github.com/PRO-Robotech/kacho/issues/104) | Поиск адреса по значению работает для внешнего адреса — той ветки, которую контракт запроса предлагает | Авторизация метода привязана к `subnet_id` (без него отказ fail-closed — единственный якорь запроса), а хранилище сужает выборку по `internal_ipv4->>'subnet_id'` — колонке, которой у внешнего адреса нет (в схеме это РАЗНЫЕ nullable jsonb, комбинация отвергается на Create). Значит на любом авторизованном вызове внешняя ветка не совпадает ни с одной строкой: недостижима **по построению**, а не по данным, и вызывающему отвечают «не найдено» про существующий адрес. RED до продуктового решения (реализовать область проекта / отвергать явно / снять поле с контракта). |

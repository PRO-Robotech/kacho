...
### Состояние применения намерения (kacho#296)

*Что видит арендатор о том, доехало ли его изменение до сети. Ровно два факта:
применено ли намерение текущей ревизии и, если нет, класс причины из закрытого
словаря. Ревизии, времени отчёта и координат фабрики на этой поверхности нет и
появиться неоткуда — состав проекции заперт пробой по дескриптору.*

| Pattern | Classes | P | Apps | Что проверяет |
|---|---|---|---|---|
| `NET-APPLY-STATE-CRUD-OK` | CRUD,CONF | P1 | 1 (net) | Свежая сеть: `Get` несёт `applyState` (не применено, класса нет), ответ операции его НЕ несёт — разница между поверхностями намеренная (APPLY-05, APPLY-21) |
| `NET-APPLY-STATE-LIST-PARITY-OK` | CRUD,CONF | P1 | 1 (net) | Три сети: каждый элемент страницы несёт `applyState`, и он совпадает с одиночным чтением. Закрывает класс «одно чтение поле заполняет, другое нет» (APPLY-12) |

Присутствие поля на чтении остальных шести ресурсов, несущих строку намерения,
утверждается ВНУТРИ их кейсов `*-CR-CRUD-*` (`assert_apply_state_in_flight`), а
не отдельными кейсами: фикстура там уже построена, и заводить вторую значило бы
платить за неё дважды (APPLY-11, сквозная половина).

**Открытый долг, названный числом:** списочная чётность (`APPLY-12`) landed
только у сети — **1 ресурс из 7**. У остальных шести свойство держит гейт по
дереву `TestApplyStateFillerIsWiredAtEveryFillingSurface`
(`services/vpc/internal/dto/toproto/apply_state_wiring_test.go`), который
требует заполнителя у ОБОИХ чтений каждого из семи ресурсов и падает, называя
пропущенную поверхность. Предикат снятия долга: по каждому из шести заведён кейс
вида `*-APPLY-STATE-LIST-PARITY-OK`.

...
>
> Снят шов исполнителя датаплейна и вместе с ним публичное поле состояния
> применения, стоявшее на семи ресурсах: производителя у поля не было ни одного,
> и арендатору оно отвечало «не применено» всегда. Удалены оба кейса раздела
> «Состояние применения намерения» (`NET-APPLY-STATE-*`), утверждение о поле
> внутри `*-CR-CRUD-*` шести остальных ресурсов и оба помощника генератора.
> Величина вернётся вместе с исполнителем, который её производит, — до тех пор
> утверждать о ней нечего.
...
### Публичная административная поверхность пула (ADM-1 S1)

Пул адресов выставлен на ПУБЛИЧНОМ крае под правом `system_admin` @ `cluster`:
закрыт не листенер, а вызывающий без права. Мутации отвечают `Operation`
(запрет 9), чтения синхронны. Внутренний глагол живёт рядом на время окна
расширения — кейсы `IPL-*` (`cases/internal-pool.py`) остаются и НЕ переписаны.

Кейсы ниже снимаются ВМЕСТЕ со стадией S3, которая снимает внутренний глагол:
проба, чей предмет снят, — находка, а не лишняя строка.

| Pattern | Classes | P | Apps | Что проверяет |
|---|---|---|---|---|
| `ADM-1-01-PUB-POOL-CREATE-OK` | CRUD,CONF | P0 | 1 (add,public-pool) | Администратор создаёт пул с ВНЕШНЕГО края: ответ — `Operation`, опрос доходит до `done` без `error`, последующее чтение отдаёт заполненный ресурс. Закрывает наблюдение владельца (раздел администрирования отвечал 404). |
| `ADM-1-03-PUB-POOL-TENANT-DENIED` | NEG,CONF | P0 | 1 (add,public-pool) | Арендатор без кластерных прав получает `403`/`PERMISSION_DENIED` на список и на чтение по id — не пустую страницу. Положительный контроль в ТОМ ЖЕ кейсе: администратор на тех же путях получает `200` и видит свой пул. |
| `ADM-1-11-PUB-POOL-LANES` | NEG,VAL,CONF | P1 | 1 (add,public-pool) | Полосы отказа по контракту: корректный, но отсутствующий id (обе формы — слитная и дефисная) → `404`/`NOT_FOUND` + `RESOURCE_NOT_FOUND`; негодный по форме → `400`/`INVALID_ARGUMENT` + `INVALID_RESOURCE_ID`. Утверждается ПАРА (статус и код) плюс признак полосы. |
| `ADM-1-19-PUB-POOL-EQUIVALENCE` | CONF | P0 | 1 (add,public-pool) | Окно расширения: созданное публичным глаголом видно внутренним чтением и наоборот, по ОБЪЯВЛЕННЫМ полям. Единственная проверка двух путей записи в одну таблицу — расхождение между ними не видно ни одному сценарию, смотрящему на один путь. |
| `ADM-1-01-PUB-POOL-CLEANUP` | CRUD | P1 | 1 (add,public-pool) | Удаление публичным глаголом отвечает `Operation`, опрос доходит до `done` без `error`, последующее чтение — `404`. |
...
| `RT-CR-CONF-QUOTA-NESTED-EXCEEDED` | REQ-RT-QUOTA-NESTED | CONF,NEG,BVA | P0 | Потолок числа ресурсов, спрошенный у КРАЯ (задача #663; прежде отказ по квоте не утверждал ни один кейс ни одного набора). Своя сеть → восемь таблиц маршрутизации проходят (положительный контроль), девятая отвергается вложенным потолком `vpc.network.routeTable`=8. Отказ приезжает ОШИБКОЙ ОПЕРАЦИИ (вложенную ось совещательная полоса не спрашивает, списывает триггер внутри writer-TX воркера): `operation.error.code` = 8 `RESOURCE_EXHAUSTED`, `ErrorInfo.reason` = `QUOTA_EXCEEDED`, `domain` = `vpc.kacho.cloud`, и ДОСЛОВНЫЙ текст единственного производителя (`pkg/quota/refusal.sql.tmpl` :: `kacho_quota_refuse`) — «vpc.network.routeTable ‹id сети› has reached its limit of 8 vpc.network.routeTable». HTTP 429 эта полоса не производит — 429 остаётся формой ПРОЕКТНОГО потолка, отвергающего синхронно; допуск `oneOf([200, 429])` был бы исходом, которого не бывает. Второй положительный контроль: сняв одну таблицу, ту же девятую заводим снова — она проходит (место возвращается, потолок не защёлка). Предусловие «проектного запаса ≥ 9» читается через `GET /vpc/v1/quotas` и отделяет «стенд не готов» от «продукт сломан». Самодостаточен: предел считается в собственной сети, ни с одним кейсом не делится; уборка — девять объектов. |

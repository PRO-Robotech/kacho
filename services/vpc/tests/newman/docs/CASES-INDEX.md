...
| `NET-PRJ-DL-NEG-NETWORK-HOLDS` | NEG,STATE,CONF | P1 | 1 (net/prj) | **Связка с iam через край платформы** (`cases/project-container.py`): живая сеть vpc удерживает свой проект — `DELETE /iam/v1/projects/{id}` → операция с `FAILED_PRECONDITION` и текстом дословно `Project <id> is not empty (vpc.network: 1, vpc.routeTable: 1, vpc.securityGroup: 1)` (одна сеть числится тремя зарегистрированными объектами — свойство композиции vpc, держится здесь), признак `REFERENCE_IN_USE`; проект и сеть остаются на месте. Парный положительный контроль в том же кейсе: после снятия сети тот же запрос проходит (ограниченный повтор на окне доставки снятия регистрации — граница, названная контрактом). Дом кейса — набор платформы, а не службы (`e2e-flow.md` §7а): чужой ресурс набор службы завести не может. Пункт 1 предиката снятия `PRO-Robotech/kacho#1231`; документ решения — `docs/architecture/project-deletion-and-live-resources.md`. |

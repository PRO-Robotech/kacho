...
# Подгрузка могла НЕ СОСТОЯТЬСЯ (файл переименован/перемещён), и без этой проверки
# самопроверка шла дальше по неопределённой функции. Наблюдалось вживую при разборе
# 2026-07-30: прежняя редакция напечатала ошибку подгрузки, затем «SKIP: нет jq» и
# вышла НУЛЁМ — то есть отчиталась успехом, не имея даже предмета проверки
# (`set -e` здесь намеренно нет, поэтому отказ подгрузки сам ничего не роняет).
if ! declare -F aggregate_verdict >/dev/null 2>&1; then
  echo "SELFTEST FAIL: не удалось подгрузить aggregate_verdict из $HERE/run.sh —" >&2
  echo "  проверять нечего, и это отказ, а не пропуск." >&2
  exit 2
fi

# Нет jq — самопроверка НЕ ВЫПОЛНИЛАСЬ, и это не «прошла».
#
# Здесь стоял `exit 0` со словом SKIP, то есть ровно тот класс, который эта
# самопроверка и ловит: «не выполнилось» засчитывалось «прошло» — в гейте, чей
# единственный предмет состоит в том, что так делать нельзя. Достаточно было
# окружения без jq, чтобы шаг CI стал зелёным, не проверив ни одного свойства
# вердикта.
#
# Отсутствие инструмента — это отказ ОСНАСТКИ, а не свойство продукта, поэтому код
# возврата отдельный (2), а не 1: читающему видно, что упало не утверждение, а
# возможность его проверить.
if ! command -v jq >/dev/null 2>&1; then
  echo "SELFTEST FAIL: нет jq — самопроверка вердикта НЕ ВЫПОЛНИЛАСЬ." >&2
  echo "  «не выполнилось» не засчитывается «прошло»: это тот самый класс, ради" >&2
  echo "  которого существует эта проверка. Установите jq." >&2
  exit 2
fi

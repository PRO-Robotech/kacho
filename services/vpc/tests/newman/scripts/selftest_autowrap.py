...
# Утверждается СОСТАВ набора, а не написание строки: у ожидания есть и другие
# слагаемые (страж неразрешённого адреса), и они законно стоят перед проверкой
# кода. Мерить по началу строки значило бы требовать формы там, где предмет —
# числа: набор HTTP-исходов обязан нести 403 и не обязан знать gRPC-кодов 5 и 9,
# которые шаг проверяет СВОИМ утверждением.
_gset = gguard.split(".includes(pm.response.code)")[0]
check("близнец-grpc: числа gRPC-кодов не попали в набор HTTP-исходов",
      "[403]" in _gset and "5" not in _gset and "9" not in _gset, gguard[:120])

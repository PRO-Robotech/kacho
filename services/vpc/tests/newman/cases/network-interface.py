    ...
    return [
        ...
        Step(name="pre-subnet", method="POST", path="/vpc/v1/subnets",
             body={"projectId": "{{_suiteProjectId}}", "networkId": "{{netId}}",
                   "name": f"nic-{suffix}-sub-{{{{runId}}}}", "placementType": "ZONAL", "zoneId": "{{existingZoneId}}",
                   "v4CidrBlocks": [cidr]},
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
...
CASES.append(Case(
    ...
    steps=[
        ...
        Step(name="pre-subnet", method="POST", path="/vpc/v1/subnets",
             body={"projectId": "{{_suiteProjectId}}", "networkId": "{{netId}}",
                   "name": "nic-v6addr-sub-{{runId}}", "placementType": "ZONAL", "zoneId": "{{existingZoneId}}",
...
CASES.append(Case(
    ...
    steps=[
        ...
        Step(name="pre-subnet", method="POST", path="/vpc/v1/subnets",
             body={"projectId": "{{_suiteProjectId}}", "networkId": "{{netId}}",
                   "name": "nic-both-sub-{{runId}}", "placementType": "ZONAL", "zoneId": "{{existingZoneId}}",

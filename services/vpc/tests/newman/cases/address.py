...
# ADR-GBV-CONF-EXT-BY-VALUE-UNREACHABLE — ЗАЯВЛЕННЫЙ КРАСНЫЙ (дефект продукта).
#
# Контракт запроса предлагает искать по ВНЕШНЕМУ адресу (`external_ipv4_address` —
# ветка oneof `address`), а областью запрос располагает ровно одной — `subnet_id`.
# Авторизация метода читает `subnet_id` и без него отказывает; хранилище, получив
# `subnet_id`, сужает выборку по `internal_ipv4->>'subnet_id'` — колонке, которой у
# внешнего адреса нет (в схеме `external_ipv4` и `internal_ipv4` — РАЗНЫЕ nullable
# jsonb, адрес бывает либо тем, либо другим). Значит на любом авторизованном вызове
# внешняя ветка не может совпасть НИ С ОДНОЙ строкой: недостижима по построению, а
# не по данным. Вызывающему при этом отвечают «не найдено» про адрес, который есть, —
# то есть ложное утверждение об отсутствии в ответ на запрос, который контракт
# рекламирует.
#
# Кейс НЕ подгоняется под текущее поведение: подгонка превратила бы дефект в
# задокументированный контракт. Он утверждает то, что контракт обещает, и остаётся
# КРАСНЫМ до продуктового решения (реализовать область проекта / отвергать явно /
# снять поле с контракта — выбор продуктовый, см. docs/RESULTS.md).
#
# verifies https://github.com/PRO-Robotech/kacho/issues/104
CASES.append(Case(
    id="ADR-GBV-CONF-EXT-BY-VALUE-UNREACHABLE",
    title="GetByValue внешнего IP с единственной доступной областью → 200 + сам Address (ЗАЯВЛЕННЫЙ КРАСНЫЙ: ветка недостижима by construction)",
    classes=["CONF", "NEG"], priority="P1",
    steps=[
        *_make_net_sub("gbvx", "10.124.0.0/24"),
        ...
        Step(name="gbv-ext-scoped", method="GET",
             ...
             test_script=[
                 *assert_status(200),
                 "pm.test('внешний адрес найден по значению', () => "
                 "pm.expect(pm.response.json().id).to.eql(pm.environment.get('addrId')));",
             ]),
        retry_until_authorized(Step(name="cleanup-addr", method="DELETE", path="/vpc/v1/addresses/{{addrId}}",
             test_script=[*save_from_response("j.id", "opId")]), retry_on=(403,)),

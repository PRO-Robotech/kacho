    ...
    steps=[
        ...
        Step(name="create", method="POST", path=POOLS,
             ...
             test_script=[*assert_status(200),
                          ...
                          *save_from_response("j.id", "adm1PoolOpId")]),
        poll_operation_until_done(op_var="adm1PoolOpId", capture_id_to="adm1PoolId",
                                  id_expr="j.metadata && (j.metadata.addressPoolId || j.metadata.address_pool_id)"),
    ...
    steps=[
        ...
        Step(name="create-public", method="POST", path=POOLS,
             ...
             test_script=[*assert_status(200), *assert_operation_envelope(),
                          *save_from_response("j.id", "adm1EqOpId")]),
        poll_operation_until_done(op_var="adm1EqOpId", capture_id_to="adm1EqPubId",
                                  id_expr="j.metadata && (j.metadata.addressPoolId || j.metadata.address_pool_id)"),
...
CASES.append(Case(
    ...
    steps=[
        Step(name="delete", method="DELETE", path=POOLS + "/{{adm1PoolId}}",
             auth="jwtBootstrap",
             test_script=[*assert_status(200), *assert_operation_envelope(),
                          *save_from_response("j.id", "adm1DelOpId")]),
        poll_operation_until_done(op_var="adm1DelOpId"),

...
> [!warning] Настройки `KACHO_VPC_DEFAULT_SG_INLINE` БОЛЬШЕ НЕ СУЩЕСТВУЕТ
> Замеры ниже сняты, когда она была, и как ИСТОРИЯ ПРОГОНА они верны — числа не
> переписываются. Но всё, что ниже читается как РЕКОМЕНДАЦИЯ («выключить и получить
> +30-40%»), исполнить нельзя: группа правил по умолчанию создаётся безусловно.
> Причина — не производительность: интерфейс наследует группу своей сети, поэтому
> сеть без неё означала бы интерфейс без единого правила, а по закрытой в обе стороны
> модели это «не разрешено ничего». Выигрыш в пропускной способности записи достигается
> другими средствами, и они здесь не измерены.

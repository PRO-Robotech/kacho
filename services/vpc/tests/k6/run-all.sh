...
# Прогон ПЯТИ k6-сценариев последовательно, от лёгкого к стрессовому (см. вызовы
# `run` внизу файла). Не всех: в `scripts/` их восемь, плюс один в корне tests/k6.
# Прежняя редакция этой строки говорила «всех» — и расходилась с собственным телом.
set -euo pipefail
cd "$(dirname "$0")"

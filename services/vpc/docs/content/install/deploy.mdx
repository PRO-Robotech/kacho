...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>kacho-migrator down [--target &lt;version&gt;]</code></td><td>Откатить последнюю миграцию (или вниз до версии). Возвращает <strong>форму</strong>, не данные — см. «Откат выкатки»</td></tr>
    <tr><td><code>kacho-migrator status</code></td><td>Показать примененные / ожидающие миграции</td></tr>
  </tbody>
</table>

Команды генерации файла миграции у CLI **нет** — имя пишется рукой. Действующая
форма номера — метка времени заведения (`date -u +%Y%m%d%H%M%S`), например
`internal/migrations/20260823001500_add_route_table.sql`. Форма объявлена в одном
месте и здесь не переписывается — разбор решения в
`docs/architecture/migration-version-namespace.md` в корне монорепо.
...
Сборка — multi-stage (`services/vpc/Dockerfile`): build-stage компилирует **оба** binary
одного модуля (`kacho-vpc` и `kacho-migrator`), runtime-stage — минимальный `alpine` с
non-root пользователем и клеймом происхождения образа.

Контекст сборки — **корень репозитория**. Дерево одно, зависимости резолвятся внутри него
(`go.mod` без `replace`), поэтому Dockerfile копирует контекст целиком (`COPY . .`).
Сборка кросс-платформенная (`TARGETOS`/`TARGETARCH`) и переиспользует общий кэш компиляции
модуля — он принадлежит builder'у, а не образу, поэтому второй и далее образы того же
дерева берут готовое.
...
:::note Рецепт живёт в одном месте
Стадии, базовые образы и строки сборки здесь намеренно не воспроизводятся: копия разошлась
бы с `services/vpc/Dockerfile` молча — у прозы нет ни конфликта при слиянии, ни красного в
сборке. Что команда выше называет **существующий** файл, держит гейт
`TestDocsBuildCommandsNameADockerfileTheTreeHas` в `internal/repohygiene`.
:::
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>repository.postgres.maxConns</code></td><td><code>40</code></td><td>Ширина пула pgx на одну реплику</td></tr>
    <tr><td><code>autoscaling.maxReplicas</code></td><td><code>5</code></td><td>Верхняя граница HPA (CPU 70%)</td></tr>
  </tbody>
</table>
...
Стенд поднимается одной командой из каталога `deploy` того же репозитория —
kind-кластер + PostgreSQL + ingress + все сервисы umbrella-чарта. Соседнего
репозитория стенда рядом с деревом нет: команды ниже запускаются **от корня
репозитория**, каталог цели называется флагом `-C`.
...
<CodeBlock language="bash">
  {dedent`
    ...
    make -C deploy reload-svc SVC=vpc

    # логи сервиса
    make -C deploy logs-svc SVC=vpc
...
`reload-svc SVC=vpc` собирает `kacho-vpc:dev` из build-context корня репозитория,
...
kubectl -n <namespace> exec deploy/<деплой kacho-vpc> -- \
...
| под работает нужным образом | `kubectl -n <namespace> get pod -l app=<деплой kacho-vpc> -o jsonpath='{..image}'` |

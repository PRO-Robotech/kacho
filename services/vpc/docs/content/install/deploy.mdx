...
Команды генерации файла миграции у CLI **нет**. Имя новой миграции выводится из
номера задачи и пишется рукой — `internal/migrations/539001_add_route_table.sql`,
где `539` — номер задачи (тот же, что в имени ветки), а `001` — порядковый номер
миграции внутри этой задачи. Разбор решения —

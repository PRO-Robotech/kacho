...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>List-polling / Operations; журнала подписки у vpc пока нет</td>
      <td>поллинг работает всегда и не требует долгоживущего соединения; подписка платформы существует, но vpc в число её владельцев пока не входит (§7)</td>
    </tr>
  </tbody>
</table>
...
**Ресурсы vpc наблюдаются опросом.** Подписка на изменения у платформы есть — одна ручка края
(`GET /subscription/v1/events`, Server-Sent Events) поверх журналов владельцев, — но владельцем
журнала vpc сегодня **не объявлен**: его `vpc_outbox` остаётся внутренним. Появится владелец —
изменится этот раздел, а не контракт ресурсов: опрос останется законным путём и после.
...
проекция наружу: чтобы журнал стал потоком для арендатора, vpc должен быть объявлен владельцем
платформенной подписки, а он пока не объявлен. До тех пор tenant-клиент наблюдает изменения через
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><strong>Опрос вместо потока — у ресурсов vpc</strong></td>
      <td>наблюдение через <code>Operation</code> и polling <code>List</code> (см. §7). Подписка на изменения у платформы есть, но владельцем её журнала vpc пока не объявлен; исход операции поток не заменяет ни у одного домена</td>
    </tr>
  </tbody>
</table>

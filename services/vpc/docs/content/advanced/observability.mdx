...
durable в таблице, но владение не регистрируется в kacho-iam → per-resource `Check` уходит
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Регистрация владения не доходит до kacho-iam (per-resource <code>Check</code> = deny)</td><td>register-outbox backlog + <code>outbox\_poisoned\_total</code></td><td>drainer не подключен / kacho-iam недоступен</td></tr>

...
операций, лаг outbox). Cross-cutting-инфраструктура вынесена в `corelib/observability`
и переиспользуется всеми сервисами платформы, поэтому формат логов, имена сигналов и
конвенции одинаковы в kacho-vpc, kaname, kacho-geo, kacho-compute и api-gateway.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>WARN</code></td><td>Fail-open authz при недоступном kaname (если включен), деградации, ретраи</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>Register-outbox</strong></td><td>Незадоставленные register-intents, число «отравленных» записей (<code>outbox\_poisoned\_total</code>), лаг доставки owner-tuple в kaname</td></tr>
...
sequenceDiagram
  ...
  participant IAM as kaname (authz)
...
internal-listener kaname (для authz-gate). HTTP-проба `/healthz` (секция `healthcheck.enable`) —
...
### Register-outbox (доставка owner-tuple в kaname)
...
Так гарантируется co-commit: либо ресурс создан И регистрация его владения поставлена в
очередь, либо ни то ни другое. Фоновый **drainer** (corelib
`outbox/drainer`) дренирует очередь через `kaname.InternalIAMService.RegisterResource`,
...
:::caution Drainer должен быть подключен к kaname
Если `authz.iam-endpoint` не задан, register-drainer **не стартует**: intents остаются
durable в таблице, но владение не регистрируется в kaname → per-resource `Check` уходит
в deny (нет пути к ресурсу). Readiness сервиса в этом режиме сигнализирует `NotReady` до подключения
drainer'а к kaname. Метрика `outbox_poisoned_total` отражает записи, исчерпавшие ретраи.
:::
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Register-outbox backlog (<code>sent\_at IS NULL</code>)</td><td>малый, быстро дренируется drainer'ом</td><td>монотонно растет (drainer не подключен / kaname недоступен)</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>UNAVAILABLE</code> на Create/Update</td><td>логи + Health peer-сервиса</td><td>недоступен kaname / kacho-geo (fail-closed валидация project / zone)</td></tr>
    ...
    <tr><td>Регистрация владения не доходит до kaname (per-resource <code>Check</code> = deny)</td><td>register-outbox backlog + <code>outbox\_poisoned\_total</code></td><td>drainer не подключен / kaname недоступен</td></tr>

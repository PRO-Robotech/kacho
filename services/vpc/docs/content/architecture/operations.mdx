...
:::info Наблюдение состояния vpc — через polling
...
одного домена. Текущее состояние ресурсов vpc — периодическим `List`: журнал `vpc_outbox`
остаётся in-cluster и наружу сегодня не проецируется, потому что владельцем платформенной
подписки vpc пока не объявлен. Подробнее — [Архитектура](/architecture/overview).
:::

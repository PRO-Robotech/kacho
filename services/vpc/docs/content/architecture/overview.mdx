...
flowchart TD
  ...
  domain["domain/<br/><small>entities: чистый Go-тип,<br/>только stdlib + стабы pkg/api/...</small>"]
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>только stdlib + стабы <code>pkg/api/...</code></td>
    ...
    <tr>
      ...
      <td>Adapter: реализует peer-порты (<code>ProjectClient</code> → kaname, <code>ZoneRegistry</code> → kacho-geo) поверх gRPC-стабов. Здесь же FGA-register-applier — drainer, применяющий owner-tuple через kaname.</td>
    </tr>
    <tr>
...
выставляет `search_path TO kacho_vpc, public`. Версионирование — Goose; `0001` — squashed
baseline, всё остальное лежит поверх неё отдельными файлами. Применённой считается всякая
миграция, доехавшая до ствола, и она не редактируется; перечня применённых здесь нет —
он растёт с каждым изменением схемы. Подробности — на странице
...
flowchart LR
  ...
  subgraph peers["Доменные сервисы"]
    iam["kaname<br/>Project · authz · FGA-proxy"]
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>исходящее</td>
      <td><code>ProjectService.Get</code> — существование проекта (owner-lookup на Create). <code>InternalIAMService.Check</code> — per-RPC authz-gate. kaname — leaf-owner, обратно не зовет.</td>
      <td>peer-call (sync)</td>
    </tr>
    <tr>
      <td><code>vpc → iam</code> (регистрация ресурсов)</td>
      <td>исходящее</td>
      <td><code>InternalIAMService.RegisterResource</code> / <code>UnregisterResource</code> — регистрация/снятие владения и места ресурса в цепи областей <strong>через kaname</strong> (модули в модель прав напрямую не пишут). Намерение пишется в transactional-outbox в той же TX, что и ресурс; отдельный drainer применяет его idempotent / at-least-once.</td>
...
(leaf platform-topology домен); **Account / Project** → kaname (leaf-owner); **Network /
Subnet / SG / RouteTable / Address / Gateway / NetworkInterface / CidrGroup** → kacho-vpc;
**Instance / MachineType** → kacho-compute; **Volume / Snapshot / Image / DiskType** →
kacho-storage. Consumer ссылается по id и валидирует через API владельца — mirror-таблицы и
cross-DB FK запрещены.

Здесь блочное хранение стояло у вычислений (`Disk / Image / Snapshot`). Раскол домена завершён,
контрактов этих ресурсов у вычислений нет вовсе — а имя владельца сегодня и есть значение
параметра, которым адресуются поток изменений, REST-префикс и префикс идентификатора операции.
:::
...
(`23503`→`FailedPrecondition`, `23505`→`AlreadyExists`/`FailedPrecondition`, `23514`→`InvalidArgument` (кроме формы имени: она backstop, значит `Internal`),

...
flowchart TD
  ...
  domain["domain/<br/><small>entities: чистый Go-тип,<br/>только stdlib + сгенерированные стабы</small>"]
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>только stdlib + сгенерированные стабы (<code>pkg/api/</code>)</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>исходящее</td>
      <td><code>ProjectService.Get</code> — существование проекта (owner-lookup на Create). <code>InternalIAMService.Check</code> — per-RPC authz-gate. kacho-iam — leaf-owner, обратно не зовет.</td>
      <td>peer-call (sync)</td>
    </tr>
    <tr>
      <td><code>vpc → iam</code> (регистрация ресурсов)</td>
      <td>исходящее</td>
      <td><code>InternalIAMService.RegisterResource</code> / <code>UnregisterResource</code> — регистрация/снятие владения и места ресурса в цепи областей <strong>через kacho-iam</strong> (модули в модель прав напрямую не пишут). Намерение пишется в transactional-outbox в той же TX, что и ресурс; отдельный drainer применяет его idempotent / at-least-once.</td>

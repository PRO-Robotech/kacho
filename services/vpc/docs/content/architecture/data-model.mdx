...
<table>
  ...
  <tbody>
    <tr><td><code>networks</code></td><td>Network (VPC-контейнер)</td><td><code>id</code></td><td>partial <code>UNIQUE … WHERE name&lt;&gt;''</code></td></tr>
    ...
    <tr><td><code>gateways</code></td><td>Gateway (вид: NAT / EGRESS_ONLY; FK на подсеть-якорь)</td><td><code>id</code></td><td>partial <code>UNIQUE</code></td></tr>
    ...
    <tr><td><code>vpc\_outbox</code></td><td>Доменный транзакционный журнал событий + <code>pg\_notify</code> (in-cluster <code>LISTEN/NOTIFY</code>; наружу журнал vpc сегодня не проецируется)</td><td><code>sequence\_no</code></td><td>—</td></tr>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>UNIQUE (project\_id, name)</code> — partial <code>WHERE name&lt;&gt;''</code> у всех восьми ресурсов: пустое имя законно по контракту и слот не занимает</td>
...
Все 8 публичных ресурсов несут DB-level `CHECK` поверх `domain.Validate` (защита от внешних
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>vpc\_outbox</code></td><td>Транзакционный outbox-журнал: каждая успешная мутация пишет событие в outbox <strong>в той же TX</strong>, что и сам ресурс. Триггер <code>vpc\_outbox\_notify\_trg</code> делает <code>pg\_notify('vpc\_outbox', sequence\_no::text)</code> — in-cluster <code>LISTEN/NOTIFY</code>-канал. Наружу этот журнал сегодня не проецируется: подписка на изменения у платформы есть (одна ручка края поверх журналов владельцев), но vpc в число объявленных владельцев пока не входит, поэтому клиенты наблюдают изменения ресурсов vpc через polling <code>List</code> / <code>OperationService.Get</code>.</td></tr>
  </tbody>
</table>
...
flowchart LR
  ...
  ob -->|"trigger pg_notify"| notify["LISTEN/NOTIFY vpc_outbox<br/>(in-cluster, наружу не проецируется)"]

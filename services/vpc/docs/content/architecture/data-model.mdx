...
инкременты — на день правки этой страницы их **52** (`ls internal/migrations/*.sql | wc -l`
даёт 53 вместе с baseline; последний файл — `912001_address_secondary_cursor_indexes.sql`).
...
- таблицу **каждого** ресурса, которым домен управляет полным набором глаголов
  ...
  контракта <code>kacho.cloud.vpc.v1</code>;
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>vpc\_outbox</code></td><td>Доменный транзакционный журнал событий + <code>pg\_notify</code> (in-cluster <code>LISTEN/NOTIFY</code>; наружу журнал vpc сегодня не проецируется)</td><td><code>sequence\_no</code></td><td>—</td><td>да</td></tr>
    ...
    <tr><td><code>fga\_register\_outbox</code></td><td>Транзакционный outbox для регистрации owner-tuple в FGA (через kacho-iam)</td><td><code>id</code></td><td>—</td><td>нет</td></tr>

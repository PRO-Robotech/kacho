...
1. **Авторизация (authz)** — на каждый RPC проверяется, что вызывающий субъект имеет нужное
   ...
   платформы; вердикт о доступе вычисляет `kacho-iam`.
...
transient-сбое kacho-iam — иначе пользователь создал бы ресурс и не увидел его
...
приходит, когда владение еще не зарегистрировано, `kacho-iam` возвращает «нет пути к объекту».

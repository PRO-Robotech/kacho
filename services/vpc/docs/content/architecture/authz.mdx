...
прямыми usersets (`[user, service_account, group#member] or super_admin`). Наличие `editor` на
...
доступного субъекту: перечисление в хранилище отношений ограничено серверным потолком и не имеет
продолжения, поэтому на долгоживущем сторе объект сверх потолка стал бы невидим владельцу при
живых правах. Ошибка проверки — fail-closed; частично отфильтрованная страница не отдаётся.
...
Модель **плоская**: `viewer`/`editor`/`admin` и глагольные `v_*` на ресурсе — прямые usersets, а
не производные от одноимённых отношений проекта. `viewer` на проекте **не** делает вызывающего

...
- **Подписки на операции у платформы нет**: их исход узнаётся поллом. Поток изменений ресурсов платформа отдаёт (одна ручка края, Server-Sent Events), но журнала подписки у vpc сегодня нет — владельцами объявлены другие домены, и ресурсы vpc наблюдаются опросом.
:::
...
- Подписки на операции у платформы **нет** — их исход узнаётся поллом `Get` (или `List` ресурса
  каждые 2–5 секунд). Поток изменений ресурсов платформа отдаёт, но журнала подписки у vpc
  сегодня нет: ресурсы vpc наблюдаются опросом.
:::

...
`Operation` (`corelib.operation.Operation`) — плоский envelope с полем-результатом
в виде `oneof`:
...
- Альтернатива поллингу одной операции — `List`-poll самого ресурса (2–5 сек) или `ListOperations` по ресурсу (см. ниже).
- **Подписки на операции у платформы нет**: их исход узнаётся поллом — и это свойство предмета, а не незакрытая работа (событие журнала пишется в той же транзакции, что и строка ресурса, поэтому у провалившейся мутации события нет вовсе). **Изменения самих ресурсов vpc при этом доступны потоком**: vpc объявлен владельцем журнала платформенной подписки (`GET /subscription/v1/events`, `owner=vpc`). Поток заменяет опрос списка, но не заменяет исход операции.
:::
...
- Подписки на операции у платформы **нет** — их исход узнаётся поллом `Get` (или `List` ресурса
  каждые 2–5 секунд). Изменения самих ресурсов vpc доступны потоком подписки (`owner=vpc`),
  но исход операции поток не несёт ни у одного домена: у провалившейся мутации события нет.
:::

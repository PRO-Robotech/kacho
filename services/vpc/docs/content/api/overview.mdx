...
`:verb` (двоеточие в конце пути) — **действие** над ресурсом, тело которого описывает
параметры действия. Сегмент после `/` (`.../subnets`, `.../operations`) — **коллекция-ребёнок**
ресурса.

**Имя действия — нижний `camelCase`** (`:addCidrBlocks`, `:removeTargets`, `:setLifecycle`),
однословное действие пишется одним словом (`:stop`, `:move`, `:revoke`). Правило одно на всю
платформу: зная написание у одного ресурса, стройте по нему путь у соседнего.

**Три ресурса VPC пока отступают от него**, и это известное расхождение, а не второе правило:

| ресурс | действующий адрес | канон |
|---|---|---|
| `Network` | `:add-cidr-blocks` / `:remove-cidr-blocks` | `:addCidrBlocks` / `:removeCidrBlocks` |
| `Subnet` | `:add-cidr-blocks` / `:remove-cidr-blocks` | `:addCidrBlocks` / `:removeCidrBlocks` |
| `CidrGroup` | `:add-cidr-blocks` / `:remove-cidr-blocks` | `:addCidrBlocks` / `:removeCidrBlocks` |

`AddressPool` тех же действий уже адресуется каноном (`:addCidrBlocks`). Пока расхождение не
снято, **сверяйтесь со страницей ресурса**, а не переносите написание по памяти: промах даёт
`404` без тела, и отказ верного написания не подсказывает.

Расхождение снимается **совместимо**: канон станет основным адресом, а нынешняя запись
останется приниматься как дополнительная привязка — уже отправленные интеграции не сломаются.
:::
...
:::caution Имя поля с CIDR-блоками различается между ресурсами
Одни и те же по смыслу блоки называются в теле **двумя способами**, и разделение проходит
не там, где разделение адресов выше:

| ресурс | адрес действия | ключ тела |
|---|---|---|
| `Network` | `:add-cidr-blocks` | `ipv4CidrBlocks` / `ipv6CidrBlocks` |
| `Subnet` | `:add-cidr-blocks` | `ipv4CidrBlocks` / `ipv6CidrBlocks` |
| `CidrGroup` | `:add-cidr-blocks` | `v4CidrBlocks` / `v6CidrBlocks` |
| `AddressPool` | `:addCidrBlocks` | `v4CidrBlocks` / `v6CidrBlocks` |

То есть две оси расходятся **накрест**: `CidrGroup` разделяет адрес с сетью и подсетью, а ключ
тела — с пулом адресов. Правило «эти три ведут себя одинаково» не работает ни в одну сторону —
**сверяйтесь со страницей ресурса**.

Обе оси признаны расхождением и снимаются своими изменениями. Переименование ключа тела
ломающее, поэтому идёт отдельно от адреса и с объявленным окном.
:::
...
отношения нет → `PERMISSION_DENIED` (HTTP `403`). Проверка идет к `kaname` (RPC
...
flowchart LR
  ...
  vpc -->|"Check(subject, relation, object)"| iamsvc["kaname"]
  iamsvc -->|"allow / deny"| vpc
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>prj</code></td><td>Project (домен kaname; ссылка)</td><td><code>prj9j6y00msn65vcfdq3</code></td></tr>
  </tbody>
</table>
...
:::note Поиск адреса по значению и по подсети
Отдельных методов для этого нет: у ресурса один список, и оба вопроса задаются ему сужением —
`GET /vpc/v1/addresses?projectId=…&ipAddress=<IP>` и `…&subnetId=<id>`. Область запроса берётся
из `projectId`, который вызывающий и так обязан назвать, а страница проверяется по правам
построчно.
:::
...
:::note Имя Gateway — та же форма, что у остальных ресурсов
Здесь стояло, что `Gateway.name` подчиняется отдельному strict-контракту «в отличие от
остальных ресурсов». Отличия **нет** с #715: валидатор имени в дереве один
(`corelib/validate/nameform`), и все ресурсы платформы подчиняются DNS label по RFC 1123 —
`^[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])?$`. Соглашение об именовании переносится между
ресурсами без поправок.
:::

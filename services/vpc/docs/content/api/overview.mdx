...
Kachō VPC управляет **9 типами ресурсов** (8 публичных + admin-only `AddressPool`). Все —
плоские (flat): domain-поля на верхнем уровне сообщения, без K8s-envelope. Каждый ресурс
project-level (обязательный `projectId`), у каждого — свой ID-префикс: у ресурсов первой
волны он трёхсимвольный и слитный (`net…`), у заводимых сейчас — отделён дефисом
(`cdg-…`). Обе формы валидны; тип ресурса читается по префиксу в любой из них.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>CidrGroup</strong></td><td><code>cdg-</code></td><td>Именованный набор префиксов: перечень сетей, на который ссылаются правила групп безопасности вместо своей копии</td><td>public</td></tr>
    <tr><td><strong>Gateway</strong></td><td><code>gtw</code></td><td>Выход наружу: трансляция исходящего IPv4 либо «только исход» IPv6; привязан к подсети</td><td>public</td></tr>
...
[SecurityGroup](/api/security-group), [CidrGroup](/api/cidr-group),
[Gateway](/api/gateway), [NetworkInterface](/api/network-interface),
[AddressPool](/api/address-pool).
...
<table>
  ...
  <tbody>
    <tr><td><code>:9090</code></td><td>public</td><td>Tenant-клиенты через <code>api-gateway</code> (TLS + JWT)</td><td>Публичные сервисы 8 ресурсов (CRUD + read). Tenant-facing «намерение и результат»</td></tr>
...
## Состояние применения намерения (`applyState`)

Ресурс, созданный успешно, — это ещё не ресурс, доехавший до сети. Между «control plane
записал ваше намерение» и «сеть его применила» лежит исполнитель датаплейна, и он может
не справиться. Поле `applyState` отвечает ровно на этот вопрос и ни на какой другой.

Оно есть у **семи** ресурсов — тех, у которых существует строка намерения: `Network`,
`Subnet`, `NetworkInterface`, `SecurityGroup`, `RouteTable`, `Gateway`, `Address`.
У `CidrGroup` его нет: это именованный перечень префиксов, на который ссылаются правила,
а не объект, который сеть применяет.

:::warning Это НЕ статус ресурса и НЕ статус операции
`Operation.done` означает «намерение записано долговечно», и только это. `applyState`
означает «исполнитель отчитался о применении ТЕКУЩЕЙ ревизии ресурса». Ресурс существует и
корректен независимо от того, что ответил исполнитель.
:::

### Три состояния, и они РАЗНЫЕ

| Тело ответа | Что это значит | Что делать |
|---|---|---|
| `"applyState": null` | платформа не делает утверждения об этом объекте (намерение снимается — ресурс удаляется) | ничего: объекта скоро не будет |
| `{"applied": false, "reason": "APPLY_FAILURE_REASON_UNSPECIFIED"}` | намерение в работе: исполнитель ещё не отчитался по этой ревизии | подождать и перечитать |
| `{"applied": true,  "reason": "APPLY_FAILURE_REASON_UNSPECIFIED"}` | применено | ничего |
| `{"applied": false, "reason": "APPLY_FAILURE_REASON_CAPACITY"}` | не применено, причина названа классом | см. таблицу классов ниже |

Поле — **сообщение**, а не пара скаляров, именно ради первой строки: у скалярного `applied`
нет способа выразить «утверждения нет», и «данных нет» немедленно читалось бы как «не
применено». Поэтому проверяйте **присутствие объекта**, прежде чем читать `applied`.

`reason` присутствует всегда: маршалер публичного края отдаёт незаполненные поля, поэтому
«класса нет» приезжает токеном `APPLY_FAILURE_REASON_UNSPECIFIED`, а не отсутствием ключа.

### Ревизия ведётся платформой, и она НЕ видна

Меняя ресурс, вы поднимаете ревизию его намерения. Отчёт исполнителя привязан к ревизии,
поэтому:

- подтверждение **применения** прошлой ревизии не выдаётся за состояние новой — после
  вашей правки `applied` снова `false`, пока исполнитель не отчитается по ней;
- отказ прошлой ревизии **не приписывается** новой — иначе вы чинили бы причину, которой
  у вашей правки нет.

Самой ревизии на этой поверхности нет: последовательность у неё одна на весь сервис, то
есть её номер на вашем объекте был бы счётчиком изменений всего облака.

### Классы причины отказа — закрытый словарь

| Класс | Что произошло |
|---|---|
| `APPLY_FAILURE_REASON_CAPACITY` | ресурса фабрики не хватило |
| `APPLY_FAILURE_REASON_CONFLICT` | намерение противоречит уже применённому |
| `APPLY_FAILURE_REASON_UNSUPPORTED` | намерение выразимо контрактом, но этой фабрикой не поддерживается |
| `APPLY_FAILURE_REASON_DEPENDENCY_NOT_READY` | намерение опирается на объект, который исполнитель ещё не применил |
| `APPLY_FAILURE_REASON_TRANSIENT` | временный отказ; исполнитель повторит сам |
| `APPLY_FAILURE_REASON_EXECUTOR_INTERNAL` | внутренний отказ исполнителя |

Перечень **закрыт**: свободной строки в этом поле нет вовсе. Имени узла, номера порта или
идентификатора туннеля сюда физически нечем положить — устройство фабрики остаётся у
фабрики.

### Какие чтения поле заполняют, а какие нет

Заполняют — **`Get` и списочный RPC** каждого из семи ресурсов. В списке значение то же,
что отдало бы одиночное чтение: обновлять состояние из списка безопасно.

Не заполняют, и у каждого случая своё основание:

| Поверхность | Почему |
|---|---|
| `Operation.response` | на момент завершения операции исполнитель заведомо ещё не отчитался — поле несло бы одно и то же всегда и читалось бы как «операция готова, но что-то не так» |
| `AttachNetworkInterfaceResponse` / `DetachNetworkInterfaceResponse` | предмет ответа — исход привязки, а не состояние сети; отчёта по новой ревизии на этот момент нет |
| внутренние проекции и поток намерения исполнителя (`:9091`) | это отчёт самого исполнителя; вернуть его обратно значило бы завести второй источник одного факта |

### Полем нельзя управлять

`applyState` — только чтение: ни один запрос создания или обновления его не принимает.
`updateMask`, содержащая `applyState`, отвергается как неизвестное поле маски
(`INVALID_ARGUMENT`) — поле не «неизменяемое», его вовсе нет во входном контракте.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>AddRoutes</strong></td><td><code>POST /vpc/v1/routeTables/&#123;id&#125;:add-routes</code></td><td><strong>501 UNIMPLEMENTED</strong> (см. «Гранулярная правка маршрутов недоступна»)</td></tr>
    <tr><td><strong>RemoveRoutes</strong></td><td><code>POST /vpc/v1/routeTables/&#123;id&#125;:remove-routes</code></td><td><strong>501 UNIMPLEMENTED</strong> (там же)</td></tr>
    <tr><td><strong>UpdateRoute</strong></td><td><code>POST /vpc/v1/routeTables/&#123;id&#125;:update-route</code></td><td><strong>501 UNIMPLEMENTED</strong> (там же)</td></tr>
...
<table>
  <thead><tr><th>RPC</th><th>REST</th><th>Тип</th></tr></thead>
  <tbody>
    <tr><td><code>Get</code></td><td><code>GET /vpc/v1/cidrGroups/&#123;id&#125;</code></td><td>sync</td></tr>
    <tr><td><code>List</code></td><td><code>GET /vpc/v1/cidrGroups</code></td><td>sync</td></tr>
    <tr><td><strong>Create</strong></td><td><code>POST /vpc/v1/cidrGroups</code></td><td>async</td></tr>
    <tr><td><strong>Update</strong></td><td><code>PATCH /vpc/v1/cidrGroups/&#123;id&#125;</code></td><td>async</td></tr>
    <tr><td><strong>AddCidrBlocks</strong></td><td><code>POST /vpc/v1/cidrGroups/&#123;id&#125;:add-cidr-blocks</code></td><td>async</td></tr>
    <tr><td><strong>RemoveCidrBlocks</strong></td><td><code>POST /vpc/v1/cidrGroups/&#123;id&#125;:remove-cidr-blocks</code></td><td>async</td></tr>
    <tr><td><strong>Delete</strong></td><td><code>DELETE /vpc/v1/cidrGroups/&#123;id&#125;</code></td><td>async</td></tr>
    <tr><td><code>ListOperations</code></td><td><code>GET /vpc/v1/cidrGroups/&#123;id&#125;/operations</code></td><td>sync</td></tr>
  </tbody>
</table>

:::note Состав набора — своими действиями, не через `Update`
`Update` правит только косметику (`name` / `description` / `labels`); имя поля состава в
`updateMask` отвергается с текстом, который называет нужное действие. Подробности и границы —
[CidrGroup](/api/cidr-group).
:::
...
[RouteTable](/api/route-table), [SecurityGroup](/api/security-group),
[CidrGroup](/api/cidr-group), [Gateway](/api/gateway),

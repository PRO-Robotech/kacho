...
Непустое имя должно быть уникальным в пределах проекта. Синхронной проверки имени здесь
нет — уникальность держит сама база, поэтому дубликат приходит **ошибкой операции**
(`ALREADY_EXISTS`), а не ответом на запрос. Пустое имя допустимо и дублей не образует.

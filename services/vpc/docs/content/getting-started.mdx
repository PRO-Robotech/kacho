...
| Нужно | Зачем |
|---|---|
| Развернутый стенд Kachō (`api-gateway` + `kacho-vpc` + `kaname` + `kacho-geo`) | Принимает запросы и выполняет authz/валидацию |
| `projectId` существующего проекта | Все ресурсы VPC — project-level; владелец проекта живет в kaname |
...
`"Project with id <id> not found"`; если kaname недоступен — `UNAVAILABLE`.
:::
...
При создании задаётся **один** адресный якорь — `ipv4CidrPrimary` либо `ipv6CidrPrimary`
(строка, не список). Пустыми оба быть не могут: из такой подсети нельзя выделить ни один
адрес. Одно из двух — можно: v4-only и v6-only подсети законны.
...
`FAILED_PRECONDITION "Subnet CIDRs can not overlap"`. Часть адресного пространства
платформа держит за собой: префикс, пересекающийся со служебным диапазоном, отвергается
синхронно (`INVALID_ARGUMENT`, признак отказа — `SUBNET_CIDR_RESERVED`, см.
[Subnet](/api/subnet)).

**Дополнительные** диапазоны добавляются уже после создания — действиями
`:add-cidr-blocks` / `:remove-cidr-blocks`, а не через `Update`. В ответах подсеть несёт
плоский набор `ipv4CidrBlocks` / `ipv6CidrBlocks`, чей нулевой элемент — первичный якорь;
на **входе** создания списка нет вовсе.
:::

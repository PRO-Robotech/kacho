...
- Mapping: `ALREADY_EXISTS` на `networks_project_id_name_key` partial UNIQUE `(project_id, name) WHERE name <> ''` (форма приведена к общей миграцией `669001`: пустое имя законно по контракту и слот не занимает). Ровно та же форма — у остальных 7 пользовательских ресурсов сервиса.

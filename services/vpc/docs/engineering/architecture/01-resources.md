...
| `status` | enum | состояние ПРИВЯЗКИ: `ACTIVE` (интерфейс занят потребителем) / `AVAILABLE` (свободен) / `STATUS_UNSPECIFIED`. На вопрос «доехало ли изменение до сети» отвечает `apply_state`, а не это поле |

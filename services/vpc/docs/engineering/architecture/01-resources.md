...
| `status` | enum | состояние ПРИВЯЗКИ: `ACTIVE` (интерфейс занят потребителем) / `AVAILABLE` (свободен) / `STATUS_UNSPECIFIED`. На вопрос «доехало ли изменение до сети» это поле не отвечает — и контур не отвечает на него ничем: поверхности, которая это утверждала, у ресурса нет |
...
> [!warning] Здесь стояло обратное — «`network_id` опционально»
> Дерево говорит в одну сторону, и опор у этого **две** — обе проверяемы командой:
> use-case отвергает пустое значение синхронно, `InvalidArgument "network_id required"`
...
>
> **Опор было три, и третья снята вместе со своим предметом** (kacho#1255): контракт
> объявлял поле обязательным опцией семейства `kacho.cloud.validation`, у которого не было
> исполнителя на пути запроса НИ ОДНОГО. Довод «дерево говорит трижды» держался на ней
> только по видимости: объявление ничего не проверяло, и ссылаться на него значило считать
> опорой то, что не держит. Оставшиеся две — держат, и именно они отвергают пустое поле.
>

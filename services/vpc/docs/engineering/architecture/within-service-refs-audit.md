...
| `external_ipv6 ->> 'address'` | глобально уникальный (паритет с v4) | `addresses_external_v6_ip_uniq` partial UNIQUE ✅ (миграция 0022) | канонизация адреса на входе Create | OK |
| внешний адрес, заданный вызывающим | занят в книге учёта пула-владельца (изъят из свободного списка v4 / записан в книгу v6) | атомарный CAS в единственном пути записи строки адреса ✅ | формат и семейство проверяются синхронно | OK |
| одна внутренняя семья на строку | generated-колонка `internal_subnet_id` авторитетна (она выбирает v4, затем v6) | CHECK `addresses_single_internal_family` ✅ (миграция 0025) | ветви спека взаимоисключающи в use-case | OK |

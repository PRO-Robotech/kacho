...
не навешивается (WARN-only), а AuthN-guard в dev-режиме не требует принципала.
...
Прежний вопрос — `viewer` на singleton `cluster:cluster_kacho_root` — не только не
подходил, но и не отсекал никого: это relation **глобального справочника** (регионы,
зоны, типы дисков), и bootstrap кластера намеренно пишет
`cluster:<root>#viewer@user:*` (`deploy/helm/umbrella/charts/openfga-bootstrap`), чтобы
справочник читал любой аутентифицированный субъект. Значит проверка пропускала каждого
аутентифицированного вызывающего и отдавала привязки любых названных им инстансов — из
чужих проектов и аккаунтов, вместе с MAC, подсетью и резолвнутыми адресами. Привязки
интерфейсов справочными данными не являются.
...
- у «любого порта» два написания — незаполненный `ports` и `-1/-1`. Они **не**
  нормализуются друг в друга: незаполненный сохраняется как `0,0` и на чтении
  снова опускается (`dto/toproto`), а `-1/-1` так и возвращается в ответе;
...
два, а три, и **одна граничная величина интервала невыразима** — её запись
неотличима в хранилище от «`ports` не заполнен». Это тот же класс, что закрыт в
§25 для протокола, но здесь у него **нет запасного написания**: там невыразимое
значение адресуется именем, здесь адресовать нечем.

Починка требует различать «не задано» и эту величину в **самом хранилище**

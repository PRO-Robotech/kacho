...
//
// Транспортные пакеты уехали из этого дерева в отдельный модуль (задача #2131),
// и словарь резолверов надо сверять с той их редакцией, которую собирает ЭТО
// дерево, — то есть с версией из его `go.mod`, а не с последней выпущенной.
...
// лежал здесь. После выноса (задача #2131) координата перестала резолвиться, и
// проба падала на собственной технике, ничего не сказав о словаре. Словарь при
// этом оказался ПОЛОН: все четыре резолвера на месте, переехало только место.

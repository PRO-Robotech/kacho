...
**Механизм с тех пор и сменился — 2026-08-11.** Все семь методов, объявленных контрактом и
не несущих реализации, переопределены рукой и отвечают `Unimplemented` **с названной
причиной и адресом владельца возможности** (`internal/handler/declared_but_absent.go`).
Прежде они наследовались от заглушки, то есть вызывающий получал `method X not implemented`:
эта страница причину называла, а клиент API не узнавал её никогда — при том что именно он
и сталкивается с ограничением. Тон сообщений — часть контракта, его держит
`declared_but_absent_refusal_test.go` (утверждает СООБЩЕНИЕ, а не только код: у названного
и унаследованного отказа код один и тот же), а свойство дерева — гейт
`internal/repohygiene` `TestEveryDeclaredRPCIsOverridden`.
...
| `Instance.Relocate` | перенос томов в другую зону (домен kacho-storage) | явный отказ `Unimplemented` с названной причиной | перенос томов у владельца + restart-семантика |

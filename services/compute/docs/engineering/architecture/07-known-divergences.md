...
## Контейнерная машина сегодня НЕ СОЗДАЁТСЯ — следствие отказа по источнику реестра

`instanceKind = CONTAINER` объявлен контрактом и обслуживается кодом, но создать
такую машину нельзя: её корневая файловая система — образ реестра, а `bootSource.type
= registry.image` **отвергается явно** (у образа реестра нет durable-координаты: у
репозитория нет неизменяемого идентификатора, адресация идёт парой «реестр, имя» при
живом переименовании).

**Это следствие названо здесь потому, что в своё изменение оно не попало.** Отказ по
форме источника вводился как правка ФОРМЫ идентификатора и был верен по своему
предмету; то, что он делает неконструируемым целый вид машины, не было сказано ни в
сообщении коммита, ни в этом документе. Нашёл это сквозной прогон — 77 упавших
утверждений от одной причины.

**Предикат снятия — тот же, что у ввоза образа:** у репозитория появляется
неизменяемый идентификатор. Тогда ветвь источника открывается, а кейс
`INST-RD-CR-VAL-CONTAINER-REGISTRY-SOURCE-REFUSED` обязан покраснеть — он утверждает
действующий отказ, а не желаемое поведение.

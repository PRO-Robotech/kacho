...
## Потоковое чтение `InternalWatchService.Watch` существует, а корпус объявляет, что его нет

**Отступление от чего.** `api-conventions.md` задаёт форму чтения прямо: `Get`/`List`
синхронны, мутации асинхронны через `Operation`, клиент **поллит**, и — дословно —
«Watch RPC не существует». У compute он существует: `kacho.cloud.compute.v1.
InternalWatchService/Watch`, server-stream из `compute_outbox` с пробуждением по
`LISTEN/NOTIFY`.

**Потребителей — ноль.** Предикат: `git grep NewInternalWatchServiceClient` → только
сгенерированный код. Ни сервис, ни край, ни консоль, ни модуль инфраструктуры, ни один
чёрный ящик его не зовут. При этом поверхность полная: сервер, регистрация на
внутреннем слушателе, запись каталога прав, пообъектное сужение по правам вызывающего,
предел одновременных потоков, три интеграционные пробы.

**Почему это записано ЗДЕСЬ, а не «оставлено как есть».** Комментарий самого контракта
называл назначение прямо: «поток событий из `compute_outbox` **для будущих
consumer'ов** / observability / admin-tooling». Поверхность, заведённая под будущего
потребителя, запрещена (ban #11, ban #14), и запрет не отменяется тем, что код
качественный. Рядом второе свидетельство: шапка сетевой политики vpc называет
`InternalWatchService.Watch(from=0)` вектором массовой выгрузки журнала — риск у
поверхности признан, выгодоприобретателя нет.

**Исход назван, и он один: СНЯТЬ** — задача **#813**. Там же предикат снятия и радиус
(47 файлов: контракт, стабы, обе копии каталога прав, край, список проксирования,
boot-guard, конфигурация, сетевая политика, пробы, документация). Эта запись
существует ровно затем, чтобы поверхность без потребителя не выглядела решённым
вопросом: до #813 она остаётся, после — снимается вместе с предметом.

**Что уже сделано этой же задачей (#761).** Приведён к реальности контракт поля
`from_sequence_no`: он обещал «0 = начать с current end (только новые события)», а
курсор ИСКЛЮЧАЮЩИЙ, поэтому ноль отдаёт весь удержанный журнал. Замерено пробой, а не
прочитано: на трёх предсуществующих строках подписчик получал три при обещанных нуле.
Держит смысл нуля `TestIntegration_WatchStreamSince_ZeroCursorReplaysWhatIsRetained` —
она утверждает ЧИСЛО доставленного, поэтому возврат прозы «только новые» ей не солгать.

**Чего делать НЕЛЬЗЯ:** «починить» это, заведя потребителя ради потребителя. Предмет
записи — отсутствие выгодоприобретателя, а не отсутствие подписчика в коде.

_Reviewed 2026-08-20 (задача #761, исход назван; снятие — #813)._

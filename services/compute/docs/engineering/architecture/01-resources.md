...
Источник истины — `corelib/ids/ids.go` (в прежней полирепо-топологии этот пакет звался
...
- `compute_outbox` — outbox-таблица событий (`resource_kind` ∈ {Instance},
  ...
  подписки платформы (`corelib/subscription`). Собственный стрим сервиса снят (#813);

...
>
> При этом **51 строка в 12 документах этого каталога** всё ещё пишет об этих ресурсах
> как о своих (предикат счёта — тот же `grep -rncE`, см. историю правки). Приведение
> глав к дереву — отдельная работа со своей приёмкой; до неё **предметом главы считай
> proto и код, а не главу**. Здесь эта оговорка стоит потому, что индекс читают первым,
> и молчание в нём означало бы, что расхождения нет.
>

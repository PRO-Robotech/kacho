...
| `golang-grpc` | `grpcsrv` из corelib (recovery + logging interceptors). Два gRPC-сервера: `:9090` public (`InstanceService`/`MachineTypeService`/`OperationService`), `:9091` internal (`InternalWatchService`/`InternalMachineTypeService`). `status.FromError`-mapping в handler через `mapRepoErr`. Streaming только в `InternalWatchService.Watch` (один-к-одному с pgx LISTEN/NOTIFY). Plaintext в dev, TLS-флаги для cross-service (`KACHO_COMPUTE_VPC_TLS`, `KACHO_COMPUTE_IAM_TLS`). |

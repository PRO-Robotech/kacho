...
## 13. У compute нет сетевой политики ни в одном профиле — эшелонирование отсутствует, авторизация нет

**Что установлено (по дереву, не рассуждением):** шаблона сетевой политики для
kacho-compute не существует (`services/compute/deploy/templates/` — certificate,
deployment, hpa, service), в блоке профиля боевых значений слова о ней нет, и в
`deploy/helm/umbrella/templates/` есть ровно один такой шаблон — для kacho-vpc
(он же намеренно выключен в отладочном профиле). То есть на сетевом уровне
internal-порт compute открыт любому поду namespace'а.

**Почему это здесь записано, а не «починено попутно»:** сетевая политика —
**эшелонирование**, а не авторизация. Её отсутствие вскрылось при разборе потока
журнала изменений (`InternalWatchService/Watch`), комментарий у которого ссылался на
неё как на действующее ограничение. Ссылка была ложной, и именно она делала
отсутствие авторизации у потока похожим на осознанный размен: «порт всё равно
закрыт сетью» — а он не закрыт. Комментарий приведён к правде
(`cmd/compute/main.go`, `registerInternalServices`), а сама авторизация закрыта **на
уровне данных** (сужение по правам вызывающего на каждую отдаваемую строку), потому
что сетевая политика её заменить не может: подтверждённый пир внутреннего CA —
по-прежнему пир, а тенантные границы TLS не различает.

**Что ограничивает доступ к internal-листенеру сейчас:** mTLS на листенере +
непустой allow-list законных отправителей чужой личности (боевая стража отказывает
в старте на пустом) + per-RPC Check на каждом замапленном RPC
(`internal/check/permission_map.go`) + для потока журнала — per-row сужение и
безусловная отсечка неназванного вызывающего. Отсечение тенантского REST от
Internal*-поверхности делает allow-list шлюза (`HasInternalSuffix`), а не сеть.

**Что осталось:** завести шаблон по образцу vpc-шного и включить его в боевом
профиле. Это добавит эшелон, но ни одного из перечисленных выше контролей не
отменяет и ни одного не заменяет.

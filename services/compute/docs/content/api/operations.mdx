...
`DetachDisk` / `AttachNetworkInterface` / …) выполняется **не синхронно**:
...
`Operation` (`corelib.operation.Operation`) — плоский envelope с полем-результатом в
виде `oneof`:
...
- **Подписки на операции нет**, и поток изменений ресурсов её не заменяет: событие пишется в той же транзакции, что и строка машины, поэтому у провалившейся операции события нет вовсе — наблюдатель ресурсов не отличит «ещё делается» от «упало». Исход операции узнаётся поллом.
- **Изменения самих машин** compute отдаёт потоком: журнал compute — владелец подписки платформы, и край проецирует его наружу ручкой `GET /subscription/v1/events?owner=compute` (вид предмета — `compute_instance`: это имя типа в модели прав, и именно в этом написании его принимает `kinds`; слово журнала владельца наружу не выходит). Опрос при этом остаётся штатным и работает всегда; подписка — второй путь. Параметры, кадры, возобновление и словари описаны в документации api-gateway, страница «Подписка на изменения».
:::
...
Ниже — метадата операций **kacho-compute**. Блочное хранение (`Volume` / `Image` / `Snapshot`)
принадлежит kacho-storage: его операции несут `sop`-префикс, метадату
`kacho.cloud.storage.v1.*` и описаны документацией владельца.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>UpdateInstanceMetadata</code></td><td><code>instanceId</code></td></tr>
    <tr><td><code>DeleteInstanceMetadata</code></td><td><code>instanceId</code></td></tr>
    <tr><td><code>StartInstanceMetadata</code> / <code>StopInstanceMetadata</code> / <code>RestartInstanceMetadata</code></td><td><code>instanceId</code></td></tr>
    <tr><td><code>AttachInstanceDiskMetadata</code></td><td><code>instanceId</code>, <code>volumeId</code></td></tr>
    <tr><td><code>DetachInstanceDiskMetadata</code></td><td><code>instanceId</code>, <code>volumeId</code></td></tr>
  </tbody>
</table>

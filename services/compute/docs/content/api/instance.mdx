...
`zoneId` и `bootSource` — **immutable**. `metadata` меняется отдельным RPC `UpdateMetadata`.
...
:::warning Изменение интерфейса и one-to-one NAT через Instance недоступны
`updateNetworkInterface`, `addOneToOneNat` и `removeOneToOneNat` объявлены в контракте, но
отвечают **`UNIMPLEMENTED`** (HTTP 501). Это не пробел в описании и не временный сбой: адрес,
NAT и группы безопасности — свойства ресурса `NetworkInterface`, который принадлежит
**kacho-vpc**, и правятся там же, а не через инстанс. Инстанс их зеркалит.

По той же причине `UNIMPLEMENTED` отвечает `relocate` (перенос машины в другую зону требует
переноса её томов, а тома принадлежат kacho-storage) и тройка `listAccessBindings` /
`setAccessBindings` / `updateAccessBindings` (права выдаёт kacho-iam).

Отказ здесь — **решение о владельце**, а не отложенная доделка: он останется отказом и
после того, как соответствующая возможность появится у владельца.
:::

...
flowchart LR
  ...
  cmp -->|"Check(subject, relation, object)"| iamsvc["kacho-iam"]
...
<code>GET /subscription/v1/events?owner=compute&amp;kinds=Instance</code> (Server-Sent Events):
события несут род изменения и, у создания и правки, полное состояние машины. Опрос
<code>List</code> при этом остаётся штатным путём и работает всегда. Адрес, параметры, кадры и
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>prj</code></td><td>Project (домен kacho-iam; ссылка), слитно</td><td><code>prj9j6y00msn65vcfdq3</code></td></tr>
  </tbody>
</table>

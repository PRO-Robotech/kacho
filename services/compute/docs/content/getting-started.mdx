...
| Нужно | Зачем |
|---|---|
| Развёрнутый стенд Kachō (`api-gateway` + `kacho-compute` + `kaname` + `kacho-geo`) | Принимает запросы и выполняет authz/валидацию |
| `projectId` существующего проекта | Все ресурсы Compute — project-level; владелец проекта живёт в kaname |
...
no durable address today"` — у образа реестра сегодня нет неизменяемого адреса, а ссылка на
пару «реестр + имя» стала бы ложной после чужого переименования. Единственный принимаемый
источник загрузки — `storage.image`; его `id` — обычный идентификатор образа (`img…`), тег
или дайджест не требуются.
:::

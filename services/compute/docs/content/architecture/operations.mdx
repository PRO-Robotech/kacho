...
Журнал читает **сам сервис** — восстановление наблюдаемого состояния
(`internal/repo/observed_state.go`). Подписки снаружи нет: серверный стрим
`InternalWatchService` снят задачей #813, потому что подписчика у него не было ни одного
дня. Публичного Watch в API нет и не планируется — клиенты поллят
`OperationService.Get` и списки (см. конвенции продукта).

...
3. ~~**Legacy-набор кейсов инстанса (77 кейсов)**~~ — **снято 2026-07-28: файла нет.**
   ...
   **Остатки снятого файла закрыты (эта ветка).** Сам файл ушёл, а ссылки на него — нет,
   ...
   `run.sh` список имён коллекций (`disk`/`image`/`snapshot`/`instance`/`disk-type`/
   `operation`), из которых в дереве осталось одно. Прогон по умолчанию перечислял ОДНУ
   коллекцию из шести и печатал пять строк `[skip]` — «не выполнилось» никуда не
   вычиталось, а отчёт выглядел исправным, потому что честно сообщал про то, что прогнал.
   Теперь набор берётся из имён коллекций на диске, названная-но-отсутствующая коллекция —
   ОТКАЗ (код 2), а не пропуск, и появилось сухое перечисление `--list`, которое печатает
   набор без стенда (и потому проверяемо на свежем checkout'е). Заодно сняты перечень
   снятого файла в `CASES-INDEX.md` и таблица «Итого 286» ниже.
...
Число кейсов НЕ выписывается здесь таблицей: оно печатается генератором
(`python3 scripts/gen.py`) и сухим перечислением прогонщика
(`./scripts/run-incremental.sh --list`, стенд не нужен), и любая переписанная руками
копия расходится с деревом молча. На момент правки — **6 коллекций, 117 кейсов**
(authz-deny 42, instance-redesign 49, machine-type 12, operation 8, list-filter 4, sec-d 2).

Прежняя таблица «Итого 286» перечисляла `disk`/`instance`/`image`/`snapshot`/`disk-type` —
пять стволов, которых в дереве нет: четыре ушли к kacho-storage вместе с дублем блочного
хранения, кейсы инстанса переехали в `instance-redesign`. Тот же список жил вторым,
независимым экземпляром в `run-incremental.js` и там был не косметикой: прогон по
умолчанию перечислял ОДНУ коллекцию из шести и печатал пять строк `[skip]`, а отчёт
выглядел исправным, потому что честно сообщал про то, что прогнал.
...
## Смена актора суиты: бутстрап-админ → проектный (kacho#72)

**Это запись об ИЗМЕНЕНИИ, а не о прогоне.** Стенд не поднимался; ниже — что изменено, чем
это удержано и что смотреть на первом прогоне. Выдавать её за результат нельзя.

**Что было.** Дефолтный Bearer всех шести коллекций — `jwtBootstrap`. Бутстрап-админ держит
права на всё, поэтому каждый шаг проходил независимо от того, работает ли project-scope
авторизация: суита не могла отличить исправное от сломанного. Класс не гипотетический — карта
прав сервиса, разошедшаяся с каталогом края по паре scope+relation, отвергает проектного
принципала на ЕГО ЖЕ ресурсах, и бутстрап этого не видит.

**Что стало.** Дефолт — `jwtProjectAdminA1` (editor на `project-A1` и `project-A2`). Паритет с
vpc, где это уже так. Cluster-admin остался ровно там, где проектный актор недоступен by
construction, и каждое место объявлено шагом:

| Где | Почему проектный актор не подходит |
|---|---|
| `InternalMachineTypeService.Create/Delete/Update` (65 шагов) | каталог прав: `system_admin` на cluster-singleton; посев выдаёт матричным учёткам только `system_viewer@cluster` |
| `OP-GET-CRUD-FAILED-OP` | спусковой крючок — создание в чужом проекте; край отвергает его до заведения Operation (anti-BOLA). Сам отказ проверяет `AUTHZ-INSTANCE-CR-CROSS-PA1` |

**Дополнительных грантов не потребовалось — установлено по коду, а не предположено:**

- Instance CRUD — привязка роли `edit` на `project-A1`/`A2` из посева; пообъектные
  `v_get`/`v_update`/`v_delete` материализуются форвардом реконсайлера (миграция 0053 сеет
  селекторы `edit`, среди типов — `compute.instance`);
- публичный read каталога размерностей (`viewer` на cluster-singleton) — `subject()` в
  `tests/authz-fixtures/prodseed_matrix.py` выдаёт каждой матричной учётке
  `system_viewer@cluster`, из которого модель выводит `viewer`. Без него не помог бы и
  подстановочный `user:*` из бутстрапа кластера: он типизирован по `user`, а все предъявители
  харнесса — служебные учётки;
- `geo.ZoneService.Get` на пути создания — тот же `system_viewer`-этаж;
- `iam.ProjectService.Get` (`v_get` на проекте) и `vpc.SubnetService.Get`
  (`v_get` на подсети, читается ПОД ИДЕНТИЧНОСТЬЮ ВЫЗЫВАЮЩЕГО) — из той же привязки `edit`
  на `project-A1`, форвардом реконсайлера.

**Что смотреть на первом прогоне (не проверено без стенда):**

1. **`v_get` проектного актора на `existingSubnetId`/`existingSgId`.** Их сеет бутстрап
   (`tests/authz-fixtures/prodseed_compute_ext.py`) в тот же `project-A1`, и видимость
   держится форвардом реконсайлера по привязке `edit`, а не владением. Порядок посева этому
   благоприятствует (привязки заводятся раньше ресурсов, значит форвард при регистрации
   подсети уже видит биндинг), но проверено это будет только прогоном. Симптом отказа —
   `Subnet <id> not found` в ошибке операции создания у `instance-redesign`/`operation`/`sec-d`
   и НЕ у `list-filter` (та ходит `useDefaultNetwork` и подсеть не называет). Если так —
   дефект в выдаче, а не в кейсе.
2. **Окно read-your-writes стало настоящим.** Под бутстрапом первый `Get` своего свежего
   ресурса разрешался плоско кластерным ярусом, поэтому обёртки ожидания фактически ни разу
   не ждали. Теперь они на своём предмете; бюджет (40×600 мс) не менять без замера.

**Чем удержано.** Проверка 4 в `scripts/validate-cases.py` (дефолт коллекции · явный актор на
cluster-scoped админ-маршруте · Operation читает её создатель) + предпосылка запрета читается
из каталога прав края, поэтому смещение основания даёт красное с объяснением, а не молчание.
Доказательство инъекцией исполняемо — `validate-cases.py --self-test`, прогоняется целью
`make -C deploy gate-self-test`.

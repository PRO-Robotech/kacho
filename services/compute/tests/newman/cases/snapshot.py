...
CASES.append(Case(
    ...
    steps=[
        *_pre_disk("crok"),
        ...
        retry_until_authorized(Step(name="get", method="GET", path=f"{SNAPS}/{{{{snapshotId}}}}",
        ...
        *_pre_disk("upd"),
        Step(name="cr", method="POST", path=SNAPS,
             ...
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.snapshotId", "snapshotId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="patch", method="PATCH", path=f"{SNAPS}/{{{{snapshotId}}}}",
             body={"updateMask": "name,description,labels", "name": "snap-upd2-{{runId}}",
                   "description": "updated-newman", "labels": {"env": "prod"}},
             test_script=[*assert_status(200), *save_from_response("j.id", "opId")])),
        poll_operation_until_done(), assert_op_success(),
        retry_until_authorized(Step(name="verify", method="GET", path=f"{SNAPS}/{{{{snapshotId}}}}",
             test_script=[*assert_status(200),
                          "const j = pm.response.json();",
...
CASES.append(Case(
    ...
    steps=[
        *_pre_disk("delafter"),
        ...
        retry_until_authorized(Step(name="del-disk", method="DELETE", path=f"{DISKS}/{{{{baseDiskId}}}}",
             test_script=[*assert_status(200), *save_from_response("j.id", "opId")])),
        poll_operation_until_done(), assert_op_success(),
        ...
        *_pre_disk("lop"),
        Step(name="cr", method="POST", path=SNAPS,
             ...
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.snapshotId", "snapshotId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="list-ops", method="GET", path=f"{SNAPS}/{{{{snapshotId}}}}/operations",

    ...
    steps=[
        ...
        Step(name="cr-disk-from-img", method="POST", path=DISKS,
             ...
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.diskId", "diskId")]),
        poll_operation_until_done(), assert_op_success(),
        retry_until_authorized(Step(name="get-disk", method="GET", path=f"{DISKS}/{{{{diskId}}}}",
             test_script=[*assert_status(200),
                          "const j = pm.response.json();",
                          "pm.test('status READY', () => pm.expect(j.status).to.eql('READY'));",
...
CASES.append(Case(
    ...
    steps=[
        ...
        retry_until_authorized(Step(name="cleanup", method="DELETE", path=f"{DISKS}/{{{{diskId}}}}", test_script=[*save_from_response("j.id", "opId")])),
        poll_operation_until_done(),
    ],
))
    ...
    steps=[
        Step(name="cr", method="POST", path=DISKS, body=_disk_body("upd", description="init", labels={"orig": "1"}),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.diskId", "diskId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="patch", method="PATCH", path=f"{DISKS}/{{{{diskId}}}}",
             body={"updateMask": "name,description,labels", "name": "disk-upd2-{{runId}}",
                   "description": "updated-newman", "labels": {"env": "prod"}},
             test_script=[*assert_status(200), *save_from_response("j.id", "opId")])),
        poll_operation_until_done(), assert_op_success(),
        retry_until_authorized(Step(name="verify", method="GET", path=f"{DISKS}/{{{{diskId}}}}",
             test_script=[*assert_status(200),
                          "const j = pm.response.json();",
    ...
    steps=[
        Step(name="cr", method="POST", path=DISKS, body=_disk_body("inc", size=_DEF_SIZE),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.diskId", "diskId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="patch-grow", method="PATCH", path=f"{DISKS}/{{{{diskId}}}}",
    ...
    steps=[
        Step(name="cr", method="POST", path=DISKS, body=_disk_body("delok"),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.diskId", "diskId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="del", method="DELETE", path=f"{DISKS}/{{{{diskId}}}}",
             test_script=[*assert_status(200), *save_from_response("j.id", "opId")])),
        poll_operation_until_done(), assert_op_success(),
    ...
    steps=[
        Step(name="cr", method="POST", path=DISKS, body=_disk_body("rel"),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.diskId", "diskId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="relocate", method="POST", path=f"{DISKS}/{{{{diskId}}}}:relocate",
    ...
    steps=[
        Step(name="cr", method="POST", path=DISKS, body=_disk_body("lop"),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.diskId", "diskId")]),
        poll_operation_until_done(),
        retry_until_authorized(Step(name="list-ops", method="GET", path=f"{DISKS}/{{{{diskId}}}}/operations",
             test_script=[*assert_status(200),

...
| `compute_watch_cursors` | Курсоры подписчиков потока изменений |
...
|---|---|---|
| `instances.project_id` | kacho-iam | `ProjectService.Get` на Create |
| `instances.zone_id` | kacho-geo | `ZoneService.Get` на Create |
| `instances.service_account_id` | kacho-iam | резолв на Create |
...
crockford-base32 (`kacho-corelib/ids`).

...
--
-- WHY: compute_fga_register_outbox carries BOTH fga.register AND fga.unregister of
...
-- a fresh successor (attempt_count = 0) even at ApplyConcurrency = 1 — and lands in a
-- LATER batch. ApplyConcurrency (compute runs 16 by default) only adds intra-batch
-- reorder on top. PartitionColumn closes the class at CLAIM level: a row is never
-- claimed while a DELIVERABLE (sent_at IS NULL AND attempt_count < MaxAttempts)
-- same-partition predecessor with a smaller id exists, via
--
...
--
-- WHY resource_id is the right partition key: every emitter (repo.emitFGARegisterIntent,
-- corelib outbox/reconciler insertIntent) writes ONE row per FGA object and stamps
...
-- index (migration 0010). The pending backlog is small on a healthy stand; on a
...
CREATE INDEX IF NOT EXISTS compute_fga_register_outbox_partition_head_idx
    ON compute_fga_register_outbox (resource_id, id)
...
DROP INDEX IF EXISTS compute_fga_register_outbox_partition_head_idx;

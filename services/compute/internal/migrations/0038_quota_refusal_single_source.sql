...
CREATE OR REPLACE FUNCTION public.kacho_quota_refuse(
    v_carrier_type text,
    v_carrier_id   text,
    v_kind         text
)
...
BEGIN
    SELECT limit_value, used INTO v_limit, v_used
      FROM public.project_resource_quotas
...
COMMENT ON FUNCTION public.kacho_quota_refuse(text, text, text) IS
...
-- обратно.
CREATE OR REPLACE FUNCTION public.kacho_quota_admit(
    v_carrier_type text,
    v_carrier_id   text,
    v_kind         text
)
...
BEGIN
    ...
    PERFORM public.kacho_quota_refuse(v_carrier_type, v_carrier_id, v_kind);
END;
$$;

COMMENT ON FUNCTION public.kacho_quota_admit(text, text, text) IS
...
DROP FUNCTION IF EXISTS public.kacho_quota_admit(text, text, text);
DROP FUNCTION IF EXISTS public.kacho_quota_refuse(text, text, text);
-- +goose StatementEnd

...
CREATE OR REPLACE FUNCTION public.kacho_quota_used_actual(
    v_carrier_type text,
    v_carrier_id   text,
    v_kind         text
)
...
COMMENT ON FUNCTION public.kacho_quota_used_actual(text, text, text) IS
...
-- таблицу целиком.
CREATE OR REPLACE FUNCTION public.kacho_quota_recount(v_apply boolean DEFAULT true)
RETURNS TABLE (
    carrier_type text,
    carrier_id   text,
    kind         text,
...
BEGIN
    FOR r IN
        SELECT q.carrier_type AS ct, q.carrier_id AS ci, q.kind AS k, q.used AS u
          FROM public.project_resource_quotas q
         ORDER BY q.carrier_type, q.carrier_id, q.kind
    LOOP
        v_seen := v_seen + 1;
        v_actual := public.kacho_quota_used_actual(r.ct, r.ci, r.k);
        ...
        IF v_apply THEN
            UPDATE public.project_resource_quotas q
...
COMMENT ON FUNCTION public.kacho_quota_recount(boolean) IS
...
DECLARE
    v_fixed bigint;
BEGIN
    SELECT count(*) INTO v_fixed FROM public.kacho_quota_recount(true);
...
DROP FUNCTION IF EXISTS public.kacho_quota_recount(boolean);
DROP FUNCTION IF EXISTS public.kacho_quota_used_actual(text, text, text);
-- +goose StatementEnd

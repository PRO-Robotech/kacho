...
//
// Предмет: прежде образ штампа не ставил вовсе, и ряд отвечал умолчаниями
...
func TestBuildInfo_StampedValuesReachTheSeriesUntouched(t *testing.T) {
	...
	want := `kacho_compute_build_info{commit="1eedc9e7ff",version="release/kaname-tail"} 1`
...
func TestBuildInfo_UnstampedBuildSaysSoInAWord(t *testing.T) {
	...
	want := `kacho_compute_build_info{commit="` + observability.BuildStampUnstamped +

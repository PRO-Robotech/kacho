...
// visibilityRelations — предикат членства строки в СТРАНИЦЕ публичного List.
//
// Ровно одно отношение, и это ТО ЖЕ отношение, которым per-RPC Check гейтит
// одиночное чтение (`InstanceService/Get` → `v_get`, permission_map.go и запись
// каталога шлюза). Совпадение — не совпадение имён, а инвариант: страница не может
// быть шире чтения.
//
// # Почему здесь больше нет союза `viewer ∪ v_list`
//
// Раньше предикат страницы был `viewer ∪ v_list`, а Get гейтился `v_get`. В модели
// это РАЗНЫЕ множества: ярусные отношения (`viewer`/`editor`/`admin`) и глагольные
// (`v_*`) развязаны — ни одно не выводится из другого (fga_model.fga; развязка
// сделана намеренно, как анти-over-grant guard, см. миграцию iam 0040). Значит союз
// и `v_get` расходились в ОБЕ стороны, и обе наблюдались:
//
//   - субъект, держащий ярус (или `v_list`) без `v_get`, получал объект в выдаче
//     List, а на Get — отказ. Это не только оракул существования: List отдаёт ТО ЖЕ
//     сообщение, что и Get, поэтому членство в странице раскрывало СОДЕРЖИМОЕ
//     целиком — строго больше, чем факт наличия;
//   - обратно: субъект, держащий `v_get` без яруса, мог прочитать объект по id, но
//     не находил его в собственном списке.
//
// Комментарии здесь и в actions.go при этом УТВЕРЖДАЛИ обратное — «видимый набор
// равен Check-allow набору (read==enforce)». Утверждение было ложным с момента
// развязки ярусов и глаголов; код ему не соответствовал.
//
// # Что стало с `v_list`
//
// `v_list` остаётся отношением модели и продолжает гейтить списки, вложенные В
// объект (`InstanceService/ListOperations` → `v_list` на `compute_instance` — там вопрос
// задаётся о РОДИТЕЛЕ, и это другой вопрос). Как предикат членства страницы он снят:
// пока List возвращает полное сообщение ресурса, «видеть в списке без содержимого»
// нереализуемо — содержимое и есть то, что возвращается. Роль с одним лишь глаголом
// `list` теперь не показывает объект; показать его можно, только дав `get`.
//
// Стоимость при этом упала вдвое: одна relation вместо двух последовательных.
var visibilityRelations = [...]string{"v_get"}
...
// Narrows сообщает, действительно ли этот фильтр СУЖАЕТ выдачу.
//
// Три состояния, в которых он этого не делает, и все три выглядят как «фильтр
// подвешен»: nil-получатель, снятый мастер-переключатель либо отсутствующий клиент к
// модели (FilterVisibleIDs возвращает идентификаторы КАК ЕСТЬ), и мягкий проход на
// ошибке (FailOpen — любой сбой обращения к модели отдаёт порцию нефильтрованной,
// см. handleErr).
//
// Спросить об этом нужно отдельно, потому что для СПИСОЧНОГО RPC любое из трёх
// защитимо: под ним остаётся per-RPC Check на проектном ярусе, то есть отказ
// вызывающему без прав всё равно наступит, а мягкий проход — задокументированный
// аварийный размен, запрещённый боевой стражей. Для RPC, у которого per-RPC Check
// снят ПО ПОСТРОЕНИЮ (единого объекта для одного вопроса не существует — поток
// журнала изменений), под фильтром не остаётся ничего, и «подвешен» перестаёт быть
// утверждением о сужении. Такой вызывающий обязан уметь отличить одно от другого,
// иначе присутствие фильтра само становится той самой формой без содержания.
func (f *FGAFilter) Narrows() bool {
	return f != nil && f.cfg.Enabled && f.cli != nil && !f.cfg.FailOpen
}
...
func (f *FGAFilter) handleErr(ids []string, err error) ([]string, error) {
	if f.cfg.FailOpen {
		// Degraded mode: оператор явно выбрал мягкий проход, поэтому per-object
		// проверка обходится (вся страница становится видимой). Это ровно тот класс
		// over-show, от которого защищает list-filter — он ОБЯЗАН быть громким и
		// СЧИТАННЫМ, а не тихой authz-деградацией. Классификация и счётчики — в openPass.

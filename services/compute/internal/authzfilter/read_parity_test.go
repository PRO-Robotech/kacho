...
// `InstanceService/Get` → `v_get`; то же значение несёт запись каталога шлюза).
//
...
func TestFGAFilter_PageMembershipRequiresReadRelation(t *testing.T) {
	const (
		outsiderObj = "ins_tier_only" // держит ярус, не держит глагол чтения
		legitObj    = "ins_readable"  // держит ровно то, чем гейтится Get
	)
...
func TestFGAFilter_AsksExactlyTheReadRelation(t *testing.T) {
	cli := newFakeAuthorizeClient().allow(readRelation, "com_a")
	f := NewFGAFilter(cli, DefaultConfig())

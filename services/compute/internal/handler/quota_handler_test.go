...
func (s *quotaRowsStub) Materialize(context.Context, []ports.QuotaRow) (int64, error) {
	return 0, nil
}
...
func quotaFixture(t *testing.T) (*QuotaHandler, *quotaRowsStub, *quotaLimitsStub) {
	...
	rows := &quotaRowsStub{byProject: map[string][]quotaread.State{
		"prj-mine": {
			{Kind: "compute.guestAccessKey", Limit: 10, Used: 1,
				SourceScope: "DEFAULT",
				CarrierType: quotaread.CarrierProject, CarrierID: "prj-mine"},
			{Kind: "compute.instance", Limit: 32, Used: 7,
				SourceScope: "PROJECT", SourceScopeID: "prj-mine",
				CarrierType: quotaread.CarrierProject, CarrierID: "prj-mine"},
		},
		"prj-theirs": {
			{Kind: "compute.instance", Limit: 999, Used: 900,
	...
	return NewQuotaHandler(quotaband.NewGuard(rows, limits, quotaAccountsStub{}, "compute"), quotaread.AuthorityDeclared()), rows, limits
}
...
func TestQuotaHandler_ListShowsTheCallersOwnLimitAndUsage(t *testing.T) {
	...
	resp, err := h.List(context.Background(), &computev1.ListQuotasRequest{ProjectId: "prj-mine"})
	...
	require.Equal(t, "compute.instance", got.GetKind())
...
//
// Это отрицание к пробе выше, и оно осмысленно только рядом с ней: дублёр держит
// строки обоих проектов, поэтому «вернулось пусто» здесь означало бы не
// исправность, а промах по адресу.
func TestQuotaHandler_ListNeverShowsAnotherProjectsNumbers(t *testing.T) {
	...
	for _, q := range resp.GetQuotas() {
		...
		require.NotEqualValues(t, 900, q.GetUsed(),
			"потребление чужого проекта видно в ответе")
	}
}

// Пустой проект — отказ ПО ИМЕНИ ПОЛЯ, а не отказ в правах.
//
// Оставленная краю, пустая строка дошла бы до извлечения области действия, не
// разрешилась бы там и вернулась отказом в правах — утверждением о доступе на
// вопрос, который никогда не был корректным.
func TestQuotaHandler_ListRefusesAnUnanswerableQuestionByName(t *testing.T) {
	h, rows, limits := quotaFixture(t)

	_, err := h.List(context.Background(), &computev1.ListQuotasRequest{})
	require.Error(t, err)
...
//
// Пустой набор здесь читался бы арендатором как «пределов нет» — ровно обратное
// действительности: место по-прежнему занимает триггер, и создание будет
// отвергнуто.
func TestQuotaHandler_ListWithNoBandRefusesInsteadOfClaimingNoQuotas(t *testing.T) {
	...
	_, err := h.List(context.Background(), &computev1.ListQuotasRequest{ProjectId: "prj-mine"})
	require.Error(t, err)

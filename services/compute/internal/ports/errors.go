...
type ResolvedLimit struct {
	...
	// двухчастный токен родительского типа у вложенного вида. Приезжает ОТВЕТОМ
	// владельца величин и не выводится из формы токена: строка, заведённая под
	// неверным носителем, не отказывает громко — её потребление просто не
	// наполняется никогда.
	Carrier       string
	SourceScope   string
	SourceScopeID string

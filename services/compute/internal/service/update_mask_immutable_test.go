...
func TestUpdateMask_ImmutableFieldMessage(t *testing.T) {
	cases := []struct {
		...
		// Instance's redesigned immutable convention uses camelCase field names
		// ("zoneId is immutable after Instance.Create") and a Reinstall-only message
		// for bootSource — covered by TestInstance_COMP_1_26/27 in instance_test.go
		// (this legacy table uses snake_case + generic tail, still valid for
		// Disk/Image/Snapshot which keep the old convention).

...
var QuotaCeilingCatalog = quotaceiling.Catalog[QuotaCeilingsConfig]{
	quotaCeilingKnob("INSTANCE", "compute.instance",
		"сколько машин в проекте. Машина — самый дорогой ресурс платформы: она держит процессор, память и диск установки одновременно",
		func(c QuotaCeilingsConfig) *int64 { return c.Instance }),
	quotaCeilingKnob("PLACEMENT_GROUP", "compute.placementGroup",
		"сколько групп размещения в проекте. Каждая сужает выбор хоста, поэтому их число влияет на плотность укладки всей установки",
		func(c QuotaCeilingsConfig) *int64 { return c.PlacementGroup }),
	quotaCeilingKnob("GUEST_ACCESS_KEY", "compute.guestAccessKey",
		"сколько ключей доступа к гостям в проекте. Ключ — путь входа, и накопление ключей ничем не ограничено без этого потолка",
		func(c QuotaCeilingsConfig) *int64 { return c.GuestAccessKey }),
}

// QuotaCeilingsConfig — величины посадки.
type QuotaCeilingsConfig struct {
	// Instance — потолок вида `compute.instance`.
	// ENV `KACHO_COMPUTE_QUOTA_CEILING_INSTANCE`.
	Instance *int64 `envconfig:"KACHO_COMPUTE_QUOTA_CEILING_INSTANCE"`
	// PlacementGroup — потолок вида `compute.placementGroup`.
	// ENV `KACHO_COMPUTE_QUOTA_CEILING_PLACEMENT_GROUP`.
	PlacementGroup *int64 `envconfig:"KACHO_COMPUTE_QUOTA_CEILING_PLACEMENT_GROUP"`
	// GuestAccessKey — потолок вида `compute.guestAccessKey`.
	// ENV `KACHO_COMPUTE_QUOTA_CEILING_GUEST_ACCESS_KEY`.
	GuestAccessKey *int64 `envconfig:"KACHO_COMPUTE_QUOTA_CEILING_GUEST_ACCESS_KEY"`
}

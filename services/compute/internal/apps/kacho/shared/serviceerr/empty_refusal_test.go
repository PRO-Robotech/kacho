...
// дальше. В журнале это неотличимо от потери сообщения, а тон отказа — часть
// контракта: он объявлен и меняется только осознанно.
//
...
func TestRefusalNeverReachesTheClientWithAnEmptyMessage(t *testing.T) {
	...
	full := MapRepoErr(fmt.Errorf("%w: project prj-1 has reached its limit of 4 things", ErrQuotaExceeded))

...
const quotaDetailExceeded = `{"carrier_type": "project", "carrier_id": "prj-1", ` +
	`"kind": "compute.instance", "limit": 4, "used": 4}`

func TestQuotaRefusalCarriesTheProducerAmounts(t *testing.T) {
	const producer = "project prj-1 has reached its limit of 4 compute.instance"
	err := quotadetail.Attach(
		fmt.Errorf("%w: %s", ErrQuotaExceeded, producer), quotaDetailExceeded)
	...
	assert.Equal(t, "compute.kacho.cloud", info.GetDomain())
...
func TestQuotaRefusalNotProvisionedCarriesTheCarrierWithoutAmounts(t *testing.T) {
	const detail = `{"carrier_type": "project", "carrier_id": "prj-1", "kind": "compute.machineimage"}`
	err := quotadetail.Attach(
		fmt.Errorf("%w: project prj-1 has no ceiling stated for compute.machineimage",
			ErrQuotaNotProvisioned), detail)
...
func TestQuotaRefusalWithoutDetailStaysAValidRefusal(t *testing.T) {
	const producer = "project prj-1 has reached its limit of 4 compute.instance"
	err := fmt.Errorf("%w: %s", ErrQuotaExceeded, producer)
...
func TestQuotaDetailKeepsTheSentinelRecognisable(t *testing.T) {
	err := quotadetail.Attach(
		fmt.Errorf("%w: project prj-1 has reached its limit of 4 compute.instance",
			ErrQuotaExceeded), quotaDetailExceeded)

...
const (
	...
	// KindInstance — единственный вид предмета журнала.
	//
...
func Journal() subscription.Journal {
	return subscription.Journal{
		...
		Storage: subscription.Storage{
			...
			// (предикат — `git grep -in 'DELETE FROM compute_outbox'` по
			...
			// прямо. Заведётся чистка — эта величина обязана поменяться ВМЕСТЕ
			// с ней: обещание удержания живёт ровно столько, сколько живёт его
			// основание.

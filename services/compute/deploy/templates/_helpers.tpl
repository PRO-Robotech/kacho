...
{{/*
kacho-compute.authMode — посадка безопасности, ОДНИМ адресом.
...
*/}}
{{- define "kacho-compute.authMode" -}}
{{- $vals := .Values | toYaml | fromYaml -}}
{{- $canonRaw := .Values.authMode -}}
{{- $legacyRaw := "" -}}
{{- $cur := $vals -}}
{{- if kindIs "map" $cur }}{{ $cur = dig "auth" (dict) $cur }}{{ else }}{{ $cur = dict }}{{ end -}}
...
{{- fail (printf "authMode=%q и прежний адрес auth.mode=%q заданы одновременно и различаются: какой из двух адресов задаёт посадку — решает оператор, а не шаблон. Оставьте один, канон — authMode" $canon $legacy) -}}
{{- end -}}
{{- if ne $canon "" }}{{ $canon }}{{ else }}{{ $legacy }}{{ end -}}
{{- end -}}

{{/*
compute.trustDomain — ДОМЕН ДОВЕРИЯ установки, ОДНИМ написанием.

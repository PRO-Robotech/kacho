...
func (b DiskTypeBinding) Validate() error {
	...
	// Номер ревизии НАЗНАЧАЕТ РЕГИСТРАЦИЯ, а не вызывающий: он обязан быть
	// следующим В МОМЕНТ ЗАПИСИ, поэтому выбранный заранее устареет между
	// чтением и вставкой. До записи номера ещё нет — ноль здесь означает «не
	// назначен», и это законное состояние на входе.
	//
	// Требование положительного номера ДО записи делало глагол неисполнимым:
	// вызывающий не может назвать величину, которую сам же не вправе выбирать.
	// Отрицательный номер при этом остаётся ошибкой — он не «не назначен», а
	// назван неверно.
	if b.Revision < 0 {
		return fmt.Errorf("disk_type_binding revision must be positive, got %d", b.Revision)
	}
	// СОСТОЯНИЕ ТОЖЕ НАЗНАЧАЕТ РЕГИСТРАЦИЯ — по той же причине, что и номер
	// ревизии: зарегистрированная ревизия действует ПО ОПРЕДЕЛЕНИЮ (регистрация
	// вставляет 'ACTIVE' и тем же стейтментом вытесняет прежнюю). Вытеснение —
	// исход записи, а не выбор вызывающего, поэтому пустое здесь означает «ещё
	// не назначено» и является законным входом.
	//
	// Требование известного состояния ДО записи делало глагол неисполнимым:
	// вызывающий обязан был назвать величину, которую регистрация всё равно
	// назначает сама, а хранилище отвергает всё, кроме действующего.
	// Непустое остаётся под закрытым словарём — назвать состояние неверно
	// по-прежнему ошибка.

...
func (r ExecRunner) Run(ctx context.Context, args ...string) ([]byte, []byte, int, error) {
	...
	// #nosec G204 -- ни бинарь, ни аргументы не приходят из запроса: путь к
	// инструменту задаёт посадка (страж старта проверяет его наличие), остальное —
	// имена объектов, выведенные из префикса установки и неизменяемого id ресурса.
	// Пользовательской строки в этом наборе нет by construction.

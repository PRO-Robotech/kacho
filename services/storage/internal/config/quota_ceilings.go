...
var QuotaCeilingCatalog = quotaceiling.Catalog[QuotaCeilingsConfig]{
	quotaCeilingKnob("VOLUME", "storage.volumes",
		"сколько томов в проекте. Том занимает место на хранилище установки, и это место не возвращается до удаления",
		func(c QuotaCeilingsConfig) *int64 { return c.Volume }),
	quotaCeilingKnob("SNAPSHOT", "storage.snapshots",
		"сколько снимков в проекте. Снимок дешевле тома при создании и дорожает со временем, поэтому накопление заметно не сразу",
		func(c QuotaCeilingsConfig) *int64 { return c.Snapshot }),
	quotaCeilingKnob("IMAGE", "storage.images",
		"сколько образов в проекте. Образ читается при каждом запуске машины, поэтому его цена не только в занятом месте",
		func(c QuotaCeilingsConfig) *int64 { return c.Image }),
}

// QuotaCeilingsConfig — величины посадки.
type QuotaCeilingsConfig struct {
	// Volume — потолок вида `storage.volumes`.
	// ENV `KACHO_STORAGE_QUOTA_CEILING_VOLUME`.
	Volume *int64 `envconfig:"KACHO_STORAGE_QUOTA_CEILING_VOLUME"`
	// Snapshot — потолок вида `storage.snapshots`.
	// ENV `KACHO_STORAGE_QUOTA_CEILING_SNAPSHOT`.
	Snapshot *int64 `envconfig:"KACHO_STORAGE_QUOTA_CEILING_SNAPSHOT"`
	// Image — потолок вида `storage.images`.
	// ENV `KACHO_STORAGE_QUOTA_CEILING_IMAGE`.
	Image *int64 `envconfig:"KACHO_STORAGE_QUOTA_CEILING_IMAGE"`
}

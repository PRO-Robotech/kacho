...
func (c Config) Validate() error {
	...
	// В БОЕВОМ режиме плоскость данных обязана быть объявлена, и это отдельное
	// требование, а не следствие блока ниже.
	//
	// Блок ниже читает вид и порождает требования, ЕСЛИ он задан. Пустой вид не
	// порождал ни одного — то есть боевое развёртывание без плоскости данных
	// поднималось молча. Что оно означает: сверщик не запускается, ничто не
	// переводит ресурс из намерения в пригодность, КАЖДЫЙ том остаётся
	// создаваемым навсегда, а сервис при этом рапортует о своём здоровье и
	// проходит проверку готовности. Это ровно тот класс, который боевой страж и
	// заведён ловить: посадка, в которой контроль присутствует и не отказывает
	// никогда, потому что ему нечего рассматривать.
	//
	// Для НЕбоевых посадок пустой вид законен, и отдельного условия на это не
	// нужно: функция возвращается раньше — всё, что ниже разбора режима,
	// исполняется только в боевом.
	if c.BlockBackendKind == "" {
		problems = append(problems,
			"block backend kind required in production mode: set KACHO_STORAGE_BLOCK_BACKEND_KIND "+
				"— without a data plane the reconciler never starts, nothing moves a resource from CREATING "+
				"to READY, and every volume stays pending forever while the service reports itself healthy")
	}
	if c.BlockBackendKind != "" {

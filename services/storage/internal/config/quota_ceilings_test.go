...
// loadCeilingCfg — разбор посадки тем же путём, каким его делает служба, с
// переменными, ограниченными текущим тестом.
func loadCeilingCfg(t *testing.T, env map[string]string) Config {
	t.Helper()
	for _, k := range QuotaCeilingCatalog {
		t.Setenv(k.Env, "")
		_ = os.Unsetenv(k.Env)
	}
	// Обязательная ручка разбора, к предмету пробы не относящаяся: без неё
	// разбор отказывает раньше стража величин, и проба судила бы не то.
	t.Setenv("KACHO_STORAGE_DB_PASSWORD", "probe")
	for k, v := range env {
		t.Setenv(k, v)
	}
	cfg, err := Load()
...
func fullCeilingEnv() map[string]string {
	out := make(map[string]string, len(QuotaCeilingCatalog))
	for i, k := range QuotaCeilingCatalog {
		if i == 0 {
			out[k.Env] = "0"
			continue
		}
...
func TestCeilingEnvVarsArmTheFields(t *testing.T) {
	cfg := loadCeilingCfg(t, fullCeilingEnv())
	stated := QuotaCeilingCatalog.Stated(cfg.QuotaCeilings)
	if len(stated) != len(QuotaCeilingCatalog) {
		t.Fatalf("каждая переменная каталога обязана доехать до своего поля: %v", stated)
	}
	zeroKind := QuotaCeilingCatalog[0].Kind
...
func TestEmptyCeilingSetIsLawful(t *testing.T) {
	cfg := loadCeilingCfg(t, map[string]string{})
	if got := QuotaCeilingCatalog.Stated(cfg.QuotaCeilings); len(got) != 0 {
...
func TestPartialCeilingSetIsRefusedNamingEveryUnstatedKind(t *testing.T) {
	...
	if got := QuotaCeilingCatalog.Stated(cfg.QuotaCeilings); len(got) != 1 {
...
func TestNegativeCeilingIsRefused(t *testing.T) {
	env := fullCeilingEnv()
	env[QuotaCeilingCatalog[0].Env] = "-1"
	cfg := loadCeilingCfg(t, env)
	err := cfg.ValidateQuotaCeilings()
	if err == nil {
...
func TestQuotaCeilingCatalogShapeIsSound(t *testing.T) {
	...
	for _, k := range QuotaCeilingCatalog {
		if !strings.HasPrefix(k.Kind, "storage.") {

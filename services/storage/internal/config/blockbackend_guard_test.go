...
func TestBootGuard_NoBackend_RaisesNoBackendRequirements(t *testing.T) {
	t.Parallel()
	// Незаданный вид бэкенда не порождает требований: предикат активности стража
	// — тот же, что читает проводка.
	c := baseProduction()
	if err := c.Validate(); err != nil {
		t.Fatalf("боевая конфигурация без плоскости данных обязана подниматься: %v", err)
	}
}

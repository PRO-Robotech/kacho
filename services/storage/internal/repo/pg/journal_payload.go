...
//
// # ГДЕ СТОРОНЫ РАСХОДЯТСЯ ЗАКОННО — и почему это не повод их «выровнять»
//
// На томе с НЕСКОЛЬКИМИ привязками конверт несёт их ВСЕ, а `scanVolume` — одну:
// запрос чтения соединяет привязки без агрегации, поэтому `Get` берёт первую
// строку соединения, а `List` кладёт том в страницу по разу на каждую. Это дефект
// ЧТЕНИЯ, заведённый задачей продукта #1559, а не расхождение сборки: у чтения там
// нет и порядка, то есть детерминированной стороны сравнения не существует вовсе.
//
// Поэтому проба равенства работает на ОДНОЙ привязке — там обе стороны
// определены, — а подгонять журнал под неполный ответ чтения нельзя: это
// закрепило бы дефект в третьем месте.
//
...
//
// Это не одобрение того, что путь чтения их теряет: предмет заведён задачей
// продукта #1557. Когда он закроется, проба равенства покраснеет и потребует
// править ОБЕ стороны одним изменением — что и требуется.

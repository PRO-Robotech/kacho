...
func TestListOperations_ReturnsOnlyCallerOwnRows(t *testing.T) {
	...
	uc := image.New(&repomock.ImageReader{}, &repomock.ImageWriter{}, &repomock.PeerClient{}, &repomock.PeerClient{}, ops, serviceerr.ToStatus)
	resID := ids.NewID(domain.PrefixImage)
	...
	got, _, err := uc.ListOperations(operations.WithPrincipal(bg, me), resID, image.Pagination{})
	if err != nil {
...
func TestListOperations_UnidentifiedCallerGetsNoRows(t *testing.T) {
	...
	got, _, err := uc.ListOperations(context.Background(), resID, image.Pagination{})
	if err != nil {
		t.Fatalf("ListOperations: %v", err)
	}
	if len(got) != 0 {

...
//
// # Почему предпосылка пережила появление плоскости данных
//
// Разрешитель судит по НАШЕЙ базе: ресурс есть — работа доведена. Это верно ровно
// до тех пор, пока предмет операции — запись строки, а не провижининг у бэкенда.
// Именно поэтому обращение к бэкенду вынесено ИЗ функции операции: оно идёт в
// сверщике, а операция фиксирует намерение и завершается.
//
// Перенеси провижининг обратно в функцию операции — и разрешитель начнёт врать
// самым дорогим способом: длинное выделение упрётся в потолок исполнителя, функцию
// убьют, а через окно ожидания разрешитель прочитает нашу строку, увидит ресурс и
// объявит клиенту «готово» при ОТСУТСТВУЮЩЕМ объекте. Ошибка при этом не покраснеет
// нигде: и строка на месте, и операция завершена успехом.
//
// Держится это не обещанием: гейт TestBackendPortIsReachableOnlyFromTheReconciler
// (services/storage/internal/check) роняет сборку, как только порт бэкенда
// оказывается достижим из слоя, исполняющего операции.
//
...
//
// Отдельно про Create с появлением плоскости данных: ресурс, найденный в базе,
// разрешает операцию успехом, даже если он в состоянии СОЗДАВАЕМЫЙ. Это не
// послабление, а точное следование предмету: клиент просил завести том, и том
// заведён; о том, доехал ли объект до бэкенда, отвечает СТАТУС ресурса, который
// клиент и обязан читать дальше.

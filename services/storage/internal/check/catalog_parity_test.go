...
// knownCatalogDivergences — none. storage's map mirrors the catalog exactly: every
// object-scoped RPC asks for the same relation on the same object type the gateway
// asks for. (This is worth stating because storage's LIST FILTER used to be wider
// than the relation the catalog gates Get on — see
// internal/authzfilter/filter_get_parity_test.go. The map was never the problem.)

...
func PermissionMap() authz.RPCMap {
	return authz.RPCMap{
		...
		// Про список доверенных форвардеров (cmd/storage/serve.go) — он ПУСТ, и
		// это НЕ значит «не принимаем ни от кого». grpcsrv.principalIsTrusted
		// сверяет сертификат со списком ТОЛЬКО когда список непуст; на пустом он
		// возвращает true, то есть переданную личность принимает ЛЮБОЙ проверенный
		// пир. Пустой список — это «не сужаем», а не «запрещаем».
		//
		// Формулировка важна: обратное утверждение стояло здесь ровно один коммит
		// и было бы прочитано как гарантия. Реально сужает круг обязательный mTLS
		// (внутрь периметра без клиентского сертификата не попасть), а не длина
		// этого списка; заполнить его — отдельное ужесточение, а не описание
		// текущего состояния.

...
func passLockName(kind Kind) string { return "kacho.storage.reconcile." + string(kind) }

// TryClaimKindPass берёт проход по ОДНОМУ виду ресурса на одну реплику.
//
// # Что это чинит
//
// Выборка расхождений детерминирована (`WHERE state <> observed_state ORDER BY …
// LIMIT`), поэтому без развода N реплик берут ТЕ ЖЕ строки и зовут по каждой
// плоскость данных: N попыток создать один том, N снятий одного объекта. База при
// этом не портится — записи исхода условны, — но проигравший получал ноль
// затронутых строк, а код считал это отказом: N−1 реплик писали ошибку на каждом
// УСПЕШНОМ проходе, и наблюдаемость деградировала ровно там, где она нужна.
//
// # Почему замок прохода, а не клейм строки
//
// Клейм строки (реплики берут непересекающиеся партии и работают одновременно)
// сильнее, но требует колонки-аренды, то есть миграции. Взять её сейчас нельзя:
// нумерация миграций в этом дереве идёт по номеру задачи, а задача #703 младше
// уже применённой 715001 — goose отказывается стартовать, встретив пропущенную
// миграцию ниже текущей версии. Предмет заведён отдельно.
//
// Замок при этом не отнимает параллелизма, которого сегодня НЕТ: проход и так
// разбирает партию последовательно, строку за строкой, а реплики не делили
// работу — они её повторяли. Разбиение по виду ресурса оставляет три прохода,
// идущих одновременно на весь кластер.
func (s *Store) TryClaimKindPass(ctx context.Context, kind Kind) (singlepass.Release, bool, error) {
	if _, err := kind.table(); err != nil {
		return nil, false, err
	}
	return singlepass.TryAcquire(ctx, s.pool, passLockName(kind))
}
...
//
// Возвращает, ПРИМЕНИЛОСЬ ли обновление. Ноль затронутых строк — штатный исход, а
// не отказ: он означает, что строку увёл другой путь, и переписывать его поверх
// нельзя. Ошибкой это было раньше, и цена была прямая — на многорепличном стенде
// проигравший гонку писал отказ на каждом успешном проходе, то есть ошибок в
// журнале становилось тем больше, чем исправнее работал сверщик.
func (s *Store) Confirm(ctx context.Context, kind Kind, id string, obs blockbackend.Observed) (bool, error) {
	table, err := kind.table()
	if err != nil {
		return false, err
	}
	...
	// Нулевой исход условного обновления НЕ засчитывается подтверждением — иначе
	// «ресурс подтверждён» и «ресурс остался в намерении» выглядели бы одинаково,
	// а разошлись бы только в жалобе арендатора. Но и отказом он не является:
	// условие не выполнилось потому, что строку правомерно увёл другой путь.
	// Вызывающий получает различимый исход и считает его отдельно.
...
// Здесь была `StaleBefore` — граница, за которой наблюдение считается
// устаревшим (#923). Её объявление описывало защиту от класса «пропажа объекта
// под ГОТОВЫМ состоянием»: расхождения нет, значит частичный индекс такую
// строку не поднимет, и без срока годности она не нашлась бы никогда.
//
// Защиты не существовало: вызывающих у функции было НОЛЬ, проб — ноль. То есть
// объявление описывало намерение, а читалось как механизм — и следующий, кто
// стал бы искать, почему пропажа не находится, нашёл бы «эту защиту» и закрыл
// вопрос.
//
// Снята вместе со своим текстом, а не оставлена «на будущее»: срок годности
// наблюдения заводится ВМЕСТЕ с тем, кто по нему обходит, — иначе повторится
// то же самое.

...
// TestStateIsProducedForVolumeAndNamedAbsentElsewhere — состояние есть у ОДНОГО
// вида, и у остальных отсутствие НАЗВАНО.
//
// Проба заменяет прежнюю, утверждавшую отсутствие у всех трёх видов. Замена, а не
// ослабление: прежняя закрепляла свойство журнала «состояния нет нигде», и её
// предмет исчез — у тома состояние появилось. Ослабить её (снять том из обхода)
// значило бы вывести из наблюдения ровно тот вид, ради которого работа делалась.
//
// # Почему у снимка и образа состояния по-прежнему нет
//
// Причина НЕ та, что была у тома, и её надо назвать точно, иначе следующий
// прочтёт «выводится через таблицы» как общий приговор. У снимка и образа
// `used_by` — перечень ТОМОВ, засеянных этим источником, а мутация ТОМА события
// снимку и образу не эмитирует ни одна. Отдай мы им состояние, оно устаревало бы
// молча: подписчик держал бы снимок с пустым `used_by` при живых детях и получил
// бы отказ удаления, которого его собственное состояние не объясняет.
//
// У тома оба перекрёстных источника (`volume_attachments`) СВОЙ триггер имеют,
// поэтому его состояние свежо по построению, а не по удаче.
func TestStateIsProducedForVolumeAndNamedAbsentElsewhere(t *testing.T) {
	...
	for kind := range kinds {
		for change := range changes {
			// Нагрузка НЕСЁТ конверт у каждой пары: иначе «состояния нет» у
			// снимка и образа выполнялось бы из-за бедной нагрузки, а не из-за
			// решения о виде, и проба не отличала бы одно от другого.
			got, absence, err := state(subscription.Row{
				Kind:    kind,
				ID:      "probe",
				Change:  change,
			...
			wantState := kind == subscriptionjournal.JournalWordVolume && change != "DELETED"
			switch {
			...
			case !wantState && got != nil:
				t.Errorf("вид %q род %q: отдано состояние. У снимка и образа `used_by` "+
					"собирается из ТОМОВ, а мутация тома им события не эмитирует — "+
					"состояние устаревало бы молча. У снятия предмета уже нет", kind, change)
			}
			if got != nil {
				produced++
				continue

...
// (`<Resource>.Get` → `v_get` на самом объекте; каталог — авторитет, его же
...
// задаваться он обязан один раз. Замок на это тождество — не литерал здесь, а
// filter_get_parity_test.go: он читает каталог и краснеет, если отношение чтения
// в каталоге разойдётся с этим набором.
//
...
// read-path'ом). Союз сняли в пользу `viewer`, потому что `viewer` тогда и был
// отношением чтения в каталоге.
//
// Затем object-self RPC storage перевели с ЯРУСА на ГЛАГОЛ, и `viewer` перестал
// быть отношением чтения: ярусный кортеж, который реконсайлер пишет на каждый
// материализованный объект, выводится из класса глаголов правила, поэтому выдача
// с одним лишь `create` резолвила `editor ⇒ viewer` — и страница показывала то,
// что `Get` уже отказывался отдавать. Набор равен `v_get` ровно по той же
// причине, по какой раньше равнялся `viewer`: это то, что спрашивает `Get`.
//

...
// ceil(1000/100) = 10 round-trip'ов на relation, а relations две и они
// последовательны по построению (`v_list` спрашивается только у отказанных
// `viewer`) — до 20 подряд, каждый под своим per-call дедлайном. Стоило латентности
// пира перешагнуть per-call дедлайн, как ПЕРВЫЙ же батч ронял весь позитивный List
// в `Unavailable`; под параллельной нагрузкой это и происходило, на холостом стенде — нет.
//
...
// visibilityRelations — отношения, союз которых и есть «видимость» в List/Get:
// `viewer` (read-tier: direct usersets ∪ editor ∪ admin) и `v_list` (object-only
// selector-грант «видеть в списке без содержимого»). Порядок значим только для
// стоимости (`viewer` покрывает подавляющее большинство, `v_list` добирает остаток).
...
// дедлайн делился на ЧИСЛО ПОСЛЕДОВАТЕЛЬНЫХ ХОПОВ — до 20 на предельной странице.
...
// 10-20 × 500ms = 5..10s. Допуск на ОДИН хоп вырос вдвое, а агрегатный worst-case
// УПАЛ и стал ОГРАНИЧЕННЫМ (бюджет 6s).
...
// relation; батчи внутри relation идут волнами по Parallelism; relations
// последовательны по построению (`v_list` только для отказанных `viewer`), и в
// худшем случае ВСЯ страница доезжает до второй relation.

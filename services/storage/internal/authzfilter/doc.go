...
// # Предикат видимости
//
// Видимость — РОВНО `v_get`: то же отношение, на котором permission-catalog
// энфорсит per-RPC Check на `Get` (и которое зеркалит
// internal/check/permission_map.go), поэтому «видно в списке» ⟺ «Get разрешён»
// (read==enforce).
//
...
// вызывающий узнаёт идентификатор недоступного ему ресурса. Так было с прежним
// союзом `viewer ∪ v_list`, и так же было бы с ярусом: ярусный кортеж пишется на
// каждый материализованный объект и выводится из класса глаголов выдачи, поэтому
// «вправе создавать» читалось бы как «вправе видеть содержимое». Подробности и
// замок — filter.go (visibilityRelations) и filter_get_parity_test.go.
//
// # Дисциплина фильтра
//

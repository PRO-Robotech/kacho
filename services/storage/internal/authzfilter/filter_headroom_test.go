...
// kacho-iam. Значение выше прежнего per-call дедлайна соседей (500ms) намеренно:
// именно этот режим ронял предельную страницу в `503 list filter:
// AuthorizeService.BatchCheck DeadlineExceeded` под параллельной нагрузкой, тогда
// как на холостом стенде тот же путь проходил. Регрессия — не логическая, а по
// ЗАПАСУ латентности, поэтому тест лочит именно запас, а не форму запроса.
const loadedPeerLatency = 600 * time.Millisecond

// TestFGAFilter_MaxPageWithinBudget_UnderLoadedPeerLatency — REGRESSION-lock.
//
// Страница ДОКУМЕНТИРОВАННОГО максимума (validate.MaxPageSize = 1000 — часть
...
func TestFGAFilter_MaxPageWithinBudget_UnderLoadedPeerLatency(t *testing.T) {
	...
	assert.Greater(t, maxInFlight, 1,
		"batches of one relation must not wait for each other — that is the regression this locks")
...
// Первая ошибка пробрасывается как есть (fail-closed), соседние батчи отменяются —
// а не «дорабатывают» страницу, которую всё равно не отдадут.
func TestFGAFilter_FirstErrorWinsAndCancelsSiblings(t *testing.T) {
	...
	ids := make([]string, 0, pageSize)
	for i := 0; i < pageSize; i++ {
		ids = append(ids, fmt.Sprintf("vol%017d", i))
	}
...
// Одиночный батч (страница ≤100) не платит за пул и не меняет наблюдаемое поведение.
...
// Бюджет ПРИНАДЛЕЖИТ ЗАПРОСУ, а не каждой партии: он выводится из контракта
// (максимум страницы ÷ предел батча ÷ параллелизм × per-call), поэтому «сумма
// worst-case ожиданий помещается в бюджет» держится ПО ПОСТРОЕНИЮ, а не по надежде
// на когерентный config.
func TestDeriveOverallTimeout_IsDerivedFromContractNotHandTuned(t *testing.T) {
	...
	// Нулевой/отрицательный parallelism не обнуляет бюджет (деление на ноль /
	// мгновенный дедлайн), а откатывается к дефолту.
	assert.Equal(t, deriveOverallTimeout(perCall, defaultParallelism), deriveOverallTimeout(perCall, 0))
}

...
-- Drop fga_register_outbox_pending_idx — the `(id) WHERE sent_at IS NULL` index created together with the
...
--
--   SELECT t.id FROM kacho_storage.fga_register_outbox t
...
--
--   Index Scan (fga_register_outbox_pending_idx) -> Sort -> LockRows -> Limit
--
...
CREATE INDEX IF NOT EXISTS fga_register_outbox_pending_idx

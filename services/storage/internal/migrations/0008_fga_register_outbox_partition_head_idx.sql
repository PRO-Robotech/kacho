...
--
-- WHY: kacho_storage.fga_register_outbox carries BOTH fga.register AND
-- fga.unregister of the SAME resource, and iam's materialisation is only PARTIALLY
-- versioned. source_version-LWW (`resource_mirror` UPSERT `WHERE
-- resource_mirror.source_version < EXCLUDED.source_version`) guards the
-- ON-CONFLICT-UPDATE branch ONLY; unregister is a HARD DELETE that leaves no
-- tombstone. So a reordered STALE register finds no row to compare against, takes the
-- INSERT branch and RESURRECTS the mirror row of a DELETED resource — permanently,
-- because iam's reconciler is level-triggered off the mirror and re-materialises the
-- owner-tuple forever. There is no self-healing.
--
...
-- a fresh successor (attempt_count = 0) even at ApplyConcurrency = 1 (storage's
-- default) — and lands in a LATER batch. PartitionColumn closes the class at CLAIM
-- level: a row is never claimed while a DELIVERABLE (sent_at IS NULL AND
...
--
-- WHY resource_id is the right partition key: emitFGARegister writes ONE row per FGA
-- object and fills resource_id from that row's tuple `Object` ("<kind>:<id>"); the id
-- half is globally unique by construction (core rule #15), so "same partition" is
-- exactly "same iam-mirror object".
--
...
-- index (migration 0006). The pending backlog is small on a healthy stand; on a
...
CREATE INDEX IF NOT EXISTS fga_register_outbox_partition_head_idx
    ON kacho_storage.fga_register_outbox (resource_id, id)
...
DROP INDEX IF EXISTS kacho_storage.fga_register_outbox_partition_head_idx;

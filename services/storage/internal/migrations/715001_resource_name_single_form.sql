...
DECLARE
    t    text;
    tbl  text[] := ARRAY['volumes', 'snapshots', 'images'];
    form text := '^[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])?$';
BEGIN
    FOREACH t IN ARRAY tbl LOOP
        EXECUTE format('UPDATE kacho_storage.%I SET name = id WHERE name IS NULL OR name !~ %L', t, form);
        EXECUTE format('ALTER TABLE kacho_storage.%I DROP CONSTRAINT IF EXISTS %I', t, t || '_name_check');
        EXECUTE format('ALTER TABLE kacho_storage.%I ADD CONSTRAINT %I CHECK (name ~ %L)',
                       t, t || '_name_check', form);
        -- Частичный индекс `WHERE name <> ''` жил ровно ради пустых имён.
        EXECUTE format('DROP INDEX IF EXISTS kacho_storage.%I', t || '_name_uniq');
        EXECUTE format('CREATE UNIQUE INDEX %I ON kacho_storage.%I (project_id, name)',
                       t || '_name_uniq', t);
    END LOOP;
END $$;

-- storage_backends и disk_types в перечень НЕ входят намеренно: это
-- административные ресурсы внутреннего листенера, их имя — не косметическая
-- метка арендатора, а установочная величина, и уникальность у них устроена
-- иначе (install-global). Приведение их к форме арендатора — отдельное решение
-- с отдельным предметом.
...
DECLARE
    t      text;
    tbl    text[] := ARRAY['volumes', 'snapshots', 'images'];
    legacy text := '^[a-z]([-a-z0-9]{0,61}[a-z0-9])?$';
BEGIN
    FOREACH t IN ARRAY tbl LOOP
        EXECUTE format('ALTER TABLE kacho_storage.%I DROP CONSTRAINT IF EXISTS %I', t, t || '_name_check');
        EXECUTE format('ALTER TABLE kacho_storage.%I ADD CONSTRAINT %I CHECK (name = '''' OR name ~ %L)',
                       t, t || '_name_check', legacy);
        EXECUTE format('DROP INDEX IF EXISTS kacho_storage.%I', t || '_name_uniq');
        EXECUTE format('CREATE UNIQUE INDEX %I ON kacho_storage.%I (project_id, name) WHERE name <> ''''',
                       t || '_name_uniq', t);
    END LOOP;
END $$;

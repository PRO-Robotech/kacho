...
- `DiskType`: `Get`/`List` sync (public); admin-регистрация (`InternalDiskTypeService`,
  ...
  без LRO).
- `InternalStorageBackendService` / `InternalDiskTypeBindingService` / `Image.Register` —
  синхронны по той же причине и **gRPC-only**: REST-привязок у них нет, потребителя вне
  кластера сегодня тоже, а объявленный путь без потребителя — поверхность, за которую
  никто не отвечает.

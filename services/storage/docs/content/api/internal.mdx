...
:::caution Путь делится с публичным чтением — различает **пара** «метод, путь»
В отличие от реестра бэкендов и ревизий привязки, у этого сервиса REST-привязки **есть** —
и мутации каталога стоят на **том же** пути, что публичное чтение
(`GET /storage/v1/diskTypes[/{id}]`), отличаясь от него только HTTP-методом.

Поэтому изоляция на крае ключуется на **паре** «метод, путь», а не на пути: классификатор,
смотрящий на один путь, эти привязки не различает и увёл бы административные мутации на
публичный мультиплексор. Источник классификации — принадлежность метода
`Internal*`-сервису, поэтому новый метод изолируется автоматически, а не по форме пути.
:::

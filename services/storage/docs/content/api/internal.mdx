...
:::info Почему реестр бэкендов и ревизии привязки — gRPC-only
У `InternalStorageBackendService` и `InternalDiskTypeBindingService` не объявлено ни одной
REST-привязки: их адресат — администратор услуги, а потребителя REST у них сегодня нет.
Объявленный путь, который никто не резолвит, — это поверхность, за которую никто не
отвечает: она переживает причину, по которой заводилась. Появится потребитель — привязка
добавится вместе с ним и только на внутреннем муксе.
:::
...
<ApiOperation method="gRPC" endpoint="InternalStorageBackendService/Create">
...
<ApiOperation method="gRPC" endpoint="InternalStorageBackendService/Update">
...
<ApiOperation method="gRPC" endpoint="InternalStorageBackendService/Delete">
...
### Create · Get · List · Supersede

<ApiOperation method="gRPC" endpoint="InternalDiskTypeBindingService/Create">
...
<ApiOperation method="gRPC" endpoint="InternalDiskTypeBindingService/Get">
...
<ApiOperation method="gRPC" endpoint="InternalDiskTypeBindingService/List">
...
<ApiOperation method="gRPC" endpoint="InternalDiskTypeBindingService/Supersede">

Снимает действующую ревизию с действия **без замены**: класс перестаёт предлагаться в этой
зоне, новые ресурсы на нём не создаются, существующие продолжают жить под своей ревизией.

Это единственный глагол ревизии, и он меняет ровно одно — полосу действия. Содержание
строки остаётся прежним, поэтому ссылка ресурса на неё продолжает значить то же, что
значила в момент создания.

Движение одностороннее: `SUPERSEDED` не возвращается в `ACTIVE`. Вернуть класс в зону —
значит завести **новую** ревизию: тогда у ресурсов, созданных под прежней, остаётся своя
политика, а не воскресшая чужая. Повтор на уже вытесненной ревизии идемпотентен.
...
<table>
  <thead><tr><th>Поле</th><th>Тип</th><th>Описание</th></tr></thead>
  <tbody>
    <tr><td><code>projectId</code></td><td><code>{TYPES.string}</code></td><td>обязателен; проверяется через kacho-iam</td></tr>
...
<ApiOperation method="gRPC" endpoint="InternalDiskTypeService/SetLifecycle">

...
CASES.append(Case(
    ...
    steps=[
        ...
        Step(name="get", method="GET", path=f"{VOL}/{{{{volumeId}}}}",
    ...
    steps=[
        Step(name="cr", method="POST", path=VOL, body=_vol_body("grow", size_bytes=_DEF_SIZE),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.volumeId", "volumeId")]),
        poll_operation_until_done(),
        Step(name="patch-grow", method="PATCH", path=f"{VOL}/{{{{volumeId}}}}",
    ...
    steps=[
        Step(name="cr", method="POST", path=VOL, body=_vol_body("upd", description="init", labels={"orig": "1"}),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.volumeId", "volumeId")]),
        poll_operation_until_done(),
        Step(name="patch", method="PATCH", path=f"{VOL}/{{{{volumeId}}}}",
    ...
    steps=[
        Step(name="cr", method="POST", path=VOL, body=_vol_body("lop"),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.volumeId", "volumeId")]),
        poll_operation_until_done(),
        Step(name="list-ops", method="GET", path=f"{VOL}/{{{{volumeId}}}}/operations?pageSize=10",
             test_script=[*assert_status(200),
    ...
    steps=[
        Step(name="cr", method="POST", path=VOL, body=_vol_body("life"),
             test_script=[*assert_status(200), *save_from_response("j.id", "opId"),
                          *save_from_response("j.metadata && j.metadata.volumeId", "volumeId")]),
        poll_operation_until_done(),
        Step(name="get-1", method="GET", path=f"{VOL}/{{{{volumeId}}}}",

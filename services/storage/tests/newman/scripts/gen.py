...
_INJECTED = {
    ...
    # СЕРИАЛИЗАТОР СТРОКИ — впрыскивается по той же причине, по какой его
    # впрыскивают iam и vpc: без него декларация НЕ МОЖЕТ исполнить #1181 даже
    # при желании автора, потому что вклейка в литерал остаётся единственной
    # доступной ей формой. Метку выше кейс обязан взять у производителя, а
    # положить её в порождаемый скрипт — сериализатором; впрыснув одно без
    # другого, набор получил бы предписание, исполнимое только нарушением.
    "js_str": js_str,
    "Step": Step,
    "Case": Case,

...
export const CODES = {
  ...
  failedPrecondition: {
    ...
    http: '400',
    when: 'Состояние не позволяет операцию: том не READY, том уже привязан, расхождение зоны / проекта / региона, имя устройства занято, свободных имён устройств нет, тип диска держат тома',
  },
  unavailable: {
    grpc: 'UNAVAILABLE',
    http: '503',

...
func checkHandlerIsWired(fset *token.FileSet, cmdDir string) ([]string, []string, int, error) {
	...
	for _, f := range files {
		ast.Inspect(f, func(n ast.Node) bool {
			...
			body := fmt.Sprintf(
				"NewRegistryHandler can be wired with a nil authorizer (%s) — that is "+
					"breakglass, and it turns every per-object List filter into a no-op", how)
			if how == howConditional && productionRefusalPinned(cmdDir) {
				...
				//
				// КООРДИНАТА СТОИТ ПОСЛЕДНЕЙ, И ЭТО НЕ ОФОРМЛЕНИЕ. `actions/setup-go`
				// регистрирует на всё задание сопоставитель вывода, у которого нет поля
				// `severity`, поэтому совпавшая строка становится аннотацией УРОВНЯ
				// ОТКАЗА — даже когда напечатавший её шаг зелён. Заметка в форме
				// `<файл>.go:<строка>:<столбец>: <текст>` делалась неотличимой от
				// вердикта, и такую аннотацию однажды прочли как причину красноты:
				// полоса ушла чинить класс, которого нет (#2542).
				//
				// Префикс от этого НЕ спасает: образец `^\s*(.+\.go):(?:(\d+):(\d+):)? (.*)`
				// начинается с жадного `(.+\.go)`, которое съедает любой текст слева, —
				// строка `audit-list-filter: note: …/serve.go:493:21: …` поднималась,
				// уже не начинаясь с координаты. Работает ровно одно: жадность всегда
				// садится на ПОСЛЕДНЕЕ `.go` строки, и если после координаты нет `: `,
				// продолжить образец нечем ни при каком тексте перед ней.
				//
				// Диагностику это не отнимает — координата остаётся дословной и
				// по-прежнему открывается редактором и находится грепом. Держит
				// свойство `.github/scripts/assert-green-gate-notes.py`: он судит вывод
				// КАЖДОГО зелёного гейта тем же образцом, снятым с пина действия.
				notes = append(notes, body+" — не фатально, пока отказ ручки в production "+
					"закреплён тестом композиционного корня (имя содержит Breakglass и "+
					"Production); см. "+pos)
				return true
			}
			// У НАХОДКИ форма компилятора СОХРАНЯЕТСЯ намеренно: гейт при находке выходит
			// ненулём, шаг краснеет, и аннотация уровня отказа там законна — она ведёт к
			// строке, которую и надо чинить. Глушить её значило бы отнять у находки ровно
			// то, что делает её починяемой.
			out = append(out, pos+": "+body)
			return true
		})
	}
	if !found {

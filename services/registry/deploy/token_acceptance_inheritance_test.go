...
func auditInheritanceMarkers(
	...
	// СЛОЙ, НЕ НАЗВАННЫЙ НИ ОДНОЙ ЦЕПОЧКОЙ, НАСЛЕДОВАТЬ НЕ МОЖЕТ — У НЕГО НЕТ
	// СЛОЯ ПОД СОБОЙ. Базовые значения чарта helm применяет сам, поэтому в
	// `stacks.txt` их нет и быть не может: там перечислены только `-f`-слои.
	//
	// Прежний предикат ключевался на самом факте `registry` в дереве и тем смешивал
	// ДВА разных утверждения: «профиль называет компонент» и «профиль настраивает
	// приём его токенов». Для `-f`-слоёв они почти совпадают; для базового — нет,
	// он называет образы всех компонентов и посадку не объявляет ни для одного.
	//
	// Наблюдалось сборкой волны release/kaname-8 (kacho#2606): в базовый слой
	// завели координату образа реестра — прежде она приезжала МЁРТВЫМ умолчанием
	// подчарта (404 при читаемом репозитории), — и гейт потребовал исхода, которого
	// у базового слоя нет ни одного: объявить приём здесь значит выбрать посадку за
	// оператора для ВСЕХ стендов, а отметку наследования он нести не может by
	// construction (ветка `inChain == 0` ниже как раз это и находит).
	//
	// Поэтому такой слой из вопроса о приёме выведен — и это НЕ молчаливое
	// исключение: он считается отдельно и печатается переписью. Обе прежние находки
	// сохранены: `-f`-слой, умолчавший без отметки, — по-прежнему находка; отметка
	// на слое вне цепочек — тоже (она ничего не утверждает).
	...
	silent, declares, inherits, outside := 0, 0, 0, 0
	for _, name := range names {
		...
		if !named[name] {
			if f.marker != "" {
				findings = append(findings, fmt.Sprintf(
					"слой %q несёт отметку наследования (%q), но ни одна цепочка его не называет — "+
						"под ним нет слоя, «перед ним» не имеет смысла, и отметка ничего не утверждает",
					name, f.marker))
				continue
			}
			outside++
	...
	return findings, fmt.Sprintf(
		"осмотрено профилей=%d, из них молчат о реестре вовсе=%d, объявляют приём=%d, "+
			"наследуют=%d, вне цепочек (наследовать не могут)=%d",
		len(names), silent, declares, inherits, outside)
}
...
func TestInheritanceMarkerAudit_SelfTest(t *testing.T) {
	...
	cases := []struct {
		...
		// (б) БАЗОВЫЙ СЛОЙ: называет компонент и молчит о приёме БЕЗ отметки — обязан
		// молчать. Он не назван ни одной цепочкой, значит слоя под ним нет и
		// наследовать не от кого; требовать от него объявления приёма значило бы
		// требовать выбора посадки за оператора для ВСЕХ стендов. Законный близнец
		// этой оси — случай «отметки нет вовсе» выше: там тот же вход, но профиль
		// ЯВЛЯЕТСЯ членом цепочки, и находка обязана быть (kacho#2606).
		{"слой вне цепочек называет компонент и молчит о приёме", func(p map[string]profileFacts) map[string][]string {
			p["values.yaml"] = profileFacts{declaresRegistry: true}
			return chains
		}, ""},
		// (а) та же ось с другой стороны: у слоя вне цепочек ОТМЕТКА — находка
		// остаётся, потому что отметка утверждает о порядке, которого нет.
		{"слой вне цепочек несёт отметку", func(p map[string]profileFacts) map[string][]string {
			p["values.yaml"] = profileFacts{declaresRegistry: true, marker: "values.dev.yaml"}
			return chains
		}, "ни одна цепочка его не называет"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantFind == "" {
				...
				// Число ВЫВОДИТСЯ из входа, а не выписывается: выписанное «=4» теряет
				// предмет при первом же случае, который добавляет профиль, — и тогда
				// проба краснеет на законном входе, то есть по ложной причине.
				if want := fmt.Sprintf("осмотрено профилей=%d", len(p)); !strings.Contains(census, want) {
					t.Fatalf("перепись не считает вход (ждали %q): %q", want, census)
				}
				return
			}
			if len(findings) == 0 {

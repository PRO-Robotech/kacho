...
func bootConfig(t *testing.T, env map[string]string) config.Config {
	...
	base := map[string]string{
		...
		"KACHO_REGISTRY_AUTHZ_IAM_GRPC_ADDR":          "kacho-iam-internal.kacho.svc.cluster.local:9091",
...
func servedMethods(t *testing.T) []string {
	...
	for _, reg := range []func(grpc.ServiceRegistrar){
		func(r grpc.ServiceRegistrar) { registerPublic(r, registryHandler, opHandler) },

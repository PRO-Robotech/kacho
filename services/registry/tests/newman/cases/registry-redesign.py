...
def _delete_registry(id_var):
    ...
    return [
        Step(name="delete-" + id_var, method="DELETE", path=REG + "/{{" + id_var + "}}",
             test_script=[
                 # Идентификатор операции СБРАСЫВАЕТСЯ ПЕРЕД захватом. На ветке `404`
                 # (ресурс уже снят — она здесь законна) тело `id` не несёт, захват
                 # не срабатывает, и без сброса имя сохранило бы операцию ПРЕДЫДУЩЕГО
                 # шага: опрос подтвердил бы чужой давно завершённый успех. Пустая
                 # строка, а не `unset`: имя остаётся определённым, поэтому страж
                 # неразрешённой подстановки до него не доберётся, а поллер видит
                 # пустое значение и выходит, не утверждая ничего, — ровно тот
                 # объявленный исход, ради которого ветка `404` и допущена.

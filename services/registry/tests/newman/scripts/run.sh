...
# ПРЕДПОЛЁТНАЯ САМОПРОВЕРКА ЧЕСТНОСТИ УТВЕРЖДЕНИЙ. Гейт, который никто не зовёт,
# неотличим от отсутствующего: он не краснеет не потому, что суита исправна, а потому,
# что его не спрашивали. Самопроверка судит СГЕНЕРИРОВАННЫЕ коллекции — стенда и сети
# не требует, стоит секунды — и находит классы, которых прогон суиты не находит by
# construction: шаг, исполняющий запрос и не спрашивающий ничего; шаг, замолчавший от
# нехватки фикстуры вместо того, чтобы отказать по имени переменной.
#
# Место — ДО подъёма стенда и до первого запроса: её отказ означает «прогон не вынесет
# вердикта о продукте», и тратить на это минуты стенда незачем.
#
# Достижимость стережёт гейт дерева `TestNewmanSelftestIsReachableFromItsRunner`
# (internal/repohygiene) — снять этот вызов молча нельзя. node здесь заведомо есть:
# newman на нём и работает.
...
if [[ -n "$SERVICE" ]]; then
  ...
  # КОД ОТБОРА ПОТРЕБОВАН, а не отдан подстановке процесса.
  #
  # Здесь стояло `done < <(newman_all_stems "$NEWMAN_DIR")`. Подстановка процесса
  # теряет код ВСЕГДА — это язык, а не случай, — а отбор УМЕЕТ отказать: нет
  # каталога `cases/` либо `collections/`. Потерянный отказ ОДНОГО слагаемого даёт
  # перечень НЕПОЛНЫЙ, а не пустой, поэтому страж «коллекций 0» ниже его
  # пропускает: прогон шёл по остатку, а перепись «коллекций к прогону N» печатала
# часть как весь набор. Замер: набор без `collections/` давал КОД 0 и ДВА стема.
  roster="$(newman_all_stems "$NEWMAN_DIR")" || {
    echo "FAIL: перечень коллекций не прочитан — причина названа выше." >&2
    echo "      Это «не выполнилось», а не «коллекций нет»: вердикта нет ни по одной." >&2
    exit 2
  }

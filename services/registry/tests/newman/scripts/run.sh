...
# has assertions.failed==0 AND newman's own exit code was 0. The verdict comes from
...
# when any stem: has no out/<stem>.json (MISSING — crash/timeout/unknown newman flag
# such as --jobs; storage --jobs incident 2026-07), assertions.failed>0, or rc!=0.
aggregate_verdict() {

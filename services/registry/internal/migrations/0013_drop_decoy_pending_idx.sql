...
-- Drop registry_outbox_pending_idx — the `(id) WHERE sent_at IS NULL` index created together with the
...
--
--   Index Scan (registry_outbox_pending_idx) -> Sort -> LockRows -> Limit
--
...
DROP INDEX IF EXISTS kacho_registry.registry_outbox_pending_idx;
...
CREATE INDEX IF NOT EXISTS registry_outbox_pending_idx
    ON kacho_registry.registry_outbox (id)

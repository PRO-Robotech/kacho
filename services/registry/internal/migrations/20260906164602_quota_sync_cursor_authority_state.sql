...
-- `corelib/quota` (AuthorityStates) и повторены здесь. Совпадение двух объявлений
...
ALTER TABLE kacho_registry.quota_sync_cursor
...
COMMENT ON COLUMN kacho_registry.quota_sync_cursor.authority_state IS
...
ALTER TABLE kacho_registry.quota_sync_cursor
    DROP CONSTRAINT IF EXISTS quota_sync_cursor_authority_state_check;
ALTER TABLE kacho_registry.quota_sync_cursor DROP COLUMN IF EXISTS authority_state;
-- +goose StatementEnd

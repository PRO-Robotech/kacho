...
--
--     SELECT … FROM kacho_registry.operations [WHERE …]
...
--
-- Что было. `operations_created_at_idx (created_at)` из `0001_init` несёт
...
-- id) WHERE account_id IS NOT NULL` (миграция 0001_init) обслуживает ТОЛЬКО
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_index i
         ...
         WHERE n.nspname = 'kacho_registry' AND c.relname = 'operations_cursor_idx' AND NOT i.indisvalid
    ) THEN
        DROP INDEX kacho_registry.operations_cursor_idx;
    END IF;
END
$$;
-- +goose StatementEnd
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS operations_cursor_idx
    ON kacho_registry.operations (created_at, id);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_index i
          JOIN pg_class c ON c.oid = i.indexrelid
          JOIN pg_namespace n ON n.oid = c.relnamespace
         WHERE n.nspname = 'kacho_registry' AND c.relname = 'operations_cursor_idx' AND i.indisvalid
    ) THEN
        RAISE EXCEPTION
            'kacho_registry.operations_cursor_idx missing or INVALID after rebuild — every operation page would sort the whole table';
    END IF;
END
$$;
-- +goose StatementEnd
...
DROP INDEX CONCURRENTLY IF EXISTS kacho_registry.operations_cursor_idx;

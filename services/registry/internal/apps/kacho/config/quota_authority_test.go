...
func TestQuotaAuthority_KAN_Q1_02_UnsetRefusesStart(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "KACHO_REGISTRY_QUOTA_AUTHORITY")
...
func TestQuotaAuthority_KAN_Q1_05_HalfAPairRefusesStart(t *testing.T) {
	...
	err := c.ValidateQuotaAuthority()
	require.Error(t, err)
	require.Contains(t, err.Error(), "KACHO_REGISTRY_QUOTA_AUTHORITY_MTLS_ENABLE",
...
func TestQuotaAuthority_AbsentAuthorityWithDeclaredTransportRefusesStart(t *testing.T) {
	...
	require.Contains(t, err.Error(), "KACHO_REGISTRY_QUOTA_AUTHORITY_MTLS_SERVERNAME",

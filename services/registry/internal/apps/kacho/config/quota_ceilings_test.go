...
func loadCeilingCfg(t *testing.T, env map[string]string) config.Config {
	t.Helper()
	...
	t.Setenv("KACHO_REGISTRY_DB_PASSWORD", "probe")
	for k, v := range env {
		t.Setenv(k, v)
	}
	cfg, err := config.Load()
...
func fullCeilingEnv() map[string]string {
	out := make(map[string]string, len(config.QuotaCeilingCatalog))
...
func TestQuotaCeilingCatalogShapeIsSound(t *testing.T) {
	...
	for _, k := range config.QuotaCeilingCatalog {
		if !strings.HasPrefix(k.Kind, "registry.") {

...
const quotaDetailExceeded = `{"carrier_type": "project", "carrier_id": "prj-1", ` +
	`"kind": "registry.registry", "limit": 4, "used": 4}`

func TestQuotaRefusalCarriesTheProducerAmounts(t *testing.T) {
	const producer = "project prj-1 has reached its limit of 4 registry.registry"
	err := quotadetail.Attach(
		fmt.Errorf("%w: %s", regerrors.ErrQuotaExceeded, producer), quotaDetailExceeded)
	...
	assert.Equal(t, "registry.kacho.cloud", info.GetDomain())
...
func TestQuotaRefusalNotProvisionedCarriesTheCarrierWithoutAmounts(t *testing.T) {
	const detail = `{"carrier_type": "project", "carrier_id": "prj-1", "kind": "registry.repository"}`
	err := quotadetail.Attach(
		fmt.Errorf("%w: project prj-1 has no ceiling stated for registry.repository",
			regerrors.ErrQuotaNotProvisioned), detail)
...
func TestQuotaRefusalWithoutDetailStaysAValidRefusal(t *testing.T) {
	const producer = "project prj-1 has reached its limit of 4 registry.registry"
	err := fmt.Errorf("%w: %s", regerrors.ErrQuotaExceeded, producer)
...
func TestQuotaDetailKeepsTheSentinelRecognisable(t *testing.T) {
	err := quotadetail.Attach(
		fmt.Errorf("%w: project prj-1 has reached its limit of 4 registry.registry",
			regerrors.ErrQuotaExceeded), quotaDetailExceeded)

	assert.True(t, errors.Is(err, regerrors.ErrQuotaExceeded))
}

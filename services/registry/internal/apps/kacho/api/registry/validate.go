...
// ValidateNamespaceID отсекает malformed namespace-id синхронно первым стейтментом
// RPC: hyphen-form prefix `ns-` (family-agnostic router) → InvalidArgument
// "invalid namespace id '<X>'" (REG-1 F1/REG-1-31). Пустой id пропускается
// (required-проверка — отдельно у caller'а). Экспортирован — единый канонический
// валидатор для use-case и handler-предчека ScopeFiltered-RPC (текст ошибки — часть
// контракта, не дублируем правило по слоям).
func ValidateNamespaceID(id string) error {
	return corevalidate.ResourceID("namespace", ids.PrefixNamespace, id)
}
...
// knownUpdateFields — whitelist update_mask Namespace. name/project_id входят как
...
// Update (смена имени только RenameRepository); namespace_id — натуральный ключ.
var immutableRepoUpdateFields = map[string]string{
	"name":         "name is immutable after Repository.Create",
	"namespace_id": "namespaceId is immutable after Repository.Create",
	"namespaceId":  "namespaceId is immutable after Repository.Create",
}

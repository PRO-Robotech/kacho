...
// registryProtoPackage — пакет, ВСЕ сервисы которого registry реально сервит
// (cmd/kacho-registry/serve.go: RegistryService на public :9090,
// InternalRegistryService на internal :9091). Других сервисов в
// `kacho.cloud.registry.v1` нет, поэтому «весь пакет» == «вся выставленная
// поверхность» — descriptor-обход даёт исчерпывающий список без ручного перечня,
// который сам по себе дрейфует.
const registryProtoPackage = protoreflect.FullName("kacho.cloud.registry.v1")

// TestPermissionMap_CoversEveryServedRegistryRPC — гейт КЛАССА gateway-catalog ↔
...
func TestPermissionMap_CoversEveryServedRegistryRPC(t *testing.T) {
	m := check.PermissionMap()

	var methods []string
	protoregistry.GlobalFiles.RangeFilesByPackage(registryProtoPackage, func(fd protoreflect.FileDescriptor) bool {
		svcs := fd.Services()
	...
	require.NotEmpty(t, methods, "registry/v1 descriptors must be linked into protoregistry")
...
// TestPermissionMap_RegistryEntriesWellFormed — запись мало «иметь»: интерсептор
...
func TestPermissionMap_RegistryEntriesWellFormed(t *testing.T) {

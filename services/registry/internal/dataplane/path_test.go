...
func TestDataplane_ParsePath(t *testing.T) {
	cases := []struct {
		...
		{name: "manifest_by_tag", path: "/v2/reg-A/app/manifests/v1", wantRoute: routeManifest, namespaceID: "reg-A", repo: "app", reference: "v1"},
		{name: "manifest_multiseg_repo", path: "/v2/reg-A/team/app/manifests/v1", wantRoute: routeManifest, namespaceID: "reg-A", repo: "team/app", reference: "v1"},
		{name: "blob_by_digest", path: "/v2/reg-A/app/blobs/sha256:abcdef", wantRoute: routeBlob, namespaceID: "reg-A", repo: "app", reference: "sha256:abcdef"},
		{name: "upload_initiate", path: "/v2/reg-A/app/blobs/uploads/", wantRoute: routeUpload, namespaceID: "reg-A", repo: "app"},
		{name: "upload_with_uuid", path: "/v2/reg-A/app/blobs/uploads/uuid-123", wantRoute: routeUpload, namespaceID: "reg-A", repo: "app"},
		{name: "tags_list", path: "/v2/reg-A/app/tags/list", wantRoute: routeTagsList, namespaceID: "reg-A", repo: "app"},
		{name: "referrers", path: "/v2/reg-A/app/referrers/sha256:dd", wantRoute: routeReferrers, namespaceID: "reg-A", repo: "app", reference: "sha256:dd"},
		// name без repo-сегмента → invalid (нет <namespaceID>/<repo>).
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantRoute != routeInvalid {
				require.Equal(t, tc.namespaceID, p.namespaceID, "namespaceID")

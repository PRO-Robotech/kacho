...
// ListOwned — тот же ownership-предикат, что и в GetOwned (ownsMatches): в pgRepo он
// живёт в SQL WHERE, и расхождение fake'а с ним скрыло бы утечку чужих операций в
// tenant-facing List. Фильтры/пагинация не моделируются (fake держит одну операцию).
func (f *fakeOwnedOpsRepo) ListOwned(_ context.Context, _ operations.ListFilter, o operations.Owner) ([]operations.Operation, string, error) {
	if f.op == nil || !f.ownsMatches(o) {
		return nil, "", nil
	}
	return []operations.Operation{*f.op}, "", nil
}

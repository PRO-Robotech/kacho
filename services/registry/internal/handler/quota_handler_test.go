...
// quotaStoreStub — подставной учёт ДВУХ проектов.
//
...
func (s *quotaStoreStub) QuotaAdmit(context.Context, string, string, string) error { return nil }

func (s *quotaStoreStub) MaterializeQuotas(context.Context, []quotaband.Row) (int64, error) {
	return 0, nil
}

func (s *quotaStoreStub) MaterializeNestedDefaults(context.Context, []quotaband.Row) (int64, error) {
	return 0, nil
}

func (s *quotaStoreStub) ListStates(
...
func quotaFixture(t *testing.T) (*QuotaHandler, *quotaStoreStub, *quotaLimitsStub) {
	t.Helper()
	store := &quotaStoreStub{byProject: map[string][]quotaread.State{
		"prj-mine": {
			{Kind: "registry.registries", Limit: 8, Used: 3,
				SourceScope: "DEFAULT",
				CarrierType: quotaread.CarrierProject, CarrierID: "prj-mine"},
			{Kind: "registry.repositories", Limit: 256, Used: 41,
				SourceScope: "PROJECT", SourceScopeID: "prj-mine",
				CarrierType: quotaread.CarrierProject, CarrierID: "prj-mine"},
		},
		"prj-theirs": {
			{Kind: "registry.repositories", Limit: 999, Used: 900,
	...
	guard := quotaband.NewGuard(store, limits, quotaAccountsStub{}, "registry")
	return NewQuotaHandler(guard, quotaread.AuthorityDeclared()), store, limits
}
...
func TestQuotaHandler_ListShowsTheCallersOwnLimitAndUsage(t *testing.T) {
	h, store, limits := quotaFixture(t)

	resp, err := h.List(context.Background(), &registryv1.ListQuotasRequest{ProjectId: "prj-mine"})
	...
	require.Equal(t, "registry.repositories", got.GetKind())
	require.EqualValues(t, 256, got.GetLimit())
	require.EqualValues(t, 41, got.GetUsed(),
	...
	require.Equal(t, quotaread.CarrierProject, store.askedType)
	require.Equal(t, "prj-mine", store.askedID)
...
func TestQuotaHandler_ListRefusesAnUnanswerableQuestionByName(t *testing.T) {
	...
	_, err := h.List(context.Background(), &registryv1.ListQuotasRequest{})
	require.Error(t, err)
	require.Equal(t, codes.InvalidArgument, status.Code(err))
	require.Equal(t, "project_id: required", status.Convert(err).Message())

	require.Empty(t, store.askedID, "запрос без проекта не должен доходить до учёта")
...
func TestQuotaHandler_ListWithNoBandRefusesInsteadOfClaimingNoQuotas(t *testing.T) {
	...
	_, err := h.List(context.Background(), &registryv1.ListQuotasRequest{ProjectId: "prj-mine"})
	require.Error(t, err)

...
// legacyIssuerClaims — выданный прежним издателем identity-JWT: sub = client_id (принципал для Check),
// aud ⊇ наш service, iss = прежний издатель.
func legacyIssuerClaims(sub string, exp time.Time) map[string]any {
...
func TestJWKS_Verify_RS256_Valid(t *testing.T) {
	...
	tok := js.mintRS256(t, "kid-rsa", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
...
// REG-TX-13 — валидный ES256-JWT прежнего издателя → Verify возвращает sub. Прежний издатель может отдавать
// как RS256, так и ES256 — data-plane обязан верифицировать оба по одному JWKS.
func TestJWKS_Verify_ES256_Valid(t *testing.T) {
	js := newJWKSServer(t)
	js.addEC(t, "kid-ec")
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	tok := js.mintES256(t, "kid-ec", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
...
// Федеративный SA-токен: `sub` прежнего издателя — это client_id, а Kachō principal id (sva…/usr…)
...
func TestJWKS_Verify_ReturnsKachoPrincipalID(t *testing.T) {
	js := newJWKSServer(t, "kid-rsa")
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	claims := legacyIssuerClaims("81f92eee-client-uuid", time.Now().Add(time.Hour))
...
func TestJWKS_Verify_FallsBackToSubWhenNoPrincipalClaim(t *testing.T) {
	js := newJWKSServer(t, "kid-rsa")
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	tok := js.mintRS256(t, "kid-rsa", legacyIssuerClaims("usr-bare", time.Now().Add(time.Hour)))
...
func TestJWKS_Verify_Expired(t *testing.T) {
	js := newJWKSServer(t, "kid-rsa")
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	tok := js.mintRS256(t, "kid-rsa", legacyIssuerClaims("cid-ci", time.Now().Add(-time.Minute)))
...
func TestJWKS_Verify_UnknownKid(t *testing.T) {
	...
	tok := js.mintRS256(t, "kid-rogue", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
...
func TestJWKS_Verify_HS256_Rejected(t *testing.T) {
	js := newJWKSServer(t, "kid-rsa")
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	signingInput := joseSigningInput("HS256", "kid-rsa", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
...
func TestJWKS_Verify_NoneAlg_Rejected(t *testing.T) {
	js := newJWKSServer(t, "kid-rsa")
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	signingInput := joseSigningInput("none", "kid-rsa", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
...
func TestJWKS_Verify_KeyTypeAlgConfusion_Rejected(t *testing.T) {
	...
	t.Run("es256-header-over-rsa-key", func(t *testing.T) {
		js := newJWKSServer(t, "kid-rsa")
		v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
		signingInput := joseSigningInput("ES256", "kid-rsa", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
	...
	t.Run("rs256-header-over-ec-key", func(t *testing.T) {
		...
		js.addEC(t, "kid-ec")
		v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
		signingInput := joseSigningInput("RS256", "kid-ec", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))
...
// добавленный издателем после первого фетча, подхватывается рефетчем → verify OK.
...
func TestJWKS_Verify_RefetchOnUnknownKid(t *testing.T) {
	...
	tok1 := js.mintRS256(t, "kid-rsa", legacyIssuerClaims("cid-ci", clock.Add(time.Hour)))
	_, err := v.Verify(context.Background(), tok1)
	require.NoError(t, err)
	require.Equal(t, int32(1), js.fetch.Load())

	// ротация ключей издателя: добавляем ES256 kid-ec2 в JWKS. Продвигаем часы за окно троттла
	// (кэш ещё свеж по TTL: minRefresh << ttl), чтобы рефетч по новому kid'у не был
	// подавлен троттлом.
	js.addEC(t, "kid-ec2")
	clock = clock.Add(defaultMinRefresh + time.Second)
	tok2 := js.mintES256(t, "kid-ec2", legacyIssuerClaims("cid-ci2", clock.Add(time.Hour)))
...
func TestJWKS_Verify_UnknownKidThrottled_NoRefetch(t *testing.T) {
	...
	tok := js.mintRS256(t, "kid-rsa", legacyIssuerClaims("cid-ci", clock.Add(time.Hour)))
	_, err := v.Verify(context.Background(), tok)
	require.NoError(t, err)
	require.Equal(t, int32(1), js.fetch.Load())
	...
	for i := 0; i < 5; i++ {
		bogus := joseSigningInput("RS256", fmt.Sprintf("bogus-%d", i),
			legacyIssuerClaims("cid-x", clock.Add(time.Hour))) + ".QUFB"
...
func TestJWKS_Verify_RefreshDetachedFromRequestCtx(t *testing.T) {
	...
	tok := js.mintRS256(t, "kid-rsa", legacyIssuerClaims("cid-ci", clock.Add(24*time.Hour)))
	_, err := v.Verify(context.Background(), tok)
	require.NoError(t, err)
	require.Equal(t, int32(1), js.fetch.Load())

	// Ротация ключей издателя: добавлен новый ES256-ключ kid-ec2. Часы за окном TTL (max-age=300) →
	...
	cancel()
	bogus := joseSigningInput("RS256", "attacker-kid", legacyIssuerClaims("cid-x", clock.Add(time.Hour))) + ".QUFB"
...
// SEC — нормальный 2048-битный ключ (типовой размер) проходит toRSA без изменений
...
// не должен принимать forgeable-токен даже если издатель по ошибке отдала слабый ключ.
func TestJWKS_Verify_SmallModulusKey_Rejected(t *testing.T) {
	...
	v := newTestVerifier(t, js.srv.URL, testAud, testLegacyIss)
	tok := js.mintRS256(t, "kid-weak", legacyIssuerClaims("cid-ci", time.Now().Add(time.Hour)))

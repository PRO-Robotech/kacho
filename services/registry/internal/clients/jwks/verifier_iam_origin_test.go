...
// JWKS теперь скачивается из iam-JWKS-proxy (а не из прежний издатель напрямую), но issuer-pin
// остаётся на прежний издатель. verifier.go — origin-agnostic (I7): он не знает и не должен знать,
...
// fail-closed. testLegacyIss / testAud / newJWKSServer / mintRS256 / priorIssuerClaims —
// общий harness из verifier_test.go (тот же package).

// iamJWKSServer — семантический alias: fake iam JWKS-proxy, отдающий зеркалённые у прежнего издателя
// ключи (kid/alg байт-в-байт прежний издатель). Для verifier'а это просто JWKS-URL.
...
func TestJWKS_Verify_IAMOrigin_LegacyIssuerPinned(t *testing.T) {
	...
	// verifier направлен на iam-JWKS-URL; issuer-pin — на прежний издатель (раздельные knob'ы).
	v := newTestVerifier(t, iam.srv.URL, testAud, testLegacyIss)
	tok := iam.mintRS256(t, "legacy-kid-1", priorIssuerClaims("cid-ci", time.Now().Add(time.Hour)))

	sub, err := v.Verify(context.Background(), tok)
	require.NoError(t, err, "подписанный прежним издателем token must verify against JWKS fetched from the iam proxy")
...
// RJU-13/n2 — issuer-pin остаётся на прежний издатель НЕСМОТРЯ на смену JWKS-URL на iam
// (декаплинг knob'ов, I5): токен подписан валидным зеркалённые у прежнего издателя ключом из iam-JWKS,
...
func TestJWKS_Verify_IAMOrigin_IssuerMismatch_Rejected(t *testing.T) {
	iam := iamJWKSServer(t, "legacy-kid-1")
	v := newTestVerifier(t, iam.srv.URL, testAud, testLegacyIss) // issuer-pin остаётся прежний издатель
	claims := priorIssuerClaims("cid-ci", time.Now().Add(time.Hour))
	claims["iss"] = iam.srv.URL // iss указывает на iam, а не на прежний издатель — mismatch

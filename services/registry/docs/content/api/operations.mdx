...
опрашивает её статус, пока фоновая запись не завершится. Подписки на операции у платформы нет:
их исход узнаётся поллингом.

Изменения самих **ресурсов** — другое дело: их registry отдаёт потоком, ручкой
`GET /subscription/v1/events?owner=registry`. Различие существенное и его стоит держать в
голове: поток говорит, что реестр создан, изменён или снят, но НЕ говорит, чем кончилась
конкретная операция, — за исходом операции по-прежнему ходят поллингом. Подробнее —
[Обзор API](/api/overview).

Операции реестра несут идентификатор с префиксом **`rop`** и хранятся в общей таблице `operations`
(из `corelib/operations`). Фоновый worker выполняет доменную запись и финализирует строку
операции.
...
:::note Путь операции домен-агностичен — имени сервиса и версии в нём нет
Контракт операции общий для всех доменов, поэтому его адрес объявлен на верхнем
уровне, а владельца край выбирает по префиксу идентификатора. Прежняя редакция
страницы ставила перед ним имя домена и версию — такого адреса край не
обслуживает; снятая форма здесь не воспроизводится, потому что адрес на этой
странице читается как рабочий и попадает в терминал прямо отсюда.
:::

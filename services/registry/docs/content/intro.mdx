...
<table>
  ...
  <tbody>
    ...
    <tr><td>OAuth2/OIDC identity-JWT</td><td>Удостоверение плоскости данных. Издателей принимается несколько: платформа (kaname) и внешний поставщик (Ory Hydra) — перечень объявляет посадка. Авторизация — per-RPC / per-request Check через kaname</td></tr>

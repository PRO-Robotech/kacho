...
`proto/`; в runtime у него исходящие рёбра в `kacho-iam` (авторизация, валидация проекта,
...
<div className={hero.featureGrid}>
  ...
  <div className={hero.featureCard}>
    ...
    <span className={hero.featureBody}>Каждый push/pull проходит per-request Check в kacho-iam. Права — на уровне реестра и репозитория.</span>
  </div>
  <div className={hero.featureCard}>
    <span className={hero.featureIcon}>⇄</span>
  ...
  <div className={hero.featureCard}>
    ...
    <span className={hero.featureBody}>При создании реестра владение регистрируется в kacho-iam через транзакционный outbox (at-least-once).</span>
  </div>
</div>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Ory Hydra</td><td>OAuth2/OIDC — identity-JWT для data-plane; авторизация — per-RPC / per-request Check через kacho-iam</td></tr>

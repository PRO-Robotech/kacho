...
Идентификатор Registry генерирует сервер (`corelib/ids`): 3-символьный префикс **`reg`** +
...
<table>
  ...
  <tbody>
    ...
    <tr><td>OAuth2/OIDC identity-JWT</td><td>Удостоверение плоскости данных. Издателей принимается несколько: платформа (kaname) и, пока открыто окно перехода, прежний издатель — перечень объявляет посадка. Авторизация — per-RPC / per-request Check через kaname</td></tr>

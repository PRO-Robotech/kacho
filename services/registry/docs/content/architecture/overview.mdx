...
(`RegisterResource` в kacho-iam), чтобы создатель видел реестр в authz-фильтрованном списке.
Прямая запись в модель прав из сервиса запрещена (модули пишут только через kacho-iam) —

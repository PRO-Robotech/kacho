...
flowchart TB
  ...
  subgraph deps["Зависимости"]
    iam["kaname<br/>Check · Project · fga-proxy · token-shim"]
...
- **Control-plane** управляет ресурсом Registry и наложением на Repository (CRUD в БД), а содержимое
  репозиториев и теги читает проекцией из zot.
...
— от домена, домен не зависит ни от чего, кроме stdlib и сгенерённых стабов контракта
...
## Регистрация владения через transactional-outbox {#owner-registration}

При создании реестра сервис должен зарегистрировать владение и место реестра в цепи областей
(`RegisterResource` в kaname), чтобы создатель видел реестр в authz-фильтрованном списке.
Прямая запись в модель прав из сервиса запрещена (модули пишут только через kaname) —
...
Отношения, писанные через proxy, — только иерархические (`project`/`owner`); `admin/editor/...`
...
Содержимое Repository и Tag — проекция из zot (наложение на Repository живёт в БД
kacho-registry, см. таблицу ресурсов). Control-plane читает проекцию через zot-клиент (GraphQL
...
Kachō Registry по сборке зависит только от `pkg/` и `proto/`. В runtime у него исходящие
рёбра в `kaname` и в zot; **входящих** cross-service рёбер от других доменов нет. Циклов в графе нет.

```mermaid
flowchart LR
  reg["kacho-registry"] -->|"InternalIAMService.Check (per-RPC authz)"| iam["kaname"]
  ...
  docker["docker / helm"] -->|"identity-JWT через /iam/token"| iam
```
...
`ProjectService.Get` зарегистрирован только на public listener kaname (`:9090`); authz-Check и

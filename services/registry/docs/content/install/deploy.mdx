...
flowchart LR
  ...
  cp9090 --> zot[("zot")]
  dp --> zot
  cp9090 -->|"authz · project · fga-proxy"| iam["kaname"]
  dp -->|"authz + token"| iam
```
...
- **Control-plane**: per-RPC authz `Check` (адрес IAM) **и** mTLS на **обоих** gRPC-listener'ах
  (`:9090`/`:9091`) обязательны, иначе процесс не поднимается. Обхода в боевом режиме нет:
  `KACHO_REGISTRY_AUTHZ_BREAKGLASS=true` здесь сам по себе отказывает в старте (он работает только
  при `AUTH_MODE=dev`).
...
kubectl -n <namespace> exec deploy/<деплой kacho-registry> -- \
...
| под работает нужным образом | `kubectl -n <namespace> get pod -l app=<деплой kacho-registry> -o jsonpath='{..image}'` |

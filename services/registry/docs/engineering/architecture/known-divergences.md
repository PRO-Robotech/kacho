...
**Остаток, который этим решением НЕ закрыт.** Предупреждение о цене живёт **только в
документации**: в момент вызова платформа ничем не показывает, что у репозитория есть
потребители, и не требует подтверждения. То есть клиент, не читавший страницу, получает
`200` и узнаёт цену позже. Механизма здесь нет, и это сказано вслух, чтобы не выдавать
намерение за факт.

Предикат, которым остаток снимается, назван здесь, чтобы его можно было потребовать:
вызов `:rename` на репозитории с непустым `downloadCount` **либо** отвергается без явного
подтверждения, **либо** отвечает величиной, называющей, сколько путей он обрывает. Пока ни
того, ни другого нет, «предупреждение» этого решения означает ровно страницу документации —
и не больше.
...
- у `RenameRepository` появляется предупреждающая полоса — поле подтверждения либо отказ
  при живых потребителях; тогда «остаток» выше закрывается, и запись правится вместе с
  ним;

...
#
# ПЛАН СОВПАДАЕТ С `deploy/scripts/seed-nlb-fixtures.sh` — у координаты
# `existingNetworkId` два автора, и набор не знает, чей отработал. Здесь стояло
# `10.196.0.0/16` + `fd00:196::/48`, и ширина плана — это ЧИСЛО ПОЗИЦИЙ, доступных
# нарезке (`carve_cidr_pre` режет /24 внутри плана): под `/16` их 256, под `/8` —
# 65536. Набор режет за прогон порядка семи десятков подсетей в ОДНОЙ сети, где
# пересечение отвергается сразу, — то есть под узким автором столкновение внутри
# прогона практически неизбежно, а под широким остаётся единицами процентов. Один
# поток при этом зелёный, и расхождение читается нестабильностью продукта.
#
# Цена решения названа: обе сети этого посева объявляют весь частный `/8` своим
# планом. Продукт этому не препятствует — объявленные блоки сети хранятся полем
# без ограничения пересечения между сетями (`0015_network_supernet.sql`: `text[]`,
# ни EXCLUDE, ни UNIQUE), а подсети не пересекаются в пределах СВОЕЙ сети. Предикат
# пересмотра: появится ограничение на пересечение объявленных планов между сетями
# либо стенд, где сеть делят наборы с несовместимыми планами, — сужать ОБОИХ
# авторов вместе. Инвариант — паритет, а `/8` лишь его сегодняшнее значение.
#
# Держит `internal/repohygiene` `TestSeedAddressPlanParityAcrossAuthors`.
...
# (`carve_cidr_pre`): ширина плана есть свойство посева, а не набора, и прошивать
# её на другой стороне нельзя.

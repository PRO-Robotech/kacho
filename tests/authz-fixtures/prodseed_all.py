...
This module is the production path setup.sh delegates to. Nothing is forged here:
  ...
  * every subject Bearer comes from iam `SAKeyService.Issue` — iam returns an ES256
    ...
    standard OAuth2 client_credentials exchange at OUR issuer (`POST /iam/v1/token`,
    `mint_rs256.PLATFORM_TOKEN_URL`). No step of this seed reaches the former OAuth
    server: the exchange there was retired with #1120, and the port-forward that
    outlived it was removed with #2685.
...
def main() -> int:
    ...
    # ПРЕДПОСЫЛКА ОДНА — клиентские сертификаты. Проброса к прежнему OAuth-серверу
    # здесь больше нет (задача #2685): обмен идёт у НАШЕГО издателя
    # (`mint_rs256.PLATFORM_TOKEN_URL` → POST /iam/v1/token), и по публичному
    # слушателю прежнего посев не ходит ни одним шагом. Условие его достижимости
    # отказывало посеву по стороне, с которой он не разговаривает. Держит
    # `prodseed_needs_no_provider_forward_test.py` — исходом, а не именем.
    ensure_certs()
    ...
    # credentials signed by our issuer, with a real lifetime — not the well-known dev HMAC strings

...
design — the gateway accepts only RS256 signed by the platform issuer, and iam's internal listener demands
...
This module is the production path setup.sh delegates to. Nothing is forged here:
  ...
  * every subject Bearer comes from iam `SAKeyService.Issue` — iam registers the machine
    client and returns an ES256 private key ONCE; we sign a private_key_jwt
    `client_assertion` with it and run the standard OAuth2 client_credentials exchange
    AT OUR OWN ISSUER (`POST /iam/v1/token`, see mint_rs256.PLATFORM_TOKEN_URL).
    Issuance, client lifecycle and JWKS all stay behind the iam facade; no foreign
    issuer is dialled, and no client mirror is registered at one (#1120).
...
def main() -> int:
    ...
    # In this posture the tokens written into the env files are genuine platform-signed

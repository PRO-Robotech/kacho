...
def stage_mirror(admin: str, ext_id: str, email: str) -> str:
    """Зеркало человека в iam — И ОЖИДАНИЕ ЕГО ЧИТАЕМОСТИ.
    ...
    Ждём ЧИТАЕМОСТЬ строки, а не завершение операции: эта операция приходит на
    ...
    по этой же причине ждёт появления строки, только через прямой запрос к базе).

    ОТКАЗ В ПРАВЕ — НЕ ЗАДЕРЖКА. Повтор идентичного чтения при 403 пройти не может,
    поэтому бюджет на него не тратится и посев не объявляется сорванным: подтвердить
    читаемость этим путём мы не смогли, и стадия говорит это вслух. Судьёй остаётся
    утверждение стадии 7 — прежнее поведение, не ослабленное.
    """
    code, _, body = _req(
    ...
    attempts, waited = 40, 0
    for _ in range(attempts):
        rc, _h, rb = _req("GET", f"{PUBLIC}/iam/v1/users/{user_id}",
                          headers={"Authorization": f"Bearer {admin}"})
        if rc == 200:
            print(f"[ceremony] 6-зеркало: человек {ext_id} отражён как {user_id} "
                  f"и ЧИТАЕТСЯ (ожидание {waited}) — хук провижининга на стенде не "
                  f"смонтирован, делаем явно")
            return user_id
        if rc == 403:
            print(f"[ceremony] 6-зеркало: человек {ext_id} отражён как {user_id}; "
                  f"читаемость НЕ подтверждена — чтение запрещено (403), а это "
                  f"состояние постоянное, бюджет на него не тратится. Судит стадия 7")
            return user_id
        waited += 1
        time.sleep(0.5)
    raise StageError(
        "6-зеркало",
        f"человек {ext_id} ({user_id}) так и не стал читаемым за {attempts} попыток — "
        f"идентификатор из метаданных предвыделен, поэтому отдать его значило бы "
        f"отдать фантом, а падение уехало бы на стадию 7 с сообщением про чужой предмет")
...
def _self_test() -> int:
    """Стадия 6 не отдаёт идентификатор раньше, чем зеркало становится ЧИТАЕМЫМ.

    Инъекция в обе стороны: настоящая задержка обязана быть переждана, настоящий
    отказ в праве — распознан как ПОСТОЯННЫЙ и не переждан. Сеть не трогается:
    подменяется единственная точка выхода `_req`.
    """
    rc = 0
    print("=== самопроверка стадии 6: зеркало ждут до читаемости ===")
    real_req, real_sleep = _req, time.sleep

    def install(read_codes):
        """read_codes — коды, которыми отвечает чтение зеркала, по порядку."""
        seq = list(read_codes)
        seen = {"reads": 0}

        def fake(method, url, *, body=None, headers=None, timeout=20, jar=None):
            if method == "POST" and "upsertFromIdentity" in url:
                return 200, {}, json.dumps({"id": "iopX",
                                            "metadata": {"userId": "usrTEST"}})
            if method == "GET" and "/iam/v1/users/" in url:
                seen["reads"] += 1
                code = seq[min(seen["reads"] - 1, len(seq) - 1)]
                return code, {}, ("{}" if code == 200 else '{"message":"no"}')
            raise AssertionError(f"самопроверка не ждала запроса {method} {url}")
        return fake, seen

    def run(read_codes):
        globals()["_req"], seen = install(read_codes)
        globals()["time"].sleep = lambda _s: None
        try:
            out, err = stage_mirror("adm", "extTEST", "a@b.c"), None
        except StageError as exc:
            out, err = None, exc
        finally:
            globals()["_req"] = real_req
            globals()["time"].sleep = real_sleep
        return out, err, seen["reads"]

    # (а) НАСТОЯЩАЯ ЗАДЕРЖКА: зеркало появляется не сразу — стадия обязана переждать.
    out, err, reads = run([404, 404, 200])
    if out == "usrTEST" and err is None and reads >= 3:
        print(f"  ОК  задержка переждана: чтений {reads}, идентификатор отдан после появления")
    else:
        print(f"  ПРОВАЛ задержка: вернулось {out!r}, ошибка {err}, чтений {reads}")
        rc = 1

    # (б) ЗЕРКАЛО ТАК И НЕ ПОЯВИЛОСЬ: это НЕ повод отдать предвыделенный
    #     идентификатор — он был бы фантомом, и падение уехало бы на стадию 7 с
    #     сообщением про чужой предмет.
    out, err, reads = run([404])
    if out is None and err is not None and "6-зеркало" in str(err) and reads > 1:
        print(f"  ОК  зеркало не появилось: стадия встаёт САМА (чтений {reads}), "
              f"а не отдаёт фантом")
    else:
        print(f"  ПРОВАЛ отсутствие зеркала: вернулось {out!r}, ошибка {err}, чтений {reads}")
        rc = 1

    # (в) ОТКАЗ В ПРАВЕ — состояние ПОСТОЯННОЕ, а не задержка. Повтор идентичного
    #     запроса не может пройти, поэтому бюджет на него не тратится и посев не
    #     объявляется сорванным: судьёй остаётся утверждение стадии 7.
    out, err, reads = run([403])
    if out == "usrTEST" and err is None and reads == 1:
        print("  ОК  отказ в праве распознан постоянным: одно чтение, бюджет не сожжён")
    else:
        print(f"  ПРОВАЛ отказ в праве: вернулось {out!r}, ошибка {err}, чтений {reads}")
        rc = 1

    print()
    print("PASS: стадия 6 ждёт читаемости зеркала" if rc == 0
          else "FAIL: стадия 6 ждёт читаемости зеркала")
    return rc

...
назначает и подписывает выдающий; укоротить его на один выпуск нельзя, а подделать —
значит вернуться ровно к тому, от чего волна и уводит. ЭТА волна его и не заполняет:
у него теперь СВОЯ волна, `services/iam/tests/newman/scripts/run-expired-bearer.sh`,
которая выпускает настоящий предъявитель и ПЕРЕЖДАЁТ его срок по стенным часам.
Отдельная она не по вкусу, а по длительности: срок — 14400 с (замер 2026-08-04;
прежде в дереве стояло 900 с, и это было неверно), то есть волна идёт часы и в общий
прогон входить не может. Пока она не отработала, утверждения кейса остаются ОТКРЫТЫМ
ДОЛГОМ С ЧИСЛОМ (приёмка IAM-INT-1, сценарий 33), и посев печатает его числом, а не
умалчивает.
...
def main() -> int:
    ...
    try:
        ...
        print(f"[ceremony] ОТКРЫТЫЙ ДОЛГ (не заполнено, не замаскировано): "
              f"{_expired_debt()} Срок назначает выдающий, укоротить его на один выпуск "
              f"нельзя. Волна, СОЗДАЮЩАЯ это условие, есть — "
              f"services/iam/tests/newman/scripts/run-expired-bearer.sh; она идёт столько "
              f"же, сколько живёт предъявитель (замер 2026-08-04: 14400 с, не 900 с), "
              f"поэтому в общий прогон не входит и требует собственного расписания.")
        return 0

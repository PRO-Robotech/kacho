...
def _st_mint_follows_mirror() -> tuple[bool, str]:
    """`main` не чеканит предъявителя раньше, чем дождалась зеркала — по AST.
    ...
    Правило одно и покрывает ОБОИХ людей церемонии: перед k-й чеканкой обязано
    стоять не менее k ожиданий. Свою предпосылку замок проверяет сам — не найдя
    ни одного вызова, он объявляет это находкой, а не чистотой: «ноль находок»
    ...
    if main_fn is None:
        return False, "в дереве нет функции main — замок не прочитал НИЧЕГО"
    order: list[tuple[str, int]] = []
    for node in ast.walk(main_fn):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) \
                and node.func.id in ("stage_mirror", "stage_ceremony"):
            order.append((node.func.id, node.lineno))
    order.sort(key=lambda it: it[1])
    mirrors = sum(1 for n, _ in order if n == "stage_mirror")
    mints = sum(1 for n, _ in order if n == "stage_ceremony")
    if mirrors == 0 or mints == 0:
        return False, (f"замок не нашёл предмета: ожиданий {mirrors}, чеканок {mints} "
                       f"— вызовы переименованы или уехали из main")
    seen_mirror = seen_mint = 0
    for name, line in order:
        if name == "stage_mirror":
            seen_mirror += 1
            continue
        seen_mint += 1
        if seen_mirror < seen_mint:
            return False, (f"строка {line}: чеканка №{seen_mint} идёт раньше, чем "
                           f"дождались зеркала (ожиданий до неё {seen_mirror}) — "
                           f"предъявитель выпускается без человека")
    return True, f"осмотрено вызовов {len(order)}: ожиданий {mirrors}, чеканок {mints}"
...
def _self_test() -> int:
    ...
    # (л) ПОРЯДОК В `main`, а не только поведение стадии. Динамический замок выше
    #     гоняет стадию НАПРЯМУЮ и о порядке в `main` ничего не знает: перенос
    #     чеканки перед ожиданием оставлял ВСЕ прочие утверждения зелёными —
    #     измерено инъекцией, не предположено.
    order_ok, order_detail = _st_mint_follows_mirror()
    check("чеканка не опережает ожидание зеркала (AST `main`)", order_ok, order_detail)

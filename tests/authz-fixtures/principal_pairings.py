...
def token_principal_id(token: str) -> str:
    ...
    Reads `ext_claims.kacho_principal_id` — the claim the edge derives the principal
    from. Deliberately NOT `sub`: on a client_credentials token that is the Hydra OAuth
    client uuid and equals no kacho id at all, so a comparison against it reports EVERY
    pairing broken, the sound ones included. (Measured: the first version of this check
    did exactly that and would have condemned a channel that was correct.)
    """
    ...
    if not isinstance(ext, dict):
        return ""
    got = ext.get("kacho_principal_id", "")
...
def make_token(principal_id: str, *, nest: bool = False) -> str:
    ...
    """
    inner = {"kacho_principal_id": principal_id, "kacho_principal_type": "service_account"}

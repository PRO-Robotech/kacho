...
по образцу `*_test.py` в каталогах проб, каждым прогоном конвейера. Имя этого файла
нигде больше не встречается, поэтому `git grep` о его исполнителе молчит — строка
выше и есть ответ на вопрос «кто меня зовёт».

...
//
// Она не выбрана здесь, а выведена из продукта: реконсайлер собирает имя как
// `"v_" + domain.NormalizeVerb(<глагол роли>)` (комментарий у `v_addtargets` в
// самой модели это и фиксирует). Поэтому «глагол» здесь — свойство имени, и это
// свойство держит сама модель, а не память автора.
const VerbPrefix = "v_"
...
func (m *Model) Compile(typeName, relation string) (Plan, error) {
	t := m.byName[typeName]
	if t == nil {
		return Plan{}, fmt.Errorf("тип %q не объявлен", typeName)
	}

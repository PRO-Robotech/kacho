...
// `corelib/ids`: у него ноль внешних зависимостей (`go list -deps` даёт только
...
func TestExternalConsumerCanBuildTheModule(t *testing.T) {
	...
	res := packAndBuild(t, packRequest{
		...
		// ЭТОГО модуля не говорил бы ничего. Взят самый дешёвый по зависимостям
		// из оставшихся — стаб ссылки тянет только `protobuf`.
		//
		// ПРЕДПОСЫЛКА ТЕХНИКИ УМЕРЛА ВМЕСТЕ С `pkg/ids`, и это замер, а не
		// впечатление: экспортируемых пакетов БЕЗ внешних зависимостей у
		// платформы теперь НОЛЬ (предикат — `go list -f '{{.Deps}}' ./...` с
		// отбросом стандартной библиотеки и внутренних путей). Песочница же
		// поднимает прокси из ОДНОГО зипа — самого модуля, — поэтому проба
		// краснеет отказом «нет зипа зависимости», пока прокси не начнёт
		// выкладывать и зависимости. Это остаток с предикатом, а не послабление:
		// красное здесь честно называет, что модуль внешним потребителем НЕ
		// собирается — ни до публикации общей библиотеки, ни после, пока
		// песочница выкладывает один зип.

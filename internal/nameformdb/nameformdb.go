...
// Package nameform — общий двигатель пробы «ограничение формы имени ДЕЙСТВУЕТ на
// живой базе».
//
// # Предмет
//
// Миграция `715001_resource_name_single_form.sql` ставит ограничение формы имени
// в пяти схемах. «Миграция применилась» и «ограничение отвергает негодную
// строку» — РАЗНЫЕ утверждения, и до задачи #721 проверялось только первое:
// перепись пробы, доказывающей второе, давала один сервис из пяти.
//
...
func (p Probe) Run(ctx context.Context, t *testing.T, db Execer) {
	...
	if err != nil {
		t.Fatalf("проба формы имени не смогла прочитать схему %s: %v", p.Schema, err)
	}
	t.Log(rep.Census())

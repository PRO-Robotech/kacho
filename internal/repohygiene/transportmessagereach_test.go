...
var transportMessageAllow = []TransportMessageAllowance{
	...
	// ── Запись фазы Ф1 эпика kacho#1016 СНЯТА, потому что ИСТЕКЛА ────────────
	//
	// Она стояла здесь ровно одну фазу: форма подписки была объявлена раньше
	// своего глагола, и `SubscriptionRequest` не назывался ни одним. Запись сама
	...
	// не дал её оставить.
	//
	// Абзац сохранён вместо пустой строки намеренно: он показывает, чем именно
	// заканчивается послабление, у которого есть предикат истечения.
}

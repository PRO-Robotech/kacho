...
//
//  4. ДОМЕНЫ ВНЕ ОХВАТА не судятся. Перечень судимых объявлен явно
//     (`JudgedDomains`), перепись печатает и его, и остаток. Это граница полосы,
//     а не свойство дерева: на день заведения в страницах vpc оставалось 4
//     токена того же класса (`net`/`rtb`/`sgr`/`sub` дефисом при слитной
//     чеканке), и они принадлежат другому предмету со своим владельцем.
//

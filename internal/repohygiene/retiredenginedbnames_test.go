...
//
// Семей ДВЕ, и различать их надо, потому что предикат снятия у них разный:
//
...
//     Имя устарело; переименование рассматривалось и отклонено, задача #1667.
//
...
//
// ЗДЕСЬ СТОЯЛА ТРЕТЬЯ СЕМЬЯ — `fga_model_version*` (iam), версия модели,
// ЗАГРУЖЕННОЙ в снятый движок. Её три строки (последовательность и два ключа)
// держались здесь НАМЕРЕННО: таблицу сняла своя миграция, а последовательность
// и ключи уходят из базы вместе с таблицей неявно, поэтому разбор, видящий
// только ЯВНЫЙ `DROP` в секции `Up`, продолжал считать их живыми — и снятие
// строк тогда читалось бы как прибавка.
//
// Предмета у этого довода больше нет. Цепочка миграций iam сведена в одну
// первичную `services/iam/internal/migrations/0001_initial.sql`, собранную по
// сыгравшей базе: в ней нет ни объявления этих объектов, ни снимающей их
// миграции — разбору стало нечего считать живым. Три строки сняты ТЕМ ЖЕ
// изменением, которым ушёл их предмет.
//
// Предикат, которым это перемеряется: `grep -c fga_model_version
// services/iam/internal/migrations/0001_initial.sql` → 0. Уборка, которой
// строки свидетельствовали, записана в `dropguard.json` сервиса — там она и
// остаётся; второе место о ней здесь завелось бы снова.

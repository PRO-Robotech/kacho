...
// готовности.
//
// На момент заведения таких сервисов было три (storage и registry отдавали
// безусловный `/healthz`, geo — только `/metrics`); СЕГОДНЯ их НОЛЬ — готовность
// строят все семь. Число здесь оставлено с обеими датами намеренно: молчание
// этого гейта о сервисе без готовности — его законная граница, а не покрытие, и
// положительную половину («готовность обязан строить КАЖДЫЙ») держит соседний
// гейт `TestEveryServiceBuildsReadinessRatherThanServingLiveness`. Без него
// исчезновение готовности у сервиса заставило бы этот гейт ЗАМОЛЧАТЬ вместо того
// чтобы покраснеть (`testing.md` §«Гейт на класс», п.9).
package repohygiene
...
// `kaname`/`kacho-geo`, а каталог кода — `iam`/`geo`. Приставка снимается

...
//   - HEAD — СЛИЯНИЕ (голова запроса слияния; `merge --no-ff` задачи в волну)
//     — база `HEAD^`: судится то, что слияние вносит в первого родителя, то есть
//     в линию;
...
//
//  1. Прогон на вершине линии судит не всю линию: вершина-слияние — последнее
//     слияние, вершина-коммит уже отправленной линии — её отрезок после
//     последнего ответвления соседней ветки-номера. На вердикт конвейера это
//     не влияет: голова запроса — слияние, и судится ровно дельта запроса.
...
func retiredVendorBaseRev(root string) (rev, how string, err error) {
	...
	case trunk != "", merge:
		...
		if trunk == "" {
			why = "HEAD — слияние: судится то, что оно вносит в первого родителя, то есть в линию"
		}
		if len(chain) < 2 {

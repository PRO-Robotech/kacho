...
var retiredVendorCeilings = map[string]int{
	// платформа: 1275 строк — по пути 18 · по имени 1219 · склейкой 1 · по
	// пути API 34 · архивов 3. Архивы: два чарта издателя по имени и
	...
	//
	// #2734: 1275 = 1646 − 375 + 4. Потолок СНИЖЕН: край стал чистым
	// проверяющим — сняты клиент интроспекции прежнего поставщика, снятие сессии
	// на его стороне при выходе, их ручки и их пробы; полоса отзыва записи
	// спрашивает нашу запись и без мягкого прохода. Слагаемые — по файлам,
	// разностью перечней адресов двух прогонов этого гейта (голова #2792 и это
	// изменение):
	//
	//	убыль 375   производственное 86 (`deployment.yaml` края 17 · `main.go` 12 ·
	//	            `logout_handler.go` 12 · `revocation_validation.go` 8 ·
	//	            `auth.go` 8 · `config.go` 7 · `introspection_cache.go` 6 ·
	//	            профили зонта 11 · прочее 5 в трёх файлах) · пробы 289
	//	            (37 файлов)
	//	прирост 4   ведомость снятых ручек (`internal/retiredknobs`) 3 — снятые
	//	            адреса по имени; `pinned_hop_client.go` 1 — переименованный
	//	            файл, в котором осталась ручка якоря набора ключей (#2842)
	//
	// Служба доступа и фундамент не изменились (1107 и 0).
	//
	// #2792: 1646 = 1804 − 159 + 1. Потолок СНИЖЕН: снят читатель чужого
	// печенья сессии вместе с клиентом его службы сессий, ручкой её адреса и
	// пробами полосы. Слагаемые — по файлам, разностью перечней адресов двух
	// прогонов этого гейта (голова #2778 и это изменение):
	//
	//	убыль 159   производственное 67 (`kratos_session.go` 30 · `auth.go` 13 ·
	//	            `session_identity_handler.go` 7 · `main.go` 6 ·
	//	            `iam_subject_client.go` 4 · `deployment.yaml` 2 · по одной —
	//	            `values.yaml` края, `config.go`, `session_carrier_names.go`,
	//	            `auth_session_stepup.go`, `values.own.yaml`) · пробы 92
	//	            (19 файлов)
	//	прирост 1   запись ведомости снятых ручек (`internal/retiredknobs`) о
	//	            снятом адресе
	//
	// Служба доступа и фундамент не изменились (1107 и 0).
	//
	// #2778: 1804 = 1802 − 0 + 2. ПОТОЛОК ПОДНЯТ СОЗНАТЕЛЬНО, и причина одна —
	// ведомость снятых ручек края (`internal/retiredknobs`): две её записи
	// называют ручки, снятые #2815, по имени, и иначе записать «это имя снято и
	// не возвращается» нельзя. Ни одна привязка продукта не вернулась: обе ручки
	// не объявляет процесс и не эмитирует ни один рендер (двухколоночный гейт
	// края и рендерная проба цепочек). Служба доступа и фундамент не изменились.
	//

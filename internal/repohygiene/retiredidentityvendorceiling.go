...
var retiredVendorCeilings = map[string]int{
	// платформа: 1594 строки — по пути 18 · по имени 1526 · склейкой 1 · по
	...
	//
	// ПЕРЕСЧЁТ #2857 (сведение с веткой `2735` @ af6d21f7d65, суд прогонщика
	// целиком): 1594 = 1599 − 6 + 1. Потолок СНИЖЕН. Сборка стояла на 1599
	// (абзац ниже), ветка `2735` сняла от 1637 до 1632 одним изменением поверх
	// 7d6350855bd, который в сборке уже есть (её абзац «суд прогонщика
	// целиком» ниже), — это изменение и есть вся её дельта к сборке.
	// Прогонщиков сборка после 7d6350855bd не трогала, и мультимножество строк
	// `2735` (убыль 6, прирост 1) переходит на сведение целиком. Оба числа —
	// этим гейтом: 1599 на df432a42559 (число равно записи), 1594 — на
	// сведении. Код гейта тот же; служба доступа и фундамент не изменились
	// (1107 и 0). Разностью перечней адресов двух прогонов — ровно один файл:
	// `deploy/scripts/newman-parallel.sh` 16 → 11; остальные 214 адресов
	// совпали, файлов с привязками по-прежнему 215.
	//
	...
	// ниже): сборка дошла до 1601, ветка `2735` — до 1637 (её абзац «часть
	// #2841» ниже).
	...
	//
	// ПЕРЕСЧЁТ #2735 (суд прогонщика целиком): 1632 = 1637 − 6 + 1. Потолок
	// СНИЖЕН. Оба числа сняты этим гейтом в ветке `2735`: 1637 — на
	// 7d6350855bd (число равно записи), 1632 — на изменении поверх него. Код
	// гейта и проб тот же; служба доступа и фундамент не изменились (1107 и 0).
	// Снят мёртвый блок волны 4 в `deploy/scripts/newman-parallel.sh`: её
	// прогонщика в дереве нет, а адреса поставщика блок передавал безусловно,
	// мимо решения по посадке. Мультимножеством (файл · ось · текст без номера):
	//
	//	убыль 6     `deploy/scripts/newman-parallel.sh` — пять строк передачи
	//	            адресов волне и объявление ручки публичного порта с прежним
	//	            хвостом комментария о волне
	//	прирост 1   то же объявление без упоминания волны
	//
	// Строки комментариев, где имя ушло вместе с волной, в число не входят:
	// гейт их не судит. По файлам, разностью перечней адресов двух прогонов:
	// 16 → 11.
	//

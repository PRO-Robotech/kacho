...
var retiredVendorCeilings = map[string]int{
	// ПЕРЕСЧЁТ #1274 (приёмка F8, волна #2796): 1747 = 1802 − 77 + 22. Потолок
	// СНИЖЕН: церемонии входа, выхода и повышения ведёт консоль своими
	// экранами, клиент поставщика `shared/src/lib/kratos.ts` снят вместе с
	// четырьмя переэкспортами, контекст личности, окно повышения, кнопка
	// «Войти» каркаса и её близнец больше не называют поставщика, оснастка
	// подъёма уровня набора ведёт наши глаголы. Прирост 22 — пробы, судящие
	// поставщика НАСТОЯЩИМ входом: подсадка переписи обращений в F8-18
	// (`identity-ceremony.spec.ts` 4) и статические переписи прод-файлов и кода
	// набора (`console-provider-not-addressed.injection.test.ts` 11,
	// `e2e-provider-not-addressed.test.ts` 4, `provider-address-census.ts` 3).
	// Снято на ветке `1274` поверх `5d1d034719c` (там — 1802 этим же гейтом);
	// код гейта и проб тот же, служба доступа и фундамент не менялись.
	//
	// платформа: 1802 строк — по пути 23 · по имени 1729 · склейкой 1 · по

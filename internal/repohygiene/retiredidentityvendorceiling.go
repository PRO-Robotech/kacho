...
var retiredVendorCeilings = map[string]int{
	// платформа: 1799 строк — по пути 23 · по имени 1726 · склейкой 1 · по
	...
	//
	// ПЕРЕСЧЁТ #2842: 1799 = 1802 − 4 + 1. Потолок СНИЖЕН веткой `2842`, где
	// ручка якоря доверия хопа за ключами названа доменом края
	// (`KACHO_API_GATEWAY_JWKS_CA_FILE`, поле `JWKSCAFile`). 1802 снято этим
	// гейтом на основании ветки — сборке волны `2796` @ 87db6534a7a, 1799 — на
	// её голове после c92fa72884f. Код гейта и проб между ними тот же (разность
	// `retiredidentityvendorceiling*` пуста); служба доступа и фундамент не
	// изменились (1107 и 0: пины `go.mod` те же).
	//
	//	убыль 4     производственное 4, все по имени и по строке на файл:
	//	            прежнее имя ручки в `gateway/internal/config/config.go`
	//	            (поле с тегом) · `gateway/cmd/api-gateway/admin_hop_client.go`
	//	            (константа) · `gateway/cmd/api-gateway/main.go` (чтение
	//	            поля) · `gateway/deploy/templates/deployment.yaml` (имя
	//	            переменной среды)
	//	прирост 1   проба 1, по имени: `gateway/deploy/jwks_ca_render_test.go`
	//	            задаёт рендеру ключ профиля `hydra.jwksCa.secretName`. Ключ
	//	            профиля ветка не переименовывала, и проба рендера задаёт его
	//	            настоящим именем: проба с подставным ключом судила бы не тот
	//	            чарт, который ставится.
	//
	// Слагаемые — в единице гейта, мультимножеством (файл · ось · текст без
	// номера), как у пересчётов ниже. Контроль вторым выражением — разностью
	// перечней адресов, которые гейт печатает сам на двух прогонах (244 файла
	// на 245): те же −4 и +1, по строке в каждом из пяти файлов, и все пять
	// тронуты только c92fa72884f.
	//

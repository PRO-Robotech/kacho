...
var retiredVendorCeilings = map[string]int{
	// платформа: по пути 17 · по имени 1579 · склейкой 1 · по пути API 41 ·
	// двоичных 0 · архивов 3 — сумма 1641, ровно потолок ниже. Разбивка
	// СПИСАНА С ПЕРЕПИСИ, которую печатает сам гейт, и обязана сходиться с
	// потолком: слагаемое, оставшееся от прежнего замера, делает запись ложной
	// молча — сумма читается глазами, а глаз складывает то, что написано.
	// Здесь так и было: разбивка несла «по имени 1586» (сумма 1648) при
	// потолке 1647, потому что снижение на единицу прошло ИМЕННО по этому
	// слагаемому и в прозе не отразилось. Архивы: два чарта издателя по имени и
	...
	//
	// СНИЖЕНО 1648 → 1647 (−1) переименованием ПРОИЗВОДИТЕЛЯ якоря доверия хопа
	// за ключами: шаблон чарта края эмитил прежнее имя ручки, а читатель
	// переехал на новое раньше и получал пусто. Слагаемое — «по имени»
	// (1586 → 1585); прочие четыре не изменились.
	//
	// СНИЖЕНО 1647 → 1641 (−6) переводом ФИКСТУР инъекционных проб гейта
	// «посадка ↔ флаги чужой службы» на синтетическое имя компонента. Фикстура
	// была набрана именем снимаемого издателя, и это дефект дважды: такая
	// фикстура истекает ВМЕСТЕ со своим предметом — проба покраснела бы на
	// достижении собственной цели, — и вдобавок считается привязкой к издателю,
	// то есть проба о снятии сама двигала бы число вверх. Предикат инъекции
	// судит ПУСТОТУ РАДИУСА ведомости, а не чьё-то имя, и синтетическое имя ему
	// достаточно. Слагаемое — «по имени» (1585 → 1579).
	//
	// Почему всего одна строка, когда правка шире. Ведомости снятых и
	// переименованных ручек ПЕРЕЕХАЛИ из пробы в общий источник настроек — это
	// перенос, а не рост: столько же строк ушло из `f1d_*_test.go`, сколько
	// пришло в `renamed_knobs.go`. Новый гейт парности имён чужого продукта не
	// называет вовсе: ведомость он ВЫВОДИТ из объявления процесса, а не
	// выписывает вторым литералом — иначе снятие оплачивалось бы ростом числа,
	// которое оно же и снижает.
	//
	// Число измерено ЭТИМ гейтом на коммите правки.
	vendorTreePlatform: 1641,

...
func TestMembershipOracle_TheQuestionIsNotAskableOnThePublicSurface(t *testing.T) {
	...
	if !c.IDComputable {
		t.Errorf("предпосылка полосы C больше не верна: идентификатор членства "+
			"перестал выводиться из пары «человек:аккаунт» (%s не несёт %q). "+
			"Полоса C держалась ИМЕННО этим — пересмотрите её вместе с предпосылкой, "+
			"а не оставляйте по наследству",
			oracleMembershipIDMigration, oracleMembershipIDMark)
	}

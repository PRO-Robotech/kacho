...
var newmanSelftestExempt = map[string]string{
	// #1453: самопроверка registry красна СЕГОДНЯ — 21 находка, накопленная ровно
	// за то время, пока её никто не звал. Вставить вызов сейчас значило бы сделать
	// прогон суиты красным по причинам, к продукту отношения не имеющим: шестнадцать
	// находок судят утверждения кейсов, пять — дефект самой подделки (её `pm` беднее
	// настоящего прогонщика, и её вердикт по этим шагам не является вердиктом).
	// Запись истекает сама: появится вызов — покраснеет она.
	"services/registry": "самопроверка красна, вызов сделал бы прогон суиты красным не по продукту (#1453)",
}

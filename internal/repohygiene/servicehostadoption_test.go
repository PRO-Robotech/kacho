...
//  2. ОСНОВАНИЕ решения — факт о носителе, на котором решение стоит. Оно может
//     стать ложным, ПОКА предмет жив: край продолжает собирать сервер сам,
//     находки на месте, запись выглядит действующей, — а основание уже неверно.
//     Здесь «нет находок» молчит by construction.
//
// Поэтому основание пинится ОТДЕЛЬНОЙ пробой над тем, чем оно держится
// (`edgecontourdecisionbasis_test.go` — у носителя нет ни поля обработчика
// неизвестной службы, ни поля мультиплексора). Без неё самоистечение было бы
// объявлено и наполовину не исполнено — тот самый класс, который этот гейт ловит
// в чужом коде.
//
// ЗДЕСЬ БЫЛА ВТОРАЯ ЗАПИСЬ-РЕШЕНИЕ — служба доступа (`iam`), оставленная вне
// контура решением владельца 2026-08-10. Она снята вместе со своим предметом:
// каталога `services/iam` в индексе больше нет, служба вынесена отдельным
// продуктом, и композиционного корня, про который решение принималось, этот обход
// не видит. Снята и проба её основания. Решение владельца при этом НЕ отменено —
// оно просто перестало относиться к этому дереву; отменять его вправе только
// владелец, и в новом дереве службы оно будет предметом её собственного гейта.

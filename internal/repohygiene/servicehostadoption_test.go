...
var hostAdoptionExceptions = map[string]hostAdoptionException{
	...
	"iam": {
		...
		basis: "носитель ставит ОДНО звено решения о доступе (`decisionLink` в pkg/servicehost " +
			...
			"держит ПЯТЬ, и ни один картой не выражается: анти-аноним · пер-RPC политика " +
			"вызывающего на ОБОИХ слушателях · этаж system_viewer на читающих RPC · этаж step-up " +
			"· приложение машиночитаемой причины отказа. Полей интерсепторного типа в " +
			"servicecontract.Spec нет НАМЕРЕННО — в этом суть пакета («цепочку невозможно " +
			"принести, поэтому её невозможно собрать по-своему»), — значит внести эти пять в " +
	...
	"registry": {kind: adoptionPending, why: "переводится вслед за первым"},
}

...
var foreignIDPNameLedger = []ForeignIDPNameLedgerEntry{
	// Записи `gateway/` нет: её `Until` исполнен (#2734). Край больше не
	// разговаривает с поставщиком ни по одной полосе, и объявленных имён этой оси
	// в `gateway/**` обход не находит ни одного. Вернувшееся имя краснеет здесь
	// как рост области без записи.
	{
		Area: "deploy/",

...
var foundationClasses = map[string]foundationClass{
	...
	"quota":           classCorelib,
	// refusal — словарь полос отказа на СНЯТИЕ ресурса. Класс ПЛАТФОРМЫ, и это
	// решение, а не соседство: сегодня все четыре полосы производит только она,
	// а служба доступа отказывает своими текстами («account is not empty»,
	// «role is in use by access bindings») и признака пока не несёт.
	//
	// Класс фундамента здесь отвергнут ЗАМЕРОМ, а не вкусом: каталогов класса
	// corelib под `pkg/` этого дерева не осталось ни одного, переезд объявлен
	// завершённым, и ось прозы о переезжающих путях на этом закрылась. Запиши
	// сюда classCorelib — и она возобновится с ведомостью, у которой восемь
	// записей потеряли предмет: новый каталог открыл бы чужой закрытый долг.
	//
	// Предикат пересмотра ВНЕШНИЙ: служба доступа начинает производить те же
	// токены. Тогда потребителей двое, класс становится фундаментом, и пакет
	// ПЕРЕЕЗЖАЕТ туда выпуском версии — копия в двух деревьях запрещена
	// (ban #20), поэтому «оставить обе» исходом не является.
	"refusal":         classKacho,
...
var foundationSubtrees = []struct {
	...
	// Переопределений под `pkg/api/kacho/cloud/…` здесь БОЛЬШЕ НЕТ ни одного.
	// Три контракта фундамента носили там имя платформы — форма учёта, форма
	// подписки и форма операции (все три — kacho#2601); каждый переименован под
	// дом фундамента (`corelib.quota.v1`, `corelib.subscription`,
	// `corelib.operation`) и лёг под нейтральный корень, то есть класс им
	...
	// Запись `pkg/quota/quotaiam` снята вместе со своим предметом: клиент к
	// авторитету величин службы доступа убран, каталога в дереве нет. Держатель
	// карты (`TestEveryDeclaredPathPrefixHasASubjectInTheTree`) назвал её
	// приставкой без предмета в тот же прогон — объявление, пережившее свой
	// каталог, есть слепая зона: класс оно раздаёт, а проверить его нечем.
	...
	// Величины потолков, объявленные ПОСАДКОЙ домена, — ПЛАТФОРМА по тому же
	// правилу и тому же счёту, что сосед выше: потребителей пять, и все пять —
	// службы платформы; служб доступа среди них нет ни одной. Об объявленных
	// величинах фундаменту знать неоткуда — каталог видов принадлежит домену.
	//
	// Объявляется ЯВНО по той же причине: родительский каталог назван классом
	// фундамента, и без записи каталог унаследовал бы его МОЛЧА, попав в
	// перечень переезжающих вместе с модулем, которому не принадлежит.
	{"pkg/quota/quotaceiling", classKacho},

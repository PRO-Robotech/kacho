...
// `internal/apps/kacho/api/<resource>/`. Второй раскладки нет — ни как
...
// `internal/apps/kacho/api/`): geo 2, vpc 8, storage 4, nlb 7, registry 1,
...
//
//   - `internal/apps/kacho/services/` у vpc (addressref / networkinternal /
...
// файлов вне пакета — почти все `apps/kacho/api/*` и все pg-адаптеры. Разложить
// это по домам — решение о декомпозиции (какой use-case чей; что из общего
// уходит в `apps/kacho/shared`), а не переименование каталога, и делать его
...
//     `internal/apps/kacho/api/<resource>/` (перекладка, не рефакторинг: имена
...
//     ресурсом) -> он живёт ВНУТРИ `apps/`, как `apps/kacho/services/` у vpc, и
...
func TestUseCaseLayerHasOneLayout(t *testing.T) {
	...
	if len(hits) > 0 {
		t.Errorf("найдена вторая раскладка слоя бизнес-логики — каталог слоя вне "+
			...
			"`internal/apps/kacho/api/<resource>/`:\n  %s\n\n"+
			"Исходы: переложить в `apps/kacho/api/<resource>/` (если это use-case) / "+
			"внести внутрь `apps/` с честным godoc (если это не-resource service слоя "+
			"apps, как `apps/kacho/services/` у vpc) / перенести в `internal/domain/` "+
...
// `internal/apps/kacho/api/`. Пока это так, «второй дом» ни для чего не нужен.
...
func TestUseCaseLayoutPremiseHolds(t *testing.T) {
	...
	for _, svc := range serviceDirs(t, root) {
		...
		if err != nil {
			t.Fatalf("%s: нет `internal/apps/kacho/api/` (%v). Предпосылка запрета "+
		...
		if pkgs == 0 {
			t.Fatalf("%s: `internal/apps/kacho/api/` есть, но ни одного пакета с .go "+

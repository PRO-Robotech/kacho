...
// (`gs "…/corelib/grpcsrv"`) пишется так же коротко, и разбор по имени обойти можно
...
//  2. **точечный импорт** владельца (`import . "…/corelib/grpcsrv"`) — вызов пишется
...
const (
	...
	//
	// Предмет живёт в пакете `grpcsrv` общего фундамента
	// (`github.com/PRO-Robotech/corelib`), и дерево этого каталога больше не
	// несёт: префикс "corelib/" метит синтетический путь, которым
	// `corelibPackageGoFiles` называет файлы, прочитанные из кэша модулей.
	TrustDomainOwnerDir = "corelib/grpcsrv/"

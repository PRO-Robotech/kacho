...
var nameFormOtherReferents = map[string]string{
	// ПЕРЕЧЕНЬ ПУСТ, и это цель, а не поломка.
	//
	// Единственная запись — `/iam/v1/roles` («идентификатор роли, а не
	// косметическая метка») — снята вместе со своим предметом: служба доступа
	// вынесена отдельным продуктом, её коллекций в `tests/newman/` нет ни одной,
	// и освобождать запись стало нечего. Снятия потребовала эта же проба
	// (`освобождение ... не освободило ни одного шага`) — самоистечение
	// сработало ровно как задумано.
	//
	// Заводя новую запись, назови, ЧЕМ имя этого ресурса судится вместо канона:
	// каждая запись здесь — слепая зона, выданная вперёд.
}
...
// попала.
//
// Прокси пережил свой предмет ровно так, как проксям и положено. Цепь iam была
// сведена в одну первичную миграцию: форма на месте, файла нет — и iam молча
// выпал из перечня. Наблюдалось это не как «сервис не проверен», а с другого
// конца и непохоже: освобождение `/iam/v1/roles` перестало освобождать хоть
// один шаг, потому что коллекции iam вообще не читались, и гейт объявил
// находкой ЕГО — запись, у которой предмет был на месте всё это время.
//
// Сегодня у той записи предмета нет уже по-настоящему: служба доступа вынесена
// отдельным продуктом вместе со своими коллекциями, и перечень освобождённых
// пуст (см. `nameFormOtherReferents`). Разбор оставлен потому, что он называет
// СПОСОБ ошибиться — судить сервис по имени файла миграции вместо содержимого
// схемы, — а способ пережил свой экземпляр.
//
...
//
// Перечень освобождений приезжает ПАРАМЕТРОМ, а не читается из пакета. Довод
// тот же, что у самого вынесения ядра: доказательство, привязанное к ЖИВОЙ
// записи ведомости, истекает вместе с ней — и истекает молча, объявляя находкой
// не дефект, а собственную опустевшую фикстуру. Наблюдалось ровно это: запись
// `/iam/v1/roles` ушла вместе со службой доступа, и близнец, доказывавший
// РАБОТУ механизма освобождения, покраснел, хотя механизм цел.
func judgeFixtureNames(rel string, c pmCollection, spare map[string]string) (
	...
	walkItems(c.Item, func(it pmItem) {
		...
		if ref := otherReferent(it, spare); ref != "" {
			spared[ref]++
...
func otherReferent(it pmItem, spare map[string]string) string {
	if it.Request == nil {
		return ""
	}
	url := strings.TrimSpace(rawURL(it.Request.URL))
	for path := range spare {
...
func TestFixtureNamesObeyTheCanonWhereTheServiceMigrated(t *testing.T) {
	...
	for svc := range migrated {
		...
		err := rootedWalk(base, func(rel string) bool {
			...
			f, withName, j, sp := judgeFixtureNames(filepath.ToSlash(rel), c, nameFormOtherReferents)

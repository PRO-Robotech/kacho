...
func TestClientDocsDoNotPresentDeprecatedVerbsAsCurrent(t *testing.T) {
	...
	// пометки к снятию стоят в контракте службы доступа (предикат:
	// `git grep -ln 'deprecated = true' -- 'proto/**/*.proto'` → три файла, все
	// под её каталогом контрактов), а её САЙТ документации уехал вместе с самой
	// службой (`git ls-tree -d --name-only HEAD services/iam/docs` резолвился до
	// выноса, сегодня каталога нет). Контракт остался здесь, страницы — нет,
	// поэтому сверять помеченный глагол не с чем: ноль сверенных блоков есть
	// ФАКТ о дереве, а не непрочитанное.
	//

...
var retiredRPCSurface = []RetiredRPC{
	...
	},
	{
		...
		Reason: "то же объявление в домене vpc — без сервера, клиента и неgenerated-ссылок. " +
			...
			"ЗАПИСЬ ОСТАЁТСЯ В СИЛЕ, хотя доставка намерения датаплейну у vpc теперь ЕСТЬ: " +
			"она заведена ДРУГИМ именем — InternalDataplaneService/WatchIntent, — и это не " +
			"косметика. Снятое имя обещает фид ЖИЗНЕННОГО ЦИКЛА (что случилось с ресурсом: " +
			"создан, удалён, переехал); новый метод отдаёт ЖЕЛАЕМОЕ СОСТОЯНИЕ целиком — полную " +
			"выдачу от нуля, ревизию на каждом объекте, снятие намерения и отдельный исход " +
			"«твоя ревизия слишком стара». Ни одного из этих свойств событийный фид не несёт, " +
			"поэтому занять его имя значило бы вернуть имя под чужой замысел — ровно то, что " +
			"надгробие и запрещает. Читателю, который снова захочет этот фид: сперва спроси, " +
			"нужен ли тебе поток СОБЫТИЙ, или ты ищешь уже существующий поток НАМЕРЕНИЯ",
	},
	{
	...
	// `kacho_iam.fga_outbox`, на котором стоит проекция `relation_fact`
	...
	// на журнал»: перевод дал бы работу без выгодоприобретателя.
	{
		...
		Reason: "административная запись набора кортежей мимо журнала. Вызывающих ноль; " +
			...
			"на самом деле пишет в движок своим дренажом, а не этим RPC. Имя не должно " +
			"вернуться молча: следующему, кому понадобится массовая правка отношений, " +
			"придётся сперва объяснить, почему её не выразить строкой журнала",
	},
	{
		...
		Reason: "синхронная запись кортежа создателя мимо журнала. Все пять соседей ушли " +
			...
			"RegisterResource/UnregisterResource: намерение ложится в журнал, применение " +
			"остаётся ускорителем",
	},
	{

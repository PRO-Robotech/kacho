...
// Написание его лежит в коде, СВОЕЙ КОНСТАНТОЙ У ФУНДАМЕНТА.
//
// Прежде оно жило ДВАЖДЫ — своей константой у фундамента и своей у службы
// доступа, потому что модуля было два и общей константы у них нет by
// construction. Служба вынесена отдельным продуктом; объявление в дереве
// осталось одно, и вместе со вторым ушла проба их согласия (разбор — в
// `clusteranchorliteral_test.go`). Эта половина свойства от числа модулей не
// зависит и остаётся в силе при любом их числе.
//
// Пока каждое обращение идёт через объявленную константу, переход написания —
// правка объявления. Место, повторившее строку РУКОЙ, из этой арифметики

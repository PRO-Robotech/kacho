...
var shortGatedOutsideSelection = []string{
	...
	// Сервер потока подписки. Приехал сюда 2026-09-13 вместе с контрактом
	// (kacho#2601) из общего фундамента, где его интеграционные пробы гонял
	// конвейер ФУНДАМЕНТА. В этом дереве шага для них нет: отбор интеграционной
	// джобы смотрит внутрь `services/<svc>/internal/(repo|clients|reconciler|
	// subscriptionjournal)`, а пакет лежит в `pkg/`.
	//
	// Это ДОЛГ С ИМЕНЕМ, а не решение: пробы настоящие (Postgres, журнал,
	// пробуждение каналом), и сегодня в конвейере платформы они не исполняются
	// НИГДЕ. Закрывается не отсюда — расширением отбора либо своим шагом, то
	// есть правкой `.github/`, которая этой записи не принадлежит. Запись
	// истекает сама: перестанет пакет гейтить пробы кратким режимом или войдёт
	// в отбор — гейт назовёт её как освобождение без предмета.
	"pkg/subscription",
}
...
// находкой.
// ЗДЕСЬ СТОЯЛО ШЕСТНАДЦАТЬ ЗАПИСЕЙ СЛУЖБЫ ДОСТУПА, и все они сняты ВМЕСТЕ СО
// СВОИМ ПРЕДМЕТОМ: служба вынесена отдельным репозиторием, каталога
// `services/iam` в дереве нет. Освобождению стало нечего освобождать, а запись
// без предмета достаётся следующему читателю слепой зоной — она выглядит
// действующей и не сработает никогда.
//
// Шесть из них жили в перечне долга выше, десять — здесь; вместе с ними ушли и
// строки запуска, называвшие цели, которых в дереве больше нет. Пакеты никуда
// не делись: их гоняет теперь конвейер их собственного репозитория.
// ЗДЕСЬ СТОЯЛИ ЗАПИСИ ФУНДАМЕНТА, и все они сняты ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ:
// каталоги уехали в общую библиотеку (задача #2131), пакетов с такими путями в
// дереве больше нет. Освобождению стало нечего освобождать, а запись без
// предмета достаётся следующему читателю слепой зоной: она выглядит
// действующей и не сработает никогда.
//
// Из перечня долга ушло 11 записей, из перечня своего шага — 4. Ни одно
// свойство при этом не перестало проверяться ЗДЕСЬ: оно и не проверялось здесь,
// предмет был чужой. Кто его проверяет теперь — конвейер общей библиотеки, и
// пока такой полосы у неё нет, это остаток с предикатом (см. корневой Makefile,
// абзац о снятых записях PG_OUTSIDE_SELECTION_PKGS).

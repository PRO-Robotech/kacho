...
var (
	jcOrderAsc = regexp.MustCompile(`(?is)ORDER\s+BY\s+([A-Za-z_0-9.%\[\]]+)\s+ASC`)
...
var jcOpaqueTypes = []string{"text", "uuid", "varchar", "char", "character", "citext"}
...
func jcClassify(text string, schema map[string]string) (jcResumableRead, bool) {
	om := jcOrderAsc.FindStringSubmatch(text)
	if om == nil {
		return jcResumableRead{}, false
	}
	col := om[1]
	if !regexp.MustCompile(regexp.QuoteMeta(col) + `\s*>\s*\$\d`).MatchString(text) {
		return jcResumableRead{}, false
	}
	...
	r := jcResumableRead{
		Column:  col,
		Bounded: regexp.MustCompile(regexp.QuoteMeta(col) + `\s*<=?\s*\$\d`).MatchString(text),
	}

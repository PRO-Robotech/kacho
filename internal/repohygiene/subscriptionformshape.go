...
//
// «Ни одна ось не обязательна» он тоже НЕ проверяет: в proto3 обязательных
// полей нет by construction, и такая проверка не могла бы упасть никогда.
// Проверка, неспособная упасть, хуже отсутствующей — она занимает место и

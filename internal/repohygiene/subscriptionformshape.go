...
func AuditSubscriptionFormShape(
	...
	//
	// # Предмет утверждения ПЕРЕПРИВЯЗАН (kacho#1018), а не снят
	//
	// Прежде оно стерегло ПОРЯДОК ФАЗ: форма обязана быть утверждена до того, как
	// под неё пишут сервер. Порядок соблюдён и пройден — глагол объявлен, сервер
	// написан, — то есть тот предмет исчерпан, и утверждение, оставленное с
	// прежним объяснением, стало бы описывать не то, что охраняет.
	//
	// Живой предмет — ПЕРЕНОСИМОСТЬ ФОРМЫ, и он тот же, что у соседнего
	// утверждения ниже: глагол несёт аннотации каталога прав, то есть зависимость
	// от контракта другого домена. Лежи он в файле формы, форма перестала бы
	// импортироваться кем угодно, и «зависимости от домена нет» держалось бы
	// ровно до первой аннотации. Поэтому глагол живёт в СОСЕДНЕМ файле того же
	// пакета, а этот файл остаётся тем, что берут все.
	c.Assertions++
	if len(f.Services) > 0 || len(f.RPCs) > 0 {
		add("verb-declared", 0,
			fmt.Sprintf("в файле ФОРМЫ объявлено служб %v и глаголов %v. Глагол несёт "+
				"аннотации каталога прав, то есть зависимость от контракта другого домена; "+
				"в файле формы он делает форму недоступной тому, у кого этой зависимости "+
				"нет — и рядом появится вторая. Место глагола — соседний файл того же "+
				"пакета (`subscription_service.proto`)", f.Services, f.RPCs))
	}

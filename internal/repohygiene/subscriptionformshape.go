...
// проверяемые свойства перечислены ниже. Отдельный разбор для каждого свойства
// повторял бы чтение того же файла и проверку тех же предпосылок. Общий разбор
// даёт проверкам одну форму, с которой они работают.
//
// # Что он утверждает
//
...
//	НОСИТЕЛЬ — ВЫБОР         форма объявляет ветвление ровно из двух ветвей;
//	                         заполненность конкретного сообщения не проверяется.
...
//
// # Единицы счёта: вид находки, место внесения и утверждение в прогоне
//
// Вид — значение Kind; место внесения — отдельный вызов add в анализаторе.
// SubscriptionShapeCensus.Assertions считает проверки при конкретном разборе
// формы, в том числе внутри циклов. Это разные единицы, и счётчик прогона не
// является числом видов или мест в исходнике.
//
// Есть виды, вносимые разными ветвями:
//
...
//
// Различие вида и места существенно: проба,
...
//
// Поэтому эти ветви различаются в инъекции ТЕКСТОМ (`requireKindSaying`):
// TestSubscriptionShapeCarrierCanFail и TestSubscriptionShapeAnchorCanFail
// требуют причину именно внесённого дефекта. Остальные виды вносятся одним
// местом каждый и проверяются по виду. Соответствие каждому виду отдельной
// пробе сверяется чтением её утверждений и результатом исполнения.
//
// Команды ниже исполняются в Bash из корня репозитория. Места и виды считаются
// отдельно. Предикат вызова анкерён НА НАЧАЛО СТРОКИ: он читает вызов в коде,
// а цитату `add(` в комментарии, включая собственное объяснение, пропускает.
// Сопоставление видов с литералами проб показывает отсутствующее имя, но не
// заменяет чтения проверочных утверждений и фактического прогона.
//
//	cd internal/repohygiene
//	grep -cE '^[[:space:]]+add\("' subscriptionformshape.go                    # мест внесения
//	grep -oE '^[[:space:]]+add\("[a-z-]*"' subscriptionformshape.go |
//		grep -o '"[a-z-]*"' | sort -u | wc -l                             # видов находок
...
type SubscriptionShapeExpectation struct {
	...
	// Здесь стояло поле `MandatoryOption` — опция, делавшая ось обязательной.
	// Утверждение снято ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ (задача #1255): семейство
	// расширений `kacho.cloud.validation`, которым обязательность в контрактах
	// выражалась, снято целиком — исполнителя на пути запроса у него не было ни
	// одного. Помечать ось обязательной больше НЕЧЕМ: файл, объявлявший опцию,
	// удалён, и `[(required) = true]` теперь не компилируется вовсе.
	//
	// ПОЧЕМУ СНЯТО, А НЕ ОСТАВЛЕНО «НА ВСЯКИЙ СЛУЧАЙ». Утверждение было
	// НЕГАТИВНЫМ («ни одна ось не вправе быть помечена обязательной»), и в этом
	// вся разница. Позитивная проверка на исчезнувшем предмете КРАСНЕЕТ и зовёт к
	// себе; негативная — ЗАМОЛКАЕТ: вход, на котором она находит нарушение,
	// перестаёт быть представимым, ветвь остаётся в коде, счётчик утверждений
	// продолжает расти, а находка становится непроизводимой ни при каком входе.
	// Молчание такой проверки неотличимо от исправности. Наблюдалось прямо здесь:
	// снятие семейства не покраснило этот файл ничем.
	//
	// САМО СВОЙСТВО НЕ ОТМЕНЕНО и держится теперь построением, а не проверкой:
	// обязательной ось сделать нечем. Появится стандартная аннотация
	// обязательности (`google.api.field_behavior` вендорен в дереве и не
	// употребляется ни разу) — утверждение возвращается сюда вместе с ней, и
	// способность падать доказывается инъекцией заново.
}
...
func AuditSubscriptionFormShape(
	...
	if okReq {
		...
		for _, fl := range req.Fields {
			...
			c.Axes++
			// Здесь стояло утверждение «ось не помечена обязательной». Снято вместе
			// со своим предметом — см. разбор у снятого поля `MandatoryOption` выше.
			// Счётчик утверждений уменьшен на единицу СОЗНАТЕЛЬНО: оставить его
			// растущим значило бы отчитываться о проверке, которой нет.
			c.Assertions++
	...
	if okEvent {
		...
		if _, ok := event.Oneof(e.CarrierOneof); !ok {
			add("carrier-not-a-choice", event.Line,
				"носитель нагрузки не выражен ветвлением `"+e.CarrierOneof+"`. Форма обязана объявлять выбор: "+
					"состояние ЛИБО признак, что состояния нет. "+
					"Заполненность конкретного сообщения этот анализатор "+
					"не проверяет")
		} else {
			...
			if len(branches) != 2 {
				add("carrier-not-a-choice", event.Line,
					fmt.Sprintf("ветвление носителя несёт %d ветв(ей), а обязано ровно две: "+
						"состояние ЛИБО признак, что состояния нет. Здесь "+
						"проверяется число объявленных ветвей, а не заполненность "+
						"конкретного сообщения", len(branches)))
			}
			c.Assertions++

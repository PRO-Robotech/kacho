...
// limitexportprocedure_injection_test.go — доказательство, что гейт процедуры
...
const injectedGuide = "services/iam/INSTALL.md"
...
func TestExportProcedureGate_SecondSpellingIsAFinding(t *testing.T) {
	...
	_, findings := JudgeExportProcedure(g, required())
	requireExportFindingNames(t, findings, injectedGuide, "kaname.limits")
}

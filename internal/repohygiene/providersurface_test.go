...
var providerSurfaceLedger = []ProviderLedgerEntry{
	{
		...
		Why: "ОБРАЗЕЦ значения в декларативном перечне обязательных величин, из которого " +
			...
			"и образец без пути заставил бы его достраивать путь догадкой. Это не разговор " +
			"с поставщиком — здесь нет вызова, — но поверхность к нему описывает, и потому " +
			"названо здесь, а не спрятано",
		Until: "полоса external снята: величина живёт под Lanes external, и вместе с полосой " +
			"исчезает сама запись перечня — тогда исчезнет и этот образец",
	},
	{

...
// ИСТЕКЛИ вместе со своим предметом (#2734): ни один файл прод-кода края больше
// не говорит с поставщиком по его API, потому что самих полос нет. Гейт назвал
// все три поимённо (ProviderFindingStale) — требование «ведомость обязана
// СОКРАЩАТЬСЯ» исполнилось до нуля. Прежде так же истекли четыре записи файлов
// `services/iam/…`: служба доступа вынесена отдельным продуктом.
//

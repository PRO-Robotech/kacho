...
var providerSurfaceLedger = []ProviderLedgerEntry{
	...
	// ЗДЕСЬ БЫЛИ ЧЕТЫРЕ ЗАПИСИ файлов `services/iam/…` — они ИСТЕКЛИ вместе со
	// своим предметом, а не были вычеркнуты: служба доступа вынесена отдельным
	// продуктом, и ни один из этих файлов в дереве больше не говорит с
	// поставщиком, потому что самих файлов нет. Гейт назвал все четыре поимённо
	// (ProviderFindingStale), то есть требование «ведомость обязана СОКРАЩАТЬСЯ»
	// исполнилось ровно так, как задумано.
	//
	// Три записи края остались и служат положительным контролем: ведомость не
	// опустела, поэтому молчание гейта не вакуумно, а рост поверхности
	// по-прежнему есть чем поймать.

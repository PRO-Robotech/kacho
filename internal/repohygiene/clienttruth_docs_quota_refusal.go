...
func collectDocsQuotaRefusal(tree *treecorpus.Tree, owners []string) (docsQuotaCensus, error) {
	...
	// сегменту пути.
	//
	// ОБХОД ИДЁТ ПО ВСЕМУ `proto/` ЭТОГО ДЕРЕВА, а не по одному его корню. Корней
	// в нём было два — платформа объявляла свои контракты в `proto/kacho/…`, а
	// служба доступа в `proto/kaname/…`, и служба чтения её квот переехала во
	// второй (kacho#2362, решение `Д9`). Разбор, знающий один корень, второго не
	// видит ВОВСЕ — и это не красное, а молчание: владелец учёта выпадает из
	// перечня путей, а гейт объявляет его предпосылку неверной, хотя неверен
	// разбор.
	//
	// Корень здесь не перечисляется поимённо намеренно: перечень устарел бы при
	// следующем переезде так же молча, как устарел прежний единственный корень.
	//
	// ЧТО ИЗМЕНИЛОСЬ ПОСЛЕ ВЫНОСА СЛУЖБЫ, названо прямо: решением владельца
	// kacho#2616 (исход C, 2026-09-13) контракты службы доступа уехали в её
	// репозиторий, и под `proto/` этого дерева корня `kaname` БОЛЬШЕ НЕТ —
	// утверждение «служба доступа объявляет контракты в `proto/kaname/…` этого
	// дерева» ОТМЕНЕНО. В обход он не возвращается, и это не слепая зона: гейт
	// сверяет ПАРУ — поверхность чтения из контракта и страницу сайта, которая её
	// называет, — а сайт службы (`services/iam/docs`) уехал тем же решением
	// (`git ls-files services/iam` → 0). Обе стороны пары ушли вместе, поэтому
	// предмета у этой полосы в этом дереве нет; читать контракт из модуля значило
	// бы требовать страницу, которой здесь тоже не бывает. Сверку ведёт теперь
	// дерево службы.
	byHead := map[string][]string{}
	for _, rel := range clientTruthTreeFiles(tree, "proto", true, ".proto") {

...
func collectDocsQuotaRefusal(tree *treecorpus.Tree, owners []string) (docsQuotaCensus, error) {
	...
	// сегменту пути.
	//
	// ОБХОД ИДЁТ ПО ВСЕМУ ДЕРЕВУ КОНТРАКТОВ, а не по одному его корню. Корней
	// стало два: платформа объявляет свои контракты в `proto/kacho/…`, а служба
	// доступа — в `proto/kaname/…`, и служба чтения её квот переехала во второй
	// (kacho#2362, решение `Д9`). Разбор, знающий один корень, второго не видит
	// ВОВСЕ — и это не красное, а молчание: владелец учёта выпадает из перечня
	// путей, а гейт объявляет его предпосылку неверной, хотя неверен разбор.
	//

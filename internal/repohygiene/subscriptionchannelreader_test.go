...
const (
	...
	// подписки. Предмет ВЕРНУЛСЯ в дерево платформы (kacho#2601): контракт
	// подписки несёт платформенное имя, служба его не зовёт вовсе, и сервер
	// уехал вместе с контрактом. Закреплённый фундамент при этом читается
	// по-прежнему — чтение оттуда попадает в «прочее», и пока пин не поднят,
	// перепись честно показывает вторую копию, не выдавая её за владельца.
...
func TestChannelIsReadByTheStreamServerAlone(t *testing.T) {
	...
	// Закреплённый фундамент читается и после возврата сервера в дерево: пока
	// пин не поднят, его копия ещё приезжает модулем, и перепись обязана её
	// ПОКАЗАТЬ — в полосе «прочего», а не владельцем. Перестань мы её читать —
	// «сервера в фундаменте больше нет» стало бы утверждением, которое ничем
	// не проверяется.
	...
	if len(findings) > 0 {
		t.Errorf("владелец домена читает канал сам — %d место(а):\n  %s\n\n"+
			...
			"Снятие: отдать журнал через github.com/PRO-Robotech/kacho/pkg/subscription либо, если предмет иной подсистемы, "+

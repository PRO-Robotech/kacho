...
//
// Почему именно эти деревья: их правят соседние сеансы того же свипа, и
...
//
// Запись службы доступа снята вместе со своим предметом (вынос `services/iam`
// отдельным продуктом): её тестового дерева в индексе больше нет, поэтому число
// стало утверждением о том, чего не существует. Двусторонний храповик это и
// показал — «закрытий 0 при записанных 456».

...
//
// ЧТО ИЗМЕРЕНО на ревизии bdafe2c4: 294 строки каталога, все до одной резолвятся в
// существующий метод контракта (то есть ни одна не пережила снятие метода), и ПЯТЬ
// из них называют метод, чей сервис не смонтирован ни в одном композиционном корне.
// Ни один гейт дерева на этом не падал.
package repohygiene
...
var knownInertCatalogRows = []string{
	...
	"kacho.cloud.iam.v1.InternalIamHooksService/RefreshTokenHook",
	"kacho.cloud.iam.v1.InternalIamHooksService/TokenHook",
...
//
// Гоняется с ПУСТЫМ списком исключений, поэтому проверяет сразу две вещи: что
// анализатор находит инертные строки на настоящем дереве (иначе зелёный вердикт
// теста выше получен даром), и что таких строк ровно столько и именно те, что
// перечислены. Появилась новая — она здесь и покажется, с именем.
func TestCatalogReachability_InertRowsAreExactlyTheAllowedServices(t *testing.T) {
	findings, _, err := AuditCatalogReachability(catalogReachabilityOptions(t, nil), nil)
	if err != nil {
		t.Fatalf("анализатор не отработал: %v", err)
	}
	...
	if len(got) == 0 {
		t.Fatalf("с пустым списком исключений анализатор не нашёл НИ ОДНОЙ инертной строки — "+
			"он не способен их находить, и зелёный вердикт основного теста ничего не значит "+
			"(ожидалось %d)", len(want))
	}
	if strings.Join(got, "\n") != strings.Join(want, "\n") {

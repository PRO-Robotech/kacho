...
// Служба доступа вынесена отдельным продуктом; её композиционный корень уехал
// вместе с каталогом, а контракт остался в этом дереве (перевести внешних
// импортёров стабов на внешний модуль нельзя, пока у службы нет опубликованных
// выпусков). Отсюда: каждая строка каталога прав, называющая её метод, объявляет
// полосу для вызова, который до края этого дерева не дойдёт.
//
// Перечень ТОЧНЫЙ и истекает сам: строка ушла — метод смонтировали, снимайте
// запись; строка появилась — контракт объявил полосу для метода, которого никто
// не обслуживает. Когда контракт уедет вслед за реализацией, инвентарь опустеет
// целиком, и гейт скажет об этом первым.
var knownInertCatalogRows = []string{
	...
	"kaname.cloud.iam.v1.InternalLimitService/ListChangedSince",
	"kaname.cloud.iam.v1.InternalLimitService/Resolve",

...
//
// ОБЛАСТЬ действия текста — вторую половину того же параметра `Licensed Work`,
// отвечающую на вопрос «на что», а не «что». Её судит отдельный держатель
// [TestNoLicenseClaimsTerritoryGovernedByAnother] (licensescope_test.go). Граница
// названа не умозрительно: оба утверждения этого гейта были зелены, пока
// восемь файлов из двенадцати объявляли своей областью весь репозиторий,
// включая каталоги под Apache-2.0 и AGPL-3.0 (kacho#2161).
//
// И человекочитаемое имя перед скобкой («Kachō Network Load Balancer»). Судить

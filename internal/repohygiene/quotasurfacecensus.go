...
//
// ПРИСТАВКА НИЧЕГО НЕ ИСКЛЮЧАЕТ В ЭТОМ ДЕРЕВЕ: служба вынесена отдельным
// продуктом, `git ls-files services/iam` даёт 0. Оставлена она НАМЕРЕННО и
// только в поле `NotScope` — то есть как ВЫЧЕТ, а не как популяция: вычет,
// которому нечего вычитать, вердикта не меняет, а снятие его сделало бы границу
// B/C неверной в тот день, когда каталог службы в этом дереве появился бы снова.
// Правил, чей ПРЕДМЕТ был назван этой приставкой, в перечне больше нет — они
// сняты вместе с ним (A6 и C1, разбор при каждом), и именно их отсутствие, а не
// наличие этой строки, есть проверяемое «послабление истекло».
...
var quotaCensusRules = []quotaCensusRule{
	// ---------- A. авторитет величин ----------
	// A1 СНЯТО вместе с предметом: объявления службы величин не осталось ни в
	// одном дереве — контракт снят у владельца (`kacho#2117`), край перестал его
	// регистрировать подъёмом пина. Перепись назвала правило не совпавшим ни с
	// одним кандидатом в тот же прогон: правило, которому нечего относить, есть
	// послабление без ответственного, и следующий читатель унаследовал бы его
	// как описание действительности.
	//
	// Соседи по поверхности ОСТАЮТСЯ и предмет имеют: A2 — таблица величин
	// службы, A3 — право чтения и группа держателей. Поверхность жива, исчезло
	// одно её проявление, и снимается ровно оно.
	{
	...
	// Здесь стояло правило A6 («реализация величин в службе доступа и её
	// контракт») — пять приставок: контракт `proto/kaname/cloud/iam/v1/limit*`,
	// слой прикладной логики, тип домена, хранилище и страница документации в
	// `services/iam`.
	//
	// Снято ВМЕСТЕ С ПРЕДМЕТОМ — тем же основанием, что A8 ниже и C1 на
	// поверхности C, и по прямому предикату: `git ls-files services/iam` даёт
	// 0, `git ls-files proto/kaname` даёт 0, то есть НИ ОДНА из пяти приставок
	// не называет отслеживаемого файла. Служба вынесена отдельным продуктом
	// (kacho#2117), её контракты — решением владельца kacho#2616, исход C,
	// 2026-09-13.
	//
	// ПОЧЕМУ НЕ ПЕРЕВЕДЕНО НА `internal/contractsource`. Соблазн есть: контракт
	// величин существует, просто в модуле `github.com/PRO-Robotech/kaname`, и
	// координата резолвится. Но предмет ЭТОЙ переписи — не «где лежит контракт»,
	// а «что в ЭТОМ дереве ещё говорит слово „квота“ и подлежит снятию на стадии
	// S4»: популяция кандидатов есть индекс git этого репозитория
	// (`runQuotaSurfaceCensus` → `treecorpus.Under(root)`). Файл чужого
	// репозитория снять изменением этого дерева нельзя, и в популяции его нет;
	// правило, отнесённое к нему, относило бы кандидата, которого перепись не
	// считает. Учёт величин службы доступа судится теперь её собственным
	// деревом.
	{
	...
	// Здесь стояло правило C3 («контракт чтения учёта личности», приставка
	// `proto/kaname/cloud/iam/v1/identity_quota_service.proto`).
	//
	// ПРЕЖНЕЕ ОБЪЯСНЕНИЕ ОТМЕНЕНО, и названо это прямо, потому что оно было
	// верным и перестало быть. Оно говорило: «координата ПЕРЕЕХАЛА вместе с
	// предметом (kacho#2362, решение Д9) … предмет правила не исчез — он сменил
	// место, поэтому правило правится, а не снимается». Тогда переезд был
	// ВНУТРИ дерева, и починка действительно состояла в правке координаты.
	//
	// Решением владельца kacho#2616 (исход C, 2026-09-13) контракты службы
	// доступа уехали в её репозиторий: `git ls-files proto/kaname` даёт 0, и
	// приставка не называет отслеживаемого файла. Правится тут нечего —
	// популяция кандидатов есть индекс git ЭТОГО дерева, а файла чужого
	// репозитория в нём нет и снять его отсюда нельзя. Правило снято вместе со
	// своим предметом; глагол чтения учёта по-прежнему судится правилом C2 по
	// признаку `IdentityQuota`, то есть поверхность C слепой не осталась.
	{
	...
	},
	{
		...
		// ОПИСАНИЕ ПРОЦЕССА НАЗЫВАЕТ ФАЙЛ ПРОБЫ, А НЕ ВЕЛИЧИНУ.
		//
		// Правило заведено вместе со своим предметом: шаг, зовущий самопроверку
		// решения о посадке домена величин (`selftest-quota-posture`), сделал
		// описание процесса кандидатом признака. Машинерии величин в нём нет ни
		// одной строки — есть имя файла пробы и `id` шага, — поэтому снос
		// авторитета величин этот файл не затронет; правится он тогда, когда
		// проба переименована или снята.
		//
		// Корзиной «прочее» правило не является, и различие проверяемо: оно
		// смотрит ТОЛЬКО на описания процессов и требует ИСПОЛНЯЕМОГО вызова
		// самопроверки по имени файла. Описание процесса, выставляющее величину
		// переменной среды или зовущее ручку авторитета, этим токеном не
		// ловится и остаётся НЕОТНЕСЁННЫМ — то есть роняет гейт.
		//
		// Правило ИСТЕКАЕТ САМО: сняли шаг — у P4 не осталось попаданий, и
		// перепись назовёт его правилом без предмета.
		Why:      "описание процесса зовёт самопроверку квот по имени файла: машинерии величин нет",
		Fallback: true,
		Scope:    []string{".github/workflows/"},
		Token:    regexp.MustCompile(`(?m)^[^#\n]*\bnode\b[^#\n]*quota[a-z0-9-]*-selftest\.ts`),
	},

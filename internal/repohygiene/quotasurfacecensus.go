...
var quotaCensusRules = []quotaCensusRule{
	...
	// Здесь стояло правило B10 («перечень исключений локальной генерации называет
	// контракт учёта, переехавший в общий фундамент»). Снято ВМЕСТЕ С ПРЕДМЕТОМ
	// (kacho#2601): контракт переименован в `corelib.quota.v1` и лежит под
	// нейтральным корнем `proto/corelib/quota/v1`, поэтому во входы локальной
	// генерации он не попадает ВОВСЕ — вход объявлен корнем `kacho`, — и
	// исключать его больше не из чего. Признак правила не совпал ни с одним
	// кандидатом, и перепись назвала это в тот же прогон.

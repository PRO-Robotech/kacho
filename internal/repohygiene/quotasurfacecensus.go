...
var quotaCensusRules = []quotaCensusRule{
	...
	// деревом.
	// Здесь стояло правило A7 («консольный раздел величин», признак `LimitsPage`).
	// Снято ВМЕСТЕ С ПРЕДМЕТОМ: страница назначения величин ушла из консоли,
	// потому что у административной службы, которую она вызывала, не осталось
	// производителя, — и признак не совпал НИ С ОДНИМ кандидатом. Та же форма,
	// что у A8 ниже: правило, которому нечего относить, есть послабление без
	// ответственного, и следующий читатель унаследует его как описание
	// действительности.
	...
	// действительности.
	{
		...
		// Перечень навигации называет АДРЕС, а не имя компонента, поэтому он
		// переживает снятие самой страницы: пункт, ведущий в никуда, увидит
		// арендатор, а не перепись. Здесь стояла ссылка на правило A7, знавшее
		// страницу по имени компонента, — оно снято вместе со своим предметом,
		// и эта ось осталась единственной.

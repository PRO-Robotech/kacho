...
const (
	...
	// Отсрочка снятия истёкшего удостоверения (#1264) — по тому же требованию
	// перебора. Нижняя граница отсрочки сюда НЕ ставится: она вычисляется
	// функцией из слагаемых, числа у неё нет, и стеречь в ней нечего.
	expiredCredentialReclaimGrace = 24 * time.Hour
)
`
...
func TestQuantityScannerFindsASecondDeclarationOfTheSameQuantity(t *testing.T) {
	...
	// Число растёт вместе с ПЕРЕЧНЕМ стерегомых величин, и это не досадная
	// связанность, а его смысл: перебор ниже проходит по КАЖДОЙ величине и
	// требует от каждой ровно одного опознанного объявления. Величина, которой в
	// синтетике нет, оставила бы свою сторону инъекции недоказанной — то есть
	// гейт стерёг бы её, ничем не подтвердив, что умеет её различать.
	//
	// Отсюда правило для того, кто вносит величину в перечень: обе синтетики
	// правятся ТЕМ ЖЕ изменением. Предпосылка инъекции верна относительно
	// популяции, на которой она писалась, и расширение перечня её меняет.
	if len(decls) != 6 {
		t.Fatalf("объявлений длительности найдено %d, ожидалось 6: %+v", len(decls), decls)
	}

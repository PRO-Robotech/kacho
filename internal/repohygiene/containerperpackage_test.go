...
//
// Множество, а не одна строка: поставщиков БЫЛО два — Postgres и OpenFGA, — и
// вторым его сделала не симметрия, а замер: старт OpenFGA на тест жил в пяти
// пакетах службы доступа, в одном из них 25 раз за прогон.
//
// ВТОРАЯ ЗАПИСЬ СНЯТА ВМЕСТЕ СО СВОИМ ПРЕДМЕТОМ: инструмент сравнительного замера
// форм модели прав лежал в `services/iam/tools/authzformbench`, а служба доступа
// вынесена отдельным продуктом — ни пакета, ни файла с его доказательством в
// индексе больше нет. Сам гейт это и объявил: «санкция покрывает пустоту» плюс
// «санкция осталась без того, на чём стоит» — обе половины записи истекли разом.
// Санкция уезжает В ТО ЖЕ дерево, что и её предмет; там она и будет решаться.
//
// ОСТАВШИЙСЯ ПОСТАВЩИК (Postgres, `pkg/pgtest`) СНЯТ ОТСЮДА ЖЕ И ПО ТОЙ ЖЕ
// ПРИЧИНЕ, ЧТО И ВТОРОЙ: пакет-предмет переехал ЦЕЛИКОМ из `pkg/pgtest` этого
// дерева в пакет `pgtest` общего фундамента (github.com/PRO-Robotech/corelib).
// Обход, который эта запись должна была исключать, ходит ТОЛЬКО по дереву
// судимого репозитория (`scanTree`, ниже) — каталога `pkg/pgtest` в нём больше
// нет НИ ОДНИМ файлом, поэтому запись не встречает предмета вообще ни на одном
// прогоне и своей операционной роли не несёт: она не защищает ни один
// реальный вызов, только представляет собой находку сама по себе — гейт
// `TestSanctionedProvidersStillExist` увидел бы «санкция покрывает пустоту» и
// «санкция осталась без доказательства» одновременно, что и произошло. Санкция
// уезжает В ТО ЖЕ дерево, что и её предмет; собственная тестовая гигиена
// пакета `pgtest` (несёт ли ОН свой поведенческий тест на «сервер один,
// области разные») — предмет РЕПОЗИТОРИЯ corelib, а не этого.

...
//
// «ПОКРАСНЕЕТ ТАМ» ДЛЯ ОСТАВШЕГОСЯ ПОСТАВЩИКА НЕ ВЫПОЛНЯЕТСЯ, и это названо, а не
// сглажено. Доказательство Postgres-половины не исполняется ни одной джобой — под
// кратким режимом оно пропускается, а отбор интеграционной джобы до `pkg/pgtest`
// не достаёт, — и пакет остаётся названным долгом в shortgatedselection_test.go.
// То есть санкция здесь стоит на утверждении, которое некому произнести; закрытие
// — включить пакет в отбор джобы, а не ослабить запись.

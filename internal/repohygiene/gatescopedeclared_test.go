...
// текст, а не значения.
//
// # СКОЛЬКО ФАЙЛОВ ДЕЛЬТЫ ЭТОТ ГЕЙТ СУДИТ — ЧИСЛОМ ИЗ ПЕРЕПИСИ, А НЕ ИЗ ПРОЗЫ
//
// Величина печатается переписью ЭТОГО ЖЕ прогона: «дельта столько-то файлов ·
// в дельте обещают полноту столько-то». Здесь она НЕ ПОВТОРЯЕТСЯ намеренно.
//
// Прежде число стояло прозой в трёх местах, и в двух из них протухло в тот же
// день: головной коммит добавил восемнадцатый файл дельты, а проза
// продолжала говорить «семнадцать» — при том что перепись в том же прогоне
// печатала верное. Абзацем ниже этот же файл объясняет, почему так нельзя:
// расходятся именно копии.
//
// Мера полезности от этого не теряется, а становится проверяемой: механизм,
// объявленный частичным, обязан называть, НАСКОЛЬКО он частичен, — и называет
// это счётом, который нельзя переписать мимо дерева.
//
...
// снятия хвостового пробела; измерено на `9ae1ca86`, 2026-09-22.
//
// ЕДИНИЦЫ РАЗВЕДЕНЫ, потому что сложить их нельзя — то же правило, что абзацем
// ниже. Темнели не только файлы, но и СТРОКИ, и это разные счёты:
//
//   - файлов, переставших опознаваться, девять (референт выше). ЭТОТ ФАЙЛ В ИХ
//     ЧИСЛЕ НЕ БЫЛ: он оставался опознан через собственную строку `:12`, где
//     нужное слово стоит между обычными пробелами, без ёлочек;
//   - строк шапки этого файла, процитированных пробой, пять; байтовая граница
//     молчала на всех пяти, рунная возвращает ТРИ — две молчат по устройству
//     (объявленно невидимые формы и оборот, разорванный переносом).
//
// Референт назван потому, что рядом живёт ДРУГОЕ число о том же предмете — оно
// считано от байтовой границы, и его дом один:
// `gatescopewordboundary_test.go`, шапка. Здесь оно не повторяется намеренно:
// расходятся именно копии.
//
// Сторожа у него нет, и там же сказано почему — но НЕ потому, что число
// «историческое»: эта формулировка была неверна и отозвана. Обе его величины
// живые, перемерить их можно, поэтому при числе стоит РЕВИЗИЯ И ДАТА, а не
// ссылка на невозможность замера.
//
// Девятым в первом счёте был ложный положительный прежнего предиката: слово
// находилось подстрокой внутри другого слова. Оба числа верны, каждое о своём,
// и без референта читатель выбирал бы между ними наугад.
//

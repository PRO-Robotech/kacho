...
// текст, а не значения.
//
// # СКОЛЬКО ФАЙЛОВ ДЕЛЬТЫ ЭТОТ ГЕЙТ СУДИТ — ЧИСЛОМ, А НЕ УМОЛЧАНИЕМ
//
// Из СЕМНАДЦАТИ проверочных файлов дельты гейт судит ОДИН: остальные либо не
// обходят дерево, либо обходят его в форме, которой разбор не видит, либо не
// обещают полноты словом из перечня. Перепись печатает обе величины.
//
// Это не оговорка мелким шрифтом, а мера полезности: механизм, объявленный
// частичным, обязан называть, НАСКОЛЬКО он частичен — иначе «зелено» читается
// как «проверено», а означает «осмотрен один файл из семнадцати». Оба
// механизма этой дельты стоят в формах, ему невидимых: этот — потому что его
// корень выведен, а `gatescopewordboundary_test.go` — потому что дерева не
// обходит вовсе; сказано в шапке каждого.
//
// Измерено на `9ae1ca86`, 2026-09-22.
//

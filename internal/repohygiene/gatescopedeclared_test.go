...
// Мешает — это.
//
// # Что судится, и почему именно это решаемо машиной
//
// У обхода есть КОРЕНЬ, и он бывает двух родов:
//
//   - ВЫВЕДЕННЫЙ — корень приходит из вызова, который сам находит область:
//     корень модуля, перечень производителя, временный каталог синтетики.
//     Такой обход нельзя сузить, не тронув источник;
//   - ВЫПИСАННЫЙ — корень есть строковый литерал либо величина, которой литерал
//     присвоен. Сузить его можно молча, и знаменатель переписи станет тем,
//     каким его сделали.
//
// Выписанный корень сам по себе НЕ дефект: бывает, что область и должна быть
// узкой. Дефект — узкая область, о которой умолчали. Поэтому требование одно и
// исполнимое: у выписанного корня в файле обязана стоять КАНОНИЧЕСКАЯ ДВУЧАСТНАЯ
// строка — что осмотрено и что осталось вне осмотра.
//
// # ОБЛАСТЬ ОБХОДА И ОСТАТОК — У ЭТОГО ГЕЙТА ТОЖЕ, И ОСТАТОК ИЗМЕРЕН
//
// ОБЛАСТЬ ОБХОДА: проверочные файлы Go, ДОБАВЛЕННЫЕ ИЛИ ИЗМЕНЁННЫЕ относительно
// ствола. Область ВЫВЕДЕНА — её называет git, а не перечень в этом файле, и
// сузить её, не тронув дельту, нечем. Гейт подчиняется собственному правилу, а
// не освобождает себя от него.
//
// Почему именно дельта: род дефекта заводится ПИШУЩИМ. Требовать объявления
// области от гейта, написанного три года назад, значило бы или переписать
// половину дерева одной веткой, или молча не требовать ни от кого.
//
// ОСТАТОК: проверочные файлы того же рода в остальном модуле. Он НЕ назван
// прозой — он ПОСЧИТАН тем же обходом и напечатан переписью ЧИСЛОМ, вместе с
// тем, сколько из них обещают полноту. Это не отписка, а измеренная величина
// задачи: молча объявить остаток несуществующим значило бы совершить ровно то,
// против чего гейт и стоит.
//
// # ГРАНИЦА ЭТОГО ГЕЙТА, названная вслух
//
// Машина проверяет, что вопрос ОТВЕЧЕН, а не что ответ ВЕРЕН: «остаток» — это
// проза, и её истинность деревом не проверяется. Отличить правдивый остаток от
// отписки здесь нечем, и делать вид, что есть, значило бы завести ровно тот
// дефект, против которого гейт и стоит.
//
// Что это всё же даёт: написать узкий обход, НЕ ЗАМЕТИВ этого, больше нельзя —
// автор обязан назвать остаток вслух, а рецензенту есть что перемерить. Это
// сдвигает находку с «никто не спросил» на «сказано неверно», а второе ловится
// чтением, тогда как первое не ловилось ничем.
package repohygiene
...
func TestGateScopeIsDeclaredWhereTheWalkIsWrittenOut(t *testing.T) {
	...
	t.Logf("перепись: ОБЛАСТЬ ОБХОДА: %s (%d файлов) · ОСТАТОК: остальной модуль — ИЗМЕРЕН, "+
		...
		inScopePromising-len(undeclared))
}

...
//
// ОСЬ ОБЕЩАНИЯ. Видна ОДНА форма из СЕМИ: слово закрытого перечня ниже,
// отделённое от соседей не-буквой (граница считается по руне, поэтому кавычка,
// тире и неразрывный пробел разделителями являются).
//
// НЕ ВИДНЫ шесть: «ни один» · «все» · «исчерпывающий» · «полон» ·
// англоязычные формулировки · ОБОРОТ, РАЗОРВАННЫЙ ПЕРЕНОСОМ СТРОКИ.
//
// Последняя названа точно, а не приблизительно. Предикат судит ОДИН УЗЕЛ
// комментария: у `//` это одна строка, у блочного — весь блок целиком. Механизм
// у двух видов разный, исход один и тот же: слово ищется СПЛОШНЫМ текстом, а
// оборот, через перенос записанный как «ни» и «одного», сплошным не является
// ни в строке, ни в блоке. Разница механизма названа потому, что «судит одну
// строку» было бы верно лишь для половины видов.
//
// Шестая найдена НЕ РЕВЬЮ, а починкой границы: пока граница ошибалась на
// кириллице, этот случай прятался за ней. Назван здесь тем же изменением,
// которое его заметило.
//
...
func TestGateScopeIsDeclaredWhereTheWalkIsWrittenOut(t *testing.T) {
	...
	t.Logf("перепись: ОБЛАСТЬ ОБХОДА: %s (%d файлов) · ОСТАТОК: остальной модуль — ИЗМЕРЕН, "+
		...
		"о полноте, и обещающие в 6 формах вне перечня. Этот гейт судит ДВЕ формы корня из "+
		"тринадцати и ОДНУ форму обещания из СЕМИ.",
...
func completenessPromiseIn(f *ast.File) string {
	for _, group := range f.Comments {
		for _, c := range group.List {
			...
			// Это одна форма, а не общее решение оси — остальные ШЕСТЬ названы
			// в шапке и остаются вне наблюдения.
			//
			// Узел здесь один комментарий: у `//` строка, у блочного весь блок.
			// Для обеих форм текст ищется сплошным, поэтому оборот через
			// перенос не находится ни в той, ни в другой.
...
// containsWord — вхождение как ОТДЕЛЬНОГО слова.
//
// ГРАНИЦА СЧИТАЕТСЯ ПО РУНЕ, А НЕ ПО БАЙТУ, и это не педантизм. Прежняя
// редакция смотрела байт и считала словесным всякий байт со старшим битом — а
// в русском тексте такими записаны не только буквы, но и кавычки-ёлочки, тире,
// неразрывные пробелы. Граница требуется с ОБЕИХ сторон, поэтому слово,
// стоящее вплотную за ёлочкой, не опознавалось, хотя разделитель после него
// стоял: ДЕВЯТЬ файлов дерева перестали опознаваться — считано от редакции ДО
// снятия хвостового пробела, — и среди них строки шапки этого самого файла.
//
// Референт назван потому, что рядом живёт другое число о том же предмете:
// ВОСЕМЬ — это файлы, меняющие опознание между БАЙТОВОЙ границей и рунной
// (`gatescopewordboundary_test.go`). Девятым в первом счёте был ложный
// положительный прежнего предиката: слово находилось подстрокой внутри другого
// слова. Оба числа верны, каждое о своём, и без референта читатель выбирал бы
// между ними наугад.
//
// Держит это `gatescopewordboundary_test.go`, и держит на ДВУХ предметах: на
// синтетике всех разделителей и на дословных строках здешней шапки.
func containsWord(text, word string) bool {
	for i := 0; ; {
		...
		if !wordRuneBefore(text, start) && !wordRuneAt(text, end) {
			return true
		}
		i = start + 1
		if i >= len(text) {
...
// wordRuneBefore — словесная ли руна СЛЕВА от индекса.
func wordRuneBefore(text string, at int) bool {
	if at <= 0 {
		return false
	}
...
// wordRuneAt — словесная ли руна, начинающаяся с индекса.
func wordRuneAt(text string, at int) bool {
	if at >= len(text) {
		return false
	}
...
// isWordRune — буква либо цифра ЛЮБОГО алфавита. Кавычка, тире и неразрывный
// пробел буквами не являются, хотя записаны теми же старшими байтами.

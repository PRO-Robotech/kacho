...
//
// ─────────────────────────────────────────────────────────────────────────────
// ТОТ ЖЕ КЛАСС ВТОРОЙ РАЗ — И ВТОРОЙ РАЗ ОТКАЗ ОКАЗАЛСЯ ИСПРАВНОСТЬЮ
//
// Решением владельца kacho#2616 (исход C, 2026-09-13) контракты службы доступа
// уехали из этого дерева в её репозиторий. Держатель отказал ровно тем же
// образом — «каталога `proto/kaname/cloud/iam` нет», — и ответ на вопрос о
// живости предмета опять дала перепись: контракт существует, просто в модуле
// `github.com/PRO-Robotech/kaname`, и в нём 42 файла поверхности.
//
// ЧТО ИЗМЕНИЛОСЬ ПРОТИВ ПЕРВОГО РАЗА. Тогда починкой было переписать координату
// вслед за переездом; здесь такой починки НЕ СУЩЕСТВУЕТ: координаты внутри
// дерева контрактов не менялись, изменилось ДЕРЕВО, в котором их искать.
// Поэтому перечень оставлен как есть, а резолвинг вынесен в
// `internal/contractsource`: он отвечает на один вопрос — где на диске лежит
// дерево названного корня, — и для корня, приехавшего модулем, спрашивает сам
// `go`. Перепись после перевода приведена у записи перечня ниже.
...
var KanameSurface = []string{
	...
	// Контракт доступа переехал ДВАЖДЫ, и второй переезд вывел его из этого
	// дерева совсем.
	//
	// Первый — внутри дерева: `kacho.cloud.iam.v1` → `kaname.cloud.iam.v1`
	// (#2133). Отказ обхода на прежнем каталоге и есть то, что заставило
	// перечень последовать за переездом, — молчаливой слепой зоны он не завёл.
	//
	// Второй — решением владельца kacho#2616 (исход C, 2026-09-13): контракты
	// службы доступа уехали в её репозиторий и приезжают сюда модулем
	// `github.com/PRO-Robotech/kaname`. Утверждение «этот каталог лежит в
	// `proto/` ЭТОГО дерева» ОТМЕНЕНО; координату резолвит
	// `internal/contractsource`, и запись оставлена в прежнем написании
	// намеренно — по ней ведутся обе ведомости и её печатает находка, поэтому
	// менять её значило бы переписывать координаты вместо источника.
	//
	// ПОЧЕМУ ЗАПИСЬ ОСТАВЛЕНА, А НЕ СНЯТА ВМЕСТЕ С ДЕРЕВОМ. Вопрос тот же, что
	// разобран в шапке на первом переезде, — ЖИВ ЛИ ПРЕДМЕТ, — и отвечает на
	// него перепись, а не впечатление от красного. Перепись после перевода:
	// поверхность 69 файлов (42 из них — контракт), ось «контракт» осмотрела
	// 343 вхождения и нашла 0, ОБЕ ведомости сошлись точно, остаток тот же —
	// 189 вхождений на 8 полосах. То есть вхождения имени платформы в контракте
	// целиком лежат на границах Б2 (координата контракта фундамента) и Б3 (имя
	// чужого модуля платформы), и в ведомость ДОЛГА из чужого репозитория не
	// попало ни одного: незакрываемой отсюда строки перевод не завёл. Снять
	// запись значило бы сузить популяцию на 42 файла молча — ровно та слепая
	// зона, ради которой обход роняет прогон на пустом каталоге.
...
//
// ПРЕДМЕТА У ГРАНИЦЫ В ЭТОМ ДЕРЕВЕ СЕГОДНЯ НЕТ, и сказано это прямо, а не
// умолчано. Каталог службы уехал вместе с ней (#2598): `git ls-files
// services/iam` → 0, каталогов `docs/engineering/acceptance/` в индексе — 0.
// Перепись держателя печатает это числом («Б4 одобренная приёмка: осмотрено 367
// · признано 0»), поэтому слепой зоной граница не стала: ноль по ней виден.
// Снятие границы ПРЕДМЕТ ОТДЕЛЬНОГО ИЗМЕНЕНИЯ, и это не отсрочка: её довод —
// про цену правки вердикта, привязанного к ревизии, — переезда не переживёт сам
// собой, а решить, переносится он в дерево службы или снимается, вправе владелец
// того дерева, а не эта полоса. Остаток назван числом: приставка одна, вхождений
// под ней на поверхности 0 (перепись, ось границ), предмет — kacho#2613.
...
var KanameNameResidueStay = []NameResidueStay{
	...
	// контракт платформы (`kacho.cloud.vpc.v1`) от
...
//     бы с первым молча.
//
//     ЗДЕСЬ СТОЯЛО «переименовать чужое — сломать клеймо, которое читает
//     провенанс стенда», и это утверждение ПЕРЕЖИЛО свой предмет (#2583).
//     Платформа перевела свою пару на нейтральное имя (`OCI_IMAGE_REVISION` /
//     `OCI_IMAGE_VERSION`, названное внешним стандартом, а не продуктом), а
//     единственный производитель доводов службы — корневой provenance.mk через
//     мягкую ссылку `services/iam/provenance.mk` — передаёт ОБЕ пары. Значит
//     служба вправе переименовать свою пару В СВОЁМ репозитории, ничего не
//     сломав: клеймо будет проставлено любой из двух.
//
//     Прежняя редакция была опаснее обычной устаревшей строки: она объявляла
//     полосу НЕСНИМАЕМОЙ и тем учила следующего читателя, что работы здесь нет,
//     — тогда как платформенная половина её уже сделана. Предикат снятия полосы:
//     `git grep -c 'KACHO_IMAGE_' -- services/iam` → 0; переходную пару у
//     производителей после этого снимает гейт
//     `deploy/stand_provenance_declaration_test.go` — он краснеет, как только ни
//     один Dockerfile дерева прежнего имени не объявляет.
//
...
// этому изменению.
//
// УСЛОВИЕ П3 ПО-ПРЕЖНЕМУ НЕ ВЫПОЛНЕНО: 195 вхождений на 8 полосах (192 до роста
// на три, названного ниже, — kacho#2816). Снижение с 1499
...
// ПОЛОСА КЛЮЧЕЙ ЧАРТА ВЫРОСЛА, И ВОТ ЧЕМ: 125/14 → 128/14 (#2670, почтовая полоса
// приглашения). Блок отправителя в подчарте зонта читает узел значений
// `global.kacho.identity.smtp` — три вхождения, из них два в коде шаблона и одно
// в объясняющей его прозе. Оба файла уже стояли в ведомости, поэтому число
// файлов не двинулось: 14 и до, и после.
//
// РОСТ ВЫНУЖДЕН, и небрендированного ключа здесь не существует. Узел принадлежит
// ЗОНТУ, и его же читает конфигурация поставщика личности
// (`_kratos-identity.tpl`). Отправителей письма два, узел обязан быть ОДИН: иначе
// письма разных видов уедут в разные места, причём каждое по отдельности уйдёт
// исправно, и заметить расхождение будет неоткуда. Совпадение путей питания
// держит гейт `deploy/identity_mail_lane_feeds_both_senders_test.go`, заведённый
// ТЕМ ЖЕ изменением, — назвать второму читателю свой, небрендированный ключ
// значило бы развести их молча, то есть купить строку ведомости ценой ровно того
// класса, ради которого гейт и написан.
//
// СНИМЕТСЯ ВМЕСТЕ СО СВОЕЙ ПОЛОСОЙ, а не отдельно: ключ переименует линия
// дебрендинга ключей чарта оператора — у обоих читателей одним изменением,
// потому что разойтись им нельзя.
//
// ТРИ СТРОКИ ВЫРОСЛИ НА ОДНО ВХОЖДЕНИЕ, И ВОТ ЧЕМ (kacho#2816, стенд `own` без
// поставщика): ключ чарта 128/14 → 129/15, имя объекта 10/5 → 11/6, имя
// платформы 22/7 → 23/8. Все три — ОДНА строка нового файла ведомости
// `deploy/helm/umbrella/values.own-stand.yaml`: SPIFFE-имя клиента чеканки
// первичного удостоверения `spiffe://kacho.cloud/ns/kacho/sa/kacho-bootstrap-operator`
// в `kaname.config.authn.bootstrapMint.allowedClientSANs`. Посев стенда добывает
// первого предъявителя этой чеканкой, и на цепочке `own` без неё он не проходит.
//
// РОСТ ВЫНУЖДЕН: имя принадлежит зонту (он выпускает этот клиентский сертификат,
// mtls.bootstrapOperator), и тот же литерал уже стоит в `values.dev.yaml` у
// стенда разработки. Своего имени у службы здесь нет и быть не может — круг
// вызывающих называет СЕРТИФИКАТ зонта, а не службу. Снимется вместе со своей
// полосой: имя переименует линия дебрендинга витрины оператора у обоих слоёв.
//
// ПОЛОСА КЛЮЧЕЙ ЧАРТА СНИЖЕНА 129/15 → 116/15 (kacho#2735, посадка own во всех
// профилях). Снижение — НЕ работа линии дебрендинга: из подветки службы в
// профилях зонта сняты дороги службы к издателю поставщика, которого больше не
// поднимает ни один стенд (`kaname.platform.iam.hydra{AdminUrl,TokenURL,JwksUrl,
// Issuer}`), и вместе с ними — прозаический абзац о них. Тринадцать строк по
// файлам: values.dev-prod.yaml 5 (два адреса публичного слушателя, адрес
// административного и два упоминания публичного издателя в прозе о них),
// values.prod.yaml 3, values.fe3455-prod.yaml 3, values.dev.yaml 2 (проза
// абзаца о переопределении административного адреса). Файлов по-прежнему 15: ни один файл полосы не
// опустел. Перечень снятого снят двумя прогонами одного разбора
// (`FindKanameNameResidue`, полоса ключа чарта) на `2735` @ bd777b65572 и на
// голове этого изменения.
//
// ПОЛОСА КЛЮЧЕЙ ЧАРТА ВЫРОСЛА 116/15 → 119/15 (kacho#2735, первая фаза подъёма
// чеканит своё). Три строки, все в `deploy/helm/umbrella/values.dev.yaml`, все в
// `kaname.config.authn`: издатель своей чеканки (`https://kaname.kacho.local`),
// адресат приёма предъявленного и перечень адресатов токен-эндпоинта (оба
// называют `https://api.kacho.cloud`). Файл уже стоял в ведомости, поэтому
// файлов по-прежнему 15.
//
// РОСТ ВЫНУЖДЕН. Под посадкой `own` служба без своей чеканки не стартует — у
// публикатора набора ключей не остаётся ни одной записи, — и первая фаза `make
// dev-up` ложилась на этом отказе у каждого стенда конвейера. Чеканка держит за
// собой приём предъявленного и токен-эндпоинт — их требуют гейты
// `deploy/presented_credential_declaration_test.go` и
// `gateway/deploy/f1b_token_acceptance_declared_test.go`, — а значения
// обязаны совпадать с боевым слоем values.dev-prod.yaml, где те же три
// литерала уже стоят: одна платформа, один издатель, один адресат. Литералы
// называют ДОМЕН установки, а не службу; снимутся вместе со своей полосой — их
// переименует линия дебрендинга ключей чарта оператора у обоих слоёв разом.
// Перечень снят разбором `FindKanameNameResidue` (полоса ключа чарта) на
// голове этого изменения против `2735` @ b72fa905674.
...
var KanameNameResidueDebt = []NameResidueDebt{
	...
	{laneChartKnob, 119, 15, "линия дебрендинга: ключи чарта оператора"},
	...
	{laneObjectName, 11, 6, "Р3 эпика #2076 — витрина оператора"},
	{laneBrandInText, 12, 9, "Р3 эпика #2076 — бренд в прозе и на клиентских страницах"},
	{lanePlatformName, 23, 8, "Р3 эпика #2076 — имя платформы отдельным словом"},

...
//
//   - СЕЛЕКТОРЫ МЕТОК (`app.kubernetes.io/name=$product` в
...
//     Здесь стояла форма, выводившая имя части ПРИСТАВКОЙ от имени каталога:
//     соседний гейт от неё отказался, потому что служба управления доступом
//     получила своё имя продукта и выведенная приставкой форма стала спрашивать
//     про под, которого на стенде нет. Имя приходит теперь ОТВЕТОМ источника
//     (`internal/productnaming`). Обоснование вывода от этого не изменилось, а
//     координата, которой оно подпиралось, перестала существовать — утверждение
//     пережило свой предмет молча, ровно как ловит этот же пакет;

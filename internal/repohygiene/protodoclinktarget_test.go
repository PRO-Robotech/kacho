...
// access — 5, замер на e1275446). Послабление самоистекающее: гейт требует, чтобы
// у каждой записи БЫЛ предмет, и краснеет, когда домен вычищен, — тогда его место
// в protoDocLinkJudged, а не здесь. Записи, которой нечего исключать, в списке не
// остаётся.
var protoDocLinkDeferred = []string{"proto/kacho/cloud/access", "proto/kacho/cloud/compute/v1"}

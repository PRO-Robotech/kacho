...
func docsIDFormOptions(t *testing.T) DocsIDFormOptions {
	...
	return DocsIDFormOptions{
		...
		//
		// Здесь стояло и дерево документации службы доступа. Оно уехало вместе
		// со службой, вынесенной отдельным продуктом, и запись пережила бы свой
		// предмет ХУЖЕ, чем обычная мёртвая координата: перепись ПЕЧАТАЕТ
		// перечень судимых доменов, поэтому корень, из которого не прочитано ни
		// страницы, читался бы как осмотренный. «Ноль находок» в нём означало бы
		// «ноль прочитанного», и отличить одно от другого было бы нечем.

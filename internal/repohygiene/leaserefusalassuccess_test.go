...
func treeImportsVPCStubs(t *testing.T, dir string) bool {
	...
	_ = filepath.WalkDir(dir, func(path string, d os.DirEntry, err error) error {
		if err != nil || d.IsDir() || found || !strings.HasSuffix(path, ".go") {
			return nil //nolint:nilerr // недоступный файл не отменяет обхода
		}
...
func refusalsReturnedAsSuccess(t *testing.T, dir string) ([]string, laneCensus) {
	...
	if err != nil {
		t.Fatalf("перепись (б): каталог %s не читается: %v", dir, err)
	}

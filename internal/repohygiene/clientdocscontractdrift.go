...
var clientDocsRetiredFieldLedger = map[string]string{
	"services/nlb/docs/content/api/target-group.mdx#deregistrationDelaySeconds": "" +
		"Пример ответа группы таргетов показывает `deregistrationDelaySeconds` числом " +
		"секунд. Контракт это имя ЗАБРАЛ (`target_group.proto`, `reserved`): величина " +
		"стала `deregistration_delay` строкой длительности («300s»). Дефект ЖИВОЙ и " +
		"принадлежит домену балансировщика, а не заводящему этот гейт: правка чужой " +
		"страницы в этом изменении смешала бы две линии и сделала бы вердикт " +
		"непрослеживаемым. " +
		"Снимается, когда имя перестанет встречаться в примерах этого сайта ЛИБО " +
		"перестанет быть забранным в контракте балансировщика — гейт краснеет на " +

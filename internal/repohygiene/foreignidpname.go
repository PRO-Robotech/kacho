...
// дереве таких имён нет ни одного, и ЛАТЕНТНОСТЬ ДОКАЗЫВАЕТСЯ ПАРОЙ ЧИСЕЛ, а не
// одним: числитель @5d376227375 — 106 ИМЁН до добора и 106 ИМЁН после, ни одной
// находки не прибавилось; знаменатель того же обхода @5d376227375 вырос
// с 188088 до 206297 ОСМОТРЕННЫХ ИМЁН (+18209: имя пакета 3435, псевдоним
// импорта 1764, переменная `range` 13010 — сумма сходится с совместным замером).
//
// Половина без половины не доказывает ничего: неизменный числитель сам по себе
// совместим с вырожденным «ноль до и ноль после», то есть с распознавателем,
// который никуда не заглянул. Именно выросший знаменатель при неизменном
// числителе и означает «смотрим шире, находок по-прежнему нет».
//
// Изменилось обещание заголовка, ради которого проба и стоит.
//
// ─────────────────────────────────────────────────────────────────────────────
...
//
// ЧЕГО НЕ ВИДИТ ПРОВЕНАНС ЧИСЛА — НАЗВАНО ВСЛУХ
//
// `Measured` судится двумя осями: ФОРМА (похоже на ревизию) и РАЗРЕШИМОСТЬ
// (дерево знает этот объект-коммит). Обе вместе отсекают заглушку, выдуманный
// набор знаков и ревизию ЧУЖОГО дерева.
//
// Чего они НЕ отсекают: ревизию ЭТОГО дерева, на которой число не снималось.
// Она и форму держит, и разрешается. Подставить соседний коммит вместо того, на
// котором считали, — молчание, и гейт этого не заметит.
//
// Механически это закрывается только тем, что число ПРОИЗВОДИТСЯ обходом
// названной головы, а не объявляется рядом с нею. Прочтения здесь ДВА, и они
// не равны:
//
//	«обходом ТЕКУЩЕЙ головы» — снимает сам гейт. Ведомость сравнивала бы дерево
//	    С СОБОЙ, обе половины стали бы тождественно истинными, и наблюдение
//	    исчезло бы вместе с расхождением, ради которого заведено. Этот исход
//	    ЗАПРЕЩЁН, и он же — соблазн при каждом красном.
//	«обходом НАЗВАННОЙ головы» — требует дерева той ревизии, то есть отдельной
//	    рабочей копии на каждый прогон. Цена не замерена, и пока она не
//	    замерена, предмет остаётся открытым.
//
// И ЕЩЁ ОДНОГО ОНИ НЕ ВИДЯТ САМИ ПО СЕБЕ — ГЛУБИНЫ КЛОНА. Рабочие копии этого
// воркспейса МЕЛКИЕ: `rev-parse --is-shallow-repository` даёт `true` во всех
// трёх (измерено @c9085e45510). В мелком дереве объект, существующий у
// источника, просто не довезён, а молчаливый `--verify --quiet` отвечает на это
// тем же кодом возврата, что и на «объекта нет вовсе». Поэтому глубина
// спрашивается ОТДЕЛЬНО и ДО классификации: иначе читающему красное велят
// переписать ПРАВИЛЬНОЕ число. Это не гипотеза о чужой среде — это наша среда.
//
// ПРЕДИКАТ СНЯТИЯ ЭТОГО ОСТАТКА: ведомость пуста (цель гейта — тогда провенанса
// не у чего спрашивать) ЛИБО число производится обходом названной головы, а
// стоимость такого обхода замерена и предъявлена. До тех пор остаток открыт, и
// его граница — эта врезка, а не сообщение коммита: предикат, живущий только в
// истории, не прочтёт никто.
//
// ─────────────────────────────────────────────────────────────────────────────
...
// полный шестнадцатеричный объект, в любом регистре.
//
...
//
// РЕГИСТР ПРИНИМАЕТСЯ ЛЮБОЙ, потому что его принимает сама система контроля
// версий: заглавная запись полного объекта разрешается наравне со строчной
// (измерено `rev-parse --verify --quiet <ЗАГЛАВНАЯ>^{commit}` @5d376227375).
// Гейт, отвергающий то, что принимает инструмент, красит исправную запись.
...
const (
	// foreignIDPRevisionMinLen — НИЖНИЙ ПРЕДЕЛ ЕСТЬ РАЗМЕН, А НЕ СВОЙСТВО
	// СИСТЕМЫ, и следствие названо прямо — тем же порядком, что у правила
	// `len(fw) > 3` выше.
	//
	// Система контроля версий берёт и короче семи: шестизначная запись
	// разрешается (измерено `rev-parse --verify --quiet <6 знаков>^{commit}`
	// @5d376227375), берёт и от четырёх. Предел стоит здесь не потому, что
	// короче не бывает, а потому, что ниже семи поле шестнадцатеричных слов НЕ
	// ПУСТО: `deface`, `decade`, `facade`, `added`, `beef` — законные слова,
	// которые встанут в это поле заглушкой и пройдут проверку формы.
	//
	// Следствие названо прямо: ревизия короче семи знаков, даже настоящая, даёт
	// находку «это не ревизия». Чинится удлинением записи, и это дешевле, чем
	// принимать заглушку за провенанс.
...
func ForeignIDPNameProvenanceGaps(ledger []ForeignIDPNameLedgerEntry, resolve ForeignIDPNameRevisionResolver) []string {
	...
	for _, e := range ledger {
		...
		if err := resolve(rev); err != nil {
			// ПРИЧИНУ НАЗЫВАЕТ РАЗРЕШИТЕЛЬ, А НЕ СУДЬЯ. Внешний текст, называющий
			// свою причину поверх вложенной, противоречит ей в одной строке вывода:
			// «снято с чужой головы» против «дерево не спрошено» — разные ремонты.
			gaps = append(gaps, fmt.Sprintf("запись ведомости %q объявляет имён %d на "+
				"ревизии %s, и эта ревизия НЕ ПОДТВЕРЖДЕНА: %v", e.Area, e.Names, rev, err))
		}
	}
	return gaps
}
...
// гейтом живёт второй замер того же предмета в СТРОКАХ, и числа у них совпали
// случайно. «103 ИМЕНИ по дереву @328e1ead5ce» и «103 СТРОКИ в пакете
// @bec320cf47d» — разные предметы, разные единицы и разные головы; врезка о двух
// «103» в шапке разбирает это подробно. Перепись потому и печатается: она есть
// число о СЕГОДНЯШНЕМ дереве, и пересказывать её без единицы и ревизии — значит
// заводить третье «103».

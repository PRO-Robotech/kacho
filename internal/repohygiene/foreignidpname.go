...
// СТОЛЬКО ЖЕ до и после, каким бы оно ни было. Абсолютная величина здесь не
// несёт ничего: утверждается равенство двух прогонов, а не их значение, и
// выписанное число устарело бы при первом же изменении дерева, не сделав
// утверждение ни вернее, ни ложнее. Изменилось обещание заголовка, ради
// которого проба и стоит.
//
// ─────────────────────────────────────────────────────────────────────────────
...
// находкой 326 из них сверх 103 ИМЁН ТОГО ЖЕ ЗАМЕРА @6b8246108e4 (сегодняшнее
// число печатает перепись каждого прогона — выписывать его сюда значит заводить
// вторую копию, которая разойдётся с первой молча). Причина в том, что «ory» лежит внутри
...
//
// Из этих 28 имя даёт САМ ПОСТАВЩИК только двум — упакованным чартам
// `hydra-0.62.1.tgz` и `kratos-0.62.1.tgz`; они не наши и переименованием не
// правятся. Остальные 26 — НАШИ имена: `deploy/` 14, `gateway/` 5,
// `ui-future/` 5, `tests/` 2. Наш и подчарт `kratos-selfservice-ui`: он под
// BUSL-1.1, написан здесь, имя выбрали мы, а оборачивает он чужой образ.
//
// Здесь класс не добирается намеренно: у пути другой знаменатель (28 против
// 204888), другая цена правки (переименование тянет импорты каталога-пакета,
// ссылки документов, задания конвейера и историю) и другой отбор — предикат
// обязан различать «наше имя пути» и «имя, данное поставщиком», а это отдельная
// работа со своей ведомостью и своей инъекцией, не строка в этом гейте. До тех
// пор ни один замер по цели эти пути НЕ СЧИТАЛ: все считали имена внутри кода.
//
// ─────────────────────────────────────────────────────────────────────────────
// ДВА РАЗНЫХ «103» — ЕДИНИЦА И РЕВИЗИЯ НАЗЫВАЮТСЯ ПРИ КАЖДОМ ЧИСЛЕ
//
// У этой полосы два замера одного предмета, и числа у них совпали СЛУЧАЙНО:
// 103 ИМЕНИ (объявленный идентификатор Go) по всему дереву @328e1ead5ce —
// предмет этого гейта; 103 СТРОКИ с именем поставщика в пакете
// `services/registry/internal/clients/jwks` @bec320cf47d — предмет строчного
// замера полосы, снятый до одной строки. Разные единицы, разные области, разные
// предметы, РАЗНЫЕ РЕВИЗИИ. Поэтому перепись называет единицу при каждом числе,
// и то же требуется от всякого, кто эти числа пересказывает.
//
// И РЕВИЗИЮ — при каждом числе тоже. Совпадение выше историческое: обе величины
// названы вместе с головой, на которой сняты, и ни одна из них не есть
// утверждение о сегодняшнем дереве. Сегодняшнее число по этому гейту печатает
// перепись каждого прогона; в шапке его нет намеренно, потому что вторая копия
// числа расходится с первой молча — ровно тем дефектом, которым красное этой
// полосы и началось.
...
// foreignIDPRevisionShaped — ПОХОЖЕ ЛИ значение на ревизию: сокращённый или
// полный шестнадцатеричный объект.
//
// Это ось ПЕРВАЯ из двух, и одна она не судит ничего: выдуманный набор знаков
// форму держит. Вторую (разрешимость) держит разрешитель.
func foreignIDPRevisionShaped(s string) bool {
	if len(s) < foreignIDPRevisionMinLen || len(s) > foreignIDPRevisionMaxLen {
		return false
	}
	for _, r := range s {
		if (r < '0' || r > '9') && (r < 'a' || r > 'f') {
...
const (
	// foreignIDPRevisionMinLen — короче сокращённой ревизии не бывает: система
	// контроля версий сама не печатает короче семи знаков.
	foreignIDPRevisionMinLen = 7
	// foreignIDPRevisionMaxLen — длина полного объекта.
	foreignIDPRevisionMaxLen = 40
)

// ForeignIDPNameRevisionResolver — спрашивает дерево, знает ли оно эту ревизию.
//
// Отдельным параметром, а не обращением к системе контроля версий внутри:
// проверка, которую нельзя позвать с синтетическим входом, инъекцией не
// доказывается — а доказать здесь нужно ДВЕ оси по отдельности, и каждую парой,
// меняющей ровно один факт.
type ForeignIDPNameRevisionResolver func(rev string) error

// ForeignIDPNameProvenanceGaps — записи ведомости, чьё число НЕ ПРИВЯЗАНО к
// ревизии, обходом которой снято.
//
// Осей две, и ни одна не лишняя:
//
//	ФОРМА        — «позже», «TBD», «нет», «a» ревизиями не являются. Первая
//	               редакция судила строку на НЕПУСТОТУ и молчала на всех
//	               четырёх: объявляла, что судит ревизию, а различала
//	               заполненность поля.
//	РАЗРЕШИМОСТЬ — выдуманный набор шестнадцатеричных знаков и ревизия ЧУЖОГО
//	               дерева форму держат, а ревизиями ЭТОГО дерева не являются.
//	               Молчание здесь есть ровно та ложь, которой красное полосы и
//	               началось: число, снятое с чужой головы, объявленное снятым
//	               здесь.
func ForeignIDPNameProvenanceGaps(ledger []ForeignIDPNameLedgerEntry, resolve ForeignIDPNameRevisionResolver) []string {
	gaps := make([]string, 0, len(ledger))
	for _, e := range ledger {
		rev := strings.TrimSpace(e.Measured)
		if !foreignIDPRevisionShaped(rev) {
			gaps = append(gaps, fmt.Sprintf("запись ведомости %q объявляет имён %d, "+
				"а её ревизия записана как %q — это не ревизия, и число осталось "+
				"непроверяемым", e.Area, e.Names, e.Measured))
			continue
		}
		if err := resolve(rev); err != nil {
			gaps = append(gaps, fmt.Sprintf("запись ведомости %q объявляет имён %d на "+
				"ревизии %s, которой это дерево не знает (%v) — число снято обходом "+
				"ЧУЖОЙ головы либо ревизия названа неверно", e.Area, e.Names, rev, err))
		}
	}
	return gaps
}
...
// foreignIDPMeasuredOrUnnamed — ревизия записи для ТЕКСТА находки.
//
// Читающий красное иначе видит ту самую неразличимость, ради снятия которой
// ревизия и заводилась: «поверхность выросла» и «число снято с другой головы»
// выглядят одинаково. Неназванная ревизия говорится словом, а не пустым местом:
// пустое место читается как опечатка вывода.
func foreignIDPMeasuredOrUnnamed(e ForeignIDPNameLedgerEntry) string {
	if rev := strings.TrimSpace(e.Measured); rev != "" {
		return rev
	}
	return "НЕ НАЗВАННОЙ"
}
	...
	for _, a := range areas {
		...
		if len(got) != e.Names {
			findings = append(findings, ForeignIDPNameFinding{
				...
				Detail: fmt.Sprintf("ведомость объявляет имён %d на ревизии %s, в дереве "+
					"их %d — перепишите число на %d ВМЕСТЕ С ревизией, обходом которой "+
					"снимете, либо снимите имена; предикат снятия записи: %s",
					e.Names, foreignIDPMeasuredOrUnnamed(e), len(got), len(got), e.Until),
			})
		}
	}
	...
	for _, e := range ledger {
		if len(byArea[e.Area]) == 0 {
			findings = append(findings, ForeignIDPNameFinding{
				...
				Detail: "в области не осталось ни одного имени этой оси (число " +
					"объявлено на ревизии " + foreignIDPMeasuredOrUnnamed(e) + ") — запись " +
					"обязана быть снята вместе с предметом; предикат её снятия: " + e.Until,
			})
		}
	}

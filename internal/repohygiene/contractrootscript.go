...
//
// Второй корень (`kaname`) в примерах ниже назван БЕЗ приставки `proto/`: его
// дерева под `proto/` этого репозитория нет с переезда контрактов службы доступа
// (kacho#2616, исход C, 2026-09-13), и литералы его домена встречаются там, где
// корень СОБРАН, — то есть без приставки. Классификация от приставки не зависит:
// вопрос у неё один и тот же.
//
...
//	kaname/cloud/iam/v1          → имя члена         (домен назван) — молчим

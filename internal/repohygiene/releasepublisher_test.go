...
//
//  1. механизм публикации существует и исполняем — `scripts/release/publish-version.sh`;
//  2. проба годности опубликованной версии существует и исполняема —
//     `scripts/release/probe-published.sh`;
//  3. у КАЖДОГО из двух есть доказательство способности упасть (`*-inject.sh`),
//     и это доказательство ЗОВЁТСЯ прогонщиком `scripts/ci-local.sh`.
//
...
// # Способность упасть
//
// Доказана `releasepublisher_injection_test.go`: снятие любого из четырёх
// файлов и снятие вызова инъекции из прогонщика дают красное с координатой,
// а законный близнец — посторонний скрипт той же формы рядом — молчит.
package repohygiene

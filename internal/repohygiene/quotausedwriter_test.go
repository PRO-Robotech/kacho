...
var quotaTriggerDefiningFiles = map[string][]string{
	"project_resource_quotas": {
		...
		// Вторая полоса того же владельца: потолок числа удостоверений на
		// принципала (`PRO-Robotech/kacho#1191`). Тело списания заменено целиком
		// через `CREATE OR REPLACE` — применённую миграцию править нельзя
		// (ban #5), поэтому у одной функции два места определения во времени, и
		// оба обязаны быть названы здесь: пропусти этот файл, и гейт объявит
		// находкой сам механизм; сними прежний, и его откат станет обходом.
		//
		// Тот же файл несёт ЗАТРАВКУ, и она пишет `used` по факту — по той же
		// причине, по которой это разрешено пятерым владельцам ниже: триггер умеет
		// ровно ±1 и не умеет ПРИВЕСТИ счётчик к факту, а строки удостоверений
		// лежат в базе до появления этой оси. Второго источника истины здесь нет:
		// затравка считает те же строки, по которым потом ведёт счёт списание.
		"services/iam/internal/migrations/20260824230000_credential_ceiling_per_principal.sql",

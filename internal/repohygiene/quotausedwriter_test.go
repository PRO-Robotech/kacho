...
var quotaTriggerDefiningFiles = map[string][]string{
	"project_resource_quotas": {
		...
		// ВТОРОЕ МЕСТО ОПРЕДЕЛЕНИЯ ВО ВРЕМЕНИ — по тому же правилу, что у vpc и
		// storage выше: тело `kacho_quota_count` заменено через `CREATE OR REPLACE`,
		// а применённую миграцию править нельзя (ban #5).
		//
		// Предмет замены — ИСТОЧНИК ВЕЛИЧИНЫ, а не списание: у трёх видов, чей
		// владелец сама служба доступа, величина берётся из проекции посадки
		// (`kaname.own_ceilings`), а не у авторитета (приёмка `KAN-QUOTA-1`, `П25`,
		// задача продукта #2117). Само списание осталось тем же единственным
		// атомарным оператором в той же транзакции, что вставка строки ресурса.
		//
		// Запись обязана стоять здесь по обеим причинам, названным выше: пропусти
		// её — и гейт объявит находкой сам механизм; сними предшественницу — и её
		// откат, восстанавливающий прежнее тело, станет обходом.
		"services/iam/internal/migrations/20260910120000_own_ceilings_come_from_the_posture.sql",

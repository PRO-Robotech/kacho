...
var quotaTriggerDefiningFiles = map[string][]string{
	"project_resource_quotas": {
		...
		// ЗДЕСЬ СТОЯЛИ ДВЕ ЗАПИСИ СЛУЖБЫ ДОСТУПА — сведённая первичная миграция и
		// замена тела списания, переводившая источник величины на проекцию посадки.
		// Обе сняты вместе со своим предметом: `services/iam` вынесен отдельным
		// продуктом, миграций службы в этом дереве нет, и послаблению больше нечего
		// исключать. Гейт это и объявил дважды — «файла нет, снимите запись вместе
		// с её предметом».
		//
		// Что при этом НЕ снято и снятию не подлежит: сама норма. Служба доступа
		// была ЕДИНСТВЕННЫМ владельцем, который и назначает величину, и списывает по
		// ней, — и в своём дереве это её собственный предмет. Здесь у неё нет ни
		// файла, ни строки учёта, ни триггера, поэтому и записи быть не может: запись
		// без предмета укрыла бы следующий файл с тем же путём.

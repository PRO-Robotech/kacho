...
// migratorPackageDirs — каталоги, в которых у сервиса живёт накат. Их ДВА, потому
// что мигратор в дереве форкнут: у compute/storage/registry/geo вся команда лежит
// в `cmd/migrator`, у vpc/iam/nlb она делегирует в `internal/apps/migrator`.
// Гейт смотрит в оба и не судит, какой из них правильный, — его предмет в том,
// что вызов есть хоть где-то на пути наката этого сервиса.
func migratorPackageDirs(root, svc string) []string {
	return []string{
		filepath.Join(root, "services", svc, "cmd", "migrator"),
		filepath.Join(root, "services", svc, "internal", "apps", "migrator"),
	}
}

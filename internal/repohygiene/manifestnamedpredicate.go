...
// прав. Канон лежит НЕ В ЭТОМ ДЕРЕВЕ: решением владельца kacho#2616 (исход C,
// 2026-09-13) контракты службы доступа уехали в её репозиторий, и
// `kaname/cloud/iam/v1/fga_model.fga` принадлежит теперь
// `PRO-Robotech/kaname`, приезжая сюда опубликованным модулем
// `github.com/PRO-Robotech/kaname`. Прежняя редакция называла его как
// `proto/kaname/…` этого дерева — утверждение ОТМЕНЕНО; координату дерева
// контрактов резолвит `internal/contractsource`.
//
// СЛЕДСТВИЕ ДЛЯ САМОГО ГЕЙТА НАЗВАНО ПРЯМО: он судит координаты, названные
// манифестом, по индексу ЭТОГО дерева, — и координата канона под этот суд не
// подпадает by construction, потому что строка манифеста называет чужой
// репозиторий (ось N5, §«Исключение у N5 ровно одно»). Перепись печатает число
// таких координат отдельной величиной, чтобы исключение не стало слепой зоной.
//
// В своей шапке манифест называет КОМАНДУ, которой судится каждый его раздел:
// это единственная запись о том, кто его держит. Читатель манифеста идёт за
// этой командой — и другого способа узнать держателя у него нет.
//

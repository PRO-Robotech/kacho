...
//  6. Код, пришедший ПЕРЕМЕННОЙ (`capacityRefusal(code, …)` внутри самой
//     обёртки), разбором не устанавливается: судятся места, где код записан
//     литералом `codes.X` либо сентинелом. Обёртка под наблюдение попадает через
//     СВОИХ вызывающих, которые код именуют, — то есть предмет не уходит, а
//     координата находки смещается на вызывающего.
//  7. Текст, собранный ДО выражения отказа (`msg := …; status.Error(code, msg)`),
//     вне наблюдения: литерала в аргументах отказа нет. Расширение на поток
//     значений требует проверки типов и здесь не заведено.
//
...
//	F4-ru-povtor     повтори(те) / повторить / повторяй(те) — ТОЛЬКО директивные
//	                 основы `повтори` и `повторяй`. Существительное «повтор» и
//	                 описательное «повторяется» есть упоминание, а не совет, и под
//	                 форму НЕ подпадают. Правая граница слова здесь НЕ ставится
//	                 намеренно: `\b` в RE2 — граница ASCII-слова, а кириллическая
//	                 буква словообразующей для неё не является, поэтому `\b` после
//	                 русской основы не срабатывает НИКОГДА и форма молчала бы вся
...
var retryAdviceForms = []retryAdviceForm{
	...
	{"F4-ru-povtor", regexp.MustCompile(`(?i)(повтори|повторяй)`)},

...
// проверяемую ветвь.
//
// Вида МАЛО там, где вид вносится не одной ветвью. ВИД находки и МЕСТО её
// внесения — разные единицы: их определение и команды переписи находятся в
// шапке `subscriptionformshape.go`, §«Единицы счёта». Подпроба, утверждающая
...
// Здесь стояла проба TestSubscriptionShapeMandatoryAxisCanFail — три фикстуры,
// вносившие в ось опцию обязательности, и законный близнец без неё. Снята вместе
// со своим предметом (задача #1255): семейство расширений, которым обязательность
// выражалась, снято с контрактов, и вход, на котором проба ловила находку, больше
// не производится ничем. Фикстура, вносящая непроизводимый признак, доказывает
// способность падать по образцу, а не по существу.
//
// Разбор, почему исходов у негативного утверждения на исчезнувшем предмете ровно
// два, — у снятого поля `MandatoryOption` в subscriptionformshape.go.

...
//
// # Запись о ведомости функций схемы СНЯТА — её предикат сработал
//
// Здесь стояла запись, прощавшая держателю остатка чужого имени закрытый
// перечень имён функций общего фундамента: семь ключей той же приставки со
// значениями-константами, множество ИМЁН, а не отображение утверждений в
// величины. Запись сама назвала предикат снятия — «распознаватель научится
// отличать множество имён от отображения утверждений в значения», — и предикат
// СРАБОТАЛ (задача #2166): различитель заведён в `ScanClaimMint`, гейт покраснел
// на записи, которой больше нечего исключать, и запись ушла тем же изменением.
//
// Различитель взят СТРОЖЕ предложенного записью. Она предлагала судить по тому,
// зависят ли значения от принципала; судится ТИП ЗНАЧЕНИЯ: `map[string]bool` и
// `map[string]struct{}` суть множества, у которых значения нет вовсе. Причина —
// та же, по которой распознаватель обязан знать все законные формы записи
// предмета: «что считать постоянным» есть перечень форм и стареет молча, а тип
// значения — один узел разбора. Замер, на котором это решено: мест, признаваемых
// составом, шесть — пять со значением `any`, все у владельца, и одно со
// значением `bool`, и это ведомость.
//
// Ведомость снова ПУСТА, и это исход, к которому гейт вёл. Пустая ведомость его
// не роняет: падение на достижении собственной цели заставляло бы держать запись
// ради зелёного.

...
// Ведомость была ПУСТА, и это исход, к которому гейт вёл: сборок состава вне
// владельца в дереве не осталось. Первая запись покрывала обратный вызов
...
//
// # Запись ниже — НЕ сборка состава, и это перепроверка ПРЕДПОСЫЛКИ, а не поблажка
//
// Шапка выше утверждает: «на пороге в ТРИ ключа в находки попадают только
// настоящие сборки». Утверждение было верно ОТНОСИТЕЛЬНО ПОПУЛЯЦИИ, на которой
// порог мерили, и новый её член его опроверг: держатель остатка чужого имени
// объявил ЗАКРЫТЫЙ ПЕРЕЧЕНЬ ИМЁН функций общего фундамента — семь ключей той же
// приставки со значениями-константами. Это множество ИМЁН, а не отображение
// утверждений в значения, взятые у принципала; предмета §2.11 у него нет.
//
// Порог здесь не трогается и распознаватель не сужается: сужение — работа
// владельца этого гейта, и делать её в чужой полосе значило бы менять предмет
// проверки мимоходом. Запись называет предикат снятия, по которому это сужение
// узнаётся, и истекает от него сама.
//
// Запись СНИМАЛАСЬ ВМЕСТЕ с держателем остатка (#2133 удалил файл, и прощать
// стало нечего) и возвращается вместе с ним: предмет у неё снова есть.
var claimDebt = []claimDebtEntry{
	{
		File: "internal/repohygiene/kanamenameresidue.go",
		Func: "уровень пакета",
		Why: "не сборка состава утверждений, а закрытый перечень ИМЁН функций общего " +
			"фундамента внутри схемы службы (KanameFoundationSchemaFunctions): держатель " +
			"остатка чужого имени пропускает их как решённые остаться. Значения ключей — " +
			"константы, а не выражения над принципалом, поэтому у записи нет предмета §2.11: " +
			"разойтись «у принципала, чей токен выдан не тем путём» здесь нечему",
		Until: "распознаватель научится отличать МНОЖЕСТВО ИМЁН от ОТОБРАЖЕНИЯ утверждений " +
			"в значения (наблюдаемый признак: у сборки значения зависят от принципала, у " +
			"перечня — константы), после чего эта запись перестанет иметь предмет и " +
			"покраснеет как устаревшая",
	},
}

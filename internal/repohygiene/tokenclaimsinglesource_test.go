...
var claimDebt = []claimDebtEntry{
	{
		File: "services/iam/internal/handler/iamhooks/refresh_hook_handler.go",
		Func: "RefreshHookHandler.refreshClaims",
		Why: "обратный вызов ОБНОВЛЕНИЯ сессии прежнего провайдера лежит в другом пакете и " +
			"позвать неэкспортируемый сборщик не может; сведение — правка прод-кода вне " +
			"области этой правки, и решает её владелец сервиса",
		Until: "файл перестаёт собирать состав сам — зовёт сборщик единственного объявления " +
			"либо уходит вместе с прежним провайдером",
	},
}

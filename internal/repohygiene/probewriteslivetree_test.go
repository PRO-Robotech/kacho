...
//
// Shell-суиты он не читает — их судит СОСЕД, `shellprobewriteslivetree_test.go`
// (#724). Раздел границ остаётся здесь именно потому, что корпуса у двух
// половин разные: этот гейт молчит о shell не по недосмотру, и его «ноль
// находок» относится ровно к отслеживаемым `*_test.go`.
//
// Довод, ради которого сосед написан разбором, а не текстом, остаётся верным и
// перемерен: форму `> "$FILE"` текстовый поиск ловит, а правку через встроенный
// python (цель уезжает аргументом, запись живёт в чужом языке) — нет, то есть из
// трёх экземпляров класса, найденных в shell при заведении ЭТОГО гейта, он нашёл
// бы один. Соседний предикат ловит все три; это закреплено контролем на прежних
// редакциях тех самых суит (`shellprobewriteslivetree_injection_test.go`,
// `TestShellProbeWriteControlOnPriorRevisions`), а не заявлено.
//

...
//
// Состав корпуса гейты этого репозитория берут у git-индекса (`internal/treecorpus`)
...
// считается производителем живого корня, если её тело спрашивает рабочий каталог
// процесса (`os.Getwd`) и восходит до `go.mod`. Список имён разошёлся бы с
// деревом молча — их в дереве два десятка и они называются по-разному
// (`repoRoot`, `moduleRoot`, `monorepoRoot`, `repoRootFromTest`, …).
//
...
//
// Shell-суиты он не читает: разбор shell тем же уровнем точности здесь не
// построен. Текстовый предикат проверялся и отвергнут числом — из трёх
// экземпляров класса, найденных в shell при заведении этого гейта, формы
// `> "$FILE"` он ловит, а правку через встроенный python (`open(path, "w")`)
// не ловит, то есть нашёл бы ОДИН из трёх и объявил бы остальные два чистыми.
// Изоляция этих трёх доказана прерыванием, а не гейтом; долг назван числом и
// заведён предметом (#724), чтобы «ноль находок» не читалось шире, чем есть.
//
// Ещё две границы, купленные точностью сознательно (первое же ложное
...
// isLiveRootProducer — функция спрашивает рабочий каталог процесса и восходит до
// `go.mod`, то есть возвращает корень ЖИВОГО репозитория. Признак выведен из
// того, как эти помощники написаны в дереве, а не из их имён: имён у них два
// десятка и они разные.
func isLiveRootProducer(fn *ast.FuncDecl) bool {
	var getwd, gomod bool
	ast.Inspect(fn.Body, func(n ast.Node) bool {
		switch v := n.(type) {
		case *ast.CallExpr:
			if sel, ok := v.Fun.(*ast.SelectorExpr); ok {
				if id, ok := sel.X.(*ast.Ident); ok && id.Name == "os" && sel.Sel.Name == "Getwd" {
					getwd = true
				}
			}
		case *ast.BasicLit:
...
func (s *liveScope) isLive(e ast.Expr) bool {
	...
	case *ast.CompositeLit:
		...
		return false
	case *ast.CallExpr:
		// Временный каталог обрывает происхождение.
		if isTempDirCall(v) {
			return false
		}
...
func isTempDirCall(c *ast.CallExpr) bool {
	sel, ok := c.Fun.(*ast.SelectorExpr)
	if !ok {
		return false
	}
	...
	pkg, ok := sel.X.(*ast.Ident)
	return ok && pkg.Name == "os" && sel.Sel.Name == "MkdirTemp"
}

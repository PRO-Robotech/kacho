...
// фундаменту. Число пропущенных печатается, поэтому полоса видна. Живой
// экземпляр там есть — комментарий поля контракта реестра называет прежний
// репозиторий общего фундамента, — и он заведён задачей #2199, а не оставлен
// невидимым: держателя у него нет ни с одной стороны, и это сказано вслух.
package repohygiene
...
// foundationRoots — каталоги, чья проза судится. Фундамент один, но перечень
// оставлен перечнем: расширение популяции обязано быть решением, а не правкой
// строки в регулярке.
var foundationProseRoots = [...]string{"pkg/"}
...
func scanFoundationProse(tree *treecorpus.Tree) (polyrepoCensus, []polyrepoFinding, error) {
	...
	for _, rel := range tree.SortedFiles() {
		...
		inFoundation := false
		for _, prefix := range foundationProseRoots {

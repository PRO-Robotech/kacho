...
// (`20260817042704_имя.sql`). Форму, которую дерево не принимает: гейт
// пространства номеров отвергает номер больше номера всякой возможной задачи, а
// файлов такой формы в дереве ноль — глаголом ни разу не пользовались.
//
// Инструмент, предлагающий форму, которой дерево не принимает, хуже
// отсутствующего: он не просто бесполезен, он УЧИТ неверному, и узнаёт об этом
// тот, кто им воспользовался, — на чужом отказе.
//
...
//
// Номер миграции выводится из номера задачи и известен автору из имени
// собственной ветки — выводить его инструментом нечем и незачем; разбор в
...
func (f nameProducerFinding) String() string {
	return fmt.Sprintf(
		"%s:%d: вызов %s порождает ИМЯ файла миграции.\n"+
			"    goose без SetSequential выдаёт 14-значную метку времени — форму, которой "+
			"дерево не принимает: гейт пространства номеров отвергает номер больше номера "+
			"всякой возможной задачи.\n"+
			"    Имя новой миграции выводится из номера задачи и пишется рукой: "+
			"`<задача><порядковый:3>_<что_делает>.sql`. Разбор — "+
			"docs/architecture/migration-version-namespace.md.",

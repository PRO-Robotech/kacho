...
// отдельная работа с отдельным предметом, заведена kacho#1387; пока её нет,
// разграничение держит эта запись вместе с названной в ней пробой, и держит
// НАЗВАННО, а не молча.
//
// Следствие, которое надо знать читателю ведомости: у ЭТОЙ записи самоистечение
// не сработает никогда — предмет у неё есть и будет, потому что запрос читателя
// выглядит одинаково в обоих случаях. Она держится пробой, а не гейтом, и это
// единственная запись такого рода.
//

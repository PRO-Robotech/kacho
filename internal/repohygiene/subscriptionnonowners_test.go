...
var subscriptionNonOwners = []subscriptionNonOwner{
	...
	// Здесь стояла запись про iam (#1397): у службы доступа ленты изменений
	// ресурсов нет, четыре её очереди служат самой платформе. Запись ушла
	// вместе со своим предметом — служба вынесена отдельным продуктом, домена
	// `iam` под `services/` больше нет. Снятия потребовал сам гейт: ведомость,
	// называющая домен вне дерева, роняет прогон («записи больше нечего
	// исключать, снимите её»). Решение и его внешний предикат пересмотра
	// остались там, где были записаны, — в дереве службы доступа.
}

// subscriptionRightsModelRel — КАНОНИЧЕСКАЯ копия модели прав, лежащая в дереве
// контрактов.
//
// # Почему она, а не встроенная в службу
//
// Прежде читалась исполняемая копия (`services/iam/internal/authzmodel/`) —
// довод был «решение о видимости строки считается по той модели, которая
// встроена в службу», а байт-идентичность двух копий держали свои цели. Довод
// пережил свой предмет: служба доступа вынесена отдельным продуктом, и
// исполняемой копии в ЭТОМ дереве нет ни одной. Читать нечего, и молчание
// гейта тогда означало бы «предпосылку проверять нечем», а не «нарушений нет».
//
// Каноническая копия — не запасной путь, а единственный оставшийся производитель
// этого факта в дереве: на момент выноса обе копии были байт-идентичны
// (`git show HEAD:proto/kaname/cloud/iam/v1/fga_model.fga` против
// `git show HEAD:services/iam/internal/authzmodel/fga_model.fga` — 1432 строки,
// расхождений ноль).
//
// ГРАНИЦА НАЗВАНА: гейт спрашивает у модели ровно одно — СКОЛЬКО типов объявляет
// домен, и берёт это число как предпосылку записи ведомости. Утверждения о том,
// что именно исполняет служба, он не делает и делать не вправе: служба живёт в
// другом дереве.
//

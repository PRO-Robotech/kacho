...
//	CatalogModule  каталог контрактов `<корень>/cloud/<Y>` — КОРЕНЬ ВЫВОДИТСЯ,
//	               а не пишется литералом: домен `iam` лежит под корнем `kaname`
//	               и в дереве модуля службы (kacho#2616, исход C), остальные —
//	               под `kacho` в этом дереве. Литерал `proto/kacho/cloud/<Y>`
//	               стоял здесь и был верен, пока корень был один
...
func judgePlatformVocabularyWithExternal(declared []platformmodules.Module,
	...
	for _, m := range declared {
		...
		if _, ok := protoDirs[m.CatalogModule]; !ok {
			// Координата НЕ склеивается с литералом корня: домен резолвится в
			// свой корень обходом (`contractroot.ResolveDomain`), и корней три,
			// причём один приезжает модулем. Текст находки, называвший
			// `proto/kacho/cloud/<домен>`, послал бы читателя искать не там —
			// для домена `iam` такого пути не существует НИ В ОДНОМ дереве.
			faults = append(faults, "модуль "+m.Service+": каталога контрактов домена "+
				m.CatalogModule+" нет ни под одним объявленным корнем ни в этом дереве, "+
				"ни в деревьях контрактов приезжающих модулей — модуль каталога назван неверно")
		}

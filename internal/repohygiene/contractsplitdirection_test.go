...
func TestPlatformContractDoesNotImportTheServiceContract(t *testing.T) {
	...
	// Контроль стороны службы — импорты, прочитанные ВООБЩЕ, а не ведущие в
	// платформу. Здесь стояло «импортов службы в платформу не ноль: их два
	// десятка», и посылка истекла вместе со своим предметом: все они вели в форму
	// операции, а она получила имя фундамента (`corelib.operation`, kacho#2601).
	// Ноль рёбер «служба → платформа» теперь — факт дерева, а не слепота;
	// слепоту отличает число прочитанных импортов службы.
	require.NotZero(t, cen.ServiceImports,
		"в контрактах службы не прочитано НИ ОДНОГО импорта при %d прочитанных "+
			"описаниях: распознаватель ослеп на той стороне, ради которой гейт и заведён",
		cen.ServiceFiles)

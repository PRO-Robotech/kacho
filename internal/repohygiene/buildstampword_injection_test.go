...
// доказывалась ПОДАЧЕЙ ВХОДА, а не чтением: настоящее дерево и синтетический мир
// проходят одну функцию, поэтому доказанное на втором верно для первого.
//
// Оси:
...
func TestBuildStampWordInjection_DivergedValueIsSeen(t *testing.T) {
	t.Parallel()
	v, found, err := ConstStringValue([]byte(wordSrcDiverged), "BuildInfoUnstamped")
...
func TestBuildStampWordInjection_MatchingValueIsSilent(t *testing.T) {
	t.Parallel()
	v, found, err := ConstStringValue([]byte(wordSrcMatching), "BuildInfoUnstamped")
	require.NoError(t, err)
	require.True(t, found)
...
`
	_, found, err := ConstStringValue([]byte(renamed), "BuildInfoUnstamped")
	require.NoError(t, err)
...
`
	_, found, err := ConstStringValue([]byte(prose), "BuildInfoUnstamped")
	require.NoError(t, err)
...
`
	_, _, err := ConstStringValue([]byte(computed), "BuildInfoUnstamped")
...
func TestBuildStampWordInjection_UnparsableSourceIsAnError(t *testing.T) {
	t.Parallel()
	_, _, err := ConstStringValue([]byte("не Go вовсе"), "BuildInfoUnstamped")

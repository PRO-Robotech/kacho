...
type TableGrowthDecl struct {
	...
	Family growthFamily
	// RemovalBlocked — почему у этой таблицы оператор снятия строк появиться
	// ПОКА НЕ ВПРАВЕ. Необязательно; объявленное меняет исход появления уборки
	// с «запись потеряла предмет» на НАХОДКУ.
	//
	// # Зачем ось, обратная всем остальным
	//
	// Реестр отвечает на вопрос «чем ограничен рост», и появление уборки у
	// перечисленной таблицы — обычно хорошая новость: механизм нашёлся, запись
	// больше не нужна, гейт зовёт её снять. Ровно это и говорит полоса `stale`.
	//
	// Но у журнала, чей ЧИТАТЕЛЬ не умеет обнаружить пропуск, появление уборки
	// есть НЕ разрешение записи, а её худший исход. Снятая строка такого журнала
	// не даёт ни отказа, ни пробела: читатель по диапазону позиций просто не
	// находит её и переезжает курсором дальше. «Строк не было» и «строки убрали»
	// становятся неразличимы — а для журнала, гасящего кэш вердиктов, первое
	// означает норму, второе означает НЕПРИМЕНЁННЫЙ ОТЗЫВ ДОСТУПА, молча.
	//
	// Инструкция «снимите запись» в такой ситуации не просто бесполезна — она
	// ВЕДЁТ НЕ ТУДА: следующий читатель, увидев её, уберёт из реестра
	// единственное место, где написано, почему уборка небезопасна, и уборка
	// останется. Поэтому исход разводится по объявлению, а не по догадке гейта:
	// «чем грозит уборка ЭТОЙ таблице» есть суждение о её читателе, и в дереве
	// оно записано только здесь.
	//
	// # Блок обязан истекать, поэтому требует НОМЕРА ЗАДАЧИ
	//
	// Объявление без предмета не снял бы никто: оно выглядит осторожностью и
	// живёт вечно. Поэтому непустой RemovalBlocked требует непустого Issue —
	// задачи, в которой заводится недостающий предикат обнаружения пропуска.
	// Появится предикат — снимут это поле, и уборка станет законной обычным
	// порядком.
...
var tableGrowthRegistry = []TableGrowthDecl{
	{
		Owner: "services/iam", Table: "cluster_admin_grants",
		Tempo: tempoOurs, Verdict: verdictBound,
		Reason: "предел структурный, а не уборочный: частичный уникальный индекс " +
			"cluster_admin_grants_subject_unique(subject_type, subject_id) WHERE granted_until " +
			"IS NULL допускает не более ОДНОЙ действующей выдачи на принципала, поэтому живых " +
			"строк не больше, чем принципалов, а погашенные несут granted_until и из-под " +
			"индекса выходят. Отзыв здесь МЯГКИЙ — строка остаётся следом, и это решение: " +
			"кто был администратором кластера, обязано читаться после отзыва. " +
			"ЗАПИСЬ ЗАВЕДЕНА СВЕДЕНИЕМ ЦЕПОЧКИ (2026-09-04) и это не формальность: до него " +
			"таблица проходила по полосе снятия строк, найденной в ОТКАТНОЙ половине миграции " +
			"и в разовой правке слияния личностей. Ни то ни другое пределом роста не является " +
			"— откат не исполняется на живой базе, а разовая правка сама объявлена гейтом " +
			"«механизмом не считается». Свод унёс обе, и гейт заговорил правду: предела не " +
			"было названо. Здесь он назван",
	},
	{
	...
	},
	{
		...
		Reason: "журнал намерений, ПЕРЕСТАВШИЙ быть очередью: колонки доставки сняты " +
			...
			"Строки применяет ТРИГГЕР (`relation_fact_from_journal`, миграции 0098/0100) " +
			"AFTER INSERT FOR EACH ROW, в той же транзакции. РЕШЕНИЕ #1712 — удержание, " +
			"и оно принято по перемеру, ОПРОВЕРГШЕМУ прежнюю посылку записи. Прежде здесь " +
			"стояло «ни один запрос не читает прошлых строк»: это верно для РАНТАЙМА " +
			"(читателей журнала в прод-коде Go ноль) и неверно для МИГРАЦИЙ, которые тоже " +
			"прод-код. В секциях `Up` прошлые строки читают пять миграций тремя разными " +
			"способами: свёртка журнала в проекцию (0029, 0098 §3), подавление повторного " +
			"посева по содержимому (0058, 0081) и отзыв посеянной выдачи (0074, 0081). " +
			"Решающее при этом второе: у проекции `relation_fact` производителей в Go " +
			"НОЛЬ — её наполняют только этот триггер и та самая свёртка, — поэтому журнал " +
			"есть ЕДИНСТВЕННЫЙ источник её восстановления, и это не гипотеза: свёртку уже " +
			"выполняли. Уборка необратима и уничтожила бы обе возможности разом " +
			"(восстановить проекцию · объяснить историю выдачи). Срок хранения есть " +
			"ПОЛИТИКА, а не гигиена — ровно то решение, что уже принято приёмкой #1292 " +
			"для `audit_outbox`, свидетельства ТОЙ ЖЕ ПРИРОДЫ; решить здесь иначе значило " +
			"бы дать продукту разойтись с самим собой. Довод о природе, а НЕ о темпе: " +
			"темпы не сравнивались, и есть основание думать, что этот журнал растёт " +
			"БЫСТРЕЕ аудита iam — `RegisterResource` зовут все службы на создание всякого " +
			"ресурса. Это цена решения, и она названа, а не спрятана. Разбор — " +
			"services/iam/docs/engineering/architecture/journal-retention-is-a-policy.md",
		Family: familyJournal,
	},
	// ЗДЕСЬ БЫЛА ЗАПИСЬ `services/iam/resource_reconcile_outbox` — она ИСТЕКЛА
	// САМА (#2050), и снята она гейтом, а не памятью.
	//
	// Запись объявляла таблицу долгом: дренаж помечал доставленную строку
	// `sent_at` и не снимал её никогда. Условие легализации запись называла
	// сама — «предикат ей нужен СВОЙ, и он проще: реконсайлера
	// (`RedrivePoisoned`) над очередями iam нет ни одного, значит защищать
	// нечего; но „сегодня реконсайлера нет" есть факт о дереве, и уборка без
	// ключа обязана нести гейт, который покраснеет, когда реконсайлер
	// появится».
	//
	// Оба условия исполнены: уборщик заведён свой
	// (`retention.SubjectReconcileOutbox` → `pg.ReconcileOutboxSweeper`,
	// шестая запись реестра уборки), а предпосылку стережёт гейт
	// `services/iam/internal/check`
	// `TestReconcileOutboxRetentionPremiseHolds`.
	//
	...
	//
	// Поэтому гейт судит не службу, а ТАБЛИЦУ: он краснеет на оживителе,
	// построенном над ОЧЕРЕДЬЮ СВЕРКИ, и молчит на оживителе соседней очереди
	// — у неё свой ключ партиции и общий уборщик платформы. Гейт, красный на
	// исправном дереве, отключают первым, а с ним перестают читать и настоящую
	// находку.
	//
	// Абзац оставлен вместо молчаливого удаления затем, чтобы следующий, увидев
	// у таблицы уборку, не завёл запись заново «на всякий случай».
	// ЗДЕСЬ БЫЛА ЗАПИСЬ `services/iam/provider_compensation_outbox` — она
	// ИСТЕКЛА САМА (#2069), и снята она гейтом, а не памятью: перепись выше
	// объявила её потерявшей предмет в тот же прогон, в который у таблицы
	// появился оператор снятия строк.
	//
	// Запись объявляла таблицу долгом и называла ОБА условия легализации сама:
	// «предикат ей нужен СВОЙ и более простой» (общий уборщик платформы
	// требует ключа партиции, а он у этой очереди пуст намеренно — поток
	// коммутативен, щадить предшественника не от кого) и «послабление обязано
	// нести гейт, который покраснеет, когда реконсайлер появится».
	//
	// Оба исполнены: уборщик заведён свой
	// (`retention.SubjectProviderCompensationOutbox` → `pg.ProviderCompensationSweeper`,
	// седьмая запись реестра уборки), а предпосылку стережёт гейт
	// `services/iam/internal/check`
	// `TestProviderCompensationRetentionPremiseHolds` — по образцу соседнего,
	// заведённого для очереди сверки, и по той же причине: «оживителя над этой
	// таблицей нет» есть факт о ДЕРЕВЕ, а не свойство замысла.
	//
	// Гейт судит ТАБЛИЦУ, а не службу: оживитель над очередью писем в дереве
	// ЕСТЬ и законен — у неё свой ключ партиции и общий уборщик платформы.
	// Гейт, красный на исправном дереве, отключают первым, а с ним перестают
	// читать и настоящую находку.
	//
	// Абзац оставлен вместо молчаливого удаления затем, чтобы следующий, увидев
	// у таблицы уборку, не завёл запись заново «на всякий случай».
	{
	...
	{
		Owner: "services/iam", Table: "invite_mail_outbox",
		Tempo: tempoExternal, Verdict: verdictBound,
		Reason: "очередь писем приглашения (#1776): строка заводится в writer-транзакции " +
			...
			"строку, у которой в партиции есть несданный предшественник. Уборка выразима " +
			"ровно потому, что у очереди есть ключ партиции: общий уборщик платформы " +
			"ОТКАЗЫВАЕТ на сборке без него. Имени таблицы в операторе снятия нет — оно " +
			"приезжает значением, — поэтому запись остаётся, а не снимается",
		Family: familyDrainerQueue,
	},
	{
...
type tableGrowthCounts struct {
	...
	Retained int
	Debt     int
	// Blocked — записей, объявивших запрет на появление уборки.
	Blocked int
	// BlockedRemoval — из них тех, у кого уборка ВСЁ ЖЕ появилась. Печатается
	// рядом с Blocked: одно число не отличало бы «запретов ноль» от «запретов
	// нет ни одного нарушенного».
	BlockedRemoval int
}
...
func tableGrowthVerdict(st tableGrowthState, registry []TableGrowthDecl) (findings, stale []string, counts tableGrowthCounts) {
	...
	for _, e := range registry {
		...
		if strings.TrimSpace(e.RemovalBlocked) != "" && strings.TrimSpace(e.Issue) == "" {
			findings = append(findings, ref.String()+
				" — запрет на появление уборки (RemovalBlocked) без НОМЕРА ЗАДАЧИ. Запрет снимается "+
				"вместе с недостающим предикатом, и задача — единственное место, где этот предикат "+
				"заводится: без неё запрет выглядит осторожностью и не истекает никогда")
		}
	}
	...
	for _, t := range st.Tables {
		...
		case swept:
			counts.Swept++
			switch {
			case listed && strings.TrimSpace(entry.RemovalBlocked) != "":
				// Уборка у таблицы, чья запись объявила запрет, — НАХОДКА, а не
				// закрывшийся предмет. Полоса `stale` здесь звала бы снять
				// единственное место, где написано, почему уборка небезопасна,
				// и уборка осталась бы.
				counts.BlockedRemoval++
				findings = append(findings, t.TableRef.String()+
					" — у таблицы ПОЯВИЛСЯ оператор снятия строк, а запись реестра объявляет, что "+
					"появиться он пока не вправе: "+strings.TrimSpace(entry.RemovalBlocked)+
					". Это НЕ повод снять запись. Исходов два: завести недостающий предикат "+
					"(задача "+strings.TrimSpace(entry.Issue)+") и только затем снять запрет — либо "+
					"снять саму уборку. Оставить обе — значит убирать журнал, чей читатель пропажу "+
					"строки не обнаружит")
			case listed:
		...
		case listed:
			counts.Declared++
			if strings.TrimSpace(entry.RemovalBlocked) != "" {
				counts.Blocked++
			}
...
func TestLiveTablesNameTheirGrowthLimit(t *testing.T) {
	...
	// Запреты печатаются ДВУМЯ величинами: «сколько объявлено» и «у скольких из
	// них уборка всё же появилась». Одно число не отличало бы «запретов ноль»
	// от «ни один не нарушен», а первое означало бы, что ось перестала
	// наблюдаться вовсе.
	...
	// Заголовок НЕЙТРАЛЕН намеренно. Находки этого гейта трёх родов — таблица
	// без механизма, негодная форма записи и уборка при объявленном запрете, — и
	// прежний заголовок «таблица без названного механизма» называл первый род,
	// приписывая его остальным двум. Диагностика, называющая не тот предмет,
	// посылает читателя искать не там; сам текст находки род называет.
	for _, f := range findings {
		t.Errorf("рост таблиц: %s", f)
	}
}

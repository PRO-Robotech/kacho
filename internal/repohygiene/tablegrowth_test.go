...
var tableGrowthRegistry = []TableGrowthDecl{
	...
	},
	{
		...
		Reason: "журнал смены субъекта, ПЕРЕСТАВШИЙ быть очередью: колонки доставки сняты " +
			...
			"ПРЕДИКАТ ОБНАРУЖЕНИЯ ПРОПУСКА ЗАВЕДЁН (#1712), и запрет уборки, стоявший " +
			"здесь полем RemovalBlocked, СНЯТ ВМЕСТЕ СО СВОЕЙ ПРИЧИНОЙ. Что теперь верно: " +
			"чтение берёт пол (наблюдение и ответ из него одним вызовом) ПОСЛЕ страницы " +
			"— порядок несущий, пол раньше страницы доказывает лишь расписание; курсор " +
			"ниже пола получает OUT_OF_RANGE с машинным признаком полосы " +
			"и возобновимой позицией; читатель края на этот отказ гасит кэш вердиктов " +
			"целиком, закрывает все открытые потоки и садится на названную позицию. " +
			"Держится инвариантом, а не расписанием: уборка снимает ПРЕФИКС, поэтому " +
			"«курсор >= пол» влечёт отсутствие дыр выше курсора. ЧЕГО ЕЩЁ НЕТ, и потому " +
			"вердикт остаётся долгом: уборщика не провязано НИ ОДНОГО, рост по-прежнему " +
			"ничем не ограничен — снят БЛОКЕР, а не сам долг. Величина срока хранения — " +
			"политика, общая с audit_outbox и fga_outbox, и держателя у неё сегодня нет; " +
			"назначать её здесь отдельно значило бы завести второе место об одном " +
			"предмете. Разбор — " +
			"services/iam/docs/engineering/architecture/journal-retention-is-a-policy.md",
		Issue:  "#1712",
		Family: familyJournal,

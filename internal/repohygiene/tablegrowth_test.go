...
var tableGrowthRegistry = []TableGrowthDecl{
	...
	// ЗДЕСЬ БЫЛИ 13 ЗАПИСЕЙ владельца `services/iam` — они ИСТЕКЛИ ВСЕ СРАЗУ
	// вместе со своим владельцем: служба доступа вынесена отдельным продуктом,
	// и ни одной из названных ими таблиц в применённых миграциях дерева больше
	// нет. Снятие — не послабление: предикат «запись реестра ПОТЕРЯЛА ПРЕДМЕТ»
	// назвал все тринадцать поимённо, то есть самоистечение сработало как
	// задумано. Записи, заведённые «на будущее» под возможное возвращение
	// владельца, здесь не оставлены намеренно: такая запись не истечёт никогда
	// и станет слепой зоной шириной в одну таблицу.

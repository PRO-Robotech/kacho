...
var tableGrowthRegistry = []TableGrowthDecl{
	...
	},
	{
		...
		Tempo: tempoExternal, Verdict: verdictDebt,
		Reason: "журнал подписки: строка пишется триггером на каждой мутации ресурса " +
	...
	},
	{
		...
		Tempo: tempoExternal, Verdict: verdictRetained,
		Reason: "леджер однократности завершения восстановления, ключ — идентификатор ПОТОКА " +
			"восстановления у внешнего провайдера. Уборка рассмотрена и отвергнута предикатом " +
			"читателя (#1364): без строки повторная доставка того же потока обрабатывается как " +
			"первая — ставит новую отсечку живых сессий личности и повторяет запись аудита, — " +
			"а окно повторной доставки задаёт отправитель, величины которого в дереве нет. " +
			"Корректность строки срока не имеет, значит его нет и у строки. Темп внешний, но " +
			"человеческий: строка на КАЖДОЕ успешное восстановление пароля. Предикат " +
			"пересмотра: появится проверка свежести доставки — леджер станет убираемым по " +
			"сроку потока",
	},
}

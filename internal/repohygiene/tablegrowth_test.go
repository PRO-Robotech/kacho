...
var tableGrowthRegistry = []TableGrowthDecl{
	...
	},
	{
		...
		Reason: "журнал смены субъекта, ПЕРЕСТАВШИЙ быть очередью: колонки доставки сняты " +
			...
			"ресурсов: погасить весь кэш вердиктов и продолжить. ПЕРЕМЕРЕНО #1712 второй " +
			"раз, и приём признан ПРИМЕНИМЫМ: `pkg/subscription.Watermark` уже несёт и " +
			"`RefreshEarliest`, и пол, а этот репозиторий уже держит тот самый Watermark " +
			"ради границы устоявшегося — недостают три звена, и все три лежат ВНЕ " +
			"services/iam: экспорт пола, отказ с возобновимой позицией на проводе и ответ " +
			"читателя на такой отказ (`pkg/subjectchange`). Пока их нет, удержание — не " +
			"предпочтение, а единственное безопасное состояние, и держится оно уже не " +
			"этой прозой, а полем RemovalBlocked ниже",
		Issue: "#1712",
		RemovalBlocked: "читатель журнала (`PollSubjectChanges`, окно `id > since AND id <= settled`) " +
			"пропуска не обнаруживает НИЧЕМ — ни пола, ни отказа «позиция утрачена» у него " +
			"нет, — поэтому снятая строка неотличима от «строк не было», а полоса fail-open " +
			"by design: пропущенная строка означает непогашенный кэш вердиктов края, то есть " +

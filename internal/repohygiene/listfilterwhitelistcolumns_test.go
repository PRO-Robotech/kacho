...
func TestFilterWhitelistGateCanFail(t *testing.T) {
	...
	cases := []struct {
		...
		// Опечатка в имени колонки — НАМЕРЕННАЯ: предмет этого кейса в том, что негодное
		// имя обязано быть находкой. Исправить слово значило бы уничтожить проверяемое
		// свойство, поэтому подавляется проверка правописания, а не правится фикстура.

...
// крае. Соответствие БЕРЁТСЯ у словаря имён модулей (pkg/platformmodules), а не
// выводится из совпадения имён: вывод по совпадению молча пропустил бы ровно
// тот домен, у которого написания расходятся, и гейт отчитался бы «ноль
// находок», не осмотрев его. Оба написания настоящие, и ни одно не выводится из
// другого (`polyrepo.md` §«Ключ домена и имя каталога»).
//
// Собирается ОДИН раз: словарь неизменен на всё время процесса, и пересборка на
// каждый вызов давала бы разные карты об одном предмете.
var quotaCatalogueToConsole = platformmodules.AliasesByCatalogModule()

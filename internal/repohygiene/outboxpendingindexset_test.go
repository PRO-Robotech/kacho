...
var nonPartitionDrainedOutboxes = map[string]string{
	...
	"subject_change_outbox": "" +
		"ДРЕНАЖА У НЕЁ БОЛЬШЕ НЕТ ВОВСЕ (задача #1024): толчок к краю снят, направление " +
		"развёрнуто, и потребитель читает журнал КУРСОРОМ (`id > $1`), не касаясь `sent_at`. " +
		"Значит ни ключа партиции, ни клейма, ни анти-соединения у неё нет by construction, и " +
		"механизм квадратичного роста к ней не относится тем более. Частичные индексы по " +
		"`sent_at IS NULL` остались от прежнего устройства и уходят вместе с колонками " +
		"доставки — предмет задачи продукта #1396. Учтена здесь, чтобы не выпасть из поля " +
		"зрения до тех пор.",
}

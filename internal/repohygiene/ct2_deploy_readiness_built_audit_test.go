...
//
// # Как узнаётся именованная проверка — ФОРМ ДВЕ, и обе законны
//
// ПЕРВАЯ — по СТРУКТУРЕ литерала (поля `Name` и `Check`), а не по имени типа.
// Имя типа сегодня общее (`health.Checker`), но привязка к нему сделала бы гейт
// слепым к сервису, у которого носитель свой: iam строит те же две величины
// типом своего транспортного слоя, и гейт, судящий по имени пакета, объявил бы
// его нарушителем, ничего не нарушившего.
//
// ВТОРАЯ — вызов `…Checker("имя")`: так именует себя зависимость с отложенным
// носителем, и литерала с полями у неё нет вовсе. Форма заведена вместе с
// отложенным носителем, поэтому распознаватель обязан знать её с того же дня:
// форма, о которой он не знает, — не край и не редкость, всё записанное в ней
// оказывается ВНЕ НАБЛЮДЕНИЯ, а гейт при этом не даёт ни красного, ни зелёного —
// он молчит (`testing.md` §«Гейт на класс», п.7). Сегодня одна зависимость
// registry записана именно так.
//
		...
		if s.NamedDeps == 0 {
			findings = append(findings, readinessFinding{Service: svc, Why: fmt.Sprintf(
				"%s зарегистрирован (%s), но ни одной ИМЕНОВАННОЙ проверки зависимости сервис не "+
					"строит: ни литерала с полями Name и Check, ни вызова Checker(\"имя\") в дереве "+
					"сервиса нет. Обработчик без предмета отличается от живости только адресом — "+
...
func collectSurface(fset *token.FileSet, file *ast.File, rel string, out *serviceSurface) {
	ast.Inspect(file, func(n ast.Node) bool {
		switch node := n.(type) {
		case *ast.CallExpr:
			// Вторая законная форма именованной проверки судится ПЕРВОЙ: ниже по
			// ветке стоят ранние выходы для вызовов, не похожих на регистрацию
			// маршрута, и распознаватель, поставленный после них, не исполнялся бы
			// НИКОГДА. Проверено инъекцией: законный близнец с отложенным носителем
			// объявлялся нарушителем, пока проверка стояла после свича.
			if isCheckerConstructor(node) {
				out.NamedDeps++
				if out.DepsFile == "" {
					out.DepsFile = rel
				}
...
// isCheckerConstructor — вторая законная форма именованной проверки: вызов
// `<носитель>.Checker("имя")`. Судится ИМЯ МЕТОДА и то, что имя зависимости
// приехало строковым литералом; тип носителя не судится по той же причине, что и
// у литерала полей — привязка к пакету сделала бы гейт слепым к своему носителю.
func isCheckerConstructor(call *ast.CallExpr) bool {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok || sel.Sel.Name != "Checker" || len(call.Args) != 1 {
		return false
	}

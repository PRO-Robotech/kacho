...
//
// Пустой словарь писанных видов означает «сверять было не с чем», и судья обязан
// отвечать на него «не истёк», а не «истёк»: направление ошибки здесь
// несимметрично — ложное «истёк» вытолкнуло бы пробу в исполняемый набор, где её
// job входит в набор обязательных контекстов слияния.
//
// Сам гейт такого входа судье не подаёт (разбор страницы отвергает таблицу, из
// которой не считано ни одного вида), поэтому здесь закрепляется контракт СУДЬИ,

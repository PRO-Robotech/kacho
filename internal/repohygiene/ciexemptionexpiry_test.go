...
//
// Два экземпляра этого класса держатся здесь своими гейтами (оба измерены по
// дереву 2026-08-12):
//
...
//
// # Третий экземпляр снят ВМЕСТЕ со своим предметом
//
// Здесь же держался третий: каталог npm без своего файла блокировки, выведенный
// из-под автообновления зависимостей (dependabot). 2026-08-27 владелец снял
// автообновление целиком — вместе с ним исчез и предмет гейта, поэтому гейт снят,
// а не ослаблен: сверять объявленные исключения стало не с чем и не о чем.
//
// Строка оставлена ради ровно одного читателя — того, кто соберётся автообновление
// ВЕРНУТЬ, и найдёт её тем самым поиском по имени бота, которым такое начинают.
// Вернётся механизм — гейт обязан вернуться с ним В ТОМ ЖЕ изменении: список
// исключений без сверки с деревом немедленно начинает переживать свой предмет, и
// заметить это будет нечем.
//
// # Почему гейты, а не запись в правилах
//
// Запись в правилах переживает свой предмет молча — этим классом вызваны оба.
// Гейт же вооружается сам: у каждого здесь предпосылка ПРОВЕРЯЕТСЯ, и послабление
// становится законным ровно тогда, когда предмет возвращается (появился свой ранер
// — сужение по автору снова защитимо, и гейт отступает без чьей-либо памяти).
//
// # Перепись
//
...
// ─────────────────────────────────────────────────────────────────────────────
// ИНЪЕКЦИЯ: каждый гейт доказывается в ОБЕ стороны — он обязан
...
// ─────────────────────────────────────────────────────────────────────────────
// ТРЕТИЙ ЭКЗЕМПЛЯР КЛАССА: ПОДПОРКА, ВНЕСЁННАЯ МОЛЧА.
//
// Два экземпляра выше — послабления, сужающие ОХВАТ проверки. Этот — послабление,

...
//
// # Два способа назвать предмет, и оба нужны
//
...
//     его не отрицает.
//  2. **Отглагольным именем** — «переименование», «перенос». Требует словаря, и
//     словарь узок НАМЕРЕННО (см. ниже).
//
// # Чего он НЕ судит, и это названо, а не умолчано
//
//  1. **Отглагольные имена, не называющие ОБЪЕКТ действия.** «Обновление»,
//     «удаление», «создание» в словарь не входят и не войдут: у каждого домена
//     несколько ресурсов с этими глаголами, и решает именно объект. Живой пример из
//     дерева: «смена веса выражается снятием и добавлением, а не „обновлением цели“,
//     которого у края нет» — правда про `Target`, тогда как `UpdateTargetGroup`,
//     `UpdateListener` и `UpdateLoadBalancer` в контракте есть. Запись «обновление →
//     Update*» объявила бы это исправное утверждение находкой, и гейт сняли бы
//     первым же ложным срабатыванием. Такие утверждения СЧИТАЮТСЯ переписью как
//     нерезолвящиеся — они видны числом, а не выглядят проверенными.
...
func judgeEdgeSentence(src EdgeSource, line int, sentence string, contract EdgeContract, hasContract bool) edgeSentenceVerdict {
	...
	if len(tokens) == 0 && !denies && !affirms {
	...
	// Способ 2 — отглагольное имя из словаря.

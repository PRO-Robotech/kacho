...
//
// # Область: ОБЕ конфигурации, и послабления больше нет
//
// Построений проверяющего в дереве два: плоскость данных реестра и КРАЙ. F1
// перевела первое, Ф1б (задача #926) — второе; с тех пор в словаре ниже нет ни
// одной записи вне области, и поле Predicate не заполнено ни у одной.
//
// Послабление, которое здесь стояло, СНЯЛОСЬ САМО ровно так, как обещало: оно
// падало, как только пакет края объявит свой состав проверок, — а до тех пор
// именовало предикат втягивания, чтобы было видно, чего ждут. Механика
// сохранена целиком: следующая поверхность приёма, попавшая в словарь вне
// области, обязана назвать свой предикат и истечёт тем же способом.
//
// Гейт, зелёный оттого, что он ни на что не смотрит, здесь запрещён так же, как
// в продукте, поэтому перепись остаётся: построение поверхности всё равно
// ищется в дереве, и запись словаря без построения — находка.
//

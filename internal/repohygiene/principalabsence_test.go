...
//
// # ПЕРЕЧЕНЬ ПУСТ, и это не недосмотр
//
// Здесь стояла запись "pkg/operations/principal_test.go" — она пиннила
// контракт аксессора и репозитория («пустой ctx → SystemPrincipal», «Create
// без явной личности → SystemPrincipal»). Пакет operations переехал целиком в
// общий фундамент (github.com/PRO-Robotech/corelib): его тест того же имени
// живёт теперь ТАМ, под собственным CI отдельного продукта. Обход этого гейта
// (walkGoFiles от repoRoot) — про тестовые файлы ЭТОГО дерева и в кэш модулей
// не заходит by construction, поэтому предмета для локального освобождения у
// переехавшего файла больше нет: его не найдёт даже TestNoBlindPrincipalAssertions,
// освобождать нечего.
//
...
// не возразит. Заведёт этот продукт СВОЙ файл рядом с аксессором контракта —
// запись вернётся сюда вместе с ним, а не раньше.

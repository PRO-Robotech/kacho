...
//     ПОРЯДКЕ ПРИМЕНЕНИЯ, потому что колонку заводят и снимают (в этом дереве
//     снятие уже произошло дважды, и обе таблицы после него перестали быть
//     очередями);
...
func ScanDeliveryMarker(owner, path string, src []byte) ([]ColumnEvent, DeliveryMarkerCensus) {
	...
	for _, m := range createTableRe.FindAllSubmatchIndex(top, -1) {
		...
		census.CreateBodies++
		if !bodyDeclaresColumn(body, DeliveryMarkerColumn) {
			continue
		}
		census.Declared++
...
// СЕГОДНЯ».
//
// События обязаны приходить В ПОРЯДКЕ ПРИМЕНЕНИЯ миграций: колонку в этом дереве
// уже дважды заводили и снимали, и порядок есть единственное, что различает
// «очередь» и «журнал, переставший быть очередью».

...
var retiredQueueEventValues = map[string]string{
	// Здесь стояла запись о снятом значении очереди компенсаций службы доступа.
	// Она снята вместе со своим предметом: очередь заводилась миграциями службы,
	// и после её выноса ни очереди, ни словаря, ни значения в этом дереве нет.
	// Запись, которой нечего исключать, унаследует следующая слепая зона.
}

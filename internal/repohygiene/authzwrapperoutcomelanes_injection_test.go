...
func TestWrapperGateRedOnABoolCallFromAnotherPackage(t *testing.T) {
	rep := scanWrapperSynth(t, guardTree(t, map[string]string{
		"services/iam/internal/apps/kacho/api/access_binding/helpers.go": callerBoolSrc,
	}))
	if rep.files == 0 {
...
func TestWrapperGateSilentWhenTheOutcomeFormIsChosen(t *testing.T) {
	rep := scanWrapperSynth(t, guardTree(t, map[string]string{
		"services/iam/internal/apps/kacho/api/access_binding/helpers.go": callerOutcomeSrc,
	}))
	if len(rep.pairs) != 1 {
...
func TestWrapperGateSurvivesAnImportAlias(t *testing.T) {
	rep := scanWrapperSynth(t, guardTree(t, map[string]string{
		"services/iam/internal/apps/kacho/api/account/list_all_operations.go": callerAliasedSrc,
	}))
	if len(rep.found) != 1 {
...
func TestWrapperGateIgnoresABoolWithoutAPair(t *testing.T) {
	rep := scanWrapperSynth(t, synthCarrierTree(t, map[string]string{
		...
		"services/iam/internal/apps/kacho/svc/g.go": callerUnpairedSrc,
	}))
	if len(rep.pairs) != 0 {
...
func TestWrapperGateIgnoresAnEShapedNeighbourOfTheWrongType(t *testing.T) {
	rep := scanWrapperSynth(t, synthCarrierTree(t, map[string]string{
		...
		"services/iam/internal/apps/kacho/svc2/g.go": callerWrongShapeSrc,
	}))
	if len(rep.pairs) != 0 {
...
func TestWrapperGateIgnoresTheFormInAComment(t *testing.T) {
	rep := scanWrapperSynth(t, guardTree(t, map[string]string{
		"services/iam/internal/apps/kacho/api/doc/doc.go": mentionOnlySrc,
	}))
	if rep.files == 0 {
		t.Fatal("синтетическое дерево не прочитано")
	}
...
func TestWrapperGateIgnoresProbesAndGeneratedCode(t *testing.T) {
	...
	rep := scanWrapperSynth(t, guardTree(t, map[string]string{
		"services/iam/internal/apps/kacho/api/access_binding/helpers_test.go": callerBoolSrc,

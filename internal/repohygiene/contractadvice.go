...
//
// Такой предикат рассматривался и ОТВЕРГНУТ ЗАМЕРОМ, а не по вкусу. По дереву
// (123 своих контракта) имён формы идентификатора в бэктиках — 280, из них НЕ
// резолвятся ни во что объявленное — 90 (54 уникальных). Это коды gRPC
// (`INVALID_ARGUMENT`), имена функций Go (`HasInternalSuffix`,
// `RequireOwnerMatchesPrincipal`), имена проб, надгробия снятых глаголов
// (`GetByValue` — «Замена снятому …», `UpdateMetadata` — «которого в контракте
// нет»). Надгробие называет несуществующий глагол НАМЕРЕННО и является
// ЗАКОННЫМ близнецом: широкий предикат объявил бы находкой ровно то место, где
// автор честно сказал, что глагола нет.
//

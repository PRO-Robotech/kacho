...
if [ "${1:-}" = "--self-test" ]; then
  rc=0
  # ── ВОЗВРАТ ДЕРЕВА — ОБЯЗАННОСТЬ, СНИМАЕМАЯ ЛОВУШКОЙ, А НЕ СТРОКОЙ В КОНЦЕ ──
  #
  # Обе инъекции ниже правят ОТСЛЕЖИВАЕМЫЕ файлы на месте. Пока возврат стоял
  # обычной строкой после проверки, любой выход из окна мимо неё оставлял
  # внесённый дефект В РАБОЧЕМ ДЕРЕВЕ — и это не гипотетика: так и случилось
  # (сигнал прогонщику; наблюдалось `M …/networkpolicy-authz.yaml` в дереве после
  # прерванного прогона). Дальше хуже всего то, что дефект остаётся ровно тот,
  # который ЭТА проверка и ловит: следующий прогон краснеет по-настоящему, на
  # закладке, которую сам гейт и оставил. Мимо строки возврата ведут три пути,
  # и все три реальны: сигнал, срок ожидания и собственный `fail` внутри окна
  # (`инъекция A2 сломала рендер` — выход из-под ног прямо между правкой и
  # возвратом).
  #
  # Ловушка снимает обязанность с последовательности: файл записан — возврат уже
  # назначен. Регистрируется ДО первой правки, покрывает и обычный выход.
  SELFTEST_RESTORE=()
  restore_injected() {
    local i
  ...
  trap 'restore_injected' EXIT
  trap 'restore_injected; exit 130' INT TERM

...
// п. 7). Поэтому читаются ВСЕ ДЕВЯТЬ форм записи отказа этого дерева:
//
...
//	переменная-посредник — `msg := "…"` затем `status.New(codes.X, msg)`
//	                       БЕЗ обёртки и БЕЗ формата — деноминатор всех
//	                       project-scope отказов края (permission_denied_response.go).
//
...
//
// ДЕВЯТАЯ ФОРМА закрывает слепоту, измеренную не предположенную: 15 утверждений
// одного newman-набора (`compute/tests/newman/cases/authz-deny.py`) читали
// принятое расхождение как «производитель пишет иначе», хотя настоящий
// производитель (`gateway/internal/middleware/permission_denied_response.go`,
// project-scope authz-deny через `msg := "permission denied"`) писал ДОСЛОВНО
// то же, что утверждал кейс, — распознаватель его просто не читал: текст доезжал
// до статуса переменной, а не литералом на месте вызова. Единственный, кто
// СЛУЧАЙНО совпал под приведением регистра, был чужой производитель того же
// сервиса (`services/compute/internal/handler/tenant_interceptor.go`,
// admin-гейт internal-листенера — к этим кейсам отношения не имеющий), и
// находка указывала чинить кейс под него — то есть не туда.
//
...
var (
	...
	// обязана заканчиваться закрывающей кавычкой — конкатенация
	// (`msg = "permission denied: " + desc.Action`) сюда не подходит НАМЕРЕННО:
	// у неё нет одного статичного текста, а есть переменная часть.
	//
	// Не сам по себе производитель — им делает [rtStatusVarUse] ниже: без
	// подтверждения, что переменная доезжает до gRPC-статуса КАК ЕСТЬ, всякое
	// присваивание строки стало бы производителем, и корпус утонул бы в
	// логовых сообщениях и ключах настроек.
	rtVarAssignLiteral = regexp.MustCompile(`(?m)^\s*([A-Za-z_]\w*)\s*:?=\s*"((?:[^"\\]|\\.)*)"\s*$`)
	// rtStatusVarUse — та же переменная ВТОРЫМ аргументом `status.New`/
	// `status.Error` (без обёртки, без формата: `status.Errorf` уже читается
	// форматной строкой, ГОСПОДСТВУЮЩАЯ форма — см. rtSprintfText).
	//
	// Форма реальна и не редка: `msg := "permission denied"` затем
	// `status.New(codes.PermissionDenied, msg)` — деноминатор всех
	// project-scope отказов края (`gateway/internal/middleware/
	// permission_denied_response.go`). Без этой формы её текст «производителя
	// не имел» ни разу, а утверждение newman-кейса о нём читалось как
	// расходящееся с ЧУЖИМ, случайно похожим текстом другого владельца —
	// находка о прозе, не о продукте (`testing.md` §«Гейт на класс», п.7).
	rtStatusVarUse = regexp.MustCompile(
		`status\.(?:New|Error)\(\s*codes\.[A-Za-z]+\s*,\s*([A-Za-z_]\w*)\s*[,)]`)
...
func rtProducers(root string, goFiles []string) (map[string]map[string]bool, error) {
	...
	for _, rel := range goFiles {
		...
		// Переменная, доезжающая до gRPC-статуса КАК ЕСТЬ: сперва собрать, ЧЕМ
		// каждое имя было присвоено в ЭТОМ файле, потом отдать литерал только
		// тем именам, что и правда встретились ВТОРЫМ аргументом статуса —
		// иначе обычное `s := "…"` для лога стало бы производителем.
		varLiterals := map[string][]string{}
		for _, m := range rtVarAssignLiteral.FindAllStringSubmatch(whole, -1) {
			varLiterals[m[1]] = append(varLiterals[m[1]], m[2])
		}
		for _, m := range rtStatusVarUse.FindAllStringSubmatch(whole, -1) {
			for _, lit := range varLiterals[m[1]] {
				add(owner, strings.ReplaceAll(lit, "%q", `"%s"`))
			}
		}
		for _, raw := range strings.Split(string(b), "\n") {

...
//
// # Находка — ДОКАЗАТЕЛЬСТВО, а не догадка. Видов три.
//
...
//     недоступно.
//
// # Производителя надо УМЕТЬ ПРОЧИТАТЬ, иначе все три вида молчат
//
...
// п. 7). Поэтому читаются ВСЕ формы записи отказа этого дерева: `status.Error(f)`,
// `fmt.Errorf`, проверка формата чужого id, конструктор с именем поля —
// и литералом, и переменной, — и текст, вынесенный в ТАБЛИЦУ (`"type": "…"`),
// который доезжает до клиента через `status.Errorf(codes.X, "%s", msg)` и по
// вызову не читается вовсе. Вызовы читаются по склеенному тексту файла, а не
// построчно: перенос второго аргумента на следующую строку здесь обычен.
// Каждая форма доказана инъекцией, и рядом с ними стоит контроль — проза
// комментария производителем не становится.
//
// # Что НЕ судится, и это названо, а не умолчано
//
// Приведение регистра там, где производитель пишет текст строчными целиком,
// находкой не считается: доказательства расхождения нет, а «утверждение слабее
// заголовка» без заглавных в заголовке — суждение о прозе, у которого машинного
// предиката нет (`security.md` §«Механического детектора сборки НЕТ»). Граница
// названа прямо, чтобы молчание гейта об этих местах не читалось как их проверка;
...
func rtAuditStep(rel, title string, it nmItem, declared []string, corpus rtCorpus, cen *rtCensus) []rtFinding {
	...
	// находка была бы суждением о прозе. Та же мерка, что у видов 2 и 3, —
	// объявление автора плюс дерево, а не вкус гейта.
	for _, d := range declared {
		if !rtTitleSubst.MatchString(d) {
			continue // объявление конкретно — обобщать нечего
		}
		for _, p := range rtConstParts(d) {
			lp := strings.ToLower(p)
			asserted := false
			for _, lit := range foldedLits {
				if strings.ToLower(lit) == lp {
					asserted = true
					break
				}
			}
			...
			rich := rtRicherProducers(corpus, lp)
			if len(rich) < 2 {
				continue
			}
			sawProof = true
			why = append(why, fmt.Sprintf(
				"заголовок объявил текст %q, обобщённый подстановкой: его постоянная часть — "+
					"лишь %q, и ровно её утверждает шаг. Производитель обобщён не был: у этого "+
					"владельца %d разных отказа несут ту же часть и вдобавок называют, чей он "+
					"(например %q и %q). Утверждение проходит на отказе, которого кейс не "+
					"называл, и подмены одного отказа другим не различает НИ ПРИ КАКОМ ответе. "+
					"Утверждай текст владельца целиком, без toLowerCase()",
				d, p, len(rich), rich[0], rich[1]))
		}
	}

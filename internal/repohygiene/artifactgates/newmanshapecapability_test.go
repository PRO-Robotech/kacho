...
func TestNewmanHelperShapeCapabilityHasACaller(t *testing.T) {
	...
	// ПРЕДМЕТ ГЕЙТА ПЕРЕВЕДЁН НА ПРИЗНАК, КОТОРЫЙ ДЕРЕВО ПРОИЗВОДИТ.
	//
	// Здесь стоял отказ на нулевом числе МНОГОФОРМЕННЫХ возможностей, и довод его
	// был верен: утверждение отрицательное («форма без вызывающего»), а на
	// исчезнувшем предмете такое утверждение замолкает, а не краснеет
	// (`testing.md` §«Гейт на класс» п. 9).
	//
	// Что изменилось: многоформенных возможностей в дереве стало НОЛЬ — последняя
	// потеряла свою вторую форму вместе с её единственным вызывающим (набор службы
	// доступа уехал в её репозиторий, и форма снята там же, где стояла). Требовать
	// их присутствия значило бы требовать, чтобы дерево несло многоформенную
	// возможность ради зелёного гейта.
	//
	// Гейт при этом НЕ вакуумен, и это проверяется выше своим отказом: населением
	// остаются ПРОВЕРКИ ФОРМЫ ПАРАМЕТРА (`cen.subjects`), которых дерево
	// производит непустое число, и всякая новая многоформенная возможность попадёт
	// под требование сама, без чьей-либо памяти. Ноль многоформенных ПЕЧАТАЕТСЯ
	// отдельным числом переписи — «ноль находок» здесь отличимо от «нечего
	// осматривать».
	if cen.capabilities == 0 {
		t.Log("многоформенных возможностей в дереве НОЛЬ: населением гейта остаются проверки " +
			"формы параметра (их число в переписи выше непусто, иначе отказ). Требование " +
			"«у каждой формы свой вызывающий» предмета сейчас не имеет и вернётся вместе с " +
			"первой же многоформенной возможностью")
	}

...
// косметики.
//
// СЕЙЧАС ПЕРЕЧЕНЬ ПУСТ, и это законное состояние, а не незаконченная работа.
// Все три записи были утверждениями ПРОТИВОПОЛОЖНОГО («временный сбой НЕ должен
// становиться отказом в правах») и лежали в дереве службы доступа. Служба
// вынесена отдельным репозиторием, файлов нет, и записям стало нечего
// освобождать — гейт это сказал сам, а запись без предмета унаследует следующая
// слепая зона.
//
// ЧТО ЭТО СТОИЛО, названо вслух: отрицательный контроль НАДпредиката был снят с
// РЕАЛЬНОГО дерева, и такого контроля здесь больше нет. Синтетическим его
// заменять не стали — это была бы подмена предмета: смысл записей ровно в том,
// что совпадения существуют в живом коде. Вернётся такой код в это дерево —
// вернутся и записи.

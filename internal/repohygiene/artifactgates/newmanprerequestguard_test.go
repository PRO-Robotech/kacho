...
func jsCodeSkeleton(src string) string { return jsBlank(src, false) }

// jsCodeKeepingStrings — та же проекция ТЕМ ЖЕ разбором, но содержимое строковых
// литералов оставлено: комментарии и регулярные выражения погашены, строки целы.
//
// Нужна там, где РЕШЕНИЕ принимается по коду, а ЗНАЧЕНИЕ берётся из литерала —
// имя переменной окружения в `pm.environment.set('<имя>', …)`: в скелете это имя
// стёрто вместе с содержимым литерала. Заводить ради него второй разборщик
// нельзя: два кодека расходятся молча и расходятся ровно там, где расхождение не
// видно. Поэтому обе проекции даёт ОДИН проход, а смещения у них совпадают между
// собой и с исходником — находку из скелета можно читать в этой.
func jsCodeKeepingStrings(src string) string { return jsBlank(src, true) }

// jsBlank — общий разбор обеих проекций. `keepStrings` решает единственный
// вопрос: гасить ли СОДЕРЖИМОЕ строковых литералов. Комментарии и регулярные
// выражения гасятся всегда, в обеих.

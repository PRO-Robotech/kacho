...
func TestKnownGapsStillHaveSubject(t *testing.T) {
	...
	if scan.collections == 0 {
		...
		//
		// Это ОТКАЗ ПРЕДПОСЫЛКИ, а не «предмета нет by construction», и потому он
		// КРАСНЫЙ, а не пропуск (#2548). Ведомость законных пропусков
		// `.github/scripts/gate-skips-allowed.txt` говорит это прямо: непрочитанное
		// дерево её основанием не является и чинится настройкой клона или прогона.
		//
		// Разделяет два состояния один вопрос: могло ли дерево законно не содержать
		// newman-наборов? Нет — сюда мы дошли только потому, что knownGaps непуст, то
		// есть исключения объявлены ДЛЯ наборов. Исключения есть, а наборов не
		// прочитано ни одного: это противоречие, а не пустое множество.
		t.Fatalf("newman-наборов не прочитано ни одного, при этом объявлено исключений "+
			"knownGaps %d — судить о сроке годности исключений не по чему. Это отказ "+
			"ПРЕДПОСЫЛКИ (обход смотрит не туда либо дерево неполно), а не законный "+
			"пропуск: исключения объявлены ДЛЯ наборов, значит наборы обязаны "+
			"читаться. Чинится прогоном, а не записью в ведомость пропусков",
			len(knownGaps))
	}

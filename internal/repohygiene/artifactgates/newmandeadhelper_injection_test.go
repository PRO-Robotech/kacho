...
func nmRunDeadDriver(t *testing.T, gen, caseSrc string) deadHelperReport {
	...
	if err != nil {
		// ОТКАЗ ПРЕДПОСЫЛКИ, а не законный пропуск (#2548): неподнятая зависимость
		// ведомостью `.github/scripts/gate-skips-allowed.txt` не прощается — она
		// чинится настройкой прогона.
		//
		// Здесь довод сильнее общего правила и потому назван числом, а не мнением:
		// вердикт юнитов ПРОИЗВОДИТ сам python3 — цель `test-unit` корневого
		// Makefile гонит `go test … -json | python3
		// .github/scripts/go-test-verdict.py`. В прогоне без python3 вердикта нет НИ
		// У ОДНОЙ пробы, поэтому пропуск «python3 не найден» не может быть ни увиден,
		// ни осмыслен: он объявлял бы ось непроверенной внутри прогона, которого не
		// существует.
		t.Fatalf("python3 не найден (%v): ось разбора НЕ ПРОВЕРЕНА. Это отказ "+
			"предпосылки, а не законный пропуск — тот же python3 производит вердикт "+
			"всего прогона юнитов (`make test-unit`), поэтому его отсутствие чинится "+
			"настройкой прогона, а не терпимостью гейта", err)
	}
	dir := t.TempDir()

...
// MigratorCLIInjectionFixtureSuffix — имя файла, доказывающего способность
// гейта упасть.
//
// # Почему такие файлы гейт НЕ судит
//
// Инъекция вносит НАСТОЯЩИЙ дефект настоящим именем — иначе она доказывает не
// то, что гейт умеет падать, а лишь то, что он умеет читать. Под правилом «имя
// одно на продукт» ЛЮБОЕ имя, внесённое фикстурой внутрь дерева продукта, для
// этого продукта неверно **by construction**: верным было бы ровно то, которое
// фикстура и обязана заменить. Значит без изъятия ни одна инъекция имени в
// `services`/`deploy` существовать не может, а гейт, судящий собственное
// доказательство, лишает себя доказательства.
//
// Изъятие узкое НАМЕРЕННО — суффикс `_injection_test.go`, а не всякая проба:
// проба, УТВЕРЖДАЮЩАЯ имя (`services/iam/cmd/migrator/main_test.go`), остаётся
// под наблюдением, потому что она называет имя как ожидание, а не как дефект.
//
// ЦЕНА изъятия печатается переписью ДВУМЯ величинами, и путать их нельзя:
// файлов суффикс накрывает много (инъекций в дереве сотни), а МЕСТ, называющих
// бинарь, уносит единицы. Судить надо вторую: она и есть слепая зона. Первая
// редакция этого абзаца называла «один файл» — величину, верную для второго
// предиката и ложную для первого.
const MigratorCLIInjectionFixtureSuffix = "_injection_test.go"

// MigratorCLIIsInjectionFixture — файл, чьё назначение вносить дефект.
func MigratorCLIIsInjectionFixture(rel string) bool {
	return strings.HasSuffix(rel, MigratorCLIInjectionFixtureSuffix)
}

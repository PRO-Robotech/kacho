...
func TestClassOfPackageResolvesSplitSubtreesAndRefusesUnknownCatalogs(t *testing.T) {
	...
	cases := []struct {
		...
		{"pkg/api/corelib/operation", classCorelib},
		{"pkg/api/corelib/subscription", classCorelib},
...
//
// # Пустая ведомость — ЦЕЛЬ, и проба на ней ПРОХОДИТ (#2548)
//
// Здесь стоял `t.Skip("ведомость пуста — прощать нечего, и это цель, а не
// поломка")`. Текст был верен, а действие — нет: ноль записей означает, что рёбер
// запрещённого направления в фундаменте не осталось НИ ОДНОГО, то есть вынос
// фундамента доведён до конца. Пропуск на этом состоянии давал две беды сразу.
//
// Во-первых, причина не была объявлена в `.github/scripts/gate-skips-allowed.txt`,
// а необъявленный пропуск у гейта дерева — находка прогонщика юнитов. Значит
// УСПЕХ РАБОТЫ уронил бы прогон, и уронил бы не на дефекте.
//
// Во-вторых, это подталкивало держать запись ради зелёного — ровно то, что корпус
// запрещает: проверка, стерегущая перечень послаблений, на пустом перечне обязана
// ПРОХОДИТЬ, объявляя перепись, а не падать.
//
// Пропуск снят. Цикл по пустой ведомости не делает ничего, а перепись внизу
// печатает «строк ведомости рёбер 0» — «ноль находок» остаётся отличимо от «ноль
// прочитанного». Чтобы пустая ведомость не спрятала МЁРТВОГО судью, рядом стоит
// проверка предпосылки: карта запрещённых направлений обязана быть непуста. Без
// неё проба зеленела бы и тогда, когда прощать нечего, и тогда, когда судить
// нечем, — а это разные состояния.
func TestEveryLedgerRowNamesAGenuinelyForbiddenDirection(t *testing.T) {
	t.Parallel()

	if len(forbiddenDirections) == 0 {
		t.Fatal("карта запрещённых направлений пуста — судить нечем: на пустой " +
			"ведомости проба прошла бы, не проверив ничего, и «прощать нечего» стало " +
			"бы неотличимо от «запрещать нечего»")
	}
	...
	t.Logf("перепись: строк ведомости рёбер %d, запрещённых направлений %d; у каждой "+
		"строки названы предмет и запрещённое направление. Ноль строк — ЦЕЛЬ выноса "+
		"фундамента, а не поломка: проба на ней проходит",
		len(knownBoundaryEdges), len(forbiddenDirections))
}

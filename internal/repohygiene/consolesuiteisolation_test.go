...
//
// Своего разбора здесь нет намеренно: вопрос «зовётся ли имя» в пакете решает
// ОДИН предикат — `callsJSName`, — и вторая его копия разошлась бы с первой
// молча. Точка идентификаторным символом не считается, и это как раз то, что
// нужно: `fs.rm(` — тот же вызов записи, `goDeclaredForm(` — другое имя.
func callsFilesystemName(code, name string) bool {
	return callsJSName(code, name)
}

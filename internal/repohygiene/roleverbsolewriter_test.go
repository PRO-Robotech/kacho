...
// `replaceRoleVerbsTx`, свой сырой SQL в слое досева. Его там больше нет:
// предикат `git grep -n replaceRoleVerbsTx -- '*.go'` даёт ПУСТО. Шапка пережила
// свой предмет и посылала читателя искать координату, которой не существует, —
// ровно тот класс, против которого сам гейт и написан.

...
func TestCSD_Run1_ControlIsSilentAndBothTreesWereRead(t *testing.T) {
	...
	// Сторона службы прочитана — судится числом её импортов вообще. Рёбер
	// «служба → платформа» в дереве с kacho#2601 нет ни одного (все вели в форму
	// операции, получившую имя фундамента), поэтому их число контролем не
	// является; законное ребро этого вида доказывает прогон 3, внося его сам.
	require.NotZero(t, cen.ServiceImports,
		"импортов службы прочитано ноль — обход её дерева пуст, и молчание беспредметно")
}

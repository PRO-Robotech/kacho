...
// synthPkg раскладывает один Go-файл как пакет и отдаёт путь к каталогу.
func synthPkg(t *testing.T, name, src string) string {
	...
	if err := os.WriteFile(filepath.Join(dir, "lane.go"), []byte(src), 0o600); err != nil {
		t.Fatalf("подготовка синтетики: %v", err)
	}
	return dir
}
...
func TestGateReds_WhenARefusalIsReturnedAsSuccess(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			dir := synthPkg(t, "swallow", tc.src)
			hits, census := refusalsReturnedAsSuccess(t, dir)
			if census.files == 0 {
...
func TestGateReds_WhenPublicAddressDeleteIsCalled(t *testing.T) {
	dir := synthPkg(t, "publicdelete", injPublicDelete)
	hits, census := publicAddressDeleteCallSites(t, dir)
	if census.files == 0 {
		t.Fatalf("инъекция не прочитана — доказывать было бы нечего")
	}
	if len(hits) == 0 {
...
func TestGateStaysSilent_OnPublicRead(t *testing.T) {
	dir := synthPkg(t, "publicget", twinPublicGet)
	hits, census := publicAddressDeleteCallSites(t, dir)
	if census.files == 0 {
		t.Fatalf("близнец не прочитан — молчание ничего не доказывает")
	}

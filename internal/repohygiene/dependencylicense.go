...
//
// Самоопознания здесь быть не может: разбор читает ТОЛЬКО каталоги модулей в
// кэше (`ModuleCacheDir`) и своего дерева не касается ни одним чтением. Ровно
// поэтому литерал написан целиком, а не собран из частей: сборка выглядела бы
// защитой от того, чего нет, и следующий читатель искал бы несуществующий
// механизм.

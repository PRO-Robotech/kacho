...
//
// Служба доступа одна и единственна: её прод-код входит в её собственную
// поставку, поэтому свой источник читателей она выводит обходом своего модуля —
// в обеих посадках одинаково. Объявить его в фундаменте значило бы завести
// второе место об одном предмете там, где вывод доступен, и после разреза
// сверять запись стало бы нечем: она пережила бы предмет молча.
//
// Перечень ИСТЕКАЕТ САМ: имя, которого нет ни в объявлении, ни в дереве, —
// находка ниже.
//
// В ДЕНЬ РАЗРЕЗА ЭТА ЗАПИСЬ ОБЯЗАНА СТАТЬ КРАСНОЙ, и это её работа, а не
// поломка. Каталога службы в дереве платформы не останется, скрывать станет
// нечего — и гейт скажет об этом сам, вместо того чтобы молча пропускать имя,
// которого нет. Снятие: удалить запись тем же изменением, которым уезжает
// служба. Проверено прогоном на дереве БЕЗ каталога службы: сверка объявления
// с кодом там работает (5 модулей, 12 пар, 724 прод-файла), а красной остаётся
// ровно эта строка.
var selfGatingSkipModules = map[string]string{
	"iam": "служба доступа выводит своих читателей обходом собственного модуля",
}

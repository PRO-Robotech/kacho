...
func TestNameFormConstraintSuffixMatchesMigrations(t *testing.T) {
	...
	// Число обязано двигаться вместе с деревом — оно и есть перепись, а не
	// украшение. Шестой схемой был iam (#1279), пришедший к канону последним;
	// она ушла вместе со своим деревом при выносе службы доступа отдельным
	// продуктом, и число опущено ТЕМ ЖЕ изменением. Двусторонний храповик это и
	// показал: «найдено 5, ожидалось 6».

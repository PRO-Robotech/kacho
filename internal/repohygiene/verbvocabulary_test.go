...
var verbLiteralRoster = []verbLiteral{
	{
		path: "internal/repohygiene/membershipreadrelation.go", varName: "mrrVerbRelations",
		claims: "ВТОРАЯ сторона сравнения, а не словарь платформы: глагольные отношения типа " +
			"`account`, которые НАМЕРЕННО не читают ярус распорядителя. Литерал существует " +
			"затем, чтобы гейт чтения членства доказывал ВЫБОР отношения сравнением двух " +
			"объявлений модели, а не присутствием одного: присутствие ловит опечатку, ошибку " +
			"ловит сравнение",
		checkedBy: "repohygiene: TestMembershipReadIsGatedByTheTierRelationNotTheVerbOne читает " +
			"МОДЕЛЬ и краснеет, если названное здесь отношение начало читать ярус (тогда " +
			"сравнение перестало различать); TestMRR_ComparisonOfTwoDeclarationsIsLive " +
			"доказывает это инъекцией в обе стороны",
		retireWhen: "объявление удалено — вместе с гейтом чтения членства либо с переходом " +
			"сравнения на вывод из модели вместо перечисления",
	},
	{

...
//
// Гейты уникальности (`TestMigrationVersionsAreUniquePerService`,
// `TestMigrationVersions_UniquePerService`) остаются backstop'ом: они ловят
// столкновение, если оно всё-таки образуется. Этот разбор ловит ПРОЦЕДУРУ,
...
// Верхняя граница нужна не для симметрии: в дереве есть ТРЕТЬЯ форма,
// которую никто не заказывал. `migrator create` (любого сервиса) зовёт
// `goose.Create` без `SetSequential`, то есть выдаёт имя с 14-значной меткой
// времени (`20260817042704_имя.sql`). В дереве таких файлов ноль — глагол не
// использовали ни разу, — но он существует и исполним, и без верхней границы
// его вывод прошёл бы под видом «выведено из задачи» (порядковый разряд у
// метки времени почти всегда ненулевой). Задача номер десять миллионов на этом
			...
			if v > lastIssuedMigrationVersion {
				out = append(out, fmt.Sprintf(
					"%s: номер %d больше всякого возможного (задача №%d не существует).\n"+
						"    Так выглядит метка времени — её выдаёт `migrator create`, и это "+
						"ТРЕТЬЯ форма, которой в дереве нет ни одного файла.\n"+

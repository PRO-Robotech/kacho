...
// подать; инъекция в обе стороны — в
// tools/revocationwindowgate/ownknob_injection_test.go.
//
// ЗДЕСЬ СТОЯЛА координата с приставкой `revocationwindowownknob`, которой в
// дереве не было НИ ДНЯ (мёртвое имя целиком здесь не воспроизводится: в форме
// координаты оно читается держателем обещаний как живое утверждение). Доказательство при этом существовало и существует — просто под
// другим именем и в каталоге вердикта. Обещание с выдуманной координатой хуже
// отсутствующего: оно читается как само доказательство, а искать его посылает не
// туда. Файл переименован в форму `*_injection_test.go`, чтобы держатель
// обещаний (#2519) его РЕЗОЛВИЛ, а не считал молчаливо отсутствующим.

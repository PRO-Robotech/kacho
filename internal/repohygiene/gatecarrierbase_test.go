...
//  3. оговорка обособлена С ОБЕИХ сторон, и ОБЕ запятые выписаны здесь, а не
//     взяты у подсудимого: слева их ставит вызывающий, справа шаблон, внутри
//     [gateCarrierCauseHedge] нет ни одной. Эталон, собранный из той же
//     константы, переехал бы вместе с ней — снятие хвостовой запятой сдвинуло бы
//     и текст, и ожидание, и проба осталась бы зелёной («родителя скорее всего,
//     нет» — первая редакция, обе ветви).
//
...
func TestGateCarrierHedgeIsBoundToItsCause(t *testing.T) {
	...
	for _, shallow := range []bool{false, true} {
		...
		// ОБОСОБЛЕНИЕ СУДИТСЯ ОБЕИМИ ЗАПЯТЫМИ, И ОБЕ НАПИСАНЫ ЗДЕСЬ. Ни одна
		// не живёт внутри [gateCarrierCauseHedge]: константа несёт только слово,
		// поэтому эталон не может переехать вслед за правкой пунктуации у
		// производителя. Прежняя редакция брала правую запятую из той же
		// константы — снятие хвостовой запятой сдвигало и текст, и ожидание, и
		// проба объявляла зелёным обособление, которого не стало.
		if setOff := ", " + gateCarrierCauseHedge + ", "; !strings.Contains(text, setOff) {
			t.Errorf("мелкий=%v: оговорка обособлена не с обеих сторон — %q в отказе "+
				"нет: %q", shallow, setOff, text)
		}
	}
}

...
func TestDeclaredSegmentAgreesWithTheTree(t *testing.T) {
	...
	// Положительный контроль СПРАШИВАЕТСЯ ТОЛЬКО У НЕПУСТОЙ КАРТЫ.
	//
	// Здесь стояло безусловное требование «хоть одна служба названа своим
	// именем», и оно роняло прогон на ДОСТИЖЕНИИ СВОЕЙ ЦЕЛИ: карта опустела
	// вместе с выносом единственной такой службы, то есть в дереве не осталось
	// ни записи, ни её предмета — состояние верное, а проверка объявляла его
	// поломкой. Проверка, краснеющая на верном дереве, толкает держать запись
	// ради зелёного — ровно то послабление, против которого она заведена.
	//
	// Что осталось нормой: запись БЕЗ предмета — по-прежнему находка, и её
	// ловит ветка выше (каталога объявленного сегмента нет). Пустая карта
	// находкой не является и печатается переписью.
	if len(ownSegments) > 0 && own == 0 {

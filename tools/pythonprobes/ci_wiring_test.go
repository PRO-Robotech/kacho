...
// ТРИ, и это не дубль: `services/*/tests/newman/scripts/` — пробы генератора
...
// восемь наборов и потому не принадлежит ни одному; `tests/authz-fixtures/` —
// пробы ПОСЕВА, который поднимает фикстуры боевой посадки ДО первой коллекции.
...
//
// Каталогов ТРИ, и ни один не дубль соседа: пробы вокруг ВЕРДИКТНОГО СЛОЯ
...
// `tests/newman/scripts/`, рядом со своим предметом. Пробы ПОСЕВА
// (`tests/authz-fixtures/`) лежат рядом со своим: посев исполняется ДО первой
// коллекции, и его отказ не оставляет отчётов вовсе — вердиктный слой тогда
// сообщает, что судить нечего, а не судит. Предмет третий, дом третий.
//

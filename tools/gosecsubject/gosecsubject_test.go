...
// ── ЦЕЛЕВОЕ СОСТОЯНИЕ: ВЕДОМОСТЬ ПУСТА ──────────────────────────────────────
//
// Ведомость пустеет ровно тогда, когда долг закрыт: инертных директив в дереве
// не осталось. Проверка, падающая на достижении СВОЕЙ ЦЕЛИ, подталкивала бы
// держать запись ради зелёного, поэтому пустая обязана ПРОХОДИТЬ — а вывод
// обязан называть это целью, иначе оператор прочтёт молчание как «не смотрели».
//
// Утверждается ПАРА, и обе половины нужны: пустая проходит И перепись при этом
// печатается. Первое без второго зеленело бы на гейте, который не прочитал
// ничего. Плюс законный близнец: при НЕПУСТОЙ ведомости строки про цель нет —
// без него утверждение о ней было бы истинным всегда.
func TestEmptyLedgerWithNothingInertIsTheTargetState(t *testing.T) {
	uncovered, stale := ApplyLedger(nil, nil)
	if len(uncovered) != 0 || len(stale) != 0 {
		t.Fatalf("пустая ведомость при нуле инертных директив обязана проходить; "+
			"получено: подавлений без предмета %d, устаревших записей %d",
			len(uncovered), len(stale))
	}

	var target strings.Builder
	Print(&Report{Census: Census{
		TrackedGoFiles: 5923, Candidates: 284, Parsed: 284,
		Directives: 424, Judged: 153, Unjudged: 271,
		Live: 153, Inert: 0, LedgerRows: 0,
	}}, &target)
	if got := target.String(); !strings.Contains(got, "цель механизма") {
		t.Errorf("вывод не называет пустую ведомость целью — молчание гейта тогда "+
			"неотличимо от «не смотрели». Напечатано:\n%s", got)
	}
	if got := target.String(); !strings.Contains(got, "директив прочитано 424") {
		t.Errorf("перепись не напечатана: «ноль находок» обязано быть отличимо от "+
			"«ноль прочитанного». Напечатано:\n%s", got)
	}

	var busy strings.Builder
	Print(&Report{Census: Census{Directives: 1, Judged: 1, Live: 0, Inert: 1, LedgerRows: 1}}, &busy)
	if got := busy.String(); strings.Contains(got, "цель механизма") {
		t.Errorf("строка про цель напечатана при НЕПУСТОЙ ведомости — она не различает "+
			"состояния, и утверждение выше было бы истинным всегда. Напечатано:\n%s", got)
	}
}

...
//
// Замер, на котором это построено (пин v2.28.0, оба модуля дерева, 2026-09-06,
// ПОСЛЕ снятия инертных директив): находок с учётом подавленных 125 + 36, из них
// подавлено 120 + 36, не подавлено ничем 5 + 0. Множества сходятся, значит
// перечень подавленного полон. Числа — ОРИЕНТИР и стареют с каждой директивой:
// гейт не сверяется ни с одним из них, перемеряй прогоном.
//
// # Три исхода, а не два
//
...
//     узнать об этом было бы неоткуда. Замер на дереве (2026-09-06): 118 и 35 у
//     сканера, 118 и 35 у гейта. Сверяются они между собой на КАЖДОМ прогоне,
//     поэтому эти числа — ориентир, а не порог.

...
//
// Перед первым замером здесь стоит ПАРНЫЙ отрицательный контроль: обычный
// посторонний на том же объекте обязан получить отказ. Без него «разрешено
// каскадному» неотличимо от «разрешено всем», и самая быстрая форма — та, что
// разрешает каждому, — выиграла бы эту ячейку, не тронув ни одного соединения.
//

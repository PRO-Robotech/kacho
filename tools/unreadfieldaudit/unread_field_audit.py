...
# одного файла не прочитав). Поэтому граница объявляется НЕ по домену, а по КОРНЮ
# дерева контрактов, и признак ВЫВОДИТСЯ из дерева:
#
#   корень, НИ ОДИН домен которого не реализован в этом дереве, — ВНЕШНИЙ;
#   домен под корнем, чьи СОСЕДИ здесь реализованы, вне дерева быть не может.
#
# Признак самоистекает в обе стороны. Заведись под внешним корнем домен,
# реализованный здесь, — его соседи перестанут извиняться в тот же прогон. Уедь
# отсюда служба, чей корень остался общим (`kacho`), — она получит ОТКАЗ, а не
# тихое извинение: решение «объявить границу» придётся принять ЯВНО, перенеся
# контракт под свой корень, как это и сделано для службы доступа (kacho#2362,
# `proto/kaname/`). Перечня вынесенных служб здесь нет намеренно: рукописный
# перечень — четвёртая копия того, что уже лежит в дереве, и разошёлся бы он молча.
def domains_under(root):
    """Домены, объявленные под этим корнем дерева контрактов.

    Пустой перечень сделал бы правило границы ВАКУУМНЫМ (корень без домена
    «не реализован» by construction), поэтому его непустоту утверждает
    положительный контроль самопроверки, а не эта функция.
    """
    return sorted(
        os.path.basename(os.path.dirname(d))
...
def root_implemented_domains(index, root):
    """Домены корня, чьё СОБСТВЕННОЕ прод-дерево в индексе НЕПУСТО.

    Пустой перечень означает, что в этом дереве корня нет вовсе — только его
    контракты и стабы.

    Считается ПО ИНДЕКСУ, а не по наличию каталога на диске: каталог бывает на
    диске и не обойден (свой `go.mod`, build-тег, отказ типизации), и тогда «есть
    каталог» сказало бы о ПРОЧИТАННОМ неправду — то есть завело бы ту же ложную
    предпосылку с другой стороны.
    """
    return [d for d in domains_under(root) if own_tree_packages(index, d)]
...
def main():
    ...
    for domain in domains:
        ...
        if not sources or not own_pkgs:
            ...
            #
            # ИСХОДОВ ДВА, И РАЗЛИЧАЕТ ИХ КОРЕНЬ ДЕРЕВА КОНТРАКТОВ, а не догадка о
            # том, «переехала ли служба» (см. root_implemented_domains):
            #
            #   * корень реализован в этом дереве другими доменами ⇒ служба ЗДЕСЬ,
            #     а обход её не нашёл: ОТКАЗ (rc=2), и сообщение называет, какой
            #     каталог её запросы принимает;
            #   * корень не реализован здесь НИ ОДНИМ доменом ⇒ служба вынесена
            #     отдельным продуктом: ГРАНИЦА — объявленная, напечатанная и
            #     посчитанная. Поля её запросов не измерены и измерены быть не
            #     могут: читатель живёт в другом дереве.
            root = domain_root(domain)
            kin = root_implemented_domains(index, root)
            if kin:
                premise_failures.append(
                    (domain, prod_pkg_prefixes(domain)[0], len(sources),
                     len(own_pkgs), root, kin,
    ...
    for d, root, nreq, nmsg in out_of_tree:
        print(f"    ВНЕ ДЕРЕВА: домен {d!r} — корень контрактов {root!r} в этом "
              f"дереве НЕ реализован ни одним доменом (перечислено под ним: "
              f"{', '.join(domains_under(root))}), то есть служба вынесена отдельным "
              f"продуктом. Публичных типов запроса {nreq}, достижимых сообщений "
              f"{nmsg} — НЕ измерены и измерены быть не могут: читатель их полей "
              f"живёт в другом дереве")
    ...
    for d, pref, nsrc, nown, root, kin, handled in premise_failures:
        who = ", ".join(
        ...
        print(f"ПРЕДПОСЫЛКА НЕ ВЫПОЛНЕНА: у домена {d!r} прочитано файлов прод-кода "
              ...
              f"деревья {prod_trees(d)}) — домен НЕ измерен. Корень контрактов "
              f"{root!r} в этом дереве РЕАЛИЗОВАН ({', '.join(kin)}), значит служба "
              f"лежит здесь, а обход её не нашёл — границей это быть не может. "
              f"Обработчик её публичных запросов объявляет: {who}. Либо каталог "
              f"сервиса не сопоставлен домену в DOMAIN_SERVICE_DIR, либо служба "
              f"вынесена в свой Go-модуль, а обход в него не спустился")

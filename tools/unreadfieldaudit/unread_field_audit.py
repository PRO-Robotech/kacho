...
предполагаются; при невыполнении — выход с кодом 2, не «чисто»):
  ...
  * КАЖДОЕ ОБЪЯВЛЕНИЕ МОДУЛЯ В ДЕРЕВЕ ОБОЙДЕНО, каждый обойденный модуль дал хоть
    один пакет, и объём осмотренного печатается ПО МОДУЛЯМ ПОРОЗНЬ. `go list` в
    каталог со своим `go.mod` не спускается by construction: пока перепись знала
    одно суммарное число, обход одного дерева был неотличим от обхода обоих.
    СКОЛЬКО МОДУЛЕЙ ЖДАТЬ — ЧИТАЕТСЯ ИЗ ДЕРЕВА, А НЕ ОБЪЯВЛЯЕТСЯ ЗДЕСЬ ЧИСЛОМ, и
    различие проверено обеими сторонами: на `origin/main` объявлений два (корень и
    `services/iam` с модулем `kaname`), на линии выноса — одно, потому что служба
    ушла отдельным продуктом. Число, стоявшее у читателя переписи литералом,
    утверждало о дереве первое и роняло прогон на втором, хотя второе верно.
...
# Каталоги, в которые обход НЕ СПУСКАЕТСЯ намеренно: их содержимое не прод-код
# этого дерева. Набор совпадает с набором `nestedModulesUnder` Go-обходчика — и
# это несущее совпадение, а не аккуратность: разойдись он, и перепись объявлений
# начала бы ожидать модуль, в который обход не спускается by construction, то есть
# дала бы ВЕЧНОЕ ложное красное вместо послабления, истекающего от факта дерева.
WALK_SKIP_DIRS = {".git", "node_modules", "vendor", "testdata"}


def pattern_literal_dir(pattern):
    """Буквальный каталог пакетного образца — часть до первого `...`."""
...
def declared_module_dirs(patterns):
    """Каталоги, объявляющие Go-модуль, которые обход ОБЯЗАН накрыть.

    ЗАЧЕМ. `go list` в каталог со СВОИМ `go.mod` не спускается by construction,
    поэтому обход обязан идти ПО МОДУЛЯМ, а его перепись — сверяться с деревом.
    Пока ожидаемое число модулей стояло у читателя переписи литералом, оно о
    дереве не спрашивало, а утверждало: после того как служба доступа ушла
    отдельным продуктом, объявление в дереве осталось одно, и литерал ронял
    прогон на факте, который верен. Обратная ошибка того же литерала дороже:
    третье объявление он пропустил бы МОЛЧА, и прод-код третьего модуля стал бы
    невидим — ни находкой, ни закрытием.

    Выводится из дерева И из образцов САМОГО обхода, а не выписывается: перечень,
    записанный руками, разошёлся бы с деревом молча — ровно так эта слепота и
    завелась.

    ГРАНИЦА НАЗВАНА: объявление ВНЕ буквальных каталогов обойдённых образцов сюда
    не попадает, и это не послабление. Обход покрывает деревья прод-кода, где
    законно живёт читатель поля запроса; ожидание модуля из чужого дерева
    (оснастка, консоль) заставило бы гейт компилировать репозиторий целиком,
    ничего не добавив к вердикту о полях.
    """
    out = set()
    if os.path.isfile("go.mod"):
        out.add(".")
    for pat in patterns:
        if not pat.startswith("./") and pat != ".":
            # Образец не адресует каталог этого дерева: Go-обходчик берёт по нему
            # корневой модуль и вложенных под ним не ищет вовсе. Ожидать их здесь
            # значило бы требовать того, чего обход не делает.
            continue
        base = pattern_literal_dir(pat)
        if base == "." or not os.path.isdir(base):
            continue
        for cur, dirs, files in os.walk(base):
            dirs[:] = [d for d in dirs if d not in WALK_SKIP_DIRS]
            if "go.mod" in files and os.path.isfile(os.path.join(cur, "go.mod")):
                out.add(os.path.relpath(cur, ".").replace(os.sep, "/"))
    return out


def module_census_verdict(declared, walked):
    """Вердикт переписи модулей: (исход, объяснение); пустой исход — молчание.

    ИСХОДОВ ТРИ, И ТРЕТИЙ НЕ РАСТВОРЯЕТСЯ: обойдено всё объявленное · обойдено
    меньше объявленного · СУДИТЬ НЕЧЕМ (перепись объявлений либо обход пусты).
    Третий отдельным исходом потому, что «ноль объявлений» и «ноль обойдённых»
    дают разность, равную пустому множеству, — то есть при вычитании выглядят как
    полный обход.

    ПОРЯДОК ВЕТВЕЙ НЕСУЩИЙ: беспредметность и пустой обход стоят ПЕРЕД сравнением,
    и единственная ветвь, выносящая молчание, достижима только после того, как
    оба числа оказались больше нуля. Условие «прочитано больше нуля» стоит здесь,
    в коде решения, а не в тексте вывода: напечатанное число само ничего не
    утверждает.
    """
    walked_dirs = {((m.get("dir") or "").replace(os.sep, "/") or ".")
                   for m in walked}
    if not declared:
        return ("нечем",
                "объявлений модуля в дереве прочитано 0 — ни одного `go.mod` "
                "ни в корне, ни под обойдёнными каталогами. Сверять обход не с "
                "чем, и «обойдено всё» означало бы «ничего не прочитано»")
    if not walked_dirs:
        return ("пусто",
                f"обойдено Go-модулей 0 при {len(declared)} объявлении(ях) в "
                f"дереве ({', '.join(sorted(declared))}) — обход не состоялся, "
                f"и чтения полей во всех модулях НЕВИДИМЫ")
    missing = sorted(declared - walked_dirs)
    if missing:
        return ("пропуск",
                f"обойдено Go-модулей {len(walked_dirs)} из {len(declared)} "
                f"объявленных в дереве; не обойдены: {', '.join(missing)}. "
                f"`go list` в модуль со своим `go.mod` не спускается сам, "
                f"поэтому прод-код этих модулей НЕВИДИМ: ни находка, ни "
                f"закрытие. Обойдено: {', '.join(sorted(walked_dirs))}")
    return ("", "")
...
def proto_package(path):
    ...
    без сегмента версии (`corelib.subscription` уже такой — и он же пример того,
    что каталог на диске левую часть FQN не задаёт вовсе). Вывод по имени
...
# порты — замер даёт 8 файлов ПРОД-КОДА у домена, чья служба вынесена (интерфейсы
# `LookupSubject`, `RegisterResource`, `Revoke`), поэтому признак по сигнатуре
# обработчика дал бы ВЕЧНОЕ КРАСНОЕ на верном факте. По той же причине не годится
# и признак «кто-то здесь читает поля его сообщений»: таких пакетов вне известных
# прод-деревьев домена 8. Встраивание
# `Unimplemented<Служба>Server` признаком тоже не является: его объявляют подделки,
# и у вынесенного домена таких вхождений 18, все в пробах (прод-кода 0).
#
...
def stub_local_names(text, domain):
    ...
    маской):
      ...
      * с псевдонимом (`iamv1 "…/iam/v1"`) — импортов пакетов стабов доменов 602,
        из них у вынесенной службы 88 под двумя разными псевдонимами;
...
def server_registrations(index, domain):
    ...
    `has_handler` форм оказалось три, а не одна):
      ...
      * `<стаб>.Register<Служба>Server(s, impl)` — порождённая обёртка; обращений
        в ПРОД-КОДЕ 61 (125 вместе с пробами, но пробы индекс не читает), то есть
        единственная употребляемая;
    ...
    намеренно не распознаётся: его объявляют подделки. У домена вынесенной службы
    таких вхождений 18 (14 под одним псевдонимом, 4 под другим) и все — в пробах;
    прод-кода среди них 0. Признак по нему дал бы вечное красное, как только
    подделка переехала бы в непроверочный файл.
    """
    per = {}
    for pkg in index["packages"]:
        for rel in pkg["files"]:
...
"""
    ...
    # ── ОБЪЯВЛЕНИЯ МОДУЛЯ В ДЕРЕВЕ ПРОТИВ ОБОЙДЁННЫХ ────────────────────────
    #
    # `go list` в каталог со СВОИМ `go.mod` не спускается by construction, поэтому
    # перечень обойдённых модулей ОБЯЗАН сверяться с деревом. Пока ожидаемое число
    # стояло в конвейере литералом (`mods < 2`), оно о дереве не спрашивало, а
    # утверждало: служба доступа ушла отдельным продуктом, объявление в дереве
    # осталось ОДНО, и литерал ронял прогон на факте, который верен. Обратная
    # ошибка того же литерала дороже: третий модуль он пропустил бы МОЛЧА.
    #
    # ФОРМ РАСПОЛОЖЕНИЯ `go.mod` В ДЕРЕВЕ СЕМЬ, и каждая доказана инъекцией ниже:
    # корень · СТРОГО под буквальным каталогом образца · под `vendor/` · под
    # `testdata/` · под `node_modules/` · под `.git/` · ВНЕ обойдённых деревьев.
    # Первые две — объявления, которые обход ОБЯЗАН накрыть; последние пять ими
    # не являются: обход в эти каталоги не спускается намеренно, и ожидание их
    # дало бы вечное ложное красное вместо послабления, истекающего от дерева.
    # Форма, о которой перепись не знает, даёт не красное и не зелёное, а
    # молчание, — поэтому перечислены числом и каждая проверена порознь.
    fixture_pats = ["./services/...", "./gateway/...", "./internal/..."]
    with tempfile.TemporaryDirectory() as root:
        decl_dirs = [
            "", "services/alpha", "services/beta/vendor/x",
            "services/beta/testdata/y", "services/beta/node_modules/z",
            "services/beta/.git/w", "ui",
        ]
        for d in decl_dirs:
            os.makedirs(os.path.join(root, d) if d else root, exist_ok=True)
            with open(os.path.join(root, d, "go.mod"), "w", encoding="utf-8") as f:
                f.write(f"module example.com/{d or 'root'}\n\ngo 1.26.0\n")
        os.makedirs(os.path.join(root, "gateway"), exist_ok=True)
        os.makedirs(os.path.join(root, "internal"), exist_ok=True)
        prev = os.getcwd()
        os.chdir(root)
        try:
            declared = declared_module_dirs(fixture_pats)
            missed = module_census_verdict(declared, [{"dir": "."}])
            full = module_census_verdict(
                declared, [{"dir": "."}, {"dir": "services/alpha"}])
            subjectless = module_census_verdict(set(), [{"dir": "."}])
            no_walk = module_census_verdict(declared, [])
        finally:
            os.chdir(prev)

    # (30) КРАСНОЕ до фикса: объявление под обойдённым каталогом, которого обход
    #      не накрыл, названо поимённо И числом — иначе «обойдено меньше» не
    #      отличить от «обойдено не то».
    check("пропущенный вложенный модуль назван поимённо и числом",
          missed[0] == "пропуск" and "services/alpha" in missed[1]
          and "1 из 2" in missed[1], f"вердикт: {missed}")
    # (31) МОЛЧАНИЕ, законный близнец: оба объявления накрыты — послабление
    #      истекает от факта дерева само, без правки порога.
    check("обход, накрывший все объявления дерева, молчит",
          full[0] == "", f"вердикт: {full}")
    # (32) КРАСНОЕ: перепись объявлений пуста — это БЕСПРЕДМЕТНОСТЬ, а не «всё
    #      накрыто». Условие «прочитано больше нуля» стоит в коде решения, а не
    #      в тексте вывода: без него пустая перепись давала бы проход.
    check("пустая перепись объявлений — отказ, а не молчание",
          subjectless[0] == "нечем", f"вердикт: {subjectless}")
    # (33) КРАСНОЕ: обойдено ноль модулей при непустой переписи.
    check("пустой обход при непустой переписи — отказ",
          no_walk[0] == "пусто", f"вердикт: {no_walk}")
    # (34)-(37) МОЛЧАНИЕ по каждой форме, которую обход не видит намеренно.
    check("корневой go.mod — объявление, которое обход обязан накрыть",
          "." in declared, f"перепись: {sorted(declared)}")
    check("go.mod под vendor/ объявлением не считается",
          not any(d.startswith("services/beta/vendor") for d in declared),
          f"перепись: {sorted(declared)}")
    check("go.mod под testdata/, node_modules/, .git/ объявлением не считается",
          not any("testdata" in d or "node_modules" in d or ".git" in d
                  for d in declared), f"перепись: {sorted(declared)}")
    check("go.mod ВНЕ обойдённых деревьев объявлением не считается",
          "ui" not in declared, f"перепись: {sorted(declared)}")
...
def main():
    ...
    # них — отказ. Модулей в дереве бывает больше одного; пока перепись знала одно
    ...
    # ПЕРЕЧЕНЬ ОЖИДАЕМЫХ МОДУЛЕЙ ВЫВОДИТСЯ ИЗ ДЕРЕВА, А НЕ ВЫПИСЫВАЕТСЯ. Читатель
    # переписи знал ожидаемое число литералом — и литерал утверждал о дереве факт
    # вместо того, чтобы его прочитать: после выноса службы доступа отдельным
    # продуктом объявление в дереве осталось ОДНО, и прогон падал на факте, который
    # верен. Обратная ошибка того же литерала дороже: третье объявление он пропустил
    # бы МОЛЧА. Теперь послабление истекает от факта дерева само: появится вложенный
    # модуль — обход обязан его накрыть, исчезнет — ожидание исчезнет вместе с ним.
    walk_patterns = index.get("patterns") or []
    if not walk_patterns:
        print("ПРЕДПОСЫЛКА НЕ ВЫПОЛНЕНА: индекс не называет образцов обхода — "
              "сверить перечень обойдённых модулей с деревом нечем, и «обойдено "
              "всё» означало бы «не проверено ничего»", file=sys.stderr)
        return 2
    declared_mods = declared_module_dirs(walk_patterns)
    census_kind, census_why = module_census_verdict(declared_mods, idx_mods)
    if census_kind:
        print(f"ПРЕДПОСЫЛКА НЕ ВЫПОЛНЕНА ({census_kind}): {census_why}",
              file=sys.stderr)
        return 2
    ...
    # Ожидаемое число печатается РЯДОМ с обойдённым и выведено из дерева: читатель
    # переписи сверяет два числа, а не число с литералом, который о дереве молчит.
    print(f"объявлений модуля в дереве, которые обход обязан накрыть: "
          f"{len(declared_mods)}")
    for m in idx_mods:

...
// перепись по свойству нашла СЕМЬ. Четыре из них — включая живую дверь
// `InternalIAMService.WriteCreatorTuple` — в перечне не значились. Поэтому места
// берутся не грепом по именам, а переписью `engineplaces`, где перечень методов
// ВЫВОДИТ КОМПИЛЯТОР из якорного типа клиента движка, а метод без рода роняет саму
// перепись.
//
...
var engineWriteDoors = map[string]doorRuling{
	...
	"services/iam/internal/service/fga_tuple_writer.go": {
		Verdict: doorException, Places: 1,
		Why: "RelationProjector.WriteRaw ← InternalAuthorizeService.WriteTuples: административная дверь, пишет кортеж мимо журнала. " +
			"Вызывающих в дереве НОЛЬ (предикат: grep -rn '\\.WriteRaw(' --include=*.go | grep -v _test.go → только объявление и хендлер).",
		Predicate: "терминальный исход — СНЯТЬ RPC (ноль вызывающих); снятие ломает контракт proto и идёт своим PR с proto-api-reviewer — задача #788.",
	},
	"services/iam/internal/apps/kacho/api/internal_iam/handler.go": {
		Verdict: doorException, Places: 1,
		Why: "InternalIAMService.WriteCreatorTuple: пишет кортеж создателя мимо журнала. Вызывающих в дереве НОЛЬ — " +
			"все пять соседей ушли на RegisterResource (nlb называет это дословно: «Replaces the former direct WriteCreatorTuple»).",
		Predicate: "терминальный исход — СНЯТЬ RPC (ноль вызывающих); снятие ломает контракт proto и идёт своим PR с proto-api-reviewer — задача #788.",
	},
}

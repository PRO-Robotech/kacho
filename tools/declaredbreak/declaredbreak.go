...
//
// Здесь стояла догадка: «наиболее вероятная причина — форма путей, запускай из
// каталога контрактов». Добросовестная, правдоподобная и неверная: прогонщик зовёт
// `buf` именно изнутри каталога, а расхождение в измеренном случае пришло оттуда,
// что судимое дерево было не тем, которое уезжало (#2594). Читатель пошёл чинить
// форму путей и дефекта там не нашёл, потому что его там не было.
//
// Вторичная догадка в тексте отказа опаснее её отсутствия: она уводит от предмета и
// стоит захода. Поэтому отказ печатает обе половины вердикта числами и по ОБРАЗЦУ с
// каждой стороны — это данные, которые гейт действительно видел, — а вывод о причине
// оставляет читателю: у него есть то, чего у гейта нет, — знание, какое дерево
// судилось и каким вызовом получен вход.

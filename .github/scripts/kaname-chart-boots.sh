...
#
# ЧУЖИЕ ВИДЫ — ОТТУДА ЖЕ. Чарт везёт `PrometheusRule`, вид чужого оператора;
# в пустом кластере схема отвергает его и вместе с ним ВСЮ установку, а не одну
# тревогу. Определение вида кладёт здесь тот же оператор — по пину апстрима, а
# не своей урезанной копией: копия принимала бы то, что настоящая схема
# отвергает. Перечень печатается ключом `--foreign-kinds` и сверяется с тем, что
# чарт правда везёт, гейтом
# `TestForeignKindsTheChartShipsAreEstablishedByTheBootOwner`.
#
...
# Чужие виды:    bash .github/scripts/kaname-chart-boots.sh --foreign-kinds
...
# ── ЧУЖИЕ ВИДЫ, КОТОРЫЕ ВЕЗЁТ ЧАРТ ──────────────────────────────────────────
#
# Чарт везёт `PrometheusRule` — вид ЧУЖОГО оператора. В ПУСТОМ кластере схема
# отвергает такой объект и отвергает вместе с ним ВСЮ установку, а не одну
# тревогу. Риск был назван в шапке самого шаблона заранее; условие под него
# создаётся ЗДЕСЬ — в той же роли оператора, в которой скрипт заводит секреты,
# базу и удостоверяющие центры. В настоящей установке определение вида кладёт
# оператор Prometheus, а не наш чарт.
#
# ПОЧЕМУ НЕ ИНАЧЕ — три хода отвергнуты замером, а не вкусом:
#   • проверка возможностей в шаблоне (`.Capabilities.APIVersions.Has`) даёт
#     ТИХИЙ ПРОПУСК: `helm template` без кластера отвечает `false` ВСЕГДА,
#     поэтому объект перестал бы рендериться у всех проб каталога, зовущих
#     рендер без кластера (их 10), — молчание неотличимо от исправности;
#   • отказ рендера (`fail`) по той же причине сработал бы во всех десяти;
#   • `crds/` чарта заставил бы НАШ чарт владеть определением ЧУЖОГО оператора:
#     helm их не обновляет и не удаляет, а рядом с настоящим оператором они
#     конфликтуют.
#
# Выключить ручку правил в самой джобе — тоже не ход: он снимает красноту,
# СУЖАЯ предикат полосы, чьё объявление — «судит ВСЁ».
#
# Строка перечня: <apiVersion>§<имя определения>§<адрес определения>.
# Координата чужая и потому ПИНУЕТСЯ ВЕРСИЕЙ — ровно как `PG_IMAGE` выше.
# ПОДСТАНОВКА ЗДЕСЬ `-`, А НЕ `:-`, И ЭТО НЕСУЩЕЕ: объявленный ПУСТЫМ перечень
# обязан означать «не заводится ничего». При `:-` пустое значение молча
# вернуло бы умолчание, и инъекция «владелец не заводит вид» была бы
# недейственной — то есть доказывала бы способность падать НЕ ТЕМ входом.
FOREIGN_KINDS="${KANAME_CHART_BOOTS_FOREIGN_KINDS-monitoring.coreos.com/v1§prometheusrules.monitoring.coreos.com§https://raw.githubusercontent.com/prometheus-operator/prometheus-operator/v0.79.2/example/prometheus-operator-crd/monitoring.coreos.com_prometheusrules.yaml}"
...
# foreign_kinds_malformed — перечень чужих видов РАЗБИРАЕТСЯ. Печатает первую
# негодную запись и возвращает 1; на пустом перечне и на годном — 0.
#
# Разборщик ОДИН и на подъём, и на самопроверку: второй разошёлся бы с первым
# молча — и разошёлся бы именно там, где расхождение не видно, потому что на
# годном входе оба отвечают «годен».
foreign_kinds_malformed() {
  local roster="$1" line api rest crd url
  [ -n "$roster" ] || return 0
  while IFS= read -r line; do
    [ -n "$line" ] || continue
    api="${line%%§*}"; rest="${line#*§}"
    crd="${rest%%§*}"; url="${rest#*§}"
    # Три поля: у записи без второго разделителя `crd` и `url` совпадут — то
    # есть «разобралось» ещё не значит «разобралось в три поля».
    if [ -z "$api" ] || [ -z "$crd" ] || [ -z "$url" ] || [ "$crd" = "$url" ] || [ "$api" = "$line" ]; then
      say "негодная запись перечня чужих видов: «$line»"
      return 1
    fi
  done <<<"$roster"
  return 0
...
self_test() {
  ...
  # ПЕРЕЧЕНЬ ЧУЖИХ ВИДОВ РАЗБИРАЕТСЯ, и доказывается это В ОБЕ СТОРОНЫ: годный
  # молчит, негодный краснеет. Односторонняя проба зеленела бы на разборщике,
  # принимающем что угодно, — а негодная запись стоила бы четверти часа стенда,
  # потому что отказала бы уже в кластере.
  foreign_kinds_malformed "$FOREIGN_KINDS" >/dev/null 2>&1; rc=$?
  check "перечень дерева разбирается в три поля" 0 "$rc"
  foreign_kinds_malformed "monitoring.coreos.com/v1§только-два-поля" >/dev/null 2>&1; rc=$?
  check "запись без третьего поля отвергается" 1 "$rc"
  ( "$bash_abs" "$self" --foreign-kinds >/dev/null 2>&1 ); rc=$?
  check "перечень печатается по ключу и выходит успехом" "$EXIT_OK" "$rc"
...
case "${1:-}" in
  ...
  # --foreign-kinds печатает перечень чужих видов и выходит. Отвечает ДО опроса
  # предпосылок и не требует ни кластера, ни демона, ни сети: перечень —
  # свойство ДЕРЕВА, а не стенда, и спрашивающий его гейт не вправе зависеть от
  # того, поднимается ли здесь кластер.
  #
  # Печатается ТА ЖЕ величина, по которой виды заводятся ниже, поэтому
  # объявление и исполнение разойтись не могут по построению. Гейт
  # `TestForeignKindsTheChartShipsAreEstablishedByTheBootOwner` читает этот
  # вывод, а не исходник скрипта: чтение текстом совпало бы с комментарием,
  # объясняющим этот самый механизм.
  --foreign-kinds) [ -n "$FOREIGN_KINDS" ] && printf '%s\n' "$FOREIGN_KINDS"; exit $EXIT_OK ;;
  # --render-install-chain печатает РЕНДЕР той самой цепочки и того самого набора
  # координат, которыми полоса ставит чарт, и выходит. Заведён затем, чтобы
  # спрашивающий гейт не ВЫПИСЫВАЛ цепочку у себя: выписанная пара имён профилей
  # есть вторая копия, и расходится она молча.
  #
  # Отдельно: цепочку поставки нельзя взять из таблицы стендов — та резолвит
  # профили против каталога ЗОНТА и потому описывает другой чарт; совпадение
  # написаний `values.yaml`/`values.prod.yaml` у двух чартов есть совпадение, а
  # не общий предмет. Единственный владелец цепочки поставки — этот скрипт.
  --render-install-chain) RENDER_CHAIN=1 ;;
  "") ;;
...
if [ "${RENDER_CHAIN:-0}" = "1" ]; then render_install_chain; exit $?; fi
...
# `curl` — предпосылка УСЛОВНАЯ: он нужен ровно тогда, когда перечень чужих
# видов непуст. Требовать его всегда значило бы объявить предпосылкой то, чего
# шаг не касается, — ошибка, от которой предостерегает порядок выше.
[ "$LANE" = "cluster" ] && [ -n "$FOREIGN_KINDS" ] && need="$need curl"
missing=""
...
YAML
  ...
  # ── ЧУЖИЕ ВИДЫ: их в настоящей установке заводит ОПЕРАТОР НАБЛЮДЕНИЯ ───────
  #
  # Та же роль, что у секретов и базы выше: скрипт заводит то, что в чужом
  # облаке заводит ставящий. Определение вида берётся у АПСТРИМА по пину, а не
  # пишется нами: своя урезанная копия принимала бы объекты, которые настоящая
  # схема отвергает, — фикстура, снисходительнее продукта.
  #
  # ЛЮБОЙ отказ здесь — «УСЛОВИЕ НЕ СОЗДАНО», а не находка: недоступный апстрим
  # есть утверждение о сети, а не о нашем чарте, и в зачёт «прошло» оно тоже не
  # идёт. Ровно так же классифицируется не поднявшаяся база стенда выше.
  hdr "чужие виды, которые в чужом облаке заводит оператор наблюдения"
  if [ -z "$FOREIGN_KINDS" ]; then
    say "  перечень пуст — чужих видов не заводится ни одного"
  else
    if ! foreign_kinds_malformed "$FOREIGN_KINDS"; then
      say "УСЛОВИЕ НЕ СОЗДАНО: перечень чужих видов не разбирается — заводить нечего"
      exit $EXIT_VOID
    fi
    established=0
    while IFS= read -r line; do
      [ -n "$line" ] || continue
      # Разбор параметрическим раскрытием, а НЕ `IFS='§'`: разделитель
      # многобайтовый, и разбиение по байтам порвало бы его посередине.
      fk_api="${line%%§*}"; fk_rest="${line#*§}"
      fk_crd="${fk_rest%%§*}"; fk_url="${fk_rest#*§}"
      # Срок держит САМ `curl` (`--max-time`), а не внешний `timeout`: тот был бы
      # инструментом, которого круг предпосылок выше не называет, — а круг и
      # требуемое обязаны сходиться, иначе разойдутся молча.
      if ! curl -sSLf --max-time 120 "$fk_url" -o "$WORK/foreign-$fk_crd.yaml" 2> "$WORK/foreign.err"; then
        say "УСЛОВИЕ НЕ СОЗДАНО: определение вида $fk_api не забрано с $fk_url"
        say "  Это утверждение о сети, а не о чарте: вердикта о поставке НЕТ."
        sed 's/^/    /' < "$WORK/foreign.err"
        exit $EXIT_VOID
      fi
      if ! k apply --server-side -f "$WORK/foreign-$fk_crd.yaml" > "$WORK/foreign.apply" 2>&1; then
        say "УСЛОВИЕ НЕ СОЗДАНО: определение вида $fk_api не принято кластером"
        tail -10 "$WORK/foreign.apply" | sed 's/^/    /'
        exit $EXIT_VOID
      fi
      # Ожидание ОБЯЗАТЕЛЬНО: `apply` возвращается раньше, чем вид становится
      # обслуживаемым, и установка, начатая в это окно, получила бы ровно тот
      # отказ схемы, ради которого вид и заводится.
      if ! k wait --for=condition=Established "crd/$fk_crd" --timeout=60s > "$WORK/foreign.wait" 2>&1; then
        say "УСЛОВИЕ НЕ СОЗДАНО: вид $fk_api не стал обслуживаемым за 60с"
        tail -10 "$WORK/foreign.wait" | sed 's/^/    /'
        exit $EXIT_VOID
      fi
      established=$((established + 1))
      say "  заведён $fk_api ($fk_crd)"
      say "    из $fk_url"
    done <<<"$FOREIGN_KINDS"
    say "чужих видов заведено: $established"
  fi

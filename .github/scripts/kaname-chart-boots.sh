...
self_test() {
  ...
  ( PATH="$bin" "$bash_abs" "$0" >/dev/null 2>&1 ); rc=$?
  rm -rf "$bin"
  check "инструментов нет → условие не создано, а не находка" "$EXIT_VOID" "$rc"

  ( KANAME_CHART_BOOTS_DISK_FLOOR_GB=999999999 bash "$0" >/dev/null 2>&1 ); rc=$?
  ...
  # коротко НЕ выходит, а идёт ставить. Проверяем это, ничего не поднимая.
  ( KANAME_CHART_BOOTS_DRY_RUN=1 bash "$0" >/dev/null 2>&1 ); rc=$?
  ...
  if command -v helm >/dev/null 2>&1; then
    ...
    say "  БЕЗ ПРЕДМЕТА  исполнимость входа не проверена: helm недоступен"
  fi

  say ""
  say "случаев проверено: $cases"
  if [ "$ok" -eq 0 ]; then say "PASS: самопроверка владельца подъёма поставляемого чарта"; return 0; fi
  say "FAIL: самопроверка владельца подъёма поставляемого чарта"; return 1
}

...
#
# ─── САМОПРОВЕРКА НЕ БЕРЁТ ПРЕДПОСЫЛОК ОТ ПУЛА (#2633) ──────────────────────
#
# Шаг самопроверки объявлен статическим: «на заглушках и без сети — секунды».
# Объявление было неверно для ОДНОГО утверждения из одиннадцати. Проверка отката
# («на откате СКАЗАНО, что пин не действует») доходит до ветки отката только
# тогда, когда `browser_path_image` кого-нибудь найдёт, — а он спрашивал МАШИНУ.
# Значит утверждение держалось на том, что образ ранера случайно несёт браузер.
#
# Цена названа замером, а не словом. Один изменённый факт, всё прочее то же:
#
#     PATH со всеми 1576 инструментами, БЕЗ браузера → утверждений 11, провалов 1, код 1
#     тот же PATH плюс google-chrome                 → утверждений 11, провалов 0, код 0
#
# Второе — машина разработчика, первое — наш ранер, и оба верны. Под меткой
# `ubuntu-latest` стоят два разных окружения, поэтому вердикт этого шага
# разыгрывался жребием: на PR #2626 и #2630 он покраснел ДОСЛОВНО одним текстом,
# ни разу не сказав ничего о дереве.
#
# ЧТО ИСПРАВЛЕНО: предпосылка ОБЪЯВЛЕНА величиной `KACHO_BROWSER_IMAGE_CMD` и
# СОЗДАЁТСЯ самопроверкой — ровно тем же приёмом, каким уже создавались команда
# добычи и путь пиннутого браузера. Ни одно из десяти верных утверждений не
# тронуто; обе стороны факта проверяются теперь ЯВНО:
#
#   · браузер в образе ЕСТЬ  → откат, и он говорит «пин НЕ действует»;
#   · браузера в образе НЕТ  → третья категория: код 1 и «пробы не выполнялись».
#
# Вторая половина прежде не проверялась вовсе — та самая ветка, которая и
# срабатывала на ранере.
#
# ЧТО ЭТО НЕ ЧИНИТ, и это надо сказать прямо: на ранере без браузера в образе
# НАСТОЯЩАЯ добыча по-прежнему кончается третьей категорией, если пин не
# добылся. Это правильно (вердикта о продукте нет), но условие всё ещё берётся
# от пула — предмет отдельной работы, здесь закрыт только ВЕРДИКТ САМОПРОВЕРКИ.
#
...
# Браузер ОБРАЗА — тоже подменяемая величина, и по той же причине, что две выше.
# Пустой вывод означает «в образе браузера нет»; неустановленная переменная —
# «спроси у машины» (штатный путь, поиск ниже). Зачем это здесь — §«Самопроверка
# не берёт предпосылок от пула».
image_cmd()       { printf '%s' "${KACHO_BROWSER_IMAGE_CMD-}"; }
...
browser_path_image() {
  ...
  override="$(image_cmd)"
  if [ -n "$override" ]; then
    # shellcheck disable=SC2086  # величина задана строкой и обязана разбиться на слова
    cand="$($override 2>/dev/null)"
...
self_test() {
  ...
  # ОТКАТ НА БРАУЗЕР ОБРАЗА. Предпосылка — «в образе браузер ЕСТЬ» — СОЗДАЁТСЯ
  # здесь величиной, а не берётся от пула: см. §«Самопроверка не берёт
  # предпосылок от пула».
  outtxt="$(KACHO_BROWSER_DOWNLOAD_BUDGET=2 KACHO_BROWSER_UNPACK_BUDGET=2 \
      KACHO_BROWSER_INSTALL_CMD="$tmp/stall-unpack" \
      KACHO_BROWSER_PATH_CMD="true" \
      KACHO_BROWSER_IMAGE_CMD="printf %s /bin/echo" \
      acquire "" "" 2>&1)"
  ...
  # ЗАКОННЫЙ БЛИЗНЕЦ ТОГО ЖЕ ФАКТА: браузера в образе НЕТ. Откатываться некуда,
  # и это ТРЕТЬЯ категория — «условие не создано». Два утверждения, потому что
  # проверяются две РАЗНЫЕ вещи: что работа не выходит зелёной (код) и что исход
  # назван третьей категорией, а не вердиктом о продукте (текст).
  local unmet_rc=0
  outtxt="$(KACHO_BROWSER_DOWNLOAD_BUDGET=2 KACHO_BROWSER_UNPACK_BUDGET=2 \
      KACHO_BROWSER_INSTALL_CMD="$tmp/stall-unpack" \
      KACHO_BROWSER_PATH_CMD="true" \
      KACHO_BROWSER_IMAGE_CMD="true" \
      acquire "" "" 2>&1)" || unmet_rc=$?
  if [ "$unmet_rc" = 1 ]; then ok "без браузера в образе добыча ОТКАЗЫВАЕТ (код 1), а не зеленеет"
  else bad "без браузера в образе код $unmet_rc (ждали 1)"; fi
  case "$outtxt" in *"не выполнялись вовсе"*) ok "исход назван ТРЕТЬЕЙ категорией: пробы не выполнялись" ;;
      *) bad "третья категория не названа: $outtxt" ;; esac

  rm -rf "$tmp"
  printf 'install-pinned-browser: утверждений %s, провалов %s\n' "$n" "$f"
  [ "$n" -ge 13 ] || { echo "ОТКАЗ: утверждений меньше объявленного — проба не исполнилась целиком." >&2; return 1; }

...
#
# Тем же порядком и по той же причине снят образец `tests/authz-fixtures/*_test.py`,
# пробы слоя ПОСЕВА: обе пробы этого слоя судили прогрев проброса к публичному
# слушателю прежнего издателя, а обмен `client_credentials` переехал к нашему
# издателю (`POST /iam/v1/token`) — предмета у проб не осталось, и они сняты
# вместе с ним. Образец без единой пробы дерева пережил бы свой предмет.
#
# Снятие обратимо и восстанавливается САМО: заведёт служба (или посев) свои пробы
# этого уровня — вторая половина гейта (`tools/pythonprobes`, «всякий отслеживаемый
...
# хотя сам гейт был исправен. Тем же порядком снята `_at_seed`: её образец
# (`tests/authz-fixtures/*_test.py`) снят вместе с пробами прогрева проброса к
# прежнему издателю. Площадка, которая нужна сегодня, — `_at_root` ниже; она
# привязана к образцу, который жив.
...
def self_test() -> int:
    ...
    # Прежде здесь стояли ТРИ полосы — суитная, вердиктная, посевная. Суитная снята
    # вместе со своим предметом, посевная — вместе со своим (см. `DEFAULT_PATTERNS`
    # выше); полоса осталась одна, и разводить парой файлов больше нечего. Ось
    # проверяет, что перепись единственного живого образца называет своё число, а
    # файл вне всех образцов по-прежнему не засчитывается.
    print("(h) перепись живого образца называет своё число, обход не бланкетный")
    rc, out = run({**_at_root("verdict_layer_test.py", _OK_PROBE),
                   **_at_root("verdict_layer_two_test.py", _SCRIPT_OK)})
    check("оба файла образца найдены", rc == 0, out)
    check("перепись вердиктного слоя не схлопнута",
          "по образцу tests/newman/scripts/*_test.py: 2" in out, out)

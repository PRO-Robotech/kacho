...
def execute(root: Path, pattern: str) -> int:
    ...
    if pytest_files:
        ...
        # Расхождение объявленного и исполненного — находка: часть файла молча не
        # собралась (ошибка импорта на уровне модуля читается именно так).
        if ran and declared and ran != declared:
            problems.append(
                f"объявлено {declared} проб, исполнено {ran} — часть файла не "
                f"собралась; расхождение состава и прогона молчать не должно")

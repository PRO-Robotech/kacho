...
ПРЕДМЕТ (kacho#2807)
--------------------
Правило 2026-09-22 (PRO-Robotech/kacho-workspace#770): задачи сводятся в ветку
волны, волна — в ветку эпика, эпик — запросом в ствол, и эти ветки названы
номером своей задачи. Процессы были сужены до `main` и по `push`, и по
`pull_request`, поэтому запрос в ветку-номер приходил БЕЗ ЕДИНОГО контекста: у
PR #2854 (`2833` → `2796`, голова 3f5041eb9a5) `gh run list --commit <голова>`
вернул ноль прогонов (2026-09-24). Это не выдаёт себя ничем: у такого запроса
...
----------
  ...
  2 — ИЗМЕРЕНИЕ НЕ СДЕЛАНО: нет PyYAML, объявлений не прочитано, объявление не
      разобрано или записано формой, которой провайдер не принимает (и `on:`,
      не называющий ни одного события), на запросе идёт меньше двух файлов,
      условий или звеньев прочитано ноль, объявление контекстов пусто либо ни
      один его контекст не сопоставлен заданию. Это НЕ «находок нет».
...
def _events_of(on: yaml.Node) -> dict[str, EventFilter]:
    ...
    if not out or "" in out:
        # `on:` без значения, `on: []`, `on: {}`, пустой элемент: провайдер такое
        # объявление не принимает, а разбор иначе судил бы процесс, который не
        # запускается ничем, — и молчал бы о нём так же, как о файле без запроса.
        raise Unmeasured("on-no-event", f"`on:` не называет события (строка {line}): провайдер такое объявление "
                         "не принимает, а молчание разбора было бы вердиктом о пустом")
    return out
...
def self_test(root: Path) -> int:
    ...
    for label, raw, premise, says in (
        ...
        ("on: без значения", "name: x\non:\n" + jobs_tail, "on-no-event", "не называет события"),
        ("on: пустой последовательностью", "name: x\non: []\n" + jobs_tail, "on-no-event", "не называет события"),
        ("on: пустым отображением", "name: x\non: {}\n" + jobs_tail, "on-no-event", "не называет события"),
        ("on: с пустым элементом", "name: x\non: [push, '']\n" + jobs_tail, "on-no-event", "не называет события"),

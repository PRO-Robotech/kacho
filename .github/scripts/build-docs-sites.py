...
ЧТО ЛОВИТ. Сайты документации (по одному у края и каждого сервиса; каталог сайта
опознаётся своим `docusaurus.config.ts`, а не выписанным именем) не собирал
...
def main() -> int:
    ...
    if not sites:
        print("ОТКАЗ: не найдено ни одного сайта документации — перечень выводится из дерева, "

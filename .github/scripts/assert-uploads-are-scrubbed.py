...
#
# ИМЁН ЧУЖИХ ОБЛАКОВ В НЁМ НЕТ, И ЭТО НЕ ПРОБЕЛ. Записи вида «средство выкладки
# такого-то провайдера» отсюда сняты по трём причинам сразу:
#
#   1. второй non-negotiable запрещает имя чужого облака в дереве — и запрещает
#      его СОДЕРЖИМОМУ, а не только зависимостям. Гейт tools/foreignclouds судит
#      этот файл наравне с прочими и отказал на такой записи;
#   2. предмета у записей не было: шага, зовущего такое средство, в дереве нет
#      (предикат — поиск по объявлениям процессов и по deploy);
#   3. случай, ради которого они стояли, закрыт ДРУГИМ гейтом и раньше этого:
#      шаг, написавший такое средство, назовёт провайдера, и его поймает сам
#      tools/foreignclouds — нашей записи для этого не требуется.
#
# ГРАНИЦА НАЗВАНА ЧЕСТНО: у пункта 3 есть дыра, и она не наша. Словарь того гейта
# знает имена ПРОВАЙДЕРОВ, но не имена их командных средств, а средство зовут не
# по имени владельца. Значит шаг, выносящий байты таким средством, не поймает ни
# он, ни этот перечень. Сами имена здесь не выписаны намеренно: выписать их
# значило бы завести в дереве ровно то, что второй non-negotiable запрещает, —
# и запереть будущее расширение словаря собственным текстом. Предмет чужой, он
# в tools/foreignclouds, и назван отдельно владельцу этого гейта.
PUBLISH_VERBS = (
    "gh release upload",
    "gh release create",
    "gh api",

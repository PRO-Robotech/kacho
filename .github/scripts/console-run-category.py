...
КАКОЙ КОД ВОЗВРАТА ОН ОТДАЁТ — И ПОЧЕМУ ЭТО ИЗМЕНИЛОСЬ (2026-09-12).

Прежняя редакция выходила НУЛЁМ при любой категории и называла себя описью:
«вердикт уже вынесен прогоном проб и гейтом по отчёту, а второй вердикт рядом с
первым даёт два разных ответа на один вопрос». Половина довода верна и осталась в
силе: на КРАСНОМ вердикт действительно уже вынесен, и здесь возвращается нуль.

Вторая половина была ложной, и это наблюдалось. При категории «НЕ ВЫПОЛНИЛОСЬ»
первого вердикта НЕТ ВОВСЕ: шаги, которые его выносят, пропущены, а пропущенный шаг
не падает. Значит нуль здесь не «не спорю с вердиктом», а «вердикта не будет» —
и работа выходила `success`. Прогон 34694955300: разметчик исполнился, назвал
третью категорию, вышел нулём, и консоль вышла ЗЕЛЁНОЙ при нуле исполненных проб.
Категория, напечатанная рядом с зелёным кружком, ничего не держит: в сводке
запроса на слияние виден кружок, а не журнал.

Решение владельца 2026-09-12 («сделай так, чтобы можно было гарантировать зелёный;
если что-то не так — в красный») выбрало сторону: третья категория обязана красить
работу. Поэтому:

    НЕ ВЫПОЛНИЛОСЬ → код 1 (работа красная, текст называет причину);
    КРАСНОЕ        → код 0 (вердикт вынесен шагом проб либо гейтом по отчёту;
                     второй ответ на тот же вопрос не нужен);
    ЗЕЛЕНО         → код 0.

ЧЕГО ЭТОТ СКРИПТ НЕ ДЕЛАЕТ. Он не даёт ГАРАНТИИ «объявленное исполнилось»: перечни
вердиктных и служебных шагов приходят ему аргументами, то есть выписаны рядом, и
разойтись с объявлением процесса они могут молча. Гарантию даёт отдельный владелец,
выводящий перечень ИЗ объявления — `.github/scripts/assert-declared-verdicts-ran.py`
(последний шаг работы). Здесь остаётся то, чего тот не умеет: назвать категорию
СЛОВОМ, прочитав исходы всех шагов.
...
def categorize(
    ...
    if probe_state == "failure" or verdict_broken:
        ...
        #
        # Причин у этого свидетельства ДВЕ, и различает их тот же гейт (#2780):
        # до продукта не дошла ни одна проба — либо не создано условие прогона,
        # от которого зависят пробы (приёмка F8, §4: адреса церемоний отдаёт чужой
        # экран). Какая из двух — сказано в его журнале; здесь названы обе, чтобы
        # сводка не отправляла разбирать достижимость там, где предмет — посадка.
        if verdict_unmet:
            log.append(
                "гейт по отчёту сообщил ТРЕТЬЮ категорию: вердикта о продукте нет. "
                "Причину он назвал в своём журнале — одну из двух: до продукта не "
                "дошла ни одна проба (условные шаги отработали, значит стенд перестал "
                "быть достижим браузером позже), либо не создано условие прогона, от "
                "которого зависят пробы (посадка, где адреса церемоний отдаёт не "
                "консоль). Разбирать надо названную там причину, а не консоль."
            )
            return UNMET, log, 0
...
def verdict_code(category: str) -> int:
    """Код возврата ШАГА по названной категории.

    Разделение здесь несущее, и обе половины проверяются самопроверкой:

      · «НЕ ВЫПОЛНИЛОСЬ» — вердикта НЕТ НИ У ОДНОЙ проверки работы: шаги, которые
        его выносят, пропущены, а пропущенный шаг не падает. Нуль означал бы, что
        работа выйдет зелёной без вердикта, — наблюдалось на прогоне 34694955300;
      · «КРАСНОЕ» — вердикт УЖЕ вынесен шагом проб либо гейтом по отчёту, и работа
        красная без нас. Ненулевой код добавил бы второй ответ на один вопрос;
      · «ЗЕЛЕНО» — вердикт вынесен и положителен.
    """
    return 1 if category == UNMET else 0
...
def self_test() -> int:
    ...
    # ─── КОД ВОЗВРАТА ШАГА, ОБЕ СТОРОНЫ (2026-09-12) ───────────────────────
    #
    # Наблюдалось: разметчик исполнился, назвал третью категорию и вышел нулём —
    # работа вышла ЗЕЛЁНОЙ при нуле исполненных проб (прогон 34694955300).
    # Проверяются обе половины: третья категория обязана красить, а КРАСНОЕ и
    # ЗЕЛЁНОЕ — нет. Без второй половины правка добавила бы второй вердикт рядом
    # с первым; без первой класс возвращается целиком.
    def code_case(name: str, category: str, want: int) -> None:
        got = verdict_code(category)
        cases.append((name, got == want, ("код", want), ("код", got)))

    code_case("третья категория красит работу", UNMET, 1)
    code_case("красное не задваивается вторым вердиктом", RED, 0)
    code_case("зелёное остаётся зелёным", GREEN, 0)

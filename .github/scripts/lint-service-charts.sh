...
# ПРЕДМЕТ
#
# Шаг конвейера гонял по сервисным чартам голый `helm lint`. Часть чартов
# ОСОЗНАННО отказывает в рендере, пока не названы координаты ЧУЖОГО кластера, —
# у kacho-nlb пароль базы, у registry учётные данные хранилища слоёв. Отказ
...
# единого шаблона.
#
# Такой в дереве есть, и он ЗАКОННЫЙ: у `services/geo/deploy` только Chart.yaml и
# values.yaml, ноль документов — развёртывание geo объявлено чартом зонта
# (`deploy/helm/umbrella/charts/kacho-geo`), и это записанное решение, а не
# недосмотр (docs/architecture/migrator-form.md, §о двух корнях обхода). Поэтому
# гейт на нуле документов НЕ падает: порог здесь означал бы запрет на форму,
# которую дерево выбрало осознанно.
#
# Число печатается ради другого: чтобы «ок» у такого чарта нельзя было прочесть
# как утверждение о его манифестах. Голый `helm lint` этой разницы не показывал.
sweep() {

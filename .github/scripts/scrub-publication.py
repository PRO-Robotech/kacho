...
# ШВА В РАБОЧЕМ ПУТИ НЕТ, И ЭТО СВОЙСТВО ПО ПОСТРОЕНИЮ (#1810).
#
# Здесь стояла ручка окружения, пропускавшая одну форму ПРИ ЗАПИСИ, — заведённая
# ради того, чтобы самопроверка могла доказать работу повторной сверки. Шапка
# обещала, что выставленная в работе она ничего не открывает: материал, дескать,
# останется, и сверка его найдёт.
#
# Для формы закрытого ключа обещание оказалось ЛОЖНЫМ, и это измерено:
# с выставленной ручкой настоящий RSA-2048 уезжал, вердикт был ЧИСТО, а
# собранный из «чистого» файла ключ проходил `openssl rsa -check` и побайтно
# совпадал с исходным. Механика: пропуск блока оставлял прежнюю форму-маркер,
# та стирала начальную строку, и повторная сверка искала признак, который сама
# же и стёрла.
#
# Ручка снята, а не обставлена пробой: контроль, снимаемый переменной окружения,
# контролем не является, а проба проверяет то, что ЕСТЬ, — нам же нужно, чтобы
# ветви НЕ БЫЛО. Повторная сверка доказывается теперь иначе: самопроверка делает
# ВРЕМЕННУЮ КОПИЮ этого файла с выключенной формой и прогоняет КОПИЮ. Мутация
# живёт в пробе, прод-путь её не содержит, и утверждение выше стало верным по
# построению.
...
# ФОРМЫ ОСМОТРА и ФОРМЫ ИЗЪЯТИЯ — РАЗНЫЕ СПИСКИ, и различие несущее.
#
# `pem` здесь — МАРКЕР, а не материал. Осмотру он нужен: по нему блок и виден.
# Изъятию он не годится НИКОГДА: заменив маркер, изъятие стирает единственный
# признак и отдаёт тело — ровно то, чем эта чистка однажды превратила
# обнаружимую утечку в необнаружимую. Материал изымается блоком (RE_PEM_BLOCK),
# а незакрытый блок отказывается судиться (RE_PEM_OPEN).
#
# Следствие, ради которого списки и разведены: если изъятие блока когда-нибудь
# перестанет исполняться, маркер УЦЕЛЕЕТ, и повторная сверка назовёт находку.
# Прежде она искала признак, который сама же стёрла.
TEXT_FORMS = (
    ("header", RE_HEADER),
    ("jwt", RE_JWT),
    ("pem", RE_PEM),
...
REDACT_FORMS = tuple((n, rx) for n, rx in TEXT_FORMS if n != "pem")
...
def self_test() -> int:
    ...
    def mutant(marker: str, replacement: str) -> str:
        ...
        Мутация живёт ЗДЕСЬ, а не в прод-пути. Прежде её роль исполняла ручка
        окружения внутри самого скрипта, и ручка оказалась дырой: выставленная
        на форму ключа, она отдавала настоящий ключ при вердикте «ЧИСТО».
        Контроль, снимаемый переменной окружения, контролем не является —
        поэтому ветви в рабочем пути больше нет, а доказательство повторной
        сверки делается подменой ПРОГРАММЫ, а не её поведения изнутри.
        """
        src = Path(me).read_text(encoding="utf-8")
        # ЯКОРЬ ОБЯЗАН БЫТЬ ЕДИНСТВЕННЫМ. Первая редакция этой пробы мутировала
        # строку, которая встречается дважды — в ОСМОТРЕ и в ИЗЪЯТИИ, — и правка
        # доставалась осмотру: проба тогда доказывала не то, что заявляла.
        # Неоднозначный якорь теперь громкий отказ, а не тихая подмена предмета.
        n = src.count(marker)
        if n != 1:
    ...
    # (3в) ПОВТОРНАЯ СВЕРКА ДЕРЖИТСЯ ЭТИМИ СЛУЧАЯМИ, А НЕ ВНИМАНИЕМ.
    #
    # Проверяется так: берётся КОПИЯ этого файла с выключенной формой изъятия и
    # прогоняется она. Осмотр при этом не трогается, значит найти оставшийся
    # материал может ТОЛЬКО чтение файла после записи. Снимут повторную сверку —
    # эти случаи станут «ЧИСТО».
    #
    # Форм две, и вторая появилась после того, как первой оказалось мало: она
    # закрывала лишь тот случай, где остаток сам себя опознаёт. У закрытого
    # ключа остаток опознать было нечем — маркер стирала другая форма.
    for label, marker, replacement, payload in (
        ...
        # ЯКОРЬ СОБИРАЕТСЯ ИЗ ЧАСТЕЙ, а не пишется литералом: литерал встретился
        # бы в этом файле ВТОРЫМ вхождением — самим собой, — и проверка
        # единственности честно отказала бы. Тот же приём, что у фикстур
        # обходчика самопроверок.
        ("блок закрытого ключа",
         "    data = RE_PEM" + "_BLOCK.sub(_pem_block, data)",
         "    data = data  # мутация самопроверки",
         {"key.log": "-----BEGIN RSA PRIVATE KEY-----\n" + "\n".join(PEM_BODY)
                     + "\n-----END RSA PRIVATE KEY-----\n"}),
    ):
        cases += 1

...
// NewAuthorizeClient оборачивает соединение с kacho-iam в порт сужателя. conn
// указывает на kacho-iam, где живёт `AuthorizeService` (`BatchCheck` — публичный
...
// BatchCheck пробрасывает запрос в kacho-iam.
//
// Исходящий контекст обёрнут auth.PropagateOutgoing, чтобы извлекатель личности на
// стороне kacho-iam увидел РЕАЛЬНОГО вызывающего, а не запасное значение уровня
// начальной загрузки. Без обёртки внутренний страж kacho-iam видит служебную

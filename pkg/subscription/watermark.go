...
//
// Соединение и пул отвечают ему ОБА, и это не удобство, а условие переиспользуемости:
// у механизма подписки наблюдение живёт на выделенном соединении потока, а у
// возобновимого чтения, отвечающего на запрос (`InternalIAMService.PollSubjectChanges`),
// соединения своего нет вовсе — есть пул.
//
// Исключение СЕБЯ по идентификатору обслуживающего процесса остаётся верным и на
// пуле: наблюдатель в журнал не пишет, поэтому его собственная блокировка — не та,
// которую наблюдение ищет.
...
//
// Тип ЗАЩИЩЁН ОТ СОВМЕСТНОГО ДОСТУПА: у механизма подписки наблюдатель свой на
// каждый поток и состязания нет, а у чтения, отвечающего на запрос, экземпляр
// один на процесс и проходы идут параллельно. Замок неоспариваемый в первом
// случае и несущий во втором.
...
// newWatermark — внутренняя сборка с управляемыми часами: [stallWarnAfter]
...
// молча и навсегда.
//
// Ноль означает «ещё не устоялось»: у холодного наблюдателя первое наблюдение
// лишь ЗАПОМИНАЕТ писателей, а подтверждает их следующее. Читать ноль как
// «журнал пуст» нельзя — это состояние наблюдения, а не факт о журнале.

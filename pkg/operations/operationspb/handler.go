...
// notFound — производитель отказа «нет такой операции» для ВСЕХ семи доменов.
//
// Текст — часть контракта (`api-conventions.md`), и он же несёт СОКРЫТИЕ:
// «чужая» и «несуществующая» обязаны быть неотличимы побайтово, иначе по
// различию текста отличают «нет доступа» от «не существует» — то есть ровно то,
// что сокрытие и должно было закрыть (`security.md` §Hardening #6).
//
// ЕДИНСТВЕННЫМ В ДЕРЕВЕ ОН НЕ ЯВЛЯЕТСЯ, и писать так нельзя: край
// (`gateway/internal/opsproxy`) производит СВОЙ текст той же формы, но с
// прописной буквы. Предикат: `grep -rn 'peration %s not found' --include=*.go`
// даёт двух производителей. Две формы, различимые регистром, на одной полосе
// доступны клиенту — то есть свойство, которое этот комментарий прежде объявлял
// неотзываемым, уже отозвано. Предмет заведён задачей продукта #1370; здесь
// сказано, чтобы следующий читатель не принял желаемое за состояние дерева.

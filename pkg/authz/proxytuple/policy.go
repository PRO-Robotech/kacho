...
// Переход на словарь типов держит задача #1816: читатели литерала каталога
// переводятся на таблицу, и до неё словаря просто нет. Эпик #1087 — её
// основание, а не держатель: работы эпик не производит.
//
// ПЕРЕЕЗД ПАКЕТА ЭТОГО НЕ РЕШАЕТ — измерено, а не предположено. Пакет импортируют
...
// `services/iam/`. Выразим ПОРТ — владельца типа подаёт вызывающий, — и вызывающий
// у `ValidateTuple` в прод-коде ровно ОДИН, и он в iam
// (`services/iam/internal/apps/kacho/api/internal_iam/handler.go`, предикат:
// `git grep -n 'ValidateTuple(' -- '*.go' ':!*_test.go'`).
//
// ПОЧЕМУ ПОРТ ВСЁ РАВНО НЕ ЗАВОДИТСЯ СЕГОДНЯ — вопрос ПОРЯДКА, а не возможности.
// Поиск владельца по строке означает: тип, которого закрытая таблица не знает,
// ОТВЕРГАЕТСЯ. Сегодня такой тип принимается (зеркало держит непрозрачный тип как
// есть — `register_resource.go`, ветвь «unknown types are kept verbatim»), а сама
// таблица РУКОПИСНА: её вывод из манифеста замкнул бы круг с загрузчиком, и это
// измерено отдельным предметом (#1930, он же — вторая таблица #1092). Сделать
// рукописную таблицу несущей на пути запроса значит потребовать правки Go в iam
// прежде, чем модуль сможет зарегистрировать свой новый ресурс, — и отказ будет
// ОПАКОВЫМ by design (ниже: «без утечки того, какая оговорка отвергла»), то есть
// с худшей возможной диагностикой. Порядок обратный: сперва таблица перестаёт быть
// рукописной (#1930), затем она становится авторитетом пути запроса (#1885).

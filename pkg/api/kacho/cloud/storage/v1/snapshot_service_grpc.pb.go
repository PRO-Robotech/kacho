...
type SnapshotServiceClient interface {
	...
	//
	// Гейт спрашивает право на ИСТОЧНИК: цель резолвится по `snapshot_id`, а
	// отношение — v_get, потому что копирование исходный снимок читает и не меняет.
	// Второй цели у проверки не возникает by construction: целевого проекта у
	// глагола нет — копия остаётся в проекте источника, меняется только зона.

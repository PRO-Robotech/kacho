...
type ServiceAccountServiceClient interface {
	...
	//
	// WHY AN ACTION AND NOT A FIELD ON Update. An empty `update_mask` means
	// full-object replacement by platform convention, and a proto3 `bool` cannot
	// say "not sent" — so a client that simply did not fill the field in would
	// disable the account, silently and in bulk. The state that decides whether a
	// machine identity still works must not be reachable by forgetting something.
	...
	// token). Leaving it at the AAL1 floor would be the cheap door to the same
	// outcome.
	//

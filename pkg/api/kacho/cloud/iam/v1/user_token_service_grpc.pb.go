...
// единственный способ им обладать — БЫТЬ этим человеком (плюс плоский надзор
// администратора облака, общий для всякого отношения модели). Так и должно быть:
// персональный токен делает предъявителя самим человеком ВЕЗДЕ, где действует он,
// включая аккаунты, к которым выдавший отношения не имеет — личность в этом
// дереве глобальна, одна строка на все аккаунты человека.
//
...
// отношение на оба означало бы, что второе следует из первого; отношения уровня
// аккаунта у `token_issuer` нет намеренно (#1086).
//
...
// её имени» границы аккаунта не покидает.
//

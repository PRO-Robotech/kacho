...
func (w *Watcher) Poll(ctx context.Context) {
	...
	for pass := 0; pass < maxPassesPerTick; pass++ {
		...
		// является — и это ЕДИНСТВЕННОЕ место, где курсор поднимается до головы.
		if !w.primed {
			w.primed = true
			w.cursor = headID
			return
		...
		// Сброс СПЛОШНОЙ, и это решение (kacho#1395), а не пережиток.
		//
		// Порция несёт имена, и потоки закрываются поимённо — отсюда соблазн
		// гасить кэш так же. Он не годится для строк, чей субъект назвать НЕЛЬЗЯ:
		// право, выданное ГРУППЕ, снимается строкой с субъектом `group:<id>`,
		// словарь имён закрыт (`authz.TenantSubject`), а погаснуть обязаны
		// вердикты ЧЛЕНОВ — их имена в строке не названы и из неё не выводятся.
		// Поимённый сброс гасил бы там ноль записей.
		//
		// Цена сплошного мала и измерена: запись живёт 5 с (умолчание TTL, ни
		// один профиль не переопределяет), перепрос идёт раз в 2 с и гасит только
		// на непустой порции. То есть сужение сохранило бы самое большее 3 с из
		// 5 — и ровно в те окна, когда права меняются, а устаревшее «разрешено»
		// и опасно. Радиус закреплён `flush_radius_test.go`.
		w.cfg.Flush()

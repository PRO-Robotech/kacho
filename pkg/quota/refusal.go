...
var refusalOwners = []RefusalOwner{
	// ЗДЕСЬ СТОЯЛА СТРОКА СЛУЖБЫ ДОСТУПА — владельца ВЕЛИЧИН и владельца типа
	// «аккаунт», единственного, у кого совпало и то и другое. Она снята вместе
	// со своим предметом: служба вынесена отдельным продуктом, её прод-код и её
	// миграции живут в другом дереве.
	//
	// Оба вызывающих этого перечня работают по ЭТОМУ дереву — генератор рендерит
	// файл миграции в него, гейт разбирает в нём прод-код, — поэтому владелец,
	// чей код лежит в чужом репозитории, ни одним из них не обслуживается: у
	// генератора для него нечего рендерить, у гейта нечего разбирать, и его
	// «ноль находок» означало бы «ноль прочитанного».
	//
	// ОСТАТОК, НАЗВАННЫЙ ВСЛУХ: что служба доступа производит отказ учёта
	// наружу — кодом И признаком полосы — в этом дереве не проверяет теперь
	// НИКТО. Свойство не отменено, у него сменился судья: проверять его обязано
	// дерево службы, где лежат обе стороны. Вернётся служба сюда — вернётся и
	// строка, а вместе с ней проверка.

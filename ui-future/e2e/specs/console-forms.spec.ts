...
/**
 * dataRows — строки ТАБЛИЦЫ, несущие данные, а не устройство таблицы.
 *
 * Признак живёт НЕ здесь: он один на две стороны — на эту пробу и на гейт,
 * который доказывает, что признак вправду отделяет данные от устройства и что
 * установленная таблица помечает свои служебные строки именно так
 * (`shared/src/test/console-probe-data-rows.test.ts`). Разбор класса, цена и
 * причина отдельного модуля — в шапке самого источника.
 *
 * Копии предиката здесь нет намеренно: пока признак жил в этом файле, гейт брал
 * его чтением исходника, то есть сверялся с текстом там, где заводился ради
 * сверки с существом. Две копии к тому же разошлись бы молча — обе отвечают
 * одинаково на исправном дереве.
 */
function dataRows(page: Page): Locator {
  return page.locator(dataRowSelector(".app-main"));
}
...
test("в строках списка нет флажков и группового удаления", async ({ page }) => {
  ...
  for (const url of ["/iam/roles", "/system/zones", `/projects/${projectId}/vpc/networks`]) {
    ...
    // ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ: у таблицы есть строки ДАННЫХ — см. `dataRows`.
    // Ждём именно их появления, а не «хоть чего-нибудь в теле таблицы»: все
    // четыре утверждения ниже — отрицания, и на пустой таблице каждое верно
    // by construction. Контроль, который пустая таблица выполняет, обращает их
    // все разом в форму без содержания.
    await expect
      .poll(async () => await dataRows(page).count(), {
        message:
          `${url}: за отведённое время таблица не показала НИ ОДНОЙ строки данных (в теле ` +
          `только служебные строки таблицы). Утверждать об устройстве её строк не о чем`,
        timeout: 45_000,
      })
      .toBeGreaterThan(0);

    const rowCount = await dataRows(page).count();
    const bodyRows = await page.locator(".app-main tbody tr").count();
    ...
    expect(
      menuCount,
      `${url}: меню действий есть не у каждой строки — строк данных ${rowCount}, меню ${menuCount} ` +
        `(всего строк в теле таблицы ${bodyRows}, включая служебные). Групповых действий нет, ` +
        `значит строка без своего меню — строка, которую удалить нечем`,
    ).toBe(rowCount);
    ...
    console.log(
      `${url}: строк данных ${rowCount} (в теле таблицы ${bodyRows}, служебных ` +
        `${bodyRows - rowCount}), меню действий ${menuCount}, флажков 0`,
    );
  }
});

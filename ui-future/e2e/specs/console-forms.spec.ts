 ...
 *
 * Тело общей таблицы всегда несёт СЛУЖЕБНЫЕ `<tr>`, которых пользователь не
 * видит и в которых данных нет:
 *
 *   • строка обмера (`ant-table-measure-row`, `aria-hidden="true"`, высота 0) —
 *     рисуется ВСЕГДА, пока таблица меряет ширину столбцов;
 *   • строка-заполнитель (`ant-table-placeholder`) — рисуется, когда набор ПУСТ,
 *     и несёт текст пустого состояния.
 *
 * Обе живут в том же `<tbody>`, поэтому счёт «строк больше нуля» выполняется
 * ПУСТОЙ таблицей: у неё их ровно две. Положительный контроль, построенный на
 * таком счёте, ничего не сторожит — и утверждения после него говорят о пустой
 * таблице, где «меню действий нет» и «флажков нет» верны by construction.
 *
 * Цена измерена, а не предположена (прогон 2026-08-23, #1070): контроль
 * `tbody tr > 0` был выполнен ещё не дозагрузившимся списком зон, и проба
 * объявила отсутствующим меню, которое в тот же миг стояло в снимке страницы —
 * `button "Действия"` у КАЖДОЙ строки. На соседней ветке того же часа та же
 * проба напечатала «строк 7, меню действий 6»: шесть зон плюс одна строка
 * обмера. Разница между «строк 2, меню 0» и «строк 7, меню 6» — не свойство
 * продукта, а момент замера.
 *
 * Признак ИСКЛЮЧАЕТСЯ по устройству строки, а не по её содержимому: у
 * заполнителя текст ЕСТЬ (пустое состояние), поэтому «строка с текстом» его не
 * отсекает. И он не совпадает с тем, что проба утверждает: наличие меню в
 * определение строки данных не входит — иначе утверждение стало бы тождеством.
 *
 * Предпосылка этого определения — РАЗМЕТКА установленной таблицы, а она может
 * смениться вместе с её версией; тогда исключение перестанет срабатывать молча,
 * и контроль вернётся к прежней пустоте. Поэтому предпосылку сторожит отдельный
 * гейт, читающий И установленную библиотеку, И это объявление:
 * `shared/src/test/console-probe-data-rows.test.ts`. Селектор объявлен
 * ИМЕНОВАННОЙ строкой именно затем, чтобы гейт брал его отсюда, а не заводил
 * свою копию, которая разошлась бы с этой незаметно.
 */
const DATA_ROW = '.app-main tbody tr:not([aria-hidden="true"]):not(.ant-table-placeholder)';

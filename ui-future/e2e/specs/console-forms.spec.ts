...
import {
  ...
  dataRowSelector,
  type BodyCensus,
} from "../../shared/src/test/console-table-rows";
...
/** Область консоли, внутри которой живут списки. */
const SCOPE = ".app-main";

/**
 * Перепись тела таблицы — ВСЕ структурные числа сняты ОДНИМ обходом DOM.
 *
 * ─── ПОЧЕМУ ОДИН ОБХОД, А НЕ ЧЕТЫРЕ ЧТЕНИЯ ПОДРЯД (#2237) ──────────────────
 *
 * Прежняя редакция снимала строки данных, строки тела и меню ТРЕМЯ отдельными
 * обращениями к странице, а положительный контроль — четвёртым, ещё раньше.
 * Числа сравнивались между собой, хотя относились к РАЗНЫМ моментам одной
 * страницы, и на прогоне 34142361500 это дало отказ, противоречащий сам себе:
 * «строк данных 0, меню 48 (всего строк в теле таблицы 49)». Снимок DOM того же
 * отказа показывал 48 строк без единого служебного признака — продукт был
 * исправен, а между контролем и утверждением список успел уйти в состояние
 * загрузки (`/iam/roles` открывается дважды: без области, затем с нею; смена
 * ключа запроса роняет список в пустоту — это отдельная находка, #2240).
 *
 * ─── ПОЧЕМУ САМА ПЕРЕПИСЬ ЖИВЁТ НЕ ЗДЕСЬ ───────────────────────────────────
 *
 * Ни признак строки данных, ни перепись копий тут не имеют: и то и другое —
 * общий источник (`shared/src/test/console-table-rows.ts`), который импортируют
 * ОБЕ стороны — эта проба и гейт, доказывающий несущее свойство на
 * синтетическом теле (`shared/src/test/console-probe-data-rows.test.ts`).
 * Пока признак жил в этом файле, гейт брал его чтением исходника, то есть
 * сверялся с ТЕКСТОМ там, где заводился ради сверки с СУЩЕСТВОМ; две копии к
 * тому же разошлись бы молча — обе отвечают одинаково на исправном дереве.
 *
 * `page.evaluate` переносит в страницу ТЕКСТ функции, а не замыкание, поэтому
 * оба селектора уходят аргументом, и исполняется она В СТРАНИЦЕ синхронно:
 * перерисоваться между её строками нечему by construction.
 *
 * Даже если признак дозагруженности когда-нибудь перестанет проставляться,
 * несущее свойство не теряется: его держит сверка двух переписей вокруг чтений
 * (см. `listCensus`), а `settled` лишь сокращает ожидание и называет состояние
 * словом.
 */
async function bodyCensus(page: Page): Promise<BodyCensus> {
  return await page.evaluate(censusOfBody, {
    scope: SCOPE,
    dataSelector: dataRowSelector(SCOPE),
  });
}

/**
 * Полная перепись списка: структурные числа плюс два отрицания, которые
 * выражаются только локаторами Playwright (роль и текст), — и ДОКАЗАТЕЛЬСТВО
 * того, что всё это снято с одного состояния страницы.
 *
 * Роль и текст в один обход DOM не переносятся: их семантику знает движок
 * локаторов, а не CSS, и переписывание её вручную сузило бы отрицание — то есть
 * ослабило бы ровно то утверждение, ради которого проба стоит. Поэтому они
 * читаются как прежде, но ЗАЖАТЫ между двумя переписями: разошлись переписи —
 * страница двигалась во время чтения, и снимок недействителен целиком.
 */
type ListCensus = BodyCensus & {
  bulkActions: number;
  selectedCounters: number;
  /** обе переписи вокруг чтений сошлись и обе застали таблицу дозагруженной */
  consistent: boolean;
};

async function listCensus(page: Page): Promise<ListCensus> {
  const before = await bodyCensus(page);
  const bulkActions = await page
    .locator(SCOPE)
    .getByRole("button")
    .filter({ hasText: /выделенн|выбранн/i })
    .count();
  const selectedCounters = await page.locator(SCOPE).getByText(/Выделено\s*:/i).count();
  const after = await bodyCensus(page);
  ...
  return {
    ...
    consistent:
      before.settled &&
      after.settled &&
      before.dataRows === after.dataRows &&
      before.bodyRows === after.bodyRows &&
      before.menus === after.menus &&
      before.checkboxes === after.checkboxes,
  };
}
...
test("список, который обновляется сам, не предлагает «Обновить»", async ({ page }) => {
  // verifies #925
  test.setTimeout(300_000);
  const { projectId } = await tenantWithProject(page);
  ...
  for (const rec of urls) {
    ...
    // держит себя свежим САМ. Механизмов ДВА, и предпосылка обязана знать оба:
    //
    //   • ОПРОС — второй запрос к краю, которого никто руками не просил;
    //   • ПОТОК изменений — список прочитан, и открыт поток, который принесёт
    //     следующее чтение вместо опроса. Опрос при этом гасится РОВНО признаком
    //     покрытия (`use-resource-list`: `refetchInterval: streamed ? false :
    //     3_000`), поэтому у покрытого списка второго запроса не бывает ПО
    //     ЗАМЫСЛУ, а не по неисправности.
    //
    // Перечня покрытых списков здесь НЕТ намеренно: он принадлежит продукту
    // (`STREAM_SUBJECTS`) и словарю владельца, приезжающему по проводу, — а
    // вторая его копия в пробе разошлась бы с первой молча в тот день, когда
    // владелец заведёт вид или снимет его. Дизъюнкция спрашивает У СТРАНИЦЫ: не
    // покрыт — опрос продолжается и срабатывает первая ветвь; покрыт — вторая.
    // Ни одной ⇒ список ЗАМЕР, и это находка, ради которой контроль и стоит.
    //
    ...
    await expect
      .poll(
        () => {
          const reads = requests.filter((u) => u === rec.edge).length;
          const streams = requests.filter((u) => u === STREAM_PATH).length;
          return { reads, streams, selfUpdating: reads >= 2 || (reads >= 1 && streams >= 1) };
        },
        {
          message:
            `${rec.url}: список не держит себя свежим НИ ОДНИМ из двух способов — край ${rec.edge} ` +
            `второй раз не запрошен, и поток ${STREAM_PATH} не открыт. Значит список ЗАМЕР: правило ` +
            `«кнопки „Обновить“ быть не должно» к нему не относится, и проба обязана это сказать, ` +
            `а не молча зеленеть`,
          ...
          // по-прежнему второй запрос ЛИБО открытый поток.
          timeout: 60_000,
        },
      )
      .toMatchObject({ selfUpdating: true });
    ...
    expect(
      howMany,
      `${rec.url}: список держит себя свежим сам (${rec.edge} прочитан ${reads}×, поток открыт ` +
        `${streams}×), и при этом предлагает обновить его руками (${JSON.stringify(labels)}). ` +
        `Ручка предлагает то, что и так происходит, а её присутствие говорит обратное`,
    ).toBe(0);

    // Перепись называет ОБА механизма: «прочитан 1×, поток 1×» и «прочитан 2×,
    // поток 0×» — разные исправные состояния, и по одному числу они неразличимы.
    console.log(
      `${rec.url}: ${rec.edge} прочитан ${reads}×, поток ${streams}×, ручек обновления ${howMany}`,
    );
  }
...
test("в строках списка нет флажков и группового удаления", async ({ page }) => {
  ...
  for (const url of ["/iam/roles", "/system/zones", `/projects/${projectId}/vpc/networks`]) {
    ...
    // ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ И УТВЕРЖДЕНИЕ ЧИТАЮТ ОДИН СНИМОК (#2237).
    //
    // Контроль здесь двойной, и обе его половины несущие:
    //
    //   • «строк ДАННЫХ больше нуля» — все четыре утверждения ниже отрицания, и
    //     на пустой таблице каждое верно by construction; контроль, который
    //     пустая таблица выполняет, обращает их все разом в форму без
    //     содержания. Служебные строки таблица рисует всегда, поэтому «хоть
    //     что-нибудь в теле» контролем не является;
    //   • «снимок непротиворечив» — таблица дозагружена, и обе переписи вокруг
    //     чтений сошлись. Без этой половины контроль относился бы к ДРУГОМУ
    //     моменту, чем утверждение: ровно так прогон 34142361500 получил
    //     «строк данных 0, меню 48» при исправном продукте.
    //
    // Опрашивается ТОЛЬКО контроль. Утверждения ниже читают ту самую перепись,
    // которая его прошла, и опросом не прикрыты — иначе они бы не падали
    // никогда, а ждали, пока страница сама себя оправдает.
    let census!: ListCensus;
    await expect
      .poll(
        async () => {
          census = await listCensus(page);
          return { consistent: census.consistent, hasDataRows: census.dataRows > 0 };
        },
        {
          message:
            `${url}: за отведённое время список не дал ПРИГОДНОГО снимка. Либо таблица не ` +
            `показала ни одной строки данных (в теле только служебные строки таблицы) — тогда ` +
            `об устройстве её строк утверждать не о чем; либо она всё это время перерисовывалась, ` +
            `и числа снимка относились бы к разным её состояниям. Что именно — видно в полученном ` +
            `значении ниже`,
          timeout: 45_000,
        },
      )
      .toEqual({ consistent: true, hasDataRows: true });

    // Утверждается ПРАВИЛО: меню действий есть у КАЖДОЙ строки, а не «хотя бы у
    // одной». Групповых действий нет, поэтому строка без своего меню — строка,
    // которую нечем удалить; «хотя бы одно меню» этого не различает и зеленеет
    // на списке, где путь к удалению потеряли все строки, кроме первой.
    expect(
      census.menus,
      `${url}: меню действий есть не у каждой строки — строк данных ${census.dataRows}, меню ` +
        `${census.menus} (всего строк в теле таблицы ${census.bodyRows}, включая служебные; ` +
        `числа сняты одним обходом DOM). Групповых действий нет, значит строка без своего меню — ` +
        `строка, которую удалить нечем`,
    ).toBe(census.dataRows);
    ...
    expect(
      census.selectedCounters,
      `${url}: на странице есть счётчик выделенных строк`,
    ).toBe(0);

    console.log(
      `${url}: строк данных ${census.dataRows} (в теле таблицы ${census.bodyRows}, служебных ` +
        `${census.bodyRows - census.dataRows}), меню действий ${census.menus}, флажков 0 ` +
        `— снимок один`,
    );
  }
});

 ...
 * ЗАЧЕМ
 *
 * Вход, регистрация и выход отвечают, и экран сразу уходит документом
 * (`window.location.replace`); окно повышения и экран параметров документом не
 * уходят, но их ответы снимаются тем же путём — чтение ответа у набора одно
 * (`LANE_VERBS`). Браузер держит тело ответа, пока жив

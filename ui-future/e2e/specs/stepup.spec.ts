...
test("iam: глагол с полом «2» отвергается на первом уровне и проходит после поднятия", async ({ page }) => {
  ...
  // Уровень читается у НАШЕЙ сессии ответом края о ней, а не у чужого поставщика.
  expect(await assuranceLevel(page), "свежий арендатор обязан быть на первом уровне").toBe("1");

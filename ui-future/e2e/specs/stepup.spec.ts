...
import { assuranceLevel, raiseAssuranceToAal2, register, runTag, test } from "./fixtures";
...
test("iam: глагол с полом «2» отвергается на первом уровне и проходит после поднятия", async ({ page }) => {
  ...
  //
  // Механизм живёт в общем модуле (`fixtures.ts`) с тех пор, как подъём уровня
  // перестал быть предметом одной пробы и стал условием другой. Утверждения
  // потока переехали вместе с ним и не ослаблены: заведение второго фактора,
  // подтверждение им и сам факт смены уровня по-прежнему проверяются — просто
  // на один вызов ниже.

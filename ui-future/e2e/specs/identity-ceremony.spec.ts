 ...
 * задачу — внутри вызова `test(…)`, и у каждого сценария она своя — задача,
 * которая его завела: S1 — `#2780`, S2 — `#1274`, дописанные к S1 позже —
 * свои (`#1274`, `#2857`).
 *
 * ─────────────────────────────────────────────────────────────────────────────
...
// Ответы глаголов полосы снимаются ДО страницы: за ответами входа, регистрации и
// выхода экран уходит документом, и тело после ухода не читается
// (`answer-on-arrival.ts`).
...
test("F8-18 · после выхода следующий человек в этом браузере не видит чужих аккаунта и проекта", async ({
  ...
  await page.getByRole("button", { name: "Учётная запись" }).click();
  // Нажатие ограничено сроком: закрой «Выйти» на панели проекта страница (так
  // было, пока панель рисовалась внутри рейла, — `AccountPanel.tsx`), отказ
  // обязан назвать перехватчика нажатия, а не «ответа выхода не было» по сроку
  // всего сценария.
 ...
 * (`page.route`) значило бы строить другое условие: первый глагол до службы не
 * дошёл, сессия у неё жива, и выход совершил бы второй; а подтверждения первого
 * (`first()` — код ответа службы, условие сценария) на стадии запроса нет вовсе.
 *
 * Ответ второму выходу от стадии не зависит: выход у службы идемпотентен, и
 * второй выход — с носителем погашенной сессии или без носителя — получает тот же
 * `200` (Ф3-18). Так и наблюдалось: на откате правки оба `POST` выхода — `200`
 * (`F8-18 · панель закрыта и открыта заново…` ниже).
 *
...
test("F8-18 · панель закрыта и открыта заново в полёте выхода: второе «Выйти» не начинает второго выхода", async ({
  ...
  // закрывается и в полёте выхода, а открытая заново несла СВОЙ выход: второе
  // «Выйти» уходило на службу рядом с подтверждённым первым.
  //
  // Отказа второму служба не даёт: выход идемпотентен (Ф3-18), и на стенде на
  // откате правки ff295b19979 оба `POST` выхода получили `200`, а страница пришла
  // на `/login` без адреса возврата — адрес верен и на дефекте. Вред был бы у
  // ОТКАЗА второму: до правки отказ любого выхода снимал метку вкладки
  // безусловно, и метку подтверждённого первого тоже, — следующий `401` чтения
  // прежней страницы увёл бы тогда на вход с её адресом возврата (исход F8-18).
  // Такого отказа проба не строит: отказ выхода — предмет F8-19.
  //
  // Поэтому утверждается ЧИСЛО выходов — на стенде дефект отличает оно, а не
  // адрес; адрес утверждается как исход у человека. Чем погашено второе нажатие,
  // проба не различает: на исправленной консоли кнопка открытой заново панели в
  // ожидании (занятость — из метки вкладки) и нажатия не принимает, поэтому отказ
  // `beginTabExit` второму выходу держат модульные пробы (`tab-exit.test.ts`,
  // `auth.test.ts` каркаса и панели), а не эта.

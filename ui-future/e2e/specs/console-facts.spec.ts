...
test("тон факта следует смыслу, а не истинности", async ({ page }) => {
  ...
  expect(
    зона,
    "справочник зон ПУСТ: публичный адрес берётся из полосы адресов зоны, и без неё предмет " +
      "пробы не создать. Это УСЛОВИЕ пробы, а не её предмет",
  ).not.toBe("");

  const адрес = await page.request.post("/vpc/v1/addresses", {
    data: { projectId, name: `adr-tone-${runTag()}`, externalIpv4AddressSpec: { zoneId: зона } },
  });

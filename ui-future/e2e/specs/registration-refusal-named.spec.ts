...
test.describe("отказ регистрации назван своим текстом", () => {
  ...
  test("F8-44 · фикстура сообщает отказ НА ШАГЕ ЭКРАНА, а не отсутствие формы", async ({ page }) => {
    ...
    // verifies #2780
    //
    // ВТОРАЯ ПОЛОСА ТОГО ЖЕ МЕХАНИЗМА: у фикстуры ТРИ ожидания — форма
    // регистрации на экране, печенье сессии после отправки и уход документа на
    // успешном ответе, — и отказ представим на двух первых: третье ждётся только
    // на успехе.
    ...
    expect(failure, "и назвать, чей это отказ").toContain("ОТВЕРГНУТА службой");
    expect(failure, "и назвать, ГДЕ он случился: у ожиданий с отказом разные предметы").toContain(

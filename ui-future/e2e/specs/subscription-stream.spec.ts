...
async function installStreamReader(page: Page): Promise<void> {
  await page.addInitScript(() => {
     ...
     * допущение ЛОЖНО. Создание сети пишет в журнал ТРИ строки, потому что
     * умолчания достраиваются после самой сети:
     *
     *   `CREATED`  — сеть создана (`network/create.go`);
     *   `UPDATED`  — привязана группа безопасности по умолчанию (`network/default_sg.go`);
     *   `UPDATED`  — привязана таблица маршрутов по умолчанию (`network/default_rt.go`).
     *
     * Поэтому «три кадра» выбирались служебным открытием и ПЕРВЫМИ ДВУМЯ
     * событиями ПЕРВОЙ сети — а до второй чтение не доходило вовсе, и проба
     * объявляла потерянным предмет, которого она не дочитала.
     *

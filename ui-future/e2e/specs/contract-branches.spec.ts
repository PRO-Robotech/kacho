 ...
 *
 * ДВА ПОМОЩНИКА, А НЕ ОДИН — И ЭТО СВОЙСТВО КОНТРОЛА, А НЕ ВКУС.
 * Сегментированный переключатель рисует у каждого варианта СКРЫТЫЙ управляющий
 * элемент (`radio`, вне потока отрисовки) внутри ВИДИМОЙ обёртки, несущей текст.
 * Доступное имя есть только у скрытого; видимая обёртка своей роли не имеет.
 *
 * Следствие, стоившее прогона (2026-08-17, локальная отладка на живом стенде):
 * `getByRole("radio")` находит элемент, который существует и **невидим**, и
 * `toBeVisible` падает с `unexpected value "hidden"` — сообщение читается как
 * «варианта нет», хотя вариант есть и работает. Ложный вывод здесь дороже
 * самого падения: он уводит искать дефект в продукт.
 *
 * Поэтому:
 *   `режим`       — ВИДИМАЯ обёртка: ею проверяют присутствие и по ней щёлкают;
 *   `режимВыбран` — управляющий элемент: им проверяют СОСТОЯНИЕ (`toBeChecked`).
 *
 * Модульная проба того же контрола (`NlbVipSourceField.render.test.tsx`)
 * спрашивает `queryAllByRole("radio")` и права: она судит о СОСТАВЕ вариантов,
 * а не о том, что видит глаз. Разные вопросы — разные адресации.
 */
function режим(строка: ReturnType<typeof поле>, имя: string) {
  return строка.locator(".ant-segmented-item").filter({ hasText: имя }).first();
}

function режимВыбран(строка: ReturnType<typeof поле>, имя: string) {
  return строка.getByRole("radio", { name: имя, exact: true, includeHidden: true });
}
...
async function подполеСтроки(page: Page, полеСписка: Locator, подпись: string) {
  // СЧИТАЕТСЯ ВВОД, А НЕ БЛОК — и это не мелочь адресации.
  // Блоки разметки ВЛОЖЕНЫ друг в друга, поэтому условию «содержит подпись, не
  // содержит списка, содержит ввод» удовлетворяет не только ближайший
  // контейнер поля, но и каждый его предок до той же границы. Счёт блоков даёт
  // столько же, сколько уровней вложенности (наблюдалось «3» на живом стенде
  // 2026-08-17), и число это говорит о разметке, а не о форме.
  //
  // Предмет утверждения — «поле с такой подписью в строке ОДНО». Он выражается
  // счётом ВВОДОВ: сколько бы предков ни совпало, элемент ввода за ними один и
  // тот же, и его дубликаты схлопываются.
  const ввод = полеСписка
    .locator("div")
    .filter({ has: page.getByText(подпись, { exact: true }) })
    // Подпись подполя совпадает с ВЫБРАННЫМ ВАРИАНТОМ соседнего списка (см.
    // разбор выше), поэтому блоки со списком отсекаются — вместе с объемлющей
    // строкой целиком: список лежит в ней.
    .filter({ hasNot: page.locator(".ant-select") })
    // ВВОД БЕРЁТСЯ ПО РОЛИ, А НЕ ПО ИМЕНИ ЭЛЕМЕНТА — две ступени разбора, и обе
    // измерены на живом стенде 2026-08-17:
    //
    //   `input`                     → счёт 2: выпадающий список рисует внутри
    //                                 себя поле поиска по вариантам, и оно тоже
    //                                 `input`;
    //   `input:not([role=combobox])`→ счёт 1, но заполнение висит до предела:
    //                                 найденный элемент существует и не
    //                                 принимает ввод.
    //
    // Роль `textbox` — это то, чем элемент ЯВЛЯЕТСЯ ДЛЯ ПОЛЬЗОВАТЕЛЯ: поле, в
    // которое он печатает. Она отсекает и поиск по вариантам (у него роль
    // `combobox`), и служебные элементы, которые вводом не являются.
    .getByRole("textbox");

  // СКОЛЬКО НАШЛОСЬ — ЧАСТЬ УТВЕРЖДЕНИЯ, а не деталь реализации. `.first()` без
  // этой проверки одинаково молчит и когда ввод один, и когда их несколько, и
  // когда подошёл не тот: промах адресации виден лишь по последствию через
  // несколько шагов — ровно так и жил #636.
  await expect(
    ввод,
    `подполе «${подпись}» в строке списка не адресуется однозначно: вводов с этой подписью ` +
      `и без выпадающего списка найдено не один. Это промах АДРЕСАЦИИ, а не отказ формы`,
  ).toHaveCount(1);
  return ввод.first();
}
...
test("группа целей создаётся СРАЗУ с целью — второй заход не требуется", async ({ page }) => {
  ...
  // `подполеСтроки` возвращает САМ ввод, а не объемлющий блок: искать `input`
  // внутри него значило бы искать поле внутри поля — совпадений нет ни одного,
  // и ожидание съедало предел пробы целиком.
  const вводАдреса = await подполеСтроки(page, поле(page, "Цели"), "Внешний адрес");
  await вводАдреса.fill(адрес);

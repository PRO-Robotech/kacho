...
function поле(page: Page, подпись: string) {
  return page.locator(".ant-form-item", { has: page.getByText(подпись, { exact: false }) }).first();
}
...
async function выбрать(page: Page, подпись: string, вариант: RegExp) {
  ...
  await page.locator(".ant-select-dropdown:visible").getByText(вариант).first().click();
}
...
test("группа целей создаётся СРАЗУ с целью — второй заход не требуется", async ({ page }) => {
  ...
  await поле(page, "Внешний адрес").locator("input").first().fill(адрес);

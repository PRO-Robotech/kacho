...
/* ─────────────────────────────────────────────────────────────────────────────
 * ВТОРОЙ УРОВЕНЬ УВЕРЕННОСТИ — ОДНА РЕАЛИЗАЦИЯ НА ВЕСЬ НАБОР
 *
 * Механизм переехал сюда из `stepup.spec.ts` целиком и БЕЗ правки поведения:
 * каждое его утверждение осталось тем же и на том же месте потока. Переезд
 * понадобился потому, что подъём уровня перестал быть предметом одной пробы и
 * стал УСЛОВИЕМ другой: `Invite` объявлен полом «2»
 * (`proto/kacho/cloud/iam/v1/user_service.proto`), а без него человеку не
 * завести второго членства — значит сценарий «карточка молчит о других
 * аккаунтах» без подъёма уровня не собирается вовсе.
 *
 * ВТОРАЯ КОПИЯ ЗДЕСЬ БЫЛА БЫ НЕ УДОБСТВОМ, А ДЕФЕКТОМ. Поток службы личности
 * разъезжается молча: имя узла, шаг кода, набор полей формы — всё это меняется
 * настройкой стенда, и разошедшаяся копия краснеет не там, где сломано. Дерево
 * уже платило за копии одного предмета (`ui.md` §«Незакрытый форк»).
 *
 * ПРЕДМЕТ САМОГО ПОДЪЁМА ОСТАЛСЯ У СВОЕЙ ПРОБЫ. `stepup.spec.ts` по-прежнему
 * утверждает ПАРУ «глагол с полом „2“ отвергается на первом уровне и проходит
 * после поднятия»; сюда переехало то, ЧЕМ уровень поднимается, а не то, ЧТО об
 * этом утверждается.
 * ───────────────────────────────────────────────────────────────────────────── */
...
/**
 * Поднять уровень уверенности текущей сессии до второго — одноразовым кодом,
 * который арендатор заводит САМ.
 *
 * Каждый шаг утверждает свой исход, и это не украшение: сорвавшийся шаг иначе
 * доезжает до вызывающего отказом края шестью строками ниже, где виновником
 * называется невиновный — глагол, у которого пол «2», а не поток, который его
 * не поднял.
 */
export async function raiseAssuranceToAal2(page: Page): Promise<void> {

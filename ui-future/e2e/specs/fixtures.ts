...
/**
 * Перехват записи в буфер — вместо чтения СИСТЕМНОГО буфера.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ПОЧЕМУ НЕ `navigator.clipboard.readText()`
 *
 * Чтение системного буфера зависит от СРЕДЫ, а не от продукта: разрешение
 * `clipboard-read` браузер трактует по-разному в зависимости от профиля и
 * посадки, и в конвейере оно не действует. Локально проба зеленела, в конвейере
 * падала — и падение это утверждало о разрешениях ранера, а не о консоли.
 *
 * Перехват отвечает ровно на тот вопрос, ради которого проба написана: КАКУЮ
 * СТРОКУ продукт кладёт в буфер. Он не снисходительнее чтения — наоборот,
 * строже: чтение видит лишь итог, а перехват ловит каждый вызов и его аргумент,
 * поэтому «положили дважды» или «положили не то и потом исправили» здесь видно,
 * а через чтение — нет.
 *
 * Ставится ДО перехода на страницу: `addInitScript` исполняется в каждом
 * документе до кода страницы.
 */
export async function ловушкаБуфера(page: Page): Promise<() => Promise<string[]>> {
  await page.addInitScript(() => {
    const w = window as unknown as { __скопировано?: string[] };
    w.__скопировано = [];
    const clip = navigator.clipboard as unknown as { writeText?: (t: string) => Promise<void> };
    const было = clip.writeText?.bind(navigator.clipboard);
    Object.defineProperty(navigator.clipboard, "writeText", {
      configurable: true,
      value: async (t: string) => {
        w.__скопировано!.push(t);
        // Настоящий вызов сохраняется: продукт обязан ходить тем же путём, что и
        // в жизни, иначе проба меряла бы саму ловушку.
        try {
          await было?.(t);
        } catch {
          // Среда может не разрешить запись — предмет пробы не в этом.
        }
      },
    });
  });
  return async () =>
    page.evaluate(() => (window as unknown as { __скопировано?: string[] }).__скопировано ?? []);
}

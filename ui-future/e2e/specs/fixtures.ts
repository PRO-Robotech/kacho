...
export async function register(page: Page): Promise<string> {
  ...
  await expect
    .poll(
      async () => {
        if ((await page.context().cookies()).some((c) => /session/i.test(c.name))) {
          return "сессия";
        }
  ...
  if (refusal !== "") {
    throw new Error(
      `регистрация ОТВЕРГНУТА службой личности: ${refusal}. Печенья сессии нет не ` +
        "потому, что оно опоздало, — поток до выдачи не дошёл. Разбирать надо " +
        "названный отказ, а не ожидание печенья",
    );
  }

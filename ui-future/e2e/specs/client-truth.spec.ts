...
// nameItemOf — блок формы, принадлежащий полю «Имя».
//
// Утверждения об отказе привязываются к НЕМУ, а не к форме: нажатие «Создать»
...
test("форма создания называет ту форму имени, которую платформа принимает", async ({
  ...
  await expect
    .poll(async () => await nameItem.locator("[role=alert]").count(), {
      message:
...
test("экран выдачи прав называет уровни теми же словами, что и вся консоль", async ({
  ...
  //
  // СПРАШИВАЕТСЯ У САМОГО ПЕРЕКЛЮЧАТЕЛЯ, А НЕ У СТРАНИЦЫ (#1441 п.4). Ровно
  // такую же надпись — «Аккаунт», слово в слово — рисует КАРКАС: это заголовок
  // колонки в панели выбора области (`host` ScopePicker), которую открывают
  // пилюлей в шапке. Поиск по всей странице способен зазеленеть на шапке чужой
  // панели, то есть перестать проверять свой предмет, не изменившись сам.
  // Сегодня панель в этой пробе не открывается и совпадения не случается, но
  // «сегодня не совпадает» — не свойство пробы.
  const levels = page.locator(".ant-segmented").first();
  await expect(
    levels,
    "переключателя уровня на экране выдачи прав нет вовсе: о словах, которыми названы " +
      "уровни, такой прогон не говорит ничего",
  ).toBeVisible({ timeout: 45_000 });
  await expect(
    levels.getByText("Аккаунт", { exact: true }).first(),
...
test("пустой реестр не учит клиента, что push — единственный путь", async ({
  ...
  // Карточка реестра открывается на «Обзоре»; пустой список репозиториев живёт
  // на соседней вкладке — это видно в снимке страницы упавшего прогона, где
  // вкладка «Репозитории» стоит рядом с выбранной «Обзор».
  await page.getByRole("tab", { name: "Репозитории" }).click();
...
test("первый экран нового клиента называет первый шаг", async ({ page }) => {
  ...
  // ПРЕДПОСЫЛКА ЗАДАЧИ ПЕРЕМЕРЕНА И СУЗИЛАСЬ, и утверждение переписано под то,
  // что верно.
  //
  // Задача описывала клиента, у которого дерево слева сообщает «Аккаунтов нет»,
  // а экран велит выбрать проект. На этом стенде такого состояния НЕ НАСТУПАЕТ:
  // регистрация заводит личный аккаунт и проект `default` — снимок страницы
  // упавшего прогона показал в дереве и то, и другое сразу после `register`.
  //
  // Предмет задачи от этого не исчез, он сузился: экран обязан называть ход,
  // который клиент МОЖЕТ сделать. Это и утверждается — названный проект есть в
  // дереве, то есть указание исполнимо.
  //
  // Ветка «аккаунтов нет» в продукте остаётся оборонительной (заведение аккаунта
  // может и не удаться) и здесь НЕ утверждается: проба, ждущая недостижимого
  // состояния, красна всегда и вердикта не даёт.
  const tree = page.getByRole("tree");
  await expect(
    tree,
    "дерево областей не отрисовалось — тогда неизвестно, исполним ли названный ход",
  ).toBeVisible({ timeout: 30_000 });

  await expect(
    tree.getByRole("treeitem"),
    "экран велит выбрать проект в дереве, а дерево пусто: указание неисполнимо, " +
      "и это отказ, не восстанавливающий следующий шаг",
  ).not.toHaveCount(0);

  // Ход существует и ведёт туда, где шаг делается: выбор проекта в дереве
  // открывает модули в его границах — это и есть названный экраном первый шаг.
  await tree.getByRole("treeitem").last().click();
  await expect
    .poll(() => new URL(page.url()).pathname, {
      message:
        "выбор области в дереве никуда не привёл: экран назвал ход, которого не делает",
      timeout: 30_000,
    })
    .toContain("/projects/");
});

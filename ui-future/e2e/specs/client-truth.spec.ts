...
test("экран выдачи прав называет уровни теми же словами, что и вся консоль", async ({ page }) => {
  ...
  // Заглушка направляет к ручке ТЕМ ЖЕ словом, что на ручке написано.
  await expect(
    page.getByText(/Выберите Project|Выберите Account/),
    "заглушка зовёт выбрать «Project»/«Account» латиницей, а ручка подписана по-русски: " +
      "экран отправляет искать то, чего на ней не написано",
  ).toHaveCount(0);
});

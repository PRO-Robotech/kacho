...
export default defineConfig({
  ...
  //
  // Все 66 проб заводят арендатора ОДНИМ общим фикстуром, поэтому его поломка
  // роняет весь набор одинаково, и цена такой поломки всегда максимальная.
  // Замерено на прогоне 32602270323: упавшая проба стоит 181 с (разброс меньше
  // секунды на одиннадцати подряд), значит весь набор при широкой поломке —
  // 66 × 181 с ≈ 3 ч 19 мин против предела шага в 35 минут. Вердикт при таких
  // величинах существует, только если падает не больше 11 проб из 66.
  //
  ...
  //
  // Смысл величин это НЕ размывает: «исполнено 5 из 66» остаётся ТРЕТЬЕЙ

...
export default tseslint.config(
  ...
  {
    // ЗАПИСЬ ТРАССЫ ПРИНАДЛЕЖИТ НАБОРУ (#1242): браузерную трассу останавливает
    // разбор `page` из `specs/fixtures.ts`, потому что штатная сборка трассы
    // прогонщиком обрывается по бюджету и оставляет файл, который не
    // открывается ни одним просмотрщиком. Проба, взявшая `test` напрямую у
    // `@playwright/test`, эту фикстуру НЕ получает — и остаётся без трассы
    // молча: отличить её от пробы, чья трасса просто не понадобилась, нечем.
    // Поэтому имя `test` из `@playwright/test` в пробах запрещено; `expect` и
    // типы берутся оттуда по-прежнему.
    // Охват — ПО ВИДУ ФАЙЛА, а не по каталогу: проба остаётся пробой и тогда,
    // когда лежит вне исполняемого набора. Прежний шаблон называл `specs/`
    // поимённо, и каталог проб, ждущих своего условия, выпадал из этого правила
    // молча — измерено инъекцией: тот же запрещённый импорт в `specs/` давал
    // ошибку, а в каталоге ожидания линт проходил. Перечень каталогов здесь не
    // ведётся намеренно: следующий каталог проб выпал бы из него ровно так же.
    files: ["**/*.spec.ts"],
    rules: {
          ...
          paths: [
            {
              ...
              importNames: ["test"],
              message:
                "test берётся из ./fixtures — иначе проба останется без записи трассы (#1242).",
            },
          ],
        },
      ],

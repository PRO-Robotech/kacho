...
for (let i = 1; i <= 12; i++) {
  try {
    // Идём на КОРЕНЬ, а не на служебный ответ: браузеру нужна страница.
    //
    // Здесь стоял `/healthz`, и он дал `Download is starting`: край отдаёт этот
    // ответ так, что браузер начинает загрузку файла вместо навигации. Проверка
    // «дошёл ли браузер» на таком адресе неразрешима by construction — и она
    // отвергла бы исправный стенд, назвав это «условие не создано».
    //
    // Корень — ровно тот адрес, с которого начинают пробы, поэтому его успех и
    // означает, что условие для них создано.
    const resp = await page.goto(BASE, { timeout: 10_000 });
    if (resp && resp.status() === 200) {
      console.log(`✓ браузер достаёт консоль на ${BASE} (попытка ${i})`);
      reached = true;
      break;
    }

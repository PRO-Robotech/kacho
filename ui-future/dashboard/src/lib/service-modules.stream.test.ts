...
describe("счётчики витрины ↔ предметы потока", () => {
  ...
  it("перепись счётчиков названа числом — новая плитка требует РЕШЕНИЯ, а не умолчания", () => {
    ...
    expect(`всего ${allStats.length}, с журналом ${journaled.length}, на опросе ${polled.length}`).toBe(
      "всего 14, с журналом 14, на опросе 0",
    );
    // НОЛЬ НА ОПРОСЕ — ЦЕЛЬ, А НЕ ПОЛОМКА, и проба на ней проходит.
    //
    // Было «с журналом 11, на опросе 3» при трёх счётчиках службы доступа: она
    // журнала не вела. Журнал у неё появился, три её счётчика предмет получили,
    // и опрашиваемых в витрине не осталось. Перепись остаётся числом ровно
    // затем, ради чего заведена: следующая плитка без предмета сдвинет ноль и
    // потребует РЕШЕНИЯ, а не уедет на вечный опрос молча.
    expect([...new Set(polled.map((s) => s.module))]).toEqual([]);
  });
});

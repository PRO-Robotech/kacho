...
//
// И ЭТО НАБЛЮДАЛОСЬ, а не только задумано (2026-09-15, третье сужение). Пин
// службы поднялся до версии, где `iam_role` лишился `v_get` (kacho#1922), — и
// проба покраснела ТЕМ ЖЕ ЗАХОДОМ, одним утверждением из пятнадцати, назвав
// строку. Ни один файл этого дерева при этом не менялся: набор приехал модулем.
// Прежняя редакция — литерал против литерала — осталась бы зелёной, потому что
// сверяла фикстуру с ней же.
...
describe("verbOptions — словарь глаголов по ресурсу", () => {
  ...
  it("модуль не выбран — общий словарь каталога", () => {
    // Пересечение наборов ВСЕХ типов платформы: снятие глагола у ЛЮБОГО из них
    // сужает его. Это верное поведение поля — оно объявлено общим для всех
    // ресурсов, а не перечнем всех глаголов. Сужали дважды, оба раза осознанно:
    // `iam_user` лишился `v_delete` (#1189) — ушло `delete`; `iam_role` лишился
    // `v_get` (kacho#1922) — ушло `get`.
    //
    // Второе снятие — ПОЧИНКА, а не поломка: у отношения не было ни одного
    // читателя (чтение роли поимённо энфорсится тем же предикатом, что страница,
    // обе поверхности зовут один резолвер), право по нему не открывало ничего —
    // а превью роли его обещало, разворачивая `classes: ["*"]` в набор ТИПА.
    // Платформа с моделью сходится: из трёх записей её вшитого каталога прав на
    // `iam_role` ни одна не требует `v_get` — только `v_list`/`v_update`/`v_delete`
    // (gateway/internal/middleware/embed/permission_catalog.json).
    //
    // ПРИЧИНА утверждается здесь же, рядом со следствием. Иначе литерал ниже
    // держался бы одним комментарием: вернувшийся глагол покраснел бы ЧИСЛОМ, не
    // назвав виновника, — и искать его пришлось бы среди всех типов модели.
    expect(declaredVerbs("iam_role")).not.toContain("get");
    // Положительный контроль обязателен: на выпотрошенном типе пересечение тоже
    // схлопнулось бы, и `["list"]` оказалось бы верно по другой причине.
    expect(declaredVerbs("iam_role")).toContain("list");

...
// СЛУЖБЫ ЛИЧНОСТИ (`/.ory/…/sessions/whoami`) — и ответ не читал
...
describe("подъём оболочки консоли", () => {
  ...
  // Заменитель встаёт присвоением — ПОД стража мест выпуска окружения (F8-46,
  // `test/setup.ts`): до него доходят только обращения упорядочивающего
  // транспорта. Поэтому обход читается двумя держателями: адрес, выпущенный
  // транспортом, попадает в `asked` и судится утверждениями ниже; выпущенный мимо
  // него до сети не доходит, и пробу роняет запись стража в `afterEach`.
  beforeEach(() => {
    asked = [];

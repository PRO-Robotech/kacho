...
export default function ClusterAdminsPage() {
  ...
  const adminsQ = useQuery({
    ...
    // поллинг остаётся: надзор кластера отдельным видом журнал не ведёт — среди
    // семи его видов такого нет, а кластер ресурсом службы не является вовсе.
    // Прежняя причина («журнала у iam нет») истекла: журнал есть, и семь его
    // видов консоль называет, — но этого предмета среди них по-прежнему нет.

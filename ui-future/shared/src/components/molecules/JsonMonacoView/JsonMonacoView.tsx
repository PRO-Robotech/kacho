...
//
// Путь — от базы САМОГО модуля, а она зависит от того, ЧЕМ собрано, и различие
// здесь не косметическое. В образе каждый федеративный модуль собирается под
// своим префиксом (`KACHO_PUBLIC_BASE=/vpc-remote/`, `/nlb-remote/`, …), поэтому
// запрос уходит на `/<модуль>-remote/monaco/vs` — в его собственную раздачу.
// Локально — и в `dev`, и в `preview` — префикса нет ни у кого: база у всех `/`,
// запрос становится `/monaco/vs` и резолвится против origin СТРАНИЦЫ, то есть
// оболочки, потому что модуль исполняется в её контексте.
//
// Отсюда неочевидное: копию редактора кладут у себя ВОСЕМЬ пакетов из девяти —
// семь, которые вкладку рисуют, и оболочка, которая не рисует её никогда. У
// оболочки шаг существует ровно ради локальной сборки; убрать его как «мёртвый»
// нельзя — вкладка JSON молча перестала бы работать в `preview`, а образ
// собирался бы по-прежнему зелёным. Девятый пакет (`dashboard`) карточек не
// имеет вовсе, редактор в его граф импортов не входит, и шага у него нет — это

...
/**
 * Текст подтверждения ЗАПРЕТА над строкой — прочитанный так же, как его читает
 * человек: меню собрано, пункт нажат, окно открыто. Рендер снимается, чтобы обе
 * ветки подтверждения можно было измерить в ОДНОЙ пробе.
 *
 * ПОЧЕМУ СРАВНЕНИЕ ВЕТОК, А НЕ ДВЕ НЕЗАВИСИМЫЕ ПРОБЫ (#1219). Проба каждой ветки
 * по отдельности требует знать, каким текст ДОЛЖЕН быть, — а это и есть спорный
 * вопрос. Сравнение спрашивает другое: «решал ли кто-нибудь, что они
 * различаются» (`architecture.md` §«Параллельные полосы одного механизма обязаны
 * сверяться МЕЖДУ СОБОЙ»). На это ответ есть всегда.
 */
function blockConfirmTextFor(selfId: string | undefined): string {
  selfUserId.current = selfId;
  const view = renderUsersMenu({ id: "usr-1", email: "a@kacho.local", invite_status: "ACTIVE" });
  // ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ: меню собрано и глагол разрешился. Без него всякое
  // утверждение о тексте ниже зеленело бы на не отрисовавшемся меню.
  expect(menuLabels()).toContain("Запретить участие");
  fireEvent.click(itemByLabel("Запретить участие")!);
  const text = screen.getByRole("dialog").textContent ?? "";
  // Снятие обязательно: `screen` читает ВЕСЬ документ, и второй рендер в той же
  // пробе иначе дал бы два меню и два окна сразу — утверждения перестали бы
  // относиться к названной ветке.
  view.unmount();
  return text;
}
...
describe("действие-глагол на строке пользователя", () => {
  ...
  // и она здесь не подразумевается: пара #1208 меряется над ЧУЖОЙ строкой, а её
  // в браузере не завести — второго человека в аккаунте требует приглашение со
  // ступенью доверия 2. Долг записан с предикатом снятия в
  // `ui-future/e2e/specs/users.spec.ts`.
  //
  // Здесь стояло «обещание живёт в НЕ-СВОЕЙ ветке подтверждения» — верное до
  // #1219 и ложное после: узкий выход называется теперь в ОБЕИХ ветках, и над
  // СВОЕЙ строкой проба браузером существует (там же по файлу). Блокатор
  // остался только на чужую строку.
  ...
  // ── узкий выход назван и над СОБСТВЕННОЙ строкой (#1219) ───────────────────
  //
  // ОБРАТНАЯ СТОРОНА #1208, и цена выше. Там подтверждение называло действие,
  // которого в меню НЕТ; здесь оно НЕ НАЗЫВАЛО действие, которое ЕСТЬ. Человек,
  // запрещающий участие СЕБЕ, совершал необратимое — снять запрет самому нельзя,
  // возвращает доступ только администратор облака, — не узнав, что рядом лежит
  // узкий выход: выйти только из одного аккаунта.
  //
  // ЗАМЕР, а не предположение: узкий выход над своей строкой ДОСТУПЕН, и это
  // установлено двумя независимыми признаками.
  //   - В МЕНЮ. Проба ниже («узкий выход над СВОЕЙ строкой действительно ЕСТЬ»)
  //     — и то же наблюдается браузером на живом стенде: у арендатора проб
  //     единственная строка на странице СОБСТВЕННАЯ, и пункт на ней появляется
  //     при выбранной области (`ui-future/e2e/specs/users.spec.ts`, #1127).
  //   - В МОДЕЛИ ПРАВ. Запрет гейтится `iam_user.identity_suspender`, а он есть
  ...
  //     вправе и исключить. Обратное неверно — и это ровно та причина, по
  //     которой узкий выход стоит называть.
  // Значит второй законный исход задачи — «над собой выхода нет, и это сказано
  // прямо» — не наш: выход есть.
  it("без области узкий выход назван в ОБЕИХ ветках подтверждения — и над своей тоже", () => {
    // verifies #1219 — КРАСНАЯ до фикса: собственная ветка про исключение
    // молчала вовсе.
    const own = blockConfirmTextFor("usr-1");
    const other = blockConfirmTextFor("usr-9");

    // ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ ВЕТОК: измерены ДВЕ РАЗНЫЕ, а не одна дважды. Без
    // него «обе называют выход» было бы истинно впустую — на одном и том же
    // тексте, показанном всем.
    expect(own).toMatch(/самостоятельно/i);
    expect(other).not.toMatch(/самостоятельно/i);
    expect(own).not.toEqual(other);
    // ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ ОКНА: каждое отрисовано и несёт СВОЙ предмет. Без
    // него утверждения о тексте зеленели бы на пустой строке.
    expect(own).toMatch(/НА ПЛАТФОРМУ/);
    expect(other).toMatch(/НА ПЛАТФОРМУ/);

    // ПАРА: узкий выход назван в ОБЕИХ ветках — и вместе с условием, потому что
    // области нет и пункта в этом меню сейчас тоже нет.
    expect(own).toContain("Исключить из аккаунта");
    expect(other).toContain("Исключить из аккаунта");
    expect(own).toMatch(/выберите область/i);
    expect(other).toMatch(/выберите область/i);
  });

  it("с выбранной областью узкий выход назван в ОБЕИХ ветках БЕЗУСЛОВНО", () => {
    // verifies #1219 — вторая половина пары. Без неё «обе ветки называют
    // условие» зеленело бы и на тексте, который называет его ВСЕГДА: человек,
    // область уже выбравший, читал бы указание выбрать её ещё раз.
    contextApi.setAccount({ id: "acc-9", name: "acme" });
    ...
    expect(own).toContain("Исключить из аккаунта");
    expect(other).toContain("Исключить из аккаунта");
    expect(own).not.toMatch(/выберите область/i);
    expect(other).not.toMatch(/выберите область/i);
  });

  it("узкий выход над СВОЕЙ строкой действительно ЕСТЬ — обещание не пустое", () => {
    // verifies #1219 — половина, без которой пара выше повторила бы дефект
    // #1208 в собственной ветке: текст называл бы пункт, которого над своей
    // строкой нет. Здесь закреплён ЗАМЕР, на котором стоит решение задачи.
    contextApi.setAccount({ id: "acc-9", name: "acme" });
    selfUserId.current = "usr-1";
    renderUsersMenu({ id: "usr-1", email: "a@kacho.local", invite_status: "ACTIVE" });

    // ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ: меню собрано и глаголы разрешились.
    expect(menuLabels()).toContain("Запретить участие");
    expect(menuLabels()).toContain("Исключить из аккаунта");

    // И это ветка «над собой», а не общая: подтверждение пункта говорит о СЕБЕ.
    // Без этой половины «пункт есть» не отличало бы свою строку от чужой — то
    // есть не утверждало бы ничего о доступности выхода ИМЕННО себе.
    fireEvent.click(itemByLabel("Исключить из аккаунта")!);
    expect(screen.getByRole("dialog").textContent ?? "").toMatch(/себя/i);
  });
});

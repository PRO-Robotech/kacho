...
describe("RowActionsMenu — состав меню", () => {
  it("у изменяемого проектного ресурса есть просмотр, правка и удаление", () => {
    // Перемещения здесь НЕТ и быть не должно: `:move` у подсети не объявлен
    // (#583). Прежняя редакция этого утверждения ждала пункт «Переместить» и
    // тем закрепляла дефект — перечень был устроен исключениями, поэтому пункт
    // получал всякий ресурс, которого в нём не назвали. Состав утверждается
    // ДОСЛОВНО: `not.toContain` пропустил бы подмену одного пункта другим.
    renderMenu("subnets", { id: "sub-1", name: "frontend" });
    expect(menuLabels()).toEqual(["Просмотр", "Редактировать", "Удалить"]);
  });

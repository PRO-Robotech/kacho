...
//   1. ребро, объявившее серверное сужение, спрашивает СЕРВЕР — проверяется сам
//      запрос (адрес и параметры), а не наличие текста на экране; и проверяется
//      КАКИМ из двух механизмов: выражением `filter` (сеть → её дети) либо
//      типизированным полем запроса (подсеть → адреса);
//   2. парный контроль «ничего не объявлено — параметр не выдумывается» живёт
//      НЕ здесь, а в `related-list-query.test.ts`, где решение принимается
//      функцией и потому проверяется в обе стороны без стенда. Здесь его
//      воспроизводить нечем: после того как адреса сузились по подсети, ребра
//      без серверного сужения в этом реестре не осталось, и фикстура под него
//      была бы выдуманной — то есть проба зеленела бы на собственной выдумке;
//   3. продолжение курсора показано той же формой, что на странице списка
//      («Показать ещё»), и ровно тогда, когда за курсором есть ещё (отрицание
//      в паре с положительным);
...
/** Стенд карточки подсети с её вкладкой адресов; сам список адресов доопределяет
 *  вызывающий — у каждого утверждения своя страница. */
const addressesTab = () => ({
...
describe("ребро с серверным полем сужения — сужает СЕРВЕР", () => {
  ...
  it("вкладка адресов подсети сужается ТИПИЗИРОВАННЫМ полем, а не выражением", async () => {
    // Здесь стояло обратное утверждение — «параметра вкладка не выдумывает», — и
    // оно было верным ровно до тех пор, пока сузить адреса по подсети было нечем:
    // подсеть лежит внутри jsonb и в двух семьях, поэтому в белом списке
    // выражения `filter` её нет и быть не может. Владелец принимает эту ссылку
    // ОТДЕЛЬНЫМ полем списочного запроса, поэтому прежняя проба стала пробой,
    // требующей дефекта: она краснела бы ровно на его починке. Переписана вместе
    // с ним, а не ослаблена.
    //
    // Утверждается ЗАПРОС целиком: выражение `filter` здесь было бы отказом
    // владельца на всю вкладку, поэтому «есть параметр» мало — важно и то, что
    // рядом нет второго механизма.
    const calls = stubApi({
      ...addressesTab(),
      list: {
        ...addressesTab().list,
  ...
  it("строки сужённого по подсети ответа показаны — положительный контроль", async () => {
    stubApi({
      ...addressesTab(),
      list: {
        ...addressesTab().list,
...
describe("страница ребёнка недочитана — обрезание не молчаливое", () => {
...
// Здесь стоял набор из трёх проб «вкладка связанного ресурса: счётчик — тот же,
// что над списком». СНЯТ ВМЕСТЕ С ПРЕДМЕТОМ: счётчик показанных строк убран из
// консоли решением владельца — «число над таблицей не отвечало ни на один
// вопрос, ради которого на страницу приходят, а место в ряду ручек занимало».
// Решение записано в самом продукте, в двух местах: примечание на месте снятого
// компонента (`molecules/TableToolbar/TableToolbar.tsx`) и разбор у строки
// инструментов списка (`organisms/ResourceListPage/ResourceListPage.tsx`).
//
// Пробы снимаются ЦЕЛИКОМ, а не выхолащиваются, и снимаются ВСЕ ТРИ — включая
// ту, что была зелёной («детей нет вовсе — счётчика нет»). Её зелень уже ничего
// не означала: производителя счётчика в дереве нет ни одного, поэтому «счётчика
// нет» стало истиной по построению и сторожило бы вечно самое себя.
//
// Предпосылка набора («счётчик над списком есть, над вкладкой его не было
// вовсе») перестала быть верной в ту же волну правок, в которую набор и писался:
// две линии работы правили один предмет с разных сторон. Перечень производителей
// в рабочем дереве — предикат, которым это перепроверяется за секунду:
//   grep -rn 'Всего:\|Загружено:' --include=*.tsx <модули> | grep -v '\.test\.'
// Вернётся счётчик — вернутся и пробы, но вместе со своим предметом.

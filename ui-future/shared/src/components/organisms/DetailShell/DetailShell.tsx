...
export function DetailShell({
  ...
  // Показываем только те ссылки, которым есть куда вести (#624). Правило 9
  // `ui.md`: поле без источника не показывается — ссылка, обещающая страницу
  // документации, которой нет, хуже прочерка, потому что приглашает нажать.
  //
  // Послабление ИСТЕКАЕТ САМО: как только объявление ресурса принесёт настоящий
  // адрес, ссылка начнёт показываться без единой правки здесь. Именно поэтому
  // блок не удалён, а отфильтрован, — удаление пришлось бы отменять.
  //
  // Здесь же снят набор ссылок VPC, стоявший запасным значением: карточка, не
  // передавшая своих ссылок (аккаунт, токен), показывала документацию ЧУЖОГО
  // домена — «Облачные сети и подсети» на странице токена.
  const docs = (docLinks ?? []).filter(hasAddress);
      ...
      >
        ...
        {docs.length > 0 && (
          ...
          >
            ...
            >
              {docs.map((d, i) => (
                // Ключ — ПОЛОЖЕНИЕ строки, а не адрес: две подписи, ведущие на
                // одну страницу, — законный состав, и по адресу ключ совпал бы у
                // обеих (React: «поведение не поддержано, строки могут
                // дублироваться или пропадать»). Список позиционный, строится из
                // объявления ресурса и не переупорядочивается — индекс здесь
                // различает ровно то, что различает сам список.
...
/** Ссылке есть куда вести. `"#"` — это «адреса нет»: якорь на саму себя, по
 *  которому нажатие не делает ничего. Пустая строка — то же самое. */
function hasAddress(d: DocLink): boolean {

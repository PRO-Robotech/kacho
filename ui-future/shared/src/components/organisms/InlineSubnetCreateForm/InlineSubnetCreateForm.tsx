    ...
    <FormShell specId="subnets" mode="create" singular={subnetSpec.singular}>
      ...
      >
        ...
        {/* VPC-1: единственный immutable основной CIDR на семейство (≥1 обязателен).
            ...
            ЗВЁЗДОЧКА СТОИТ НА ПАРЕ, А НЕ НА ОДНОМ ИЗ ДВУХ (#609). Обязанность
            здесь — дизъюнкция: контракт владельца не требует ни `ipv4_cidr_primary`,
            ни `ipv6_cidr_primary` (оба без `(required) = true`, и пояснение поля
            говорит прямо: подсеть может быть только-v6), а `submit()` ниже
            отвергает ввод ровно тогда, когда пусты ОБА. Значок на одном из двух
            выражал это неверно дважды: сообщал обязанность там, где её нет, и
            молчал о единственной настоящей. Ряд группы несёт значок и НАЗЫВАЕТ
            правило словами; ряды семейств значка не несут — и это сходится с
            контрактом, что держит гейт `lib/required-mark-contract-parity`. */}
        <Form.Item label="Основной CIDR" required>
          {/* Цвет — токеном, а не литералом: у консоли есть светлая тема, и
              захардкоженное белое стало бы в ней невидимым. Соседние литералы
              в этом файле — прежний долг, а не образец. */}
          <span style={{ color: "var(--kc-text-secondary)" }}>
            Заполните хотя бы одно из двух полей ниже — подсеть может быть только-IPv4 либо только-IPv6.
          </span>
        </Form.Item>

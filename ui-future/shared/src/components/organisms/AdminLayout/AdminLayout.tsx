...
// address-pools,cluster/admins,limits}`).
//
...
// владельца #447).
//
// ПОЧЕМУ В `shared/`, А НЕ В МОДУЛЕ. Раздел маршрутизируют ДВА приложения этого
// дерева, и до #447 у каждого была своя копия оболочки. Копии успели разойтись:
// одна рисовала вертикальный рейл, другая — горизонтальный ряд с поясняющим
// абзацем, снятым решением владельца. Правка доезжала до одного места из двух, а
// пользователь читал разницу как «другое место продукта». Единый источник тут
// держится гейтом (`shared-organisms-single-source`): файл вне `shared/`,
// объявляющий этот символ, — находка.
//
...
//
// Форма-модалка здесь НЕ монтируется: её монтирует фрейм приложения (у `system` —
// `RemoteShell`, у `vpc` — `Layout`), а регионы/зоны/пулы используют панельные формы.
...
const SECTIONS: AdminSection[] = [
  ...
  },
  {
    ...
    // /iam/v1/limits требует system_admin на кластере — публичный адрес,
    // закрытый ОТНОШЕНИЕМ, а не сокрытием (ADM-1 S1, #878). Тенантская
    // витрина «Мои квоты» этой ручки не несёт вовсе: свой предел и свой
    // расход арендатор читает у владельца типа (`/{домен}/v1/quotas`).
    visible: (p) => p.isSystemAdmin,
  },
];

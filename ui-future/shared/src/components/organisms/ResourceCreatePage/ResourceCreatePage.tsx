  ...
  return (
    ...
    <div>
      ...
      <ResourceFormBody
        ...
        // Отказ, относящийся к форме ЦЕЛИКОМ, показывается СРАЗУ, а не после
        // того, как заполнены прочие поля: он говорит, что ресурс не создастся
        // ни при какой заполненности. Прежде он всплывал только из `submit`, а
        // до `submit` форма не доходила, пока оставалось хоть одно незаполненное
        // обязательное поле — и арендатор, выбрав несоздаваемый вид машины,
        // узнавал об этом, проделав работу, которая всё равно ни к чему не вела.
        formError={spec.validate?.(obj) ?? null}
        onSubmit={submit}

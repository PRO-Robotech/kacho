...
async function submit() {
  // Обязательное поле заполняется ДО отправки, потому что форма больше не
  // отправляет незаполненное: у региона идентификатор объявлен обязательным, а
  // шаблон кладёт пустую строку. Прежде проба отправляла её и утверждала о
  // запросе, который край отверг бы на первом же поле; теперь тот же запрос
  // собирается из законного ввода, и всё, что проба утверждает про адрес, тело и
  // исход операции, относится к запросу, который сервер принял бы.
  await userEvent.type(await screen.findByPlaceholderText("region-1"), "ru-central1");
  // Именно кнопка, не текст: «Создать» стоит в этом DOM ДВАЖДЫ — заголовком
  // над формой (`<h3>Создать регион`) и подписью кнопки (`<button>Создать`), —
  // и поиск по тексту нашёл бы оба. Счёт замерен, а не прикинут: временная
  // проба на этом же рендере дала 2. (На живой странице узлов больше — там
  // рисуются ещё и крошки; здесь их некому отрисовать, слот только хранит.)
  //
  // Имя задано СТРОКОЙ, а не выражением: строка сверяется целиком, поэтому
  // вернувшееся на кнопку склонение («Создать регион») эту пробу роняет.
  // Прежнее /Создать регион/i принимало бы и склонённую форму, и — как
  // подстрока — не принимало короткую; после решения владельца (канон §3, §8:
  // кнопка называет действие, предмет назван заголовком) оно называло
  // несуществующую кнопку и роняло все девять проб файла на локаторе.
  const button = await screen.findByRole("button", { name: "Создать" });
...
describe("ResourceCreatePage — Region", () => {
  // КОНТРОЛЬ КАНОНА: пара «кнопка — заголовок», а не одна её половина.
  //
  // Обе стороны утверждаются вместе, потому что порознь каждая зелёная на
  // сломанном продукте: короткий «Создать» при заголовке, который предмет НЕ
  // называет, оставляет страницу без ответа на вопрос «что создаётся», а
  // склонённая кнопка при названном заголовке называет предмет дважды в
  // двадцати точках друг от друга. Остальные пробы файла берут кнопку
  // локатором — этот контроль и есть то, что не даёт им зазеленеть на паре
  // «короткая кнопка + безымянный заголовок».
  it("кнопка называет действие, предмет назван заголовком над ней", async () => {
    stubFetch({ id: "opr-1", done: true }, { id: "opr-1", done: true });
    renderCreate();

    // Положительная сторона: предмет НАЗВАН, и назван объявленным падежом.
    expect(await screen.findByRole("heading", { name: "Создать регион" })).toBeInTheDocument();

    // Отрицательная сторона в паре с ней: предмета на кнопке нет.

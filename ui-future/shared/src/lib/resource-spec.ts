...
export interface ResourceSpec {
  ...
  related?: {
    ...
    /** Имя поля, которым ВЛАДЕЛЕЦ ребёнка принимает сужение по этому родителю
     *  НА СЕРВЕРЕ. Объявлено — вкладка просит у списка только детей этого
     *  родителя (`filter=<поле>="<id родителя>"`, один запрос, ответ точный);
     *  `filterField` при этом остаётся ПОДСТРАХОВКОЙ, а не основным путём.
     *
     *  Отличие от `filterField` существенно и не косметично, даже когда имена
     *  совпадают текстуально:
     *    · `filterField` — путь в СТРОКЕ ответа (может быть вложенным:
     *      `internal_ipv4_address.subnet_id`), и фильтрует КЛИЕНТ, то есть
     *      только то, что успело приехать на загруженные страницы;
     *    · `serverFilterField` — имя поля в белом списке выражения `filter`
     *      ВЛАДЕЛЬЦА, и сужает СЕРВЕР, то есть весь список.
     *
     *  Не объявлено — сужение остаётся клиентским, а страница списка ребёнка
     *  показывает продолжение курсора («Показать ещё»): вкладка обязана не
     *  выдавать первую страницу списка проекта за весь список ребёнка.
     *
     *  Список владельца ЗАКРЫТ: поле вне него — не «фильтр без эффекта», а
     *  `InvalidArgument` на всю вкладку. Паритет с владельцем держит проба
     *  `related-server-filter-parity.test.ts` (читает белые списки из прод-кода
     *  сервиса), форму запроса — `ResourceShell.related.test.tsx`. Типизированные
     *  поля списочного запроса (напр. `subnet_id` у адреса) — ДРУГОЙ механизм;
     *  этим полем они не объявляются. */

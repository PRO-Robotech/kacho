...
const REFUSALS: Record<string, RefusalVerdict> = {
  ...
  // Домен величин объявлен отсутствующим — ПОСАДКА, а не отказ по пределу
  // (#2515). Отдельный вердикт, а не полоса квоты, и различие несущее: у
  // «потолок не назван» есть следующий шаг — администратор заведёт величину, —
  // а здесь его НЕТ и быть не может: заводить величину негде, потому что
  // заводить её некому. Свести их значило бы отправить арендатора просить о
  // действии, которое в этой установке не выполнимо ни при каком входе.
  QUOTA_AUTHORITY_ABSENT: {
    kind: "explain",
    title: "Пределы в этой установке не задаются",
    text:
      "Домен величин не развёрнут, поэтому ограничений на количество ресурсов нет: " +
      "ни одна операция не будет отклонена из-за квоты. Обращаться за повышением " +
      "предела не к кому — предела нет.",
  },
  ...
  // Защита от удаления поднята у САМОГО ресурса. Вердикт `explain`, а не
  // `passthrough`, и это замер, а не вкус: из двух производителей платформы
  // следующий шаг называет один («clear it via Update before Delete»), а второй
  // сообщает только факт («load balancer has deletion protection enabled»).
  // Пропустить прозу значило бы показать половине смотрящих отказ, из которого
  // не следует ни одного действия.
  //
  // Полоса отдельна от `REFERENCE_IN_USE` намеренно: там снять мешают ЧУЖИЕ
  // строки, и шаг — освободить их; здесь зависимых нет ни одного, и шаг —
  // снять флаг у самого ресурса. Свести их значило бы отправить смотрящего
  // искать зависимые ресурсы, которых нет.
  DELETION_PROTECTED: {
    kind: "explain",
    title: "Ресурс защищён от удаления",
    text:
      "У ресурса включена защита от удаления. Снимите её — правкой ресурса, поле " +
      "«Защита от удаления», — и повторите удаление. Зависимых ресурсов искать не нужно: " +
      "удаление удерживает сам флаг, а не чужие ссылки.",
  },
...
/**
 * Машинный признак отказа — ЛИБО `null`, если его нет вовсе.
 *
 * Вынесен наружу ради ОДНОГО вызывающего — витрины квот: она обязана отличить
 * «пределы не прочитаны» (сбой, красная плашка) от «пределов в этой установке
 * не бывает» (посадка, сообщение). Разбор прозы для этого не годится по той же
 * причине, по которой он не годится внутри: тон — часть контракта и меняется
 * осознанно, а вывод полосы из английской фразы вернул бы пустоту при первой же
 * правке.
 */
export function refusalReasonOf(err: unknown): string | null {
  if (!err || typeof err !== "object") return null;
  const info = errorInfoOf((err as { details?: unknown }).details);
  return info === null ? null : info.reason;
}

/** Признак полосы «домен величин объявлен отсутствующим» — один на витрину и на разбор. */
export const QUOTA_AUTHORITY_ABSENT_REASON = "QUOTA_AUTHORITY_ABSENT";

...
export const REGISTRY: Record<string, ResourceSpec> = {
  ...
  users: {
    ...
    columns: [
      ...
      //
      // Ответ «к какому аккаунту относится это членство» не показывается и на
      // КАРТОЧКЕ: поле снято с контракта (#471, `reserved 6`), а ресурса
      // членства ещё нет, поэтому источника нет ни у одного чтения. Здесь
      // стояло «остаётся на карточке» — верное до #471 и ложное после.
      // Показывать подпись без источника запрещено, поэтому карточка молчит;
      // читаемая принадлежность вернётся вместе с ресурсом членства (#1085).
    ...
    rowVerbs: [
      {
        ...
        resolve: (row, ctx): RowVerbState | null => {
          ...
          return {
            ...
            // Узкое действие НАЗЫВАЕТСЯ в обоих состояниях — и вместе с
            // условием, когда пункта в этом меню сейчас нет (#1208).
            //
            // Промолчать о нём нельзя: человек, не узнавший про узкий выход,
            // выполнит широкий — ровно тот исход, ради предотвращения которого
            // фраза и написана. Но и обещать безусловно нельзя: без выбранной
            ...
            confirmText: isSelf
              ...
              : `«${who}» больше не сможет входить НА ПЛАТФОРМУ — во всех своих аккаунтах, ` +
                ...
                (accountScopeChosen(ctx)
                  ? `Чтобы вывести человека ТОЛЬКО из этого аккаунта, есть «Исключить из аккаунта».`
                  : `Чтобы вывести человека ТОЛЬКО из одного аккаунта, есть «Исключить из аккаунта» — ` +

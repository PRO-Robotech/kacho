...
// несут 16 файлов владельцев, а применяют узел через `ToSQL` — 10. Шесть
// оставшихся (nlb, storage, registry, iam) забирают значение.
//
...
describe("сопоставитель различает — контроль в обе стороны", () => {
  ...
  it("владелец, теряющий ОПЕРАТОР, отвергается — и отказ называет предмет", () => {
    // Ровно тот случай, ради которого второе условие вообще есть: белый список
    // с `name` у владельца ЕСТЬ, объявление выглядело бы безупречно, а поиск
    // отвечал бы точным равенством.
    const f = findingFor("registry", "registries", "name");
    expect(f).not.toBeNull();
    expect(f).toContain("ToSQL");
    expect(f).toContain("ОПЕРАТОР");
    // Второй владелец того же класса, в другом сервисе и в другом слое (не
    // репозиторий, а use-case): иначе «различает» могло бы означать «узнаёт один
    // знакомый файл».
    expect(findingFor("storage", "volumes", "name")).toContain("ОПЕРАТОР");
  });

  it("владелец, не разбирающий выражение вовсе, отвергается отдельной причиной", () => {
    // Пользователи iam: выражение не разбирается, незнакомое молча игнорируется.
    // «Проигнорируют» и «отвергнут» — разные исходы, и причина обязана их
    // различать: первый тише и опаснее.
    const f = findingFor("iam", "users", "name");
    expect(f).not.toBeNull();

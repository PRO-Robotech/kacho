...
// ЗДЕСЬ СТОЯЛО «proto/ — дом ВСЕХ .proto; контракт края лежит только там», и с
// 2026-09-13 это ложь: решением владельца (kacho#2616, исход C) контракты службы
// доступа уехали в `PRO-Robotech/kaname` и приезжают опубликованным модулем.
// Домов ДВА, перечень — `contractProtoRoots()`; обход одного давал поверхность без
// домена `iam` и объявлял каждый его маршрут неслужимым, не падая ни на чём.

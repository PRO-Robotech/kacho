 ...
 * поверхности не утверждал ничего. Сегодня правило перевёрнуто:
 *
 *   файл вне `shared/`, ОБЪЯВЛЯЮЩИЙ символ, который объявляет `shared/`, —
 *   находка, пока он не назван в ведомости с причиной.
 *
 * Тонкая прослойка `export * from "@shared/…"` не объявляет ничего и остаётся
 * разрешённой формой без всякой записи — именно её и требуется писать вместо
 * копии.
 *
...
describe("единый источник: реализация в shared/, приложение берёт её оттуда", () => {
  it(`перепись: приложения ${SWEEP.apps.length}, файлов приложений ${SWEEP.filesRead}, файлов shared ${SWEEP.sharedFilesRead}, символов shared ${SWEEP.sharedSymbols}`, () => {
  ...
  it("новых форков нет: каждый объявленный символ shared вне shared/ назван в ведомости", () => {

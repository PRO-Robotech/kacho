...
describe("значок состояния знает каждое состояние контракта storage", () => {
  it(`объём осмотренного: контракт прочитан, носители значка найдены (subjects=${SUBJECTS.length})`, () => {
    ...
    // Охват пинится ПО РЕСУРСАМ, а не по именам приложений (#407).
    //
    // Прежняя редакция требовала носителями `shared` И `storage`. Это был пин
    // РАСКЛАДКИ ФАЙЛОВ, и он истёк ровно в тот час, когда реестр модуля свёлся к
    // прослойке `export * from "@shared/…"`: спеки блочного хранения переехали в
    // общий реестр целиком, модуль перестал описывать ресурсы сам и законно
    // выпал из обхода — а гейт прочитал это как сужение охвата. Ровно тот довод,
    // который абзац выше приводит про таблицу тонов: требовать от сведённого
    // модуля собственного объявления значит требовать вернуть форк.
    //
    // Пин по ресурсам этого не имеет: он утверждает то, ради чего гейт заведён —
    // что КАЖДЫЙ ресурс контракта показан кем-то, — и не зависит от того, в
    // скольких приложениях лежит объявление. Сузится обход — пропадёт ресурс, и
    // это покраснеет с его именем; переедет объявление между модулями — гейт
    // промолчит, потому что для оператора ничего не изменилось.
    const covered = new Set(SUBJECTS.flatMap((s) => s.resources));
    expect([...CONTRACT.keys()].filter((apiPath) => !covered.has(apiPath))).toEqual([]);
    // Общий модуль обязан оставаться носителем: он и есть дом сведённых спек.
    expect(SUBJECTS.map((s) => s.app)).toEqual(expect.arrayContaining(["shared"]));
  });

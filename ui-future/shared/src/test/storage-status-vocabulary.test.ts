...
const SUBJECTS = apps()
  .map((app) => {
    ...
    // оно случайно держит. Требовать от сведённого модуля собственной таблицы
    // тонов значило бы требовать вернуть форк: гейт судил бы раскладку файлов
    // вместо словаря, который видит оператор.
    //
    // Признак делегирования — ОТСУТСТВИЕ своего значка, а не наличие прослойки к
    // общему. Прежняя редакция спрашивала прослойку, и её предпосылка была верна
    // ровно для той популяции, на которой гейт писался: тогда сведение
    // выражалось прослойкой по тому же адресу. Сведение до конца (#407) прослойку
    // СНИМАЕТ — модуль импортирует общий напрямую, как это делает эталон vpc, — и
    // тогда прежний признак читал пустоту как «значка нет»: таблица тонов
    // получалась пустой, а все 14 состояний контракта — «мимо таблицы». Гейт
    // краснел на дереве, где ни одно состояние не потерялось.
    //
    // Отсутствие своего файла и есть доказательство делегирования: импорт
    // несуществующего модуля не собрался бы, а сборка модуля проверяется своим
    // шагом.
    ...
    const badge = readIfPresent(ownBadge) === null ? sharedBadge : ownBadge;

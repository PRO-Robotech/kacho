...
/**
 * ТИПОВОЕ объявление — `export interface|type|enum`.
 *
 * Отдельным распознавателем, а не расширением `DECLARATION`, потому что у этих
 * двух предикатов сегодня РАЗНЫЕ потребители, и смешать их значило бы менять
 * вердикт гейта дерева попутно с правкой одного модуля.
 *
 * ЧТО ЭТО ЗАКРЫВАЕТ. `DECLARATION` знает только формы ЗНАЧЕНИЯ, поэтому копия,
 * состоящая из одних типов, признаку по символу невидима by construction — это
 * не редкий случай, а целая законная форма записи предмета
 * (`testing.md` §«Гейт на класс», п.7). Замер на дереве консоли (2026-08-29,
 * предикат — обход тем же `sourceFiles`): символов `shared/`, видимых
 * `DECLARATION`, — 647; вместе с типовыми формами — 889, прибавка 242. Файлов с
 * находкой по символу — 16 против 25, и ДЕВЯТЬ видны только расширенному
 * распознавателю; среди них конверт операции (`Operation`/`OperationList`),
 * задвоенный в трёх модулях сразу (`compute`, `nlb`, `registry`).
 *
 * ГРАНИЦА НАЗВАНА ЧЕСТНО: главный гейт (`shared-organisms-single-source`) эту
 * функцию ещё НЕ читает. Перевод его на неё меняет вердикт по шести модулям
 * разом и требует адъюдикации чужих форков с написанием причин — отдельный
 * предмет со своей задачей, а не побочный эффект де-форка одного модуля.
 * Признак адресности при этом парные по пути файлы вида `api/types.ts` видит и
 * сегодня, поэтому дыры в наблюдении нет — есть недосказанность в перечне
 * символов у такой записи.
 */
const TYPE_DECLARATION = /^[ \t]*export[ \t]+(?:interface|type|enum)[ \t]+([A-Za-z_$][\w$]*)/gm;

/** Типовые символы, объявленные исходником (по одному вхождению на имя). */
export function declaredTypeSymbols(src: string): Set<string> {
  const out = new Set<string>();
  TYPE_DECLARATION.lastIndex = 0;
  let m: RegExpExecArray | null;
  while ((m = TYPE_DECLARATION.exec(src)) !== null) out.add(m[1]);
  return out;
}
 ...
 * вместо предмета.
 *
 * Ре-экспорт БЫВАЕТ МНОГОСТРОЧНЫМ, и это не другая форма, а та же: длинный список
 * имён prettier переносит сам. Прежняя редакция судила построчно и на закрывающей
 * строке `} from "@shared/…";` не узнавала ничего — то есть мерила ФОРМАТИРОВАНИЕ
 * вместо предмета, ровно тот класс, от которого оговорка выше защищает имя файла.
 * Наблюдалось: барель молекул iam (тридцать строк чистого ре-экспорта, ноль
 * объявлений) объявился форком в день, когда у `shared/` завёлся одноимённый
 * барель и путь впервые стал парным. Закрывающая строка блока разрешена отдельной
 * ветвью: `} from` в TS/JS встречается ТОЛЬКО в импорте и ре-экспорте, то есть
 * объявить ею нечего by construction.
 */
export function isReExportOnly(src: string): boolean {
  ...
  return statements.every(
    (l) =>
      /^(import\b|export\s+(\*|type\s*\{|\{))/.test(l) ||
      /^[}\w"',\s]*;?$/.test(l) ||
      // Хвост многострочного `export { … } from "…";` — см. оговорку в шапке.
      /^\}\s*from\s*['"][^'"]*['"]\s*;?$/.test(l),
  );
}

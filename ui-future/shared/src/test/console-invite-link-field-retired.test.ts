 ...
 * обходе: путь приглашения в дереве ЕСТЬ — тот же разбор находит вызов
 * `inviteUser` (объявление и употребления). Сняты читатели поля, а не путь.
 */

const RETIRED = ["magic_link_url", "magicLinkUrl", "MagicLinkUrl"];
// Положительный контроль: сам вызов приглашения в дереве ЕСТЬ — читатели поля
// сняты, а не путь приглашения целиком.
const REPLACEMENT = "inviteUser";
...
describe("поле ссылки приглашения снято с контракта — консоль его не читает", () => {
  ...
  test("положительный контроль: путь приглашения в дереве жив", () => {
    // Объявление плюс хотя бы одно употребление: одного объявления мало —
    // объявленный и никем не зовущийся вызов есть та же форма без содержания.

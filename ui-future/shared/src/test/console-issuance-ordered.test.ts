...
/**
 * F8-46 · перепись мест выпуска — КАЖДОЕ обращение консоли к краю идёт через
 * упорядочивающий транспорт (приёмка F8, Р10, §11.1, N18).
 *
 * Модульная проба упорядочения судит транспорт; эта перепись судит, что мимо
 * него не выпускается ничего: ЧТЕНИЕ транспорта браузера вне
 * `shared/src/api/carrier-order.ts` — вызов, ссылка, привязка, доступ по строке,
 * разбор, — и переход документа на путь края — находка. Перечень форм — у
 * распознавателя (`issuance-census.ts`).
 * До упорядочения таких мест было девять в семи файлах; после — ноль.
 *

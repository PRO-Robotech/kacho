...
//
// Обратной стороны — «консоль не называет владельца, которого профиль НЕ ОБЪЯВИЛ»
// — здесь тоже нет: её держит `gateway/deploy/console_subscription_owner_test.go`,
// и второе место об одном предмете разошлось бы с ним молча. Вместе с ним пара
// пинит РАВЕНСТВО двух множеств: объявленное ⊆ названное здесь, названное ⊆
// объявленного там.
//
// До #1633 тот гейт судил не объявленным перечнем, а множеством имён, которые
// край УМЕЕТ резолвить, — оно шире, и промежуток между ними не держал никто.

...
describe("NlbPage — маршруты раздела разрешаются в свои списки", () => {
  // Тексты призыва — ТЕ, ЧТО ВИДИТ ПОЛЬЗОВАТЕЛЬ. Здесь стояло «Создать целевая
  // группа» с оговоркой «проба закрепляет действительность; править надо продукт,
  // и тогда покраснеет здесь». Продукт поправлен: подпись берёт винительный падеж
  // из объявления ресурса (`lib/resource-label`), — покраснело здесь, как и было
  // обещано, и ожидание приведено к правильной русской форме.
  it.each([
    ["load-balancers", "Балансировщики нагрузки", "Создать балансировщик нагрузки"],
    ["listeners", "Обработчики", "Создать обработчик"],
    ["target-groups", "Целевые группы", "Создать целевую группу"],

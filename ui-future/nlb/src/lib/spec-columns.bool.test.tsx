...
describe("логическая колонка в списке регионов", () => {
  ...
  it("оба исхода названы СЛЕДСТВИЕМ, а не «Да»/«Нет» (правило 6 ui.md)", () => {
    ...
    // заголовком «Размещение» оно не говорит ни что площадка открыта, ни что она
    // закрыта.
    //
    // ПОДПИСИ ЗДЕСЬ СМЕНИЛИСЬ ВМЕСТЕ С ИХ ИСТОЧНИКОМ, а не ослабли. Модуль нёс
    // свою запись реестра, и в ней ячейка рисовалась собственным `BoolFact` со
    // своими словами («Размещение доступно» / «Размещение закрыто»). Реестр у
    // консоли теперь один (#408), и открытость площадки во ВСЕЙ консоли называет
    // единственный источник — `openForPlacementLabel` домена geo, — поэтому
    // список регионов раздела `/nlb/*` и список регионов `/system/*` перестали
    // быть двумя видами одного предмета. Слова другие, правило то же: ни «Да», ни
    // «Нет», оба исхода названы состоянием площадки.
    const yes = cellText(/размещ/i, { open_for_placement: true });
    const no = cellText(/размещ/i, { open_for_placement: false });
    expect(yes).toMatch(/^\s*Открыт\s*$/);
    expect(no).toMatch(/^\s*Закрыт\s*$/);

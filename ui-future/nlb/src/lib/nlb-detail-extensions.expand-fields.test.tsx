...
function renderValue(value: ReactNode): HTMLElement {
...
function textOf(value: ReactNode): string {
  return renderValue(value).textContent ?? "";
}

/** Адреса ссылок значения — по ним ресурсы различаются ТОЧНО, тогда как
 *  показанный текст до резолва имени усечён и у похожих идентификаторов
 *  совпадает. */
function hrefsOf(value: ReactNode): string[] {
  return [...renderValue(value).querySelectorAll("a")].map((a) => a.getAttribute("href") ?? "");
}
...
describe("обзор балансировщика показывает объявленные поля", () => {
  ...
  it("группы безопасности VIP перечислены — и КАЖДАЯ ведёт на свою карточку", () => {
    // Утверждается адрес, а не показанный текст: группа безопасности — чужой
    // ресурс со своей карточкой, и строка обязана вести туда (канон §9). До
    // резолва имени показан усечённый идентификатор, поэтому по тексту две
    // соседние группы неразличимы, а по адресу — различимы точно.
    const hrefs = hrefsOf(find(itemsFor("load-balancers", LB), /групп/i).value);
    expect(hrefs).toEqual([
      "/projects/prj-1/vpc/security-groups/sg-000000000000001",
      "/projects/prj-1/vpc/security-groups/sg-000000000000002",
    ]);
  });

...
export interface Listener {
  ...
  protocol?: "TCP" | "UDP";
  port?: number;
  // `target_port` здесь не объявляется: у сообщения `Listener` его номер и имя
  // зарезервированы, значит край его не отдаёт ни в одном ответе (#512). Поле,
  // объявленное в типе, читается следующим как «край иногда его присылает» — и
  // рождает строку карточки, которую нечем наполнить.

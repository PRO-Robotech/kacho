...
// Единственный импорт файла: ссылка на чужой ресурс нужна ЗДЕСЬ как тип полей
// `used_by` у тома, снимка и образа. Стоит наверху, а не рядом со своим
// ре-экспортом ниже: импорт посреди тела модуля линт справедливо помечает —
// порядок объявлений перестаёт читаться сверху вниз.
...
// контракта (#1467), поэтому остаётся ссылка. Сам тип импортирован в шапке —
// он нужен полям `used_by` ниже, а не только ре-экспорту.

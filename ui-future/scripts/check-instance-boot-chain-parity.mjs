 ...
 *
 * Почему гейт, а не проба. Проба закрепляет утверждение внутри той копии, к
 * которой написана, и не знает, сколько копий вообще есть: вторая, заведённая
 * завтра, не покраснит ни одну — и появится расхождение, которое эта задача и
 * запрещает оставлять. Перепись копий обязана выводиться ИЗ ДЕРЕВА.
 *
 * Копий сегодня ОДНА: реестр сведён к единственному на всю консоль (#406). Это
 * и есть цель, а не поломка, — гейт остаётся зелёным и печатает перепись, по
 * которой видно, что он смотрел. Ноль копий по-прежнему падение (см. ниже).
 *

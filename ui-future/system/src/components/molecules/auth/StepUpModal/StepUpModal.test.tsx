...
describe("StepUpModal — что видит пользователь", () => {
  it("до запроса называет уровень по умолчанию", () => {
    // Положительный близнец утверждения ниже: без него «показал запрошенный
    // уровень» было бы верно и на компоненте, который печатает что угодно.
    render(<StepUpModal />);
  ...
  it("объясняет, чем подтверждать, а не только что нужно подтвердить", () => {
    render(<StepUpModal />);

...
## Сборочная цепочка консоли закреплена на шестом мажоре `vite`

**Status:** accepted / пин с названной причиной; предмет держат два гейта, а не текст.

**Что закреплено.** Девять корней из одиннадцати объявляют `vite ^6.1.0` и
`@originjs/vite-plugin-federation ^1.4.1`; `shared` и `e2e` не объявляют ни того, ни
другого. Перепись (ревизия `4bc2f9d9`, единица счёта — отслеживаемый манифест):
...
**Почему не поднимаем.** Восьмой мажор `vite` заменяет сборщик: у `6` и `7` в
...
- подъём — не «шесть на восемь»: `@vitejs/plugin-react` из четвёртой линии принимает
  сборщик до седьмого мажора включительно, под восьмым нужна шестая линия плюс два
  новых пира. Это вторая поверхность риска в том же изменении;
- **найденное место — первое, а не единственное.** Замер выше сделан на пробе, которая
...
Поэтому пин остаётся, а движение вперёд — не форк, а замена плагина: у
`@module-federation/vite` объявлена совместимость со сборщиком с пятого по восьмой
мажор. Это отдельная работа со своим предметом.
...
- `scripts/check-federation-pin-still-needed.mjs` — держит **причину**: цепочка одна
  ...
  чтобы дать причине пережить свой предмет.

**Revisit trigger:** гейт причины краснеет сам — при появлении у плагина совместимого
поведения либо при его замене. Отдельно от этого седьмой мажор сборщика на пробе
**измерен рабочим**, то есть пин сегодня шире своей причины; это заведено предметом —
задача **#337**, — а не оставлено обещанием здесь. Вердикт по девяти корням в обоих
случаях дают `check-federation-executes.mjs` и сквозные пробы консоли, а не эта таблица.

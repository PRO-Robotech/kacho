...
export function IamOperationsPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    // поллинг остаётся: это ЛЕНТА ОПЕРАЦИЙ, а подписки на операции нет и не
    // будет. Событие ресурса пишется в транзакции ресурсной строки, поэтому у
    // ПРОВАЛИВШЕЙСЯ мутации события нет вовсе — наблюдатель потока не отличил
    // бы «ещё делается» от «упало».
    //
    // Здесь стояло ещё «плюс журнала у iam нет вовсе» — довесок истёк вместе со
    // своим предметом: журнал у службы есть и семь её видов консоль называет.
    // Причина выше от этого НЕ меняется: она про операции, а не про домен, и
    // подписки на операции нет ни у одного владельца.

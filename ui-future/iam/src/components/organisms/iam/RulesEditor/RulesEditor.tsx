  ...
  if (moduleOptions.length === 0) {
    return (
      <Alert
        ...
        description="Сервер не вернул ни одного модуля, на который можно выдать доступ, — редактор правил недоступен."
      />
    );
  }
    ...
    <div data-testid="role-rules-editor">
      <Space direction="vertical" size={12} style={{ width: "100%" }}>
        ...
        <Typography.Text type="secondary" style={{ fontSize: 12 }}>
          ...
          отфильтрованы по нему). Чтобы охватить несколько модулей — добавьте <b>отдельное правило</b> на каждый.
          Модуль, тип ресурса и глагол выбираются из выпадающих списков (опции — из платформенного каталога ресурсов).
          Сужение — по именам (resourceNames) <b>или</b> по меткам (matchLabels): по меткам выбираются только типы с
          поддержкой меток. Без сужения правило покрывает все инстансы в scope. Wildcard <code>*</code> в глаголах
          допустим; в модуле/типах — только системным ролям.
        </Typography.Text>
      </Space>
    </div>
  );

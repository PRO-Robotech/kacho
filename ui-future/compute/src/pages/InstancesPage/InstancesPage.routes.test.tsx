 ...
 * `.kc-surface`, так что без ответа проба не добралась бы до предмета проверки
 * вовсе. Оболочку карточки этот модуль берёт из `shared/` (своя копия сведена к
 * ре-экспорту), поэтому координата строки здесь не пишется: она указывала бы в
 * прослойку и устаревала бы от любой правки общей реализации.
 */
function stubApi() {
...
describe("InstancesPage — раздел компьюта не двоится (issue #416)", () => {
  ...
  it("сохраняет ключи доступа при повторном заходе (регрессия «мёртвого» вынесенного узла)", async () => {
    ...
    // Раздел опознаётся по СВОЕМУ действию, а не по названию списка в шапке:
    // названия там больше нет — оно повторяло раздел, уже названный навигацией
    // и хлебными крошками. Утверждение то же (открыт раздел ключей, а не
    // редирект на машины), признак — существующий.
    await waitFor(() => expect(screen.getAllByText(/ключ доступа/i).length).toBeGreaterThan(0));
    // Отрицание в паре с положительным: редирект увёл бы на список машин.
    expect(screen.queryByText(/Создать виртуальную машину/i)).not.toBeInTheDocument();
  });
});

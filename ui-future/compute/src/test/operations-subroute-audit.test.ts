...
describe("объём осмотренного — «ноль находок» отличимо от «ноль прочитанного»", () => {
  ...
  it("реестры найдены обходом и названы поимённо — объявляющие отдельно от проекций", () => {
    ...
    //
    // `registry` переехал из объявляющих в проекции (#1508): его реестр сведён к
    // прослойке той же линией, что свела `compute`, `nlb` и `storage`. Перечни
    // здесь не подогнаны под наблюдение, а перемерены — признак тот же, каким
    // судит сам гейт (`isReExportOnly`): у всех четырёх модулей файл реестра
    // состоит из одной строки реэкспорта, у `shared` реэкспорта нет вовсе.
    // Объявляющий остался ОДИН, и это состояние дерева, а не послабление:
    // появление второго обязано быть замечено здесь.
    expect(projections).toEqual(["compute", "nlb", "registry", "storage"]);
    expect(declaring).toEqual(["shared"]);
...
describe("согласие каждого реестра со стволом", () => {
  ...
  it("спеки без вкладки названы поимённо по приложениям — это утверждение, а не умолчание", () => {
    ...
    expect(without).toEqual({
      // Записи `repositories` и `tags` пришли сюда из раздела `registry` (#1508):
      // спеки переехали в общий реестр вместе со сведением его форка, и вместе с
      // ними переехала запись о них. Довод не изменился — вложенные ресурсы
      // реестра адресуются внутри `registries/{registryId}/…`, и подмаршрута
      // операций ствол им не объявляет, как и каталогу geo; изменился ЛИШЬ
      // владелец спеки. Отдельного раздела `registry` здесь больше нет, потому
      // что его реестр стал проекцией и своих спек не несёт by construction.

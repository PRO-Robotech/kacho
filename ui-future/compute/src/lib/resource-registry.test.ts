...
describe("compute resource-registry (COMP-1 redesign)", () => {
  ...
  it("VM sanitize: оставляет vm_spec, режет container_spec, boot_source до {type,id}", () => {
    ...
    // sshPublicKeys снят с формы: сервер его не принимает (ключи никуда не
    // доставлялись), поэтому форма не вправе его предлагать и уж тем более слать.
  ...
  it("CONTAINER sanitize: оставляет container_spec, режет vm_spec, пустой working_dir не шлёт", () => {
    const out = REGISTRY["compute-instances"].sanitize!({

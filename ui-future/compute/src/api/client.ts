...
//
// Группа размещения появилась в перечне тем же порядком и по той же причине, что
// справочник прав, — от РАСШИРЕНИЯ обхода, а не от новой адресации. Её раздел
// приложение монтировало и раньше, но спека бралась псевдонимом из общего
// реестра, поэтому пути в исходнике не было ни одной строкой, и предикат её не
// видел. После сведения форка (#406) обход читает пути СПЕК, которые приложение
// монтирует, — и первая же находка была не в коде, а здесь: шапка не называла
// эндпоинт, который приложение зовёт.
//

...
describe("шапка client.ts перечисляет ровно те эндпоинты, которые адресует исходник", () => {
  ...
  it("прочитан непустой объём: файлы, объявленные и адресуемые токены", () => {
    ...
    //
    // Порог заведомо НИЖЕ настоящего числа файлов и не подтягивается к нему: он
    // ловит сломавшийся обход (тот дал бы около нуля), а не колебания корпуса.
    // Прежний порог 100 был подтянут вплотную и покраснел на снятии мёртвого
    // дерева — 93 файла вместо 116 (#591), то есть на достижении цели, а не на
    // дефекте. Проверка, падающая от того, что дерево стало чище, толкает держать
    // мусор ради зелёного.
